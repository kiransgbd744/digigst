sap.ui.define([
	...
	return BaseController.extend("com.ey.digigst.controller.GSTR1A", {
		formatter: Formatter,
		...
		onPressAdaptFilter: function (oEvent) {
			if (!this._oAdpatFilter) {
				this._oAdpatFilter = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.AdaptFilter", this);
		...
		onPressSaveStatus: function (oEvent, view) {
			...
			if (!this._oDialogSaveStats) {
				this._oDialogSaveStats = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.SaveStatus", this);
		...
		onPressDifference: function (oEvent) {
			if (!this._oDialogDifference) {
				this._oDialogDifference = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.Difference", this);
		...
		_getDifferenceData: function () {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1ADifferenceSummary.do",
					data: JSON.stringify(oPayload),
					contentType: "application/json"
				})
				.done(function (data, status, jqXHR) {
		...
		_getProcessSummary: function (searchInfo, flag) {
			...
			searchInfo.req.returnType = "GSTR1A";
			...
			$.when(
					$.ajax({
						method: "POST",
						url: "/aspsapapi/gstr1ASummaryScreen.do",
						data: JSON.stringify(searchInfo),
						contentType: "application/json"
					}),
					$.ajax({
		...
		_getSubmitStatus: function (payload) {
			...
			oPayload.req.returnType = "GSTR1A";
			$.ajax({
					method: "POST",
					url: "/aspsapapi/getGstr1AGstnSubmitStatus.do",
					data: JSON.stringify(oPayload),
					contentType: "application/json"
				})
				.done(function (data, status, jqXHR) {
		...
		_getSignFileStatus: function (payload) {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/getGstr1AGstnSaveFileStatus.do",
					data: JSON.stringify(oPayload),
					contentType: "application/json"
				})
				.done(function (data, status, jqXHR) {
		...
		onPressGstr1GetDetails: function (view) {
			...
			if (view === "P") {
				if (!this._getGstr1Detail) {
					this._getGstr1Detail = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.GetGstr1Details", this);
		...
		onPressClear: function (flag, clearFlag) {
			...
			if (clearFlag == "C") { //eslint-disable-line
				if (flag == "P") { //eslint-disable-line
					var vPage = "dpProcessRecord",
						vGstin = "slGstin",
				...
				} else if (flag == "S") { //eslint-disable-line
					vPage = "dpGstr1Summary";
		...
		onPressStatusForNavToInv: function (refId) {
			...
			var oPayload = this.byId("dtSaveStats").getValue(); //eslint-disable-line
			var oFilterData = {
				"req": {
					"dataType": "outward",
					"criteria": "taxPeriod",
		...
		onDownloadOutward: function (section) {
			...
			if (aOutward.includes(section)) {
				this.reportDownload(oPayload, "/aspsapapi/downloadGstr1ASectionWiseReports.do");
			} else {
				this.excelDownload(oPayload, "/aspsapapi/downloadGstr1ASecReports.do");
			}
		},
		...
		_getB2csSummary: function (payload) {
			...
			if (!this._oB2csSummary) {
				this._oB2csSummary = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.B2csSummary", this);
		...
		_getB2csaSummary: function (payload) {
			...
			if (!this._oB2csaSummary) {
				this._oB2csaSummary = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.B2csaSummary", this);
		 ...
		 */
		_getAspVerticalSaveObj: function (view, data) { //eslint-disable-line
			return {
				"sNo": data.sNo,
				"id": data.id || null,
				"returnType": "GSTR1A",
		...
		onPressSaveAsp: function (view) {
			...
			for (var i = 0; i < aIndex.length; i++) {
				...
				var flag = this.validateOutwardField(view, oData[aIndex[i]]);
				if (!flag && oData[aIndex[i]].visi) {
					oPayload.req.push(this._getAspVerticalSaveObj(view, oData[aIndex[i]]));
				}
			}
			if (flag) {
				...
				$.ajax({
					...
					that._bindAspResponse(oDialog, data);
					that._getAspVertical(oDialog, false);
				...
				onClose: function (action) {
					if (action === sap.m.MessageBox.Action.OK) {
						...
						if (flag) {
							...
							$.ajax({
								method: "POST",
								url: "/aspsapapi/gstr1aRecordDelete.do",
								data: JSON.stringify(oPayload),
								contentType: "application/json"
							}).done(function (data, status, jqXHR) {
		...
		_advancesSummary: function (payload) {
			...
			if (!this._oAdvances) {
				this._oAdvances = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.Advances", this);
		...
		_advancesAmendSummary: function (payload) {
			...
			if (!this._oAdvAmendment) {
				this._oAdvAmendment = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.AdvancesAmend", this);
		 ...
		 */
		_getAdvanceObj: function (type, oDialog, data) { //eslint-disable-line
				...
				for (var i = 0; i < oData.length; i++) {
					...
					for (var j = 0; j < data.resp.length; j++) {
						if (oData[i].sNo === data.resp[j].sNo && !data.resp[j].errorList) {
							...
							for (var f in data.resp[j]) { //eslint-disable-line
								if (aFields.includes(f)) { //eslint-disable-line
									continue;
								}
								if (f === "newAdvance") { //eslint-disable-line
		...
		_bindValueState: function (oData, errors, oFields) {
			...
			for (var i = 0; i < errors.length; i++) {
				...
				for (var j = 0; j < aErrField.length; j++) {
					var field = oFields[aErrField[j]];
					if (field && oData[field + "State"] !== "Error") {
						oData[field + "State"] = (errors[i].errorType === "ERROR" ? "Error" : "Information");
		...
		onPressNilExemptLink: function (oEvent) {
			var oPayload = this.byId("dpGstr1Summary").getModel("SummaryPayload").getData(),
				oProperty = this._popupProperty();
			if (!this._oNilExempt) {
				this._oNilExempt = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.NilExempt", this);
		...
		_getNilExemptData: function () {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1ANilExmpNonGstStauts.do",
					data: JSON.stringify(payload),
					contentType: "application/json"
				})
				.done(function (data, status, jqXHR) {
					...
					if (data.hdr.status === "S") {
						...
						for (i = 0; i < data.resp.verticalData.length; i++) {
							data.resp.verticalData[i].sNo = i + 1;
							data.resp.verticalData[i].visi = false;
							data.resp.verticalData[i].edit = false;
						}
						var oData = $.extend(true, [], data.resp);
						that._oNilExempt.setModel(new JSONModel(data.resp), "NilExempt");
						that._oNilExempt.setModel(new JSONModel(oData), "ExemptData");
					}
				})
				.fail(function (jqXHR, status, err) {
					that._oNilExempt.setBusy(false);
			...
			sap.m.MessageBox.confirm(oBundle.getText("msgHsnMoveConfirm"), {
				...
				onClose: function (oAction) {
					if (oAction === sap.m.MessageBox.Action.OK) {
						...
						$.ajax({
								method: "POST",
								url: "/aspsapapi/gstr1ANilExmpNonGstSave.do",
								data: JSON.stringify(oPayload),
								contentType: "application/json"
							})
							.done(function (data, status, jqXHR) {
		 ...
		 */
		_getExemptSavePayload: function (data, payload) { //eslint-disable-line
			return {
				"id": data.id || null,
				"sNo": data.sNo,
		...
		onPressSaveNilExempt: function () {
			var oBundle = this.getResourceBundle(),
				...
				oNilModel = this._oNilExempt.getModel("NilExempt"),
				oNilData = oNilModel.getData().verticalData,
				flag = false,
				oPayload = {
					"req": []
				};
			...
			if (flag) {
				...
				$.ajax({
						method: "POST",
						url: "/aspsapapi/gstr1ANilExmpNonVerticalSave.do",
						data: JSON.stringify(oPayload),
						contentType: "application/json"
					})
					.done(function (data, status, jqXHR) {
		...
		_bindNil: function (oDialog, data) {
			...
			if (data.hdr.status === "S") {
				...
				// 	this._setValueStateNone(oAspData[i]);
				// 	for (var j = 0; j < data.resp.length; j++) {
				// 		if (oAspData[i].sNo === Number(data.resp[j].sNo) && !data.resp[j].errorList) {
				// 			for (var f in data.resp[j]) {
				// 				oAspData[i][f] = data.resp[j][f];
				// 			}
				// 			oAspData[i].visi = false;
				// 			break;
				// 		} else if (oAspData[i].sNo === Number(data.resp[j].sNo)) {
				// 			oAspData[i].id = data.resp[j].id;
				// 			count = this._bindValueStateNil(oAspData[i], data.resp[j].errorList, "hsn");
				// 			flag1 = true;
				// 			if (count === 0) {
				// 				for (f in data.resp[j]) {
				// 					oAspData[i][f] = data.resp[j][f];
				...
				onClose: function (action) {
					if (action === sap.m.MessageBox.Action.OK) {
						...
						if (flag) {
							...
							$.ajax({
									method: "POST",
									url: "/aspsapapi/nilGstr1AExmpNonVerticalDelete.do",
									data: JSON.stringify(oPayload),
									contentType: "application/json"
								})
								.done(function (data, status, jqXHR) {
		...
		onPressHsnSummaryLink: function () {
			...
			if (!this._oHsnSummary) {
				this._oHsnSummary = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.HsnSummary", this);
		...
		_getHsnSummaryData: function () {
			var oHsnPayload = this._oHsnSummary.getModel("Payload").getData(),
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1aHsnAspVertical.do",
					data: JSON.stringify(payload),
					contentType: "application/json"
				})
				.done(function (data, status, jqXHR) {
			...
			sap.m.MessageBox.confirm(oBundle.getText("msgHsnMoveConfirm"), {
				...
				onClose: function (oAction) {
					if (oAction === sap.m.MessageBox.Action.OK) {
						...
						$.ajax({
								method: "POST",
								url: "/aspsapapi/moveGstr1aDigiGstToUserEdit.do",
								data: JSON.stringify(oPayload),
								contentType: "application/json"
							})
							.done(function (data, status, jqXHR) {
		 ...
		 */
		_getHsnSavePayload: function (data) { //eslint-disable-line
			return {
				"sNo": data.sNo,
				"docKey": data.docKey || null,
				"sgstn": data.sgstn,
		...
		onSaveHsnSummary: function () {
			...
			if (flag && oPayload.req.length > 0) {
				...
				$.ajax({
						method: "POST",
						url: "/aspsapapi/gstr1aHsnUserUpdateData.do",
						data: JSON.stringify(oPayload),
						contentType: "application/json"
					})
					.done(function (data, status, jqXHR) {
		...
		onPressDocIssedLink: function () {
			...
			var oPayload = this.byId("dpGstr1Summary").getModel("SummaryPayload").getData(),
				oProperty = this._popupProperty();
			if (!this._oDocIssued) {
				this._oDocIssued = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.DocIssued", this);
		...
		lastUpdated: function (identifier) {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1aDocSeriesAutoCompStatus.do",
					data: JSON.stringify(req),
					contentType: "application/json"
				})
				.done(function (data, status, jqXHR) {
		...
		onSaveDocIssued: function () {
			...
			if (flag) {
				...
				$.ajax({
						method: "POST",
						url: "/aspsapapi/gstr1adocseriesSaveAndEdit.do",
						data: JSON.stringify(oPayload),
						contentType: "application/json"
					})
					.done(function (data, status, jqXHR) {
				...
				onClose: function (action) {
					if (action === sap.m.MessageBox.Action.OK) {
						...
						if (flag) {
							...
							$.ajax({
									method: "POST",
									url: "/aspsapapi/gstr1ADocseriesDelete.do",
		...
		onCloseDialog: function (view) {
			var oBundle = this.getResourceBundle();
			if (view === "b2cs") {
				var obj1 = this._oB2csSummary.getModel("AspVertical").getProperty("/verticalData"),
					obj2 = this._oB2csSummary.getModel("AspData").getProperty("/verticalData"),
					aFields = [
		 ...
		 */
		_checkMandatory: function (obj, view) { //eslint-disable-line
		...
		_compareAspAndAdvanceObj: function (oDialog, view) {
			...
			var flag = true;
			if (view === "b2cs") {
				var obj1 = oDialog.getModel("AspVertical").getData().verticalData,
					obj2 = oDialog.getModel("AspData").getData().verticalData;
				for (var i = 0; i < obj1.length; i++) {
					if (obj1[i].visi) {
						flag = false;
					}
				...
				obj1 = oDialog.getModel("Advances").getData().verticalData;
				obj2 = oDialog.getModel("AdvData").getData().verticalData;
		...
		_confirmPopupClose: function (oDialog, flag, view) {
			...
			if (flag) {
				oDialog.close();
			} else if (this.getView().getModel("buttonVis").getData().dataEditable === false) {
				oDialog.close();
			} else {
				var oBundle = this.getResourceBundle(),
					that = this;
				...
				sap.m.MessageBox.confirm(oBundle.getText("msgPopupPendingChange"), {
					...
					onClose: function (action) {
						if (action === sap.m.MessageBox.Action.YES) {
							switch (view) {
							case "b2cs":
								that.onPressSaveAsp(view);
							case "b2csa":
								that._saveB2csSummary(oDialog, view);
								break;
							case "advances":
							case "amendment":
								that._saveAdvance(oDialog, view);
								break;
							case "nilExempt":
								that.onPressSaveNilExempt();
								break;
							case "hsn":
								that._saveHsnSummary();
								break;
							case "docIssued":
								that._saveDocIssued();
								break;
		...
		_saveNilData: function () {
			...
			if (oPayload.req.length > 0) {
				var vUrl = "/aspsapapi/gstr1ANilExmpNonGstSave.do";
			} else {
				vUrl = "/aspsapapi/gstr1ANilExmpNonVerticalSave.do";
				oPayload.req = [];
		...
		_msgSaveToGstn: function (data) {
			...
			if (!this._saveToGstnStats) {
				this._saveToGstnStats = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.SaveToGstnStats", this);
		...
		onPressDownloadReport: function (oEvent, parameter, view) {
			...
			var oBundle = this.getResourceBundle(),
				aReportType = ["gstr1aAsUploaded", "gstr1aAspError", "gstr1aGstnError", "shipBillDetail"],
				aReportTypeFilter = ["nilRateSavable", "processedSaveable", "b2csSavable", "hsnSummary", "gstr1aAsUploaded", "gstr1aAspError",
					"gstr1aGstnError",
			...
			if (view === "P") {
				...
				if (aReportTypeFilter.includes(parameter.getKey())) {
				...
				if (aReportType.includes(parameter.getKey())) {
					oPayload.req.tableType = [];
					oPayload.req.docType = [];
				}
			...
			if (aReportType.includes(parameter.getKey())) {
				var vUrl = "/aspsapapi/downloadGstr1aProcessedCsvReports.do";
				oPayload.req.reportCateg = (view === "P" ? "ProcessedSummary" : "ReviewSummary");
				if (parameter.getKey() === "shipBillDetail") {
					oPayload.req.type = "Gstr1A Shipping Bill";
					//	oPayload.req.reportCateg = "InvoiceManagement";
				}
				this.reportDownload(oPayload, vUrl);

			} else if (parameter.getKey() === "entityLevel") {
				if (!this._dateFilter) {
					this._dateFilter = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.dateFilter", this);
			...
			} else if (parameter.getKey() === "stockTransferRepo") {
				oPayload.req.type = "Stock Transfer";
				oPayload.req.reportCateg = "Stock Transfer";
				oPayload.req.tableType = [];
				oPayload.req.docType = [];
				oPayload.req.taxPeriod = this.byId("dtProcessed").getValue();
				vUrl = "/aspsapapi/downloadGstr1aProcessedCsvReports.do";
				this.reportDownload(oPayload, vUrl);

			} else if (SelectedKey === "taxLiabilitysumrepo") {
				if (!this.TaxLiability) {
					this.TaxLiability = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.TaxLiabilitySummaryreportF4",
						this);
					this.getView().addDependent(this.TaxLiability);
				}
				this.TaxLiability.open();
				var that = this;
				this.byId("DP1").addEventDelegate({
					onAfterRendering: function () {
						that.byId("DP1").$().find("input").attr("readonly", true);
					}
				});

				this.byId("DP2").addEventDelegate({
					onAfterRendering: function () {
						that.byId("DP2").$().find("input").attr("readonly", true);
					}
				});

				var vDate = new Date();
				this.byId("DP1").setMaxDate(vDate);
				this.byId("DP1").setDateValue(vDate);

				this.byId("DP2").setMaxDate(vDate);
				this.byId("DP2").setDateValue(vDate);

			} else if (parameter.getKey() === "shipBillDetail") {
				var vUrlIn = "/aspsapapi/downloadGstr1aProcessedCsvReports.do";
				oPayload.req.type = "Shipping Bill";
				oPayload.req.reportCateg = "ProcessedSummary";
				this.reportDownload(oPayload, vUrlIn);
			} else if (parameter.getKey() === "RateLevelData") {
				var vUrlIn1 = "/aspsapapi/downloadGstr1aProcessedCsvReports.do";
				oPayload.req.type = "RateLevelReport";
				oPayload.req.reportCateg = view === "P" ? "ProcessedSummary" : "ReviewSummary";
				this.reportDownload(oPayload, vUrlIn1);
			} else if (parameter.getKey() === "ConsolidatedHSNSummary") {
				var vUrlIn1 = "/aspsapapi/hsnSummaryReport.do";
				oPayload.req.type = "VerticleUploadHSN";
				oPayload.req.reportCateg = "GSTR1";
				this.reportDownload(oPayload, vUrlIn1);
			} else {
				vUrl = "/aspsapapi/downloadGstr1aRSReports.do";
				oPayload.req.returnType = "GSTR1A";
				oPayload.req.dataType = "outward";
				this.excelDownload(oPayload, vUrl);
			}
		},
		...
		onPressDateFilterClose: function (flag) {
			...
			if (flag === "Apply") {
				var vUrl = "/aspsapapi/downloadGstr1aProcessedCsvReports.do";
				this.oPayload.req.taxPeriodFrom = this.byId("idFromPeriodELS").getValue();
				this.oPayload.req.taxPeriodTo = this.byId("idToPeriodELS").getValue();
		...
		onExportExcel: function (button) {
			...
			if (button === "P") {
				...
				var vUrl = "/aspsapapi/downloadGstr1aRSReports.do",
					oPayload = {
						"req": {
							"dataType": "outward",
							"type": vType,
		...
		onFetchGstnData: function () {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/fetchGstr1AGetSummary.do",
					contentType: "application/json",
					data: JSON.stringify(payload)
				})
				.done(function (data, status, jqXhr) {
			...
			sap.m.MessageBox.confirm(oBundle.getText("confirmUpdate"), {
				...
				onClose: function (oAction) {
					if (oAction === sap.m.MessageBox.Action.OK) {
						...
						$.ajax({
								method: "POST",
								url: "/aspsapapi/gstr1AGetSummary.do",
								contentType: "application/json",
								data: JSON.stringify(oPayload)
							})
							.done(function (data, status, jqXHR) {
		...
		onPressSignNFile: function (view) {
			...
			this.excelDownload(oPayload, "/aspsapapi/execSignAndFileGstr1AStage1.do");
		},
		...
		onPressGetGstr1Detail: function (oEvent, flag) {
			if (!this._oDialog) {
				this._oDialog = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.ProcessStatus", this);
		...
		getGstr1ASucessStatusDataFinal: function (postData) {
			...
			var oView = this.getView(); //eslint-disable-line
			var that = this;
			sap.ui.core.BusyIndicator.show(0);
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: "/aspsapapi/getGstr1ADetailStatus.do",
					contentType: "application/json",
		...
		bindGstr1DetailStatus: function (data) {
			...
			for (var i = 0; i < data.resp.lastSuccess.length; i++) { //eslint-disable-line
		 ...
		 * @memberOf com.ey.digigst.view.GSTR1
		 */
		onPressGetBtn: function (oEvent) { //eslint-disable-line
			...
			if (vSelectAll) {
				...
				if (oBtProcess === "LCS") {
					if (data.resp.lastCall[vRowIndex].lsFlag) {
						...
						var oSelectedFlag = true; //eslint-disable-line
					}
					...
					if (data.resp.lastSuccess[vRowIndex].lsFlag) {
						var oSelectedFlag = false; //eslint-disable-line
					} else {
						var oSelectedFlag = true; //eslint-disable-line
					}
		 ...
		 * @memberOf com.ey.digigst.view.GSTR1
		 */
		onSelectAllCheckHeader: function (oEvent, field) { //eslint-disable-line
		...
		onPressSaveGstn1: function (oEvent, flag) {
			if (!this._oDialogSaveStatusBtn) {
				this._oDialogSaveStatusBtn = sap.ui.xmlfragment("com.ey.digigst.fragments.gstr1A.SaveToGstin", this);
			...
			if (flag === "P") {
				...
				var oModel = this.getView().setModel(new JSONModel({
					"P": true
				}), "visiSummPopup"); //eslint-disable-line
				sap.ui.getCore().byId("idA_TaxProcess12").setValue(this.oView.byId("dtProcessed").getValue());
				sap.ui.getCore().byId("id_select1ANd").setSelected(false);
				sap.ui.getCore().byId("id_select2ANd").setSelected(false);
				...
				sap.ui.getCore().byId("idA_TaxProcess12").setValue(this.oView.byId("dtSummary").getValue());
				sap.ui.getCore().byId("slASummaryGstin11").setSelectedKey(this.oView.byId("slSummaryGstin").getSelectedKey());
				sap.ui.getCore().byId("id_select1ANd").setSelected(false);
				sap.ui.getCore().byId("id_select2ANd").setSelected(false);
				this.onRadioBtnselect();
		 ...
		 * @memberOf com.ey.digigst.view.GSTR1
		 */
		onSelectSaveStatausPopup: function (oEvent, flag) { //eslint-disable-line
			//eslint-disable-line
		...
		getSavetoGstinIntrigration: function (flag) {
			//eslint-disable-line
			var oTaxPeriod = sap.ui.getCore().byId("idA_TaxProcess12").getValue();
			if (oTaxPeriod === "") {
				oTaxPeriod = null;
			}
			var vGstins = sap.ui.getCore().byId("slASummaryGstin11").getSelectedKey(),
				postData = {
					"req": [{
						"gstin": vGstins,
						"ret_period": oTaxPeriod
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1ASaveToGstnSectionSelection.do",
					contentType: "application/json",
					data: JSON.stringify(postData)
				})
				.done(function (data, status, jqXHR) {
		 ...
		 */
		bindGetSavetoGstinIntrigration: function (data, flag) { //eslint-disable-line
			var aField = [
		 ...
		 * @param {Object} oEvent Eventing object
		 */
		onPressProcessSumBtngstr1: function (oEvent) { //eslint-disable-line
			var oBtProcess = this.byId("idProcessStatusBtn1").getSelectedKey();
			if (oBtProcess === "LCS") {
				var oModelForTab1 = this.byId("idgetVtablegstr1pro").getModel("Gstr1ASucess").getProperty("/resp"),
					oTaxPeriod = this.byId("id_TaxProcess1").getValue();
		...
		onPressdeleteAuotodreafted: function (oEvent, flag) {
			...
			if (flag === "P") {
				...
				if (flag1 == true && flag2 == true) {
					MessageBox.show(
						"Auth token for few GSTINs is inactive,hence GSTR-1A data available on GST common portal will be deleted for GSTINs for which auth token is active. Do you want to proceed?", {
		...
		gstr1SaveToGstnReset: function () { //eslint-disable-line
			//eslint-disable-line
			var that = this;
			if (this.ResetFlag === "P") {
				var oModelForTab1 = this.byId("tabGstr1Process").getModel("ProcessedRecord").getProperty("/"),
				...
				var payload = {
					"req": {
						"gstins": aGstin,
						"retPeriod": sap.ui.getCore().byId("idA_TaxProcess12").getValue(),
						"tableSections": [],
					}
				};
				...
				payload = {
					"req": {
						"gstins": [sap.ui.getCore().byId("slASummaryGstin11").getSelectedKey()],
						"retPeriod": sap.ui.getCore().byId("idA_TaxProcess12").getValue(),
						"tableSections": [],
						"isNilUserInput": oSummaryData.nilUserInput,
						"isHsnUserInput": oSummaryData.hsnUserInput
						...
						onClose: function (oAction) {
							if (oAction === "YES") {
								if (!this._oDialogResetBtn) {
									this._oDialogResetBtn = sap.ui.xmlfragment("com.ey.digigst.fragments.gstr1A.Reset", this);
		...
		onResetProcedDialog: function () { //eslint-disable-line
			//eslint-disable-line
			var that = this;
			if (this.ResetFlag === "P") {
				var oSelectedItem = this.getView().byId("tabGstr1Process").getSelectedIndices();
		...
		gstr1SaveToGstnResetFinal: function (payload) {
			...
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1ASaveToGstnReset.do",
					contentType: "application/json",
					data: JSON.stringify(payload)
				}).done(function (data, status, jqXHR) {
					sap.ui.core.BusyIndicator.hide();
		...
		gstr1SaveToGstnResetnonRespondedFinal: function (payload) {
			...
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1AAspResetNonRespondedDataOnly.do",
					contentType: "application/json",
					data: JSON.stringify(payload)
				}).done(function (data, status, jqXHR) {
					sap.ui.core.BusyIndicator.hide();
		...
		gstr1SaveToGstnResetAndSaveJob: function () { //eslint-disable-line
			if (this.ResetFlag === "P") {
				var oModelForTab1 = this.byId("tabGstr1Process").getModel("ProcessedRecord").getProperty("/"),
					oSelectedItem = this.getView().byId("tabGstr1Process").getSelectedIndices(),
					flag1 = false,
			...
			MessageBox.warning("Warning: The E-invoice data will be deleted in case you have not performed E-invoice recon!", {
				...
				onClose: function (sAction) {
					if (sAction === "Okay") {
						if (this.ResetFlag === "P") {
							if (flag1 == true && flag2 == true) {
								MessageBox.information("Auth token for few GSTINs is inactive, please activate to initiate Save GSTR-1A", {
									actions: ["Yes", "No"],
									emphasizedAction: "Yes",
									onClose: function (sAction) {
										if (sAction === "Yes") {
		...
		gstr1SaveToGstnResetAndSaveJobFinal: function (payload) {
			...
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1ASaveToGstnResetAndSaveJob.do",
					contentType: "application/json",
					data: JSON.stringify(payload)
				}).done(function (data, status, jqXHR) {
					sap.ui.core.BusyIndicator.hide();
		...
		onPressGetGst1Einv: function (oEvent, flag) {
			...
			if (!this._oDialog1) {
				this._oDialog1 = sap.ui.xmlfragment("com.ey.digigst.fragments.gstr1A.InvoiceStatus", this);
			...
			if (flag === "P") {
				...
				var oTaxPeriod = this.oView.byId("dtSummary").getValue(); //eslint-disable-line
				if (oTaxPeriod === "") {
					oTaxPeriod = null;
				}
		 ...
		 * @memberOf com.ey.digigst.view.GSTR1
		 */
		onPressGetBtnEINV: function (oEvent) { //eslint-disable-line
		...
		onSignFileDSC: function () {
			...
			if (!this._oDialogSaveStats56) {
				this._oDialogSaveStats56 = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.SignandFile", this);
			...
			$(document).ready(function ($) {
				$.ajax({
					...
					if (that.DSCEVCflag === "DSC") {
						data.resp.header = "Please select the authorised signatory for which DSC is attached for initiating filing of GSTR-1A"
					} else {
						data.resp.header = "Please select the authorised signatory for which EVC is attached for initiating filing of GSTR-1A"
					}
		...
		onSaveSign: function () {
			...
			var that = this;
			MessageBox.show(
				"Do you want to proceed with filing GSTR-1A?", {
					icon: MessageBox.Icon.INFORMATION,
					title: "Information",
					actions: [MessageBox.Action.YES, MessageBox.Action.NO],
					onClose: function (oAction) {
		...
		onSaveSign2: function () {
			...
			var GstnsList = "/aspsapapi/execSignAndFileGstr1AStage1.do";
			this.excelDownload(Request, GstnsList);
			this._oDialogSaveStats56.close();
			var that = this;
			MessageBox.show(
				"GSTR1A filing is initiated, click on filing status to view the status", {
					icon: MessageBox.Icon.INFORMATION,
					title: "Information",
					onClose: function (oAction) {
						that.onSearch();
		...
		onSaveSignEVCConformation: function () {
			...
			if (!this._oDialogSaveStatsConfirm) {
				this._oDialogSaveStatsConfirm = sap.ui.xmlfragment(this.getView().getId(),
					"com.ey.digigst.fragments.gstr1A.SignandFileConformation", this);
		...
		_getSignandFileOTP: function (data) {
			if (!this._oDialogSignandFileOTP) {
				this._oDialogSignandFileOTP = sap.ui.xmlfragment(this.getView().getId(),
					"com.ey.digigst.fragments.gstr1A.SignandFileOTP", this);
		...
		onFilingStatus: function () {
			if (!this._oDialogSaveStats9) {
				this._oDialogSaveStats9 = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.FilingStatus", this);
		...
		onSaveOkay12: function () {
			...
			var GstinSaveModel = new JSONModel();
			sap.ui.core.BusyIndicator.show(0);
			var GstnsList = "/aspsapapi/getGstr1AFilingStatus.do";
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: GstnsList,
		...
		onSaveStatusDownload1: function (oEvt) {
			...
			var oReqExcelPath = "/aspsapapi/downloadGstr1AErrors.do?id=" + this.oReqId1 + "";
		...
		onDeleteProcessScreenGstr1: function (payload) {
			var that = this;
			MessageBox.show(
				"GSTR-1A data available on GST common portal will be deleted. Do you want to proceed?", {
					icon: MessageBox.Icon.INFORMATION,
					title: "Information",
					actions: ["Yes", "No"],
					emphasizedAction: "Yes",
		...
		onPressdeleteAuotodreaftedfullFinal: function (payload) {
			...
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1AGstnResetJob.do",
					contentType: "application/json",
					data: JSON.stringify(payload)
				}).done(function (data, status, jqXHR) {
					sap.ui.core.BusyIndicator.hide();
		...
		onUpdateGstr1Summ1: function () {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/gstr1ABulkGetSumm.do",
					contentType: "application/json",
					data: JSON.stringify(payload)
				})
				.done(function (data, status, jqXHR) {
					if (data.hdr.status === "S") {
						if (!this._dGstr1Msg) {
							this._dGstr1Msg = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.BulkSave", this);
							this.getView().addDependent(this._dGstr1Msg);
						}
						this._dGstr1Msg.setTitle("Generate GSTR1A Summary Status");
			...
			$.ajax({
				...
				.done(function (data, status, jqXHR) {
					...
					if (data.hdr.status === "S") {
						...
						this._dGstr1Msg.setTitle("Fetch GSTR1A Summary Status");
						this.getView().byId("bulkSaveID").setModel(new JSONModel(data.resp), "BulkGstinSaveModel");
						this._dGstr1Msg.open();
					} else {
						MessageBox.error("Error Occured While Saving the Changes");
		...
		RFA: function () {
			...
			if (!this._Filter) {
				this._Filter = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.RFA", this);
		...
		onPressRequestMadeTo: function (oEvt) {
			...
			if (!this._oGstinPopover) {
				this._oGstinPopover = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.Popover2A", this);
		...
		onPressRequestCheckers: function (oEvt) {
			...
			if (!this._oRequestCheckers) {
				this._oRequestCheckers = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.PopoverRequestCheckers",
					this);
		...
		RFAS: function () {
			...
			if (!this._Filter1) {
				this._Filter1 = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.RFAS", this);
		...
		RPS: function () {
			if (!this._Filter) {
				this._Filter = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.RPS", this);
				this.getView().addDependent(this._Filter);
			}
			this._Filter.open();
		},
		...
		onPressInvoiceSericeE: function () {
			if (!this._oDialogbulkSaveStats2) {
				this._oDialogbulkSaveStats2 = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.BulkSave2", this);
		...
		onFileNILReturnP: function () {
			if (!this._oDialogbulkSaveStats1) {
				this._oDialogbulkSaveStats1 = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.BulkSave1", this);
				this.getView().addDependent(this._oDialogbulkSaveStats1);
			}

			if (!this.Importentmsg) {
				this.Importentmsg = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr1A.Importentmsg", this);
