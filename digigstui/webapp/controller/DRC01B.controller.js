sap.ui.define([
	...
	return BaseController.extend("com.ey.digigst.controller.DRC01B", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.ey.digigst.view.DRC01B
		 */
		onInit: function () {
			var that = this;
			var vDate = new Date();
		...
		onPressDiffLink: function (oEvt, data) {
			...
			if (!this._diffAmount) {
				this._diffAmount = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr3b.drc01b.diffAmount", this);
		...
		getReasonMaster: function () {
			return new Promise(function (resolve, reject) {
				...
				$.ajax({
					method: "GET",
					url: "/aspsapapi/getAllDrcReasons.do",
		...
		getDRC01BData: function () {
			...
			return new Promise(function (resolve, reject) {
				...
				$.ajax({
					method: "POST",
					url: "/aspsapapi/getDrcDetails.do",
					contentType: "application/json",
		...
		onPressReasonDiff: function (data, gstin, refid) {
			...
			if (!this._oDialogReasonDif) {
				this._oDialogReasonDif = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr3b.drc01b.drc01bReasonDiff",
					this);
		...
		OnSaveReason: function (flag) {
			...
			$.ajax({
				method: "POST",
				url: "/aspsapapi/saveUserReasons.do",
				contentType: "application/json",
				data: JSON.stringify(payload)
			}).done(function (data, status, jqXHR) {
				sap.ui.core.BusyIndicator.hide();
		...
		onPressAddDiff: function (data, gstin, refid) {
			...
			if (!this._oDialogAddDif) {
				this._oDialogAddDif = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr3b.drc01b.drc01bAddDiff", this);
		...
		onSaveChallan: function () {
			...
			$.ajax({
				method: "POST",
				url: "/aspsapapi/saveDifferentialDtls.do",
				contentType: "application/json",
				data: JSON.stringify(payload)
			}).done(function (data, status, jqXHR) {
				sap.ui.core.BusyIndicator.hide();
		...
		OnEditReason: function () {
			var tab = this.getView().byId("idDRC01BTable");
				...
				$.ajax({
					...
					if (data.hdr.status === "S") {
						if (data.resp.length === 0) {
							...
							that.onDialogPress("Fetch DRC01B Details");
						}
					} else {
		...
		onFileDRC01PartB: function () {
			...
			$.ajax({
					method: "POST",
					url: "/aspsapapi/drcPartBSaveBulk.do",
					contentType: "application/json",
					data: JSON.stringify(postData)
				}).done(function (data, status, jqXHR) {
					sap.ui.core.BusyIndicator.hide();
		...
		onPressFilingStatus: function (gstin) {
			if (!this._oDialogFilingStatus) {
				this._oDialogFilingStatus = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr3b.drc01b.FilingStatus", this);
		...
		getFilingStatus: function (oPayload) {
			...
			$.ajax({
				method: "POST",
				url: "/aspsapapi/getDrcFilingStatus.do",
		...
		onPressSaveStatus: function (gstin) {
			if (!this._oDialogSaveStatus) {
				this._oDialogSaveStatus = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr3b.drc01b.SaveStatus", this);
		...
		getSaveStatus: function () {
			...
			$.ajax({
				method: "POST",
				url: "/aspsapapi/getDrcSaveStatus.do",
				data: JSON.stringify(oPayload),
				contentType: "application/json"
			}).done(function (data, status, jqXHR) {
				sap.ui.core.BusyIndicator.hide();
		...
		onDownloadReport: function () {
			debugger //eslint-disable-line
			var oSelectedItem = this.getView().byId("idDRC01BTable").getSelectedIndices();
			var oModelForTab1 = this.byId("idDRC01BTable").getModel("ProessData").getData();
			...
			var url = "/aspsapapi/downloaddrcreport.do";
		...
		onSaveStatusDownload: function (createdOn, dataType) {
			...
			var oReqExcelPath = "/aspsapapi/gstr3bDrcJsonDownloadDocument.do";
		...
		onSignFileDSC: function () {
			...
			if (!this._oDialogSaveStats56) {
				this._oDialogSaveStats56 = sap.ui.xmlfragment(this.getView().getId(), "com.ey.digigst.fragments.gstr3b.drc01b.SignandFile", this);
			...
			$(document).ready(function ($) {
				$.ajax({
					...
					if (that.DSCEVCflag === "DSC") {
						data.resp.header = "Please select the authorised signatory for which DSC is attached for initiating filing of DRC01B"
					} else {
						data.resp.header = "Please select the authorised signatory for which EVC is attached for initiating filing of DRC01B"
					}
		...
		onSaveSign: function () {
			...
			var that = this;
			MessageBox.show(
				"Do you want to proceed with filing DRC01B?", {
		...
		onSaveSignEVCConformation: function () {
			...
			if (!this._oDialogSaveStatsConfirm) {
				this._oDialogSaveStatsConfirm = sap.ui.xmlfragment(this.getView().getId(),
					"com.ey.digigst.fragments.gstr3b.drc01b.SignandFileConformation", this);
		...
		_getSignandFileOTP: function (data) {
			if (!this._oDialogSignandFileOTP) {
				this._oDialogSignandFileOTP = sap.ui.xmlfragment(this.getView().getId(),
					"com.ey.digigst.fragments.gstr3b.drc01b.SignandFileOTP", this);
