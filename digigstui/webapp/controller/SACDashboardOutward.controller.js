sap.ui.define([
	...
	return BaseController.extend("com.ey.digigst.controller.SACDashboardOutward", {
		_bExpanded: true,
		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.ey.digigst.view.SACDashboardOutward
		 */
		onInit: function () {
			this.getOwnerComponent().getRouter().getRoute("SACDashboardOutward").attachPatternMatched(this._onRouteMatched, this);
		...
		_onRouteMatched: function (oEvent) {
			...
			oHashChanger.setHash("SACDashboardOutward");
		...
		getTopCustB2B: function (aPayload) {
			...
			$(document).ready(function ($) {
				$.ajax({
					...
					sap.ui.core.BusyIndicator.hide();
					if (data.hdr.status === "S") {
						data.resp.topCustomers.sort(function (a, b) {
							// Compare yAxis values first
							if (a.yAxis > b.yAxis) return -1;
							if (a.yAxis < b.yAxis) return 1;

							// If yAxis values are equal, compare xAxis values
							if (a.xAxis < b.xAxis) return -1;
							if (a.xAxis > b.xAxis) return 1;

							// If both yAxis and xAxis are equal, return 0
							return 0;
						});
						var oData = $.extend(true, {}, data);
