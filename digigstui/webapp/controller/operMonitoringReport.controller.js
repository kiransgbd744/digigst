sap.ui.define([
	...
	return Controller.extend("com.ey.digigst.controller.operMonitoringReport", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.ey.digigst.view.operMonitoringReport
		 */
		onInit: function () {
			this.getOwnerComponent().getRouter().getRoute("operMonitoringReport").attachPatternMatched(this._onRouteMatched, this);
		...
		_getDailyReconResults: function () {
			...
			var postData = {
				"req": {
					// "extractedFrom": this.byId("iFromDate").getValue(),
					// "extractedTo": this.byId("iToDate").getValue()
			...
			$(document).ready(function ($) {
				$.ajax({
					method: "POST",
					url: "/aspsapapi/getDailyReconResults.do",
					contentType: "application/json",
					data: JSON.stringify(postData)
				}).done(function (data, status, jqXHR) {
				
					sap.ui.core.BusyIndicator.hide();
					that.getOwnerComponent().setModel(new JSONModel(data), "DailyReconResults");
