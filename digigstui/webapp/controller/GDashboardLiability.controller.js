sap.ui.define([
	...
	return Controller.extend("com.ey.digigst.controller.GDashboardLiability", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf com.ey.digigst.view.GDashboardLiability
		 */
		onInit: function () {
			this.getOwnerComponent().getRouter().getRoute("GDashboardLiability").attachPatternMatched(this._onRouteMatched, this);
		},
		...
		_loadSupGstin: function () {
			return new Promise(function (resolve, reject) {
				var aEntity = this.getView().getModel("GloblePayload").getProperty("/entity"),
					PayLoad = {
						"req": {
		...
		onSelectTrendValue: function (oevent) {
			var vIndex = this.getView().byId("idRdValue").getSelectedIndex(),
				...
				feedValueAxis = this.getView().byId('valueAxisFeed1');
			...
			switch (vValue) {
			case "INVOICE_VALUE":
				oData.resp.displayList = oData.resp.totalLiabList;
				break;
			case "TAXABLE_VALUE":
				oData.resp.displayList = oData.resp.liabItcList;
				break;
			case "TAX_AMOUNT":
				oData.resp.displayList = oData.resp.liabCashList;
				break;
			}
			this.getView().setModel(new JSONModel(oData), "oMonthTrendModel");
			this.getView().getModel("oMonthTrendModel").refresh(true);
		...
		onGstinChange: function (evt) {
			...
			if (vkeys.includes("All")) {
				...
				for (var i = 0; i < oData.length; i++) {
					oSelectedKey.push(oData[i].gstin);
				};
				this.byId("idSupplier").setSelectedKeys(oSelectedKey);
			} else {
				var vLength1 = oData.length - 1;
