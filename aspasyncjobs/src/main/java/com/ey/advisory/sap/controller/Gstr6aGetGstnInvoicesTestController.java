...
@RestController
@Slf4j
public class Gstr6aGetGstnInvoicesTestController {
	...
	@PostMapping(value = "/ui/Gstr6aGstnGetSectionTest", produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> createGstr6aGstnGetJob(@RequestBody String request) {
		...
		try {
			...
			Type listType = new TypeToken<List<Gstr6aGetInvoicesReqDto>>() {
			}.getType();
			List<Gstr6aGetInvoicesReqDto> dtos = gson.fromJson(asJsonArray, listType);
			...
			for (Gstr6aGetInvoicesReqDto dto : dtos) {
				...
				if (isFailedGet == true) {
					...
					failedBatchs = batchRepo.findBatchByStatus(dto.getGstin(), dto.getReturnPeriod(),
							APIConstants.GSTR6A.toUpperCase(), failedStatus);
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("GSTR6a Failed GET eligible sections are {} ", failedBatchs);
					}
				} else {
					if (LOGGER.isDebugEnabled()) {
					...
					initAndInprogressBatchs = batchRepo.findBatchByStatus(dto.getGstin(), dto.getReturnPeriod(),
							APIConstants.GSTR6A.toUpperCase(), initAndInProgressStatus);
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("GSTR6a All GET Not eligible sections are {} ", initAndInProgressStatus);
					}
				}

				Long userRequestId = gstnUserRequestUtil.createGstnUserRequest(gstin, taxPeriod, APIConstants.GET,
						APIConstants.GSTR6A.toUpperCase(), groupCode,JobConstants.SYSTEM.toUpperCase(), false, false, false);
				dto.setUserRequestId(userRequestId);

				if (isEligible(isFailedGet, failedBatchs, initAndInprogressBatchs, APIConstants.B2B, gstr6aSections)) {
					dto.setType(APIConstants.B2B.toUpperCase());
					dto = createBatchAndSave(groupCode, dto);
					jsonParam = gson.toJson(dto);
					job = asyncJobsService.createJob(groupCode, JobConstants.GSTR6A_GSTN_GET_SECTION, jsonParam,
				...
				if (isEligible(isFailedGet, failedBatchs, initAndInprogressBatchs, APIConstants.B2BA, gstr6aSections)) {
				...
				if (isEligible(isFailedGet, failedBatchs, initAndInprogressBatchs, APIConstants.CDN, gstr6aSections)) {
				...
				if (isEligible(isFailedGet, failedBatchs, initAndInprogressBatchs, APIConstants.CDNA, gstr6aSections)) {
				...
				JsonObject json = new JsonObject();
				json.addProperty("gstin", gstin);
				json.addProperty("msg", "Get GSTR6a  Request initiated Successfully");
				respBody.add(json);
	...
	private Gstr6aGetInvoicesReqDto createBatchAndSave(String groupCode, Gstr6aGetInvoicesReqDto dto) {

		TenantContext.setTenantId(groupCode);
		// InActiveting Previous Batch Records
		batchRepo.softlyDelete(dto.getType().toUpperCase(), APIConstants.GSTR6A.toUpperCase(), dto.getGstin(),
				dto.getReturnPeriod());
		GetAnx1BatchEntity batch = batchUtil.makeBatchGstr6a(dto, dto.getType());
		// Save new Batch
	...
	private JsonArray getAllActiveGstnList(List<Gstr6aGetInvoicesReqDto> dtos) {
		...
		String msg = "";
		List<Gstr6aGetInvoicesReqDto> inActiveGstinDtos = new ArrayList<>();
	...
	private Boolean isEligible(Boolean isFailedGet, List<String> failedBatchs, List<String> initAndInprogressBatchs,
			String section, List<String> gstr6aSections) {
