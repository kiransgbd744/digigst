...
@RestController
@Slf4j
public class Gstr2XTestGetApiJobInsertionController {
	...
	@PostMapping(value = "/ui/Gstr2xGstnGetSectionTest", produces = {
	...
	public ResponseEntity<String> createGstr2XGstnGetJob(
		...
		try {
			...
			for (Gstr1GetInvoicesReqDto dto : dtos) {
				...
				String userName = null;/*SecurityContext.getUser()
						.getUserPrincipalName();*/
				Long userRequestId = gstnUserRequestUtil.createGstnUserRequest(
						gstin, taxPeriod, APIConstants.GET,
						APIConstants.GSTR2X.toUpperCase(), GROUP_CODE, userName, false, false, false);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					dto = createBatchAndSave(GROUP_CODE, dto);
					jsonParam = gson.toJson(dto);
					job = asyncJobsService.createJob(GROUP_CODE,
