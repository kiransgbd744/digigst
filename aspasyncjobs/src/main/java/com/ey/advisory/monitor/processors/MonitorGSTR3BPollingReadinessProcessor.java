...
@Slf4j
@Component("MonitorGSTR3BPollingReadinessProcessor")
public class MonitorGSTR3BPollingReadinessProcessor
	...
	final static List<String> STATUS_LIST = ImmutableList.of(
			APIConstants.SAVE_INITIATED, APIConstants.RECOMPUTE_INITIATED,
			APIConstants.SAVEPSTLIAB_INITIATED, APIConstants.POLLING_FAILED,
			...
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
						"Executing Monitoring"
								+ " MonitorGSTR3BPollingReadinessProcessor"
								+ ".executeForGroup()  method for group: '%s'",
						group.getGroupCode());
				LOGGER.debug(logMsg);
			}

			createGSTR3BPollingTasks(message, group.getGroupCode());

			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
						"Completed one cycle of periodic Monitoring"
								+ " job for GSTR3BPolling group '%s'",
						group.getGroupCode());
				LOGGER.debug(logMsg);
			}
	...
	private void createGSTR3BPollingTasks(Message message, String groupCode) {
		List<Gstr3BSaveStatusEntity> gstinListEligibleForPolling = null;
		if (LOGGER.isDebugEnabled()) {
		...
		gstinListEligibleForPolling = gstr3BSaveStatusRepository
		...
		if (gstinListEligibleForPolling != null
			...
			List<Long> saveStatusList = gstinListEligibleForPolling.stream()
					.map(Gstr3BSaveStatusEntity::getId)
					.collect(Collectors.toList());
			gstr3BSaveStatusRepository.updateRefIdStatusForList(saveStatusList,
					APIConstants.POLLING_INITIATED);
			gstinListEligibleForPolling.forEach(entity -> {
				if (LOGGER.isDebugEnabled()) {
					String logMsg = String.format(
							"GSTR3BPolling Job is created for combination:'%s'",
							entity);
					LOGGER.debug(logMsg);
				}
				postGSTR3BPollingJob(message.getId(), entity, groupCode);
			});
		} else {
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
	...
	private void postGSTR3BPollingJob(Long curJobId,
			Gstr3BSaveStatusEntity saveStatusEntity, String groupCode) {
		...
		AsyncExecJob execJob = asyncJobsService.createJob(groupCode,
				"PollGSTR3B", jobParams, "SYSTEM", 5L, curJobId, 0L);
		if (LOGGER.isDebugEnabled()) {
