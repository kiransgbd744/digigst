 ...
 */
@Slf4j
@Service("MonitorGstr2XPollingReadinessProcessor")
public class MonitorGstr2XPollingReadinessProcessor extends DefaultMultiTenantTaskProcessor {
	...
	public void executeForGroup(Group group, Message message, AppExecContext ctx) {
		try {
			...
			+ " MonitorGstr2XPollingReadinessProcessor"
						+ ".executeForGroup()  method for group: '%s'", 
						group.getGroupCode());
				LOGGER.debug(logMsg);
			}

			createGstr2XPollingTasks(message, group.getGroupCode());

			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
						"Completed one cycle of periodic Monitoring" 
				+ " job for Gstr2XPolling group '%s'",
						group.getGroupCode());
				LOGGER.debug(logMsg);
			}
		} catch (Exception ex) {
	...
	public void createGstr2XPollingTasks(Message message, String groupCode) {
		List<Gstr1SaveBatchEntity> gstinListEligibleForPolling = null;
		if (LOGGER.isDebugEnabled()) {
			String logMsg = String.format("Fetching saveStatus entities " + "but Polling yet to happen");
			LOGGER.debug(logMsg);
		}
		TenantContext.setTenantId(groupCode);
		gstinListEligibleForPolling = gstr1BatchRepository
				.findReferenceIdForPooling(APIConstants.GSTR2X.toUpperCase());
		...
		if (gstinListEligibleForPolling != null && !gstinListEligibleForPolling.isEmpty()) {
			List<Long> saveStatusList = gstinListEligibleForPolling.stream().map(Gstr1SaveBatchEntity::getId)
					.collect(Collectors.toList());
			LocalDateTime now = EYDateUtil.toUTCDateTimeFromLocal(LocalDateTime.now());
			gstr1BatchRepository.updateRefIdStatusForList(saveStatusList, APIConstants.POLLING_INPROGRESS, now);
			gstinListEligibleForPolling.forEach(entity -> {
				if (LOGGER.isDebugEnabled()) {
					String logMsg = String.format("Gstr2XPolling Job is created for combination:'%s'", entity);
					LOGGER.debug(logMsg);
				}
				postGstr2XPollingJob(message.getId(), entity, groupCode);
			});
		} else {
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format("There are no eligible Save Status entities for Polling '%s'",gstinListEligibleForPolling);
				LOGGER.debug(logMsg);
			}
		}
	}

	private void postGstr2XPollingJob(Long curJobId, Gstr1SaveBatchEntity saveStatusEntity, String groupCode) {
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("Posting Polling Job job for -> GSTIN: '%s', " 
					+ "GroupCode: '%s'",
		...
		PollingMessage calMessage = new PollingMessage(gstin, taxPeriod, 
				refId, batchId, returnType, section,
				userRequestId, retryCount, saveStatusEntity.getOperationType());
		String jobParams = JsonUtil.newGsonInstance().toJson(calMessage);
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("Created JobParams JSON for Calculation Job Task" 
					+ "for -> GSTIN: '%s'", gstin);
			LOGGER.debug(msg);
		}
		// Post a job to the job table.
		AsyncExecJob execJob = asyncJobsService.createJob(groupCode, 
				JobConstants.GSTR2X_RETURNSTATUS, jobParams,
				JobConstants.SYSTEM.toUpperCase(), 5L, curJobId, 0L);
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Created new Gstr2XReturnStatus Job with Job Id = '%d' " 
							+ "for -> jobparams: '%s',",
