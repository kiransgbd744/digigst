 ...
 */
@Slf4j
@Service("MonitorGstr8PollingReadinessProcessor")
public class MonitorGstr8PollingReadinessProcessor extends DefaultMultiTenantTaskProcessor{
			...
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
						"Executing Monitoring"
								+ " MonitorGstr7PollingReadinessProcessor"
	...
	public void createGstr1PollingTasks(Message message, String groupCode) {
		...
		gstinListEligibleForPolling = gstr1BatchRepository
				.findReferenceIdForPooling(APIConstants.GSTR8.toUpperCase());
		...
		if (gstinListEligibleForPolling != null
			...
			gstinListEligibleForPolling.forEach(entity -> {
				if (LOGGER.isDebugEnabled()) {
					String logMsg = String.format(
							"Gstr8Polling Job is created for combination:'%s'",
							entity);
					LOGGER.debug(logMsg);
				}
				postGstr8PollingJob(message.getId(), entity, groupCode);
			});
		} else {
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
	...
	private void postGstr8PollingJob(Long curJobId,
		...
		AsyncExecJob execJob = asyncJobsService.createJob(groupCode,
				JobConstants.GSTR8_RETURNSTATUS, jobParams,
				JobConstants.SYSTEM.toUpperCase(), 5L, curJobId, 0L);
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Created new Gstr7ReturnStatus Job with Job Id = '%d' "
							+ "for -> jobparams: '%s',",
