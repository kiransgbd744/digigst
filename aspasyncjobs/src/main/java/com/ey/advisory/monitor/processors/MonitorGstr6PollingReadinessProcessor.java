 ...
 */
@Slf4j
@Service("MonitorGstr6PollingReadinessProcessor")
public class MonitorGstr6PollingReadinessProcessor
			...
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format(
						"Executing Monitoring"
								+ " MonitorGstr6PollingReadinessProcessor"
	...
	public void createGstr1PollingTasks(Message message, String groupCode) {
		...
		gstinListEligibleForPolling = gstr1BatchRepository
				.findReferenceIdForPooling(APIConstants.GSTR6.toUpperCase());
		...
		if (gstinListEligibleForPolling != null
			...
			gstinListEligibleForPolling.forEach(entity -> {
				if (LOGGER.isDebugEnabled()) {
					String logMsg = String.format(
							"Gstr6Polling Job is created for combination:'%s'",
							entity);
					LOGGER.debug(logMsg);
				}
	...
	private void postGstr1PollingJob(Long curJobId,
		...
		AsyncExecJob execJob = asyncJobsService.createJob(groupCode,
				JobConstants.GSTR6_RETURNSTATUS, jobParams,
				JobConstants.SYSTEM.toUpperCase(), 5L, curJobId, 0L);
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Created new Gstr6ReturnStatus Job with Job Id = '%d' "
							+ "for -> jobparams: '%s',",
