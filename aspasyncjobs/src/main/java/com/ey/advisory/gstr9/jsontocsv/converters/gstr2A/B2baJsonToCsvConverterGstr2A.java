...
import com.ey.advisory.gstr9.jsontocsv.model.gstr2A.B2baInvoiceGstr2A;
import com.google.gson.stream.JsonReader;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Component("B2baJsonToCsvConverterGstr2A")
public class B2baJsonToCsvConverterGstr2A implements JsonToCsvConverter {

	private static final String CLASS_NAME = B2baJsonToCsvConverterGstr2A.class
			.getName();

	private static final String HEADER = "CustomerGstin,CounterPartyFlag,"
			+ "CheckSum,InvoiceNumber,InvoiceDate,Original Invoice,"
			+ "Original Invoice Date,InvoiceValue,Pos,ReverseCharge,"
			+ "InvoiceType,Difference Percentage,LineNumber,"
			+ "Taxable Value, IGST Amount, SGST Amount,"
			+ " CGST Amount, Cess Amount,Rate\r\n";
	...
	public void convertJsonTOCsv(JsonReader reader,
		...
		while (reader.hasNext()) {
			...
			if (rootName.equals(JobStatusConstants.GSTR2A_B2BA_TYPE)) {
				reader.beginArray();
				while (reader.hasNext()) {
					String ctin = "";
					String cfs = "";
					...
					while (reader.hasNext()) {
						...
						if (name.equals("ctin")) {
							...
							reader.beginArray();
							while (reader.hasNext()) {
								B2baInvoiceGstr2A invoice = JsonUtil
										.newGsonInstance(false)
										.fromJson(reader,
												B2baInvoiceGstr2A.class);
	...
	private static List<String[]> getLineItemList(B2baInvoiceGstr2A invoice,
