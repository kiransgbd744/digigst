...
import com.ey.advisory.gstr9.jsontocsv.model.gstr1.AtaInvoice;
import com.google.gson.stream.JsonReader;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Component("AtaJsonToCsvConverter")
public class AtaJsonToCsvConverter implements JsonToCsvConverter {
	...
	public void convertJsonTOCsv(JsonReader reader, BufferedWriter... csvWriter)
		...
		stateName = defaultStateCache
				.getStateName(CommonContext.getGstin().substring(0, 2));

		reader.beginObject();
		while (reader.hasNext()) {
			String rootName = reader.nextName();
			if (rootName.equals(JobStatusConstants.ATA_TYPE)) {
				reader.beginArray();
				while (reader.hasNext()) {
					AtaInvoice invoice = JsonUtil.newGsonInstance(false)
							.fromJson(reader, AtaInvoice.class);
	...
	private static void writeToCSV(AtaInvoice invoice, BufferedWriter bw)
			throws IOException {
		...
		GenUtil.appendStringToJoiner(joiner, GenUtil.toCsvString("")); // original
																		// supplytype
		GenUtil.appendStringToJoiner(joiner, GenUtil.toCsvString(stateName));
		GenUtil.appendStringToJoiner(joiner,
		...
		for (AtLineItemDetail lineItem : atLineItemList) {
			...
			GenUtil.appendStringToBuffer(buf, GenUtil
					.toCsvString(GenUtil.checkExpo(lineItem.getIgstAmount())));
			GenUtil.appendStringToBuffer(buf, ",");
			GenUtil.appendStringToBuffer(buf, GenUtil
					.toCsvString(GenUtil.checkExpo(lineItem.getCgstAmount())));
			GenUtil.appendStringToBuffer(buf, ",");
			GenUtil.appendStringToBuffer(buf, GenUtil
					.toCsvString(GenUtil.checkExpo(lineItem.getSgstAmount())));
			GenUtil.appendStringToBuffer(buf, ",");
			GenUtil.appendStringToBuffer(buf, GenUtil
					.toCsvString(GenUtil.checkExpo(lineItem.getCessAmount())));
			GenUtil.appendStringToBuffer(buf, ",");
			GenUtil.appendStringToBuffer(buf,
	...
	private void checkIsFiled() {
...

		{
			obj = getAnx1BatchRepo.findIsFiled(CommonContext.getGstin(),
					CommonContext.getTaxPeriod(), "ATA", "GSTR1A");
		} else {
			obj = getAnx1BatchRepo.findIsFiled(CommonContext.getGstin(),
					CommonContext.getTaxPeriod(), "ATA", "GSTR1");
		}
		if (obj != null) {
