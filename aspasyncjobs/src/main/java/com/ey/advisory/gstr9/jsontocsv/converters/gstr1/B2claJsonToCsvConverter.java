...
import com.ey.advisory.gstr9.jsontocsv.model.gstr1.B2claInvoice;
import com.google.gson.stream.JsonReader;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Component("B2claJsonToCsvConverter")
public class B2claJsonToCsvConverter implements JsonToCsvConverter {
	...
	public void convertJsonTOCsv(JsonReader reader, BufferedWriter... csvWriter)
		...
		while (reader.hasNext()) {
			...
			if (rootName.equals(JobStatusConstants.B2CLA_TYPE)) {
				reader.beginArray();
				while (reader.hasNext()) {
					String pos = "";
					reader.beginObject();
					...
					while (reader.hasNext()) {
						...
						if (name.equals("pos")) {
							...
							reader.beginArray();
							while (reader.hasNext()) {
								B2claInvoice invoice = JsonUtil
										.newGsonInstance(false)
										.fromJson(reader, B2claInvoice.class);
	...
	private static List<String[]> getLineItemList(B2claInvoice invoice,
