...
import com.ey.advisory.gstr9.jsontocsv.model.gstr2A.IsdInvoiceGstr2A;
import com.google.gson.stream.JsonReader;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Component("IsdJsonToCsvConverterGstr2A")
public class IsdJsonToCsvConverterGstr2A implements JsonToCsvConverter {

	private static final String HEADER = "CustomerGstin,CounterPartyFlag,"
			+ "CheckSum,DocumentNumber,DocumentDate,DocumentType,EligibleITC,"
	...
	public void convertJsonTOCsv(JsonReader reader,
		...
		while (reader.hasNext()) {
			...
			if (rootName.equals(JobStatusConstants.GSTR2A_ISD_TYPE)) {
				reader.beginArray();
				while (reader.hasNext()) {
					String ctin = "";
					String cfs = "";
					...
					while (reader.hasNext()) {
						...
						if (name.equals("ctin")) {
							...
							reader.beginArray();
							while (reader.hasNext()) {
								IsdInvoiceGstr2A invoice = JsonUtil
										.newGsonInstance(false)
										.fromJson(reader,
												IsdInvoiceGstr2A.class);
	...
	private static List<String[]> getLineItemList(IsdInvoiceGstr2A invoice,
