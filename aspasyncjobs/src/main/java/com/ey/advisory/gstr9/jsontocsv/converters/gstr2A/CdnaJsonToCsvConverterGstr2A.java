...
import com.ey.advisory.gstr9.jsontocsv.model.gstr2A.CdnaInvoiceGstr2A;
import com.google.gson.stream.JsonReader;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Component("CdnaJsonToCsvConverterGstr2A")
public class CdnaJsonToCsvConverterGstr2A implements JsonToCsvConverter {

	private static final String HEADER = "Customer Gstin,CounterPartyFlag,"
			+ "CheckSum,Note Type,Note Number,Original Note Number,Note Date,"
			+ "Original Note Date,Invoice Number,PreGST,Difference Percentage,"
			+ "Note Value,Invoice Date,Line Number,TaxableValue,IGST Amount,"
			+ "CGST Amount,SGST Amount,Cess Amount,Rate\r\n";
	...
	public void convertJsonTOCsv(JsonReader reader,
		...
		while (reader.hasNext()) {
			...
			if (rootName.equals(JobStatusConstants.GSTR2A_CDNA_TYPE)) {
				reader.beginArray();
				while (reader.hasNext()) {
					String ctin = "";
					String cfs = "";
					...
					while (reader.hasNext()) {
						...
						if (name.equals("ctin")) {
							...
							reader.beginArray();
							while (reader.hasNext()) {
								CdnaInvoiceGstr2A invoice = JsonUtil
										.newGsonInstance(false)
										.fromJson(reader,
												CdnaInvoiceGstr2A.class);
	...
	private static List<String[]> getLineItemList(CdnaInvoiceGstr2A invoice,
		...
		for (CdnLineItemGstr2A lineItem : cdnaLineItemList) {
			...
			arr[i++] = GenUtil.toCsvString(invoice.getOrgNoteNumber());
			arr[i++] = GenUtil.toCsvString(invoice.getNoteDate());
			arr[i++] = GenUtil.toCsvString(invoice.getOrgNoteDate());
