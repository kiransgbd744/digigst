...
@Component("GSTR6ISDJsonToCsvConverter")
@Slf4j
public class GSTR6ISDJsonToCsvConverter implements JsonToCsvConverter {

	private static final String GSTR6_ISD_HEADER = "Category,ReturnPeriod,IsdGstin,"
	...
	public void convertJsonTOCsv(JsonReader reader,
		...
		Gstr6IsdDetailsDto gstr6IsdDetailsDto = gson.fromJson(reader,
				Gstr6IsdDetailsDto.class);

		writeDataToCSV(gstr6IsdDetailsDto, csvWriter);
	}

	private void writeDataToCSV(Gstr6IsdDetailsDto gstr6IsdDetailsDto,
			BufferedWriter csvWriter) throws IOException {

		LOGGER.debug("Entering Gstr6ISD writeDataToCSV");
		StringJoiner finalJoiner = new StringJoiner("\n");

		List<Gstr6IsdElglstDto> elglstDtos = gstr6IsdDetailsDto.getElglst();
		List<Gstr6IsdElglstDto> inElglstDtos = gstr6IsdDetailsDto.getInelglst();
			...
			elglstDtos.forEach(elgstDto -> {
				...
				docListItems.forEach(lstItm -> {
					...
					GenUtil.appendStringToJoiner(joiner,
							GenUtil.toCsvString("Distribution")); // category
					...
					if(elgstDto.getTyp()!=null && elgstDto.getTyp().equalsIgnoreCase("R")){
						GenUtil.appendStringToJoiner(joiner,
								GenUtil.toCsvString(elgstDto.getCpty())); // recipient gstin
					}
					else{
						GenUtil.appendStringToJoiner(joiner,
			...
			inElglstDtos.forEach(inElgstDto -> {
				...
				docListItems.forEach(lstItm -> {
					...
					if(inElgstDto.getTyp()!=null && inElgstDto.getTyp().equalsIgnoreCase("R")){
						GenUtil.appendStringToJoiner(joiner,
								GenUtil.toCsvString(inElgstDto.getCpty())); // recipient gstin
					}
					else{
						GenUtil.appendStringToJoiner(joiner,
								GenUtil.toCsvString("URD")); // doubt
		...
		csvWriter.write("\r\n");
		LOGGER.debug("Exiting Gstr6ISD writeDataToCSV");

	}

	@Override
	public String[] getCsvHeaderStrings() {
		return new String[] { GSTR6_ISD_HEADER };
	}
