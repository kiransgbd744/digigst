...
import com.ey.advisory.gstr9.jsontocsv.model.gstr2A.TdsaInvoiceGstr2A;
import com.google.gson.stream.JsonReader;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@Component("TdsaJsonToCsvConverterGstr2A")
public class TdsaJsonToCsvConverterGstr2A implements JsonToCsvConverter {

	private static final String HEADER = "OriginalGstinDeducted,"
			+ "OriginalAmountDeducted,GstinDeducted,AmountDeducted,"
			+ "OriginalMonth,IGST Amount,CGST Amount,SGST Amount\r\n";
	...
	public void convertJsonTOCsv(JsonReader reader,
		...
		while (reader.hasNext()) {
			...
			if (rootName.equals(JobStatusConstants.GSTR2A_TDSA_TYPE)) {
				reader.beginArray();
				while (reader.hasNext()) {
					TdsaInvoiceGstr2A invoice = JsonUtil
							.newGsonInstance(false)
							.fromJson(reader, TdsaInvoiceGstr2A.class);
	...
	private static void writeToCSV(TdsaInvoiceGstr2A invoice, 
					BufferedWriter bw) throws IOException {
		StringJoiner joiner = new StringJoiner(",");
		GenUtil.appendStringToJoiner(joiner,
				GenUtil.toCsvString(invoice.getOrgGstinDeducted()));
		GenUtil.appendStringToJoiner(joiner,
				GenUtil.toCsvString(invoice.getOrgAmtDeducted()));
		GenUtil.appendStringToJoiner(joiner,
				GenUtil.toCsvString(invoice.getGstinDeducted()));
		GenUtil.appendStringToJoiner(joiner,
				GenUtil.toCsvString(invoice.getAmtDeducted()));
		GenUtil.appendStringToJoiner(joiner,
				GenUtil.toCsvString(invoice.getOrgMonth()));
		GenUtil.appendStringToJoiner(joiner,
