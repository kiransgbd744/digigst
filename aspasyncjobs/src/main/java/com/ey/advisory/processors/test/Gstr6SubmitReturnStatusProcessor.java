 ...
 */
@Slf4j
@Service("Gstr6SubmitReturnStatusProcessor")
public class Gstr6SubmitReturnStatusProcessor implements TaskProcessor {
	...
	public void execute(Message message, AppExecContext context) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("GSTR6_SUBMIT ReturnStatus Data Execute method is ON with message {} ", message);
		}
		String groupcode = message.getGroupCode();
		String jsonReq = message.getParamsJson();
		if (jsonReq != null && groupcode != null) {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Pooling Gstr6 Submit ReturnStatus- Request is {} ", jsonReq);
			}
			Gson gson = new Gson();
			...
			if (reqDto != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Pooling Gstr6 Submit ReturnStatus- batch is {} ", batch);
				}
