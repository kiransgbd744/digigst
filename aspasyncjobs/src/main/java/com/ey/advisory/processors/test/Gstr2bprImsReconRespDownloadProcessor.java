 ...
 */
@Slf4j
@Component("Gstr2bprImsReconRespDownloadProcessor")
public class Gstr2bprImsReconRespDownloadProcessor implements TaskProcessor {

	@Autowired
	@Qualifier("GSTR2bprImsPsdErrInfoReportDwnldServiceImpl")
	private GSTR2bprImsPsdErrInfoReportDwnldService reportDwnldService;
	...
	public void execute(Message message, AppExecContext context) {
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Gstr2bprImsReconRespDownloadProcessor Upload Job"
		...
		try {
			...
			if (reportName.equalsIgnoreCase("Error Reports")) {
				filePath = reportDwnldService.getErrorData(Long.valueOf(id));

			} else if (reportName.equalsIgnoreCase("Processed Reports")) {
				filePath = reportDwnldService.getPsdData(Long.valueOf(id));

			} /*else if (reportName.equalsIgnoreCase("Information Reports")) {
				filePath = reportDwnldService.getInfoData(Long.valueOf(id));
...
*/
			...
			if (filePath != null) {
				fileStatusDownloadReportRepo.updateStatus(id,
						"REPORT_GENERATED", filePath.getValue0(),
			...
			LOGGER.error("Exception in Gstr2bprImsReconRespDownloadProcessor ",
					e.getMessage());
			fileStatusDownloadReportRepo.updateStatus(id,
