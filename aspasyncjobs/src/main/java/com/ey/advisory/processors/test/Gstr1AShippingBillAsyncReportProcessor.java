 ...
 */
@Slf4j
@Service("Gstr1AShippingBillAsyncReportProcessor")
public class Gstr1AShippingBillAsyncReportProcessor implements TaskProcessor {

	@Autowired
	FileStatusDownloadReportRepository fileStatusDownloadReportRepo;

	@Autowired
	@Qualifier("AsyncGstr1AInvoiceManagementDownloadServiceImpl")
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Inside Shipping Bill Gstr1A Async Report Download processor with Report id : %d",
						id);
				LOGGER.debug(msg);
			}
