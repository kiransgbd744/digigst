...
import com.ey.advisory.app.services.jobs.erp.GstinValidatorMetadataRevIntHandler;
...
@Service("GstinValidatorMetaDataRevIntgProcessor")
public class GstinValidatorMetaDataRevIntgProcessor implements TaskProcessor {

	private final static Logger LOGGER = LoggerFactory
			.getLogger(GstinValidatorMetaDataRevIntgProcessor.class);

	@Autowired
	@Qualifier("GstinValidatorMetadataRevIntHandler")
	private GstinValidatorMetadataRevIntHandler handler;

	@Autowired
	@Qualifier("GstinValidatorPayloadRepository")
	private GstinValidatorPayloadRepository repository;

	@Override
	public void execute(Message message, AppExecContext context) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("GstinValidatorMetaDataRevIntgProcessor execute Begin");
		}
