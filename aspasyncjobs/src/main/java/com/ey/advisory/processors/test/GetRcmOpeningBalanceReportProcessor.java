...
@Slf4j
@Component("GetRcmOpeningBalanceReportProcessor")
public class GetRcmOpeningBalanceReportProcessor implements TaskProcessor {

	@Autowired
	@Qualifier("RcmOpeningBalanceReportServiceImpl")
	...
	public void execute(Message message, AppExecContext context) {
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("Begin GetRcmOpeningBalanceReport processor :%s",
					message.toString());
			LOGGER.debug(msg);
		}
		...
		Gson gson = new Gson();
		 id = json.get("id").getAsLong();
			...
			FileStatusDownloadReportEntity entity = fileStatusEntity.get();
