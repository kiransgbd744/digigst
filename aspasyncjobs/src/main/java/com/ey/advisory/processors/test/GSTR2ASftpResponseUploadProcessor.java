 ...
 */	
@Slf4j
@Component("GSTR2ASftpResponseUploadProcessor")
public class GSTR2ASftpResponseUploadProcessor implements TaskProcessor {
	...
	@Autowired
	@Qualifier("GSTR2ASftpRespUploadServiceImpl")
	private GSTR2AAutoReconRespUploadService service;

	@Override
	public void execute(Message message, AppExecContext context) {

		LOGGER.debug("Begin GSTR2AResponseUploadProcessor job");
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"GSTR2AResponseUploadProcessor Upload Job"
		...
		try {
			...
			LOGGER.debug("GSTR2A Response Upload File is in progress ");

			service.validateResponse(fileId, fileName,
			 folderName);
		} catch (Exception ex) {
			String msg = String.format(
					"Error occured in GSTR2AResponseUploadProcessor for %s ",
					jsonString);
			LOGGER.error(msg, ex);
