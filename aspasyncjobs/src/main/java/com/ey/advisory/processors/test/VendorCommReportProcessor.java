...
@Slf4j
@Component("VendorCommReportProcessor")
public class VendorCommReportProcessor implements TaskProcessor {
	...
	@Autowired
	@Qualifier("AsyncVendorCommReportUploadServiceImpl")
	AsyncVendorCommReportUploadService asyncVendorCommReportUploadService;
	...
	public void execute(Message message, AppExecContext context) {
		...
		try {
			...
			boolean isAPOpted = json.get("isAPOpted").getAsBoolean();
			...
			asyncVendorCommReportUploadService.generateVendorCommReports(id,isAPOpted);
		} catch (Exception ex) {
