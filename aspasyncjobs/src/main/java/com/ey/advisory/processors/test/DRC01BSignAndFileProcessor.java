...
@Slf4j
@Component("DRC01BSignAndFileProcessor")
public class DRC01BSignAndFileProcessor implements TaskProcessor {
	...
	public void execute(Message message, AppExecContext context) {
		...
		try {
			...
			Optional<SignAndFileEntity> optionalEntity = signAndFileRepo.findById(Long.valueOf(signId));
			...
			GstnAPIAuthInfo gstinAuth = gstinAuthInfo.findByGstinAndProviderName(gstin, "GSTN");
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"DRC01B Sign&File GSTN payload about to send is {}",
						data);
			}
			APIResponse drc01BFileApiResp = hitGstnServer.drc01BFileApiCall(
					TenantContext.getTenantId(), data, gstin, taxPeriod);
			if (drc01BFileApiResp.isSuccess()) {

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("SignAndFie Sandbox api success {} ",
							drc01BFileApiResp.getResponse());
				}
				JsonObject responseParams = (new JsonParser())
						.parse(drc01BFileApiResp.getResponse())
						.getAsJsonObject();

				String ackNum = responseParams.get("ack_num").getAsString();
				drc01BSignAndFileService.updateDrc01BTables(taxPeriod, gstin,
						signId, ackNum);

			} else {
				LOGGER.error("DRC01B Sign&File Failed with response {} ",
						drc01BFileApiResp.getError());
				signAndFileRepo.updateStatus(null, "Failed",
						drc01BFileApiResp.getError().getErrorDesc(),
						Long.valueOf(signId));
			}
		} catch (Exception ex) {
