...
public class ManageEinvoiceVersionController {
	...
	public ResponseEntity<String> generateBulkEinvoice(
		...
		try {
			...
			if (ex instanceof SocketException) {
				...
				response.setErrorMessage("Error While Generating E Invoice, Please Contact HelpDesk");
			}
			resp.add("hdr", gson.toJsonTree(APIRespDto.creatErrorResp()));
			resp.add("resp", gson.toJsonTree(response));
			reqLogHelper.updateResponsePayload(resp.toString(), nicStatus);
	...
	public ResponseEntity<String> cancelEinvoiceApi(
		...
		try {
			...
			if (errorList.isEmpty()) {
				...
				reqLogHelper.logInvDtlsParams(hdr.getIrn(),
						null, null, null);
			...
			if (ex instanceof SocketException) {
				...
				response.setErrorMessage("Error While Cancelling E Invoice, Please Contact HelpDesk");
			}
			resp.add("hdr", gson.toJsonTree(APIRespDto.creatErrorResp()));
			resp.add("resp", gson.toJsonTree(response));
			reqLogHelper.updateResponsePayload(resp.toString(), nicStatus);
	...
	public ResponseEntity<String> generateEWBByIrn(String jsonString) {
		...
		try {
			...
			if (ex instanceof SocketException) {
				...
				response.setErrorMessage("Error While Generating E-Invoice By IRN, Please Contact HelpDesk");
			}
			resp.add("hdr", gson.toJsonTree(APIRespDto.creatErrorResp()));
			resp.add("resp", gson.toJsonTree(response));
			reqLogHelper.updateResponsePayload(resp.toString(), nicStatus);
