...
public class EWBNICUserServiceImpl implements EWBNICUserService {
	...
	@Override
	public List<EWBNICUser> getAllEWBNICUsers(List<String> gstins) {
		List<EWBNICUser> ewdList = new ArrayList<EWBNICUser>();
		try {
			ewdList = ewbNICUserRepo.findByGstinIn(gstins);
	...
	public int updateEWBNICUser(EWBNICUser ewbNICUser) {
		...
		try {
			...
			if (persistedEntity == null) {
				...
				if (ewbNICUser.getClientId() != null
						&& ewbNICUser.getClientSecret() != null) {
					persistedEntity.setClientId(ewbNICUser.getClientId());
					persistedEntity
							.setClientSecret(ewbNICUser.getClientSecret());
				}
	...
	@Override
	public EWBNICUser getEwbNicUserByGstin(String gstin) {
		EWBNICUser ewbUser = null;
		try {
			ewbUser = ewbNICUserRepo.findByGstin(gstin);
		} catch (Exception ee) {
			LOGGER.error("Exception while fetching the EWB Users", ee);
		}
		return ewbUser;
