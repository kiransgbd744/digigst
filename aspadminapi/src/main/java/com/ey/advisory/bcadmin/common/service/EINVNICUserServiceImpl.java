...
@Slf4j
@Component("BcAdminEINVNICUserServiceImpl")
public class EINVNICUserServiceImpl implements EINVNICUserService {
	...
	@Override
	public List<EINVNICUser> getAllEINVNICUsers() {
		List<EINVNICUser> eInvList = null;
		try {
			eInvList = einvNICUserRepo.findAll();
	...
	public int updateEINVNICUser(EINVNICUser einvNICUser) {
		...
		try {
			EINVNICUser persistedEntity = einvNICUserRepo
					.findByGstin(einvNICUser.getGstin());
			...
			LOGGER.info("saved EINVuser {}", einvNICUser);
			if (updatedResult < 1) {
