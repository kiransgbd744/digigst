...
public class NICAPIAuthInfo implements APIAuthInfo, Serializable {
	 ...
	 * expiry time is not returned by NIC API, we keep a buffer of 5 mins. So we
	 * assume that the auth token expiry time is 5 hrs and 55 mins (or 355
	 * mins).
	 * 
	 */
	public static final long AUTHTOKEN_EXPIRY_IN_MILLIS = 355L * 60L * 1000L;

//	public static final long IRP_AUTHTOKEN_EXPIRY_IN_MILLIS = 355L * 60L * 1000L;

	public NICAPIAuthInfo(String appKey, String sessionEncKey,
			String sessionKey, String authToken, String apiProvider) {
		super();
		this.appKey = appKey;
		...
		long expTimeInMillis = authTokenTime.getTime()
				+ AUTHTOKEN_EXPIRY_IN_MILLIS;

//		if (apiProvider.equalsIgnoreCase(APIProviderEnum.EYEINV.name())) {
//			expTimeInMillis = authTokenTime.getTime()
//					+ IRP_AUTHTOKEN_EXPIRY_IN_MILLIS;
//		}
	...
	public String toString() {
		return "NICAPIAuthInfo [appKey=" + appKey + ", sessionEncKey="
				+ sessionEncKey + ", sessionKey=" + sessionKey + ", authToken="
				+ authToken + "]";
	}
