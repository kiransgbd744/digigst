 ...
 *
 */
@Component("GroupLevelAppPermissionServiceImpl")
public class GroupLevelAppPermissionServiceImpl implements AppPermissionService {

	@Autowired
	@Qualifier("GroupLevelPermissionsMasterRepository")
	private GroupLevelPermissionsMasterRepository permissionsMasterRepository;
	...
	private GroupLevelUserPermissionsRepository userPermissionsRepository;
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(GroupLevelAppPermissionServiceImpl.class);
	...
	public List<AppPermissionRespDto> getPermissions(
		...
		List<GroupLevelPermissionsMasterEntity> masterEntities = permissionsMasterRepository
				.getAllPermissions();
		...
		masterEntities.forEach(masterEntity -> {
			...
			GroupLevelUserPermissionsEntity userApp = new GroupLevelUserPermissionsEntity();

			if (userInfo != null) {
				userApp = userPermissionsRepository.getUserPermissions(userInfo.getId(),
	...
	public void updatePermissions(List<AppPermissionReqDto> appPermReqDtos) {
		...
		List<GroupLevelUserPermissionsEntity> upentities = new ArrayList<>();
		Long userId1 = userCreationRepository
				.findIdByUserName(appPermReqDtos.get(0).getUserName());
		
		userPermissionsRepository.updateIsFlagUserPerm(userId1);
		
		appPermReqDtos.forEach(appPermReqDto -> {
			Long userId = userCreationRepository
					.findIdByUserName(appPermReqDto.getUserName());
		
			GroupLevelUserPermissionsEntity uapentity = new GroupLevelUserPermissionsEntity();
			uapentity.setId(appPermReqDto.getId());
			uapentity.setGrpCode(TenantContext.getTenantId());
