...
@Repository("Gstr8AB2BASummaryDetailsRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr8AB2BASummaryDetailsRepository
		extends CrudRepository<Gstr8AGetB2BASummaryEntity, Long> {

	List<Gstr8AGetB2BASummaryEntity> findByCgstinAndFinYearAndIsDeleteFalse(
			String gstin, String finYear);

	@Modifying
	@Query("Update Gstr8AGetB2BASummaryEntity SET isDelete = true  WHERE "
			+ "finYear = :finYear AND cgstin = :gstin AND isDelete = false")
	void softDelete(@Param("gstin") String gstin, @Param("finYear") String finYear);
	
	@Modifying
	@Query("UPDATE Gstr8AGetB2BASummaryEntity SET isDelete = true, updatedOn = :updatedOn, updatedBy = :updatedBy "
