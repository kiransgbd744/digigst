...
import com.ey.advisory.common.OutwardTransDocSearchPredicateBuilder;
...
public interface DocRepository
		extends JpaRepository<OutwardTransDocument, Long>,
		JpaSpecificationExecutor<OutwardTransDocument>,
		DocSearchForSaveCustomRepository, ProcedureCallRepository {

	public List<OutwardTransDocument> findByDocNoAndDocDateAndDocTypeAndSgstin(
			String docNo, LocalDate docDate, String docType, String sgstin);

	public List<OutwardTransDocument> findByDocNoAndDocDateAndCgstinAndDocTypeAndSgstin(
			String docNo, LocalDate docDate, String cgstin, String docType,
			String sgstin);

	public List<OutwardTransDocument> findByDocNoAndDocDateAndDocTypeAndSgstinAndIsProcessedAndIsDeleted(
			String docNo, LocalDate docDate, String docType, String sgstin,
			boolean isProcessed, boolean isDeleted);

	public default Page<OutwardTransDocument> findDocsBySearchCriteria(
		...
		return findAll((root, criteriaQuery, criteriaBuilder) -> {
			...
			OutwardTransDocSearchPredicateBuilder builder = StaticContextHolder
					.getBean("DefaultOutwardTransDocSearchPredicateBuilder",
							OutwardTransDocSearchPredicateBuilder.class);
	...
	public default Page<OutwardTransDocument> findDocsByEinvoiceSearchCriteria(
		...
		return findAll((root, criteriaQuery, criteriaBuilder) -> {
			...
			EinvoiceDefaultOutwardDocSearchPredicateBuilder builder = StaticContextHolder
					.getBean("EinvoiceDefaultOutwardDocSearchPredicateBuilder",
							EinvoiceDefaultOutwardDocSearchPredicateBuilder.class);
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.isDeleted=true,"
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.gstnSaveRefId=:gstnSaveRefId,"
	...
	public List<OutwardTransDocument> findByGstnBatchIdAndDocKeyIn(
			Long gstnBatchId, List<String> docKey);

	// and doc.isDeleted = false is removed from below Query because this is
	// already been selected and sent to Gstn So even if it is deleted update
	// should happen.
	@Query("SELECT doc.docKey FROM OutwardTransDocument doc WHERE "
	 ...
	 * @Param("origDocDate") LocalDate origDocDate);
	 */
	@Query("SELECT count(g) from OutwardTransDocument g "
			+ "where g.isDeleted = false and g.isSubmitted = true "
			+ "and g.docKey = :docKey ")
	public int findSubmitTag(@Param("docKey") String docKey);

	@Modifying
	@Query("UPDATE OutwardTransDocument g SET g.isDeleted = true "
	...
	/*
	 * @Query("SELECT count(doc) from OutwardTransDocument doc where " +
	 ...
	 */
	@Query("SELECT count(outWardEntity) from OutwardTransDocument "
	 ...
	 * List<OutwardTransDocument> getDuplicateKeyDocnoAndDocdate(
	 * 
	 * @Param("sgstin") String sgstin,
	 * 
	 * @Param("origDocNo") String origDocNo,
	 ...
	 * "and doc.docNo=:origDocNo " +
	 * "and doc.docDate=:origDocDate AND doc.isDeleted = false") public
	 * List<OutwardTransDocument> getDuplicateKeyDocnoAndDocdateCR(
	 * 
	 * @Param("sgstin") String sgstin,
	 * 
	 * @Param("origDocNo") String origDocNo,
	 ...
	 * "and doc.docNo=:origDocNo " +
	 * "and doc.docDate=:origDocDate AND doc.isDeleted = false ") public
	 * List<OutwardTransDocument> getDuplicateKeyDocnoAndDocdateDR(
	 * 
	 * @Param("sgstin") String sgstin,
	 * 
	 * @Param("origDocNo") String origDocNo,
	...
	public List<OutwardTransDocument> findDocsByDocKey(
			@Param("docKey") String docKey);

	@Query("SELECT doc FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey IN (:docKeys) AND doc.supplyType <> 'CAN' AND doc.aspInvoiceStatus=2")
	public List<OutwardTransDocument> findDocsByDocKeyIn(
			@Param("docKeys") List<String> docKeys);

	@Query("SELECT doc FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey = :docKey "
			+ "and doc.supplyType <> 'CAN' AND doc.isSent=true")
	public List<OutwardTransDocument> findOrgDocsByDocKey(
			@Param("docKey") String docKey);

	@Query(value = "Select top 1 * from ANX_OUTWARD_DOC_HEADER where DOC_KEY =:docKey "
			+ "and SUPPLY_TYPE <> 'CAN' and "
			+ " IS_SENT_TO_GSTN = true order by ID desc", nativeQuery = true)
	public Optional<OutwardTransDocument> findLatestSavedDoc(
			@Param("docKey") String docKey);
	...
	@Query("SELECT doc.docKey,isSubmitted FROM OutwardTransDocument doc "
	...
	@Query("SELECT doc.docKey,COUNT(doc.id) FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey IN (:docKeys) "
	...
	// Gstn Invoive Errors
	public default Page<OutwardTransDocument> findGstnDocsBySearchCriteria(
		...
		return findAll((root, criteriaQuery, criteriaBuilder) -> {
			...
			GstnOutwardTransDocSearchPredicateBuilder builder = StaticContextHolder
					.getBean("GstnDefaultOutwardTransDocSearchPredicateBuilder",
							GstnOutwardTransDocSearchPredicateBuilder.class);
	...
	@Query("SELECT doc.preceedingInvoiceNumber,doc.preceedingInvoiceDate "
			+ "FROM OutwardTransDocument doc WHERE doc.docKey = :invKey AND "
			+ "doc.isDeleted = false")
	...
	@Query("SELECT doc.cgstin," + "COUNT(doc.id),SUM(doc.docAmount),"
			...
			+ "FROM OutwardTransDocument doc WHERE "
	...
	@Query("SELECT doc.id, doc.sgstin, doc.docNo, doc.docType, doc.docDate, "
			...
			+ "FROM OutwardTransDocument doc INNER JOIN OutwardTransDocError "
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.erpBatchId  = :batchId,"
	...
	@Query("SELECT COUNT(*) FROM OutwardTransDocument "
			+ "WHERE sgstin=:gstin AND taxperiod = :taxperiod ")
	public int gstinCount(@Param("gstin") String gstin,
			@Param("taxperiod") String taxperiod);

	@Query("SELECT doc.sgstin FROM OutwardTransDocument doc WHERE "
	...
	public List<OutwardTransDocument> findKeyByOrgDocnoAndOrgDocdate(
			@Param("docType") String docType, @Param("sgstin") String sgstin,
			@Param("finYear") String finYear,
			@Param("origDocNo") String origDocNo);

	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.irn  = :irn,"
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.eInvErrorDesc = "
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.ewbNoresp  = :ewbNo,"
	...
	@Query("UPDATE OutwardTransDocument doc SET doc.ewbNoresp  = :ewbNo,"
			+ " doc.updatedDate = CURRENT_TIMESTAMP,doc.ewbDateResp = :ewbDt ,"
			+ "doc.ewbStatus = :status, "
			+ "doc.ewbProcessingStatus = :ewbProcessingStatus,doc.ewbErrorCode = :errorCode, "
			+ "doc.ewbErrorDesc = :errorDesc,doc.aspInvoiceStatus = :aspInvStatus "
			+ " WHERE doc.id = :id")
	public void updateEwayBillNo(@Param("id") Long id,
			@Param("ewbNo") Long ewbNo, @Param("ewbDt") LocalDateTime ewbDt,
			@Param("status") Integer status,
	...
	@Query("SELECT doc FROM OutwardTransDocument doc WHERE "
			+ "doc.id IN (:ids)")
	public List<OutwardTransDocument> fetchEnivStatusById(
			@Param("ids") List<Long> ids);

	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET "
	...
	@Query("SELECT doc.docKey FROM  OutwardTransDocument doc "
			+ "WHERE doc.id = :oldId AND doc.isDeleted= false")
	String findDocKeyById(@Param("oldId") Long oldId);

	@Query("SELECT doc.id FROM  OutwardTransDocument doc "
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.ewbErrorCode = :errorCode, "
	...
	@Query("SELECT doc.docKey,doc.docDate FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey IN (:docKeys) AND doc.isDeleted = false and  "
			+ "doc.isSubmitted = false")
	List<Object[]> findActiveOrgDocsByDocKeys(
			@Param("docKeys") List<String> docKeys);

	@Query("SELECT doc.docKey,doc.docDate,doc.docAmount FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey = :docKey AND doc.isDeleted = false and  "
			+ "doc.isSubmitted = false")
	List<Object[]> findActiveOrgDocsByDocKeys(@Param("docKey") String docKey);

	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.isSubmitted=true,"
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.isFiled=true,"
	...
	@Query("SELECT doc.docKey,doc.docDate,doc.ewbNoresp,doc.ewbDateResp,"
			+ "doc.ewbStatus FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey IN (:docKeys) AND doc.isDeleted = false and  "
			+ "doc.isSubmitted = false and doc.ewbNoresp IS NOT NULL and "
			+ "doc.ewbDateResp IS NOT NULL and doc.ewbStatus <> 6 and doc.isProcessed = true and doc.supplyType <> 'CAN' ")
	List<Object[]> findActiveEwbDocsByDocKeys(
			@Param("docKeys") List<String> docKeys);

	@Query("SELECT MAX(doc.id) FROM OutwardTransDocument doc WHERE doc.sgstin "
	...
	List<OutwardTransDocument> findActiveOrgDocumentByDocKeys(
			@Param("docKeys") List<String> docKeys);

	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.isSent = false, doc.isSaved "
	...
	@Query("SELECT doc.docKey,doc.docDate,doc.irnResponse,doc.ackDate,"
			+ "doc.irnStatus,doc.ackNum FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey IN (:docKeys) AND doc.isDeleted = false and  "
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.ackNum = :ackNum, "
	...
	@Query("SELECT d.irnStatus FROM OutwardTransDocument d WHERE d.id = :id")
	public int findByIrnStatus(@Param("id") Long id);
	...
	public List<OutwardTransDocument> findByFileId(@Param("id") Long id);

	@Query("SELECT doc FROM OutwardTransDocument doc "
			+ "WHERE doc.id IN (:ids)  ")
	public List<OutwardTransDocument> findById(@Param("ids") List<Long> ids);
	...
	@Query("SELECT doc.docNo, doc.sgstin FROM OutwardTransDocument doc "
			+ "WHERE doc.id = :id  ")
	...
	@Query("Select docKey, aspInvoiceStatus from OutwardTransDocument "
			+ "where docKey In(:docKeys) AND isDeleted = false")
	public List<Object[]> findAspInvoiceStatusByDocKeys(
			@Param("docKeys") List<String> docKeys);

	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.isSaved =:isSaved, "
	...
	@Query("SELECT doc.id,doc.isSent,doc.isSaved ,doc.isGstnError from OutwardTransDocument doc where doc.id In (:docId)")
	public List<Object[]> getGstnSaveSentAndErrorFlagsById(
			@Param("docId") List<Long> docId);

	@Query("SELECT doc.id from OutwardTransDocument doc where doc.id IN :ids AND doc.isDeleted = false")
	...
	@Modifying
	@Query("UPDATE OutwardTransDocument doc SET doc.einvGstnSaveStatus =:einvStatus, doc.updatedDate = CURRENT_TIMESTAMP,"
			+ " doc.reconResponseId = :reconResponseId,doc.reconResponseSource =:reconResponseSource "
			+ " WHERE doc.id In (:docId)")
	...
	@Query("SELECT doc.docKey,doc.docDate,doc.einvGstnSaveStatus "
			+ "FROM OutwardTransDocument doc "
			+ "WHERE doc.docKey IN (:docKeys) AND doc.isDeleted = false and  "
			+ "doc.isSubmitted = false and doc.einvGstnSaveStatus <> 'R' ")
	List<Object[]> findActiveEinvGstinSaveStatus(
			@Param("docKeys") List<String> docKeys);

	@Query("SELECT doc.docKey,doc.gstnBifurcation FROM OutwardTransDocument doc "
	...
	@Query(value = "SELECT HDR.ID,COUNT(ITM.DOC_HEADER_ID) "
			+ "FROM ANX_OUTWARD_DOC_HEADER HDR,ANX_OUTWARD_DOC_ITEM ITM "
	...
	@Query("select distinct cgstin from OutwardTransDocument where isDeleted = false "
	...
	@Query("select distinct derivedCgstinPan from OutwardTransDocument where isDeleted = false "
	...
	public Optional<OutwardTransDocument> findDocByEwbNo(
			@Param("ewbNoresp") Long ewbNoresp);

	@Query("SELECT doc from OutwardTransDocument doc where doc.id=:id AND doc.isDeleted = false")
	public Optional<OutwardTransDocument> getActiveRecord(@Param("id") Long id);

	@Query("SELECT concat(doc.taxperiod,doc.docKey) FROM OutwardTransDocument doc"
			+ " WHERE doc.docKey IN (:docs) and supplyType <> 'CAN' and doc.isDeleted=false and isProcessed = true")
	...
	public List<String> findByDocKeyInAndIsDeletedFalseWithTaxPeriodWithCan(
			@Param("docs") List<String> docs);
