...
@Slf4j
@Component("Gstr2AutoReconErpReportFetchDetailsImpl")
public class Gstr2AutoReconErpReportFetchDetailsImpl
		implements Gstr2AutoReconErpReportFetchDetails {
		...
		if (noOfChunk != 0) {
			...
			try {
				...
				String invoiceHeadersTemplate = commonUtility
						.getProp("gstr2.auto.recon.erp.report.header");
				writer.append(invoiceHeadersTemplate);

				String[] columnMappings = commonUtility
						.getProp("getr2.auto.recon.erp.report.column")
						.split(",");
				StatefulBeanToCsv<Gstr2AutoReconErpReportDto> beanWriter 
							= getBeanWriter(
				...
				while (j < noOfChunk) {
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("ERP Report no of records after proc "
								+ "call {} ", records.size());
					}
	...
	private StatefulBeanToCsv<Gstr2AutoReconErpReportDto> getBeanWriter(
		...
		ColumnPositionMappingStrategy<Gstr2AutoReconErpReportDto> 
		mappingStrategy = new ColumnPositionMappingStrategy<>();
		mappingStrategy.setType(Gstr2AutoReconErpReportDto.class);
		mappingStrategy.setColumnMapping(columnMappings);
		StatefulBeanToCsvBuilder<Gstr2AutoReconErpReportDto> builder =
	...
	private Gstr2AutoReconErpReportDto convert(Object[] arr) {
		...
		obj.setDocumentNumber2A((arr[16] != null) ? DownloadReportsConstant.CSVCHARACTER
				.concat(arr[16].toString()) : null);
		obj.setDocumentNumberPR((arr[17] != null) ? DownloadReportsConstant.CSVCHARACTER
		...
		obj.setSysDefField3((arr[99] != null)
				? (new BigDecimal(arr[99].toString())).toString() : null);
		obj.setSysDefField4((arr[100] != null)
				? (new BigDecimal(arr[100].toString())).toString() : null);
		obj.setSysDefField5((arr[101] != null) ? arr[101].toString() : null);
		obj.setSysDefField6((arr[102] != null) ? arr[102].toString() : null);
		obj.setSysDefField7(
				(arr[103] != null) ? DownloadReportsConstant.CSVCHARACTER
						.concat(arr[103].toString()) : null);
		obj.setSysDefField8(
				(arr[104] != null) ? DownloadReportsConstant.CSVCHARACTER
