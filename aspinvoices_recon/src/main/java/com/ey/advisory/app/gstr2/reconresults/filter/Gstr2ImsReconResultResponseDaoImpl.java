...
@Slf4j
@Component("Gstr2ImsReconResultResponseDaoImpl")
public class Gstr2ImsReconResultResponseDaoImpl
		...
		if (reconType.equalsIgnoreCase("2A_PR")) {
			...
			try {
				...
				if (!ReconStatusConstants.SUCCESS.equalsIgnoreCase(response)) {
					...
					StoredProcedureQuery imsStoredProc = null;
					String imsResponse = null;
					try {
						imsStoredProc = entityManager
								.createStoredProcedureQuery(
										"USP_RECON_RESULT_IMS_RESP_PSD");

						if (LOGGER.isDebugEnabled()) {
							String msg = String.format(
									"About to execute Ims Recon SP with ConfigId :%s",
									batchId.toString());
							LOGGER.debug(msg);
						}

						imsStoredProc.registerStoredProcedureParameter(
								"IN_BATCHID", Long.class, ParameterMode.IN);

						imsStoredProc.setParameter("IN_BATCHID", batchId);

						imsResponse = (String) imsStoredProc.getSingleResult();

						LOGGER.debug("USP_RECON_RESULT_IMS_RESP_PSD"
								+ " :: " + imsResponse);

						LOGGER.debug(
								"ims recon response is Processed after executing proc for fileId: {}",
								fileId);
						...
						String msg = String.format(
								"Exception while retrieving the Details from ims proc USP_RECON_RESULT_IMS_RESP_PSD ",
								e);
						LOGGER.error(msg, e);
			...
			try {
				...
				if (!ReconStatusConstants.SUCCESS.equalsIgnoreCase(response)) {
					 ...
					 * , batchId.toString(), "TBL_2BPR_RECON_RESP_UPLOAD");
					 * LOGGER.debug(msg); }
					 */

					/*
					 * String failurePocName =
					 * "USP_RECON_2BPR_RESP_PSD_FAILURE"; try { Map<Integer,
					 * String> procs = fetchStoredGSTR2bProcedures(); for
					 * (Integer indx : procs.keySet()) { String procName =
					 * procs.get(indx); try { String status =
					 * executeGstr2bProcedure(procName, Long.valueOf(batchId));
					 * if (!(ReconStatusConstants.SUCCESS
					 * .equalsIgnoreCase(status) || "Success with Errors"
					 * .equalsIgnoreCase(status))) { String msg = String.format(
					 * "Batch Id is '%s', Response " + "from PROC %s did not " +
					 * "return success," + " Hence updating to Failed", batchId,
					 * procName); LOGGER.error(msg); throw new
					 * AppException(msg); } } catch (Exception ex) { String msg
					 * = String.format(
					 * "Calling recon response failure Proc '%s' for Batch Id is '%s'"
					 * , failurePocName, batchId); LOGGER.debug(msg);
					 * executeGstr2bProcedure(failurePocName,
					 * Long.valueOf(batchId)); msg = String.format(
					 * "Batch Id is '%s', Response " +
					 * "from TBL_2BPR_RECON_RESP_UPLOAD procs did not " +
					 * "return success," + " Hence updating to Failed",
					 * batchId.toString()); LOGGER.error(msg); throw new
					 * AppException(msg);
					 * 
					 * } } LOGGER.debug(
					 * "recon response is Processed for fileId: {}", fileId);
					 * 
					 * return ReconStatusConstants.SUCCESS;
					 * 
					 * } catch (Exception e) { String msg = String.format(
					 * "Exception Occure while fetching Proc List for recon response from TBL :'%s'"
					 * , failurePocName); LOGGER.error(msg, e); throw new
					 * AppException(e); }
					 */
					StoredProcedureQuery imsStoredProc = null;
					String imsResponse = null;
					try {
						imsStoredProc = entityManager
								.createStoredProcedureQuery(
										"USP_2BPR_RECON_RESULT_IMS_RESP_PSD");
						...
						LOGGER.debug("USP_2BPR_RECON_RESULT_IMS_RESP_PSD"
								+ " :: " + imsResponse);
						...
						String msg = String.format(
								"Exception while retrieving the Details from ims proc USP_2BPR_RECON_RESULT_IMS_RESP_PSD ",
								e);
						LOGGER.error(msg, e);
						throw new AppException(e);
