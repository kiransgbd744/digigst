...
import com.ey.advisory.app.asprecon.gstr2.pr2b.reports.Gstr2Recon2BPRReportDto;
 ...
 */
@Slf4j
@Component("Gstr2BPRReconResultsRptDownloadServiceImpl")
public class Gstr2BPRReconResultsRptDownloadServiceImpl
		implements Gstr2ReconResultRptDownloadService {
	...
	@Autowired
	@Qualifier("Gstr2BPRReconResultReportCommonServiceImpl")
	...
	private static final String CONF_KEY = "recon.result.2bpr.report.chunk.size";
	private static final String CONF_CATEG = "RECON_RESULT_REPORTS";

	public String generateReconResultReport(Long configId) throws IOException {
		...
		tempDir = createTempDir(configId, "Consolidated_2BvsPR_(Recon Result)");

		fullPath = tempDir.getAbsolutePath() + File.separator
				+ getUniqueFileName("Consolidated_2BvsPR_(Recon Result)") + "_1"
				+ ".csv";
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_RECON_2BPR_INS_CHUNK_RECON_RESULT_UI");

			storedProc.registerStoredProcedureParameter("P_REPORT_DOWNLOAD_ID",
					Long.class, ParameterMode.IN);

			storedProc.setParameter("P_REPORT_DOWNLOAD_ID", configId);
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format(
						"Executing chunking proc"
								+ " USP_RECON_2BPR_INS_CHUNK_RECON_RESULT_UI: '%s'",
						configId.toString());
				LOGGER.debug(msg);
			}
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format("Chunking proc Executed"
						+ " USP_RECON_2BPR_INS_CHUNK_RECON_RESULT_UI : configId '%d', "
		...
		if (noOfChunk > 0) {
			...
			try {
				...
				String[] columnMappings = commonUtility
						.getProp("gstr2.recon.2bpr.report.column").split(",");
				...
				while (j < noOfChunk) {
					...
					StoredProcedureQuery storedProc = entityManager
							.createStoredProcedureQuery(
									"USP_RECON_2BPR_DISP_CHUNK_RECON_RESULT_UI");
					...
					if (LOGGER.isDebugEnabled()) {
						msg = String.format("call stored proc with "
								...
								configId.toString(),
								"Consolidated_2BvsPR_(Recon Result)", j);
						LOGGER.debug(msg);
					}
					...
					if (LOGGER.isDebugEnabled()) {
						msg = String.format(
								"Total Time taken to load the Data"
										...
										+ " '%s' - '%s'",
								dbLoadTimeDiff,
								"Consolidated_2BvsPR_(Recon Result)",
								records.size());
						LOGGER.debug(msg);
					}
					...
					if (records != null && !records.isEmpty()) {
						...
						List<Gstr2Recon2BPRReportDto> reconDataList = records
						...
						if (count >= maxLimitPerFile) {
							...
							if (isReportAvailable) {
								...
								commonService.chunkZipFiles(tempDir, configId,
										configId,
										"Consolidated_2BvsPR_(Recon Result)",
							...
							fullPath = tempDir.getAbsolutePath()
									+ File.separator
									+ 
											getUniqueFileName("Consolidated_2BvsPR_(Recon Result)")
									+ "_" + fileIndex + ".csv";
				...
				LOGGER.error(
						"Exception while executing the query for "
								+ "Report Type :{}",
						"Consolidated_2BvsPR_(Recon Result)", ex);
	...
	private Gstr2Recon2BPRReportDto convert(Object[] arr) {
		...
		obj.setOrgDocNumber2B((arr[43] != null) ? arr[43].toString() : null);
		...
		if (arr[118] != null && (!arr[118].toString().trim().isEmpty())) {
			obj.setTaxPeriodGSTR3B((arr[118] != null)
	...
	if (arr[173] != null && (!arr[173].toString().trim().isEmpty())) {
		obj.setTaxPeriodGSTR3B((arr[173] != null) ?"'".concat(arr[173].toString()) : null); 
	...
	private static File createTempDir(Long requestId, String reportType)
		...
		String tempFolderPrefix = reportType + "_" + requestId;
