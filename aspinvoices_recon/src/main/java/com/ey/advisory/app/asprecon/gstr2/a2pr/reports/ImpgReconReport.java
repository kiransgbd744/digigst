...
@Slf4j
@Component("ImpgReconReport")
public class ImpgReconReport {
	...
	public void getGstr2ImpgReconReport(Long configId) throws IOException {
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("Get Consolidated IMPG Report Details "
		...
		if (noOfChunk != 0) {
			...
			String logMsg = String.format("Get Recon Report Details with "
					...
					+ "endId %d, chunkSize %d," + " Consolidated IMPG Report ",
			...
			addlReportRepo.updateAndSave(configId, "Consolidated IMPG Report");
			int j = 0;
				...
				while (j < noOfChunk) {
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"Total Time taken to load the Data"
										...
										+ " '%s' - '%s'",
								dbLoadTimeDiff, "Consolidated IMPG Report",
								records.size());
						LOGGER.debug(msg);
					}
					...
					if (records != null && !records.isEmpty()) {
						...
						if (reconDataList != null && !reconDataList.isEmpty()) {
							...
							ColumnPositionMappingStrategy<Gstr2ImpgReconDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
							mappingStrategy.setType(Gstr2ImpgReconDto.class);
							mappingStrategy.setColumnMapping(columnMappings);
							StatefulBeanToCsvBuilder<Gstr2ImpgReconDto> builder = new StatefulBeanToCsvBuilder<>(
									writer);
	...
	private Gstr2ImpgReconDto convert(Object[] arr) {
		...
		obj.setSupplyTypePR((arr[63] != null) ? arr[63].toString() : null);
		...
		obj.setSupplierTypeA2((arr[127] != null) ? arr[127].toString() : null);
