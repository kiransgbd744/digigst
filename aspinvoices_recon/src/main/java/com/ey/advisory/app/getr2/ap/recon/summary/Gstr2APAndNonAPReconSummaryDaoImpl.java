...
@Slf4j
@Component("Gstr2APAndNonAPReconSummaryDaoImpl")
public class Gstr2APAndNonAPReconSummaryDaoImpl
		implements Gstr2APAndNonAPReconSummaryDao {
	...
	public List<Gstr2ReconSummaryDto> findReconSummary(Long configId,
			...
			String reconType, Integer toTaxPeriod_2A, Integer fromTaxPeriod_2A, String criteria) {
		...
		try {
			...
			LOGGER.info(
					"Inside Gstr2APAndNonAPReconSummaryDaoImpl.findReconSummary():: "
							+ "Invoking USP_AUTO_2APR_MANUAL_RECON_SUMMARY Stored Proc");
			if ("2BPR".equalsIgnoreCase(reconType))
			{
				...
				if(criteria != null) {
					...
					storedProc.setParameter("P_B2_FROM_RETURN_PERIOD", fromTaxPeriod_2A);

					storedProc.registerStoredProcedureParameter("P_B2_TO_RETURN_PERIOD",
							Integer.class, ParameterMode.IN);

					storedProc.setParameter("P_B2_TO_RETURN_PERIOD", toTaxPeriod_2A);
			...
			{
				LOGGER.debug("Inside USP_EINVPR_RECON_SUMMARY block");
				String returnPer = "";
				
				storedProc = entityManager.createStoredProcedureQuery(
						"USP_EINVPR_RECON_SUMMARY");	
			...
			storedProc = entityManager.createStoredProcedureQuery(
					"USP_AUTO_2APR_MANUAL_RECON_SUMMARY");
			...
			storedProc.registerStoredProcedureParameter("P_A2_FROM_RETURN_PERIOD",
					Integer.class, ParameterMode.IN);

			storedProc.setParameter("P_A2_FROM_RETURN_PERIOD", fromTaxPeriod_2A);

			storedProc.registerStoredProcedureParameter("P_A2_TO_RETURN_PERIOD",
					Integer.class, ParameterMode.IN);

			storedProc.setParameter("P_A2_TO_RETURN_PERIOD", toTaxPeriod_2A);
			
			if(criteria == null){
				criteria = "2APRtaxperiod";
			}
