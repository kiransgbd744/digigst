...
@Slf4j
@Component("AutoReconSummaryServiceImpl")
public class AutoReconSummaryServiceImpl implements AutoReconSummaryService {
	...
	@Override
	public AutoReconSummaryTabDto getReconSummaryDetails(
			List<String> recipientGstins, String fromTaxPeriodPR,
			String toTaxPeriodPR, String fromTaxPeriod2A, String toTaxPeriod2A,
		...
		try {
			...
			LOGGER.debug("AutoReconSummaryServiceImpl.getReconSummaryDetails"
					+ " objList {} ", objList);
			...
			List<AutoReconSummaryDto> overallData = new ArrayList<>();
			overallData.addAll(respListLock);
			overallData.addAll(respListUnlock);

			return new AutoReconSummaryTabDto(overallData, updatedOnItc);
		} catch (Exception ee) {
	...
	@Override
	public IncrementalDataSummaryTabDto getIncremenatalDataSummaryDetails(
		...
		try {
			...
			Object[] obj = (Object[]) autoReconSummaryDao
					.getIncrementalDataUpdatedOn(entityId);
			Timestamp updatedOn = obj[0] != null ? (Timestamp) obj[0] : null;
			String updatedOnItc = "";
			String toDateItc = "";
			if (updatedOn != null) {
				updatedOnItc = EYDateUtil.toISTDateTimeFromUTC(updatedOn)
						.toString();
				updatedOnItc = String.format("%-20s", updatedOnItc).replace(' ', '0');
				updatedOnItc = updatedOnItc.substring(0, 19);
				updatedOnItc = updatedOnItc.replace("T", " ");
				String toDate = updatedOnItc.substring(0, 10);
				LocalDate date = LocalDate.parse(toDate);
				date = date.plusDays(1);
				toDateItc = date + updatedOnItc.substring(10);
			}
			List<IncrementalDataSummaryDto> retList = objList.stream()
					.map(o -> convertToIncrementalDataDto(o))
					.collect(Collectors.toCollection(ArrayList::new));
			
			LOGGER.debug("AutoReconSummaryServiceImpl."
					+ "getIncremenatalDataSummaryDetails retList {} ", retList);

			Collections.sort(retList, (a, b) -> a.getOrder()
					.compareToIgnoreCase(b.getOrder()));
			
			return new IncrementalDataSummaryTabDto(retList, updatedOnItc,
					toDateItc);
		} catch (Exception ee) {
			String msg = String
					.format("Error while Executing getIncremenatalDataSummaryDetails  "
							+ "  for recipientGstins :%s", recipientGstins);
			LOGGER.error(msg, ee);
			throw new AppException(msg);
		}
	}
	...
	private List<AutoReconSummaryDto> convertScreenLevelData(
		...
		for (AutoReconSummaryDto resp : retList) {
			...
			if (type.equalsIgnoreCase(resp.getLockStatus())) {
				...
				else if (perticularName.equalsIgnoreCase("Mismatch (Imports)")) {
	...
	@Override
	public List<Gstr2ReconSummaryStatusDto> getAutoReconGstinDetails(
			Long entityId) {
		try {
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Inside AutoReconSummaryServiceImpl  entityId %s",
						entityId);
				LOGGER.debug(msg);
			}
			...
			// sgstinsList = Arrays.asList("33GSPTN0482G1Z9",
			// "33GSPTN0481G1ZA");
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"sGstins after dataSecurity are :%s for entityId :%s",
						sgstinsList, entityId);
				LOGGER.debug(msg);
			}
			List<Object[]> objList = autoReconSummaryDao
					.getAutoReconGstinsData(entityId);

			List<Gstr2ReconSummaryStatusDto> gstr2ReconStatus = objList
					.parallelStream().map(o -> convert(o))
					.collect(Collectors.toCollection(ArrayList::new));

			List<Gstr2ReconSummaryStatusDto> finalAutoReconStatus = new ArrayList<>();
			...
			for (Gstr2ReconSummaryStatusDto obj : gstr2ReconStatus) {
				...
				if (!CollectionUtils.isEmpty(sgstinsList)) {
					if (sgstinsList.contains(gstin)) {
						finalAutoReconStatus.add(obj);
					}
				}
			}
			return finalAutoReconStatus;
		} catch (Exception ee) {
			String msg = String
					.format("Exception occured while generating getAutoReconGstinDetails"
							+ " for entityId :%s", entityId);
			LOGGER.error(msg, ee);
			throw new AppException(msg, ee);
		}
	}
