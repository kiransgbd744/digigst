...
@Component("Anx2PRSDetailsB2B")
public class Anx2PRSDetailsB2B implements Anx2PRSDetailsSection {
	...
	public Anx2PRSDetailHeaderDto getAnx2PRSummaryDetail(
		...
		Anx2PRSDetailResponseDto dritem = new Anx2PRSDetailResponseDto();
		Anx2PRSDetailResponseDto inv = new Anx2PRSDetailResponseDto();
		Anx2PRSDetailResponseDto rcritem = new Anx2PRSDetailResponseDto();
		Anx2PRSDetailResponseDto rdritem = new Anx2PRSDetailResponseDto();
		...
		if (entityResponse.size() > 0) {
			b2b = entityResponse.stream().filter(
					p -> GSTConstants.B2B.equalsIgnoreCase(p.getTaxDocType()))
		...
		if (entityResponse.size() <= 0 || b2b.size() <= 0) {
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.INV))) {
			...
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.DR))) {
				dritem.setTable(GSTConstants.DEBIT_NOTE);
				dritem.setCount(0);
				dritem.setInvValue(BigDecimal.ZERO);
				dritem.setTaxableValue(BigDecimal.ZERO);
				dritem.setTotalTaxPayable(BigDecimal.ZERO);
				dritem.setIGST(BigDecimal.ZERO);
				dritem.setCGST(BigDecimal.ZERO);
				dritem.setSGST(BigDecimal.ZERO);
				dritem.setCess(BigDecimal.ZERO);
				dritem.setTotalCreditEligible(BigDecimal.ZERO);
				dritem.setCeIGST(BigDecimal.ZERO);
				dritem.setCeCGST(BigDecimal.ZERO);
				dritem.setCeSGST(BigDecimal.ZERO);
				dritem.setCeCess(BigDecimal.ZERO);
				sectionList.add(dritem);
			}
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.CR))) {
			...
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.RNV))) {
			...
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.RDR))) {
				rdritem.setTable(GSTConstants.DEBIT_NOTE_A);
				rdritem.setCount(0);
				rdritem.setInvValue(BigDecimal.ZERO);
				rdritem.setTaxableValue(BigDecimal.ZERO);
				rdritem.setTotalTaxPayable(BigDecimal.ZERO);
				rdritem.setIGST(BigDecimal.ZERO);
				rdritem.setCGST(BigDecimal.ZERO);
				rdritem.setSGST(BigDecimal.ZERO);
				rdritem.setCess(BigDecimal.ZERO);
				rdritem.setTotalCreditEligible(BigDecimal.ZERO);
				rdritem.setCeIGST(BigDecimal.ZERO);
				rdritem.setCeCGST(BigDecimal.ZERO);
				rdritem.setCeSGST(BigDecimal.ZERO);
				rdritem.setCeCess(BigDecimal.ZERO);
				sectionList.add(rdritem);
			}
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.RCR))) {
		...
		if (entityResponse.size() > 0 || b2b.size() > 0) {
			...
			List<Anx2PRSDetailResponseDto> drList = b2b.stream().filter(
					p -> GSTConstants.DR.equalsIgnoreCase(p.getInvType()))
					.collect(Collectors.toList());

			int drcount = 0;
			BigDecimal drinvVal = BigDecimal.ZERO;
			BigDecimal drinvtaxableValue = BigDecimal.ZERO;
			BigDecimal drtaxPayable = BigDecimal.ZERO;
			...
			BigDecimal drinvCreditEligible = BigDecimal.ZERO;
			BigDecimal drceigst = BigDecimal.ZERO;
			BigDecimal drcecgst = BigDecimal.ZERO;
			BigDecimal drcesgst = BigDecimal.ZERO;
			BigDecimal drcecess = BigDecimal.ZERO;

			if (drList != null && drList.size() > 0) {

				for (Anx2PRSDetailResponseDto drdata : drList) {
					drcount = drcount + drdata.getCount();
					drinvVal = drinvVal.add(drdata.getInvValue());
					drinvtaxableValue = drinvtaxableValue
							.add(drdata.getTaxableValue());
					drtaxPayable = drtaxPayable
							.add(drdata.getTotalTaxPayable());
					drigst = drigst.add(drdata.getIGST());
					drcgst = drcgst.add(drdata.getCGST());
					drsgst = drsgst.add(drdata.getSGST());
					drcess = drcess.add(drdata.getCess());
					drinvCreditEligible = drinvCreditEligible
							.add(drdata.getTotalCreditEligible());
					drceigst = drceigst.add(drdata.getCeIGST());
					drcecgst = drcecgst.add(drdata.getCeCGST());
					drcesgst = drcesgst.add(drdata.getCeSGST());
					drcecess = drcecess.add(drdata.getCeCess());
			...
			dritem.setInvValue(drinvVal);
			dritem.setTaxableValue(drinvtaxableValue);
			dritem.setTotalTaxPayable(drtaxPayable);
			dritem.setIGST(drigst);
			dritem.setCGST(drcgst);
			dritem.setSGST(drsgst);
			dritem.setCess(drcess);
			dritem.setTotalCreditEligible(drinvCreditEligible);
			dritem.setCeIGST(drceigst);
			dritem.setCeCGST(drcecgst);
			dritem.setCeSGST(drcesgst);
			dritem.setCeCess(drcecess);
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.DR))) {
				sectionList.add(dritem);
			}
			...
			List<Anx2PRSDetailResponseDto> rdrList = b2b.stream().filter(
					p -> GSTConstants.RDR.equalsIgnoreCase(p.getInvType()))
					.collect(Collectors.toList());

			int rdrcount = 0;
			BigDecimal rdrinvVal = BigDecimal.ZERO;
			BigDecimal rdrinvtaxableValue = BigDecimal.ZERO;
			BigDecimal rdrtaxPayable = BigDecimal.ZERO;
			BigDecimal rdrigst = BigDecimal.ZERO;
			BigDecimal rdrcgst = BigDecimal.ZERO;
			BigDecimal rdrsgst = BigDecimal.ZERO;
			BigDecimal rdrcess = BigDecimal.ZERO;
			BigDecimal rdrinvCreditEligible = BigDecimal.ZERO;
			BigDecimal rdrceigst = BigDecimal.ZERO;
			BigDecimal rdrcecgst = BigDecimal.ZERO;
			BigDecimal rdrcesgst = BigDecimal.ZERO;
			BigDecimal rdrcecess = BigDecimal.ZERO;

			if (rdrList != null && rdrList.size() > 0) {

				for (Anx2PRSDetailResponseDto rdrdata : rdrList) {
					rdrcount = rdrcount + rdrdata.getCount();
					rdrinvVal = rdrinvVal.add(rdrdata.getInvValue());
					rdrinvtaxableValue = rdrinvtaxableValue
							.add(rdrdata.getTaxableValue());
					rdrtaxPayable = rdrtaxPayable
							.add(rdrdata.getTotalTaxPayable());
					rdrigst = rdrigst.add(rdrdata.getIGST());
					rdrcgst = rdrcgst.add(rdrdata.getCGST());
					rdrsgst = rdrsgst.add(rdrdata.getSGST());
					rdrcess = rdrcess.add(rdrdata.getCess());
					rdrinvCreditEligible = rdrinvCreditEligible
							.add(rdrdata.getTotalCreditEligible());
					rdrceigst = rdrceigst.add(rdrdata.getCeIGST());
					rdrcecgst = rdrcecgst.add(rdrdata.getCeCGST());
					rdrcesgst = rdrcesgst.add(rdrdata.getCeSGST());
					rdrcecess = rdrcecess.add(rdrdata.getCeCess());
				}
			}
			rdritem.setTable(GSTConstants.DEBIT_NOTE_A);
			rdritem.setCount(rdrcount);
			rdritem.setInvValue(rdrinvVal);
			rdritem.setTaxableValue(rdrinvtaxableValue);
			rdritem.setTotalTaxPayable(rdrtaxPayable);
			rdritem.setIGST(rdrigst);
			rdritem.setCGST(rdrcgst);
			rdritem.setSGST(rdrsgst);
			rdritem.setCess(rdrcess);
			rdritem.setTotalCreditEligible(rdrinvCreditEligible);
			rdritem.setCeIGST(rdrceigst);
			rdritem.setCeCGST(rdrcecgst);
			rdritem.setCeSGST(rdrcesgst);
			rdritem.setCeCess(rdrcecess);
			if (docType != null && docType
					.contains(trimAndConvToUpperCase(GSTConstants.RDR))) {
				sectionList.add(rdritem);
			}
		...
		totCount = dritem.getCount() + inv.getCount() + critem.getCount()
				+ rdritem.getCount() + rnvitem.getCount() + rcritem.getCount();
		invValue = dritem.getInvValue().add(inv.getInvValue())
				.subtract(critem.getInvValue()).add(rdritem.getInvValue())
				.add(rnvitem.getInvValue()).subtract(rcritem.getInvValue());
		tottaxableVal = dritem.getTaxableValue().add(inv.getTaxableValue())
				.subtract(critem.getTaxableValue())
				.add(rdritem.getTaxableValue()).add(rnvitem.getTaxableValue())
		...
		totalTaxPayable = dritem.getTotalTaxPayable()
				...
				.add(rdritem.getTotalTaxPayable())
				.add(rnvitem.getTotalTaxPayable())
				.subtract(rcritem.getTotalTaxPayable());
		totIgst = dritem.getIGST().add(inv.getIGST()).subtract(critem.getIGST())
				.add(rdritem.getIGST()).add(rnvitem.getIGST())
				.subtract(rcritem.getIGST());
		totCgst = dritem.getCGST().add(inv.getCGST()).subtract(critem.getCGST())
				.add(rdritem.getCGST()).add(rnvitem.getCGST())
				.subtract(rcritem.getCGST());
		totSgst = dritem.getSGST().add(inv.getSGST()).subtract(critem.getSGST())
				.add(rdritem.getSGST()).add(rnvitem.getSGST())
				.subtract(rcritem.getSGST());
		totCess = dritem.getCess().add(inv.getCess()).subtract(critem.getCess())
				.add(rdritem.getCess()).add(rnvitem.getCess())
				.subtract(rcritem.getCess());
		totCreditEligible = dritem.getTotalCreditEligible()
				.add(inv.getTotalCreditEligible())
				.subtract(critem.getTotalCreditEligible())
				.add(rdritem.getTotalCreditEligible())
				.add(rnvitem.getTotalCreditEligible())
				.subtract(rcritem.getTotalCreditEligible());
		totCeIgst = dritem.getCeIGST().add(inv.getCeIGST())
				.subtract(critem.getCeIGST()).add(rdritem.getCeIGST())
				.add(rnvitem.getCeIGST()).subtract(rcritem.getCeIGST());
		totCeCgst = dritem.getCeCGST().add(inv.getCeCGST())
				.subtract(critem.getCeCGST()).add(rdritem.getCeCGST())
				.add(rnvitem.getCeCGST()).subtract(rcritem.getCeCGST());
		totCeSgst = dritem.getCeSGST().add(inv.getCeSGST())
				.subtract(critem.getCeSGST()).add(rdritem.getCeSGST())
				.add(rnvitem.getCeSGST()).subtract(rcritem.getCeSGST());
		totCeCess = dritem.getCeCess().add(inv.getCeCess())
				.subtract(critem.getCeCess()).add(rdritem.getCeCess())
				.add(rnvitem.getCeCess()).subtract(rcritem.getCeCess());

		prsDetail.setTable("3B-B2B");
