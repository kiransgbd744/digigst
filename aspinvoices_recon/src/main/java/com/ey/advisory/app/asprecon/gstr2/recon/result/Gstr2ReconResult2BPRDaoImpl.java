 ...
 */
@Slf4j
@Service("Gstr2ReconResult2BPRDaoImpl")
public class Gstr2ReconResult2BPRDaoImpl implements Gstr2ReconResult2BPRDao {
    ...
    Integer> getReconResult2BPR(
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"The parameters passed to USP_2BPR_RECON_RESULT_SCREEN_UI proc "
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_2BPR_RECON_RESULT_SCREEN_UI");
			...
			StoredProcedureQuery storedProcSummary = entityManager
					.createStoredProcedureQuery(
							"USP_2BPR_RECON_RESULT_SCREEN_UI_SMRY");

			if (LOGGER.isDebugEnabled()) {
				String msg = String
						.format("About to execute USP_2BPR_RECON_RESULT_SCREEN_UI_SMRY SP with "
								+ "gstin :%s", gstins);
				LOGGER.debug(msg);
			}
	...
	private ReconSummaryDto convertSummary(Object[] arr) {
	    ...
	    dto.setB2VendorGstinCount(col0  != null ? Long.valueOf(col11)       : null);
	    dto.setB2DocumentCount   (col1  != null ? Integer.valueOf(col12)    : null);
	    dto.setB2TotalTax        (col2  != null ? new BigDecimal(col13)     : null);
	    dto.setB2Igst            (col3  != null ? new BigDecimal(col14)     : null);
	    dto.setB2Cgst            (col4  != null ? new BigDecimal(col15)     : null);
	    dto.setB2Sgst            (col5  != null ? new BigDecimal(col16)     : null);
	    dto.setB2Cess            (col6  != null ? new BigDecimal(col17)     : null);
	...
	private String createQueryForCount(String condtion,
			Gstr2ReconResultReqDto reqDto) {
		StringBuilder query = new StringBuilder();
		query.append("SELECT Top 1000 ");  
		query.append(
				"  IFNULL( PR_RECIPIENT_GSTIN, B2_RECIPIENT_GSTIN ) AS GSTIN, ");
		query.append(
				"  IFNULL( PR_SUPPLIER_GSTIN, B2_SUPPLIER_GSTIN ) AS VENDOR_GSTIN, ");
		query.append("  PR_DOC_TYPE,    B2_DOC_TYPE,    PR_DOC_NUM, ");
		query.append("  B2_DOC_NUM,    PR_DOC_DATE,    B2_DOC_DATE, ");
		query.append(
		...
		query.append("  IFNULL(PR_IGST, 0) AS PR_IGST, ");
		query.append("  IFNULL(B2_IGST, 0) AS B2_IGST, ");
		query.append("  IFNULL(PR_CGST, 0) AS PR_CGST, ");
		query.append("  IFNULL(B2_CGST, 0) AS B2_CGST, ");
		query.append("  IFNULL(PR_SGST, 0) AS PR_SGST, ");
		query.append("  IFNULL(B2_SGST, 0) AS B2_SGST, ");
		query.append("  IFNULL(PR_CESS, 0) AS PR_CESS, ");
		query.append("  IFNULL(B2_CESS, 0) AS B2_CESS, ");
		query.append("  CURRENT_MISMATCH_REASON,    LT.USER_RESPONSE, ");
		query.append("  STG.FMCOMMENTS,    NULL AS PAYMENT_STATUS, ");
		query.append("  LT.B2_ID,    LT.PR_ID,    LT.RECON_LINK_ID, ");
		query.append("  GSTR_3B_TAX_PERIOD,    LT.CURRENT_REPORT_TYPE, ");
		query.append("  LT.B2_TAXABLE_VALUE,    LT.PR_TAXABLE_VALUE, ");
		query.append("  LT.B2_POS,    LT.PR_POS  FROM ");
		...
		query.append(
				"LEFT OUTER JOIN TBL_2BPR_RECON_RESP_PSD PSD ON ( LT.PR_INVOICE_KEY = PSD.INVOICEKEYPR OR LT.B2_INVOICE_KEY = PSD.INVOICEKEYB2 ) AND PSD.ENDDTM IS NULL ");
		query.append(
		...
		if (!Strings.isNullOrEmpty(reqDto.getFromDocDate())
			...
			query.append(
					" AND ((PR_DOC_DATE BETWEEN :fromDocDate AND :toDocDate) OR "
							+ " (B2_DOC_DATE BETWEEN :fromDocDate AND :toDocDate))  ");
		}
	...
	private String queryCondition(Gstr2ReconResultReqDto reqDto) {

		if (LOGGER.isDebugEnabled()) {
			String msg = " Begin Gstr2ReconResult2BPRDaoImpl.queryCondition() ";
			LOGGER.debug(msg);
		}
		...
		// && !Strings.isNullOrEmpty(reqDto.getToDocDate())) {
		// condition1.append(
		// " OR ( FROM_DOC_DATE >=:fromDocDate AND TO_DOC_DATE <=:toDocDate) ");
		// }
		condition1.append(")");

		if (!CollectionUtils.isEmpty(reqDto.getDocType())) {
			/*
			 * condition1.append(" AND IFNULL(LT.PR_INV_TYPE ,LT.B2_INV_TYPE ) "
			 * + "IN (:docType) ");
			 */
			condition1.append(
					" AND (LT.PR_INV_TYPE IN (:docType) OR LT.B2_INV_TYPE IN (:docType)) ");

		}

		/*
		 * if (!CollectionUtils.isEmpty(reqDto.getReportType())) {
		 * condition1.append(" AND  LT.CURRENT_REPORT_TYPE IN (:reportType) ");
		 * }
		 */
		if (!CollectionUtils.isEmpty(reqDto.getDocNumber())) {

			condition1.append(
					"AND (UPPER(PR_DOC_NUM) IN (:docNum) OR UPPER(B2_DOC_NUM) IN (:docNum) )");
		}
	...
	private Gstr2ReconResultDto convert(Object[] arr, String reconCriteria) {
		...
		try {
			...
			if (arr[26] != null
					&& arr[26].toString()
							.equalsIgnoreCase(APIConstants.ADDITION2B)
					|| arr[26].toString()
							.equalsIgnoreCase(APIConstants.ADDITION_2B_IMPG)) {
				convertDtoForAddition2BIMPG(arr, dto,reconCriteria);
				...
				convertDtoForOtherRptType(arr, dto, reconCriteria);
			}
	...
	private void convertDtoForOtherRptType(Object[] arr,
	...
	private void convertDtoForForceMatch(Object[] arr,
		...
		String docTypePR = checkNull(arr[2]);
		String docType2A = checkNull(arr[3]);
		String id2B = checkNull(arr[22]);
		String idPR = checkNull(arr[23]);

		if (id2B != null && idPR != null) {
			if (docTypePR != null && (docTypePR.equalsIgnoreCase("CR")
					|| docTypePR.equalsIgnoreCase("C")
					|| docTypePR.equalsIgnoreCase("RCR"))) {
	...
	private void convertDtoForAddition2BIMPG(Object[] arr,
			Gstr2ReconResultDto dto, String reconCriteria) {
		String docType2A = checkNull(arr[3]);
		dto.setDocType2A(docType2A);
		dto.setDocTypeMatch(true);
		...
		if ((arr[18] != null && (arr[26] != null
				&& arr[26].toString().equalsIgnoreCase(APIConstants.ADDITION2B)
				|| arr[26].toString()
						.equalsIgnoreCase(APIConstants.ADDITION_2B_IMPG)))) {
			dto.setMismatchReason(checkNull(arr[18]));
		} else {
	...
	@Override
	public Integer getReconResult2BPRCount(Gstr2ReconResultReqDto reqDto) {
		List<String> gstins = reqDto.getGstins();
		String toTaxPeriod = reqDto.getToTaxPeriod();
		String fromTaxPeriod = reqDto.getFromTaxPeriod();
		String fromDocDate = reqDto.getFromDocDate();
		String toDocDate = reqDto.getToDocDate();
		List<String> docNum = reqDto.getDocNumber();
		List<String> reportType = reqDto.getReportType();
		List<String> docType = reqDto.getDocType();

		try {
			String condtion = queryCondition(reqDto);
			String queryString = createQueryForCount(condtion, reqDto);
	...
	private void createReportTypeQuery(Gstr2ReconResultReqDto reqDto,
			StringBuilder query) {
