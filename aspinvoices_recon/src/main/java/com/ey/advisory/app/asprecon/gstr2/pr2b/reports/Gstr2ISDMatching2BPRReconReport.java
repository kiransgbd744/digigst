...
@Slf4j
@Component("Gstr2ISDMatching2BPRReconReport")
public class Gstr2ISDMatching2BPRReconReport {
	...
	public void getGstr2IsdReconReport(Long configId) throws IOException {
		...
		if (LOGGER.isDebugEnabled()) {
			 msg = String.format("Get ISD Matching Report Details "
					+ "with configId:'%s'", configId);
			LOGGER.debug(msg);
		}

		String reportType = "ISD Matching Report";
		String reportTypeUi = "ISD Matching Report";
					...
					if (LOGGER.isDebugEnabled()) {
						 msg = String.format(
								"call stored proc with "
										...
										+ "chunksize is %d",
								configId.toString(), "ISD Matching Report",
								startId, chunkSize);
						LOGGER.debug(msg);
					}
	...
	private StatefulBeanToCsv<Gstr2Recon2BPRReportDto> getBeanWriter(
		...
		ColumnPositionMappingStrategy<Gstr2Recon2BPRReportDto> mappingStrategy = 
