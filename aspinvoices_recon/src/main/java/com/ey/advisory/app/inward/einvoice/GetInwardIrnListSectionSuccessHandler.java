...
@Service("GetInwardIrnListSectionSuccessHandler")
@Slf4j
public class GetInwardIrnListSectionSuccessHandler implements SuccessHandler {
	...
	try{
		...
		if(APIConstants.INV_TYPE_DEXP.equalsIgnoreCase(dto.getType())){
	        typDxp = APIConstants.INV_TYPE_DXP;
	        dto.setType(APIConstants.INV_TYPE_DXP);
			}
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"GET Call is Success for the batchId {} inside "
							+ "GetInwardIrnListSectionSuccessHandler.java with dto {} ",
					batchId,dto );
		}
		...
		processorType.processJsonResponse(dto.getGstin(), dto.getReturnPeriod(), batchId,
				resultIds, false, dto.getType(), dto);
