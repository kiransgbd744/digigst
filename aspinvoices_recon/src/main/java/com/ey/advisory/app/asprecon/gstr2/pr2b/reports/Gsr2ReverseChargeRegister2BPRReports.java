...
@Slf4j
@Component("Gsr2ReverseChargeRegister2BPRReports")
public class Gsr2ReverseChargeRegister2BPRReports {
	...
	public void getGsr2ReverseChargeRegisterReports(Long configId)
		...
		if (LOGGER.isDebugEnabled()) {
			msg = String.format("Get Reverse Charge Register Report Details "
					+ "with configId:'%s'", configId);
			LOGGER.debug(msg);
		}

		String reportType = "Reverse Charge Register";
		String reportTypeUi = "Reverse_Charge_Register";
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format(
						"Executing chunking proc"
								+ " USP_RECON_2BPR_INS_CHUNK_DATA: '%s'",
						configId.toString());
				LOGGER.debug(msg);
			}
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format("Chunking proc Executed"
						+ " USP_RECON_2BPR_INS_CHUNK_DATA: configId '%d', "
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format("Error while executing chunking "
						+ "USP_RECON_2BPR_INS_CHUNK_DATA proc configId %d",
						configId);
				LOGGER.error(msg, ex);
		...
		if (noOfChunk > 0) {
			...
			try {
				...
				while (j < noOfChunk) {
					...
					StoredProcedureQuery storedProc = entityManager
							.createStoredProcedureQuery(
									"USP_RECON_RPT_2B_RCHRG_REGISTER");
					...
					if (LOGGER.isDebugEnabled()) {
						msg = String.format("call stored proc with "
								...
								+ "reportType is Reverse Charge Register, "
								+ " chunkNo is %d ", configId.toString(), j);
						LOGGER.debug(msg);
					}
