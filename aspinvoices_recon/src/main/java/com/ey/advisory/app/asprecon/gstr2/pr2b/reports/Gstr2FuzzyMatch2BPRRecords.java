...
@Slf4j
@Component("Gstr2FuzzyMatch2BPRRecords")
public class Gstr2FuzzyMatch2BPRRecords {
	...
	public void getFuzzyMatchRecords(Long configId) throws IOException {
		...
		String reportType = getUniqueFileName("Fuzzy Match Records");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_2B_MASTER");
			...
			storedProc.setParameter("P_REPORT_TYPE", "Fuzzy Match");
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility
								.getProp("gstr2.recon.force.match.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp("gstr2.recon.force.match.column")
								.split(",");
						...
						String fuzzyFilePath = null;
						if (tempDir.list().length > 0) {
							zipFileName = combineAndZipCsvFiles
									.zipfolder(configId, tempDir, reportType);

							File zipFile = new File(tempDir, zipFileName);

							fuzzyFilePath = DocumentUtility.uploadZipFile(
									zipFile, "Gstr2ReconReports");

						}
						if (fuzzyFilePath != null) {

							// addlReportRepo.updateReconFilePath(fuzzyFilePath,
							// configId,
							// "Fuzzy_Match_Records");
							...
							commonService.saveReportChunks(chunkDetails.getId(),
									"Fuzzy_Match_Records_1", fuzzyFilePath, true, null);
						}

						if (LOGGER.isDebugEnabled()) {
							String msg = String.format(
									"Update file path for Fuzzy_Match_Records :%s : ",
									fuzzyFilePath);
							LOGGER.debug(msg);
						}
	...
	private Gstr2InitiateReconReportDto convert(Object[] arr) {
		...
		obj.setOrgDocNumberPR(addApostrophe(arr[43]));
		obj.setOrgDocNumber2A(addApostrophe(arr[44]));
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Fuzzy_Match_Records" + "_" + configId;
