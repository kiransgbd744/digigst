...
@Slf4j
@Component("Dropped2A6ARecordsReport")
public class Dropped2A6ARecordsReport {
	...
	public void getGstr2Dropped2A6ARecordsReport(Long configId)
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String
					.format("Get Dropped_2A_6A_Records_Report Recon Report Details "
		...
		tempDir = createTempDir(configId, "Dropped_2A_6A_Records_Report");

		fullPath = tempDir.getAbsolutePath() + File.separator
				+ getUniqueFileName(reportType) + ".csv";
		...
		if (noOfChunk != 0) {
			...
			addlReportRepo.updateAndSave(configId,
					"Dropped 2A_6A Records Report");
			int j = 0;
			...
			try (Writer writer = new BufferedWriter(new FileWriter(fullPath),
				...
				String invoiceHeadersTemplate = commonUtility
						.getProp("gstr2.initiate.recon.report.header.template");
				writer.append(invoiceHeadersTemplate);
				String[] columnMappings = commonUtility
						.getProp("gstr2.initiate.recon.report.column.mapping")
						.split(",");
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"call stored proc with "
										...
										+ "chunksize is %d",
								configId.toString(),
								"Dropped_2A_6A_Records_Report", startId,
								chunkSize);
						LOGGER.debug(msg);
					}
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"Total Time taken to load the Data"
										...
										+ " '%s' - '%s'",
								dbLoadTimeDiff, "Dropped_2A_6A_Records_Report",
								records.size());
						LOGGER.debug(msg);
					}
				...
				LOGGER.error(
						"Exception while executing the query for "
								+ "Report Type :{}",
						"Dropped_2A_6A_Records_Report", ex);
