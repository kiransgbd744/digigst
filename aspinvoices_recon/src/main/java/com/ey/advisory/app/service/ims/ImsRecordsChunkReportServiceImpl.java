...
@Slf4j
@Component("ImsRecordsChunkReportServiceImpl")
public class ImsRecordsChunkReportServiceImpl
	...
	private static final String IMS_REPORT_CHUNK_ROWS = "ims.record.report.chunk.rows";
	private static final String IMS_REPORT_CHUNK_FILES = "ims.record.report.chunk.files";

	@Override
	public Pair<Integer, Integer> getChunkingSizes() {
		Map<String, Config> configMap = configManager.getConfigs(
				"IMS_RECORD_REPORTS",
				"ims.record.report.chunk");
		String fileSize = configMap.get(IMS_REPORT_CHUNK_ROWS) != null
				? configMap.get(IMS_REPORT_CHUNK_ROWS).getValue()
				: "200000";
		String reportSize = configMap.get(IMS_REPORT_CHUNK_FILES) != null
				? configMap.get(IMS_REPORT_CHUNK_FILES).getValue() : "1";
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("IMS Summary  Directory is empty");
			return;
		}

		Set<File> fileNames = new TreeSet<File>();
		File[] files = tempDir.listFiles();
		LOGGER.debug("IMS Summary Directory file size {} ", files.length);
...
//							"IMS Records Report"+"_"+fileIndex);
				...
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"uploadedDocName is : %s ",uploadedDocName);
					LOGGER.debug(msg);
				}
