 ...
 *
 */
@Component("Anx2IsdReviewSummeryImpl")
public class Anx2IsdReviewSummeryImpl implements Anx2reviewSummery {
	...
	public Anx2PrReviewSummeryHeaderDto getreviewSummery(
		...
		if (entityResponse.size() > 0) {
			b2b = entityResponse.stream()
					.filter(p -> "ISD".equalsIgnoreCase(p.getTaxDocType()))
	...
	/*	if (sectionFileList.size() > 0) {
			for (Anx2PRReviewSummeryResponseDto sectionfile : sectionFileList) {

				a2totalRec_Count = a2totalRec_Count + sectionfile.getCount();
				Invoicevalue = Invoicevalue.add(sectionfile.getInvValue());
				a2taxablevalue = a2taxablevalue
						.add(sectionfile.getTaxableValue());
				totalTaxPayable = totalTaxPayable
						.add(sectionfile.getTotalTaxPayable());
				a2totigst = a2totigst.add(sectionfile.getTpIGST());
				a2totcgst = a2totcgst.add(sectionfile.getTpCGST());
				a2totsgst = a2totsgst.add(sectionfile.getTpSGST());
				a2totcess = a2totcess.add(sectionfile.getTpCess());
				totalCreditEligible = totalCreditEligible
						.add(sectionfile.getTotalCreditEligible());
				prtotigst = prtotigst.add(sectionfile.getCeIGST());
				prtotcgst = prtotcgst.add(sectionfile.getCeCGST());
				prtotsgst = prtotsgst.add(sectionfile.getCeSGST());
				prtotcess = prtotcess.add(sectionfile.getCeCess());
			}
		}
*/
