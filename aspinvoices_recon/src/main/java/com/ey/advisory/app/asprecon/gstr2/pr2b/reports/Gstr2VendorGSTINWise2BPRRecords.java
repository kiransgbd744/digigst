 ...
 */
@Slf4j
@Component("Gstr2VendorGSTINWise2BPRRecords")
public class Gstr2VendorGSTINWise2BPRRecords {
	...
	public void getVendorGSTINWiseRecords(Long configId) throws IOException {
		...
		String reportType = getUniqueFileName("Vendor GSTIN Wise Report");
		...
		try {
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility.getProp(
								"gstr2bpr.recon.vendor.gstin.wise.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp("gstr2.recon.vendor.gstin.wise.column")
								.split(",");
				...
				if (tempDir.list().length > 0) {
					...
					/*
					 * vendorGSTINWiseRecordsFilePath = DocumentUtility
				...
				if (uploadedDocName != null) {
					// addlReportRepo.updateReconFilePath(vendorGSTINWiseRecordsFilePath,
					// configId, "Vendor_GSTIN_Wise_Records");
				...
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"Update file path for Vendor_GSTIN_Wise_Records "
									+ ":%s : ",
							uploadedDocName);
					LOGGER.debug(msg);
				}
	...
	private Gstr2VendorGSTINPeriodWiseRecordsDto convert(Object[] arr) {
		...
		if (docType != null && (docType.equalsIgnoreCase("CR")
			...
			obj.setDocNoDocDateMisMatchTaxable(CheckForNegativeValue(arr[94]));
			obj.setDocNoDocDateMisMatchIGST(CheckForNegativeValue(arr[95]));
			obj.setDocNoDocDateMisMatchCGST(CheckForNegativeValue(arr[96]));
			obj.setDocNoDocDateMisMatchSGST(CheckForNegativeValue(arr[97]));
			obj.setDocNoDocDateMisMatchCESS(CheckForNegativeValue(arr[98]));
			obj.setForceGSTR3BMatchTaxable(CheckForNegativeValue(arr[100]));
			obj.setForceGSTR3BMatchIGST(CheckForNegativeValue(arr[101]));
			obj.setForceGSTR3BMatchCGST(CheckForNegativeValue(arr[102]));
			obj.setForceGSTR3BMatchSGST(CheckForNegativeValue(arr[103]));
			obj.setForceGSTR3BMatchCESS(CheckForNegativeValue(arr[104]));
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Vendor_GSTIN_Wise_Records" + "_" + configId;
