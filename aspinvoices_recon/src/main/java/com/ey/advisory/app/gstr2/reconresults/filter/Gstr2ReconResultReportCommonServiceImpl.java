...
@Slf4j
@Component("Gstr2ReconResultReportCommonServiceImpl")
public class Gstr2ReconResultReportCommonServiceImpl
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("2APR recon result Directory is empty");
			return;
		}

		Set<File> fileNames = new TreeSet<File>();
		File[] files = tempDir.listFiles();
		LOGGER.debug("2APR recon result Directory file size {} ", files.length);
		...
		try {
			...
			while (it.hasNext()) {
				...
				String rptType = "Consolidated 2A-6AvsPR Report (Recon Result)" + " " + index;
			...
			for (Pair<Long, List<File>> rptChunks : reportIds) {
				...
				reportTypeRepo.updateFilePathDocId(uploadedFileName,
