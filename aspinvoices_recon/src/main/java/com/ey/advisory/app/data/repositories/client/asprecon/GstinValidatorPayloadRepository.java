 ...
 *
 */
@Repository("GstinValidatorPayloadRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface GstinValidatorPayloadRepository
		extends CrudRepository<GstinValidatorPayloadEntity, Long> {

	@Modifying
	@Query("UPDATE GstinValidatorPayloadEntity op SET op.status=:status,"
	...
	@Query("SELECT op.payloadId FROM GstinValidatorPayloadEntity op "
			+ "WHERE op.payloadId =:payloadId ")
	List<Object> findBypayloadId(@Param("payloadId") String payloadId);

	@Query("SELECT op FROM GstinValidatorPayloadEntity op "
			+ "WHERE op.payloadId =:payloadId ")
	GstinValidatorPayloadEntity getGstinValidatorPayload(
			@Param("payloadId") String payloadId);

	@Query("SELECT op.payloadId,op.createdBy FROM GstinValidatorPayloadEntity op "
			+ "WHERE op.revIntPushStatus=0")
	public List<String[]> findPayloadIdForFailedErpPush();

	@Modifying
	@Query("UPDATE GstinValidatorPayloadEntity SET revIntPushStatus=:revIntPushStatus "
	...
	Optional<GstinValidatorPayloadEntity> findByCloudCheckSumAndStatus(
