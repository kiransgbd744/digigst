...
@Component("Anx2PRSDetailsDeemedExports")
public class Anx2PRSDetailsDeemedExports implements Anx2PRSDetailsSection {
	...
	public Anx2PRSDetailHeaderDto getAnx2PRSummaryDetail(
		...
		if (entityResponse.size() > 0) {
			b2b = entityResponse.stream().filter(
					p -> GSTConstants.DXP.equalsIgnoreCase(p.getTaxDocType()))
		...
		if (entityResponse.size() <= 0 || b2b.size() <= 0) {
			...
			if (docType != null && docType.contains(trimAndConvToUpperCase(GSTConstants.DR))) {
			...
			if (docType != null && docType.contains(trimAndConvToUpperCase(GSTConstants.RDR))) {
		...
		prsDetail.setTable("3G-Deemed Exports");
