...
@Slf4j
@Service(value = "VendorMasterApiMetadataRevIntHandler")
public class VendorMasterApiMetadataRevIntHandler {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(VendorMasterApiMetadataRevIntHandler.class);
	...
	@Autowired
	@Qualifier("VendorMasterApiMetadataRevIntServiceImpl")
	private VendorMasterApiMetadataRevIntService serviceImpl;
	...
	public Integer getPayloadMetadataDetails(
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"VendorMasterApiMetadataRevIntHandler getPayloadMetaDataDetails Begin");
		}
		...
		try {
			...
			VendorMasterApiPayloadMetaDataDto payloadMetaDataDto = serviceImpl
					.payloadErrorInfoMsg(null, payloadId);

			long currentBatchSize = 0;
			if (payloadMetaDataDto != null
					&& payloadMetaDataDto.getVendorDetailDto() != null
					&& payloadMetaDataDto.getVendorDetailDto()
							.getVendorDetailDtoList() != null) {
				currentBatchSize = payloadMetaDataDto.getVendorDetailDto()
						.getVendorDetailDtoList().size();
			...
			if (permEntity != null) {
				AnxErpBatchEntity batchEntity = erpBatchHandler.createErpBatch(
						...
						scenarioId, currentBatchSize,
						"VENDOR_MASTER_API_STATUS",
				...
				responseCode = destinationConn.pushToErp(payloadMetaDataDto,
						"VendorMasterApiPayloadMetaDataDto", batchEntity);
			}
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"VendorMasterApiMetadataRevIntHandler getVendorValidatorMetaPayload End");
		}
		return responseCode;
	}
}
