...
@Slf4j
@Component("Gstr2FuzzyMatchRecords")
public class Gstr2FuzzyMatchRecords {
	...
	public void getFuzzyMatchRecords(Long configId) throws IOException {
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_MASTER");
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						StatefulBeanToCsvBuilder<Gstr2InitiateReconReportDto> 
						 builder = new StatefulBeanToCsvBuilder<>(
								writer);
						StatefulBeanToCsv<Gstr2InitiateReconReportDto> 
						 beanWriter = builder
						...
						if (tempDir.list().length > 0) {
							...
							fuzzyFilePath = DocumentUtility.uploadZipFile(zipFile,
									"Gstr2ReconReports");

						}
						if(fuzzyFilePath != null)
							
						addlReportRepo.updateReconFilePath(fuzzyFilePath, configId,
								"Fuzzy_Match_Records");
