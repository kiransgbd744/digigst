 ...
 *
 */
@Component("InwardEinvoiceFileUploadServiceImpl")
public class InwardEinvoiceFileUploadServiceImpl
		implements InwardEinvoiceFileUploadService {

	public static final Logger LOGGER = LoggerFactory
			.getLogger(InwardEinvoiceFileUploadServiceImpl.class);
	...
	public ResponseEntity<String> uploadDocuments(MultipartFile[] files,
		...
		try {
			...
			if (!anxList.isEmpty() && anxList.size() > 0) {
				{
					for (MultipartFile file : files) {
						...
						if (!file.isEmpty()) {
							...
							fileStatus.setFileType(GSTConstants.INWARD_EINVOICE);
							...
							//dto.setType(GSTConstants.INWARD_EINVOICE);
							dto.setFileId(fileStatus.getId());
							dto.setDocumentId(docId);
							dto.setApiSection(
									GSTConstants.INWARD_EINVOICE.toUpperCase());
							...
							if (uniqueName.equalsIgnoreCase(
									GSTConstants.INWARD_EINVOICE_FILE_NAME)) {
								String jobCategory = JobConstants.INWARDEINVOICEFILEUPLOAD;
					...
					APIRespDto dto = new APIRespDto("Success",
							"Inward E-Invoice file uploaded successfully");
					JsonObject resp = new JsonObject();
					JsonElement respBody = gson.toJsonTree(dto);
					resp.add("hdr",
			...
			APIRespDto dto = new APIRespDto("Failed",
					"Inward E-Invocie file uploaded Failed");
			JsonObject resp = new JsonObject();
			JsonElement respBody = gson.toJsonTree(dto);
			String msg = "Unexpected error while uploading Inward E-Invocie file";
