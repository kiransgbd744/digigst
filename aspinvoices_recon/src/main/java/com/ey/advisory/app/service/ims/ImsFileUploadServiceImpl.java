 ...
 *
 */
@Component("ImsFileUploadServiceImpl")
public class ImsFileUploadServiceImpl
		implements ImsFileUploadService {

	public static final Logger LOGGER = LoggerFactory
			.getLogger(ImsFileUploadServiceImpl.class);
	...
	public ResponseEntity<String> uploadDocuments(MultipartFile[] files,
		...
		LOGGER.info(
				"Inside get Ims file upload method and uniqueName is {}  ",
				uniqueName);
		...
		try {
			...
			if (!anxList.isEmpty() && anxList.size() > 0) {
				{
					for (MultipartFile file : files) {
						...
						if (!file.isEmpty()) {
							...
							fileStatus.setFileType(GSTConstants.IMS);
							...
							if (uniqueName.equalsIgnoreCase(
									GSTConstants.IMS_FILE_NAME)) {
								String jobCategory = JobConstants.IMSFILEUPLOAD;
					...
					APIRespDto dto = new APIRespDto("Success",
							"IMS file uploaded successfully");
					JsonObject resp = new JsonObject();
					JsonElement respBody = gson.toJsonTree(dto);
					resp.add("hdr",
			...
			APIRespDto dto = new APIRespDto("Failed",
					"IMS file uploaded Failed");
			JsonObject resp = new JsonObject();
			JsonElement respBody = gson.toJsonTree(dto);
			String msg = "Unexpected error while uploading IMS file";
