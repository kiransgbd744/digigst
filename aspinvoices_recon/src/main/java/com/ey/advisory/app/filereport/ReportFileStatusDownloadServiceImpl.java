 ...
 */
@Slf4j
@Component("ReportFileStatusDownloadServiceImpl")
public class ReportFileStatusDownloadServiceImpl
	...
	public List<ReportDownloadDto> getDownloadData(Long requestId) {
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Inside ReportFileStatusDownloadServiceImpl for reqId {}",
						requestId);
			}
			Optional<FileStatusDownloadReportEntity> parntEntityList = fileStatusRepo
					.findById(requestId);
			List<ReportDownloadDto> respList = new ArrayList<>();
			if (parntEntityList.isPresent()) {
				FileStatusDownloadReportEntity parntEntity = parntEntityList
						.get();
				if ("REPORT_GENERATED"
						.equalsIgnoreCase(parntEntity.getReportStatus())) {
					if (parntEntity.getFilePath() != null) {
						ReportDownloadDto dto = new ReportDownloadDto();
						dto.setConfigId(requestId);
						dto.setReportCateg(parntEntity.getReportCateg());
						dto.setDwnld(true);
						dto.setReportType(
								getReportType(parntEntity.getReportType(),
										parntEntity.getStatus()));
						respList.add(dto);
					} else {
						List<AsyncFileStatusRptTypeEntity> chdENtiy = requestChildConfigRepo
								.findByReportDwnldIdOrderByIdAsc(requestId);
						for (AsyncFileStatusRptTypeEntity entity : chdENtiy) {
							ReportDownloadDto dto = new ReportDownloadDto();
					...
					dto.setReportCateg(parntEntity.getReportCateg());
					dto.setDwnld(false);
					dto.setReportType(parntEntity.getReportType());
					respList.add(dto);
				}
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Ended ReportFileStatusDownloadServiceImpl for reqId {} with respList size {}",
	...
	private String getReportType(String reportType, String status) {
		...
		try {
			...
			case ReportTypeConstants.INWARD_EINVOICE_DETAILED_REPORT:
				reportType = "Detailed Report (Invoice Level)";
				break;
			...
			case "IMS Records Report":
				reportType = "IMS Records Report";//IMS Records Active+Inactive Report
				break;
			...
			case "IMS Records Active+Inactive Report":
				reportType = "IMS Records Active+Inactive Report";//IMS Records Active+Inactive Report
				break;	
