...
@Slf4j
@Component("Gstr2ForceMatchRecords")
public class Gstr2ForceMatchRecords {
	...
	public void getForceMatchRecords(Long configId) throws IOException {
		...
		String reportType = getUniqueFileName("Force Match");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_FM3B");
			...
			if (tempDir.list().length > 0) {
				...
				fmFilePath = DocumentUtility.uploadZipFile(zipFile,
						"Gstr2ReconReports");

			}
			if (fmFilePath != null)
				addlReportRepo.updateReconFilePath(fmFilePath, configId,
						"Force_Match_Records");

			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Update file path for Force_Match_Records" + " :%s : ",
						fmFilePath);
				LOGGER.debug(msg);
			}
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Force_Match_Records" + "_" + configId;
