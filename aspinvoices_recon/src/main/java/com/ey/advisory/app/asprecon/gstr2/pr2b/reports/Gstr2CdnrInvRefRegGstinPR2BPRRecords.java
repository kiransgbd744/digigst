 ...
 */
@Slf4j
@Component("Gstr2CdnrInvRefRegGstinPR2BPRRecords")
public class Gstr2CdnrInvRefRegGstinPR2BPRRecords {
	...
	public void getCdnrInvRefRegGstinPRRecords(Long configId)
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_2B_CDNRINVPR");
			...
			if (records != null && !records.isEmpty()) {
				...
				if (uploadedDocName != null) {
					// addlReportRepo.updateReconFilePath(cdrPRFilePath,
					// configId,
					// "CRD-INV_Ref_Reg_PR_Records");
					...
					commonService.saveReportChunks(chunkDetails.getId(),
							"CRD-INV_Ref_Reg_PR_Records_1", uploadedDocName.getValue0(),
							true, uploadedDocName.getValue1());
				}
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"Update file path for"
									+ "	 CRD-INV_Ref_Reg_PR_Records :%s : ",
									uploadedDocName);
					LOGGER.debug(msg);
				}
	...
	private Gstr2CdnrInvRefRegGstinPRDto convert(Object[] arr) {
		...
		obj.setRequestIDCR((arr[51] != null) ? (arr[51].toString()) : null);
