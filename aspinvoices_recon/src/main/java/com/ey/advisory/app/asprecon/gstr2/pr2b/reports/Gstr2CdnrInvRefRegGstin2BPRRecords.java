...
@Slf4j
@Component("Gstr2CdnrInvRefRegGstin2BPRRecords")
public class Gstr2CdnrInvRefRegGstin2BPRRecords {
	...
	public void getCdnrInvRefRegGstin2BRecords(Long configId)
			throws IOException {
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_2B_CDNRINV2A");
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility
								.getProp("gstr2bpr.recon.cdnr.ref.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp("gstr2.recon.cdnr.ref.column")
								.split(",");

						ColumnPositionMappingStrategy<Gstr2CdnrInvRefRegDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
						mappingStrategy.setType(Gstr2CdnrInvRefRegDto.class);
						mappingStrategy.setColumnMapping(columnMappings);
						StatefulBeanToCsvBuilder<Gstr2CdnrInvRefRegDto> builder = new StatefulBeanToCsvBuilder<>(
								writer);
						StatefulBeanToCsv<Gstr2CdnrInvRefRegDto> beanWriter = builder
				...
				if (uploadedDocName != null) {
					...
					commonService.saveReportChunks(chunkDetails.getId(),
							"CRD-INV_Ref_Reg_GSTR_2B_Records_1", uploadedDocName.getValue0(),
							true, uploadedDocName.getValue1());

					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"Update file path for "
										+ "CRD-INV_Ref_Reg_GSTR_2B_Records :%s : ",
										uploadedDocName);
						LOGGER.debug(msg);
					}
				}
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "CRD-INV_Ref_Reg_GSTR_2B_Records" + "_"
				+ configId;
