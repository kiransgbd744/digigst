...
@Slf4j
@Component("Gstr2RecipientGSTINPeriodWiseRecords")
public class Gstr2RecipientGSTINPeriodWiseRecords {
	...
	public void getRecipientGSTINPeriodWiseRecords(Long configId)
		...
		String reportType = getUniqueFileName("Recipient GSTIN Tax Period Wise Report");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_RGSTIN_TXPRD");
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String[] columnMappings = commonUtility
								.getProp("gstr2.recon.rgpwr.column")
								.split(",");
				...
				String recipientGPRecordsFilePath = null;
				if (tempDir.list().length > 0) {
					...
					recipientGPRecordsFilePath = DocumentUtility.uploadZipFile(zipFile,
							"Gstr2ReconReports");

				}
				if(recipientGPRecordsFilePath != null)
				addlReportRepo.updateReconFilePath(recipientGPRecordsFilePath, configId,
						"Recipient_GSTIN_Period_Wise_Record");
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"Update file path for Recipient_GSTIN_Period_Wise_Record "
							+ ":%s : ", recipientGPRecordsFilePath);
					LOGGER.debug(msg);
				}
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Recipient_GSTIN_Period_Wise_Record" + "_" + configId;
