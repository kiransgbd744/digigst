...
@Slf4j
@Component("EinvoiceSummaryChunkReportServiceImpl")
public class EinvoiceSummaryChunkReportServiceImpl
		implements EinvoiceSummaryChunkReportService {
	...
	private static final String SUMMARY_REPORT_CHUNK_ROWS = "einvoice.detailed.lineitem.report.chunk.rows";
	private static final String SUMMARY_REPORT_CHUNK_FILES = "einvoice.detailed.lineitem.report.chunk.files";

	@Override
	public Pair<Integer, Integer> getChunkingSizes() {
		Map<String, Config> configMap = configManager.getConfigs(
				"DETAILED_LINEITEM_REPORTS",
				"einvoice.detailed.lineitem.report.chunk");
		String fileSize = configMap.get(SUMMARY_REPORT_CHUNK_ROWS) != null
				? configMap.get(SUMMARY_REPORT_CHUNK_ROWS).getValue()
				: "100000";
		String reportSize = configMap.get(SUMMARY_REPORT_CHUNK_FILES) != null
				? configMap.get(SUMMARY_REPORT_CHUNK_FILES).getValue() : "1";
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
			String reportType, int maxLimitPerZip, int fileIndex) {
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("Einvoice Summary  Directory is empty");
			return;
		}

		Set<File> fileNames = new TreeSet<File>();
		File[] files = tempDir.listFiles();
		LOGGER.debug("Einvoice Summary Directory file size {} ", files.length);
		...
		try {
			...
			while (it.hasNext()) {
				if ("SUMMARY".equalsIgnoreCase(type)) {
					String rptType = "Inward E-invoice_Summary Report" + " " + index;
					Long id = saveReportChunks(reportId, rptType, null, false);
					reportIds.add(new Pair<>(id, it.next()));
				} else {
					String rptType = "Inward E-invoice_Detailed (Invoice Level) Report" + " " + index;
					Long id = saveReportChunks(reportId, rptType, null, false);
					reportIds.add(new Pair<>(id, it.next()));
				}if("IMSSummary".equalsIgnoreCase(type)){
					String rptType = "IMS Summary Report" + " " + index;
					Long id = saveReportChunks(reportId, rptType, null, false);
					reportIds.add(new Pair<>(id, it.next()));
					
				}else if("IMSRecords".equalsIgnoreCase(type)){
					String rptType = "IMS Records Report" + " " + index;
					Long id = saveReportChunks(reportId, rptType, null, false);
					reportIds.add(new Pair<>(id, it.next()));
				}
			}
			...
			for (Pair<Long, List<File>> rptChunks : reportIds) {
				if ("SUMMARY".equalsIgnoreCase(type)) {
					zipFileName = combineAndZipCsvFiles.zipfolder(configId, tempDir,
							"Inward E-invoice_Summary Report"+"_"+fileIndex);
				}else{
					zipFileName = combineAndZipCsvFiles.zipfolder(configId, tempDir,
							"Inward E-invoice_Detailed (Invoice Level) Report"+"_"+fileIndex);
				}if("IMSSummary".equalsIgnoreCase(type)){
					zipFileName = combineAndZipCsvFiles.zipfolder(configId, tempDir,
							"IMS Summary Report"+"_"+fileIndex);
					
				}else if("IMSRecords".equalsIgnoreCase(type)){
					zipFileName = combineAndZipCsvFiles.zipfolder(configId, tempDir,
							"IMS Records Report"+"_"+fileIndex);
				}
				...
				if("IMSSummary".equalsIgnoreCase(type) || "IMSRecords".equalsIgnoreCase(type)){
				
					uploadedZipName = DocumentUtility.uploadFile(zipFile,
							ConfigConstants.ASYNC_IMS_REPORT);
			     }else{
