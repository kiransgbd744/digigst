...
@Slf4j
@Component("Gstr2VendorPANPeriodWise2BPRRecords")
public class Gstr2VendorPANPeriodWise2BPRRecords {
	...
	public void getVendorPANPeriodWiseRecords(Long configId)
			throws IOException {
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_RECON_RPT_2B_VENDOR_PAN_PERIOD");
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility.getProp(
								"gstr2bpr.recon.vendor.pan.period.wise.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp(
										"gstr2.recon.vendor.pan.period.wise.column")
								.split(",");
				...
				if (uploadedDocName != null) {
					...
					commonService.saveReportChunks(chunkDetails.getId(),
							"Vendor_PAN_Period_Wise_Records_1",
							uploadedDocName.getValue0(), true, uploadedDocName.getValue1());
				}
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"Update file path for Vendor_PAN_Period_Wise_Records "
									+ ":%s : ",
									uploadedDocName);
					LOGGER.debug(msg);
				}
	...
	private Gstr2VendorPANPeriodWiseRecordsDto convert(Object[] arr) {
		...
		if (docType != null && (docType.equalsIgnoreCase("CR")
			...
			obj.setForceGSTR3BMatchTaxable(CheckForNegativeValue(arr[95]));
			obj.setForceGSTR3BMatchIGST(CheckForNegativeValue(arr[96]));
			obj.setForceGSTR3BMatchCGST(CheckForNegativeValue(arr[97]));
			obj.setForceGSTR3BMatchSGST(CheckForNegativeValue(arr[98]));
			obj.setForceGSTR3BMatchCESS(CheckForNegativeValue(arr[99]));
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Vendor_PAN_Period_Wise_Records" + "_"
				+ configId;
