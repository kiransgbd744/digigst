...
@Slf4j
@Component("Gsr2ReverseChargeRegisterReports")
public class Gsr2ReverseChargeRegisterReports {

	// Reverse_Charge_Register
	...
	public void getGsr2ReverseChargeRegisterReports(Long configId)
			throws IOException {
		...
		String reportType = getUniqueFileName("Reverse Charge Register");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_RCHRG_REGISTER");
			...
			if (records != null && !records.isEmpty()) {
				...
				String reverseChargeRegisterPath = null;
				if (tempDir.list().length > 0) {
					...
					reverseChargeRegisterPath = DocumentUtility
							.uploadZipFile(zipFile, "Gstr2ReconReports");

				}
				if (reverseChargeRegisterPath != null)
					addlReportRepo.updateReconFilePath(
							reverseChargeRegisterPath, configId,
							"Reverse_Charge_Register");
				if (LOGGER.isDebugEnabled()) {
					String msg = String
							.format("Update file path for Reverse_Charge_Register "
									+ ":%s : ", reverseChargeRegisterPath);
					LOGGER.debug(msg);
				}
			}
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Reverse_Charge_Register" + "_" + configId;
