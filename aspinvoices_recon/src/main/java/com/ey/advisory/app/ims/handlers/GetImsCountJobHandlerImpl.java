...
@Service("GetImsCountJobHandlerImpl")
@Slf4j
public class GetImsCountJobHandlerImpl implements GetImsCountJobHandler {
	...
	@Autowired
	@Qualifier("GetImsCountDataAtGstnImpl")
	private Gstr2aDataAtGstn gstr2aDataAtGstn;

	@Override
	public void getImsCountGetCall(String jsonReq, String groupCode) {
		...
		try {
			...
			if (batch != null) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(" Get IMS COUNT Call for {} is Started.",
							batch.getType());
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Get IMS COUNT Call for {} is Ended.",
							batch.getType());
				}
			} else {
				LOGGER.error(
						"batch.getType() has NULL value for the params {} in IMS COUNT GET Call client {} :",
						jsonReq, groupCode);
				return;
			}
	...
	public Long findInvFromGstn(Gstr1GetInvoicesReqDto dto, String groupCode,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					" IMS {} Get List BatchId {} and framework requestId {} .",
					type, batchId, reqId);
		}
