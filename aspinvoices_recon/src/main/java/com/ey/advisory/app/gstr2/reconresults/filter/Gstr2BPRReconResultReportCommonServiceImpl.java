...
@Slf4j
@Component("Gstr2BPRReconResultReportCommonServiceImpl")
public class Gstr2BPRReconResultReportCommonServiceImpl
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("2BPR recon result Directory is empty");
			return;
		}

		Set<File> fileNames = new TreeSet<File>();
		File[] files = tempDir.listFiles();
		LOGGER.debug("2BPR recon result Directory file size {} ", files.length);
		...
		try {
			...
			while (it.hasNext()) {
				...
				String rptType = "Consolidated 2BvsPR Report (Recon Result)" + " " + index;
