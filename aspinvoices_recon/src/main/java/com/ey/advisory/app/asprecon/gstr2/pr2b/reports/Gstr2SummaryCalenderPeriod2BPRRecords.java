...
import com.ey.advisory.app.asprecon.gstr2.a2pr.reports.Gstr2SummaryCalenderPeriodRecordsDto;
 ...
 */
@Slf4j
@Component("Gstr2SummaryCalenderPeriod2BPRRecords")
public class Gstr2SummaryCalenderPeriod2BPRRecords {
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_RECON_RPT_2B_CALMONTH_SMRY");
			...
			if (records != null && !records.isEmpty()) {
				...
				fullPath = tempDir.getAbsolutePath() + File.separator
						+ reportType + ".csv";
				List<Gstr2SummaryCalenderPeriodRecordsDto> reconDataList = new ArrayList<>();
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility.getProp(
								"gstr2bpr.recon.summary.calender.period.header");
						...
						ColumnPositionMappingStrategy<Gstr2SummaryCalenderPeriodRecordsDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
						mappingStrategy.setType(
								Gstr2SummaryCalenderPeriodRecordsDto.class);
						mappingStrategy.setColumnMapping(columnMappings);
						StatefulBeanToCsvBuilder<Gstr2SummaryCalenderPeriodRecordsDto> builder = new StatefulBeanToCsvBuilder<>(
								writer);
						StatefulBeanToCsv<Gstr2SummaryCalenderPeriodRecordsDto> beanWriter = builder
				...
				if (uploadedDocName != null) {
					...
					commonService.saveReportChunks(chunkDetails.getId(),
							"Summary_CalendarPeriod_Records_1",
							uploadedDocName.getValue0(), true, uploadedDocName.getValue1());
				}
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"Update file path for Summary_CalendarPeriod_Records "
									+ ":%s : ",
									uploadedDocName);
					LOGGER.debug(msg);
				}
	...
	private Gstr2SummaryCalenderPeriodRecordsDto convert(Object[] arr) {
		...
		Gstr2SummaryCalenderPeriodRecordsDto obj = new Gstr2SummaryCalenderPeriodRecordsDto();

		obj.setCalendarPeriod(
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Summary_CalendarPeriod_Records" + "_"
				+ configId;
