...
import com.ey.advisory.app.data.repositories.client.asprecon.Gstr2aprReconImsRespUploadProcRepository;
 ...
 */
@Slf4j
@Component("GSTR2aprImsReconRespUploadServiceImpl")
public class GSTR2aprImsReconRespUploadServiceImpl
	...
	@Autowired
	@Qualifier("Gstr2aprReconImsRespUploadProcRepository")
	Gstr2aprReconImsRespUploadProcRepository gstr2aProcRepo;
	...
	private static final List<String> EXPECTED_HEADERNAMES_LIST = Arrays.asList(
			...
			"TaxPeriod-GSTR 3B", "ResponseRemarks", "MatchingScoreOutof-12",
			"MatchReason", "MismatchReason", "ReportCategory", "ReportType",
			"ERP Report Type", "PreviousReportType(2A)",
			"PreviousReportType(PR)", "TaxPeriod(2A)", "TaxPeriod(2B)",
			"TaxPeriod(PR)", "CalendarMonth", "RecipientGSTIN(2A)",
			"RecipientGSTIN(PR)", "Supplier PAN(2A)", "Supplier PAN(PR)",
			"SupplierGSTIN(2A)", "SupplierGSTIN(PR)", "SupplierLegalName(2A)",
			"SupplierTradeName(2A)", "SupplierName(PR)", "DocType(2A)",
			"DocType(PR)", "DocumentNumber(2A)", "DocumentNumber(PR)",
			"DocumentDate(2A)", "DocumentDate(PR)", "POS(2A)", "POS(PR)",
			"TaxableValue(2A)", "TaxableValue(PR)", "IGST(2A)", "IGST(PR)",
			"CGST(2A)", "CGST(PR)", "SGST(2A)", "SGST(PR)", "Cess(2A)",
			"Cess(PR)", "TotalTax(2A)", "TotalTax(PR)", "InvoiceValue(2A)",
			...
			"VendorComplianceTrend", "2B-Generation Date", "2A-Generation Date",
			"OrgInvAmendmentPeriod", "OrgAmendmentType", "CDN-DelinkingFlag",
			...
			"UserDefinedField28", "IMS UniqueID", "EWBNumber", "EWBDate",
			"MatchingID", "RequestID", "IDPR", "ID2A", "InvoiceKeyPR",
			"InvoiceKey2A", "ReferenceIDPR", "ReferenceID2A");
	...
	public void validateResponse(Long fileId, String fileName,
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = "Validating GSTR2APR IMS Recon Response file";
			LOGGER.debug(msg);
		}
		try {
	...
	private void validation(List<GSTR2AAutoReconRespUploadDTO> dtoList,
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = " Inside GSTR2aprImsReconRespUploadServiceImpl "
					+ "Validation method ";
			LOGGER.debug(msg);
		}
		try {
			...
			for (GSTR2AAutoReconRespUploadDTO rowData : dtoList) {
				...
				boolean isGstr2AprValidationReq = false;
				boolean isImsValidationReq = false;

				if (!Strings.isNullOrEmpty(rowData.getUserIMSResponse())) {
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(" IMS user response value {} ",
								rowData.getUserIMSResponse());
					}
				...
				strucuralValidation.rowDataValidation(validationResult, rowData,
						activeDocNo2A, activeDocNoPR, imsUniqueIdList,
						isImsValidationReq, isGstr2AprValidationReq);
				...
				if (isGstr2AprValidationReq) {
					if (validationResult.isEmpty()) {
						errorMessage = checkForDuplicates(rowData,
								invoiceKey2ASet, invoiceKeyPRSet,
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Getting proc list from TBL_2APR_RECON_RESP_IMS_UPLOAD for, BatchId {}",
						Long.valueOf(batchId));
			}
			String failurePocName = "USP_RECON_RESP_IMS_PSD_FAILURE";
			try {
					...
					try {
						...
						if (!(ReconStatusConstants.SUCCESS
							...
							String msg = String.format(
									"GSTR2APR+ IMS Batch Id is '%s', Response "
											+ "from PROC %s did not "
											+ "return success,"
						...
						String msg = String.format(
								"Calling GSTR2APR+IMS failure Proc '%s' for Batch Id is '%s'",
				...
				LOGGER.debug("GSTR2APR + IMS File is Processed for fileId: {}",
						fileId);

			} catch (Exception e) {
				String msg = String.format(
						"Exception Occure while fetching Proc List for GSTR2APR + IMS from TBL :'%s'",
						failurePocName);
				LOGGER.error(msg, e);
				fileStatusRepository.updateFileStatus(fileId, "Failed");
			...
			String msg = "Exception Occure while validating GSTR2APR IMS resp file";
			LOGGER.error(msg, ex);
			throw new AppException(msg, ex);
		}
	}
	...
	private String executeGstr2aProcedure(String procName, Long batchId)
		...
		try {
			...
			LOGGER.debug(msg);
			LOGGER.error(msg, e);
			if ("USP_RECON_RESP_IMS_PSD_FAILURE".equalsIgnoreCase(procName)) {
				failedBatAltUtility.prepareAndTriggerAlert(
						String.valueOf(batchId), "RECON_RESPONSE",
	...
	private Map<Integer, String> fetchStoredGSTR2aProcedures() {
		...
		try {
			...
			List<Gstr2aprReconImsRespUploadProcEntity> gstr2aProcList = gstr2aProcRepo
					.findAllActiveProc();
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Getting alls GSTR 2APR IMS recon response upload proc list '%s'",
						gstr2aProcMap.toString());
				LOGGER.debug(msg);
			}
		} catch (Exception ex) {
			String msg = String
					.format("Exception while fetching GSTR 2APR IMS recon "
					...
					DateFormatForStructuralValidatons.parseObjToDate(
							rowData.getBillOfEntryDate2A()) != null
									? DateFormatForStructuralValidatons
											.parseObjToDate(rowData
													.getBillOfEntryDate2A())
											.toString()
	...
	private InputStream getFileInpStream(String fileName, String folderName,
		...
		try {
			...
			LOGGER.error("Exception occured in reading File "
					+ "GSTR2aprImsReconRespUploadServiceImpl", e);
	...
	private GSTR2AAutoReconRespUploadDTO convertRowsToDto(Object[] arr,
		...
		obj.setUserIMSResponse((arr[0] != null) ? arr[0].toString() : null);
		obj.setImsResponseRemarks((arr[1] != null) ? arr[1].toString() : null);
		...
		obj.setActionDigiGST((arr[5] != null) ? arr[5].toString() : null);
		obj.setActionDigiGSTDateTime(
				(arr[6] != null) ? arr[6].toString() : null);
		obj.setSavedtoGSTN((arr[7] != null) ? arr[7].toString() : null);
		obj.setActiveinIMSGSTN((arr[8] != null) ? arr[8].toString() : null);
		...
		obj.setErpReportType((arr[18] != null) ? arr[18].toString() : null);
		obj.setPreviousReportType2A(
				(arr[19] != null) ? arr[19].toString() : null);
		obj.setPreviousReportTypePR(
				(arr[20] != null) ? arr[20].toString() : null);
		obj.setTaxPeriod2A((arr[21] != null) ? arr[21].toString() : null);
		obj.setTaxPeriod2B((arr[22] != null) ? arr[22].toString() : null);
		obj.setTaxPeriodPR((arr[23] != null) ? arr[23].toString() : null);
		obj.setCalendarMonth((arr[24] != null) ? arr[24].toString() : null);
		obj.setRecipientGstin2A((arr[25] != null) ? arr[25].toString() : null);
		obj.setRecipientGstinPR((arr[26] != null) ? arr[26].toString() : null);
		obj.setSupplierPan2A((arr[27] != null) ? arr[27].toString() : null);
		obj.setSupplierPanPR((arr[28] != null) ? arr[28].toString() : null);
		obj.setSupplierGstin2A((arr[29] != null) ? arr[29].toString() : null);
		obj.setSupplierGstinPR((arr[30] != null) ? arr[30].toString() : null);
		obj.setSupplierLegalName2A(
				(arr[31] != null) ? arr[31].toString() : null);
		obj.setSupplierTradeName2A(
				(arr[32] != null) ? arr[32].toString() : null);
		obj.setSupplierNamePR((arr[33] != null) ? arr[33].toString() : null);
		obj.setDocType2A((arr[34] != null) ? arr[34].toString() : null);
		obj.setDocTypePR((arr[35] != null) ? arr[35].toString() : null);
		obj.setDocumentNumber2A((arr[36] != null) ? arr[36].toString() : null);
		obj.setDocumentNumberPR((arr[37] != null) ? arr[37].toString() : null);
		obj.setDocumentDate2A((arr[38] != null) ? arr[38].toString() : null);
		obj.setDocumentDatePR((arr[39] != null) ? arr[39].toString() : null);
		obj.setPos2A((arr[40] != null) ? arr[40].toString() : null);
		...
		obj.setTaxableValuePR(
				(arr[43] != null) ? checkForNegative(arr[43].toString())
						: null);
		obj.setIgst2A((arr[44] != null) ? checkForNegative(arr[44].toString())
				: null);
		obj.setIgstPR((arr[45] != null) ? checkForNegative(arr[45].toString())
				: null);
		obj.setCgst2A((arr[46] != null) ? checkForNegative(arr[46].toString())
				: null);
		obj.setCgstPR((arr[47] != null) ? checkForNegative(arr[47].toString())
				: null);
		obj.setSgst2A((arr[48] != null) ? checkForNegative(arr[48].toString())
				: null);
		obj.setSgstPR((arr[49] != null) ? checkForNegative(arr[49].toString())
				: null);
		obj.setCess2A((arr[50] != null) ? checkForNegative(arr[50].toString())
				: null);
		obj.setCessPR((arr[51] != null) ? checkForNegative(arr[51].toString())
				: null);
		obj.setTotalTax2A(
				(arr[52] != null) ? checkForNegative(arr[52].toString())
						: null);
		obj.setTotalTaxPR(
				(arr[53] != null) ? checkForNegative(arr[53].toString())
						: null);
		obj.setInvoiceValue2A(
				(arr[54] != null) ? checkForNegative(arr[54].toString())
						: null);
		obj.setInvoiceValuePR(
				(arr[55] != null) ? checkForNegative(arr[55].toString())
						: null);
		...
		obj.setAvailableIGST(
				(arr[59] != null) ? checkForNegative(arr[59].toString())
						: null);
		obj.setAvailableCGST(
				(arr[60] != null) ? checkForNegative(arr[60].toString())
						: null);
		obj.setAvailableSGST(
				(arr[61] != null) ? checkForNegative(arr[61].toString())
						: null);
		obj.setAvailableCESS(
				(arr[62] != null) ? checkForNegative(arr[62].toString())
						: null);

		obj.setItcAvailability2B((arr[63] != null) ? arr[63].toString() : null);
		...
		obj.setGSTR1FilingStatus((arr[66] != null) ? arr[66].toString() : null);
		obj.setGSTR1FilingDate(
				(arr[67] != null) ? removeQuotes(arr[67].toString()) : null);
		obj.setGSTR1FilingPeriod((arr[68] != null) ? arr[68].toString() : null);
		obj.setGSTR3BFilingStatus(
				(arr[69] != null) ? arr[69].toString() : null);
		obj.setGSTR3BFilingDate(
				(arr[70] != null) ? removeQuotes(arr[70].toString()) : null);
		...
		obj.setOrgDocDate2A(
				(arr[83] != null) ? removeQuotes(arr[83].toString()) : null);
		obj.setOrgDocDatePR(
				(arr[84] != null) ? removeQuotes(arr[84].toString()) : null);
		obj.setOrgSupplierGstinPR(
				(arr[85] != null) ? arr[85].toString() : null);
		obj.setOrgSupplierNamePR((arr[86] != null) ? arr[86].toString() : null);
		obj.setCRDRPreGST2A((arr[87] != null) ? arr[87].toString() : null);
		obj.setCRDRPreGSTPR((arr[88] != null) ? arr[88].toString() : null);
		obj.setBoeReferenceDate(
				(arr[89] != null) ? removeQuotes(arr[89].toString()) : null);
		obj.setPortCode2A((arr[90] != null) ? arr[90].toString() : null);
		obj.setPortCodePR((arr[91] != null) ? arr[91].toString() : null);
		obj.setBillOfEntry2A((arr[92] != null) ? arr[92].toString() : null);
		obj.setBillOfEntryPR((arr[93] != null) ? arr[93].toString() : null);
		obj.setBillOfEntryDate2A(
				(arr[94] != null) ? removeQuotes(arr[94].toString()) : null);
		obj.setBillOfEntryDatePR(
				(arr[95] != null) ? removeQuotes(arr[95].toString()) : null);
		obj.setBoeAmended((arr[96] != null) ? arr[96].toString() : null);
		obj.setTableType2A((arr[97] != null) ? arr[97].toString() : null);
		obj.setSupplyType2A((arr[98] != null) ? arr[98].toString() : null);
		obj.setSupplyTypePR((arr[99] != null) ? arr[99].toString() : null);
		obj.setUserID((arr[100] != null) ? arr[100].toString() : null);
		obj.setSourceIdentifier(
				(arr[101] != null) ? arr[101].toString() : null);
		obj.setSourceFileName((arr[102] != null) ? arr[102].toString() : null);
		obj.setCompanyCode((arr[103] != null) ? arr[103].toString() : null);
		...
		obj.setClaimRefundFlag((arr[141] != null) ? arr[141].toString() : null);
		...
		obj.setITCEntitlement((arr[146] != null) ? arr[146].toString() : null);
		...
		obj.setAccountingVoucherDate(
				(arr[150] != null) ? removeQuotes(arr[150].toString()) : null);
		obj.setGlPostingDate((arr[151] != null) ? arr[151].toString() : null);
		...
		obj.setVendorType((arr[155] != null) ? arr[155].toString() : null);
		...
		obj.setVendorRemarks((arr[158] != null) ? arr[158].toString() : null);
		obj.setApprovalStatus((arr[159] != null) ? arr[159].toString() : null);
		obj.setRecordStatus((arr[160] != null) ? arr[160].toString() : null);
		obj.setKeyDescription((arr[161] != null) ? arr[161].toString() : null);
		obj.setReconGeneratedDate(
				(arr[162] != null) ? removeQuotes(arr[162].toString()) : null);
		obj.setReverseIntegratedDate(
				(arr[163] != null) ? removeQuotes(arr[163].toString()) : null);
		obj.setSourceType((arr[164] != null) ? arr[164].toString() : null);
		obj.setEInvoiceApplicability(
				(arr[165] != null) ? arr[165].toString() : null);
		obj.setIrn2A((arr[166] != null) ? arr[166].toString() : null);
		obj.setIrnPR((arr[167] != null) ? arr[167].toString() : null);
		obj.setIrnDate2A(
				(arr[168] != null) ? removeQuotes(arr[168].toString()) : null);
		obj.setIrnDatePR(
				(arr[169] != null) ? removeQuotes(arr[169].toString()) : null);
		obj.setQrCodeCheck((arr[170] != null) ? arr[170].toString() : null);
		...
		obj.setImsUniqueID((arr[191] != null) ? arr[191].toString() : null);
		...
		obj.setInvoiceKeyPR((arr[198] != null) ? arr[198].toString() : null);
		obj.setInvoiceKeyA2((arr[199] != null) ? arr[199].toString() : null);
	...
	private List<Gstr2ReconResponseUploadEntity> makeEntityList(
		...
		entity.setOrgDocDate2A(
				checkNullAndTrim(DateFormatForStructuralValidatons
						.parseObjToDate(dto.getOrgDocDate2A()) != null
								? DateFormatForStructuralValidatons
										.parseObjToDate(dto.getOrgDocDate2A())
										.toString()
								: dto.getOrgDocDate2A(),
						true, 500));
		entity.setOrgDocDatePR(
				checkNullAndTrim(DateFormatForStructuralValidatons
						.parseObjToDate(dto.getOrgDocDatePR()) != null
								? DateFormatForStructuralValidatons
										.parseObjToDate(dto.getOrgDocDatePR())
										.toString()
								: dto.getOrgDocDatePR(),
						true, 500));
		...
		entity.setBillOfEntryDate2A(
				checkNullAndTrim(
						DateFormatForStructuralValidatons.parseObjToDate(
								dto.getBillOfEntryDate2A()) != null
										? DateFormatForStructuralValidatons
												.parseObjToDate(dto
														.getBillOfEntryDate2A())
												.toString()
						...
						true, 500));
		entity.setBillOFEntryDatePR(
				checkNullAndTrim(
						DateFormatForStructuralValidatons.parseObjToDate(
								dto.getBillOfEntryDatePR()) != null
										? DateFormatForStructuralValidatons
												.parseObjToDate(dto
														.getBillOfEntryDatePR())
												.toString()
		...
		entity.setGlPostingDate(
				checkNullAndTrim(DateFormatForStructuralValidatons
						.parseObjToDate(dto.getGlPostingDate()) != null
								? DateFormatForStructuralValidatons
										.parseObjToDate(dto.getGlPostingDate())
										.toString()
								: dto.getGlPostingDate(),
						true, 500));
		entity.setCustomerPORefNumber(
				checkNullAndTrim(dto.getCustomerPORefNumber(), true, 500));
		entity.setCustomerPORefDate(
				checkNullAndTrim(
						DateFormatForStructuralValidatons.parseObjToDate(
								dto.getCustomerPORefDate()) != null
										? DateFormatForStructuralValidatons
												.parseObjToDate(dto
														.getCustomerPORefDate())
												.toString()
		...
		entity.setIrnDate2A(checkNullAndTrim(DateFormatForStructuralValidatons
				.parseObjToDate(dto.getIrnDate2A()) != null
						? DateFormatForStructuralValidatons
								.parseObjToDate(dto.getIrnDate2A()).toString()
						: dto.getIrnDate2A(),
				true, 500));
		entity.setIrnDatePR(checkNullAndTrim(DateFormatForStructuralValidatons
				.parseObjToDate(dto.getIrnDatePR()) != null
						? DateFormatForStructuralValidatons
								.parseObjToDate(dto.getIrnDatePR()).toString()
						: dto.getIrnDatePR(),
				true, 500));
		...
		entity.setEwbDate(checkNullAndTrim(DateFormatForStructuralValidatons
				.parseObjToDate(dto.getEWayBillDate()) != null
						? DateFormatForStructuralValidatons
								.parseObjToDate(dto.getEWayBillDate())
								.toString()
						: dto.getEWayBillDate(),
				true, 500));
		...
		entity.setImsActionDigi(
				checkNullAndTrim(dto.getActionDigiGST(), true, 500));
		entity.setImsActionDigiDateTime(
				checkNullAndTrim(dto.getActionDigiGSTDateTime(), true, 50));
		entity.setImsActionGstn(
				checkNullAndTrim(dto.getActionGSTN(), true, 500));
		entity.setImsGetCallDateTime(
				checkNullAndTrim(dto.getImsGetCallDateTime(), true, 50));
		entity.setImsIsActive(
				checkNullAndTrim(dto.getActiveinIMSGSTN(), true, 5));
		entity.setImsPendingActionBlocked(
				checkNullAndTrim(dto.getPendingActionBlocked(), true, 500));
		entity.setImsResponseRemark(
				checkNullAndTrim(dto.getImsResponseRemarks(), true, 500));
		entity.setImsSavedToGstn(
				checkNullAndTrim(dto.getSavedtoGSTN(), true, 5));
		entity.setImsUniqueId(
				checkNullAndTrim(dto.getImsUniqueID(), true, 500));
		entity.setImsUserResponse(
				checkNullAndTrim(dto.getUserIMSResponse(), true, 500));
