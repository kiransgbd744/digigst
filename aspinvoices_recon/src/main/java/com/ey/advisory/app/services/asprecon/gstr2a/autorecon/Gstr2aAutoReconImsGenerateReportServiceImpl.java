 ...
 */
@Slf4j
@Component("Gstr2aAutoReconImsGenerateReportServiceImpl")
public class Gstr2aAutoReconImsGenerateReportServiceImpl
		implements Gstr2aGenerateReportService {
	...
	@Autowired
	private Gstr2aGenerateReportCommonService commonService;
	...
	@Override
	public void generateReport(Long requestId) throws Exception {
		...
		tempDir = createTempDir(requestId);
		
		autoReconRequestRepo.updateStatus(
				ReportStatusConstants.REPORT_GENERATION_INPROGRESS,
				LocalDateTime.now(), requestId, null, null);

		fullPath = tempDir.getAbsolutePath() + File.separator + requestId
				+ "_AIM_ReconReport_1.csv";

		StoredProcedureQuery storedProc = entityManager
				.createStoredProcedureQuery("USP_AUTO_2APR_INS_CHUNK_CLOUD_IMS");

		storedProc.registerStoredProcedureParameter("P_REQUEST_ID", Long.class,
				ParameterMode.IN);

		storedProc.setParameter("P_REQUEST_ID", requestId);

		@SuppressWarnings("unchecked")
		Integer chunksize = (Integer) storedProc.getSingleResult();
		
		if (LOGGER.isDebugEnabled()) {
			msg = String.format(" executed USP_AUTO_2APR_INS_CHUNK_CLOUD - "
		...
		if (chunksize != 0) {
			...
			try {
				...
				String invoiceHeadersTemplate = commonUtility
						.getProp("gstr2.generate.recon.2apr.autorecon.ims.report.header.template");
				writer.append(invoiceHeadersTemplate);
				String[] columnMappings = commonUtility
						.getProp("gstr2.generate.recon.2apr.autorecon.ims.report.report.column.mapping")
						.split(",");
				...
				while (j < noOfChunk) {
					...
					StoredProcedureQuery dispProc = entityManager
							.createStoredProcedureQuery(
									"USP_AUTO_2APR_DISP_CHUNK_CLOUD_IMS");

					dispProc.registerStoredProcedureParameter("P_REQUEST_ID",
							Long.class, ParameterMode.IN);

					dispProc.setParameter("P_REQUEST_ID", requestId);
					...
					if (records != null && !records.isEmpty()) {
						...
						if (count >= maxLimitPerFile) {
							...
							if (LOGGER.isDebugEnabled()) {
								LOGGER.debug(
										"Invoking Gstr2aGenerateReportCommonService to"
												+ " upload a zip file : "
							...
							if (isReportAvailable) {
								...
								commonService.chunkZipFiles(tempDir, requestId,
										requestId,
										requestId + "_AIM_ReconReport", 1);
							}
						...
						if (noOfChunk == 1 || j == noOfChunk) {
							flushWriter(writer);
							commonService.chunkZipFiles(tempDir, requestId,
									requestId, requestId + "_AIM_ReconReport",
									1);
							break;
						}
			...
			autoReconRequestRepo.updateStatus(
					ReportStatusConstants.NO_DATA_FOUND, LocalDateTime.now(),
					requestId, null);
		}
	...
	private Gstr2InitiateReconReportDto convert(Object[] arr) {
		...
		obj.setSuggestedFMResponse(
				(arr[0] != null)
						? (StringUtils.isNumeric(arr[0].toString())
		...
		obj.setGSTR1FilingDate((arr[60] != null) ? arr[60].toString() : null);
		...
		/*
		 * obj.setProfitCentre2((arr[94] != null) ? arr[94].toString() : null);
		 * obj.setProfitCentre3((arr[95] != null) ? arr[95].toString() : null);
		 * obj.setProfitCentre4((arr[96] != null) ? arr[96].toString() : null);
		 * obj.setProfitCentre5((arr[97] != null) ? arr[97].toString() : null);
		 * obj.setProfitCentre6((arr[98] != null) ? arr[98].toString() : null);
		 * obj.setProfitCentre7((arr[99] != null) ? arr[99].toString() : null);
		 */
		...
		//Ims auto recon 2A vs PR IMS columns
		obj.setUserIMSResponse((arr[202] != null) ? arr[202].toString() : null);
		obj.setImsResponseRemarks(
	...
	private static File createTempDir(Long requestId) throws IOException {

		String tempFolderPrefix = "Gstr2a" + "_" + requestId + "_"
