...
@Slf4j
@Component("Gstr2LogicalMatchReport")
public class Gstr2LogicalMatchReport {
	...
	public void getLogicalMatchReport(Long configId) throws IOException {
		...
		if (LOGGER.isDebugEnabled()) {
			msg = String.format(
					"Get Recon LogicalMatchReport Details with configId:'%s'",
					configId.toString());
			LOGGER.debug(msg);
		}
		...
		if (LOGGER.isDebugEnabled()) {
			msg = String.format(
					"Update file path for Logical Match Records " + ":%s : ",
					uploadedDocName);
			LOGGER.debug(msg);
		}
