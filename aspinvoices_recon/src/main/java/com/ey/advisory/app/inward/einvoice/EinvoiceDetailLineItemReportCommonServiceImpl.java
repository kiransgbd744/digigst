...
@Slf4j
@Component("EinvoiceDetailLineItemReportCommonServiceImpl")
public class EinvoiceDetailLineItemReportCommonServiceImpl
		implements Gstr2ReconResultReportCommonService {

	@Autowired
	private AsyncFileStatusReportTypeRepository reportTypeRepo1;
	...
	private static final String DETAILED_REPORT_CHUNK_ROWS = 
			"einvoice.detailed.lineitem.report.chunk.rows";
	private static final String DETAILED_REPORT_CHUNK_FILES = 
			"einvoice.detailed.lineitem.report.chunk.files";

	@Override
	public Pair<Integer, Integer> getChunkingSizes() {
		Map<String, Config> configMap = configManager
				.getConfigs("DETAILED_LINEITEM_REPORTS", "einvoice.detailed.lineitem.report.chunk");
		String fileSize = configMap.get(DETAILED_REPORT_CHUNK_ROWS) != null
				? configMap.get(DETAILED_REPORT_CHUNK_ROWS).getValue()
				: "100000";
		String reportSize = configMap.get(DETAILED_REPORT_CHUNK_FILES) != null
				? configMap.get(DETAILED_REPORT_CHUNK_FILES).getValue()
				: "1";
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("Detailed Line Item Directory is empty");
			return;
		}

		Set<File> fileNames = new TreeSet<File>();
		File[] files = tempDir.listFiles();
		LOGGER.debug("Detailed Line Item Directory file size {} ", files.length);
		...
		try {
			...
			while (it.hasNext()) {
				...
				String rptType = "Inward E-invoice_Detailed (Line Item) Report" + " " + index;
			...
			for (Pair<Long, List<File>> rptChunks : reportIds) {
				...
				reportTypeRepo1.updateFilePath(uploadedDocName,
