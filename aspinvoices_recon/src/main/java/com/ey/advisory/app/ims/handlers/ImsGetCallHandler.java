...
@Service("ImsGetCallHandler")
@Slf4j
public class ImsGetCallHandler {
	...
	public Map<String, String> getCallImsList(List<Gstr1GetInvoicesReqDto> dtos,
			List<AsyncExecJob> jobList,
			Map<String, String> getCallCountfailedgstin) {
		...
		try {
			...
			for (Gstr1GetInvoicesReqDto dto : dtos) {
				...
				if (getCallCountfailedgstin.containsKey(gstin)) {
					...
					/*
					 * if (regDate != null) { // need to pass the current year
					 * and current month
					 * 
					 * YearMonth thisYearMonth = YearMonth.now(); // Last day of
					 * the taxperiod
					 * 
					 * LocalDate lastDayOfTaxPeriod =
					 * thisYearMonth.atEndOfMonth(); // TaxPeriod is greater
					 * than GSTIN registration Date as // per // onboarding. if
					 * (lastDayOfTaxPeriod.compareTo(regDate) >= 0) { if
					 ...
					 * APIRespDto.creatErrorResp(); } JsonObject json = new
					 * JsonObject(); msg =
					 * "Registation date less than ReturnPeriod, as per Onboarding"
					 * ; json.addProperty("gstin", gstin);
					 * json.addProperty("msg", msg); respBody.add(json);
					 * LOGGER.error(msg + " for {} ", gstin); continue; }
					 * 
					 * } else {
					 * 
					 * if (apiResp == null) { apiResp =
					...
					if (isFailedGet) {
						...
						if (LOGGER.isDebugEnabled()) {
							LOGGER.debug(
									" Get IMS List All GET Request received");
						}
					...
					for (String suplyTyplist : GETIMS_SUPPLY_TYPES) {
					...
					if (isAllsectionsGetInProgress) {
						...
						getCallCountfailedgstin.put(dto.getGstin(),
								"IMS - Get Invoice is Already Inprogress, New Request can't be taken");
						/*
			...
			return getCallCountfailedgstin;
