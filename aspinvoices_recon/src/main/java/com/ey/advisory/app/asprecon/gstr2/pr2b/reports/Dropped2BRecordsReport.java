...
@Slf4j
@Component("Dropped2BRecordsReport")
public class Dropped2BRecordsReport {
	...
	public void getGstr2Dropped2A6ARecordsReport(Long configId)
			throws IOException {
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Get Dropped 2B Records Report Recon Report Details "
							+ "with configId:'%s'",
					configId);
			LOGGER.debug(msg);
		}

		String reportType = "Dropped 2B Records Report";

		tempDir = createTempDir(configId, "Dropped 2B Records Report");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_RECON_2BPR_DROP_2B_INS_CHUNK_DATA");
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Executing chunking proc"
								+ " USP_RECON_2BPR_DROP_2B_INS_CHUNK_DATA: '%s'",
						configId.toString());
				LOGGER.debug(msg);
			}
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format("Chunking proc Executed"
						+ " USP_RECON_2BPR_DROP_2B_INS_CHUNK_DATA: configId '%d', "
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format("Error while executing chunking "
						+ "USP_RECON_2BPR_DROP_2B_INS_CHUNK_DATA proc configId %d",
						configId);
				LOGGER.error(msg, ex);
		...
		if (noOfChunk != 0) {
			...
			if (LOGGER.isDebugEnabled()) {
				String logMsg = String.format("Get Recon Report Details with "
						+ "configId:'%s' Before proc call " + " chunkSize %d,"
						+ " Dropped 2B Records Report ", configId.toString(),
						chunkSize);
				LOGGER.debug(logMsg);
			}
			...
			try {
				...
				String invoiceHeadersTemplate = commonUtility
						.getProp("gstr2bpr.dropout.pr.report.header");
				writer.append(invoiceHeadersTemplate);
				String[] columnMappings = commonUtility
						.getProp("gstr2bpr.dropout.pr.report.column").split(",");
				...
				while (j < noOfChunk) {
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"Total Time taken to load the Data"
										...
										+ " '%s' - '%s'",
								dbLoadTimeDiff, "Dropped 2B Records Report",
								records.size());
						LOGGER.debug(msg);
					}
					...
					if (records != null && !records.isEmpty()) {
						...
						List<Gstr2Recon2BPRReportDto> reconDataList =
								new ArrayList<>();
