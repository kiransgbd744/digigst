...
import com.ey.advisory.app.asprecon.gstr2.reconresponse.upload.GSTR2AAutoReconErrInfoDTO;
 ...
 */
@Slf4j
@Component("GSTR2aprImsPsdErrInfoReportDwnldServiceImpl")
public class GSTR2aprImsPsdErrInfoReportDwnldServiceImpl implements GSTR2aprImsPsdErrInfoReportDwnldService {
	...
	public Pair<String,String> getErrorData(Long batchId) {
		...
		try {
			...
			String errFileName = "ReconResponse_2APR+IMS_Error_" + batchId;
			...
			String invoiceHeadersTemplate = commonUtility
					.getProp("gstr2.recon.2apr.ims.download.report.error.headers");

			String[] columnMappings = commonUtility
					.getProp("gstr2.recon.2apr.ims.download.report.error.column")
					.split(",");
			...
			while (j < noOfChunk) {
				...
				List<Object[]> records = getData(
						"USP_RECON_DISP_CHUNK_RPT_FM3B_IMS_ERR", batchId, j);
...
*/
	...
	public Pair<String,String> getPsdData(Long batchId) {
		...
		try {
			String psdFile = "ReconResponse_2APR+IMS_Processed_" + batchId;
			...
			String invoiceHeadersTemplate = commonUtility
					.getProp("gstr2.recon.2apr.ims.download.report.psd.headers");
			String[] columnMappings = commonUtility
					.getProp("gstr2.recon.2apr.ims.download.report.psd.column").split(",");
			...
			while (j < noOfChunk) {
				...
				List<Object[]> records = getData(
						"USP_RECON_DISP_CHUNK_RPT_FM3B_IMS_PSD", batchId, j);
	...
	private GSTR2AAutoReconErrInfoDTO convertRowsToDto(Object[] arr,
		...
		if (isError) {
			obj.setErrorId((arr[212] != null) ? arr[212].toString() : null);
