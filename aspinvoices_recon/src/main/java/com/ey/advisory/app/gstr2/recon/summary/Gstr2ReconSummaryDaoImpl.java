...
@Slf4j
@Component("Gstr2ReconSummaryDaoImpl")
public class Gstr2ReconSummaryDaoImpl implements Gstr2ReconSummaryDao {
	...
	public List<Gstr2ReconSummaryDto> findReconSummary(Long configId,
		...
		try {
			...
			if (returnPeriod != null && !returnPeriod.isEmpty()) {
				returnPer = String.join(",", returnPeriod);
			} else {
				returnPer = "";
			}
			...
			if("2BPR".equalsIgnoreCase(reconType)){
				...
				LOGGER.info(
						"Inside Gstr2ReconSummaryDaoImpl.findReconSummary():: "
							+ "Invoking USP_2BPR_RECON_SUMMARY Stored Proc");
				 storedProc = entityManager
						.createStoredProcedureQuery("USP_2BPR_RECON_SUMMARY");
				...
				LOGGER.info(
						"Inside Gstr2ReconSummaryDaoImpl.findReconSummary():: "
							+ "Invoking USP_EINVPR_RECON_SUMMARY Stored Proc");
				 storedProc = entityManager
						.createStoredProcedureQuery("USP_EINVPR_RECON_SUMMARY");

			}else{
				LOGGER.info(
						"Inside Gstr2ReconSummaryDaoImpl.findReconSummary():: "
							+ "Invoking USP_2APR_RECON_SUMMARY Stored Proc");
				 storedProc = entityManager
						.createStoredProcedureQuery("USP_2APR_RECON_SUMMARY");
			...
			storedProc.setParameter("P_RET_PERIOD", returnPer);
			...
			String msg = String.format(
					"Error while calling store proc "
							+ "USP_2APR_RECON_SUMMARY :: configId %s ",
					configId.toString());
			LOGGER.error(msg,e);
	...
	private List<Gstr2ReconSummaryDto> convertScreenLevelData(
		...
		for (int i = 0; i < particular.size(); i++) {
			...
			if ("Match".equalsIgnoreCase(particular.get(i))) {
				...
				gstr2Recon.setPrPercenatge(prPercentMatch);
				...
				gstr2Recon.setA2Percenatge(gstr2APercentMatch);
				...
				gstr2Recon.setPrPercenatge(prPercentMisMatch);
				...
				gstr2Recon.setA2Percenatge(gstr2APercentMisMatch);
				...
				gstr2Recon.setPrPercenatge(prPercentPot);
				...
				gstr2Recon.setA2Percenatge(gstr2APercentPot);
				...
				gstr2Recon.setPrPercenatge(prPercentLog);
				...
				gstr2Recon.setA2Percenatge(gstr2APercentLog);
				...
				gstr2Recon.setPrPercenatge(prPercentAE);
				...
				gstr2Recon.setA2Percenatge(gstr2APercentAE);
				...
				gstr2Recon.setPrPercenatge(prPercentImpg);
				...
				gstr2Recon.setA2Percenatge(gstr2APercentImpg);
	...
	private Gstr2ReconSummaryDto convert(Object[] arr) {
		...
		try {
			...
			BigInteger cnt = GenUtil.getBigInteger(arr[10]);
			...
			if (perticularName.equalsIgnoreCase("Exact Match")
					...
					|| perticularName.equalsIgnoreCase("Addition in 2B(Imports)")){
				obj.setLevel("L2");
			}
		} catch (Exception e) {
