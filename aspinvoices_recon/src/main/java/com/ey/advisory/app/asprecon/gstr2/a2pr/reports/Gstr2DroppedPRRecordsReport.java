...
@Component("Gstr2DroppedPRRecordsReport")
@Slf4j
public class Gstr2DroppedPRRecordsReport {
	...
	public void getGstr2DroppedPRRecordsReport(Long configId)
			throws IOException {
		...
		String reportType = getUniqueFileName("Dropped PR Records Report");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_DROPOUT_PR");
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility
								.getProp("gstr2.dropout.pr.report.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp("gstr2.dropout.pr.report.column")
								.split(",");
						ColumnPositionMappingStrategy<Gstr2InitiateReconReportDto> mappingStrategy =
				...
				String dropedPrReportPath = null;
				if (tempDir.list().length > 0) {
					...
					dropedPrReportPath = DocumentUtility.uploadZipFile(zipFile,
							"Gstr2ReconReports");

				}
				if (dropedPrReportPath != null)
					addlReportRepo.updateReconFilePath(dropedPrReportPath,
							configId, "Dropped_PR_Records_Report");
				if (LOGGER.isDebugEnabled()) {
					String msg = String
							.format("Update file path for Dropped_PR_Records_Report "
									+ ":%s : ", dropedPrReportPath);
					LOGGER.debug(msg);
				}
			}
	...
	private Gstr2InitiateReconReportDto convert(Object[] arr) {
		...
		obj.setIrn2A((arr[175] != null) ? arr[175].toString() : null);
		obj.setIrnPR((arr[176] != null) ? arr[176].toString() : null);
		obj.setIrnDate2A((arr[177] != null) ? arr[177].toString() : null);
		obj.setIrnDatePR((arr[178] != null) ? arr[178].toString() : null);
		...
		obj.setSourceType((arr[182] != null) ? arr[182].toString() : null);
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Dropped_PR_Records_Report" + "_" + configId;
