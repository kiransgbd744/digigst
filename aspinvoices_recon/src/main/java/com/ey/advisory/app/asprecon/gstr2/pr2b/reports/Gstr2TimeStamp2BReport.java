...
@Slf4j
@Component("Gstr2TimeStamp2BReport")
public class Gstr2TimeStamp2BReport {
	...
	public void getGstr2TimeStampReport(Long configId) throws IOException {
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery("USP_RECON_RPT_2B_TIMESTAMP");
			...
			if (records != null && !records.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility
								.getProp("gstr2bpr.recon.time.stamp.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp("gstr2.recon.time.stamp.column")
								.split(",");

						ColumnPositionMappingStrategy<Gstr2TimeStampReportDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
						mappingStrategy.setType(Gstr2TimeStampReportDto.class);
						mappingStrategy.setColumnMapping(columnMappings);
						StatefulBeanToCsvBuilder<Gstr2TimeStampReportDto> builder = new StatefulBeanToCsvBuilder<>(
								writer);
						StatefulBeanToCsv<Gstr2TimeStampReportDto> beanWriter = builder
				...
				if (uploadedDocName != null) {
					// addlReportRepo.updateReconFilePath(cdr2AFilePath,
					// configId,
					// "GSTR_2A_Time_Stamp_Report");
				...
				if (LOGGER.isDebugEnabled()) {
					String msg = String.format(
							"Update file path for GSTR_2B_Time_Stamp_Report :%s : ",
							uploadedDocName);
					LOGGER.debug(msg);
				}
	...
	private Gstr2TimeStampReportDto convert(Object[] arr) {
		...
		String ldt2 = dt2 != null
				? "'".concat(EYDateUtil
						.toISTDateTimeFromUTC(dt2.toLocalDateTime()).toString())
				: null;
		obj.setB2B(ldt2);
		...
		String ldt3 = dt3 != null
				? "'".concat(EYDateUtil
						.toISTDateTimeFromUTC(dt3.toLocalDateTime()).toString())
				: null;
		...
		String ldt4 = dt4 != null
				? "'".concat(EYDateUtil
						.toISTDateTimeFromUTC(dt4.toLocalDateTime()).toString())
				: null;
		...
		String ldt5 = dt5 != null
				? "'".concat(EYDateUtil
						.toISTDateTimeFromUTC(dt5.toLocalDateTime()).toString())
				: null;
		...
		String ldt6 = dt6 != null
				? "'".concat(EYDateUtil
						.toISTDateTimeFromUTC(dt6.toLocalDateTime()).toString())
				: null;
		obj.setISD(ldt6);
		...
		String ldt7 = dt7 != null
				? "'".concat(EYDateUtil
						.toISTDateTimeFromUTC(dt7.toLocalDateTime()).toString())
				: null;
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "GSTR_2B_Time_Stamp_Report" + "_" + configId;
