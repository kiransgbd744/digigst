...
@Slf4j
@Component("Gstr2ForceMatch2BPRRecords")
public class Gstr2ForceMatch2BPRRecords {
	...
	public void getForceMatchRecords(Long configId) throws IOException {
		...
		if (LOGGER.isDebugEnabled()) {
			msg = String.format(
					"Get Recon 2B ForceMatchRecords Details with configId:'%s'",
					configId.toString());
			LOGGER.debug(msg);
		}

		String reportType = "Force Match";
		...
		Gstr2Recon2BPRAddlReportsEntity chunkDetails = addlReportRepo
				.getChunckSizeforReportType(configId, "Force_Match_Records");
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format("Chunking proc Executed"
						+ " USP_RECON_INS_2B_FM3B: configId '%d', "
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format(
						"Error while executing chunking "
								+ "USP_RECON_INS_2B_FM3B proc configId %d",
						configId);
				LOGGER.error(msg, ex);
				...
				while (j < noOfChunk) {
					...
					if (records != null && !records.isEmpty()) {
						...
						if (count >= maxLimitPerFile) {
							...
							if (isReportAvailable) {
								...
								commonService.chunkZipFiles(tempDir,
										...
										"Force_Match_Records", maxReportSizes.getValue1());
							}
						...
						if (noOfChunk == 1 || j == noOfChunk) {
							flushWriter(writer);
							commonService.chunkZipFiles(tempDir,
									chunkDetails.getId(), configId, "Force_Match_Records",
