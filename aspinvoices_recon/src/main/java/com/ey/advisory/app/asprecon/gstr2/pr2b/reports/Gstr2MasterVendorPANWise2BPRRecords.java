...
import com.ey.advisory.app.asprecon.gstr2.a2pr.reports.Gstr2MasterPanDto;
...
@Slf4j
@Component("Gstr2MasterVendorPANWise2BPRRecords")
public class Gstr2MasterVendorPANWise2BPRRecords {
	...
	public void getMasterVendorPANWiseRecords(Long configId)
			throws IOException {
		...
		String reportType = "Vendor PAN Wise Detailed Report";

		tempDir = createTempDir(configId);

		List<String> pnList = findPan(configId);

		String[] arr = new String[pnList.size()];
		arr = pnList.toArray(arr);
			...
			try {
				...
				StoredProcedureQuery storedProc = entityManager
						.createStoredProcedureQuery(
								"USP_RECON_RPT_2B_MASTER_VENDOR_PAN");
				...
				storedProc.setParameter("P_VENDOR_PAN", arr[i]);
				...
				if (records != null && !records.isEmpty()) {
					...
					fullPath = tempDir.getAbsolutePath() + File.separator
							+ reportType + "_" + getUniqueFileName(arr[i])
							+ ".csv";
					List<Gstr2MasterPanDto> reconDataList = new ArrayList<>();
					...
					try (Writer writer = new BufferedWriter(
						...
						if (reconDataList != null && !reconDataList.isEmpty()) {
							...
							String invoiceHeadersTemplate = commonUtility
									.getProp(
											"gstr2bpr.recon.master.pan.header");
							writer.append(invoiceHeadersTemplate);
							String[] columnMappings = commonUtility
									.getProp("gstr2bpr.recon.master.pan.column")
									.split(",");

							ColumnPositionMappingStrategy<Gstr2MasterPanDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
							mappingStrategy.setType(Gstr2MasterPanDto.class);
							mappingStrategy.setColumnMapping(columnMappings);
							StatefulBeanToCsvBuilder<Gstr2MasterPanDto> builder = new StatefulBeanToCsvBuilder<>(
									writer);
							StatefulBeanToCsv<Gstr2MasterPanDto> beanWriter = builder
						...
						LOGGER.error(
								"Exception while executing the Store Proc for "
										+ "Report Type :{}",
								reportType, ex);
		...
		if (tempDir.list().length > 0) {
			...
			/*vendorMasterRecordsFilePath = DocumentUtility.uploadZipFile(zipFile,
					"Gstr2ReconReports");*/
		...
		if (uploadedDocName != null) {
			// addlReportRepo.updateReconFilePath(vendorMasterRecordsFilePath,
			// configId, "Vendor_Records_PAN");
			Gstr2Recon2BPRAddlReportsEntity chunkDetails = addlReportRepo
					.getChunckSizeforReportType(configId, "Vendor_Records_PAN");

			commonService.saveReportChunks(chunkDetails.getId(),
					"Vendor_Records_PAN_1", uploadedDocName.getValue0(),
					true, uploadedDocName.getValue1());
		}
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Update file path for Vendor_Records_PAN " + ":%s : ",
					uploadedDocName);
			LOGGER.debug(msg);
		}
	...
	private Gstr2MasterPanDto convert(Object[] arr) {

		Gstr2MasterPanDto obj = new Gstr2MasterPanDto();
		...
		if (arr[118] != null && (!arr[118].toString().trim().isEmpty())) {
			obj.setTaxPeriodforGSTR3B((arr[118] != null)
					? "`".concat(arr[118].toString()) : null);
		}
		...
		if (docTypePr != null && (docTypePr.equalsIgnoreCase("CR")
				|| docTypePr.equalsIgnoreCase("C")
				|| docTypePr.equalsIgnoreCase("RCR"))) {
			obj.setTaxableValueAdjusted(CheckForNegativeValue(arr[136]));
			obj.setIntegratedTaxAmountAdjusted(CheckForNegativeValue(arr[137]));
			obj.setCentralTaxAmountAdjusted(CheckForNegativeValue(arr[138]));
			obj.setStateUTTaxAmountAdjusted(CheckForNegativeValue(arr[139]));
			obj.setAdvaloremCessAmountAdjusted(CheckForNegativeValue(arr[140]));
			obj.setSpecificCessAmountAdjusted(CheckForNegativeValue(arr[141]));
			obj.setStateCessAmountAdjusted(CheckForNegativeValue(arr[142]));
		} else {
		...
		obj.setContractDate((arr[147] != null) ? arr[147].toString() : null);
		obj.setReferenceIDPR((arr[148] != null) ? arr[148].toString() : null);
		obj.setReferenceID2A((arr[149] != null) ? arr[149].toString() : null);
		if (arr[150] != null && (!arr[150].toString().isEmpty())) {
			obj.setForceMatchResponse((arr[150] != null)
					? "`".concat(arr[150].toString()) : null);
		}
		obj.setComments((arr[151] != null) ? arr[151].toString() : null);
		obj.setOrgSupplierGstinPR(
				(arr[152] != null) ? arr[152].toString() : null);
		obj.setOrgSupplierNamePR(
				(arr[153] != null) ? arr[153].toString() : null);

		obj.setSupPanPR((arr[154] != null) ? arr[154].toString() : null);
		obj.setSupPan2A((arr[155] != null) ? arr[155].toString() : null);

		// newlogic
		obj.setMatchReason((arr[156] != null) ? arr[156].toString() : null);
		obj.setReportCategory((arr[157] != null) ? arr[157].toString() : null);
		obj.setSupplierTradeName2A(
				(arr[158] != null) ? arr[158].toString() : null);
		obj.setBoeReferenceDate(
				(arr[159] != null) ? arr[159].toString() : null);
		obj.setBoeAmended((arr[160] != null) ? arr[160].toString() : null);
		obj.setPortCode2A((arr[161] != null) ? arr[161].toString() : null);
		obj.setBillOfEntry2A((arr[162] != null) ? arr[162].toString() : null);
		obj.setBillOfEntryDate2A(
				(arr[163] != null) ? arr[163].toString() : null);
		if (arr[164] != null && (!arr[164].toString().trim().isEmpty())) {
			obj.setTaxPeriodforGSTR3B((arr[164] != null)
					? "`".concat(arr[164].toString()) : null);
		}
		obj.setGSTR1FilingStatus(
				(arr[165] != null) ? arr[165].toString() : null);
		obj.setGSTR1FilingDate((arr[166] != null) ? arr[166].toString() : null);
		obj.setGSTR1FilingPeriod(
				(arr[167] != null) ? "`".concat(arr[167].toString()) : null);
		obj.setGSTR3BFilingStatus(
				(arr[168] != null) ? arr[168].toString() : null);
		// Suggested Response
		if ((arr[169] != null && (!arr[169].toString().isEmpty())
				&& (arr[169].toString().matches("[0-9]+")))) {

			obj.setSuggestedFMResponse(arr[169].toString().length() == 5
					? "`".concat(("0").concat(arr[169].toString()))
					: "`".concat((arr[169].toString())));
		} else {
		...
		obj.setIrn2A((arr[171] != null) ? arr[171].toString() : null);
		obj.setIrnPR((arr[172] != null) ? arr[172].toString() : null);
		obj.setIrnDate2A((arr[173] != null) ? arr[173].toString() : null);
		obj.setIrnDatePR((arr[174] != null) ? arr[174].toString() : null);
		obj.setGenerationDate2B(arr[175] != null ? arr[175].toString() : null);
		...
		obj.setSourceTypeofIrn((arr[178] != null) ? arr[178].toString() : null);
		obj.setSourceIdentifier((arr[179] != null) ? arr[179].toString() : null);
		return obj;
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Vendor PAN Wise Detailed Report" + "_"
				+ configId;
	...
	private List<String> findPan(Long configId) {
		...
		String queryString = "SELECT DISTINCT "
				+ "IFNULL(SUBSTRING(B2_SUPPLIER_GSTIN,3,10), "
				+ " SUBSTRING(PR_SUPPLIER_GSTIN,3,10)) AS VENDOR_PAN FROM "
				+ " TBL_LINK_2B_PR WHERE RECON_REPORT_CONFIG_ID=:configId "
				+ " AND REPORT_TYPE_ID BETWEEN 1 AND 14";
