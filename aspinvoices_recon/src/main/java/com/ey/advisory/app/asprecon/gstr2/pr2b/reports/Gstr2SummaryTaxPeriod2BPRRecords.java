...
@Slf4j
@Component("Gstr2SummaryTaxPeriod2BPRRecords")
public class Gstr2SummaryTaxPeriod2BPRRecords {
	...
	public void getSummaryTaxPeriodRecords(Long configId) throws IOException {
		...
		try {
			...
			if (configDetails.getToTaxPeriodPR() != null) {
				toTaxPeriodPR = configDetails.getToTaxPeriodPR() != null
						? configDetails.getToTaxPeriodPR().toString() : null;

			} else if (configDetails.getToTaxPeriodPR() != null) {
				toTaxPeriodPR = configDetails.getToDocDate() != null
						? configDetails.getToDocDate().toString() : null;

			}

			if (configDetails.getFromTaxPeriodPR() != null) {
				fromTaxPeriodPR = configDetails.getFromTaxPeriodPR() != null
						? configDetails.getFromTaxPeriodPR().toString() : null;

			} else if (configDetails.getFromDocDate() != null) {
				fromTaxPeriodPR = configDetails.getFromDocDate() != null
						? configDetails.getFromDocDate().toString() : null;
		...
		if (toTaxPeriodPR != null && toTaxPeriodPR.length() == 6) {
			month = toTaxPeriodPR.substring(4, 6);
			year = toTaxPeriodPR.substring(0, 4);
		}

		String toTaxPeriod = year + month;

		String month1 = null;
		String year1 = null;
		if (fromTaxPeriodPR != null && fromTaxPeriodPR.length() == 6) {
			month1 = fromTaxPeriodPR.substring(4, 6);
			year1 = fromTaxPeriodPR.substring(0, 4);
		}

		String fromTaxPeriod = year1 + month1;

		String reportType = null;

		if (fromTaxPeriod != null && !fromTaxPeriod.isEmpty()) {
			reportType = getUniqueFileName("Summary Report Tax Period" + "_"
					+ fromTaxPeriod + "_" + toTaxPeriod);
			// String reportType = getUniqueFileName("Summary Report Tax
			// Period");
		} else {
			reportType = getUniqueFileName("Summary Tax Period Report");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_RECON_RPT_2B_TAXPERIODS_SMRY");
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility.getProp(
								"gstr2bpr.recon.summary.taxperiod.header");
