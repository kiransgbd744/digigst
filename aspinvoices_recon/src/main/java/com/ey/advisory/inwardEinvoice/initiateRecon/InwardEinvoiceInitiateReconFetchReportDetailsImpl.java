...
@Slf4j
@Component("InwardEinvoiceInitiateReconFetchReportDetailsImpl")
public class InwardEinvoiceInitiateReconFetchReportDetailsImpl
		implements InwardEinvoiceInitiateReconFetchReportDetails {
	...
	@Autowired
	@Qualifier("InwardEinvoiceAdditionalReportFetchDetails")
	InwardEinvoiceAdditionalReportFetchDetails reportFetchAddlDetais;
	...
	@Override
	public void getInwardEinvoiceReconReportData(Long configId,
		...
		String[] reportType = { "Exact Match", "Match With Tolerance",
				...
				"Addition in PR", "Addition in Inward E-Inv",
				"Consolidated Inward E-Inv vs PR Report" };
		...
		if (!reconConfigRepo.findByConfigId(configId).getIsMandatory()) {

			reportType = Arrays.asList("Consolidated Inward E-Inv vs PR Report")
					.toArray(new String[0]);
			msg = String.format(
					"inside if - reportType :'%s' with configId:'%s'",
					configId.toString(), reportType);
			LOGGER.debug(msg);
		} else if (!addlInwardReportRepo.getAddlnReportTypeList(configId)
				.contains("Consolidated Inward E-Inv vs PR Report")) {

			reportType = Arrays.asList("Exact Match", "Match With Tolerance",
					"Value Mismatch", "POS Mismatch", "Doc Date Mismatch",
					"Doc Type Mismatch", "Doc No Mismatch I", "Multi-Mismatch",
					"Addition in PR", "Addition in Inward E-Inv")
					.toArray(new String[reportType.length]);
			msg = String.format(
					"inside else if - reportType :'%s' with configId:'%s'",
					configId.toString(), reportType);
			LOGGER.debug(msg);
		}
		 ...
		 * "Doc Type Mismatch", "Doc No Mismatch I", "Multi-Mismatch",
		 * "Addition in PR", "Addition in Inward E-Inv",
		 * "Consolidated Inward E-Inv vs PR") .toArray(new
		 * String[reportType.length]);
		 */
		...
		// case
		if (addnReportList != null && !addnReportList.isEmpty()) {

			List<String> commonElementsList = new ArrayList<>();
			for (String element : addnReportList) {
				if (Arrays.asList(reportType).contains(element)) {
					commonElementsList.add(element);
				}
			}

			reportType = commonElementsList.toArray(new String[0]);
		}

		Set<String> withoutDuplicateReportType = new HashSet<>(
				Arrays.stream(reportType).collect(Collectors.toSet()));

		reportType = withoutDuplicateReportType.toArray(new String[0]);

		msg = String.format("final reportType size :'%d' with configId:'%s'",
				reportType.length, configId.toString());
		LOGGER.debug(msg);
			...
			if (noOfChunk != 0) {
				...
				try {
					...
					String[] columnMappings = commonUtility
							.getProp("inward.einvoice.vs.pr.report.mapping")
							.split(",");
					...
					while (j < noOfChunk) {
						...
						if (records != null && !records.isEmpty()) {
							...
							if (count >= maxLimitPerFile) {
								...
								if (LOGGER.isDebugEnabled()) {
									LOGGER.debug(
											"After Invoking INWARD EINVOICE "
		...
		try {
			...
			if (addnReportList.contains("Doc No Mismatch II"))
				reportFetchAddlDetais.getAdditionalReport(configId,
						"Doc No Mismatch II");
	...
	private InwardEInvoiceReconReportDto convert(Object[] arr) {
		...
		if ("CR".equals(docTypeEINV) || "RCR".equals(docTypeEINV)
				|| "C".equals(docTypeEINV)) {
		...
		if ("CR".equals(docTypePR) || "RCR".equals(docTypePR)
				|| "C".equals(docTypePR)) {
