...
@Slf4j
@Component("Gstr2APAndNonAPLockedCFSNAmendedRecords")
public class Gstr2APAndNonAPLockedCFSNAmendedRecords {
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_AUTO_2APR_INS_CHUNK_RPT_CFSN_AMDLINK");
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format("Executing chunking proc"
						+ " USP_AUTO_2APR_INS_CHUNK_RPT_CFSN_AMDLINK: '%s'",
						configId.toString());
				LOGGER.debug(msg);
			}
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format("Chunking proc Executed"
						+ " USP_AUTO_2APR_INS_CHUNK_RPT_CFSN_AMDLINK: configId '%d', "
			...
			if (LOGGER.isDebugEnabled()) {
				msg = String.format(
						"Error while executing chunking "
								+ "USP_AUTO_2APR_INS_CHUNK_RPT_CFSN_AMDLINK proc configId %d",
						configId);
				LOGGER.error(msg, ex);
		...
		if (noOfChunk > 0) {
			...
			try {
				...
				while (j < noOfChunk) {
					...
					StoredProcedureQuery storedProc = entityManager
							.createStoredProcedureQuery(
									"USP_AUTO_2APR_DISP_CHUNK_RPT_CFSN_AMDLINK");
	...
	private CSFFlagNDto convert(Object[] arr) {
		...
		String docType2APrevious = (arr[15] != null) ? arr[15].toString()
				: null;
		String docType2ACurrtent = (arr[16] != null) ? arr[16].toString()
				: null;
		obj.setDOCTYPEPRPrevious(docTypePRPrevious);
		obj.setDOCTYPE2APrevious(docType2APrevious);
		obj.setDOCTYPE2ACurrent(docType2ACurrtent);
		...
		if (docType2ACurrtent != null
				&& (docType2ACurrtent.equalsIgnoreCase("CR")
						|| docType2ACurrtent.equalsIgnoreCase("C")
						|| docType2ACurrtent.equalsIgnoreCase("RCR"))) {
		...
		obj.setOrgDocNumber2APrevious(addApostrophe(arr[62]));
		obj.setOrgDocNumber2ACurrent(addApostrophe(arr[63]));
