...
@Slf4j
@Component("ImpgAPReconReport")
public class ImpgAPReconReport {
	...
	public void getGstr2ImpgReconReport(Long configId) throws IOException {
		...
		try {
			...
			storedProc.setParameter("P_REPORT_TYPE", "Consolidated IMPG Report");
		...
		if (noOfChunk > 0) {
			...
			try {
				...
				String[] columnMappings = commonUtility
						.getProp("gstr2.impg.report.column")
						.split(",");
				...
				while (j < noOfChunk) {
					...
					if (LOGGER.isDebugEnabled()) {
						msg = String.format("call stored proc with "
								...
								+ "reportType is Consolidated IMPG Report, "
								+ " chunkNo is %d ",
								configId.toString(),  j);
						LOGGER.debug(msg);
					}
					...
					if (records != null && !records.isEmpty()) {
						...
						List<Gstr2ImpgReconDto> reconDataList = records
								.stream()
								.map(o -> convert(o))
								.collect(Collectors
