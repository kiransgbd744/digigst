...
import com.ey.advisory.app.asprecon.gstr2.a2pr.reports.Gstr2RecipientGSTINPeriodWiseRecordDto;
...
@Slf4j
@Component("Gstr2APAndNonAPRecipientGSTINWiseRecords")
public class Gstr2APAndNonAPRecipientGSTINWiseRecords {
	...
	public void getRecipientGSTINWiseRecords(Long configId) throws IOException {
		...
		String reportType = getUniqueFileName("Recipient GSTIN Wise Report");
		...
		try {
			...
			StoredProcedureQuery storedProc = entityManager
					.createStoredProcedureQuery(
							"USP_AUTO_2APR_RPT_RGSTIN_DOCTYPE");
			...
			if (records != null && !records.isEmpty()) {
				...
				fullPath = tempDir.getAbsolutePath() + File.separator
						+ reportType + ".csv";
				List<Gstr2RecipientGSTINPeriodWiseRecordDto> reconDataList = new ArrayList<>();
				...
				try (Writer writer = new BufferedWriter(
					...
					if (reconDataList != null && !reconDataList.isEmpty()) {
						...
						String invoiceHeadersTemplate = commonUtility
								.getProp("gstr2.recon.gstinwise.header");
						writer.append(invoiceHeadersTemplate);
						String[] columnMappings = commonUtility
								.getProp("gstr2.recon.gstinwise.column")
								.split(",");

						ColumnPositionMappingStrategy<Gstr2RecipientGSTINPeriodWiseRecordDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
						mappingStrategy.setType(
								Gstr2RecipientGSTINPeriodWiseRecordDto.class);
						mappingStrategy.setColumnMapping(columnMappings);
						StatefulBeanToCsvBuilder<Gstr2RecipientGSTINPeriodWiseRecordDto> builder = new StatefulBeanToCsvBuilder<>(
								writer);
						StatefulBeanToCsv<Gstr2RecipientGSTINPeriodWiseRecordDto> beanWriter = builder
				...
				if (tempDir.list().length > 0) {
					...
					/*gstinWiseRecordsFilePath = DocumentUtility
				...
				if (uploadedDocName != null)
					/*addlReportRepo.updateReconFilePath(gstinWiseRecordsFilePath,
							configId, "Recipient_GSTIN_Wise_Records");*/
				...
				if (LOGGER.isDebugEnabled()) {
					String msg = String
							.format("Update file path for Recipient_GSTIN_Wise_Records "
									+ ":%s : ", uploadedDocName);
					LOGGER.debug(msg);
				}
	...
	private Gstr2RecipientGSTINPeriodWiseRecordDto convert(Object[] arr) {

		Gstr2RecipientGSTINPeriodWiseRecordDto obj = new Gstr2RecipientGSTINPeriodWiseRecordDto();

		obj.setRecipientGSTIN((arr[0] != null) ? arr[0].toString() : null);
		obj.setPeriodPR(null);
		...
		obj.setDocNoDoctDateMismatchCount(
	...
	private static File createTempDir(Long configId) throws IOException {
		...
		String tempFolderPrefix = "Recipient_GSTIN_Wise_Records" + "_"
				+ configId;
