...
@Slf4j
@Component
public class Gstr6IsdDistributionReportCommonServiceImpl
		implements Gstr6IsdDistributionReportCommonService {
	...
	private static final String ISD_REPORT_CHUNK_ROWS = "gstr6.isd.anx.report.chunk.rows";

	@Override
	public Integer getChunkingSizes() {
		Map<String, Config> configMap = configManager.getConfigs(
				"ISD_ANX_REPORTS", "gstr6.isd.anx.report.chunk");
		String fileSize = configMap.get(ISD_REPORT_CHUNK_ROWS) != null
				? configMap.get(ISD_REPORT_CHUNK_ROWS).getValue() : "200000";
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info(
					"Gstr6IsdDistributionReportCommonService Directory is empty");
			return;
		}
			...
			/*	String uploadedDocName = DocumentUtility.uploadZipFile(zipFile,
				...
				*/
				...
				Pair<String, String> uploadedZipName = DocumentUtility
						.uploadFile(zipFile, "GSTR6IsdAnnexReport");
	...
	private static String getIndexNumber(String fileName) {
		...
		if (fileName.startsWith("Credit Distribution Annexure Report_")) {
			index = name[2];
		}
