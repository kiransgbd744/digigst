 ...
 *
 */
public class HsnRetPeriodValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(Object obj, Object[] row,
		...
		if (!isPresent(obj)) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER5703",
			...
			if (!taxPeriod.matches("[0-9]+")) {
				...
				if (taxPeriod.length() != 6
						|| (Integer.valueOf(taxPeriod.substring(0, 2)) > 12
								|| Integer.valueOf(
										taxPeriod.substring(0, 2)) < 01)
						|| (Integer.valueOf(taxPeriod.substring(2)) > 9999
								|| Integer.valueOf(
										taxPeriod.substring(2)) < 0000)) {
					Set<String> errorLocations = new HashSet<>();
					errorLocations.add(RETURN_PREIOD);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
