...
import com.ey.advisory.app.data.views.client.Gstr1GstnADVRCVDErrorDto;
 ...
 *
 */
@Component("Gstr1GstnAdvRecErrorDaoImpl")
public class Gstr1GstnAdvRecErrorDaoImpl implements Gstr1GstnErrorDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1GstnAdvRecErrorDaoImpl.class);
	...
	public List<Object> getGstr1GstnErrorReport(SearchCriteria criteria) {
		...
		String queryStr = creategstnAdvRecErrorQueryString(
				buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertgstnAdvRecError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1GstnADVRCVDErrorDto convertgstnAdvRecError(Object[] arr) {
		Gstr1GstnADVRCVDErrorDto obj = new Gstr1GstnADVRCVDErrorDto();
	...
	private String creategstnAdvRecErrorQueryString(String buildQuery) {

		return "SELECT HDR.POS PLACE_OF_SUPPLUY,HDR.SUPPLY_TYPE,"
				+ "HDR.DIFF_PERCENT,HDR.ADVREC_AMT, ITM.TAX_RATE,ITM.IGST_AMT,"
				+ "ITM.CGST_AMT,ITM.SGST_AMT,ITM.CESS_AMT, TRIM(', ' FROM "
				+ "IFNULL(ERROR_CODE,'')) AS GSTN_ERROR_CODE, TRIM(', ' "
				+ "FROM IFNULL(ERROR_DESCRIPTION,'')) AS "
				+ "GSTN_ERROR_DESCRIPTION FROM GETGSTR1_AT_HEADER HDR "
