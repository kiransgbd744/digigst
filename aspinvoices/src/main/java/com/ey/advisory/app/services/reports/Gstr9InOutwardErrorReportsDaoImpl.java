 ...
 *
 */
@Component("Gstr9InOutwardErrorReportsDaoImpl")
@Slf4j
public class Gstr9InOutwardErrorReportsDaoImpl implements Gstr1VerticalReportsDao {
	...
	public List<Object> getGstr1VerticalReports(Gstr1VerticalDownloadReportsReqDto request) {
		...
		List<Object> retList = list.parallelStream().map(o -> convertVerticalError(o))
	...
	private String createVerticalErrorQueryString(String buildQuery) {
		...
		if ((GSTConstants.INWARD_OUTWARD).equalsIgnoreCase(fileType)) {
            ...
            builder.append("GSTIN,FY,TABLE_NUMBER,NATURE_OF_SUPPLIES, ");
			builder.append("TAXABLE_VALUE,IGST,CGST,SGST,CESS,INTEREST, ");
			builder.append("LATE_FEE,PENALTY,OTHER ");
			builder.append("FROM GSTR9_OUTWARD_INWARD_AS_ENTERED HI LEFT OUTER JOIN  ");
			builder.append("TF_VERTICAL_ERROR_INFO() ERR ON  ");
			builder.append("HI.ID = ERR.COMMON_ID  AND HI.FILE_ID=ERR.FILE_ID  ");
			builder.append("LEFT OUTER JOIN FILE_STATUS FIL ");
			builder.append("ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID AND ");
			builder.append("ERR.INV_KEY=HI.DOC_KEY WHERE HI.IS_ERROR = TRUE  AND ");
