...
import com.ey.advisory.core.search.SearchCriteria;
	...
	@Component("AspB2CSASavableTotalSummaryDaoImpl")
	public class AspB2CSASavableTotalSummaryDaoImpl
			implements Gstr1ASPB2CSSavableTotalDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(AspB2CSASavableTotalSummaryDaoImpl.class);
		...
		public List<Object> getGstr1B2CSSavableReports(SearchCriteria criteria) {
			...
			if (taxPeriodFrom != null && taxPeriodTo != null) {
				buildQueryheader.append(" AND B2CS.DERIVED_RET_PERIOD BETWEEN ");
		...
		private String createB2CSSavableTotalQueryString(
			...
			return "SELECT SUPPLIER_GSTIN, RETURN_PERIOD, TRAN_TYPE,MONTH, "
					...
					+ "MAX(NEW_HSNORSAC)NEW_HSNORSAC,MAX(NEW_UOM)NEW_UOM, "
					+ "MAX(NEW_QNT)NEW_QNT,MAX(NEW_RATE)NEW_RATE, "
					...
					+ "IFNULL(SUM(TOT_VAL),0) AS TOT_VAL, "
					+ "MAX(PROFIT_CENTRE)PROFIT_CENTRE,"
					+ "MAX(PLANT_CODE)PLANT_CODE, MAX(DIVISION)DIVISION, "
					+ "MAX(LOCATION)LOCATION, MAX(SALES_ORGANIZATION)SALES_ORGANIZATION, "
					...
					+ "FROM ( SELECT (IFNULL(B2CS.SUPPLIER_GSTIN,'') ||'|'|| "
					+ "IFNULL(B2CS.RETURN_PERIOD,'') "
					+ "||'|'||IFNULL(TRAN_TYPE,'') ||'|'||IFNULL(NEW_POS,9999)||'|'||IFNULL(NEW_ECOM_GSTIN,'')||'|'||IFNULL(MONTH,9999) ) "
					+ "DOC_KEY, B2CS.SUPPLIER_GSTIN,B2CS.RETURN_PERIOD, TRAN_TYPE, "
					+ "MONTH,NULL AS ORG_POS,0 ORG_HSNORSAC,''ORG_UOM, 0 ORG_QNT,NULL "
					+ "AS ORG_RATE, ORG_TAXABLE_VALUE,NULL AS ORG_ECOM_GSTIN, "
					+ "ORG_ECOM_SUP_VAL,NEW_POS, 0 NEW_HSNORSAC,"
					+ " '' NEW_UOM,0 NEW_QNT,NULL AS NEW_RATE,NEW_TAXABLE_VALUE, "
					+ "NEW_ECOM_GSTIN, NEW_ECOM_SUP_VAL,IGST_AMT,CGST_AMT,SGST_AMT, "
					+ "CESS_AMT, IFNULL(NEW_TAXABLE_VALUE,0)+IFNULL(IGST_AMT,0)+IFNULL(CGST_AMT,0)"
					+ " +IFNULL(SGST_AMT,0)+IFNULL(CESS_AMT,0) AS TOT_VAL, '' PROFIT_CENTRE,"
					...
					+ " '' FILE_NAME, '' ASP_DATE_TIME FROM GSTR1_PROCESSED_B2CS B2CS "
					+ "LEFT OUTER JOIN (SELECT COMMON_ID,VAL_TYPE,FILE_ID,INV_KEY,"
					+ "TABLE_TYPE, STRING_AGG(INFO_ERROR_CODE_ASP,',') INFO_ERROR_CODE_ASP, "
					...
					+ "FROM ANX_VERTICAL_ERROR) GROUP BY COMMON_ID, VAL_TYPE,FILE_ID,INV_KEY,TABLE_TYPE) "
					+ "GSTRERR ON B2CS.AS_ENTERED_ID= GSTRERR.COMMON_ID AND "
					+ "B2CS.FILE_ID=GSTRERR.FILE_ID AND GSTRERR.TABLE_TYPE='B2CS' "
					+ "AND RETURN_TYPE='GSTR1' LEFT OUTER JOIN "
					+ "FILE_STATUS FIL ON B2CS.FILE_ID=FIL.ID AND GSTRERR.FILE_ID=FIL.ID "
					+ "AND GSTRERR.INV_KEY=B2CS.B2CS_INVKEY LEFT OUTER JOIN "
					+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = B2CS.BATCH_ID "
					+ "WHERE B2CS.IS_DELETE=FALSE AND IS_AMENDMENT=TRUE "
			        + buildQueryheader  + ") "
					+ "GROUP BY SUPPLIER_GSTIN, RETURN_PERIOD, TRAN_TYPE,NEW_POS,NEW_ECOM_GSTIN,MONTH ";
