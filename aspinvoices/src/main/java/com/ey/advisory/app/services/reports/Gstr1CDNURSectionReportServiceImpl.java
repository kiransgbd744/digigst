...
@Service("Gstr1CDNURSectionReportServiceImpl")
public class Gstr1CDNURSectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1CDNURSectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1CDNURSectionReportDaoImpl")
	...
	public Workbook findGstr1ReviewSummRecords(SearchCriteria criteria,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 CDNUR processed data item level response"
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 CDNUR isActiveFieldSel response"
			...
			if (responseFromView != null && responseFromView.size() > 0) {
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					String msg = String
							.format("CDNUR Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 CDNUR custom file name -"
			...
			LOGGER.error(
					"Exception Occuered in CDNUR Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
