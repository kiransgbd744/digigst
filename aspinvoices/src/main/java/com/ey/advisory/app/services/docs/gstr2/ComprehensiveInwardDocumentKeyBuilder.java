...
@Slf4j
public class ComprehensiveInwardDocumentKeyBuilder
	...
	public String buildDataBlockKey(Object[] arr, TabularDataLayout config) {
		...
		String docNo = (arr[6] != null && !arr[6].toString().isEmpty())
				? String.valueOf(arr[6]).trim() : "";

		String docType = (arr[5] != null && !arr[5].toString().isEmpty())
				? String.valueOf(arr[5]).trim() : "";
		Object docDate = (arr[7] != null && !arr[7].toString().isEmpty())
				? arr[7] : "";

		String sgstin = (arr[9] != null && !arr[9].toString().isEmpty())
				? String.valueOf(arr[9]).trim() : "";
		String cgstin = (arr[19] != null && !arr[19].toString().isEmpty())
				? String.valueOf(arr[19]).trim() : "";
		...
		if (!Strings.isNullOrEmpty(sgstin)) {
			...
			LOGGER.debug("Inside comprehensive - "+ arr[11]);
			String suppName = (arr[11] != null && !arr[11].toString().isEmpty())
					? StringUtils.truncate(String.valueOf(arr[11]).trim(), 25)
					: "";
			return new StringJoiner(DOC_KEY_JOINER).add(finYear).add(cgstin)
