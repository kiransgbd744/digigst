 ...
 */
	...
	@Component("Gstr2xProcessedReportDaoImpl")
	public class Gstr2xProcessedReportDaoImpl implements Gstr2xReportDao {

		@PersistenceContext(unitName = "clientDataUnit")
		private EntityManager entityManager;

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr2xProcessedReportDaoImpl.class);
		...
		private String createVerticalTotalQueryString(String buildQuery) {
			return "SELECT TRIM(', ' FROM IFNULL(INFO_ERROR_CODE_ASP,'') ) "
					+ "AS ASP_INFO_CODE, "
					+ " '' AS ASP_INFO_DESCRIPTION, RECORD_TYPE "
					+ "AS CATEGORY,GSTIN,RET_PERIOD,CTIN AS DED_COL_GSTIN,"
					+ "DEDUCTOR_UPL_MONTH, CASE WHEN RECORD_TYPE IN ('TDSA','TCSA') "
					+ "THEN ORG_DEDUCTOR_UPL_MONTH ELSE '' END AS ORG_DEDUCTOR_UPL_MONTH, "
					+ "TAXABLE_VALUE AS TOT_AMT, CASE WHEN RECORD_TYPE IN ('TCS','TCSA') "
					...
					+ " GROUP BY "
					+ "COMMON_ID,INV_KEY,FILE_ID) ERR ON HDR.AS_ENTERED_ID= ERR.COMMON_ID "
					...
					+ " WHERE HDR.IS_DELETE=FALSE AND DATAORIGINTYPECODE='E' "
					+ buildQuery;
