 ...
 *
 */
public class TxpdSgstinValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(Object obj, Object[] row,
		...
		if (!isPresent(obj)) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER5176",
		...
		Matcher matcher = pattern.matcher(String.valueOf(obj).trim());

		if (String.valueOf(obj).trim().length() != 15 || !matcher.matches()) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.SGSTIN);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
