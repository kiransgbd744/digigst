...
import com.ey.advisory.app.data.repositories.client.Gstr1InvoiceGstnRepository;
 ...
 *
 */
@Component("Gstr1InvoiceBusinessService")
@Slf4j
public class Gstr1InvoiceBusinessService {
	...
	@Autowired
	@Qualifier("Gstr1AsEnteredInvoiceRepository")
	private Gstr1AsEnteredInvoiceRepository gstr1AsEnteredInvoiceRepository;
	...
	@Autowired
	@Qualifier("SRFileToInvoiceDetailsConvertion")
	private SRFileToInvoiceDetailsConvertion sRFileToInvoiceDetailsConvertion;

	@Autowired
	@Qualifier("Gstr1InvoiceGstnRepository")
	private Gstr1InvoiceGstnRepository gstr1InvoiceGstnRepository;
	...
	public void processBusinessData(List<Object[]> listOfInvoices,
			List<Gstr1AsEnteredInvEntity> strProcessRecords,
			List<Gstr1AsEnteredInvEntity> strErrRecords,
			Gstr1FileStatusEntity updateFileStatus) {

		List<Gstr1AsEnteredInvEntity> busErrorRecords = new ArrayList<>();
		List<Gstr1AsEnteredInvEntity> infoProcessed = new ArrayList<>();
		List<Gstr1AsEnteredInvEntity> busProcessRecords = new ArrayList<>();
		...
		for (Gstr1AsEnteredInvEntity invoice : strProcessRecords) {
		...
		for (Gstr1AsEnteredInvEntity process : strProcessRecords) {
		...
		if (!busProcessRecords.isEmpty()) {
			...
			List<Gstr1InvoiceFileUploadEntity> invoiceDoc = sRFileToInvoiceDetailsConvertion
			...
			List<String> existProcessData = invoiceDoc.parallelStream()
					.map(Gstr1InvoiceFileUploadEntity::getInvoiceKey).distinct()
					.collect(Collectors.toList());
