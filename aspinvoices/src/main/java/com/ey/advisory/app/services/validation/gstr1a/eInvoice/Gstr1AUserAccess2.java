 ...
 *
 */
@Component("Gstr1AUserAccess2")
public class Gstr1AUserAccess2
			...
			entitAtConfigInnerMap.entrySet().forEach(entitAtConfMap -> {
				...
				if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
					boolean isAttrValid = onboardAttributesChecking.isAttrValid(
							entityAtValMap, OnboardingConstant.UD2, ua2,
							document.getEntityId());
					if (ua2 == null || ua2.isEmpty()) {
						...
						errors.add(
								new ProcessingResult(APP_VALIDATION, "ER0133",
										"User Access 2 is not as per master "
				...
				if (OnboardingConstant.AT_O.equalsIgnoreCase(atOutward)) {
					...
					if (!isAttrValid) {
						...
						errors.add(new ProcessingResult(APP_VALIDATION,
								ProcessingResultType.INFO, "IN0017",
								"User Access 2 is not as per master "
										+ "provided during On-Boarding.",
								location));
		...
		if (ua2 != null && !ua2.isEmpty()) {
			...
			if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
				...
				boolean isAttrUserValid = onboardAttributesChecking.isAttrValid(
						OnboardingConstant.UD2, ua2, document.getEntityId(),
						document.getCreatedBy());
				if (!isAttrUserValid) {
					errorLocations.add(GSTConstants.USER_ACCESS2);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER0160",
