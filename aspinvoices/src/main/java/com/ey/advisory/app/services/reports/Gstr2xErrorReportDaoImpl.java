 ...
 * 
 */
@Component("Gstr2xErrorReportDaoImpl")
public class Gstr2xErrorReportDaoImpl implements Gstr2xReportDao {

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr2xErrorReportDaoImpl.class);
	...
	private String createVerticalTotalQueryString(String buildQuery) {
		return " SELECT TRIM(', ' FROM IFNULL(ERROR_CODE_ASP,'') )  AS ASP_ERROR_CODE, "
				+ " '' AS ASP_ERROR_DESCRIPTION, RECORD_TYPE AS CATEGORY,"
				+ "GSTIN,RET_PERIOD,CTIN AS DED_COL_GSTIN,DEDUCTOR_UPL_MONTH, "
				+ "CASE WHEN RECORD_TYPE IN ('TDSA','TCSA') "
				+ "THEN ORG_DEDUCTOR_UPL_MONTH ELSE '' END AS ORG_DEDUCTOR_UPL_MONTH, "
				+ "TAXABLE_VALUE AS TOT_AMT, CASE "
				+ "WHEN RECORD_TYPE IN ('TCS','TCSA') "
				+ "THEN REG_SUP ELSE 0 END REG_SUP, "
				+ "CASE WHEN RECORD_TYPE IN ('TCS','TCSA') "
				+ "THEN REG_SUPRET ELSE 0 END REG_SUPRET, "
				+ "CASE WHEN RECORD_TYPE IN ('TCS','TCSA') "
				+ "THEN UNREG_SUP ELSE 0 END UNREG_SUP, "
				+ "CASE WHEN RECORD_TYPE IN ('TCS','TCSA') "
				+ "THEN UNREG_SUPRET ELSE 0 END UNREG_SUPRET, "
				+ "IGST_AMT,CGST_AMT,SGST_AMT,SAVED_ACTION,USER_ACTION "
				+ "FROM GSTR2X_AS_ENTERED_TCS_TDS HDR "
				+ "LEFT OUTER JOIN (SELECT COMMON_ID,INV_KEY,FILE_ID ,"
