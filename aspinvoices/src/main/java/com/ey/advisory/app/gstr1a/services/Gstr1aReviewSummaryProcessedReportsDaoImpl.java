...
import com.ey.advisory.app.services.reports.Gstr1ReviewSummaryReportsDao;
...
@Component("Gstr1aReviewSummaryProcessedReportsDaoImpl")
public class Gstr1aReviewSummaryProcessedReportsDaoImpl
		implements Gstr1ReviewSummaryReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1aReviewSummaryProcessedReportsDaoImpl.class);
	...
	private DataStatusEinvoiceDto convertProcessed(Object[] arr) {
		...
		/*
		 * LocalDateTime irnDate = (LocalDateTime) arr[7]; String itnStatus =
		 * obj.getIrnStatus();
		 * 
		 * if ("GENERATED".equalsIgnoreCase(itnStatus) &&
		 * CommonUtility.deriveEinvStatus(irnDate)) {
		 * obj.setIrn("Generated – Available for cancellation "); }
		 */
		...
		obj.setEINVvsGstr1Reponse(arr[312] != null ? arr[312].toString() : null);
	...
	private String createApiProcessedQueryString(String buildHeader) {
		...
		return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, TRIM(',' FROM IFNULL(HDR.INFORMATION_CODES,'')||','|| "
				+ "IFNULL(ITM.INFORMATION_CODES,'')) AS INFO_ERROR_CODE_ASP, '' AS INFO_ERROR_DESCRIPTION_ASP, CASE WHEN HDR.IRN_STATUS = 1 "
				+ "THEN 'NOT OPTED' WHEN HDR.IRN_STATUS = 2 THEN 'NOT APPLICABLE' "
				+ "when HDR.IRN_STATUS = 3 then 'Pending (No Action)' "
				+ "WHEN HDR.IRN_STATUS = 4 THEN 'GENERATION ERROR' WHEN HDR.IRN_STATUS = 5 THEN 'GENERATED' WHEN HDR.IRN_STATUS = 6 "
				+ "THEN 'CANCELLED' WHEN HDR.IRN_STATUS = 7 THEN 'DigiGST Error' WHEN HDR.IRN_STATUS = 8 THEN 'DUPLICATE_IRN' "
				+ "when HDR.IRN_STATUS = 13 then 'Generated as provided by user' "
				+ "when HDR.IRN_STATUS = 11 then 'Pending (Error)' "
				+ "WHEN HDR.IRN_STATUS = 10 THEN 'PUSHED TO NIC' END AS IRN_STATUS, HDR.IRN_RESPONSE AS IRN_NO, HDR.ACK_NUM AS IRN_ACK_NO, "
				+ "TO_CHAR(HDR.ACK_DATE,'DD-MM-YYYY HH24:MI:SS') AS IRN_ACK_DATE_TIME, EINV.SIGNED_QR AS SIGNED_QR_CODE, NULL "
				+ "AS SIGNED_INVOICE, HDR.EINV_ERROR_CODE AS IRP_ERROR_CODE, HDR.EINV_ERROR_DESC AS IRP_ERROR_DESCRIPTION, "
				...
				+ "AND HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END AS GSTIN_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, "
				+ "GSTNBATCH.BATCH_DATE AS GSTIN_REFID_DATE_TIME, NULL AS GSTIN_ERROR_CODE, NULL AS GSTIN_ERROR_DESCRIPTION_ASP, "
				+ "HDR.RETURN_TYPE AS RET_TYPE, HDR.TABLE_SECTION AS TABLE_NUMBER, HDR.IRN, HDR.IRN_DATE,HDR.TAX_SCHEME, "
				+ "HDR.CANCEL_REASON, HDR.CANCEL_REMARKS, ITM.SUPPLY_TYPE, HDR.DOC_CATEGORY, HDR.DOC_TYPE,HDR.DOC_NUM,HDR.DOC_DATE, "
				+ "HDR.REVERSE_CHARGE, HDR.SUPPLIER_GSTIN,HDR.SUPP_TRADE_NAME, HDR.SUPP_LEGAL_NAME, HDR.SUPP_BUILDING_NUM, "
				+ "HDR.SUPP_BUILDING_NAME AS SUPPLIERADDRESS2, HDR.SUPP_LOCATION, HDR.SUPP_PINCODE, HDR.SUPP_STATE_CODE, HDR.SUPP_PHONE, "
				+ "HDR.SUPP_EMAIL, HDR.CUST_GSTIN, HDR.CUST_TRADE_NAME, HDR.CUST_SUPP_NAME, HDR.CUST_SUPP_ADDRESS1,HDR.CUST_SUPP_ADDRESS2, "
				+ "HDR.CUST_SUPP_ADDRESS4,HDR.CUST_PINCODE,HDR.BILL_TO_STATE, HDR.POS,HDR.CUST_PHONE,HDR.CUST_EMAIL,HDR.DISPATCHER_GSTIN, "
				+ "HDR.DISPATCHER_TRADE_NAME,HDR.DISPATCHER_BUILDING_NUM, HDR.DISPATCHER_BUILDING_NAME,HDR.DISPATCHER_LOCATION, "
				+ "HDR.DISPATCHER_PINCODE,HDR.DISPATCHER_STATE_CODE , HDR.SHIP_TO_GSTIN ,HDR.SHIP_TO_TRADE_NAME, HDR.SHIP_TO_LEGAL_NAME,"
				+ "HDR.SHIP_TO_BUILDING_NUM, HDR.SHIP_TO_BUILDING_NAME,HDR.SHIP_TO_LOCATION, ITM.SHIP_TO_PINCODE,ITM.SHIP_TO_STATE,ITM.ITM_NO, "
				+ "ITM.SERIAL_NUM2,ITM.PRODUCT_NAME,ITM.ITM_DESCRIPTION, ITM.IS_SERVICE,ITM.ITM_HSNSAC,ITM.BAR_CODE, ITM.BATCH_NAME_OR_NUM,"
				+ "ITM.BATCH_EXPIRY_DATE, ITM.WARRANTY_DATE, ITM.ORDER_ITEM_REFERENCE, ITM.ATTRIBUTE_NAME, ITM.ATTRIBUTE_VALUE, "
				+ "ITM.ORIGIN_COUNTRY,ITM.ITM_UQC_USER,ITM.QTY_USER, ITM.FREE_QTY, ITM.UNIT_PRICE, ITM.ITEM_AMT_UP_QTY, ITM.ITEM_DISCOUNT,"
				+ "ITM.PRE_TAX_AMOUNT,ITM.TAXABLE_VALUE, ITM.IGST_RATE, ITM.IGST_AMT,ITM.CGST_RATE,ITM.CGST_AMT, ITM.SGST_RATE,ITM.SGST_AMT,"
				+ "ITM.CESS_RATE_ADVALOREM, ITM.CESS_AMT_ADVALOREM, ITM.CESS_RATE_SPECIFIC, ITM.CESS_AMT_SPECIFIC, ITM.STATECESS_RATE, "
				+ "ITM.STATECESS_AMT, ITM.STATE_CESS_SPECIFIC_RATE, ITM.STATE_CESS_SPECIFIC_AMOUNT, ITM.OTHER_VALUES, TOT_ITEM_AMT, "
				+ "ITM.INV_OTHER_CHARGES, ITM.INV_ASSESSABLE_AMT,ITM.INV_IGST_AMT, ITM.INV_CGST_AMT, ITM.INV_SGST_AMT,ITM.INV_CESS_ADVLRM_AMT, "
				+ "ITM.INV_CESS_SPECIFIC_AMT,ITM.INV_STATE_CESS_AMT, ITM.INV_STATE_CESS_SPECIFIC_AMOUNT, "
				...
				+ "ITM.TCS_RATE_INCOME_TAX, ITM.TCS_AMOUNT_INCOME_TAX, HDR.CUSTOMER_PAN_OR_AADHAAR, HDR.FOREIGN_CURRENCY,HDR.COUNTRY_CODE, "
				+ "HDR.INV_VAL_FC, HDR.SHIP_PORT_CODE,HDR.SHIP_BILL_NUM, HDR.SHIP_BILL_DATE, HDR.INV_REMARKS,HDR.INV_PERIOD_START_DATE, "
				+ "HDR.INV_PERIOD_END_DATE, ITM.PRECEEDING_INV_NUM, ITM.PRECEEDING_INV_DATE,ITM.INV_REFERENCE, "
				+ "ITM.RECEIPT_ADVICE_REFERENCE,ITM.RECEIPT_ADVICE_DATE, ITM.TENDER_REFERENCE ,ITM.CONTRACT_REFERENCE, "
				+ "ITM.EXTERNAL_REFERENCE, ITM.PROJECT_REFERENCE, ITM.CUST_PO_REF_NUM,ITM.CUST_PO_REF_DATE,HDR.PAYEE_NAME, "
				+ "HDR.MODE_OF_PAYMENT,HDR.BRANCH_IFSC_CODE,HDR.PAYMENT_TERMS, HDR.PAYMENT_INSTRUCTION,HDR.CR_TRANSFER, HDR.DB_DIRECT, "
				+ "HDR.CR_DAYS,ITM.PAID_AMT,ITM.BAL_AMT, HDR.PAYMENT_DUE_DATE, HDR.ACCOUNT_DETAIL,HDR.ECOM_GSTIN, HDR.ECOM_TRANS_ID, "
				+ "ITM.SUPPORTING_DOC_URL,ITM.SUPPORTING_DOC_BASE64, ITM.ADDITIONAL_INFORMATION,HDR.TRANS_TYPE,HDR.SUB_SUPP_TYPE, "
				+ "HDR.OTHER_SUPP_TYPE_DESC,HDR.TRANSPORTER_ID, HDR.TRANSPORTER_NAME, HDR.TRANSPORT_MODE,HDR.TRANSPORT_DOC_NUM, "
				+ "HDR.TRANSPORT_DOC_DATE, HDR.DISTANCE ,HDR.VEHICLE_NUM, HDR.VEHICLE_TYPE, HDR.RETURN_PERIOD,HDR.ORIGINAL_DOC_TYPE, "
				+ "HDR.ORIGINAL_CUST_GSTIN, HDR.DIFF_PERCENT, HDR.SECTION7_OF_IGST_FLAG,HDR.CLAIM_REFUND_FLAG, HDR.AUTOPOPULATE_TO_REFUND,"
				+ "HDR.CRDR_PRE_GST,HDR.CUST_SUPP_TYPE, HDR.CUST_SUPP_CODE AS CUSTOMERCODE,ITM.PRODUCT_CODE,ITM.ITM_TYPE, "
				+ "ITM.ITC_FLAG,HDR.STATE_APPLYING_CESS,ITM.FOB, ITM.EXPORT_DUTY, HDR.EXCHANGE_RATE,ITM.CRDR_REASON, HDR.TCS_FLAG ,"
				+ "ITM.TCS_AMT, TCS_CGST_AMT,TCS_SGST_AMT, HDR.TDS_FLAG,TDS_IGST_AMT, TDS_CGST_AMT,TDS_SGST_AMT, HDR.USER_ID AS USERID,"
				+ "HDR.COMPANY_CODE, HDR.SOURCE_IDENTIFIER, HDR.SOURCE_FILENAME,ITM.PLANT_CODE, HDR.DIVISION, SUB_DIVISION,ITM.LOCATION,"
				+ "HDR.SALES_ORGANIZATION, HDR.DISTRIBUTION_CHANNEL,ITM.PROFIT_CENTRE,HDR.PROFIT_CENTRE2, HDR.USERACCESS1,HDR.USERACCESS2,"
				+ "HDR.USERACCESS3, HDR.USERACCESS4, HDR.USERACCESS5,HDR.USERACCESS6, ITM.GLCODE_TAXABLEVALUE, ITM.GLCODE_IGST,"
				+ "ITM.GLCODE_CGST, ITM.GLCODE_SGST, ITM.GLCODE_ADV_CESS,ITM.GLCODE_SP_CESS, ITM.GLCODE_STATE_CESS, HDR.GL_STATE_CESS_SPECIFIC, "
				+ "HDR.GL_POSTING_DATE, HDR.SALES_ORD_NUM,HDR.EWAY_BILL_NUM, CAST(HDR.EWAY_BILL_DATE AS DATE), HDR.ACCOUNTING_VOUCHER_NUM,"
				...
				+ "ITM.USERDEFINED_FIELD13,ITM.USERDEFINED_FIELD14, ITM.USERDEFINED_FIELD15,ITM.USERDEFINED_FIELD16, ITM.USERDEFINED_FIELD17,"
				+ "ITM.USERDEFINED_FIELD18, ITM.USERDEFINED_FIELD19, ITM.USERDEFINED_FIELD20, ITM.USERDEFINED_FIELD21,ITM.USERDEFINED_FIELD22, "
				+ "ITM.USERDEFINED_FIELD23,ITM.USERDEFINED_FIELD24, ITM.USERDEFINED_FIELD25, ITM.USERDEFINED_FIELD26, ITM.USERDEFINED_FIELD27,"
				+ "ITM.USERDEFINED_FIELD28, ITM.USERDEFINED_FIELD29,ITM.USERDEFINED_FIELD30, HDR.SUPPLY_TYPE AS SUPPLYTYPEASP, "
				+ "EWB. ASP_DISTANCE AS APPROXIMATEDISTANCE_ASP, EWB.REMAINING_DISTANCE AS DISTANCESAVEDTOEWB, HDR.CREATED_BY AS USER_ID,HDR.FILE_ID, "
				+ "FIL.FILE_NAME AS FILE_NAME,HDR.INV_OTHER_CHARGES AS INVOICEOTHERCHARGESASP, HDR.INV_ASSESSABLE_AMT AS INVOICEASSESSABLEAMOUNTASP, "
				+ "HDR.INV_IGST_AMT AS INVOICEIGSTAMOUNTASP,HDR.INV_CGST_AMT AS INVOICECGSTAMOUNTASP, HDR.INV_SGST_AMT AS INVOICESGSTAMOUNTASP,"
				+ "HDR.INV_CESS_ADVLRM_AMT AS INVOICECESSADVALOREMAMOUNTASP, HDR.INV_CESS_SPECIFIC_AMT AS INVOICECESSSPECIFICAMOUNTASP, "
				+ "HDR.INV_STATE_CESS_AMT AS INVOICESTATECESSADVALOREMAMOUNTASP, HDR.INV_STATE_CESS_SPECIFIC_AMOUNT AS INVOICESTATECESSSPECIFICAMOUNTASP, "
				+ "HDR.DOC_AMT, HDR.MEMO_VALUE_IGST,HDR.MEMO_VALUE_CGST, HDR.MEMO_VALUE_SGST, ITM.MEMO_VALUE_CESS_ADV AS CESSADVALOREMAMOUNT_ASP, "
				+ "ITM.MEMO_VALUE_STATE_CESS_ADV AS STATECESSADVALOREMAMOUNT_ASP, ITM.IGST_RET1_IMPACT, ITM.CGST_RET1_IMPACT,ITM.SGST_RET1_IMPACT, "
				+ "ITM.CESS_ADV_RET1_IMPACT AS CESSADVALOREMAMOUNT_DIFFERENCE, ITM.STATE_CESS_ADV_RET1_IMPACT AS STATECESSADVALOREMAMOUNT_DIFFERENCE, "
				+ "CASE WHEN HDR.IS_DELETE = TRUE THEN 'INACTIVE' WHEN HDR.IS_DELETE = FALSE THEN 'ACTIVE'END AS RECORD_STATUS, "
				...
				+ "TO_CHAR(HDR.EWB_DATE_RESP,'DD-MM-YYYY HH24:MI:SS') AS EWB_DATE_TIME, ITM.ADJ_REF_NO,ITM.ADJ_REF_DATE, ITM.ADJ_TAXABLE_VALUE, "
				...
				+ "ITM.TCS_AMT AS TCSAMOUNT, HDR.CUST_SUPP_ADDRESS3, HDR.TAX_DOC_TYPE AS DATA_CATEGORY,ITM.ITM_UQC,ITM.ITM_QTY,"
				//+ " CASE WHEN PSD.USER_RESPONSE='S' "
				//+ " THEN 'S-Save to GSTN' WHEN PSD.USER_RESPONSE ='N' THEN 'N-Do not save to GSTN'  "
				//+ " ELSE '' END "
				+ "NULL AS EINVVSGSTR1RESPONSE "
				+ "FROM ANX_OUTWARD_DOC_HEADER_1A HDR INNER JOIN ANX_OUTWARD_DOC_ITEM_1A ITM ON HDR.ID = ITM.DOC_HEADER_ID "
				+ "AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN "
				+ "EINV_MASTER EINV ON HDR.IRN_RESPONSE = EINV.IRN "
				//+ "LEFT OUTER JOIN TBL_EINV_RECON_RESP_PSD PSD ON PSD.DOC_HEADER_ID=ITM.DOC_HEADER_ID "
				//+ " AND PSD.IS_DELETE=FALSE AND PSD.IS_PROCESSED=TRUE "
				+ " LEFT OUTER JOIN EWB_MASTER EWB ON HDR.EWB_NO_RESP = EWB.EWB_NUM "
				+ "WHERE HDR.ASP_INVOICE_STATUS = 2 AND COMPLIANCE_APPLICABLE=TRUE "
				+ "AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE NOT IN ('B2CS','NILEXTNON') "
