...
@Service("GstrB2CLARespHandler")
public class GstrB2CLARespHandler {
	...
	public JsonElement handleB2CLAResp(List<? extends Gstr1SummaryDto> list,
			Gstr1SummaryReqDto gstr1SummaryRequest) {
		
		List<Gstr1BasicSummarySectionDto> b2claEySummary = new ArrayList<>();
		for (Gstr1SummaryDto gstr1Summary : list) {
			b2claEySummary = gstr1Summary.getBcla().getEySummary();
		}
		...
		for (Gstr1SummaryDto gstr1Summary : gstnListResp) {
			gstnSummary = gstr1Summary.getBcla().getGstnSummary();
		}
...
*/				
				...
				.b2claStructure(b2claEySummary,gstnList);
