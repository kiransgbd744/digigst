 ...
 * 
 */
@Component("Gstr1SecHsnSummaryDaoImpl")
public class Gstr1SecHsnSummaryDaoImpl /* implements Gstr1AspHsnSummaryDao */ {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1SecHsnSummaryDaoImpl.class);
	...
	public List<Gstr1AspVerticalHsnDto> getGstr1RSReports(
		...
		if ("A".equalsIgnoreCase(supplyType)) {
			...
			buildSupplyType
				...
				buildHdr1SupplyType.append(
						" OR HDR.SUPPLY_TYPE NOT IN ('NON','SCH3','EXT') ");
		...
		if (APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
			queryStr = createHsnSummaryQueryStringForGstr1a(
					buildQuery.toString(), buildHeader.toString(),
					buildheader.toString(), supplyTypeQuery, buildHdrSupplyType,
					docTypeQuery);

		} else {
			queryStr = createHsnSummaryQueryString(buildQuery.toString(),
					buildHeader.toString(), buildheader.toString(),
					supplyTypeQuery, buildHdrSupplyType, docTypeQuery,
					multiSupplyTypeAns);
	...
	private Gstr1AspVerticalHsnDto convertProcessedHsn(Object[] arr,
		...
		/*
		 * BigDecimal bigDecimalTotalQuantity = (BigDecimal) arr[6]; if
		 * (bigDecimalTotalQuantity != null) { BigDecimal total = new
		 * BigDecimal( bigDecimalTotalQuantity.doubleValue());
		 * obj.setTotalQuantity(total); }
		 */

		String hsnCheck = hsnCode != null && hsnCode.trim().length() > 1
	...
	private static String createHsnSummaryQueryString(String buildQuery,
		...
		bufferString.append(
			...
			bufferString.append(
					" ITM.SUPPLY_TYPE  AS SUPPLY_TYPE,DOC_TYPE, ITM.ITM_HSNSAC,IFNULL(ITM.ITM_UQC,'OTH') AS ITM_UQC,(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) SAVE_STATUS, ");
		...
		// IFNULL(ONB_LINE_ITEM_AMT,0) AS TOTAL_VALUE, ");
			...
			bufferString.append(
					" CASE WHEN 'B' = (SELECT MAX(ANSWER) FROM ENTITY_CONFG_PRMTR WHERE QUESTION_CODE='O26' AND ENTITY_ID =:entityId AND  IS_DELETE=FALSE ) AND DOC_TYPE='CR' AND IFNULL(CRDR_REASON,'NA')  <>'SR' THEN 0  ");
			bufferString.append(
					" WHEN 'B' = (SELECT MAX(ANSWER) FROM ENTITY_CONFG_PRMTR WHERE QUESTION_CODE='O26' AND ENTITY_ID =:entityId AND  IS_DELETE=FALSE ) AND DOC_TYPE='DR' AND IFNULL(CRDR_REASON,'NA')  <>'SR' THEN 0  ");
		...
		bufferString.append(
			...
			bufferString
					.append(" ITM.ITM_TAX_DOC_TYPE  AS TAX_DOC_TYPE,"
							+ "  ITM.ITM_TABLE_SECTION  AS TABLE_SECTION,ONB_LINE_ITEM_AMT ,ITM.OTHER_VALUES,TAX_RATE FROM ANX_OUTWARD_DOC_HEADER ");
		...
		bufferString.append(" WHERE ASP_INVOICE_STATUS = 2 ");
		bufferString.append(
		...
		bufferString.append(
				"+IFNULL(CESS_AMT,0) ONB_LINE_ITEM_AMT,0.00 OTHER_VALUES,NEW_RATE TAX_RATE  FROM GSTR1_PROCESSED_B2CS ");
		bufferString.append(
		...
		bufferString
				.append(" FROM GSTR1_SUMMARY_NILEXTNON HDR LEFT OUTER JOIN ");
		bufferString.append(
		...
		bufferString.append(
			...
			bufferString.append(
					"GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION,TAX_RATE (SELECT  ITM.ITM_TAX_DOC_TYPE AS TAX_DOC_TYPE,HDR.DERIVED_RET_PERIOD,");
		...
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,NEW_RATE TAX_RATE FROM GSTR1_PROCESSED_B2CS HDR LEFT OUTER JOIN FILE_STATUS FIL ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,0.00 TAX_RATE  FROM GSTR1_SUMMARY_NILEXTNON HDR LEFT OUTER JOIN ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,TAX_RATE  FROM GSTR1_USERINPUT_HSNSAC HSN LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ");
		bufferString.append(
	...
	private static String createHsnSummaryQueryStringForGstr1a(
			String buildQuery, String buildHeader, String buildheader,
			String supplyTypeQuery, String buildHdrSupplyType,
			String docTypeQuery) {
