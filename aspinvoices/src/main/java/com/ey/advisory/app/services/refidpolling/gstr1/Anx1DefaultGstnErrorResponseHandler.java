 ...
 */
@Slf4j
@Service("Anx1DefaultGstnErrorResponseHandler")
public class Anx1DefaultGstnErrorResponseHandler
		implements Anx1GstnResponseHandler {
	...
	public void handleResponse(JsonElement root, Gstr1SaveBatchEntity batch) {
		...
		if (batch.getSection() != null && batch.getReturnType() != null) {
			...
			if (SECTIONS.contains(batch.getSection().toLowerCase())) {
				...
				anx1B2cProcRepository.updateIserrorfalg(batch.getId());
				docRepo.markDocsAsErrorForBatch(batch.getId(), now);
				b2cRepo.updateIserrorfalg(batch.getId());
			} else if (batch.getSection().equalsIgnoreCase(APIConstants.REV)) {

				anx1RevProcRepository.updateIserrorfalg(batch.getId());
				inDocRepo.updateIserrorfalg(batch.getId());
				table3h3i.updateIserrorfalg(batch.getId());
			} else if (batch.getSection().equalsIgnoreCase(APIConstants.IMPS)) {

				anx1ImpsProcRepository.updateIserrorfalg(batch.getId());
				inDocRepo.updateIserrorfalg(batch.getId());
				table3h3i.updateIserrorfalg(batch.getId());
			} else if (batch.getSection().equalsIgnoreCase(APIConstants.ECOM)) {

				anx1EcomProcRepository.updateIserrorfalg(batch.getId());
				docRepo.markDocsAsErrorForBatch(batch.getId(),now);
				b2cRepo.updateIserrorfalg(batch.getId());
				table4Repo.updateIserrorfalg(batch.getId());
