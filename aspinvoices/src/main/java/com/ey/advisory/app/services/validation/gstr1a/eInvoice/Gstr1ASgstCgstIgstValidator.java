 ...
 *
 */
@Component("Gstr1ASgstCgstIgstValidator")
public class Gstr1ASgstCgstIgstValidator
	...
	private static final String INTRA_STATE_ERR_MSG = "IGST cannot be applied in case of INTRA state supply";

	private static final String INTER_STATE_ERR_MSG = "CGST / SGST cannot be applied in case of INTER state supply";
	...
	public List<ProcessingResult> validate(Gstr1AOutwardTransDocument document,
		...
		for (int idx = 0; idx < items.size(); idx++) {
			...
			if (isIntra && igstExists(item)) {
				if (GSTConstants.Y
						.equalsIgnoreCase(document.getSection7OfIgstFlag()))
					return errors;
				/*
				...
				ProcessingResult res = createErrorsForFields(idx,
						INTRA_STATE_ERR_CODE, INTRA_STATE_ERR_MSG, IGST_AMOUNT);
				errors.add(res);
			}

			if (!isIntra && cgstOrSgstExists(item)) {
				ProcessingResult res = createErrorsForFields(idx,
						INTER_STATE_ERR_CODE, INTER_STATE_ERR_MSG, CGST_AMOUNT,
						SGST_AMOUNT);
				errors.add(res);
			}
		}
	...
	private boolean sgstinExists(Gstr1AOutwardTransDocument doc) {
		return (doc.getSgstin() != null) && (!doc.getSgstin().isEmpty());
	}

	private boolean posExists(Gstr1AOutwardTransDocument doc) {
		return (doc.getPos() != null) && (!doc.getPos().isEmpty());
	}

	private boolean supplyTypeExists(Gstr1AOutwardTransDocument doc) {
	...
	private boolean isTaxOrDxp(Gstr1AOutwardTransDocument doc) {
	...
	private boolean igstExists(Gstr1AOutwardTransDocLineItem item) {
	...
	private boolean cgstOrSgstExists(Gstr1AOutwardTransDocLineItem item) {
