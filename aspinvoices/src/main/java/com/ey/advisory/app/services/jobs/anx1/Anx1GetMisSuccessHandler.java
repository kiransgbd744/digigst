...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1MisInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetMisSuccessHandler")
@Slf4j
public class Anx1GetMisSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Anx1MisDataParserImpl")
	private Anx1MisDataParser anx1MisDataParser;

	@Autowired
	@Qualifier("GetAnx1MisInvoicesRepository")
	private GetAnx1MisInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetMisSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1MisInvoicesHeaderEntity> b2cEntities = anx1MisDataParser
						.parseMisData(dto, apiResp);
				if (!b2cEntities.isEmpty()) {
					headerRepo.softlyDeleteMisHeader(dto.getGstin(),
							dto.getReturnPeriod());
