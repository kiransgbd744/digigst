 ...
 *
 */
public class Gstr6OrgCgstinPosValidation
	...
	public List<ProcessingResult> validate(
		...
		if (document.getOrgCustGstin() == null
				|| document.getOrgCustGstin().isEmpty())
			return errors;

		stateCache = StaticContextHolder.getBean("DefaultStateCache",
				StateCache.class);

		String statecode = document.getOrgCustGstin().substring(0, 2);
		...
		if (n <= 0) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER3012",
