...
@Service("Anx1ApiTotalReportsServiceImpl")
	public class Anx1ApiTotalReportsServiceImpl
	implements Anx1ApiTotalReportsService {

private static final Logger LOGGER = LoggerFactory
		.getLogger(Anx1ApiTotalReportsServiceImpl.class);

@Autowired
CommonUtility commonUtility;

@Autowired
@Qualifier("Anx1ApiTotalReportsDaoImpl")
private Anx1ApiTotalReportsDao anx1ApiTotalReportsDao;
...
public Workbook findApiTotalRec(SearchCriteria criteria,
	...
	List<Anx1ApiTotalRecordsDto> responseFromView = new ArrayList<>();
	responseFromView = anx1ApiTotalReportsDao
			.getApiTotalReports(request);

	if (responseFromView != null && responseFromView.size() > 0) {
		String[] invoiceHeaders = commonUtility
				.getProp("anx1.api.totrec.report.headers").split(",");
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ANX1_APITotalRecReport.xlsx");
