...
@Component("GoodsServicesCancelPDFDaoImpl")
@Slf4j
public class GoodsServicesCancelPDFDaoImpl implements GoodsServicesCancelPDF {
	...
	public JasperPrint generateCancelPdfReport(String id, String docNo,
		...
		String source = "jasperReports/GoodsInvoice (004).jrxml";
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Inside GoodsServicesCancelPDFDaoImpl for docNo: %s", docNo);
			LOGGER.debug(msg);
		}
		...
		try {
			...
			if (docCategory != null
				...
				case "CMB": {
					...
					/*
					 * String stateName =
					 * statecodeRepository.findStateNameByCode(out.getSgstin().
					 * substring (0, 2)); String placeOfSupply1 =
					 * out.getSgstin().substring(0, 2) + "-" + stateName; String
					 * supplierState = (out.getSupplierPincode() != null ?
					 * placeOfSupply1 + "," +
					 * out.getSupplierPincode().toString() : "");
					 * parameters.put("SupplierStateCodeNamePincode",
					 * supplierState);
					...
					/*
					 * String cgstinState =
					 * statecodeRepository.findStateNameByCode(out.getCgstin().
					 * substring (0, 2));
					 */
					String placeOfSupply02 = "";
					String customerState0 = "";
			...
			for (OutwardTransDocLineItem line : lineItems) {
				...
				productDetails
						.setInvoicestatecesscamount(stateCessAmount != null
			...
			if ((StringUtils.isEmpty(irn) && DOC_TYPE
					...
					|| ((Strings.isNullOrEmpty(out.getCgstin())
							|| GSTConstants.URP
									.equalsIgnoreCase(out.getCgstin()))
							&& GSTConstants.BOS
			...
			} else if (StringUtils.isEmpty(irn)
					&& DOC_TYPE
							.contains(trimAndConvToUpperCase(out.getDocType()))
					&& (!Strings.isNullOrEmpty(out.getCgstin()))
					&& (out.getCgstin().length() == 15)

					&& CGSTIN_FORMAT
							.contains(out.getCgstin().substring(12, 14))) {
