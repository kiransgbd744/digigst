...
@Service("Anx1SumLevelServiceImpl")
public class Anx1SumLevelServiceImpl implements Anx1TransacttionalLevelService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1SumLevelServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1SummaryLevelSummaryDaoImpl")
	...
	public Workbook findTransLevelSummary(SearchCriteria criteria,
		...
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ANX-1_GSTN_Summary Tables_SavedSubmitted.xlsx");

		if (responseFromTrans != null && responseFromTrans.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.Sum.saved.report.headers").split(",");
