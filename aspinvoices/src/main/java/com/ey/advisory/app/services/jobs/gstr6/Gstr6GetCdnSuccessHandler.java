 ...
 *
 */
@Service("Gstr6GetCdnSuccessHandler")
@Slf4j
public class Gstr6GetCdnSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr6CdnDataParserImpl")
	private Gstr6CdnGetDataParser gstr6CdnGetDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GET Call is Success for the batchId {} inside Gstr6GetCdnSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				if (APIConstants.CDN.equalsIgnoreCase(dto.getType())) {
					cdnHeaderRepo.softlyDeleteCdnHeader(dto.getGstin(), dto.getReturnPeriod(), now);
				}
			}
			...
			resultIds.forEach(id -> {
				...
				if (APIConstants.CDN.equalsIgnoreCase(dto.getType())) {
					List<GetGstr6CdnHeaderEntity> cdnEntities = gstr6CdnGetDataParser.parseCdnGetData(dto, apiResp,
							dto.getType(), ctxParamsObj.get("batchId").getAsLong(), now);
					if (cdnEntities != null && !cdnEntities.isEmpty()) {
						cdnHeaderRepo.saveAll(cdnEntities);
					}
				}
