...
@Slf4j
@Component("DefaultItc04BatchIdPollingManager")
public class DefaultItc04BatchIdPollingManager implements SaveBatchIdPollingManager{
	...
	@Autowired
	@Qualifier("DefaultItc04GstnResponseHandler")
	private Itc04GstnResponseHandler itc04GstnResponseHandler;
	...
	public ReturnStatusRefIdDto processBatch(Gstr1SaveBatchEntity batch) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Pooling Itc04 ReturnStatus- Sandbox response is {} ",
					resp);
		}
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Pooling Itc04 ReturnStatus Sandbox status is {} ",
						gstnStatus);
			}
			...
			if (P.equalsIgnoreCase(gstnStatus)) {
				...
				saveToGstnEventStatus.Itc04EventEntry(batch.getReturnPeriod(),
						batch.getSgstin(), 40, batch.getGroupCode());
				itc04GstnResponseHandler.phandleResponse(null, batch);
				...
				itc04GstnResponseHandler.pEhandleResponse(jsonRoot, batch);
				...
				itc04GstnResponseHandler.ehandleResponse(null, batch);
			}else {				
				LOGGER.error("Pooling Itc04 ReturnStatus- intermediate status"
						+ gstnStatus);
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Pooling Itc04 ReturnStatus- IP/REC response handling started for {} ",
							batch.getSection());
				}
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Pooling Itc04 ReturnStatus Processing Completed.");
			}
		} catch (Exception ex) {			
			batchSaveStatusRepository.updateStatusById(batch.getId(),
					APIConstants.POLLING_FAILED,now);
