 ...
 */
@Slf4j
@Component("BasicEcomSupScreenSectionDaoImpl")
public class BasicEcomSupScreenSectionDaoImpl
	...
	public List<Gstr1SummarySectionDto> loadBasicSummarySection(
		...
		if (taxPeriodFrom != 0 && taxPeriodTo != 0) {
			build.append(" AND DERIVED_RET_PERIOD BETWEEN :taxPeriodFrom AND :taxPeriodTo ");
			buildHdr.append(" AND HDR.DERIVED_RET_PERIOD BETWEEN :taxPeriodFrom AND :taxPeriodTo ");
			buildVertical.append(" AND DERIVED_RET_PERIOD BETWEEN :taxPeriodFrom AND :taxPeriodTo ");
	...
	if(!Strings.isNullOrEmpty(request.getReturnType()) && APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())){		
		...
		else
		{
			...
			LOGGER.debug(" INSIDE GSTR1 block ");
	 queryStr = createQueryString(buildQuery, buildQueryHdr,
				buildQuery2);
	 ...
	 * @return
	 */
	private static String createQueryString(String buildQuery1,
		...
		LOGGER.debug("Outward Query Execution BEGIN ");
			String queryStr = "SELECT TABLE_SECTION, COUNT(DISTINCT ECOM_GSTIN) RECORD_COUNT, IFNULL( SUM( "
					...
					+ " FROM (SELECT CASE WHEN  ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C') AND "
					+ " UPPER(HDR.TCS_FLAG) = 'Y' AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN '14(i)' "
					+ "ELSE  ITM.ITM_TABLE_SECTION END AS TABLE_SECTION, "
					+ " HDR.ECOM_GSTIN, DOC_TYPE, HDR.TCS_FLAG,  "
					+ "  IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C')  "
					+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.TAXABLE_VALUE WHEN UPPER(HDR.TCS_FLAG) = 'E'  "
					+ " AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.TAXABLE_VALUE ELSE 0 END,0) AS TAXABLE_VALUE,  "
					+ " IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y'  AND ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C')  "
					+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.IGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E'  "
					+ " AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.IGST_AMT ELSE 0 END,0) AS IGST_AMT,  "
					+ " IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C')  "
					+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.SGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E'  "
					+ "  AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.SGST_AMT ELSE 0 END,0) AS SGST_AMT,  "
					+ "  IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C') "
					+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.CGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E'  "
					+ " AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.CGST_AMT ELSE 0 END,0) AS CGST_AMT,  "
					+ " IFNULL(IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' "
					+ " AND ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C') "
					+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.CESS_AMT_SPECIFIC  "
					+ "  WHEN UPPER(HDR.TCS_FLAG) = 'E' AND ITM.ITM_TABLE_SECTION = '14(ii)'  "
					+ " THEN ITM.CESS_AMT_SPECIFIC ELSE 0 END,0)+ IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND  "
					+ " ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C')  "
					+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.CESS_AMT_ADVALOREM WHEN UPPER(HDR.TCS_FLAG) = 'E'  "
					+ " AND ITM.ITM_TABLE_SECTION = '14(ii)' "
					...
					+ " AND ITM.ITM_TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C', '14(ii)') AND "
					+ buildQueryHdr + " AND DOC_TYPE IN ('INV', 'DR', 'CR') ) "
					+ "  WHERE TABLE_SECTION IN ('14(i)','14(ii)')  GROUP BY TABLE_SECTION "
					+ " UNION ALL"
					+ " SELECT  ITM.ITM_TABLE_SECTION AS TABLE_SECTION, COUNT( DISTINCT CASE WHEN ITM.ITM_TABLE_SECTION = '15(i)' THEN DOC_KEY WHEN ITM.ITM_TABLE_SECTION = '15(ii)' THEN IFNULL(HDR.ECOM_GSTIN, '')|| '|' || IFNULL(HDR.POS, '')|| '|' || IFNULL(ITM.TAX_RATE, 9999) WHEN ITM.ITM_TABLE_SECTION = '15(iii)' THEN DOC_KEY WHEN ITM.ITM_TABLE_SECTION = '15(iv)' THEN IFNULL(HDR.POS, '')|| '|' || IFNULL(ITM.TAX_RATE, 9999) END ) AS RECORD_COUNT, "
					...
					+ "AND RETURN_TYPE = 'GSTR1' AND IS_DELETE = FALSE AND IS_PROCESSED = TRUE "
					+ "AND ITM.ITM_TABLE_SECTION IN ( '15(i)', '15(ii)', '15(iii)', '15(iv)' ) AND "
					+ buildQueryHdr + " GROUP BY  ITM.ITM_TABLE_SECTION  "
					+ "UNION ALL SELECT '15' AS TABLE_SECTION, "
					+ "COUNT(DISTINCT CASE WHEN ITM.ITM_TABLE_SECTION = '15(i)' THEN DOC_KEY WHEN ITM.ITM_TABLE_SECTION = '15(ii)' THEN IFNULL(HDR.ECOM_GSTIN, '')|| '|' || IFNULL(HDR.POS, '')|| '|' || IFNULL(ITM.TAX_RATE, 9999) WHEN ITM.ITM_TABLE_SECTION = '15(iii)' THEN DOC_KEY WHEN ITM.ITM_TABLE_SECTION = '15(iv)' THEN IFNULL(HDR.POS, '')|| '|' || IFNULL(ITM.TAX_RATE, 9999) END) AS RECORD_COUNT,"
					+ " SUM(IFNULL(CASE WHEN HDR.DOC_TYPE='CR' THEN -1*ITM.TAXABLE_VALUE ELSE ITM.TAXABLE_VALUE END, 0)) AS TAXABLE_VALUE,"
					...
					+ " SUM(IFNULL(CASE WHEN HDR.DOC_TYPE='CR' THEN -1*ITM.CGST_AMT ELSE ITM.CGST_AMT END, 0)) AS CGST_AMT,"
					+ " SUM(IFNULL(CASE WHEN HDR.DOC_TYPE='CR' THEN -1*ITM.CESS_AMT_SPECIFIC ELSE ITM.CESS_AMT_SPECIFIC END, 0)+IFNULL(CASE WHEN DOC_TYPE='CR' THEN -1*ITM.CESS_AMT_ADVALOREM ELSE ITM.CESS_AMT_ADVALOREM END, 0)) AS CESS_AMT "
					+ "FROM ANX_OUTWARD_DOC_HEADER HDR INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID WHERE ASP_INVOICE_STATUS = 2 AND COMPLIANCE_APPLICABLE = TRUE AND RETURN_TYPE = 'GSTR1' AND IS_DELETE = FALSE AND IS_PROCESSED = TRUE AND "
					+ " ITM.ITM_TABLE_SECTION IN ( '15(i)', '15(ii)', '15(iii)', '15(iv)' ) AND "
					+ buildQueryHdr
					+ "UNION ALL SELECT '14' AS TABLE_SECTION, "
					...
					+  "SUM(CASE WHEN DOC_TYPE IN ('CR') THEN CESS_AMT END), 0) AS CESS_AMT FROM (SELECT CASE WHEN ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
					+ "AND UPPER(HDR.TCS_FLAG) = 'Y' AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN '14(i)' ELSE TABLE_SECTION END AS TABLE_SECTION, HDR.ECOM_GSTIN, DOC_TYPE, HDR.TCS_FLAG, "
					+  "IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.TAXABLE_VALUE WHEN UPPER(HDR.TCS_FLAG) = 'E' "
					+  "AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.TAXABLE_VALUE ELSE 0 END, 0) AS TAXABLE_VALUE, IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
					+  "AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.IGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E' AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.IGST_AMT ELSE 0 END, 0) AS IGST_AMT, "
					+  "IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.SGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E' "
					+  "AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.SGST_AMT ELSE 0 END, 0) AS SGST_AMT, IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
					+  "AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.CGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E' AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.CGST_AMT ELSE 0 END, 0) AS CGST_AMT, "
					+  "IFNULL(IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.CESS_AMT_SPECIFIC WHEN UPPER(HDR.TCS_FLAG) = 'E' "
					+   "AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.CESS_AMT_SPECIFIC ELSE 0 END, 0)+ IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
					+   "AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.CESS_AMT_ADVALOREM WHEN UPPER(HDR.TCS_FLAG) = 'E' AND ITM.ITM_TABLE_SECTION = '14(ii)' THEN ITM.CESS_AMT_ADVALOREM ELSE 0 END, 0), 0) AS CESS_AMT "
					+   "FROM ANX_OUTWARD_DOC_HEADER HDR INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID WHERE ASP_INVOICE_STATUS = 2 AND COMPLIANCE_APPLICABLE = TRUE "
					+   "AND RETURN_TYPE = 'GSTR1' AND IS_DELETE = FALSE AND IS_PROCESSED = TRUE AND ITM.ITM_TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C', '14(ii)') AND "
					+ buildQueryHdr 
					+ "AND DOC_TYPE IN ('INV', 'DR', 'CR'))" 
	...
	public static void main(String[] args) {
		System.out.println(createQueryString("buildQuery1", "buildQueryHdr", "buildQuery2"));
	}
	...
	//TODO
			private static String createGstr1AQueryString(String buildQuery1,
				...
				String queryStr = "SELECT TABLE_SECTION, COUNT(DISTINCT ECOM_GSTIN) RECORD_COUNT, IFNULL( SUM( "
						...
						+ " HDR.ECOM_GSTIN, DOC_TYPE, HDR.TCS_FLAG,  "
						+ "  IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A','5A','7A(1)','7B(1)','9B','6B','6C')  "
						+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.TAXABLE_VALUE WHEN UPPER(HDR.TCS_FLAG) = 'E'  "
						...
						+ " AND IFNULL(HDR.ECOM_GSTIN,'')<>'' THEN ITM.CESS_AMT_SPECIFIC  "
						+ "  WHEN UPPER(HDR.TCS_FLAG) = 'E' AND HDR.TABLE_SECTION = '14(ii)'  "
						+ " THEN ITM.CESS_AMT_SPECIFIC ELSE 0 END,0)+ IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND  "
						...
						+ "AND UPPER(HDR.TCS_FLAG) = 'Y' AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN '14(i)' ELSE TABLE_SECTION END AS TABLE_SECTION, HDR.ECOM_GSTIN, DOC_TYPE, HDR.TCS_FLAG, "
						+  "IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.TAXABLE_VALUE WHEN UPPER(HDR.TCS_FLAG) = 'E' "
						+  "AND HDR.TABLE_SECTION = '14(ii)' THEN ITM.TAXABLE_VALUE ELSE 0 END, 0) AS TAXABLE_VALUE, IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
						+  "AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.IGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E' AND HDR.TABLE_SECTION = '14(ii)' THEN ITM.IGST_AMT ELSE 0 END, 0) AS IGST_AMT, "
						+  "IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.SGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E' "
						+  "AND HDR.TABLE_SECTION = '14(ii)' THEN ITM.SGST_AMT ELSE 0 END, 0) AS SGST_AMT, IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
						+  "AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.CGST_AMT WHEN UPPER(HDR.TCS_FLAG) = 'E' AND HDR.TABLE_SECTION = '14(ii)' THEN ITM.CGST_AMT ELSE 0 END, 0) AS CGST_AMT, "
						+  "IFNULL(IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.CESS_AMT_SPECIFIC WHEN UPPER(HDR.TCS_FLAG) = 'E' "
						+   "AND HDR.TABLE_SECTION = '14(ii)' THEN ITM.CESS_AMT_SPECIFIC ELSE 0 END, 0)+ IFNULL(CASE WHEN UPPER(HDR.TCS_FLAG) = 'Y' AND HDR.TABLE_SECTION IN ('4A', '5A', '7A(1)', '7B(1)', '9B', '6B', '6C') "
						+   "AND IFNULL(HDR.ECOM_GSTIN, '')<> '' THEN ITM.CESS_AMT_ADVALOREM WHEN UPPER(HDR.TCS_FLAG) = 'E' AND HDR.TABLE_SECTION = '14(ii)' THEN ITM.CESS_AMT_ADVALOREM ELSE 0 END, 0), 0) AS CESS_AMT "
