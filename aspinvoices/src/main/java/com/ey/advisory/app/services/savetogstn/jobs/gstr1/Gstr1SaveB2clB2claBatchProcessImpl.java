 ...
 */
@Slf4j
@Service("saveB2clB2claBatchProcessImpl")
public class Gstr1SaveB2clB2claBatchProcessImpl
	...
	private SaveToGstnBatchRefIds process(
		...
		try {
			...
			Gstr1SaveBatchEntity saveBatch = batchHandler.saveBatch(
					...
					returnType, idsList.size(), operationType, retryCount,
					userRequestId, origin);
	...
	private Pair<List<Long>, SaveGstr1> setChunkCtinData(SaveGstr1 gstr1,
			List<B2CLInvoices> cTinList, String section) {
		...
		if (APIConstants.B2CL.equals(section)) {
			gstr.setB2clInvoice(cTinList);
		} else {
			gstr.setB2claInvoice(cTinList);
		}
		for (B2CLInvoices cData : cTinList) {
			for (B2CLInvoiceData iData : cData.getB2CLInvoiceData()) {
	...
	private Pair<List<Long>, SaveGstr1> setChunkInvData(SaveGstr1 gstr1,
			List<B2CLInvoiceData> invList, String section) {

		SaveGstr1 gstr = new SaveGstr1();
		List<Long> idsList = new ArrayList<>();
		List<B2CLInvoices> cTinList = new ArrayList<>();
		...
		B2CLInvoices cTin = null;
		if (APIConstants.B2CL.equals(section)) {
			gstr.setB2clInvoice(cTinList);
			cTin = gstr1.getB2clInvoice().get(0);
		} else {
			gstr.setB2claInvoice(cTinList);
			cTin = gstr1.getB2claInvoice().get(0);
		}
		cTin.setB2CLInvoiceData(invList);
		cTinList.add(cTin);

		for (B2CLInvoiceData data : invList) {
	...
	private List<Triplet<List<Long>, SaveGstr1, String>> batchChunk(
		...
		int ctinSize = 0;
		if (APIConstants.B2CL.equals(section)) {
			ctinSize = gstr1.getB2clInvoice().size();
		} else {
			ctinSize = gstr1.getB2claInvoice().size();
		}
		Pair<List<Long>, SaveGstr1> pair1 = null;
		Pair<List<Long>, SaveGstr1> pair2 = null;

		if (ctinSize > 1) {
			List<B2CLInvoices> cTinList1 = null;
			List<B2CLInvoices> cTinList2 = null;
			if (APIConstants.B2CL.equals(section)) {
				cTinList1 = gstr1.getB2clInvoice().subList(0, ctinSize / 2);
				cTinList2 = gstr1.getB2clInvoice().subList(ctinSize / 2,
						ctinSize);
			} else {
				cTinList1 = gstr1.getB2claInvoice().subList(0, ctinSize / 2);
				cTinList2 = gstr1.getB2claInvoice().subList(ctinSize / 2,
						ctinSize);
			}
			...
			B2CLInvoices cTin1 = null;
			if (APIConstants.B2CL.equals(section)) {
				cTin1 = gstr1.getB2clInvoice().get(0);
			} else {
				cTin1 = gstr1.getB2claInvoice().get(0);
			}
			int invSize = cTin1.getB2CLInvoiceData().size();
			List<B2CLInvoiceData> invList1 = cTin1.getB2CLInvoiceData()
					.subList(0, invSize / 2);
			List<B2CLInvoiceData> invList2 = cTin1.getB2CLInvoiceData()
