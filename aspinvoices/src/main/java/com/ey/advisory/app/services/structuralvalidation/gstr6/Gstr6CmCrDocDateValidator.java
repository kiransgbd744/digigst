 ...
 *
 */
public class Gstr6CmCrDocDateValidator 
	...
	public List<ProcessingResult> validate(Gstr6DistributionExcelEntity document,
		...
		/*	String docDate1 = document.getOrgCrDate().toString();
			...
			DateTimeFormatter formatter = DateTimeFormatter
					.ofPattern("ddMMyyyy");
			LocalDate docDate = document.getOrgCrDate();
			LocalDate gstStartDate = LocalDate.parse("01072017", formatter);
			...
			if (gstin != null) {
				if (gstin.getRegDate() != null) {
					...
					if ((docDate.compareTo(gstinRegDate) < 0)) {
						...
						errors.add(
								new ProcessingResult(APP_VALIDATION, "ER3055",
										"Original Credit note Date cannot be before "
