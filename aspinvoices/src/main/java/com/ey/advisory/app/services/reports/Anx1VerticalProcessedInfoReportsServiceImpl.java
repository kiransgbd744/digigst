 ...
 *
 */
@Service("Anx1VerticalProcessedInfoReportsServiceImpl")
public class Anx1VerticalProcessedInfoReportsServiceImpl
		implements Anx1VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1VerticalProcessedInfoReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1VerticalProcessedInfoReportsDaoImpl")
	...
	public Workbook findReports(SearchCriteria criteria, PageRequest pageReq) {
		...
		responseFromView = anx1VerticalReportsDao.getVerticalReports(request);
		if (responseFromView != null && responseFromView.size() > 0) {
			...
			if ((DownloadReportsConstant.B2C)
				...
				String[] invoiceHeaders = commonUtility
						.getProp("anx1.vertical.b2c.processedInfo.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_B2CVerticalProcessedInfo.xlsx");
				...
				errorDumpCells.importCustomObjects(responseFromView,
					...
					.equalsIgnoreCase(request.getFileType())) {
				String[] invoiceHeaders = commonUtility
						.getProp("anx1.vertical.3h3i.processedInfo.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_3H3IVerticalProcessInfo.xlsx");
				LOGGER.debug("workbook reading ending");
				...
				String[] invoiceHeaders = commonUtility
						.getProp(
								"anx1.vertical.table4.processedInfo.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_Table4VerticalProcessedInfo.xlsx");
