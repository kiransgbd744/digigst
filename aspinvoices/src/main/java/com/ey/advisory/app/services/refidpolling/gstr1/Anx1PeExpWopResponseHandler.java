 ...
 *
 */
@Component("Anx1PeExpWopResponseHandler")
public class Anx1PeExpWopResponseHandler implements Anx1ResponseHandler{
	...
	public Triplet<List<String>, Map<String, OutwardTransDocError>,Integer> response(
		...
		for (Anx1ExpwpAndExpwopData invoice : anx1ErrorReportData.getExpwopInvoice()) {

			String docNo = invoice.getDoc().getDocNum();

			String idt = invoice.getDoc().getDocDate();
				...
				errObj.setErrorCode(invoice.getErrorDetails().getErrCd());
				errObj.setErrorType(GSTConstants.ERR);
				errObj.setSource(GSTConstants.GSTN_VALIDATION);
				errObj.setErrorDesc(invoice.getErrorDetails().getErrMsg());
		...
		n = anx1ErrorReportData.getExpwpInvoice().size();
