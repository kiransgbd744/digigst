...
@Slf4j
@Service("GSTR2BFailureHandler")
public class GSTR2BFailureHandler implements FailureHandler {
	...
	public void handleFailure(FailureResult result, String apiParams) {
		try {
			...
			if (ERROR_CODES.contains(errorCode)) {
				...
				gstnStatusRepo.updateStatus(batchDto.getSgstin(),
						batchDto.getTaxPeriod(), APIConstants.GSTR2B,
						APIConstants.SUCCESS_WITH_NO_DATA, null, errorDesc,LocalDateTime.now());
			} else {
