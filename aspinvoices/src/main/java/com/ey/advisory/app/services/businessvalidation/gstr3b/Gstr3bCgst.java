...
public class Gstr3bCgst implements BusinessRuleValidator<Gstr3bExcelEntity> {
	...
	public List<ProcessingResult> validate(Gstr3bExcelEntity document,
		...
		String cgst = (document.getCgstAmnt() != null)
				? document.getCgstAmnt().trim() : null;
			...
			if (GSTConstants.desriptionGstr3B.equalsIgnoreCase(description)) {
				...
				if (cgstAmt.compareTo(BigDecimal.ZERO) < 0) {
					...
					errors.add(new ProcessingResult(GSTConstants.APP_VALIDATION,
							"ER6108", "Invalid CGST Amount.", location));
					return errors;
				}
			}
		} else {
