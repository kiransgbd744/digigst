...
@Component("Gstr1CDNRSectionReportDaoImpl")
public class Gstr1CDNRSectionReportDaoImpl
		implements Gstr1ReviewSummaryCustomizedReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1CDNRSectionReportDaoImpl.class);
	...
	public Pair<List<Object[]>, List<Object>> getGstr1RSReports(
		...
		if (plant != null && !plant.isEmpty()) {
			if (plantList != null && plantList.size() > 0) {
				buildHeader.append(" AND ITM.PLANT_CODE IN :plantList");
			}
		}
		...
		{
			LOGGER.debug(" multiSupplyTypeAns {} DB TIME {} ",multiSupplyTypeAns,org.joda.time.LocalDateTime.now());
		}
		...
		if(LOGGER.isDebugEnabled())
		{
			LOGGER.debug(" time taken to load the data from DB  {} ",dbLoadTimeDiff);
		}
		...
		if(LOGGER.isDebugEnabled())
		{
			LOGGER.debug(" time taken to convert the data  {} ",dbLoadTimeDiff);
		}
	...
	private DataStatusEinvoiceDto convertProcessed(Object[] arr) {
		...
		obj.setQuantity(qty != null ? qty : null);
			...
			LOGGER.error("Error occured in convert method {}", e);
	...
	private static String createApiProcessedQueryString(String buildHeader, String multiSupplyTypeAns) {
			...
			return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES, '') || ',' || IFNULL(ITM.ERROR_CODES, '')) AS ERROR_CODE_ASP "
					...
					+ "	AND  ITM.ITM_TAX_DOC_TYPE IN ('CDNR') "
					+ buildHeader
					+ " ORDER BY DOC_NUM "
					+ "	,ITM.ITM_NO";
		
	}
	private static String createApiProcessedQueryStringForGstr1a(String buildHeader) {
		...
		+ "HDR.ACK_NUM AS IRN_ACK_NO, "
		+ "TO_CHAR(HDR.ACK_DATE,'DD-MM-YYYY HH24:MI:SS') "
		...
		+ "end as EWBPartAStatusA,"
		+ "TO_CHAR(EWB.VALID_UPTO,'DD-MM-YYYY HH24:MI:SS') "
		...
		+ "HDR.SUPP_BUILDING_NUM, HDR.SUPP_BUILDING_NAME "
		+ "AS SUPPLIERADDRESS2,HDR.SUPP_LOCATION, "
		...
		+ "TO_CHAR(EINV.CANCELLATION_DATE,'DD-MM-YYYY HH24:MI:SS') "
		+ "AS IRN_CANCELLATION_DATE, "
		+ "TO_CHAR(EWB.CANCELLATION_DATE,'DD-MM-YYYY HH24:MI:SS') "
		+ "AS EWB_CANCELLATION_DATE, "
		...
		+ "AS EWB_DATE_TIME, ITM.ADJ_REF_NO,ITM.ADJ_REF_DATE, "
		...
		//+ "CASE WHEN PSD.USER_RESPONSE='S' THEN 'S-Save to GSTN' WHEN PSD.USER_RESPONSE='N' THEN 'N-Do Not save to GSTN'  ELSE '' END "
		...
		//+ " LEFT OUTER JOIN TBL_EINV_RECON_RESP_PSD PSD ON PSD.DOC_HEADER_ID = ITM.DOC_HEADER_ID AND PSD.IS_DELETE=FALSE AND PSD.IS_PROCESSED=TRUE"
		+ " LEFT OUTER JOIN "
		+ "EWB_MASTER EWB ON HDR.EWB_NO_RESP = EWB.EWB_NUM "
		+ "WHERE HDR.ASP_INVOICE_STATUS = 2 "
		+ "AND COMPLIANCE_APPLICABLE=TRUE AND HDR.IS_DELETE=FALSE "
		+ "AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE IN ('CDNR') "
