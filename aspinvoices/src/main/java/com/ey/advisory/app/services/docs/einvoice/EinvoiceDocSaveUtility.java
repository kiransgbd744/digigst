 ...
 */
@Slf4j
@Service("EinvoiceDocSaveUtility")
public class EinvoiceDocSaveUtility {
	...
	public void updateDocwithGenIrnReq(OutwardTransDocument doc,
		...
		doc.setId(null);
		for (OutwardTransDocLineItem lineItems : doc.getLineItems()) {
			lineItems.setId(null);

			if (!lineItems.getAttribDtls().isEmpty()) {
				for (AttributeDetails attDtls : lineItems.getAttribDtls()) {
					attDtls.setId(null);
				}
			}
		}

		for (PreceedingDocDetails preDoc : doc.getPreDocDtls()) {
			preDoc.setId(null);
		}
		for (ContractDetails conDtls : doc.getContrDtls()) {
			conDtls.setId(null);
		}
		for (AdditionalDocDetails addDtls : doc.getAddlDocDtls()) {
			addDtls.setId(null);
		}
	...
	public void saveEinvoiceEntity(OutwardTransDocument savedoc, Long id) {
	...
	private void saveEwbEntity(OutwardTransDocument savedoc, Long id,
			Long lifeCycleId) {
	...
	public void saveEwbLifeCycleEntity(OutwardTransDocument savedoc, Long id) {
