 ...
 *
 */
@Service("Gstr1B2csB2csaSuccessHandler")
@Slf4j
public class Gstr1B2csB2csaSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("gstr1B2csB2csaDataParserImpl")
	private Gstr1B2csB2csaDataParser gstr1B2csB2csaDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GET Call is Success for the batchId {} inside Gstr1B2csB2csaSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				if (APIConstants.B2CS.equalsIgnoreCase(dto.getType())) {
					gstr1GetB2csGstnRepository.softlyDeleteB2csHeader(dto.getGstin(), dto.getReturnPeriod(), now);
				} else {
					gstr1GetB2csaGstnRepository.softlyDeleteB2csaHeader(dto.getGstin(), dto.getReturnPeriod(), now);
				}
			}
			resultIds.forEach(id -> {
				...
				if (APIConstants.B2CS.equalsIgnoreCase(dto.getType())) {
					List<GetGstr1B2csHeaderEntity> b2csEntities = gstr1B2csB2csaDataParser.parseB2csData(dto, apiResp);
					if (b2csEntities != null && !b2csEntities.isEmpty()) {
						gstr1GetB2csGstnRepository.saveAll(b2csEntities);
					}
				} else {
					List<GetGstr1B2csaHeaderEntity> b2csaEntities = gstr1B2csB2csaDataParser.parseB2csaData(dto,
							apiResp);
					if (b2csaEntities != null && !b2csaEntities.isEmpty()) {
						gstr1GetB2csaGstnRepository.saveAll(b2csaEntities);
					}
				}
