 ...
 *
 */
@Component("EInvManagmntDownloadDaoImpl")
@Slf4j
public class EInvManagmntDownloadDaoImpl implements EInvManagmntDownloadDao {
	...
	@Override
	public List<EinvoiceMangementResponseDto> getEInvMngmtScreendwnld(
			EInvoiceDocSearchReqDto criteria) {
	...
	private EinvoiceMangementResponseDto convertEInv(Object[] arr) {
	...
	private String createEInvdownloadQueryString(String buildQuery) {
		...
		return " SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| "
				+ "IFNULL(ITM.ERROR_CODES,'')) AS ERROR_CODE_ASP, '' AS "
				+ "ERROR_DESCRIPTION_ASP, TRIM(',' FROM IFNULL"
				+ "(HDR.INFORMATION_CODES,'')||','||IFNULL(ITM.INFORMATION_CODES,'')) "
				+ "AS INFO_ERROR_CODE_ASP, '' AS INFO_ERROR_DESCRIPTION_ASP, "
				+ "case when HDR.IRN_STATUS = 1 then 'Not Opted' when "
				+ "HDR.IRN_STATUS = 2 then 'Not Applicable' when "
				+ "HDR.IRN_STATUS = 3 then 'Pending' when HDR.IRN_STATUS = 4 "
				+ "then 'Generation Error' when HDR.IRN_STATUS = 5 then "
				+ "'Generated' when HDR.IRN_STATUS = 6 then 'Cancelled' "
				+ "when HDR.IRN_STATUS = 7 then 'ASP Error' when "
				+ "HDR.IRN_STATUS = 8 then 'Duplicate_Irn' "
				+ "when HDR.IRN_STATUS = 9 then 'In Progress Cancellation,'"
				+ "when HDR.IRN_STATUS = 10 then 'Pushed to NIC' end as IRN_STATUS, "
				+ "HDR.IRN_RESPONSE AS IRN_NO, HDR.ACK_NUM AS IRN_ACK_NO, "
				+ "HDR.ACK_DATE " + "AS IRN_ACK_DATE_TIME,EINV.SIGNED_QR AS "
				+ "SIGNED_QR_CODE, NULL AS SIGNED_INVOICE, "
				+ "HDR.EINV_ERROR_CODE AS IRP_ERROR_CODE, HDR.EINV_ERROR_DESC AS "
				+ "IRP_ERROR_DESCRIPTION, case when HDR.EWB_STATUS = 1 then "
				+ "'Not Applicable' when HDR.EWB_STATUS = 2 then 'Pending' "
				+ "when HDR.EWB_STATUS = 3 then 'Generation Error' when "
				+ "HDR.EWB_STATUS = 4 then 'Part A Generated' when "
				+ "HDR.EWB_STATUS = 5 then 'EWB Active' when HDR.EWB_STATUS = 6 "
				+ "then 'Cancelled' when HDR.EWB_STATUS = 7 then 'Discarded' "
				+ "when HDR.EWB_STATUS = 8 then 'Rejected' when HDR.EWB_STATUS = 9 "
				+ "then 'Expired' when HDR.EWB_STATUS = 10 then 'Pushed to NIC' "
				+ "when HDR.EWB_STATUS = 11 then 'ASP Error' WHEN "
				+ "HDR.EWB_STATUS = 12 then 'Not_Opted' end as EWBPartAStatusA, "
				+ "EWB.VALID_UPTO AS EWBValidUpTo,"
				+ "HDR.EWAY_BILL_NUM AS EWBNO, CAST(HDR.EWAY_BILL_DATE AS "
				+ "DATE) AS EWBDATE, HDR.EWB_ERROR_CODE AS EWB_ERROR_CODE, "
				+ "HDR.EWB_ERROR_DESC AS EWB_ERROR_DESCRIPTION, case when "
				...
				+ "HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END AS GSTIN_STATUS, "
				+ "GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, "
				+ "GSTNBATCH.BATCH_DATE "
				+ "AS GSTIN_REFID_DATE_TIME, HDR.GSTN_ERROR_CODE AS "
				+ "GSTIN_ERROR_CODE, HDR.GSTN_ERROR_DESC AS GSTIN_ERROR_DESCRIPTION_ASP, "
				// + "HDR.RETURN_TYPE AS RET_TYPE,HDR.TABLE_SECTION AS
				// TABLE_NUMBER,"
				+ "CASE WHEN ASP_INVOICE_STATUS = 2 THEN "
				...
				+ "AS TABLE_NUMBER_19,"
				+ " HDR.IRN, HDR.IRN_DATE,HDR.TAX_SCHEME, HDR.CANCEL_REASON, "
				...
				+ "HDR.SUPP_BUILDING_NUM, HDR.SUPP_BUILDING_NAME AS "
				+ "SUPPLIERADDRESS2,HDR.SUPP_LOCATION, HDR.SUPP_PINCODE, "
				...
				+ "ITM.INV_STATE_CESS_SPECIFIC_AMOUNT, CASE WHEN "
				+ "HDR.DATAORIGINTYPECODE IN('A','AI') THEN HDR.DOC_AMT WHEN "
				+ "HDR.DATAORIGINTYPECODE IN ('E','EI') THEN ITM.LINE_ITEM_AMT "
				+ "END LINE_ITEM_AMT, HDR.ROUND_OFF,HDR.TOT_INV_VAL_WORLDS,"
				+ "HDR.TCS_FLAG_INCOME_TAX, ITM.TCS_RATE_INCOME_TAX,"
				+ "ITM.TCS_AMOUNT_INCOME_TAX, HDR.CUSTOMER_PAN_OR_AADHAAR, "
				+ "HDR.FOREIGN_CURRENCY,HDR.COUNTRY_CODE, HDR.INV_VAL_FC, "
				+ "HDR.SHIP_PORT_CODE,HDR.SHIP_BILL_NUM, HDR.SHIP_BILL_DATE, "
				+ "HDR.INV_REMARKS,HDR.INV_PERIOD_START_DATE, HDR.INV_PERIOD_END_DATE,"
				+ " ITM.PRECEEDING_INV_NUM, ITM.PRECEEDING_INV_DATE,ITM.INV_REFERENCE, "
				...
				+ "ITM.GLCODE_TAXABLEVALUE,HDR.GLCODE_IGST,HDR.GLCODE_CGST,"
				+ " HDR.GLCODE_SGST,HDR.GLCODE_ADV_CESS,HDR.GLCODE_SP_CESS,"
				+ " HDR.GLCODE_STATE_CESS, HDR.GL_STATE_CESS_SPECIFIC, "
				...
				+ " HDR.SUPPLY_TYPE AS SUPPLYTYPEASP, EWB. ASP_DISTANCE AS"
				+ " ApproximateDistance_ASP, EWB.REMAINING_DISTANCE AS "
				+ "DistanceSavedtoEWB, HDR.CREATED_BY as USER_ID,HDR.FILE_ID, "
				+ "FIL.FILE_NAME as FILE_NAME,HDR.INV_OTHER_CHARGES as "
				+ "InvoiceOtherChargesASP, HDR.INV_ASSESSABLE_AMT as "
				...
				+ "InvoiceStateCessSpecificAmountASP, HDR.DOC_AMT,"
				...
				+ "EINV.CANCELLATION_DATE AS "
				+ "IRN_CANCELLATION_DATE,EWB.CANCELLATION_DATE AS EWB_CANCELLATION_DATE,"
				+ "CASE WHEN HDR.ASP_INVOICE_STATUS=1 THEN 'ASPERROR' "
				+ "WHEN HDR.ASP_INVOICE_STATUS=2 THEN 'ASPPROCESSED'"
				+ " END AS DIGI_GST_STATUS, HDR.INFO_ERROR_CODE AS EWB_INFO_CODE, "
				+ "HDR.INFO_ERROR_MSG AS EWB_INFO_ERROR_MSG, HDR.EWB_NO_RESP AS "
				+ "EWB_NUM,HDR.EWB_DATE_RESP "
				+ "AS EWB_DATE_TIME,ITM.ADJ_REF_NO,"
				+ "ITM.ADJ_REF_DATE, ITM.ADJ_TAXABLE_VALUE, ITM.ADJ_IGST_AMT,"
				+ "ITM.ADJ_CGST_AMT, ITM.ADJ_SGST_AMT, ITM.ADJ_CESS_AMT_ADVALOREM,"
				+ " ITM.ADJ_CESS_AMT_SPECIFIC, ITM.ADJ_STATECESS_AMT, ITM.TCS_AMT "
				+ "AS Tcsamount, HDR.CUST_SUPP_ADDRESS3, HDR.TAX_DOC_TYPE AS "
				+ "DATA_CATEGORY FROM ANX_OUTWARD_DOC_HEADER HDR "
				+ "INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = "
				+ "ITM.DOC_HEADER_ID AND HDR.DERIVED_RET_PERIOD = "
				+ "ITM.DERIVED_RET_PERIOD LEFT OUTER JOIN FILE_STATUS "
				+ "FIL ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH "
				+ "GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN "
				+ "EINV_MASTER EINV ON HDR.IRN_RESPONSE = EINV.IRN LEFT "
				+ "OUTER JOIN EWB_MASTER EWB ON HDR.EWB_NO_RESP = "
				+ "EWB.EWB_NUM WHERE HDR.IS_DELETE=FALSE AND "
				+ "IS_SUBMITTED = FALSE " + buildQuery
				+ " ORDER BY HDR.ID DESC, ITM.ITM_NO asc ";
