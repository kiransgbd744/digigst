...
@Component("Gstr1ANilNonExptValidationChain")
public class Gstr1ANilNonExptValidationChain
		implements DocRulesValidatorChain<Gstr1ANilNonExemptedAsEnteredEntity> {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1ANilNonExptValidationChain.class);
	...
	private static final B2csBusinessRuleValidator[] VALIDATORS = {
			new Gstr1ANilNonExptSupplierGstin(),
			new Gstr1ANilNonExptReturnPeriod(), new Gstr1ANilNonExptdHsn(),
			new Gstr1ANilNonExptedUQC(),
			new Gstr1ANilNonExptHsnCodeLengthValidation(),
			new Gstr1ANilNonExptGstr1ReturnFiled(),
			new Gstr1ANilNonExptHsnLengthValidation(),
	...
	private static final B2csBusinessRuleValidator[] VALIDATORS_OLD = {
			new Gstr1ANilNonExptSupplierGstin(),
			new Gstr1ANilNonExptReturnPeriod(),
			new Gstr1ANilNonExptGstr1ReturnFiled(),
			new Gstr1ANilNonExptHsnLengthValidation(),
			// new NilNonExptdHsn(),
			// new NilNonExptedUQC(),
	...
	public List<ProcessingResult> validate(
		...
		for (B2csBusinessRuleValidator<Gstr1ANilNonExemptedAsEnteredEntity> validator : VALIDATORS) {
			if (validator == null) {
	...
	public List<ProcessingResult> validateOld(
			...
			ProcessingContext context) {
		List<ProcessingResult> results = new ArrayList<>();
		for (B2csBusinessRuleValidator<Gstr1ANilNonExemptedAsEnteredEntity> validator : VALIDATORS_OLD) {
			if (validator == null) {
				LOGGER.warn("Validator cannot be null. "
						+ "Ignoring the null validator!!");
				continue;
