 ...
 *
 */
@Repository("GetGstr2B2bInvoicesRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface GetGstr2B2bInvoicesRepository
        extends CrudRepository<GetGstr2aB2bInvoicesHeaderEntity, Long> {

    // Curently isDelete is not added in table
    @Modifying
    @Query("UPDATE GetGstr2aB2bInvoicesHeaderEntity b SET b.isDelete = true,b.modifiedBy = 'SYSTEM'  , b.modifiedOn =:modifiedOn  WHERE"
    ...
    @Query("from GetGstr2aB2bInvoicesHeaderEntity where sgstin=:sgstin and cgstin=:cgstin and invDate=:invDate and invNum=:invNum and invType=:invType")
    List<GetGstr2aB2bInvoicesHeaderEntity> findByInvoiceKey(
    ...
    @Query("SELECT COUNT(*) FROM GetGstr2aB2bInvoicesHeaderEntity "
            + "WHERE cgstin=:gstin AND returnPeriod = :taxperiod and isDelete = false")
    public int gstinCount(@Param("gstin") String gstin,
            @Param("taxperiod") String taxperiod);

    @Modifying
    @Query("UPDATE GetGstr2aB2bInvoicesHeaderEntity b SET b.isDelete = true,b.modifiedOn=:modifiedOn  WHERE"
