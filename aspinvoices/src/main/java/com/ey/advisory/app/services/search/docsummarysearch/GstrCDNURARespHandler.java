...
@Service("GstrCDNURARespHandler")
public class GstrCDNURARespHandler {
	...
	public JsonElement handleCDNURAResp(List<? extends Gstr1SummaryDto> list,
			Gstr1SummaryReqDto gstr1SummaryRequest) {
		
		List<Gstr1BasicSummarySectionDto> cdnuraEySummary = new ArrayList<>();
		for (Gstr1SummaryDto gstr1Summary : list) {
			cdnuraEySummary = gstr1Summary.getCdnura().getEySummary();
		}
...
*/				
				...
				JsonElement cdnuraRespBody = gstr1SummaryStructure
				.cdnuraStructure(cdnuraEySummary,gstnList);
	

				
				return cdnuraRespBody;
