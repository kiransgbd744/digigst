...
import static com.ey.advisory.common.GSTConstants.AvailableCess;
...
public class AvailableCess implements DocRulesValidator<InwardTransDocument> {
	...
	if(document.getDocType()!=null && !document.getDocType().isEmpty()){
		if(document.getSupplyType()!=null 
			...
			if((!ORGDOCNUM_REQUIRING_IMPORTS.contains(
				...
				if(item.getAvailableCess().compareTo(BigDecimal.ZERO) < 0){
					
			
			errorLocations.add(AvailableCess);
			TransDocProcessingResultLoc location = 
					                  new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER109",
					"Invalid Available Cess",
					location));
		}
