 ...
 */
@Slf4j
@Service("anx1SaveImpgAndImpgSezBatchProcessImpl")
public class Anx1SaveImpgAndImpgSezBatchProcessImpl implements Anx1SectionWiseSaveBatchProcess {
	...
	private SaveToGstnBatchRefIds process(
		...
		try {
			...
			if (resp.getResponse() != null) {
				 ...
				 */
				batchHandler.updateInwardBatch(gstnBatchId, groupCode, idsList, null, null);
				/**
	...
	/*private Pair<List<Long>, SaveAnx1> setChunkCtinData(SaveAnx1 anx1,
			List<Anx1ImpgAndImpgSezData> cTinList, String section) {
		...
		if (APIConstants.IMPG.equals(section)) {
			anx.setImpgInvoice(cTinList);
		} else if (APIConstants.IMPGSEZ.equals(section)) {
			anx.setImpgsezInvoice(cTinList);
		}

		for (Anx1ImpgAndImpgSezData cData : cTinList) {
			for (Anx1ImpgAndImpgSezDocumentData iData : cData.getDocs()) {
	...
	private Pair<List<Long>, SaveAnx1> setChunkInvData(SaveAnx1 anx1,
			List<Anx1ImpgAndImpgSezDocumentData> invList, String section) {

		SaveAnx1 anx = new SaveAnx1();
		List<Long> idsList = new ArrayList<>();
		List<Anx1ImpgAndImpgSezData> cTinList = new ArrayList<>();

		anx.setSgstin(anx1.getSgstin());
		anx.setTaxperiod(anx1.getTaxperiod());

		Anx1ImpgAndImpgSezData cTin = null;

		if (APIConstants.IMPG.equals(section)) {
			anx.setImpgInvoice(cTinList);
			cTin = anx1.getImpgInvoice().get(0);
		} else if (APIConstants.IMPGSEZ.equals(section)) {
			anx.setImpgsezInvoice(cTinList);
			cTin = anx1.getImpgsezInvoice().get(0);
		}
		cTin.setDocs(invList);
		cTinList.add(cTin);

		for (Anx1ImpgAndImpgSezDocumentData data : invList) {
	...
	private List<Triplet<List<Long>, SaveAnx1, String>> batchChunk(
		...
		int ctinSize = 0;
		if (APIConstants.IMPG.equals(section)) {
			ctinSize = anx1.getImpgInvoice().size();
		} else if (APIConstants.IMPGSEZ.equals(section)) {
			ctinSize = anx1.getImpgsezInvoice().size();
		}
		...
		if (ctinSize > 1) {
			List<Anx1ImpgAndImpgSezData> cTinList1 = null;
			List<Anx1ImpgAndImpgSezData> cTinList2 = null;
			if (APIConstants.IMPG.equals(section)) {
				cTinList1 = anx1.getImpgInvoice().subList(0, ctinSize / 2);
				cTinList2 = anx1.getImpgInvoice().subList(ctinSize / 2, ctinSize);
			} else if (APIConstants.IMPGSEZ.equals(section)) {
				cTinList1 = anx1.getImpgsezInvoice().subList(0, ctinSize / 2);
				cTinList2 = anx1.getImpgsezInvoice().subList(ctinSize / 2, ctinSize);
			}
			...
			Anx1ImpgAndImpgSezData cTin1 = null;
			
			if (APIConstants.IMPG.equals(section)) {
				cTin1 = anx1.getImpgInvoice().get(0);
			} else if (APIConstants.IMPGSEZ.equals(section)) {
				cTin1 = anx1.getImpgsezInvoice().get(0);
			}
			int invSize = cTin1.getDocs().size();
			List<Anx1ImpgAndImpgSezDocumentData> invList1 = cTin1.getDocs().subList(0,
					invSize / 2);
			List<Anx1ImpgAndImpgSezDocumentData> invList2 = cTin1.getDocs()
