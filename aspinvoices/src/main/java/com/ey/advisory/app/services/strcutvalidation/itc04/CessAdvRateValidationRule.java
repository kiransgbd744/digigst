...
public class CessAdvRateValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		Object cessAdvRate = obj.toString().trim();
		if (!isDecimal(cessAdvRate)) {
			Set<String> errorLocations = new HashSet<>(); 
			errorLocations.add(GSTConstants.CESS_RATE_ADV);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER5846",
					"Invalid CessAdvaloremRate.", location));
			// if it's not a number, then we can return the errors
			// immediately
			return errors;
		}
		boolean isValid = NumberFomatUtil.is73digValidDec(cessAdvRate);
		if (!isValid) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.CESS_RATE_ADV);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
