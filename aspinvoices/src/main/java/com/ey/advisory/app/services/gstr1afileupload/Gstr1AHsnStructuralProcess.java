 ...
 *
 */
@Component("Gstr1AHsnStructuralProcess")
@Slf4j
public class Gstr1AHsnStructuralProcess {

	@Autowired
	@Qualifier("Gstr1AHsnBusinessProcess")
	private Gstr1AHsnBusinessProcess gstr1HsnBusinessProcess;
	...
	private Gstr1AUserInputHsnSacRepository gstr1HsnAsUserInputRepository;
	...
	private static final Map<String, String> ERROR_CODE_MAP = new ImmutableMap.Builder<String, String>()
			...
			.put("ER1276", "GSTR1A for this tax period is already filed")
			.put("ER1277", "Invalid RecordType")
			.build();
	...
	public void convertToProcess(Map<String, List<Object[]>> processMapObj,
			...
			Boolean isError) {
		List<Gstr1AUserInputHsnSacEntity> hsnUserInputs = new ArrayList<>();
		try {
			...
			processMapObj.entrySet().forEach(entry -> {
				...
				Gstr1AUserInputHsnSacEntity userInputEntity = new Gstr1AUserInputHsnSacEntity();
				for (Object[] obj : objs) {
					...
					userInputEntity.setUsrQunty(quantity);
					userInputEntity
							.setDerivedRetPeriod(deriRetPeriod.toString());
		...
		try {
			...
			docMapObj.entrySet().forEach(entry -> {
				...
				for (Object[] obj : objs) {
					...
					Gstr1AAsEnteredHsnEntity hsnEntity = new Gstr1AAsEnteredHsnEntity();
	...
	public String generateHsnKey(Object[] arr) {
		...
		if (arr[2] != null && !arr[2].toString().isEmpty()) {
			...
			if (StringUtils.isNumeric(taxPeriod) && taxPeriod.length() == 6) {
				returnPeriod = arr[2] != null ? GenUtil
						.getDerivedTaxPeriod(String.valueOf(arr[2])).toString()
						: "";
			}
		}
		if (arr[2] != null && !arr[2].toString().isEmpty()) {
		...
		/*return new StringJoiner(GSTConstants.WEB_UPLOAD_KEY).add(returnPeriod)
	...
	private void softDelete(List<String> gstinTaxPeriodKey) {
		...
		for (List<String> chunk : chunks) {
			...
			String msg = String.format(
					" %d rows affected in GSTR1A_USERINPUT_HSNSAC table",
					softDeletedCount);
			LOGGER.debug(msg);
