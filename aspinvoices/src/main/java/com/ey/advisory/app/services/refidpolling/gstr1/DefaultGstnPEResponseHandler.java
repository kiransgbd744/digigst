 ...
 */
@Slf4j
@Component("DefaultGstnPEResponseHandler")
public class DefaultGstnPEResponseHandler implements Gstr1GstnResponseHandler {

	@Autowired
	@Qualifier("CdnurResponseHandler")
	private Gstr1ResponseHandler cdnurResponseHandler;

	@Autowired
	@Qualifier("CdnrResponseHandler")
	private Gstr1ResponseHandler cdnrResponseHandler;

	@Autowired
	@Qualifier("ExpResponseHandler")
	private Gstr1ResponseHandler expResponseHandler;

	@Autowired
	@Qualifier("PEB2clResponseHandler")
	private Gstr1ResponseHandler pEB2clResponseHandler;

	@Autowired
	@Qualifier("PEB2bResponserHandler")
	private Gstr1ResponseHandler pEB2bResponserHandler;
	...
	public void handleResponse(JsonElement root, Gstr1SaveBatchEntity batch) {
		...
		if (batch.getSection() != null) {
			...
			if (!SECTIONS.contains(batch.getSection().toLowerCase())) {
				...
				Map<String, OutwardTransDocError> docErrorMap = secResponse
						.getValue1();
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
				...
				if (APIConstants.DELETE_FILE_UPLOAD
					...
					pair = handleAutoDraftRepo(docErrorMap, errorCount, batch,
							isOriginalInvoiceNotFound);

				} else if (APIConstants.DELETE_RESPONSE
						.equalsIgnoreCase(batch.getOperationType())) {
					pair = handleDeleteResponseRepo(docErrorMap, errorCount,
							batch, isOriginalInvoiceNotFound);

				} else {
					pair = handleDocRepo(docErrorMap, errorCount, batch,
							isOriginalInvoiceNotFound);
	...
	private Pair<Integer, Set<String>> handleDeleteResponseRepo(
			Map<String, OutwardTransDocError> docErrorMap, Integer errorCount,
		...
		// Set<Entry<String, OutwardTransDocError>> entrySet =
		// docErrorMap.entrySet();
		// Extra map to avoid java.util.ConcurrentModificationException
		Map<String, OutwardTransDocError> docErrorMap1 = new HashMap<>();
		for (Entry<String, OutwardTransDocError> entry : docErrorMap
				.entrySet()) {

			OutwardTransDocError value = entry.getValue();
		...
		List<Gstr1vsEinvReconRespProcessedEntity> headerDocs1 = new ArrayList<>();
		...
		for (Gstr1vsEinvReconRespProcessedEntity doc : headerDocs) {
			...
			// Using the invoice key, get the error object
			OutwardTransDocError error = docErrorMap.get(key);
			/*
	...
	private Pair<Integer, Set<String>> handleAutoDraftRepo(
		...
		LocalDate utcDateTimeFromLocal = EYDateUtil
				.toUTCDateTimeFromLocal(LocalDate.now());
		// List<Object[]> docIdKeysArr = new ArrayList<>();
		List<Gstr1GSTINDeleteDataEntity> headerDocs = new ArrayList<>();
		// Set<Entry<String, OutwardTransDocError>> entrySet =
		// docErrorMap.entrySet();
		// Extra map to avoid java.util.ConcurrentModificationException
		Map<String, OutwardTransDocError> docErrorMap1 = new HashMap<>();
		...
		return new Pair<Integer, Set<String>>(errorCount,OriginalInvoiceErrorCodes);
	}
	...
	private Pair<Integer, Set<String>> handleDocRepo(
		...
		LocalDate utcDateTimeFromLocal = EYDateUtil
				.toUTCDateTimeFromLocal(LocalDate.now());
		// List<Object[]> docIdKeysArr = new ArrayList<>();
		List<OutwardTransDocument> headerDocs = new ArrayList<>();
		// Set<Entry<String, OutwardTransDocError>> entrySet =
		// docErrorMap.entrySet();
		// Extra map to avoid java.util.ConcurrentModificationException
		Map<String, OutwardTransDocError> docErrorMap1 = new HashMap<>();
		for (Entry<String, OutwardTransDocError> entry : docErrorMap
			...
			if (CUST_GSTIN_ERROR_CODES.contains(errorCode)) {
				...
				String docKey = entry.getKey();
				List<OutwardTransDocument> errorDocs = outwardTransDocumentRepository
						.findDocsByDocKey(docKey);

				if (errorDocs != null && !errorDocs.isEmpty()) {
					// At any given moment only one record exist with
					// active docKey
					OutwardTransDocument errorDoc = errorDocs.get(0);
					String cgstin = errorDoc.getCgstin();
					// Get the list of document id/doc key pairs for which we
					// need to store the errors. This is done to get all
					// the required doc ids/keys in a single stretch.
		...
		Set<String> OriginalInvoiceErrorCodes = new HashSet<>();
		List<OutwardTransDocError> errorDocList = new ArrayList<>();
		List<OutwardTransDocument> headerDocs1 = new ArrayList<>();
		// Attach the errors with the corresponding document header ids.
		// The docIds array will contain an object array where first
		// element is the document header id and the second element
		// is the doc key.
		for (OutwardTransDocument doc : headerDocs) {
			String key = String.valueOf(doc.getDocKey());
			// Using the invoice key, get the error object
			OutwardTransDocError error = docErrorMap.get(key);
			error.setDocHeaderId(doc.getId());
