 ...
 *
 */
@Component("Anx1CsvReportsDaoImpl")
@Slf4j
public class Anx1CsvReportsDaoImpl implements Anx1CsvReportsDao {

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	static Integer cutoffPeriod = null;
	static String serviceOption = null;
	private static final String OLDFARMATTER = "yyyy-MM-dd";
	private static final String NEWFARMATTER = "dd-MM-yyyy";

	@Override
	public List<DataStatusEinvoiceDto> getCsvReports(SearchCriteria criteria) {
		...
		if (DownloadReportsConstant.ERROR.equalsIgnoreCase(request.getType())) {
			buildQuery1.append(" WHERE IS_ERROR = TRUE ");

			buildRetTypeTableNo.append(" '' AS RETURN_TYPE,"
					+ "'' AS TABLE_NUMBER,");

			buildTaxDocType.append("'' AS DATA_CATEGORY ");
		}
		if (DownloadReportsConstant.PROCESSED
				.equalsIgnoreCase(request.getType())) {
			buildQuery1.append(" WHERE IS_PROCESSED = TRUE ");
		...
		if (DownloadReportsConstant.TOTALERECORDS
				.equalsIgnoreCase(request.getType())) {
			buildQuery1.append(" WHERE (IS_PROCESSED = TRUE "
					+ "OR IS_ERROR = TRUE OR IS_INFORMATION = TRUE ) ");
		...
		if (DownloadReportsConstant.PROCESSEDINFO
				.equalsIgnoreCase(request.getType())) {
			buildQuery1.append(
					" WHERE IS_PROCESSED = TRUE AND IS_INFORMATION=TRUE ");
		...
		String queryStr = createApiQueryString(buildQuery.toString(),
				buildQuery1.toString(), buildRetTypeTableNo.toString(),
				buildTaxDocType.toString());
	...
	private DataStatusEinvoiceDto convertError(Object[] arr) {
		...
		if (DownloadReportsConstant.ServiceOptions.ASYN_COMP.getServiceOption()
			...
			obj.setAdjRefDate(arr[292] != null ? arr[292].toString() : null);
			...
			obj.setDataCategory(arr[302] != null ? arr[302].toString() : null);
		}
	...
	private String createApiQueryString(String buildQuery, String buildQuery1,
		...
		return "SELECT TRIM(', ' FROM IFNULL(ERRH.ERROR_CODE_ASP,'') ||','|| "
				...
				+ "HDR.IRN_DATE AS IRN_ACK_DATE_TIME, EINV.SIGNED_QR "
				+ "AS SIGNED_QR_CODE, EINV.SIGNED_INV AS SIGNED_INVOICE,"
				...
				+ "AS GSTIN_ERROR_DESCRIPTION_ASP, " + buildRetTypeTableNo
				+ " HDR.IRN, HDR.IRN_DATE,HDR.TAX_SCHEME,"
				+ "HDR.CANCEL_REASON, HDR.CANCEL_REMARKS,"
				+ "ITM.SUPPLY_TYPE,HDR.DOC_CATEGORY, HDR.DOC_TYPE,HDR.DOC_NUM,"
				...
				+ "HDR.SHIP_TO_PINCODE,HDR.SHIP_TO_PINCODE AS SHIPTOSTATECODE,ITM.ITM_NO,"
				...
				+ "ITM.TENDER_REFERENCE ,ITM.CONTRACT_REFERENCE,ITM.EXTERNAL_REFERENCE,"
				...
				+ "TDS_IGST_AMT,TDS_CGST_AMT,TDS_SGST_AMT, HDR.USER_ID AS userid,"
				...
				+ "ITM.GLCODE_CGST, ITM.GLCODE_SGST,ITM.GLCODE_ADV_CESS,ITM.GLCODE_SP_CESS,"
				...
				+ "ITM.SHIP_TO_STATE," + buildTaxDocType
				+ " FROM ANX_OUTWARD_DOC_HEADER HDR LEFT OUTER JOIN "
				+ "TF_OUTWARD_HEADER_GSTIN_ASP_ERROR_INFO ()ERRH "
				+ "ON HDR.ID = ERRH.DOC_HEADER_ID INNER JOIN ANX_OUTWARD_DOC_ITEM ITM "
				...
				+ "AND ITM.ITEM_INDEX = ERRI.ITEM_INDEX AND "
				+ "TO_CHAR(ITM.ITM_NO) = TO_CHAR(ERRI.ITM_NO) "
				+ "LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID = HDR.BATCH_ID LEFT OUTER JOIN "
				+ "EINV_MASTER EINV ON EINV.DOC_HEADER_ID = HDR.ID "
				+ "AND ITM.DOC_HEADER_ID = EINV.DOC_HEADER_ID " + buildQuery1
