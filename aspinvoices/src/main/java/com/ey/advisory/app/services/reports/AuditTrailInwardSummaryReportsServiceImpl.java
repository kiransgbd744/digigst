 ...
 *
 */
@Service("AuditTrailInwardSummaryReportsServiceImpl")
@Slf4j
public class AuditTrailInwardSummaryReportsServiceImpl
		implements Gstr1VerticalReportsService {
	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("AuditTrailInwardSummaryReportsDaoImpl")
	private AuditTrailReportsDao auditTrailReportsDao;
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		responseFromView = auditTrailReportsDao.getAuditTrailReports(request);

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"Audit trail Inward Summary response" + responseFromView);
		}

		if (responseFromView != null && !responseFromView.isEmpty()) {

			String[] invoiceHeaders = commonUtility
					.getProp("audit.trail.inward.summary.report.headers")
					.split(",");
			if (LOGGER.isDebugEnabled()) {
			...
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"Audit_Trail_Inward_Summary.xlsx");
			if (LOGGER.isDebugEnabled()) {
