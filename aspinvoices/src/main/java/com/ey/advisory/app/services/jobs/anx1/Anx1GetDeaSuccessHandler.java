...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1DeaInvoicesRepository;
...
@Service("Anx1GetDeaSuccessHandler")
@Slf4j
public class Anx1GetDeaSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Anx1DeaDataParserImpl")
	private Anx1DeaDataParser anx1DeaDataParser;

	@Autowired
	@Qualifier("GetAnx1DeaInvoicesRepository")
	private GetAnx1DeaInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetDeaSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1DeaInvoicesHeaderEntity> deaEntities = anx1DeaDataParser
						.parseDeaData(dto, apiResp);
				if (!deaEntities.isEmpty()) {
					headerRepo.softlyDeleteDeaHeader(dto.getGstin(),
							dto.getReturnPeriod());

					headerRepo.saveAll(deaEntities);
				}
