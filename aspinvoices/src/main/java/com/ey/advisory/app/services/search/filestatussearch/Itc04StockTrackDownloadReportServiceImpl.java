 ...
 *
 */
@Component("Itc04StockTrackDownloadReportServiceImpl")
@Slf4j
public class Itc04StockTrackDownloadReportServiceImpl
	...
	public void generateReports(Long id) {
		...
		try {
			...
			ITC04RequestDto criteria = gson.fromJson(reqPayload,
					ITC04RequestDto.class);
			criteria = processedRecordsCommonSecParam
					.setItc04DataSecuritySearchParams(criteria);

			setDataSecurityAttribute(criteria, entity);

			// List<Object> responseFromcompAmt = new ArrayList<>();
			// responseFromcompAmt =
			// itcReversalOutwardDao.getItcReports(request);
			...
			String fullPath = tempDir.getAbsolutePath() + File.separator
					+ "ITC04_Stock_Tracking_Download_Report_" + entityName
					+ format.format(convertISDDate) + ".csv";
			...
			StatefulBeanToCsv<Itc04StockTrackingDownloadDto> beanWriter = getBeanWriterInwardRecords(
					reportConfig, writer);
			...
			if (tempDir.list().length > 0) {
				...
				uploadedZipName = DocumentUtility.uploadFile(zipFile,
						ConfigConstants.STOCK_TRACKING_REPORT);
	...
	private void WritetoCsvInwardrecords(Integer chunkNo, Long id,
			...
			StatefulBeanToCsv<Itc04StockTrackingDownloadDto> beanWriter,
			String reportCateg) {

		List<Object[]> list = null;
		List<Itc04StockTrackingDownloadDto> responseFromDao = null;
			...
			for (int i = 1; i <= chunkNo; i++) {
				...
				StoredProcedureQuery reportDataProc = entityManager
						.createStoredProcedureQuery(
								"USP_ITC04_DISP_CHUNK_STOCK_TRACKING_REPORT");

				reportDataProc.registerStoredProcedureParameter("P_REQUEST_ID",
						Long.class, ParameterMode.IN);
				reportDataProc.setParameter("P_REQUEST_ID", id);
				...
				if (list.size() > 0) {
					...
					responseFromDao = list.stream()
							.map(o -> (Itc04StockTrackingDownloadDto) reportConvertor
	...
	private Integer getChunkCount(Long id) {
		StoredProcedureQuery storedProc = entityManager
				.createStoredProcedureQuery(
						"USP_ITC04_INS_CHUNK_STOCK_TRACKING_REPORT");
		...
		storedProc.setParameter("P_CHUNK_SPLIT_VAL", 10000);

		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Executing chunking proc"
							+ " USP_ITC04_INS_CHUNK_STOCK_TRACKING_REPORT: '%s'",
					id.toString());
			LOGGER.debug(msg);
		}
	...
	private StatefulBeanToCsv<Itc04StockTrackingDownloadDto> getBeanWriterInwardRecords(
		...
		ColumnPositionMappingStrategy<Itc04StockTrackingDownloadDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
		mappingStrategy.setType(Itc04StockTrackingDownloadDto.class);
		mappingStrategy.setColumnMapping(
				env.getProperty(config.getColumnMappingsKey()).split(","));

		StatefulBeanToCsvBuilder<Itc04StockTrackingDownloadDto> builder = new StatefulBeanToCsvBuilder<>(
				writer);
		StatefulBeanToCsv<Itc04StockTrackingDownloadDto> beanWriter = builder
	...
	private void setDataSecurityAttribute(ITC04RequestDto request,
