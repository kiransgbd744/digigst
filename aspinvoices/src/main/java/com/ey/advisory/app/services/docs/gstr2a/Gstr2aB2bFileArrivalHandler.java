 ...
 *
 */
@Service("Gstr2aB2bFileArrivalHandler")
public class Gstr2aB2bFileArrivalHandler {
	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr2aB2bFileArrivalHandler.class);
	...
	@Autowired
	@Qualifier("Gstr2aFileProcessService")
	private Gstr2aFileProcessService gstr2aFileProcessService;
	...
	public void processGstr2aB2bFile(Message message, AppExecContext context) {

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("processGstr2aFile");
		}
		...
		try {
			int count = gstr2aFileProcessService.processGstr2File(inputStream, fileName, batchId);
			...
			batchUtil.updateById(batchId, APIConstants.FAILED, "CATCH_BLOCK",
					e.getMessage(), false);
			String msg1 = "Error occured while processing the Gstr2a file:: ";
			e.printStackTrace();
			LOGGER.error(msg1, e);
			throw new APIException(msg1, e);
		}
