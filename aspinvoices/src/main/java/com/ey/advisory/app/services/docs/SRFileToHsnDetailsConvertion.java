 ...
 *
 */
@Service("SRFileToHsnDetailsConvertion")
public class SRFileToHsnDetailsConvertion {

	public List<Gstr1AsEnteredHsnEntity> convertSRFileToOutwardTransDoc(
			List<Object[]> listOfHsns, Gstr1FileStatusEntity fileStatus) {

		List<Gstr1AsEnteredHsnEntity> hsns = new ArrayList<>();
		Gstr1AsEnteredHsnEntity hsn = null;
		for (Object[] obj : listOfHsns) {
			hsn = new Gstr1AsEnteredHsnEntity();
	...
	public String generateHsnKey(Object[] arr) {
		...
		return new StringJoiner(GSTConstants.WEB_UPLOAD_KEY).add(gstin)
				.add(returnPeriod).add(hsnOrSac).add(taxRate).add(uqc).toString();
	}
	...
	public List<Gstr1HsnFileUploadEntity> convertSRFileToHsnTransDoc(
			List<Gstr1AsEnteredHsnEntity> busProcessRecords,
			Gstr1FileStatusEntity fileStatus) {

		List<Gstr1HsnFileUploadEntity> hsns = new ArrayList<>();
		Gstr1HsnFileUploadEntity hsn = null;
		for (Gstr1AsEnteredHsnEntity obj : busProcessRecords) {
			hsn = new Gstr1HsnFileUploadEntity();
			Integer deriRetPeriod = 
					GenUtil.convertTaxPeriodToInt(obj.getReturnPeriod());
			hsn.setDerivedRetPeriod(deriRetPeriod);
			...
			if (strQnt != null && !strQnt.isEmpty()) {
				...
				hsn.setQuentity(
						qnt.setScale(3, BigDecimal.ROUND_HALF_EVEN));
			}
			...
			if (strCess != null && !strCess.isEmpty()) {
				...
				hsn.setCess(
						cessAmt.setScale(2, BigDecimal.ROUND_HALF_EVEN));
			}
