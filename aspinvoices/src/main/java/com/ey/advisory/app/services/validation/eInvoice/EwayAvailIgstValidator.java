 ...
 *
 *//*
public class EwayAvailIgstValidator
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			BigDecimal aigst = item.getAvailableIgst();
			BigDecimal igst = item.getIgstAmount();
			if (aigst == null) {
				aigst = BigDecimal.ZERO;
			}
			if (igst == null) {
				igst = BigDecimal.ZERO;
			}
			if (aigst.compareTo(igst) > 0) {
				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(GSTConstants.Availableigstamount);
				TransDocProcessingResultLoc location 
				= new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER1104",
						"Available credit cannot be more than Tax "
								+ "Payable.",
						location));
			}
		});
