 ...
 */
@Slf4j
@Component("Gstr1SignAndFileServiceImpl")
public class Gstr1SignAndFileServiceImpl implements Gstr1SignAndFileService {
	...
	@Override
	public Pair<Boolean, String> getGstr1GstnSummary(
		...
		try {
			APIResponse resp = getGstr1Summary(taxPeriod, gstin);
			if (resp.isSuccess()) {
				...
				LOGGER.error(
						"we have received Error Response from GSTN"
								+ " for GETGSTR1Summ GSTIN {}, Taxperiod {}, Response is {} ",
	...
	@Override
	public void updateGstr1Tables(String taxperiod, String gstin, String signId,
		...
		try {
			...
			if (i == 0) {
				...
				returnStatusentity
						.setReturnType(APIConstants.GSTR1.toUpperCase());
	...
	private void saveOrUpdateGstnUserRequest(String gstin, String taxPeriod,
		...
		gstnUserRequestEntity = gstnUserRequestRepo
				.findTop1ByGstinAndTaxPeriodAndReturnTypeAndRequestTypeAndIsDeleteFalseOrderByIdDesc(
						gstin, taxPeriod, APIConstants.GSTR1.toUpperCase(),
						"GET");
		Clob reqClob;
		try {
	...
	public APIResponse getGstr1Summary(String taxPeriod, String gstin) {
		...
		try {
			...
			APIParams params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_SUMMARY, param1, param2);
			...
			String msg = String.format(
					...
					APIIdentifiers.GSTR1_GET_SUMMARY, gstin, taxPeriod);
			LOGGER.error(msg, ex);
			throw ex;
		}
	}
	...
	@Override
	public List<Gstr3BFilingDetailsDTO> getGstr1FilingDetails(String gstin,
			String taxPeriod) {
	...
	@Override
	public void downloadGstr1ErrorResp(String id,
	...
	private void writeMsgToTxt(SignAndFileEntity entity,
		...
		try {
			...
			String fileName = "GSTR1FilingErrors_" + entity.getGstin() + "_"
					+ entity.getTaxPeriod();
			...
			String msg = "Error while writing Gstr1 errors to txt";
			LOGGER.error(msg, e);
			throw new AppException(msg, e);
		} finally {
				...
				try {
					...
					String msg = String.format(
							"Failed to remove the temp "
									+ "directory created for Gstr1 errors: '%s'. This will "
