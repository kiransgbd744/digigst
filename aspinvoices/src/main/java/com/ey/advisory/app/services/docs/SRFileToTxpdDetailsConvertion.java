 ...
 *
 */
@Service("SRFileToATADetailsConvertion")
@Slf4j
public class SRFileToTxpdDetailsConvertion {
	...
	public List<Gstr1AdvanceAdjustmentFileUploadEntity> convertSRFileToAtaDoc(
			List<Gstr1AsEnteredTxpdFileUploadEntity> busProcessRecords,
		...
		for (Gstr1AsEnteredTxpdFileUploadEntity obj : busProcessRecords) {
	...
	public List<Gstr1AsEnteredTxpdFileUploadEntity> convertSRFileToTxpdExcel(
			List<Object[]> listOfAtas, Gstr1FileStatusEntity updateFileStatus) {

		List<Gstr1AsEnteredTxpdFileUploadEntity> list = new ArrayList<>();

		Gstr1AsEnteredTxpdFileUploadEntity obj = null;
		for (Object[] arr : listOfAtas) {
			obj = new Gstr1AsEnteredTxpdFileUploadEntity();
	...
	private String getGstnKey(Object[] obj) {
		...
		return new StringJoiner(GSTConstants.WEB_UPLOAD_KEY).add(sgstin).add(retPeriod)
				.add(transType).add(month).add(orgPos).add(orgRate).add(newPos)
				.add(newRate).toString();
	}
	...
	public String getTxpdKey(Object[] obj) {
		...
		return new StringJoiner(GSTConstants.WEB_UPLOAD_KEY).add(sgstin).add(retPeriod)
				.add(transType).add(month).add(orgPos).add(orgRate).add(newPos)
				.add(newRate).add(profitCenter).add(plant).add(divison)
				.add(location).add(salesOrg).add(distributedChannel).add(user1)
				.add(user2).add(user3).add(user4).add(user5).add(user6)
				.toString();
	}
	...
	public List<Gstr1AsEnteredTxpdFileUploadEntity> convertSRFileToTxpd(
			List<Gstr1AsEnteredTxpdFileUploadEntity> strProcessRecords,
			Gstr1FileStatusEntity updateFileStatus) {
		List<Gstr1AsEnteredTxpdFileUploadEntity> listTxpd = new ArrayList<>();
		...
		Gstr1AsEnteredTxpdFileUploadEntity txpd = null; 
		for (Gstr1AsEnteredTxpdFileUploadEntity arr : strProcessRecords) {
			txpd = new Gstr1AsEnteredTxpdFileUploadEntity();
			...
			if (arr.getGstnTxpdKey() != null
				...
				txpd.setGstnTxpdKey(
						arr.getGstnTxpdKey().substring(0, 1000));
			} else {
			...
			if (arr.getTxpdInvKey() != null
				...
				txpd.setTxpdInvKey(
						arr.getTxpdInvKey().substring(0, 2200));
			} else {
