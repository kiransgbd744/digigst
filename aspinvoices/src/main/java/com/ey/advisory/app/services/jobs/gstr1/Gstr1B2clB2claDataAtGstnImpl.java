 ...
 *
 */
@Component("gstr1B2clB2claDataAtGstnImpl")
@Slf4j
public class Gstr1B2clB2claDataAtGstnImpl implements Gstr1B2clB2claDataAtGstn {
	...
	public Long findB2CLDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.B2CL.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_B2CLA);

			}
		}
		
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1B2clB2claSuccessHandler", "Gstr1GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
