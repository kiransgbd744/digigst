...
@Service("Anx1AspProcessedUploadedServiceImpl")
public class Anx1AspProcessedUploadedServiceImpl
		implements Anx1AspProcessedUploadedService {
	...
	public Workbook findProcessedUploaded(SearchCriteria criteria,
		...
		List<Object> responseFromViewOutward = new ArrayList<>();
		responseFromViewOutward = anx1ProcessedUploadedDao
				.getAnx1ProcessedUploadedDaoReport(request);
		...
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ANX-1_ASP_Processed_Records_Transactional.xlsx");

		LOGGER.debug("Anx1 Asp Processed Uploaded data response"
				+ responseFromViewOutward);

		if (responseFromViewOutward != null
				&& responseFromViewOutward.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.Asp.outward.process.report.headers")
					.split(",");

			Cells errorDumpCells = workbook.getWorksheets().get(0).getCells();
			errorDumpCells.importCustomObjects(responseFromViewOutward,
					invoiceHeaders, isHeaderRequired, startRow, startcolumn,
					responseFromViewOutward.size(), true, "yyyy-mm-dd", false);
		}
