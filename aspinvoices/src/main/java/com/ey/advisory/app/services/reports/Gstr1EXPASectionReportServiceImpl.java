...
@Service("Gstr1EXPASectionReportServiceImpl")
public class Gstr1EXPASectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1EXPASectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1EXPASectionReportDaoImpl")
	...
	public Workbook findGstr1ReviewSummRecords(SearchCriteria criteria,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 EXPA processed data item level response"
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 EXPA isActiveFieldSel response"
			...
			if (responseFromView != null && responseFromView.size() > 0) {
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					String msg = String
							.format("Exports-A Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 EXPORTSA custom file name -"
			...
			LOGGER.error(
					"Exception Occuered in EXPORTSA Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
