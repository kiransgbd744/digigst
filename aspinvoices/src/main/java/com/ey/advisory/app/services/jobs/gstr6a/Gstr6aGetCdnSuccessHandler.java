 ...
 *
 */
@Service("Gstr6aGetCdnSuccessHandler")
@Slf4j
public class Gstr6aGetCdnSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr6aCdnDataParserImpl")
	private Gstr6aCdnGetDataParser gstr6aCdnGetDataParser;

	@Autowired
	@Qualifier("GetGstr6aStagingCdnHeaderRepository")
	private GetGstr6aStagingCdnHeaderRepository cdnStagingRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Gstr6aGetCdnSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				if (APIConstants.CDN.equalsIgnoreCase(dto.getType())) {
					cdnStagingRepo.softlyDeleteCdnHeader(dto.getGstin(),
			...
			resultIds.forEach(id -> {
				...
				// Long batchId = null;
				if (APIConstants.CDN.equalsIgnoreCase(dto.getType())) {
					List<GetGstr6aStagingCdnHeaderEntity> atEnties = gstr6aCdnGetDataParser
							.parseCdnGetData(dto, apiResp, dto.getType(),
									ctxParamsObj.get("batchId").getAsLong(),
									now);
					if (!atEnties.isEmpty()) {
						cdnStagingRepo.saveAll(atEnties);
			...
			String msg = "exception occured while GSTR6A get call CDN success handler";
			LOGGER.error(msg, e);
