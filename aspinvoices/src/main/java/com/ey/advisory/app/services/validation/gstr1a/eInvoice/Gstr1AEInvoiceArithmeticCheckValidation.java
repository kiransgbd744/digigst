 ...
 *
 */
public class Gstr1AEInvoiceArithmeticCheckValidation
	...
	public List<ProcessingResult> validate(Gstr1AOutwardTransDocument document,
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			if (!isValidTax(taxableValue, quantity, unitPrice)) {
			...
			if (!isValidTax(cgstAmount, taxableValue, CsgstRate)) {
				String[] errorLocations = new String[] { CGST_AMOUNT };
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations);
				errors.add(new ProcessingResult(APP_VALIDATION,
			...
			if (!isValidTax(sgstAmount, taxableValue, SgstRate)) {
			...
			if (!isValidTax(igstAmount, taxableValue, IgstRate)) {
			...
			if (!isValidTax(specificCessAmount, taxableValue,
					specificCessRate)) {
			...
			if (!isValidTax(cessAmountAdvalorem, taxableValue,
					cessRateAdvalorem)) {
			...
			if (!isValidTax(stateCessAmount, taxableValue, stateCessRate)) {
