 ...
 *
 */
public class ITC04ReviewSummaryUtility {
	...
	private static Map<String, String> buildTableDesctiptionMap() {
		...
		tableDescriptionMap.put(
				"Goods received back - Job Worker to Manufacturer (5A)",
				"Goods received back - Job Worker to Manufacturer (5A)");
		tableDescriptionMap.put(
				"Goods received back - Other Job Worker to Manufacturer (5B)",
				"Goods received back - Other Job Worker to Manufacturer (5B)");
		tableDescriptionMap.put("Goods sold from Job Worker Premises (5C)",
				"Goods sold from Job Worker Premises (5C)");
	 ...
	 * @param time
	 */
	public static void buildAndGenerateItc04ReviewReport(Workbook workbook,
			List<ITC04EntitylevelDto> responseFromView, String entityName,
		...
		try {
			...
			if (CollectionUtils.isEmpty(itc04EntitylevelDtos)) {
				...
				gstins.forEach(gstin -> {
					itc04EntitylevelDtos.add(new ITC04EntitylevelDto(gstin, "",
			...
			if (!CollectionUtils.isEmpty(itc04EntitylevelDtos)) {
				List<String> dbGstins = itc04EntitylevelDtos.stream()
						.map(ITC04EntitylevelDto::getGstin)
			...
			String[][] dataArray = new String[70
					+ itc04EntitylevelDtos.size()][100
							+ itc04EntitylevelDtos.size()];
			applyStyles(row.get(), column.get(), worksheet, 0, false, false,
					FontUnderlineType.NONE, null, false, false);

			dataArray[row.incrementAndGet()][column
					.incrementAndGet()] = "ITC04 Review Summary";
			applyStyles(row.get(), column.get(), worksheet, 0, true, true,
					FontUnderlineType.NONE, null, false, false);

			dataArray[row.incrementAndGet()][column.get()] = "Entity Name - "
					+ entityName;
			applyStyles(row.get(), column.get(), worksheet, 0, true, true,
					FontUnderlineType.NONE, null, false, false);
			...
			dataArray[row.get()][column.incrementAndGet()] = "Tax Period - "
					+ taxPeriod;
			applyStyles(row.get(), column.get(), worksheet, 0, true, true,
					FontUnderlineType.NONE, null, false, false);
			...
			applyStyles(row.get(), column.get(), worksheet, 1, false, true,
					FontUnderlineType.NONE, Color.getLightSteelBlue(), false,
					true);
			applyStyles(6, column.get(), worksheet, 1, false, true,
					FontUnderlineType.NONE, Color.getLightSteelBlue(), false,
					true);

			worksheet.getCells().merge(row.get(), column.incrementAndGet(), 2,
					1);
			...
			AtomicInteger endColumn = new AtomicInteger(2);
			...
			applyStyles(row.get(), column.get(), worksheet, 1, false, false,
					FontUnderlineType.NONE, Color.getLightYellow(), false,
					false);
			...
			Map<String, List<ITC04EntitylevelDto>> gstinsMap = itc04EntitylevelDtos
					.stream().collect(Collectors
							.groupingBy(ITC04EntitylevelDto::getGstin));
			System.out.println(gstinsMap);

			Map<String, String> descMap = buildTableDesctiptionMap();
			row.set(7);
			column.set(1);
			Map<String, List<ITC04EntitylevelDto>> dataMap = fillTheMissigDataForGstins(
					gstinsMap);
			dataMap.keySet().forEach(gstin -> {
				List<ITC04EntitylevelDto> itc04EntitylevelDtos1 = dataMap
						.get(gstin);
				Set<String> docStrings = descMap.keySet();
				docStrings.forEach(docString -> {
					itc04EntitylevelDtos1.stream()
							.filter(dto -> dto.getTableDesc().equals(docString))
							.forEach(itc04EntitylevelDto -> {
								try {
									dataArray[row.get()][column
											.get()] = itc04EntitylevelDto
													.getGstin();
									applyStyles(row.get(), column.get(),
											worksheet, 1, false, false,
											FontUnderlineType.NONE, null, false,
											false);
									dataArray[row.get()][column
											.incrementAndGet()] = descMap
													.get(itc04EntitylevelDto
															.getTableDesc());
									applyStyles(row.get(), column.get(),
											worksheet, 1, false, false,
											...
											false);
									dataArray[row.get()][column
											.incrementAndGet()] = isNoData.get()
													? "0"
													: itc04EntitylevelDto
															.getAvailabledigigstCount();
									applyStyles(row.get(), column.get(),
											worksheet, 1, false, false,
											FontUnderlineType.NONE, null, false,
											true);
									...
									dataArray[row.get()][column
											.incrementAndGet()] = isNoData.get()
													? "0"
													: itc04EntitylevelDto
															.getAvailablegstnCount();
									applyStyles(row.get(), column.get(),
											worksheet, 1, false, false,
											FontUnderlineType.NONE, null, false,
											true);

									if ((itc04EntitylevelDto
											.getAvailablegstnTaxablevalue() == null
											|| itc04EntitylevelDto
													.getAvailablegstnTaxablevalue()
													.equals("0")
											|| itc04EntitylevelDto
													.getAvailablegstnTaxablevalue()
													.equals("0.00")
										...
										dataArray[row.get()][column
												.incrementAndGet()] = isNoData
														.get() ? "0"
																: itc04EntitylevelDto
																		.getAvailablegstnTaxablevalue();
										applyStyles(row.get(), column.get(),
												worksheet, 1, false, false,
												FontUnderlineType.NONE, null,
												false, true);
									}
									dataArray[row.get()][column
											.incrementAndGet()] = isNoData.get()
				...
				for (int i = 1; i < 9; i++) {
					try {
						applyStyles(row.get(), i, worksheet, 1, false, false,
	...
	private static Map<String, List<ITC04EntitylevelDto>> fillTheMissigDataForGstins(
		...
		Map<String, List<ITC04EntitylevelDto>> dataMap = Maps
				.newLinkedHashMap();
		gstinsMap.keySet().forEach(gstin -> {
			List<ITC04EntitylevelDto> itc04EntitylevelDtos = gstinsMap
					.get(gstin);
			List<String> dbStrings = itc04EntitylevelDtos.stream()
					.map(ITC04EntitylevelDto::getTableDesc)
					.collect(Collectors.toList());
			...
			if (!CollectionUtils.isEmpty(descStrings)) {
				descStrings.forEach(docString -> {
					itc04EntitylevelDtos.add(new ITC04EntitylevelDto(gstin,
							docString, "0", "0.00", "0.00", "0.00", "0", "0.00",
							"0", "0.00", "0.00", "0.00"));
				});
			}
