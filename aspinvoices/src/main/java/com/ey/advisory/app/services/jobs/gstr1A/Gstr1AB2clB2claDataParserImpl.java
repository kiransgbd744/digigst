...
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AB2clItemEntity;
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AB2claHeaderEntity;
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AB2claItemEntity;
 ...
 *
 */
@Component("gstr1AB2clB2claDataParserImpl")
@Slf4j
public class Gstr1AB2clB2claDataParserImpl implements Gstr1AB2clB2claDataParser {

	@Autowired
	private Anx1DocKeyGenerator docKeyGenerator;

	@Override
	public List<GetGstr1AB2clHeaderEntity> parseB2clData(Gstr1GetInvoicesReqDto dto, String apiResp) {

		Gson gson = GsonUtil.newSAPGsonInstance();
		JsonArray respObject = null;
		List<GetGstr1AB2clHeaderEntity> headerList = new ArrayList<>();
		...
		for (B2CLInvoices eachInv : baseEntity) {
			...
			for (B2CLInvoiceData eachInvData : eachInv.getB2CLInvoiceData()) {
				...
				List<GetGstr1AB2clItemEntity> lineItems = new ArrayList<>();
				...
				if (eachInvData.getLineItems() != null) {
					...
					for (B2CLLineItem b2cItems : eachInvData.getLineItems()) {
						...
						// New Item Entity
						GetGstr1AB2clItemEntity item = new GetGstr1AB2clItemEntity();
						/**
					...
					// New Header Entity
					GetGstr1AB2clHeaderEntity header = new GetGstr1AB2clHeaderEntity();
					/**
					 * Input data
					 */
		 ...
		 * "failed to parse Gstr1A B2cl response"; LOGGER.error(msg, ex); throw
	...
	@Override
	public List<GetGstr1AB2claHeaderEntity> parseB2claData(Gstr1GetInvoicesReqDto dto, String apiResp) {

		Gson gson = GsonUtil.newSAPGsonInstance();
		JsonArray respObject = null;
		List<GetGstr1AB2claHeaderEntity> headerList = new ArrayList<>();
		...
		for (B2CLInvoices eachInv : baseEntity) {
			...
			for (B2CLInvoiceData eachInvData : eachInv.getB2CLInvoiceData()) {
				...
				BigDecimal igstAmt = BigDecimal.ZERO;
				BigDecimal cessAmt = BigDecimal.ZERO;
				List<GetGstr1AB2claItemEntity> lineItems = new ArrayList<>();
				...
				if (eachInvData.getLineItems() != null) {
					for (B2CLLineItem b2cItems : eachInvData.getLineItems()) {
						...
						B2clLineItemDetail b2cItem = b2cItems.getItemDetail();
						// New Item Entity
						GetGstr1AB2claItemEntity item = new GetGstr1AB2claItemEntity();
						/**
					...
					// New Header Entity
					GetGstr1AB2claHeaderEntity header = new GetGstr1AB2claHeaderEntity();
					/**
					 * Input data
					 */
					header.setGstin(dto.getGstin());
		...
		/*
		 * } catch (Exception ex) { String msg =
		 * "failed to parse Gstr1A B2cla response"; LOGGER.error(msg, ex); throw
		 * new APIException(msg); }
		 */
		return headerList;
	}
