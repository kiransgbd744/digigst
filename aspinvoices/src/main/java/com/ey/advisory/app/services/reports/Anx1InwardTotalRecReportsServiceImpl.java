...
@Service("Anx1InwardTotalRecReportsServiceImpl")
public class Anx1InwardTotalRecReportsServiceImpl 
  implements Anx1InwardTotalRecReportsService {
	
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1InwardTotalRecReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;
	
	@Autowired
	@Qualifier("Anx1InwardTotalReportsDaoImpl")
	private Anx1InwardTotalReportsDao anx1InwardTotalReportsDao;
	@Autowired
	@Qualifier("Gstr1FileStatusRepository")
	private Gstr1FileStatusRepository gstr1FileStatusRepository;
	@Override
	public Workbook findInwardTotalRecords(SearchCriteria criteria,
			PageRequest pageReq) {
		Anx1FileStatusReportsReqDto request = (Anx1FileStatusReportsReqDto) criteria;
		
		/*Long id = request.getFileId();
		List<Gstr1FileStatusEntity> findById = gstr1FileStatusRepository.findByIds(id);
		//String fileType = findById.get(0).getFileType();
		*/
		Workbook workbook = new Workbook();
		int startRow = 1;
		int startcolumn = 0;
		boolean isHeaderRequired = false;
		List<Anx1InwardTotalRecordsDto> responseFromView = new ArrayList<>();
		//if(fileType != null && fileType.equalsIgnoreCase(GSTConstants.RAW)){
		responseFromView = anx1InwardTotalReportsDao.getInwardTotalReports(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.inward.total.report.headers").split(",");
			LOGGER.debug("workbook properties reading starting ");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_InwardTotalRecReport.xlsx");
