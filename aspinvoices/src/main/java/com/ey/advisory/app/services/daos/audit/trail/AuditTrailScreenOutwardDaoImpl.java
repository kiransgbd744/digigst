 ...
 *
 */
@Component("AuditTrailScreenOutwardDaoImpl")
@Slf4j
public class AuditTrailScreenOutwardDaoImpl
		implements AuditTrailScreenOutwardDao {
	...
	@Override
	public AuditTrailScreenSummaryRespDto getAuditTrailOutwardData(
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Reading Resultset for auditoutwardscreen" + list);
		}
	...
	private AuditTrailScreenSummaryRespDto convertAuditTrailOutwardQuery(
		...
		auditTrailOutwScreen.setLatestStatus(
				latestRecord[3] != null ? latestRecord[3].toString() : null);
		...
		if (latestRecord[1] != null) {
			...
			LOGGER.debug(
					"processing Date and Time :" + latestRecord[1].toString());
		...
		for (Object[] arr : arrList) {
			...
			auditTrailItemData
					.setTotalTax(arr[10] != null ? (BigDecimal) arr[10] : null);
	...
	private String createAuditTrailOutwardQuery(String buildQuery) {
		...
		return "select HDR.CREATED_BY ,	HDR.CREATED_ON,CASE WHEN "
				...
				+ "WHEN DATAORIGINTYPECODE IN ('AI','EI','SI','CI','BI') THEN 'Invoice Management' "
				+ "END PROCESSING_SOURCE, CASE WHEN ASP_INVOICE_STATUS = 1 "
				+ "THEN 'Error at DigiGST' WHEN ASP_INVOICE_STATUS = 2 "
				+ "THEN 'Processed at DigiGST' END PROCESSING_STATUS,"
				+ " ASP_INVOICE_STATUS,CASE WHEN HDR.SUPPLY_TYPE = 'CAN' AND ASP_INVOICE_STATUS = 2"
				+ " THEN 'Yes' ELSE 'No' END WHETHER_CANCEL,HDR.TABLE_SECTION,"
				+ "HDR.CUST_GSTIN,CASE WHEN ITM_ROW_COUNT > 0 THEN "
				+ "ITM_ROW_COUNT ELSE COUNT(ITM.DOC_HEADER_ID) END ITEM_CNT,"
				+ "HDR.TAXABLE_VALUE,SUM(IFNULL(ITM.IGST_AMT,0)+"
				+ "IFNULL(ITM.CGST_AMT,0)+ IFNULL(ITM.SGST_AMT,0)+ "
				+ "IFNULL(ITM.CESS_AMT_SPECIFIC,0)+ "
				+ "IFNULL(ITM.CESS_AMT_ADVALOREM,0)) AS TOTAL_TAX,"
				+ "HDR.DOC_AMT INV_VALUE,HDR.TAX_DOC_TYPE,DATAORIGINTYPECODE,HDR.USER_ID"
				+ " FROM ANX_OUTWARD_DOC_HEADER HDR, "
				+ "ANX_OUTWARD_DOC_ITEM ITM WHERE HDR.ID = ITM.doc_header_id "
				+ buildQuery
				+ " GROUP BY HDR.CREATED_BY,HDR.CREATED_ON,DATAORIGINTYPECODE,"
				+ "HDR.USER_ID,ASP_INVOICE_STATUS,HDR.TABLE_SECTION,"
				+ "HDR.SUPPLY_TYPE,HDR.CUST_GSTIN,ITM_ROW_COUNT,"
				+ "HDR.TAXABLE_VALUE,HDR.TAX_DOC_TYPE, HDR.ID, HDR.DOC_AMT"
