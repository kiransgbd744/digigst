 ...
 */
@Slf4j
@Service("anx1SaveMisBatchProcessImpl")
public class Anx1SaveMisBatchProcessImpl implements Anx1SectionWiseSaveBatchProcess {
	...
	private Pair<List<Long>, SaveAnx1> setChunkCtinData(SaveAnx1 anx1,
			List<Anx1MisData> cTinList, String section) {
		...
		anx.setMisInvoice(cTinList);

		for (Anx1MisData cData : cTinList) {
			for (Anx1MisDocumentData iData : cData.getDocs()) {
	...
	private Pair<List<Long>, SaveAnx1> setChunkInvData(SaveAnx1 anx1,
			List<Anx1MisDocumentData> invList, String section) {

		SaveAnx1 anx = new SaveAnx1();
		List<Long> idsList = new ArrayList<>();
		List<Anx1MisData> cTinList = new ArrayList<>();

		anx.setSgstin(anx1.getSgstin());
		anx.setTaxperiod(anx1.getTaxperiod());

		anx.setMisInvoice(cTinList);
		Anx1MisData cTin = anx1.getMisInvoice().get(0);

		cTin.setDocs(invList);
		cTinList.add(cTin);

		for (Anx1MisDocumentData data : invList) {
	...
	private List<Triplet<List<Long>, SaveAnx1, String>> batchChunk(
		...
		int ctinSize = anx1.getMisInvoice().size();

		Pair<List<Long>, SaveAnx1> pair1 = null;
		Pair<List<Long>, SaveAnx1> pair2 = null;

		if (ctinSize > 1) {
			List<Anx1MisData> cTinList1 = anx1.getMisInvoice().subList(0,
					ctinSize / 2);
			List<Anx1MisData> cTinList2 = anx1.getMisInvoice()
			...
			Anx1MisData cTin1 = anx1.getMisInvoice().get(0);
			int invSize = cTin1.getDocs().size();
			List<Anx1MisDocumentData> invList1 = cTin1.getDocs().subList(0,
					invSize / 2);
			List<Anx1MisDocumentData> invList2 = cTin1.getDocs()
