...
@Component("ITC04AspProcessAsUploadedReportDaoImpl")
public class ITC04AspProcessAsUploadedReportDaoImpl {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ITC04AspProcessAsUploadedReportDaoImpl.class);
	...
	private ItcTotalRecords convertVerticalTotal(Object[] arr) {
		...
		obj.setDeliveryChallanDate(fmtDate(arr[8]));
		obj.setjWDeliveryChallanNumber(
				arr[9] != null ? arr[9].toString() : null);
		obj.setjWDeliveryChallanDate(fmtDate(arr[10]));
		obj.setGoodsReceivingDate(fmtDate(arr[11]));
		obj.setInvoiceNumber(arr[12] != null ? arr[12].toString() : null);
		obj.setInvoiceDate(fmtDate(arr[13]));
		...
		obj.setQuantity(removeDecimalDigit(arr[26]));
		obj.setLossesUQC(arr[27] != null ? arr[27].toString() : null);
		obj.setLossesQuantity(removeDecimalDigit(arr[28]));
		obj.setItemAssessableAmount(appendDoubleDecimalDigit(arr[29]));
		obj.setiGSTRate(appendDoubleDecimalDigit(arr[30]));
		obj.setiGSTAmount(appendDoubleDecimalDigit(arr[31]));
		obj.setcGSTRate(appendDoubleDecimalDigit(arr[32]));
		obj.setcGSTAmount(appendDoubleDecimalDigit(arr[33]));
		obj.setsGSTRate(appendDoubleDecimalDigit(arr[34]));
		obj.setsGSTAmount(appendDoubleDecimalDigit(arr[35]));
		obj.setCessAdvaloremRate(appendDoubleDecimalDigit(arr[36]));
		obj.setCessAdvaloremAmount(appendDoubleDecimalDigit(arr[37]));
		obj.setCessSpecificRate(appendDoubleDecimalDigit(arr[38]));
		obj.setCessSpecificAmount(appendDoubleDecimalDigit(arr[39]));
		obj.setStateCessAdvaloremRate(appendDoubleDecimalDigit(arr[40]));
		obj.setStateCessAdvaloremAmount(appendDoubleDecimalDigit(arr[41]));
		obj.setStateCessSpecificRate(appendDoubleDecimalDigit(arr[42]));
		obj.setStateCessSpecificAmount(appendDoubleDecimalDigit(arr[43]));
		obj.setTotalValue(appendDoubleDecimalDigit(arr[44]));
		...
		// obj.setGstnRefidDateTime(arr[72] != null ? arr[72].toString() :
		// null);
		if (arr[72] != null) {
			Timestamp timeStamp = (Timestamp) arr[72];
			LocalDateTime localDT = timeStamp.toLocalDateTime();
			LocalDateTime convertref = EYDateUtil.toISTDateTimeFromUTC(localDT);
			obj.setGstnRefidDateTime(convertref.toString());
		} else {
	...
	private String AspProcessedUploadedQueryString(String buildQuery) {
		...
		return "SELECT TRIM(', ' FROM IFNULL(HDR.INFORMATION_CODES,'') ||','|| "
				...
				+ " '' AS INFO_ERROR_DESCRIPTION_ASP, TABLE_NUMBER ,ACTION_TYPE ,"
				+ "FI_YEAR,HDR.RETURN_PERIOD,"
				+ "HDR.SUPPLIER_GSTIN, DELIVERY_CHALLAN_NO,DELIVERY_CHALLAN_DATE,"
				+ "JW_DELIVERY_CHALLAN_NO,JW_DELIVERY_CHALLAN_DATE, "
				+ "GOODS_RECEIVING_DATE,INV_NUM ,INV_DATE,JW_GSTIN ,JW_STATE_CODE ,"
				+ "JW_WORKER_TYPE,JW_ID,JW_NAME, GOODS_TYPE,ITM_SER_NO,"
				+ "PRODUCT_DESC,PRODUCT_CODE,NATURE_OF_JW, ITM_HSNSAC ,ITM_UQC,"
				+ "ITM_QTY ,LOSSES_UQC,LOSSES_QTY, "
				...
				+ "ITM.STATE_CESS_AMT_SPECIFIC,ITM.TOTAL_VALUE, POSTING_DATE,"
				+ "USER_ID,COMPANY_CODE,SOURCE_IDENTIFIER, SOURCE_FILE_NAME , "
				...
				+ "FIL.FILE_NAME FILE_NAME,(case when DATAORIGINTYPECODE IN ('A','AI') "
				...
				+ "AS GSTIN_ERROR_DESCRIPTION_ASP,HDR.QRETURN_PERIOD FROM ITC04_HEADER "
