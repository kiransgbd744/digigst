...
import jakarta.persistence.Query;
	...
	@Component("Gstr1ReviewSummaryProcessedReportDaoImpl")
	public class Gstr1ReviewSummaryProcessedReportDaoImpl
			implements Gstr1ReviewSummaryReportsDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr1ReviewSummaryProcessedReportDaoImpl.class);
		...
		private String createApiProcessedQueryString(String buildQuery,String buildHeader ) {
			...
			return "SELECT '' AS ERROR_CODE_ASP,"
					...
					+ "AS EWBDATE, HDR.EWB_ERROR_CODE AS EWB_ERROR_CODE,"
					...
					+ "HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) "
					+ "AS SAVE_GSTIN_STATUS,"
					+ "GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID,"
					+ "GSTNBATCH.BATCH_DATE AS GSTIN_REFID_DATE_TIME,"
					+ " ''GSTIN_ERROR_CODE,''GSTIN_ERROR_DESCRIPTION_ASP,"
					+ "HDR.RETURN_TYPE, HDR.TABLE_SECTION "
					+ "AS TABLE_NUMBER,HDR.IRN, HDR.IRN_DATE,"
					+ "HDR.TAX_SCHEME, HDR.CANCEL_REASON,"
					+ "HDR.CANCEL_REMARKS,ITM.SUPPLY_TYPE,"
					+ "HDR.DOC_CATEGORY, HDR.DOC_TYPE,"
					...
					+ "ITM.INV_IGST_AMT, ITM.INV_CGST_AMT, "
					...
					+ "HDR.CR_TRANSFER, HDR.DB_DIRECT,"
					+ "HDR.CR_DAYS,ITM.PAID_AMT,"
					...
					+ "TDS_CGST_AMT,TDS_SGST_AMT, "
					+ "HDR.USER_ID AS userid, "
					+ "HDR.COMPANY_CODE,HDR.SOURCE_IDENTIFIER, "
					+ "HDR.SOURCE_FILENAME, ITM.PLANT_CODE,"
					+ "HDR.DIVISION, SUB_DIVISION,ITM.LOCATION, "
					+ "HDR.SALES_ORGANIZATION, HDR.DISTRIBUTION_CHANNEL, "
					+ "HDR.PROFIT_CENTRE,HDR.PROFIT_CENTRE2, "
					+ "HDR.USERACCESS1, HDR.USERACCESS2,"
					+ "HDR.USERACCESS3, HDR.USERACCESS4,"
					+ "HDR.USERACCESS5, HDR.USERACCESS6, "
					+ "ITM.GLCODE_TAXABLEVALUE,ITM.GLCODE_IGST, "
					+ "ITM.GLCODE_CGST, ITM.GLCODE_SGST,"
					...
					+ "as USER_ID, HDR.FILE_ID, FIL.FILE_NAME as FILE_NAME,"
					...
					+ " FROM ANX_OUTWARD_DOC_HEADER HDR LEFT OUTER JOIN ( SELECT DOC_HEADER_ID , "
					+ "ITEM_INDEX , ITM_NO , STRING_AGG(ERROR_CODE_GSTIN,',') ERROR_CODE_GSTIN , "
					...
					+ "SUPPLIER_GSTIN, FILE_ID, DERIVED_RET_PERIOD FROM ( SELECT DOC_HEADER_ID, ITEM_INDEX, "
					+ "ITM_NO, ' 'AS ERROR_CODE_GSTIN,' ' AS ERROR_DESCRIPTION_GSTIN, "
					...
					+ "AS INFO_ERROR_DESCRIPTION_ASP ,SUPPLIER_GSTIN, FILE_ID, DERIVED_RET_PERIOD "
					+ "FROM ANX_OUTWARD_DOC_ERROR WHERE ITEM_INDEX IS NULL AND ERROR_SOURCE='ASP' "
					+ buildQuery +" ) "
					+ " GROUP BY "
					+ "DOC_HEADER_ID, ITEM_INDEX,FILE_ID, ITM_NO,SUPPLIER_GSTIN, DERIVED_RET_PERIOD ) "
					+ "ASPH_ER ON HDR.ID = ASPH_ER.DOC_HEADER_ID AND ASPH_ER.FILE_ID = HDR.FILE_ID "
					+ "AND ASPH_ER.SUPPLIER_GSTIN = HDR.SUPPLIER_GSTIN AND "
					+ "ASPH_ER.DERIVED_RET_PERIOD = HDR.DERIVED_RET_PERIOD INNER JOIN "
					+ "ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID AND "
					+ "HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD LEFT OUTER JOIN "
					+ "(SELECT DOC_HEADER_ID , ITEM_INDEX , ITM_NO , STRING_AGG(ERROR_CODE_GSTN,',') "
					+ "ERROR_CODE_GSTN , STRING_AGG(ERROR_DESCRIPTION_GSTN,',') ERROR_DESCRIPTION_GSTN , "
					+ "STRING_AGG(INFO_ERROR_CODE_GSTN,',') INFO_ERROR_CODE_GSTN , "
					+ "STRING_AGG(INFO_ERROR_DESCRIPTION_GSTN,',') INFO_ERROR_DESCRIPTION_GSTN , "
					+ "STRING_AGG(ERROR_CODE_ASP,',') ERROR_CODE_ASP , STRING_AGG(ERROR_DESCRIPTION_ASP,',') "
					+ "ERROR_DESCRIPTION_ASP , STRING_AGG(INFO_ERROR_CODE_ASP,',') INFO_ERROR_CODE_ASP , "
					+ "STRING_AGG(INFO_ERROR_DESCRIPTION_ASP,',') INFO_ERROR_DESCRIPTION_ASP ,FILE_ID, "
					+ "SUPPLIER_GSTIN, DERIVED_RET_PERIOD FROM ( SELECT DOC_HEADER_ID, ITEM_INDEX, ITM_NO, "
					+ " ' 'AS ERROR_CODE_GSTN,' ' AS ERROR_DESCRIPTION_GSTN, ' ' AS INFO_ERROR_CODE_GSTN, "
					+ " ' ' AS INFO_ERROR_DESCRIPTION_GSTN, (CASE WHEN ERROR_TYPE='ERR' THEN ERROR_CODE END) "
					+ "AS ERROR_CODE_ASP, (CASE WHEN ERROR_TYPE='ERR' THEN ERROR_DESCRIPTION END) "
					+ "AS ERROR_DESCRIPTION_ASP , (CASE WHEN ERROR_TYPE='INFO' THEN ERROR_CODE END) "
					+ "AS INFO_ERROR_CODE_ASP, (CASE WHEN ERROR_TYPE='INFO' THEN ERROR_DESCRIPTION END) "
					+ "AS INFO_ERROR_DESCRIPTION_ASP , SUPPLIER_GSTIN, FILE_ID, DERIVED_RET_PERIOD "
					+ "FROM ANX_OUTWARD_DOC_ERROR WHERE ITEM_INDEX IS NOT NULL AND ERROR_SOURCE='ASP' "
					+ buildQuery +" ) "
					+ " GROUP BY "
					+ "DOC_HEADER_ID, ITEM_INDEX, FILE_ID,ITM_NO,SUPPLIER_GSTIN, DERIVED_RET_PERIOD ) "
					+ "ASPI_ER ON ITM.DOC_HEADER_ID = ASPI_ER.DOC_HEADER_ID AND ITM.ITM_NO = ASPI_ER.ITM_NO "
					+ "AND ASPI_ER.DERIVED_RET_PERIOD= HDR.DERIVED_RET_PERIOD AND "
					+ "ASPI_ER.SUPPLIER_GSTIN= HDR.SUPPLIER_GSTIN AND ASPI_ER.FILE_ID = HDR.FILE_ID "
					+ "AND ASPH_ER.FILE_ID = ASPI_ER.FILE_ID LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID "
					+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON "
					+ "GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN EINV_MASTER EINV "
					+ "ON EINV.DOC_HEADER_ID = HDR.ID AND ITM.DOC_HEADER_ID = EINV.DOC_HEADER_ID "
