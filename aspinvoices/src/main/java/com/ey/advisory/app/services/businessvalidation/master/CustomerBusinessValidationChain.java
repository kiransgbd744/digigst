 ...
 *
 */
@Component("CustomerBusinessValidationChain")
public class CustomerBusinessValidationChain 
                   implements BusinessRuleValidator<MasterCustomerEntity> {
	private static final Logger LOGGER = 
			LoggerFactory.getLogger(CustomerBusinessValidationChain.class);
...
private static final BusinessRuleValidator[] VALIDATORS = {
		//new CustomerSgstin(),
		new CustomerRgstin(),
		new CustomerLeagalName()
	
};

@Override
public List<ProcessingResult> validate(MasterCustomerEntity cust,
		ProcessingContext context) {
	List<ProcessingResult> results = new ArrayList<>();
	for(BusinessRuleValidator<MasterCustomerEntity> 
		...
		try {
			...
			List<ProcessingResult> tmpResults = 
					validator.validate(cust, context);
