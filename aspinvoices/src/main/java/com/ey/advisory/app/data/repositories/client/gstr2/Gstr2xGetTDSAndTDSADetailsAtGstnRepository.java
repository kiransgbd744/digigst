 ...
 *
 */
@Repository("Gstr2xGetTDSAndTDSADetailsAtGstnRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr2xGetTDSAndTDSADetailsAtGstnRepository
		extends JpaRepository<GetGstr2xTdsAndTdsaInvoicesEntity, Long> {
	// Curently isDelete is not added in table
	@Modifying
	@Query("UPDATE GetGstr2xTdsAndTdsaInvoicesEntity b SET b.isDelete = true ,b.modifiedBy = 'SYSTEM'  , b.modifiedOn =:modifiedOn  WHERE"
	...
	@Query("SELECT doc FROM GetGstr2xTdsAndTdsaInvoicesEntity doc "
			+ "WHERE doc.docKey IN (:docKey) AND " + " doc.isDelete = false ")
	List<GetGstr2xTdsAndTdsaInvoicesEntity> findByfilproKey(
			@Param("docKey") List<String> docKey);

	@Modifying
	@Query("UPDATE GetGstr2xTdsAndTdsaInvoicesEntity doc SET doc.userActionFlag = 'U' "
			+ "WHERE doc.docKey IN (:docKey) AND doc.isDelete = false")
	void updateUserActionFlag(@Param("docKey") List<String> docKey);

	@Query("SELECT doc.id FROM GetGstr2xTdsAndTdsaInvoicesEntity doc "
			+ "WHERE doc.docKey IN (:docKeys) AND doc.isDelete = false ")
	List<Long> findActiveDocsByDocKeys(@Param("docKeys") List<String> docKeys);

	@Modifying
	@Query("UPDATE GetGstr2xTdsAndTdsaInvoicesEntity doc SET doc.userActionFlag = 'E', "
	...
	@Query("SELECT doc FROM GetGstr2xTdsAndTdsaInvoicesEntity doc WHERE "
			...
			+ "doc.isDelete = false ")
	List<GetGstr2xTdsAndTdsaInvoicesEntity> findRecords(
	...
	List<GetGstr2xTdsAndTdsaInvoicesEntity> findRecordsWithOrgDrUplMonth(
			@Param("recordType") String recordType,
			@Param("gstin") String gstin, @Param("retPeriod") String retPeriod,
			@Param("ctin") String ctin,
			@Param("deductorUplMonth") String deductorUplMonth);
	...
	List<GetGstr2xTdsAndTdsaInvoicesEntity> findByfilPSproKey(
			@Param("docKey") List<String> docKey);
	...
	List<GetGstr2xTdsAndTdsaInvoicesEntity> findActiveRecords();
