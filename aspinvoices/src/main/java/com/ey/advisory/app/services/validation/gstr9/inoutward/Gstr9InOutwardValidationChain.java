 ...
 *
 */
@Component("Gstr9InOutwardValidationChain")
@Slf4j
public class Gstr9InOutwardValidationChain
		implements DocRulesValidatorChain<Gstr9OutwardInwardAsEnteredEntity> {
	...
	private static final B2csBusinessRuleValidator[] VALIDATORS = {
			new Gstr9InOutwardGstin(), new Gstr9InOutwardFyValidation(),
			new Gstr9InOutwardTaleNumber()
	};

	@Override
	public List<ProcessingResult> validate(
			Gstr9OutwardInwardAsEnteredEntity gstr9InOutward,
			ProcessingContext context) {
		List<ProcessingResult> results = new ArrayList<>();
		for (B2csBusinessRuleValidator<Gstr9OutwardInwardAsEnteredEntity> validator : VALIDATORS) {
			if (validator == null) {
			...
			try {
				...
				List<ProcessingResult> tmpResults = validator
						.validate(gstr9InOutward, context);
