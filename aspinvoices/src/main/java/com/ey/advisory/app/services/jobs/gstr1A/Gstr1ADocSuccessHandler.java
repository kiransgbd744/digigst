...
@Service("Gstr1ADocSuccessHandler")
@Slf4j
public class Gstr1ADocSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("gstr1ADocIssuesDataParserImpl")
	private Gstr1ADocIssuesDataParser gstr1ADocIssuesDataParser;

	@Autowired
	@Qualifier("Gstr1AGetDocGstnRepository")
	private Gstr1AGetDocGstnRepository gstr1AGetDocGstnRepository;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Gstr1ADocSuccessHandler.class",
						ctxParamsObj.get("batchId").getAsLong());
			}
			
			TenantContext.setTenantId(dto.getGroupcode());
			if (resultIds != null && !resultIds.isEmpty()) {
				gstr1AGetDocGstnRepository.softlyDeleteDocHeader(
						dto.getGstin(), dto.getReturnPeriod());
			}
			resultIds.forEach(id -> {
				String apiResp = APIInvokerUtil.getResultById(id);
				// Long batchId = null;
				List<GetGstr1ADocIssuedEntity> docEnties = gstr1ADocIssuesDataParser
						.dataParseFromGstr1(dto, apiResp);
				if (!docEnties.isEmpty()) {
					
					gstr1AGetDocGstnRepository.saveAll(docEnties);
				}
			});
