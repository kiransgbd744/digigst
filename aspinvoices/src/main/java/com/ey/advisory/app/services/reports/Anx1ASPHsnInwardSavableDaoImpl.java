...
@Component("Anx1ASPHsnInwardSavableDaoImpl")
public class Anx1ASPHsnInwardSavableDaoImpl
		implements Anx1AspInwardSavableSummaryDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ASPIMPSSavableSummaryLevelDaoImpl.class);

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	@Override
	public List<Object> getAnx1HsnInwardSavableReports(
		...
		String queryStr = createHsnInwardSummaryQueryString(
				buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertHsnInwardSummary(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1ProcessedRecordsAspInwardDto convertHsnInwardSummary(
			Object[] arr) {
	...
	private String createHsnInwardSummaryQueryString(String buildQuery) {

		return "SELECT HDR.USER_ID, HDR.SOURCE_FILENAME,"
				...
				+ "HDR.DOC_DATE, TO_CHAR(ITM.ORIGINAL_DOC_NUM) AS "
				+ "ITM_ORIGINAL_DOC_NUM, TO_CHAR(ITM.ORIGINAL_DOC_DATE) AS"
				+ " ITM_ORIGINAL_DOC_DATE, HDR.CRDR_PRE_GST, ITM.ITM_NO,"
				+ " HDR.SUPPLIER_GSTIN, HDR.CUST_SUPP_TYPE,HDR.DIFF_PERCENT,"
				...
				+ " ITM.CIF_VALUE, ITM.CUSTOM_DUTY, '' AS ITM_HSNSAC,"
				+ "ITM.PRODUCT_CODE,ITM.ITM_DESCRIPTION,ITM.ITM_TYPE, "
				+ "ITM.ITM_UQC, ITM.ITM_QTY, HDR.SECTION7_OF_IGST_FLAG,"
				+ "SUM(ITM.TAXABLE_VALUE), ITM.IGST_RATE,SUM(ITM.IGST_AMT),"
				+ " ITM.CGST_RATE,SUM(ITM.CGST_AMT), ITM.SGST_RATE, "
				+ "SUM(ITM.SGST_AMT), ITM.CESS_RATE_ADVALOREM,"
				+ " SUM(ITM.CESS_AMT_ADVALOREM), SUM(ITM.CESS_AMT_SPECIFIC),"
				+ " ITM.CESS_RATE_SPECIFIC,ITM.STATECESS_RATE,SUM(ITM.STATECESS_AMT),"
				+ " SUM(ITM.OTHER_VALUES), SUM(HDR.DOC_AMT) AS INV_VALUE,"
				+ " HDR.CLAIM_REFUND_FLAG,HDR.AUTOPOPULATE_TO_REFUND,"
				+ " ITM.ADJ_REF_NO,ITM.ADJ_REF_DATE, SUM(ITM.ADJ_TAXABLE_VALUE),"
				+ "SUM(ITM.ADJ_IGST_AMT), SUM(ITM.ADJ_CGST_AMT), "
				+ "SUM(ITM.ADJ_SGST_AMT), SUM(ITM.ADJ_CESS_AMT_ADVALOREM),"
				+ " SUM(ITM.ADJ_CESS_AMT_SPECIFIC), SUM(ITM.ADJ_STATECESS_AMT),"
				+ " HDR.REVERSE_CHARGE, TO_CHAR(ITM.ELIGIBILITY_INDICATOR) AS"
				+ " ITM_ELIGIBILITY_INDICATOR, ITM.COMMON_SUP_INDICATOR,"
				+ "SUM(ITM.AVAILABLE_IGST), SUM(ITM.AVAILABLE_CGST),"
				+ " SUM(ITM.AVAILABLE_SGST), SUM(ITM.AVAILABLE_CESS), "
				+ "HDR.ITC_ENTITLEMENT,HDR.ITC_REVERSAL_IDENTIFER,"
				+ " ITM.CRDR_REASON,ITM.PURCHASE_VOUCHER_NUM,"
				+ " ITM.PURCHASE_VOUCHER_DATE,HDR.POSTING_DATE,"
				+ " ITM.PAYMENT_VOUCHER_NUM,ITM.PAYMENT_DATE,"
				...
				+ " HDR.AN_RETURN_TYPE AS RETURN_TYPE, HDR.AN_TAX_DOC_TYPE AS"
				+ " DATA_CATEGORY, HDR.AN_TABLE_SECTION AS TABLE_NUMBER,TRIM(', ' "
				+ "FROM IFNULL(ERRH.INFO_ERROR_CODE_ASP,'') ||','|| IFNULL"
				...
				+ "INFO_ERROR_DESCRIPTION_ASP,SUM(HDR.DOC_AMT) AS INV_VALUE_ASP,"
				+ "case when HDR.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED'"
				+ " WHEN HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = "
				+ "TRUE THEN 'IS_ERROR' WHEN HDR.IS_SAVED_TO_GSTN = FALSE AND"
				+ " HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END AS SAVE_STATUS,"
				+ "GSB.GSTN_SAVE_REF_ID AS GSTIN_REF_ID,GSB.GSTN_RESP_DATE AS"
				+ " GSTIN_REF_ID_TIME, TRIM(', ' FROM IFNULL(ERRH.ERROR_CODE_GSTIN,'') "
				+ "||','|| IFNULL(ERRI.ERROR_CODE_GSTIN,'')) AS GSTIN_ERROR_CODE_ASP, "
				+ "TRIM(', ' FROM IFNULL(ERRH.ERROR_DESCRIPTION_GSTIN,'') ||','||"
				+ " IFNULL(ERRI.ERROR_DESCRIPTION_GSTIN,'')) AS"
				+ " GSTIN_ERROR_DESCRIPTION_ASP, FIL.SOURCE AS UPLOAD_SOURCE,"
				+ " FIL.CREATED_BY AS SOURCE_ID,FIL.FILE_NAME, HDR.CREATED_ON "
				+ "AS ASP_DATE_TIME FROM ANX_INWARD_DOC_HEADER HDR LEFT OUTER"
				+ " JOIN TF_INWARD_HEADER_GSTIN_ASP_ERROR_INFO () ERRH ON"
				...
				+ " TF_INWARD_ITEM_GSTIN_ASP_ERROR_INFO () ERRI ON"
				+ " ITM.DOC_HEADER_ID= ERRI.DOC_HEADER_ID AND IFNULL"
				+ "(ITM.ITEM_INDEX,'-1' ) = IFNULL(ERRI.ITEM_INDEX,'-1' ) "
				+ "AND IFNULL(ITM.ITM_NO,'0' ) = IFNULL(ERRI.ITM_NO,'0' )"
				+ " INNER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID LEFT"
				+ " OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID = "
				+ "HDR.BATCH_ID WHERE IS_ERROR=FALSE AND HDR.AN_RETURN_TYPE IN ('ANX1') "
				+ buildQuery
				+ "GROUP BY HDR.ID,HDR.USER_ID, HDR.AN_RETURN_TYPE ,"
				+ " HDR.AN_TAX_DOC_TYPE , HDR.AN_TABLE_SECTION ,"
				+ " HDR.SOURCE_FILENAME, ITM.PROFIT_CENTRE,ITM.PLANT_CODE,"
				+ "HDR.DIVISION, ITM.LOCATION,HDR.PURCHASE_ORGANIZATION, "
				+ "HDR.BILL_OF_ENTRY,HDR.BILL_OF_ENTRY_DATE, HDR.ITC_ENTITLEMENT,"
				...
				+ "HDR.DOC_NUM,HDR.DOC_DATE,ITM.ORIGINAL_DOC_DATE,"
				+ " ITM.ORIGINAL_DOC_NUM,HDR.CRDR_PRE_GST,HDR.CUST_GSTIN, "
				+ "HDR.CUST_SUPP_TYPE,HDR.DIFF_PERCENT, HDR.ORIG_SUPPLIER_GSTIN,"
				+ "HDR.CUST_SUPP_NAME, HDR.CUST_SUPP_CODE,HDR.CUST_SUPP_ADDRESS1,"
				+ " HDR.CUST_SUPP_ADDRESS2,HDR.CUST_SUPP_ADDRESS3, "
				+ "HDR.CUST_SUPP_ADDRESS4,HDR.POS,HDR.STATE_APPLYING_CESS, "
				+ "HDR.SHIP_PORT_CODE,HDR.SECTION7_OF_IGST_FLAG, HDR.REVERSE_CHARGE,"
				...
				+ "ITM.GLCODE_STATE_CESS,ITM.ITM_NO,ITM.PRODUCT_CODE,"
				+ "ITM.COMMON_SUP_INDICATOR, ITM.PURCHASE_VOUCHER_DATE,"
				+ "ITM.PURCHASE_VOUCHER_NUM,ERRH.ITM_NO, ITM.PAYMENT_DATE,"
				+ "ITM.ITM_DESCRIPTION,ITM.ITM_TYPE, ITM.ITM_UQC, ITM.ITM_QTY, "
				+ "ITM.CRDR_REASON, ITM.USERDEFINED_FIELD1,ITM.USERDEFINED_FIELD2,"
				+ " ITM.USERDEFINED_FIELD3,ITM.USERDEFINED_FIELD4, "
				+ "ITM.USERDEFINED_FIELD5,ITM.USERDEFINED_FIELD6, "
				+ "ITM.USERDEFINED_FIELD7,ITM.USERDEFINED_FIELD8, "
				+ "ITM.USERDEFINED_FIELD9,ITM.USERDEFINED_FIELD10, "
				+ "ITM.USERDEFINED_FIELD11,ITM.USERDEFINED_FIELD12, "
				+ "ITM.USERDEFINED_FIELD13,ITM.USERDEFINED_FIELD14, "
				+ "ITM.USERDEFINED_FIELD15, ITM.ADJ_REF_NO,ITM.ADJ_REF_DATE,"
				+ " ITM.IGST_RATE,ITM.CGST_RATE, ITM.PAYMENT_VOUCHER_NUM,"
				+ "ITM.CONTRACT_NUMBER, ITM.CONTRACT_DATE, ITM.CONTRACT_VALUE,"
				+ " ITM.SGST_RATE, ITM.CESS_RATE_SPECIFIC, ITM.CESS_RATE_ADVALOREM,"
				+ " ITM.STATECESS_RATE, ITM.CIF_VALUE, ITM.CUSTOM_DUTY,"
				+ "ERRH.INFO_ERROR_CODE_ASP,ERRI.INFO_ERROR_CODE_ASP,"
				+ " ERRH.INFO_ERROR_DESCRIPTION_ASP, ERRI.INFO_ERROR_DESCRIPTION_ASP,"
				+ " ITM.ORIGINAL_DOC_NUM, ITM.ORIGINAL_DOC_DATE,"
				+ " ITM.ELIGIBILITY_INDICATOR, HDR.FILE_ID,FIL.ID, FIL.SOURCE ,"
				+ " FIL.CREATED_BY ,FIL.FILE_NAME, ERRH.ERROR_CODE_GSTIN,"
				+ "ERRI.ERROR_CODE_GSTIN, ERRH.ERROR_DESCRIPTION_GSTIN, "
				+ "ERRI.ERROR_DESCRIPTION_GSTIN, GSB.GSTN_SAVE_REF_ID,"
				+ "GSB.GSTN_RESP_DATE , HDR.CREATED_ON ,IS_SAVED_TO_GSTN,GSTN_ERROR"
