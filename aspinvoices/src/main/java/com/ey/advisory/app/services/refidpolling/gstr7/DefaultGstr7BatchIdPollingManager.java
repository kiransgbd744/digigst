 ...
 */
@Slf4j
@Component("DefaultGstr7BatchIdPollingManager")
public class DefaultGstr7BatchIdPollingManager
	...
	@Autowired
	@Qualifier("DefaultGstr7GstnResponseHandler")
	private Gstr7GstnResponseHandler gstr7GstnResponseHandler;
	...
	public ReturnStatusRefIdDto processBatch(Gstr1SaveBatchEntity batch) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Pooling Gstr7 ReturnStatus Sandbox status is {} ",
						gstnStatus);
			}
			...
			if (P.equalsIgnoreCase(gstnStatus)) {
				...
				gstr7GstnResponseHandler.phandleResponse(null, batch);
			} else if (PE.equalsIgnoreCase(gstnStatus)) {
				LOGGER.info("PE is processed.");
				saveToGstnEventStatus.EventEntry(batch.getReturnPeriod(),
						batch.getSgstin(), 50, batch.getGroupCode());
				gstr7GstnResponseHandler.pEhandleResponse(jsonRoot, batch);
				...
				gstr7GstnResponseHandler.ehandleResponse(null, batch);
			}else {				
				LOGGER.error("Pooling Gstr7 ReturnStatus- intermediate status"
						+ gstnStatus);
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Pooling Gstr7 ReturnStatus- IP/REC response handling started for {} ",
							batch.getSection());
				}
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Pooling Gstr7 ReturnStatus Processing Completed.");
			}
		} catch (Exception ex) {
			batchSaveStatusRepository.updateStatusById(batch.getId(),
					APIConstants.POLLING_FAILED, now);
