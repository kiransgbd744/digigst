 ...
 *
 */
public class Gstr7TdsIgstValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errorLocations.add(GSTConstants.TDS_IGST);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2015",
					"Invalid TDS IGST", location));
		...
		if (result.compareTo(BigDecimal.ZERO) < 0) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2016",
