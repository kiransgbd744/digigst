/**
 * 
 */
package com.ey.advisory.app.data.services.pdf;

import java.io.BufferedOutputStream;
import java.io.File;
import java.io.FileOutputStream;
import java.io.FilenameFilter;
import java.io.OutputStream;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.stream.Collectors;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.stereotype.Component;

import com.ey.advisory.app.data.repositories.client.FileStatusDownloadReportRepository;
import com.ey.advisory.app.docs.dto.Gstr1CompleteSummaryDto;
import com.ey.advisory.common.CombineAndZipXlsxFiles;
import com.ey.advisory.core.dto.Annexure1SummaryReqDto;

import lombok.extern.slf4j.Slf4j;
import net.sf.jasperreports.engine.JasperExportManager;
import net.sf.jasperreports.engine.JasperPrint;

/**
 * @author Laxmi.Salukuti
 *
 */

@Component("Gstr1SummaryMultiPDFService")
@Slf4j
public class Gstr1SummaryMultiPDFService {

	@Autowired
	@Qualifier("Gstr1SummaryMultiPDFGenerationReportImpl")
	Gstr1SummaryMultiPDFGenerationReport gstr1SummaryMultiPDFGenerationReport;

	@Autowired
	FileStatusDownloadReportRepository downloadRepository;

	public String generateGstr1SummaryPdfZip(File tempDir,
			Annexure1SummaryReqDto annexure1SummaryRequest,
			List<? extends Gstr1CompleteSummaryDto> gstnResult, String sgstin,
			Long id, Boolean isDigigst, boolean isGstr1a) throws Exception {
		JasperPrint jasperPrint = null;

		// for (String sgstin : gstinList) {

		String fileName = null;
		if(Boolean.TRUE.equals(isGstr1a)){
			fileName = "Gstr1AEntityLevelSummaryPDFReport" + "_" + sgstin;
		} else {
			fileName = "Gstr1EntityLevelSummaryPDFReport" + "_" + sgstin;
		}
		String fullPath = tempDir.getAbsolutePath() + File.separator + fileName
				+ ".pdf";
		try (OutputStream outStream = new BufferedOutputStream(
				new FileOutputStream(fullPath), 8192)) {

			if (isDigigst) {
				jasperPrint = gstr1SummaryMultiPDFGenerationReport
						.generateGstr1SummaryPdfReport(annexure1SummaryRequest,
								gstnResult, sgstin, isDigigst);
			} else {
				jasperPrint = gstr1SummaryMultiPDFGenerationReport
						.generateGstr1SummaryPdfReportNew(annexure1SummaryRequest,
								gstnResult, sgstin, isDigigst);
			}
			
			if (LOGGER.isDebugEnabled()) {
				String msg = " exportReportToPdfStream method begings";
				LOGGER.debug(msg);
			}

			JasperExportManager.exportReportToPdfStream(jasperPrint, outStream);
			
			if (LOGGER.isDebugEnabled()) {
				String msg = " exportReportToPdfStream method executed successfully";
				LOGGER.debug(msg);
			}

			outStream.flush();
			outStream.close();

		} catch (Exception e) {
			String msg = String.format(
					"Exception occured during report generation for gstr1");
			LOGGER.error(msg, e);
			return "FAILED";
		}
		return "SUCCESS";

	}

	public String zipEinvoicePdfFiles(File tempDir, Long id, boolean isGstr1a) throws Exception {

		if (LOGGER.isDebugEnabled()) {
			String msg = " getting files to be zipped from temp directory";
			LOGGER.debug(msg);
		}
		
		List<String> filesToZip = getAllFilesToBeZipped(tempDir);
		String fileName = null;
		if(Boolean.TRUE.equals(isGstr1a)){
			fileName = "Gstr1AEntityLevelSummaryPDFReport_" + id;
		} else {
			fileName = "Gstr1EntityLevelSummaryPDFReport_" + id;
		}
		String compressedFileName = fileName;

		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("compressedFileName : %s",
					compressedFileName);
			LOGGER.debug(msg);
		}

		CombineAndZipXlsxFiles.compressFiles(tempDir.getAbsolutePath(),
				compressedFileName + ".zip", filesToZip);
		String zipFileName = compressedFileName + ".zip";

		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("zipFileName : %s", zipFileName);
			LOGGER.debug(msg);
		}
		return zipFileName;

	}

	private static List<String> getAllFilesToBeZipped(File tmpDir)
			throws Exception {

		if (LOGGER.isDebugEnabled()) {
			String msg = String
					.format("Temporary directory containing files to "
							+ "be zipped: %s", tmpDir.getAbsolutePath());
			LOGGER.debug(msg);
		}
		FilenameFilter pdfFilter = new FilenameFilter() {
			public boolean accept(File tmpDir, String name) {
				return name.toLowerCase().endsWith(".pdf");
			}
		};
		File[] files = tmpDir.listFiles(pdfFilter);
		List<String> retFileNames = Arrays.stream(files)
				.map(f -> f.getAbsolutePath())
				.collect(Collectors.toCollection(ArrayList::new));

		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("List of files to be zipped %s",
					retFileNames);
			LOGGER.debug(msg);
		}
		// Return the list of files.
		return retFileNames;
	}

}
