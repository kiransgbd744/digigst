 ...
 *
 */
@Repository("Gstr1NilNonExtAsRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr1NilNonExtAsRepository
		extends JpaRepository<Gstr1NilNonExemptedAsEnteredEntity, Long>,
		JpaSpecificationExecutor<Gstr1NilNonExemptedAsEnteredEntity> {

	@Query("SELECT INV FROM Gstr1NilNonExemptedAsEnteredEntity INV WHERE INV.nKey "
			+ "IN (:structuralValKeys) AND INV.id IN (:id) ")

	List<Gstr1NilNonExemptedAsEnteredEntity> getAllExcelData(
			@Param("structuralValKeys") List<String> structuralValKeys,
			@Param("id") List<Long> id);

	@Modifying
	@Query("UPDATE Gstr1NilNonExemptedAsEnteredEntity INV SET INV.isError = TRUE "
			+ "WHERE INV.nKey IN (:invKey) AND INV.id IN (:ids) ")
	public void invUpdateStructuralError(@Param("invKey") List<String> invKey,
			@Param("ids") List<Long> ids);

	@Modifying
	@Query("UPDATE Gstr1NilNonExemptedAsEnteredEntity INV SET INV.isInfo = TRUE,"
			+ " INV.isDelete = false "
	...
	@Modifying
	@Query("UPDATE Gstr1NilNonExemptedAsEnteredEntity EQ SET EQ.isError = TRUE"
	...
	List<Gstr1NilNonExemptedAsEnteredEntity> getAllProcessedWithInfoData(
			@Param("structuralValKeys") List<String> structuralValKeys,
			@Param("id") List<Long> id);
}
