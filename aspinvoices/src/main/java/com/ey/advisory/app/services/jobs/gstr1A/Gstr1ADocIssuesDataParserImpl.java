...
@Component("gstr1ADocIssuesDataParserImpl")
public class Gstr1ADocIssuesDataParserImpl implements Gstr1ADocIssuesDataParser {

	@Autowired
	private Gstr1GetKeyGenerator gstr1GetKeyGenerator;

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr1ADocIssuesDataParserImpl.class);

	@Override
	public List<GetGstr1ADocIssuedEntity> dataParseFromGstr1(Gstr1GetInvoicesReqDto dto, String apiResp) {

		JsonObject jsonObject = null;
		Gson gson = GsonUtil.newSAPGsonInstance();
		List<GetGstr1ADocIssuedEntity> entities = new ArrayList<>();
		//try {
			...
			docIssueDetails.forEach(docIssueD -> {
				...
				docIssueList.forEach(docIssueL -> {
					// New  Entity
					GetGstr1ADocIssuedEntity docIssuesInvoicesEntity = new GetGstr1ADocIssuedEntity();
