 ...
 */
@Slf4j
@Service("Gstr1GstnResetDataJobHandler")
public class Gstr1GstnResetDataJobHandler {
			...
			if ((gstin != null && !gstin.isEmpty()) && (retPeriod != null && !retPeriod.isEmpty())) {
				...
				if (deleteData != null) {
					...
					try {
						...
						resp = gstnServer.gstr1DeleteApiCall(groupCode, batch, gstin, retPeriod, gstnBatchId);
					} catch (java.lang.UnsupportedOperationException | AppException ex) {
						LOGGER.error("GSTR1 delete from gstn api call exception", ex.getMessage());
				...
				//	String txnId = null;
					...
					oneResp.setReturnType(APIConstants.GSTR1.toUpperCase());*/
