...
@Component
@Scope("prototype")
public class InvoiceSeriesGenerator {

	private static final String CLASS_NAME = InvoiceSeriesGenerator.class
			.getName();
		...
		try {
			...
			if (list.size() == 1) {
				...
				if (hasListEmpty(filteredList)) {
					...
					List<Long> getNumbersList = convertInvoiceNumberFromStringToLong(
							list);
					...
					LOGGER.debug(CLASS_NAME + "::" + "Min value:" + minValue);
					Long maxValue = longSummaryStatistics.getMax();
					LOGGER.debug(CLASS_NAME + "::" + "Max value:" + maxValue);
					String delimiter = invsAlphaPattrenDto
							.getAlphaPattren() == null ? (String) delimterType
					...
					if (Optional.ofNullable(
							invsAlphaPattrenDto.getAlphaPattrenDelimiter())
							.isPresent()) {
						minBuilder.append(
								invsAlphaPattrenDto.getAlphaPattrenDelimiter());
						if (Optional
					...
					totalDocumentsIssued = getTotalValue(maxValue, minValue);
					if (totalDocumentsIssued == 1 && Optional
							.ofNullable(minAndMaxDelimiterList).isPresent()) {
						longSummaryStatistics = minAndMaxDelimiterList.stream()
								.collect(Collectors
										.summarizingLong(Long::valueOf));
						minValue = longSummaryStatistics.getMin();
						LOGGER.debug(
								CLASS_NAME + "::" + "Min value:" + minValue);
						maxValue = longSummaryStatistics.getMax();
						LOGGER.debug(
								CLASS_NAME + "::" + "Max value:" + maxValue);
						totalDocumentsIssued = getTotalValue(maxValue,
								minValue);
						LOGGER.debug(
								CLASS_NAME + "::" + "Total issued items count :"
						...
						// .count();
						LOGGER.debug(
								CLASS_NAME + "::" + "Total Cancel items Count::"
					...
					prepareFinalinvoiceSeries(gstinId, taxPeriod,
							...
							netNumber, invoiceType, delimterType.toString());
				}
			}
		} catch (Exception e) {
			String errMsg = String.format(
