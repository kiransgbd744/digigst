...
import com.ey.advisory.app.docs.dto.anx1.Anx1DeemadExportsDocumentDetails;
 ...
 *
 */
@Service("anx1RateDataToDeemedExportsConverter")
@Slf4j
public class Anx1RateDataToDeemedExportsConverter
	...
	public SaveBatchProcessDto convertToAnx1Object(List<Object[]> objects,
		...
		try {
			if (objects != null && !objects.isEmpty()) {
				...
				List<Anx1DeemadExportsData> deList = new ArrayList<>();
				List<Anx1DeemadExportsDocumentData> invList = new ArrayList<>();
				List<Anx1DeemadExportsItemDetails> itmsList = new ArrayList<>();
				...
				for (int counter = 0; counter < totSize; counter++) {
					...
					Anx1DeemadExportsItemDetails itms = setItemDetail(arr1);
					itmsList.add(itms);
					if (isNewInvoice(arr1, arr2, idsList, totSize, counter2)) {
						 ...
						 */
						idsList.add(id);
						Anx1DeemadExportsDocumentData inv = setInvData(arr1,
								itmsList, taxDocType);
						invList.add(inv);
					...
					if (isNewCtin(arr1, arr2, idsList, totSize, counter2)) {
						Anx1DeemadExportsData de = setInv(arr1, invList);
						deList.add(de);
						itmsList = new ArrayList<>();
					...
					if (isNewBatch(idsList, totSize, counter2)) {
						SaveAnx1 anx1 = setBatch(arr1, section, deList);
	...
	private Anx1DeemadExportsItemDetails setItemDetail(Object[] arr1) {

		BigDecimal cessAmt = arr1[19] != null ? new BigDecimal(String.valueOf(arr1[19])) : BigDecimal.ZERO;
		BigDecimal sgstAmt = arr1[18] != null ? new BigDecimal(String.valueOf(arr1[18])) : BigDecimal.ZERO;
		BigDecimal cgstAmt = arr1[17] != null ? new BigDecimal(String.valueOf(arr1[17])) : BigDecimal.ZERO;
		BigDecimal igstAmt = arr1[16] != null ? new BigDecimal(String.valueOf(arr1[16])) : BigDecimal.ZERO;
		BigDecimal taxableVal = arr1[15] != null ? new BigDecimal(String.valueOf(arr1[15])) : null;
		BigDecimal taxRate = arr1[14] != null ? new BigDecimal(String.valueOf(arr1[14])) : null;
		String hsnsac = arr1[13] != null ? String.valueOf(arr1[13]) : null;
		
		Anx1DeemadExportsItemDetails item = new Anx1DeemadExportsItemDetails();

		String pos = arr1[12] != null ? String.valueOf(arr1[12]) : null;
	...
	private Anx1DeemadExportsDocumentData setInvData(Object[] arr1,
			List<Anx1DeemadExportsItemDetails> itmsList, String taxDocType) {
		...
		String clmrfnd = arr1[11] != null ? String.valueOf(arr1[11]) : APIConstants.N;
		String pos = arr1[12] != null ? String.valueOf(arr1[12]) : null;

		Anx1DeemadExportsDocumentDetails invDetail = new Anx1DeemadExportsDocumentDetails();
		invDetail.setDocDate(docDate);
		invDetail.setDocNum(docNum);
		invDetail.setDocVal(docAmt);
		Anx1DeemadExportsDocumentData inv = new Anx1DeemadExportsDocumentData();
	...
	private Anx1DeemadExportsData setInv(Object[] arr1, List<Anx1DeemadExportsDocumentData> invList) {

		String cGstin = arr1[3] != null ? String.valueOf(arr1[3]) : null;
		Anx1DeemadExportsData de = new Anx1DeemadExportsData();
		de.setCgstin(cGstin);
		de.setDocs(invList);
		return de;
	}
	...
	private SaveAnx1 setBatch(Object[] arr1, String section,
			List<Anx1DeemadExportsData> deList) {
