...
import com.ey.advisory.app.data.views.client.Anx1InwardApiProcessedInfoRecordsDto;
...
@Component("Anx1InwardApiProcessedInfoReportsDaoImpl")
public class Anx1InwardApiProcessedInfoReportsDaoImpl
		implements Anx1InwardApiProcessedInfoReportsDao {
	...
	@Override
	public List<Anx1InwardApiProcessedInfoRecordsDto> getApiProcessedInfoReports(
		...
		String queryStr = createInwardApiProcessedInfoRecQueryString(
				buildQuery.toString());
		...
		return list.parallelStream()
				.map(o -> convertApiInwardProcessedInfoRecords(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1InwardApiProcessedInfoRecordsDto convertApiInwardProcessedInfoRecords(
			Object[] arr) {
		Anx1InwardApiProcessedInfoRecordsDto obj = new Anx1InwardApiProcessedInfoRecordsDto();
	...
	private String createInwardApiProcessedInfoRecQueryString(
			String buildQuery) {
		return "SELECT HDR.ID, HDR.USER_ID,"
				...
				+ "HDR.REVERSE_CHARGE,HDR.POSTING_DATE,HDR.ITC_REVERSAL_IDENTIFER,"
				...
				+ "WHERE HDR.IS_ERROR = FALSE AND IS_INFORMATION = TRUE AND "
				+ "HDR.DATAORIGINTYPECODE IN ('A','AI') " + buildQuery
