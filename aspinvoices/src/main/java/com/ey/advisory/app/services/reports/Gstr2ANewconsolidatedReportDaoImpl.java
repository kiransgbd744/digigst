...
@Component("Gstr2ANewconsolidatedReportDaoImpl")
public class Gstr2ANewconsolidatedReportDaoImpl {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr2ANewconsolidatedReportDaoImpl.class);
	...
	public List<Object> getGstr1RSReports(
		...
		List<String> monthList = criteria.getMonth();
		...
		if (GSTIN != null && !GSTIN.isEmpty()) {
			if (gstinList != null && gstinList.size() > 0) {
				...
				buildQuery1.append(" AND CTIN IN :gstinList");
				buildHeader.append(" AND HDR.GSTIN IN :gstinList");

			}
		}

		buildQuery.append(
				" AND HDR.DERIVED_RET_PERIOD BETWEEN :derivedStartPeriod AND :derivedEndPeriod");
		buildQuery1.append(
				" AND HDR.DERIVED_RET_PERIOD BETWEEN :derivedStartPeriod AND :derivedEndPeriod");
		buildHeader.append(
				" AND HDR.DERIVED_RET_PERIOD BETWEEN :derivedStartPeriod AND :derivedEndPeriod");
		...
		if (monthList != null && monthList.size() > 0) {
			...
			buildQuery1
					.append(" AND SUBSTRING(TAX_PERIOD,1,2) IN (:monthList) ");
			buildHeader
					.append(" AND SUBSTRING(RET_PERIOD,1,2) IN (:monthList) ");

		}

		String queryStr = creategstnTransQueryString(buildQuery.toString(),
				buildQuery1.toString(), buildHeader.toString(),
		...
		if (monthList != null && monthList.size() > 0) {
			outquery.setParameter("monthList", monthList);
		}
	...
	private ConsolidatedGstr2ADto convertTransactionalLevel(Object[] arr) {
		...
		if (bigDecimalTaxVal != null) {
			if (docType != null && (docType.equalsIgnoreCase("CN")
					|| docType.equalsIgnoreCase("CNA"))) {
		...
		if (docType != null && (docType.equalsIgnoreCase("CN")
			...
			obj.setOriginalTaxableValue(CheckNegativeValue(
					arr[31]));
			obj.setOriginalIGSTAmount(CheckNegativeValue(
					arr[32]));
			obj.setOriginalCessAmount(CheckNegativeValue(
					arr[33]));
		} else {
			obj.setOriginalTaxableValue(
					arr[31] != null ? arr[31].toString() : null);
			obj.setOriginalIGSTAmount(
					arr[32] != null ? arr[32].toString() : null);
			obj.setOriginalCessAmount(
	...
	private String creategstnTransQueryString(String buildQuery,
			String buildQuery1, String buildHeader, String buildheader) {
		...
		return "SELECT RETURN_PERIOD,CGSTIN,SGSTIN,LEGAL_NAME,TRADE_NAME, DOC_TYPE,SUPPLY_TYPE ,TABLE_TYPE, "
				...
				+ "SUPPLIER_INV_DATE DOC_DATE, IFNULL(ITM.TAX_VALUE,0) TAXABLE_VALUE,TAX_RATE, IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "IFNULL(ITM.CGST_AMT,0) CGST_AMT, IFNULL(ITM.SGST_AMT,0) SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CGST_AMT,0) +IFNULL(ITM.SGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, "
				+ "SUPPLIER_INV_VAL AS INVOICE_VALUE, POS, STATE_NAME,'' PORT_CODE,0 BOE_NUM,'' BOE_CREATED_DATE, "
				...
				+ "ITM ON HDR.ID = ITM.HEADER_ID AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				...
				+ " GROUP BY TAX_PERIOD,CFS,CGSTIN,SGSTIN,LEGAL_NAME_BS ,"
				+ "TRADE_NAME,SUPPLIER_INV_NUM ,SUPPLIER_INV_DATE,ITM.TAX_VALUE,ITM.IGST_AMT,ITM.CGST_AMT,ITM.SGST_AMT,ITM.CESS_AMT,"
				...
				+ "SUPPLIER_INV_NUM DOC_NUM, SUPPLIER_INV_DATE DOC_DATE, IFNULL(ITM.TAX_VALUE,0) TAXABLE_VALUE,TAX_RATE, IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "IFNULL(ITM.CGST_AMT,0) CGST_AMT, IFNULL(ITM.SGST_AMT,0) SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CGST_AMT,0) +IFNULL(ITM.SGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, "
				+ "SUPPLIER_INV_VAL AS INVOICE_VALUE, POS,STATE_NAME,'' PORT_CODE,0 BOE_NUM, '' BOE_CREATED_DATE,"
				+ " '' BOE_REF_DATE,'' IS_AMENDMENT_BOE , '' ORIGINAL_SUPPLIER_GSTIN,'' ORIGINAL_SUPPLIER_TRADENAME, '' ORIGINAL_PORT_CODE ,"
				...
				+ "TAX_RATE, POS,STATE_NAME,HDR.ORG_INV_NUM,HDR.ORG_INV_DATE ,ITM.TAX_VALUE,ITM.IGST_AMT,ITM.CGST_AMT,ITM.SGST_AMT ,ITM.CESS_AMT,"
				+ "HDR.SUPPLIER_INV_VAL ,ORG_INV_AMD_PERIOD,ORG_INV_AMD_TYPE,RCHRG,HDR.FILE_DATE,HDR.FILE_PERIOD,HDR.CANCEL_DATE ,"
				...
				+ "CNT,TAX_PERIOD RETURN_PERIOD,HDR.CTIN CGSTIN,HDR.GSTIN SGSTIN, LEGAL_NAME_BS LEGAL_NAME,TRADE_NAME, "
				+ " NOTE_TYPE DOC_TYPE, INV_TYPE SUPPLY_TYPE, 'CDN' TABLE_TYPE,NOTE_NUMBER DOC_NUM,"
				+ "NOTE_DATE DOC_DATE,IFNULL(ITM.TAX_VALUE,0) TAXABLE_VALUE,TAX_RATE, IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "IFNULL(ITM.CGST_AMT,0) CGST_AMT, IFNULL(ITM.SGST_AMT,0) SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CGST_AMT,0) +IFNULL(ITM.SGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, "
				+ "NOTE_VALUE AS INVOICE_VALUE, POS,STATE_NAME, "
				...
				+ " HDR.D_FLAG AS CDN_DELINKING_FLAG , "
				...
				+ "HDR INNER JOIN GETGSTR2A_CDN_ITEM ITM ON HDR.ID = ITM.HEADER_ID "
				+ "AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				+ "LEFT JOIN (SELECT DISTINCT VENDOR_GSTIN AS GSTIN,"
				...
				+ buildQuery1 + " GROUP BY TAX_PERIOD,CFS,CTIN, "
				+ "HDR.GSTIN,LEGAL_NAME_BS,TRADE_NAME,NOTE_TYPE,NOTE_NUMBER,NOTE_DATE, TAX_RATE ,POS,STATE_NAME,INV_NUMBER,INV_DATE,ITM.TAX_VALUE,ITM.IGST_AMT,"
				+ "ITM.CGST_AMT,ITM.SGST_AMT ,ITM.CESS_AMT,HDR.NOTE_VALUE, ORG_INV_AMD_PERIOD,ORG_INV_AMD_TYPE,RCHRG,HDR.FILE_DATE,"
				+ "HDR.FILE_PERIOD,HDR.CANCEL_DATE , HDR.CFS_GSTR3B,DIFF_PERCENT,ITEM_NUMBER,HDR.CREATED_ON, HDR.DERIVED_RET_PERIOD,INV_TYPE, "
				+ "HDR.IRN_NUM,HDR.IRN_GEN_DATE,HDR.IRN_SOURCE_TYPE, HDR.D_FLAG "
				...
				+ "TRADE_NAME, NOTE_TYPE DOC_TYPE, INV_TYPE SUPPLY_TYPE, "
				+ " 'CDNA' TABLE_TYPE,NOTE_NUMBER DOC_NUM,NOTE_DATE DOC_DATE, IFNULL(ITM.TAX_VALUE,0) TAXABLE_VALUE,TAX_RATE, IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "IFNULL(ITM.CGST_AMT,0) CGST_AMT, IFNULL(ITM.SGST_AMT,0) SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CGST_AMT,0) +IFNULL(ITM.SGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, "
				+ "NOTE_VALUE AS INVOICE_VALUE, POS,STATE_NAME, '' PORT_CODE,0 BOE_NUM,'' BOE_CREATED_DATE,'' BOE_REF_DATE, "
				...
				+ "IS NOT NULL THEN 'Y' ELSE 'N' END) GSTR1_FILING_STATUS ,HDR.FILE_DATE GSTR1_FILING_DATE, HDR.FILE_PERIOD GSTR1_FILING_PERIOD ,"
				+ "HDR.CFS_GSTR3B GSTR3B_FILING_STATUS, HDR.CANCEL_DATE CANCELLATION_DATE , "
				+ " HDR.D_FLAG AS CDN_DELINKING_FLAG , '' CR_DR_PRE_GST,'' ITC_ELIGIBLE,DIFF_PERCENT,ITEM_NUMBER, '' ECOM_GSTIN,"
				...
				+ "TAX_RATE ,POS,STATE_NAME,ORG_NOTE_NUMBER, ORG_NOTE_DATE,INV_NUMBER,INV_DATE , ITM.TAX_VALUE,ITM.IGST_AMT,"
				+ "ITM.CGST_AMT,ITM.SGST_AMT ,ITM.CESS_AMT,HDR.NOTE_VALUE, ORG_INV_AMD_PERIOD,ORG_INV_AMD_TYPE,RCHRG,HDR.FILE_DATE,HDR.FILE_PERIOD , "
				+ "HDR.CANCEL_DATE,HDR.CFS_GSTR3B,DIFF_PERCENT,ITEM_NUMBER, HDR.CREATED_ON,HDR.DERIVED_RET_PERIOD,INV_TYPE, HDR.D_FLAG "
				...
				+ "IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "IFNULL(ITM.CGST_AMT,0) CGST_AMT, IFNULL(ITM.SGST_AMT,0) SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CGST_AMT,0) +IFNULL(ITM.SGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, "
				+ "0 INVOICE_VALUE , '' POS,STATE_NAME,'' PORT_CODE,0 BOE_NUM ,'' BOE_CREATED_DATE, '' BOE_REF_DATE,"
				+ " '' IS_AMENDMENT_BOE ,'' ORIGINAL_SUPPLIER_GSTIN, '' ORIGINAL_SUPPLIER_TRADENAME,'' ORIGINAL_PORT_CODE , "
				+ " '' ORIGINAL_BILL_OF_ENTRY_NUMBER,'' ORIGINAL_BILL_OF_ENTRY_DATE, '' ORIGINAL_BILL_OF_ENTRY_REF_DATE ,"
				+ " '' ORIGINAL_TAXABLE_VALUE, '' ORIGINAL_IGST_AMOUNT,'' ORIGINAL_CESS_AMOUNT ,'' ORG_DOC_NUM, '' ORG_DOC_DATE,"
				...
				+ buildHeader + " GROUP BY RET_PERIOD,CFS,CTIN, "
				+ "HDR.GSTIN,LEGAL_NAME_BS,TRADE_NAME,ISD_DOC_TYPE,DOC_NUM, ITM.IGST_AMT,"
				+ "ITM.CGST_AMT,ITM.SGST_AMT ,ITM.CESS_AMT, DOC_DATE,ITC_ELG ,"
				...
				+ "TABLE_TYPE,DOC_NUM, DOC_DATE, 0 TAXABLE_VALUE,0 TAX_RATE , "
				+ "IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "IFNULL(ITM.CGST_AMT,0) CGST_AMT, IFNULL(ITM.SGST_AMT,0) SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CGST_AMT,0) +IFNULL(ITM.SGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, "
				+ " 0 INVOICE_VALUE , '' POS,STATE_NAME,'' PORT_CODE,0 BOE_NUM , '' BOE_CREATED_DATE,"
				...
				+ "ISD_DOC_TYPE,DOC_NUM,DOC_DATE, ITM.IGST_AMT,"
				+ "ITM.CGST_AMT,ITM.SGST_AMT ,ITM.CESS_AMT, ITC_ELG ,"
				...
				+ "LEGAL_NAME_BS LEGAL_NAME,TRADE_NAME, 'IMPG' DOC_TYPE,'R' SUPPLY_TYPE,'IMPG' TABLE_TYPE,'' DOC_NUM,"
				+ " '' DOC_DATE, IFNULL(ITM.TAXABLE_VALUE,0) TAXABLE_VALUE,0 TAX_RATE , IFNULL(ITM.IGST_AMT,0) "
				+ "IGST_AMT, 0 CGST_AMT, 0 SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, (IFNULL(ITM.IGST_AMT,0) +IFNULL(ITM.CESS_AMT,0)) "
				+ "AS TOTAL_TAX ,"
				+ "0 INVOICE_VALUE,'' POS, '' STATE_NAME,PORT_CODE,BOE_NUM ,BOE_CREATED_DATE,BOE_REF_DATE, "
				...
				+ "FROM TBL_VENDOR_MASTER_CONFIG WHERE GSTIN_STATUS IN ('Active','Suspended','Cancelled') "
				+ "AND IS_ACTIVE = TRUE AND IS_FETCHED = TRUE) VD ON HDR.GSTIN = VD.GSTIN "
				+ "WHERE HDR.IS_DELETE = FALSE  " + buildHeader
				+ " GROUP BY RET_PERIOD,HDR.GSTIN,LEGAL_NAME_BS,TRADE_NAME, ITM.TAXABLE_VALUE,"
				+ "ITM.IGST_AMT,ITM.CESS_AMT, PORT_CODE,BOE_NUM,BOE_CREATED_DATE,BOE_REF_DATE,IS_AMENDMENT_BOE , "
				+ "HDR.CREATED_ON,HDR.DERIVED_RET_PERIOD "
				+ " UNION ALL SELECT COUNT(DISTINCT HDR.ID) CNT,RET_PERIOD "
				+ "RETURN_PERIOD,HDR.GSTIN CGSTIN,SGSTIN, LEGAL_NAME_BS LEGAL_NAME,VD.TRADE_NAME, 'IMPGSEZ' DOC_TYPE, "
				+ " 'R' SUPPLY_TYPE,'IMPGSEZ' TABLE_TYPE,'' DOC_NUM,'' DOC_DATE, IFNULL(ITM.TAXABLE_VALUE,0) TAXABLE_VALUE,"
				+ "0 TAX_RATE, IFNULL(ITM.IGST_AMT,0) IGST_AMT, 0 CGST_AMT, 0 SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, "
				+ "(IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CESS_AMT,0)) AS TOTAL_TAX, 0 INVOICE_VALUE,'' POS,'' STATE_NAME, "
				...
				+ "ITM.TAXABLE_VALUE,ITM.IGST_AMT,ITM.CESS_AMT, PORT_CODE,BOE_NUM,BOE_CREATED_DATE,BOE_REF_DATE,"
				...
				+ "IFNULL(ITM.TAXABLE_VALUE,0) TAXABLE_VALUE,0 TAX_RATE, IFNULL(ITM.IGST_AMT,0) IGST_AMT, "
				+ "0 CGST_AMT, 0 SGST_AMT, IFNULL(ITM.CESS_AMT,0) CESS_AMT, (IFNULL(ITM.IGST_AMT,0)+IFNULL(ITM.CESS_AMT,0)) "
