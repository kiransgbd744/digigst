...
@Component("gstr1ANilRatedSupDataATGstnImpl")
@Slf4j
public class GSTR1ANilRatedSupDataATGstnImpl
		implements GSTR1ANilRatedSupDataATGstn {

	@Autowired
	@Qualifier("DefaultAPIExecutor")
	APIExecutor apiExecuter;
	...
	public Long findNilRatedSupDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.NIL.equals(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1A_GET_NIL_RATED, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_NIL_RATED);
			}
		}

		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1ANilRatedSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
		...
		/*
		 * APIResponse response = apiExecuter.execute(params, null); if
		 * (!response.isSuccess()) { String msg =
		 * "Failed to get Nil Rated Supply from GSTN";
		 * if(LOGGER.isErrorEnabled()){ LOGGER.error(msg); } JsonObject resp =
		 * new JsonObject(); resp.add("hrd", new Gson().toJsonTree(new
		 * APIRespDto("E", msg))); } return response.getResponse();
		 */
	}
