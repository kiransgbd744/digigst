...
@Component("GoodsServicesPDFDaoImpl")
@Slf4j
public class GoodsServicesPDFDaoImpl implements GoodsServicePdf {
	...
	private static final List<String> TAX_DOC_TYPE = ImmutableList.of("B2CS",
			"B2CL", "CDNUR-B2CL", "CDNR", "NILEXTNON", "B2B");
	...
	private static final List<String> DOC_TYPE = ImmutableList.of("INV", "CR",
			"DR", "BOS");
	private static final List<String> NILNONEXT = ImmutableList.of("NIL", "NON",
			"EXT");
	...
	public JasperPrint generatePdfReport(String id, String docNo,
		...
		String source = "jasperReports/GoodsInvoice (002).jrxml";
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Inside GoodsServicesPDFDaoImpl for docNo: %s", docNo);
			LOGGER.debug(msg);
		}
		...
		try {
			...
			parameters.put("SupplierFSSAI", out.getSupplierEmail() != null
					? out.getSupplierEmail() : "");
			parameters.put("irn", out.getIrnResponse() != null
					? out.getIrnResponse() : "N/A");
			parameters.put("irnDate", out.getAckDate() != null
					? (formatter1.format(out.getAckDate())) : "N/A");
			parameters.put("InvoiceNo",
					out.getDocNo() != null ? out.getDocNo() : "");
			parameters.put("InvoiceDate", out.getDocDate() != null
					? formatter2.format(
							EYDateUtil.toISTDateTimeFromUTC(out.getDocDate()))
					: "");
			parameters
					.put("InvoicePeriodStartDate",
							out.getInvoicePeriodStartDate() != null
									? formatter2.format(
											EYDateUtil.toISTDateTimeFromUTC(
													out.getInvoicePeriodStartDate()))
									: "");
			parameters
					.put("InvoicePeriodEndDate",
							out.getInvoicePeriodEndDate() != null
									? formatter2.format(
											EYDateUtil.toISTDateTimeFromUTC(
													out.getInvoicePeriodEndDate()))
									: "");
			...
			parameters.put("TermNConditions",
					StringUtils.truncate(textdata.toString(), 305));
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Account details {}, branch ifsc code {}, bankName {} and bankAddress {}",
						bankDetailsEntity.getBankAcct(),
						bankDetailsEntity.getIfscCode(),
						bankDetailsEntity.getBankName(),
						bankDetailsEntity.getBankAdd());
			}
			...
			if (StringUtils.isEmpty(irn)) {
				if ((DOC_TYPE.contains(trimAndConvToUpperCase(out.getDocType()))
						&& TAX_DOC_TYPE.contains(trimAndConvToUpperCase(
								out.getGstnBifurcation()))))
				/*
				 * || ((StringUtils.isEmpty(out.getCgstin()) || GSTConstants.URP
				 * .equalsIgnoreCase(out.getCgstin())) && GSTConstants.BOS
				 * .equalsIgnoreCase(out.getDocType())
				 * 
				 * && NILNONEXT.contains(out.getSupplyType()))
				 */ {
					...
					if ("A".equalsIgnoreCase(a14)) {
						signedqr = out.getSgstin() + slash + out.getDocNo()
								+ slash + out.getDocDate() + slash
								+ out.getDocAmount();
					}
			...
			parameters.put("PaymentInstruction",
					out.getPaymentInstruction() != null ? StringUtils.truncate(
							String.valueOf(out.getPaymentInstruction()), 100)
							: "");
