...
@Service("CDNURAEYFinalStructure")
public class CDNURAEYFinalStructure {
	

	public List<Gstr1SummaryRespDto> getcdnuraEyList(
		...
		if (view9BFiltered != null & view9BFiltered.size() > 0) {
			...
			List<Gstr1SummaryRespDto> default9BFiltered = cdnuraEYList.stream()
					.filter(p -> p.getTableSection().equalsIgnoreCase("9B"))
					.collect(Collectors.toList());
		...
		Collections.sort(cdnuraEYList, new Comparator<Gstr1SummaryRespDto>() {
			@Override
