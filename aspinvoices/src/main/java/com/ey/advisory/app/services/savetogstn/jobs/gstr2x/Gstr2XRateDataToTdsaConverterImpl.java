 ...
 *
 */
@Service("Gstr2XRateDataToTdsaConverterImpl")
@Slf4j
public class Gstr2XRateDataToTdsaConverterImpl
	...
	public SaveBatchProcessDto convertToGstr2XObject(List<Object[]> objects,
		...
		try {
			if (objects != null && !objects.isEmpty()) {
				...
				List<Gstr2XTdsaDto> tdsaList = new ArrayList<>();
				...
				for (int counter = 0; counter < totSize; counter++) {
					...
					if (isNewInvoice(arr1, arr2, idsList, totSize, counter2)) {
						Gstr2XTdsaDto tdsaData = setInv(arr1);
						tdsaList.add(tdsaData);
					} else {
						continue;
					}
					if (isNewBatch(idsList, totSize, counter2)) {
						Gstr2XDto gstr2x = setBatch(arr1, tdsaList);
						if (LOGGER.isDebugEnabled()) {
	...
	private Gstr2XTdsaDto setInv(Object[] arr1) {
		Gstr2XTdsaDto data = new Gstr2XTdsaDto();
		...
		data.setMonth(arr1[8] != null ? String.valueOf(arr1[8]) : null);
		data.setInum(arr1[9] != null ? String.valueOf(arr1[9]) : null);
		data.setIdt(arr1[10] != null ? String.valueOf(arr1[10]) : null);
		String invDate = null;
	...
	private Gstr2XDto setBatch(Object[] arr1, List<Gstr2XTdsaDto> tdsaList) {
