...
public class Gstr1ATxpdProfitCenter implements
	...
	public List<ProcessingResult> validate(
		...
		String profitCentre = document.getProfitCentre();
		...
		entitAtConfigInnerMap.entrySet().forEach(entitAtConfMap -> {
			...
			if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
				boolean isAttrValid = onboardAttributesChecking.isAttrValid(
						entityAtValMap, OnboardingConstant.PC, profitCentre,
						document.getEntityId());
				if (profitCentre == null || profitCentre.isEmpty()) {
					errorLocations.add(GSTConstants.PROFITCENTRE);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5078",
							" Profit Centre is not as per master "
			...
			if (OnboardingConstant.AT_O.equalsIgnoreCase(atOutward)) {
				...
				if (!isAttrValid) {
					...
					errors.add(new ProcessingResult(APP_VALIDATION,
							ProcessingResultType.INFO, "IN5113",
							" Profit Centre is not as per master "
									+ "provided during On-Boarding.",
							location));
		...
		if (profitCentre != null && !profitCentre.isEmpty()) {
			...
			if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
				...
				if (!isAttrUserValid) {
					...
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5163",
							"Profit Centre is not as per user access level"
