...
@Component("Gstr1AARStructValidationChain")
public class Gstr1AARStructValidationChain {
	...
	private static final Map<Integer, ValidationRule[]> STRUCT_VAL_RULES = new ImmutableMap.Builder()
			.put(0, new ValidationRule[] { new ARSgstinValidationRule() })
			.put(1, new ValidationRule[] { new ARRetPeriodValidationRule() })
			.put(2, new ValidationRule[] { new ARTransactionType() })
			.put(3, new ValidationRule[] { new ARMonthValidationRule() })// month
			.put(4, new ValidationRule[] { new AROrgPOSValidationRule() })// orgpos
			.put(5, new ValidationRule[] { new AROrgRateValidationRule() })
			.put(6, new ValidationRule[] {
					new AROrgGrossAdvancedValidationRule() })
			.put(7, new ValidationRule[] { new ARNewPOSValidationRule() })
			.put(8, new ValidationRule[] { new ARNewRateValidationRule() })
			.put(9, new ValidationRule[] {
					new ARNewGrossAdvancedReceivedValidation() })// new gross
			.put(10, new ValidationRule[] {
					new ARIntegratedTaxAmountValidationRule() })// igst
			.put(11, new ValidationRule[] {
					new ARCentralTaxAmountValidationRule() })// cgst
			.put(12, new ValidationRule[] {
					new ARStateUTTaxAmountValidationRule() })// sgst
			.put(13, new ValidationRule[] { new ARCessAmountValidationRule() })
			.put(14, new ValidationRule[] {
					new ARProfitCentreValidationRule() })
			.put(15, new ValidationRule[] { new ARPlantValidationRule() })
			.put(16, new ValidationRule[] { new ARDivisionValidationRule() })
			.put(17, new ValidationRule[] { new ARLocationValidationRule() })
			.put(18, new ValidationRule[] {
					new ARSalesOrganizationValidationRule() })
			.put(19, new ValidationRule[] {
					new ARDistributionChannelValidationRule() })
			.put(20, new ValidationRule[] { new ARUserAccess1ValidationRule() })
			.put(21, new ValidationRule[] { new ARUserAccess2ValidationRule() })
			.put(22, new ValidationRule[] { new ARUserAccess3ValidationRule() })
			.put(23, new ValidationRule[] { new ARUserAccess4ValidationRule() })
			.put(24, new ValidationRule[] { new ARUserAccess5ValidationRule() })
			.put(25, new ValidationRule[] { new ARUserAccess6ValidationRule() })
			/*
			 * .put(26, new ValidationRule[] { new ARUserDefinedValidationRule()
			 * }) .put(27, new ValidationRule[] { new
			 * ARUserDefinedValidationRule() }) .put(28, new ValidationRule[] {
			 * new ARUserDefinedValidationRule() })
			 */
			.build();

	public Map<String, List<ProcessingResult>> validation(
			List<Object[]> rawDocMap,
			List<Gstr1AAsEnteredAREntity> excelDataSave) {
	...
	public String getInvKey(Object[] obj) {
		...
		return new StringJoiner(WEB_UPLOAD_KEY).add(sgstin).add(retPeriod)
				.add(transactionType).add(month).add(orgPOS).add(orgRate)
				.add(newPOS).add(newRate).add(profitCentre).add(plant)
				.add(division).add(location).add(salesOrganization)
