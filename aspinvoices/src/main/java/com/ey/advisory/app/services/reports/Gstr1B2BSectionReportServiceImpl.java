...
@Service("Gstr1B2BSectionReportServiceImpl")
public class Gstr1B2BSectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1B2BSectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1B2BSectionReportDaoImpl")
	...
	public Workbook findGstr1ReviewSummRecords(SearchCriteria criteria,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 B2B processed data item level response-"
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 B2B isActiveFieldSel response-"
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					workbook = gstr1CommonUtility
							.createWorkbookWithExcelTemplate("ReportTemplates",
									"GSTR1_B2B_ASP_Processed_TransactionalReport.xlsx");

					String msg = String
							.format("B2B Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 B2B custom file name -"
			...
			LOGGER.error(
					"Exception Occuered in B2B Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
