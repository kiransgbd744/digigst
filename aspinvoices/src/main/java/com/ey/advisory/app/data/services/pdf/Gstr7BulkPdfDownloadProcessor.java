 ...
 */
@Slf4j
@Component("Gstr7BulkPdfDownloadProcessor")
public class Gstr7BulkPdfDownloadProcessor implements TaskProcessor {
			...
			try {
				...
				try {
					...
					for (String gstn : gstinList) {
						...
						String fileName = "";
						if (isDigigst) {
							fileName = String.format("GSTR7_%s_%s_DigiGST", gstn,
									taxPeriod);
						} else {
							fileName = String.format("GSTR7_%s_%s_GSTN", gstn, taxPeriod);
						}
						...
						if (LOGGER.isDebugEnabled()) {
							String msg = String.format(
									"inside service impl before calling to Gstr7BulkPdfDownloadProcessor this method    id : %d",
									id);
							LOGGER.debug(msg);
						}
						
						JasperPrint jprint = gstr7ReportImpl
								.generatePdfGstr7Report(request, gstn);
						if (LOGGER.isDebugEnabled()) {
							String msg = String.format(
									"inside service impl before  after calling to Gstr7BulkPdfDownloadProcessor this method ",
									id);
							LOGGER.debug(msg);
						}
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"inside Gstr7BulkPdfDownloadProcessor  after uploading file to the temp folder file path ");
						LOGGER.debug(msg);
					}
	...
	private String zipEinvoicePdfFiles(File tempDir, Long id) throws Exception {
		...
		String fileName = "Gstr7EntityLevelSummaryPDFReport_" + id;
