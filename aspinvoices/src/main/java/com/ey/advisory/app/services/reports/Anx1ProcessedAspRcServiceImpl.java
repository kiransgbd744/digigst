...
@Service("Anx1ProcessedAspRcServiceImpl")
public class Anx1ProcessedAspRcServiceImpl implements Anx1ASPRCSavableService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ProcessedAspRcServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1ASPRCSavableTotalSummaryDaoImpl")
	private Anx1ASPRCSavableSummaryDao anx1ASPRCSavableTotalSummaryDao;

	@Autowired
	@Qualifier("Anx1ASPRCSavableSummaryLevelDaoImpl")
	private Anx1ASPRCSavableSummaryDao anx1ASPRCSavableSummaryDao;

	@Autowired
	@Qualifier("Anx1ASPRCSavableTransDaoImpl")
	private Anx1ASPRCSavableSummaryDao anx1AspRCSavableTransDao;
	...
	public Workbook findRCSavableReports(SearchCriteria criteria,
		...
		responseFromViewTotal = anx1ASPRCSavableTotalSummaryDao
				.getAnx1RCSavableReports(request);

		List<Object> responseFromViewSum = new ArrayList<>();
		responseFromViewSum = anx1ASPRCSavableSummaryDao
				.getAnx1RCSavableReports(request);

		List<Object> responseFromViewTrans = new ArrayList<>();
		responseFromViewTrans = anx1AspRCSavableTransDao
				.getAnx1RCSavableReports(request);

		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ANX-1_ASP_Reverse_Charge_3H_Savable.xlsx");
		...
		if (responseFromViewTotal != null && responseFromViewTotal.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.Savable.rc.totalsummary.report.headers")
					.split(",");
		...
		if (responseFromViewSum != null && responseFromViewSum.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.Savable.rc.sumlevel.report.headers")
					.split(",");
		...
		if (responseFromViewTrans != null && responseFromViewTrans.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.Savable.rc.trans.report.headers").split(",");
