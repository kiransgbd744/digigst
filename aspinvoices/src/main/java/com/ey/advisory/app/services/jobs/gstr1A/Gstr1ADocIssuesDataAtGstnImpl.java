...
@Component("gstr1ADocIssuesDataAtGstnImpl")
@Slf4j
public class Gstr1ADocIssuesDataAtGstnImpl implements Gstr1ADocIssuesDataAtGstn {
	...
	public Long findDocIssueDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.DOC_ISSUE.equals(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1A_GET_DOC_ISSUED, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_DOC_ISSUED);
			}
		}

		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1ADocSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
		 ...
		 * if(!resp.isSuccess()){ String msg =
		 * "failed to get Gstr1 BTCL from Gstn"; if(LOGGER.isErrorEnabled()){
