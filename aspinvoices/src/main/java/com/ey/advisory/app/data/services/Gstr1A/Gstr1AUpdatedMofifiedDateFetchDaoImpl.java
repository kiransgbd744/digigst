...
@Component("Gstr1AUpdatedMofifiedDateFetchDaoImpl")
public class Gstr1AUpdatedMofifiedDateFetchDaoImpl {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1AUpdatedMofifiedDateFetchDaoImpl.class);

	public String gstr1AloadBasicSummarySection(Annexure1SummaryReqDto request) {
	...
	private String createQueryString(String buildQuery) {
		...
		String queryStr = "SELECT MAX(CREATED_ON) LAST_UPDATED_DATE "
				+ "FROM GETGSTR1A_RATE_SUMMARY WHERE " + "IS_DELETE = FALSE AND "
				+ buildQuery;
...
*/	
	...
	private String createLastNilQueryString(String buildQuery) {
		...
		String queryStr = "SELECT NIL_LAST_UPDATED FROM ( "
				...
				+ "FROM GSTR1_GSTN_SAVE_BATCH WHERE IS_DELETE=FALSE AND RETURN_TYPE = 'GSTR1A' AND "
				+ buildQuery
	...
	public String getLastUpdatedGstr1ASummStatus(
			Annexure1SummaryReqDto request) {
		...
		try {
			String queryStr = "SELECT TOP 1 CREATED_ON,STATUS "
					...
					+ " RETURN_PERIOD =:taxPeriodReq AND API_SECTION = 'GSTR1A'"
					+ " AND GET_TYPE = 'GSTR1A_GETSUM' ORDER BY CREATED_ON DESC";
			Query q = entityManager.createNativeQuery(queryStr);
			q.setParameter("gstinList", gstinList);
			q.setParameter("taxPeriodReq", taxPeriodReq);
	...
	private String createHSNFlagQueryString(String buildQuery) {
		...
		String queryStr = "select IS_NIL_UI,IS_HSN_UI from GSTN_USER_REQUEST WHERE "
				+ "REQUEST_TYPE = 'SAVE' "
				+ "AND RETURN_TYPE = 'GSTR1A' AND IS_DELETE = FALSE AND "
	...
	public GetGstnSubmitStatusDto loadForGstr1AGetGstnSaveStatus(
			Annexure1SummaryReqDto request) {
		// TODO Auto-generated method stub
		String taxPeriodReq = request.getTaxPeriod();
	...
	// SaveGstn
		...
		private String createSaveGstnQueryString(String buildQuery) {
			...
			String queryStr = "select max(CREATED_ON) from GSTN_USER_REQUEST "
					+ "WHERE IS_DELETE = FALSE AND REQUEST_TYPE = 'SAVE' AND RETURN_TYPE = 'GSTR1A' "
					+ buildQuery;
