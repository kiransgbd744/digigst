...
import com.ey.advisory.app.data.views.client.Anx1GSTN3LErrorReportsView;
...
@Component("Anx1GSTN3LErrorReportDaoImpl")
public class Anx1GSTN3LErrorReportDaoImpl implements Anx1GstnErrorDao {
	...
	public List<Object> getAnx1GstnErrorReport(SearchCriteria criteria) {
		...
		String queryStr = GSTN3LErrorQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertGSTN3LError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1GSTN3LErrorReportsView convertGSTN3LError(Object[] arr) {
		Anx1GSTN3LErrorReportsView obj = new Anx1GSTN3LErrorReportsView();
		...
		obj.setSection7ofIGSTFlag(arr[15] != null ? arr[15].toString() : null);
		obj.setClaimRefundFlag(arr[16] != null ? arr[16].toString() : null);
	...
	private String GSTN3LErrorQueryString(String buildQuery) {
		return "SELECT CUST_GSTIN,HT.SUPPLIER_GSTIN,DOC_TYPE,"
				...
				+ "SUM(IT.SGST_AMT) SGST_AMT,"
				+ "SUM(IT.CESS_AMT_SPECIFIC) CESS_AMT,SUM(DOC_AMT) INVVAL,"
				+ "HT.POS,HT.DIFF_PERCENT,HT.SECTION7_OF_IGST_FLAG,"
				+ "HT.CLAIM_REFUND_FLAG,"
				...
				+ "AND UCASE(HT.AN_TAX_DOC_TYPE) = UCASE('MD') "
