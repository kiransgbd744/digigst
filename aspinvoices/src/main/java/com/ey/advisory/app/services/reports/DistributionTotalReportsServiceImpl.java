...
@Service("DistributionTotalReportsServiceImpl")
public class DistributionTotalReportsServiceImpl
		implements DistributionReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(DistributionTotalReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("DistributionTotalReportsDaoImpl")
	...
	public void generateFileStatusCsv(SearchCriteria criteria,
		...
		List<DistributionTotalDto> responseTotalDao = distributionReportsDao
				.generateFileStatusCsv(request);

		if (responseTotalDao != null && !responseTotalDao.isEmpty()) {
			...
			try (Writer writer = new BufferedWriter(new FileWriter(fullPath),
				...
				invoiceHeadersTemplate = commonUtility
						.getProp("dist.api.csv.report.headers");
				writer.append(invoiceHeadersTemplate);
				columnMappings = commonUtility
						.getProp("dist.api.csv.report.headers.mapping")
						.split(",");
				...
				beanWriter.write(responseTotalDao);
