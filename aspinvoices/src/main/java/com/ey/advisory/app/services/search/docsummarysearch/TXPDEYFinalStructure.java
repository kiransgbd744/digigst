...
@Service("TXPDEYFinalStructure")
public class TXPDEYFinalStructure {
	
	public List<Gstr1SummaryRespDto> gettxpdEyList(
		...
		if (view11AFiltered != null & view11AFiltered.size() > 0) {
			...
			List<Gstr1SummaryRespDto> default11AFiltered = txpdEYList.stream()
					.filter(p -> p.getTableSection().equalsIgnoreCase("11B"))
					.collect(Collectors.toList());

			// If the default filtered list is not null
			default11AFiltered.forEach(default11A -> {
				// then remove it from List
				txpdEYList.remove(default11A);
			});
		...
		Collections.sort(txpdEYList, new Comparator<Gstr1SummaryRespDto>() {
			@Override
