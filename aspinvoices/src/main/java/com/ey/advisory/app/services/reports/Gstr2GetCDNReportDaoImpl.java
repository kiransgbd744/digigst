 ...
 * 
 */
@Component("Gstr2GetCDNReportDaoImpl")
@Slf4j
public class Gstr2GetCDNReportDaoImpl implements Gstr1aGetDao {
	...
	public List<Object> getGstnConsolidatedReports(SearchCriteria criteria) {
		...
		if (LOGGER.isDebugEnabled()) {
			...
			LOGGER.debug("CDN GSTR2A query : " + queryStr);
		}
	...
	private String creategstnB2CSSummaryQueryString(String buildQuery) {
		...
		return "SELECT RETURN_PERIOD,CGSTIN,SGSTIN,LEGAL_NAME,TRADE_NAME,"
				...
				+ " NOTE_TYPE DOC_TYPE,"
				+ " INV_TYPE SUPPLY_TYPE, 'CDN' TABLE_TYPE,"
			...
			/*	+ "(CASE WHEN HDR.FILE_DATE IS NOT NULL AND "
				+ "HDR.FILE_PERIOD IS NOT NULL THEN 'Y' ELSE 'N' END) "
			*/	+ " HDR.D_FLAG AS CDN_DELINKING_FLAG ,'' CR_DR_PRE_GST,'' ITC_ELIGIBLE,"
