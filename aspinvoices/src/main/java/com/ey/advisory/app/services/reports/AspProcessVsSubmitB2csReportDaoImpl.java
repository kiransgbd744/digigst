 ...
 */
@Slf4j
@Component("AspProcessVsSubmitB2csReportDaoImpl")
public class AspProcessVsSubmitB2csReportDaoImpl
	...
	private static String createApiProcessedQueryString(String buildQueryb2cs,String buildQuery, String buildHeaderB2cs, String multiSupplyTypeAns) {
		...
		StringBuilder build = new StringBuilder();
			...
			build.append("'B2CS' TABLE_TYPE FROM GETGSTR1_B2CS_HEADER HDR ");
			build.append("INNER JOIN GETGSTR1_B2CS_ITEM ITM ON HDR.ID = ITM.HEADER_ID  ");
			...
			*/
			...
			build.append("Ifnull(SUM(CASE WHEN doc_type IN ( 'INV', 'DR' ) THEN new_taxable_value WHEN doc_type IN ( 'CR' )");
			build.append("THEN - new_taxable_value END), 0) NEW_TAXABLE_VALUE, new_ecom_gstin, SUM(new_ecom_sup_val) NEW_ECOM_SUP_VAL, ");
			build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN IGST_AMT WHEN DOC_TYPE IN ('CR') THEN - IGST_AMT ELSE 0 END),0)AS IGST_AMT,");
			build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN CGST_AMT WHEN DOC_TYPE IN ('CR') THEN - CGST_AMT ELSE 0 END),0)AS CGST_AMT,");
			build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN SGST_AMT WHEN DOC_TYPE IN ('CR') THEN - SGST_AMT ELSE 0 END),0)AS SGST_AMT,");
			build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN CESS_AMT WHEN DOC_TYPE IN ('CR') THEN - CESS_AMT ELSE 0 END),0)AS CESS_AMT,");
			build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN TOT_VAL WHEN DOC_TYPE IN ('CR') THEN - TOT_VAL ELSE 0 END),0)AS TOT_VAL,");
			...
			build.append("'' FILE_NAME, '' ASP_DATE_TIME  FROM GSTR1_PROCESSED_B2CS B2CS  ");
			...
			build.append("FROM ANX_VERTICAL_ERROR) GROUP BY COMMON_ID, VAL_TYPE,");
			...
			build.append("AND RETURN_TYPE='GSTR1' LEFT OUTER JOIN FILE_STATUS FIL ON B2CS.FILE_ID=FIL.ID  ");
			...
			build.append("'' ASP_DATE_TIME FROM ANX_OUTWARD_DOC_HEADER B2CS ");
			build.append("INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON B2CS.ID = ITM.DOC_HEADER_ID ");
			...
			build.append("LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = B2CS.BATCH_ID ");
			build.append("WHERE B2CS.RETURN_TYPE = 'GSTR1'  ");
			build.append("AND B2CS.ASP_INVOICE_STATUS = 2 AND B2CS.COMPLIANCE_APPLICABLE=TRUE ");
			build.append("AND B2CS.IS_DELETE=FALSE AND ITM.ITM_TAX_DOC_TYPE = 'B2CS' ");
			build.append(buildHeaderB2cs);
			build.append(" )  WHERE ");
		...
		build.append("( SELECT HDR.GSTIN SUPPLIER_GSTIN,HDR.RETURN_PERIOD RETURN_PERIOD,");
		build.append("HDR.DERIVED_RET_PERIOD DERIVED_RET_PERIOD,");
		build.append("(IFNULL(HDR.GSTIN,'')  ");
		build.append("||'|'|| IFNULL(HDR.RETURN_PERIOD,'')  ");
		build.append("||'|'||IFNULL(MAP(HDR.DIFF_PERCENT,0.65,'L65','N'),'') ");
		build.append("||'|'||IFNULL(TO_NUMBER(POS), 9999) ");
		build.append("||'|'||IFNULL(HDR.ETIN,'') ");
		build.append("||'|'||IFNULL(cast(ITM.TAX_RATE as DECIMAL(15,2)), 9999)  ");
		build.append(") DOC_KEY,");
		build.append("SUPPLY_TYPE,SUBSTR(HDR.GSTIN,1,2) AS STATE_CODE,'' STATE_NAME, ");
		build.append("POS,ITM.TAX_RATE,SUM(ITM.TAXABLE_VALUE) TAXABLE_VALUE,");
		build.append("SUM(ITM.IGST_AMT) GSTN_IGST_AMT,");
		build.append("SUM(ITM.CGST_AMT) GSTN_CGST_AMT,");
		build.append("SUM(ITM.SGST_AMT) GSTN_SGST_AMT,");
		build.append("SUM(ITM.CESS_AMT) GSTN_CESS_AMT,");
		build.append("HDR.DIFF_PERCENT,HDR.ECOM_TYPE,HDR.ETIN ECOM_GSTIN,");
		build.append("BT.IS_FILED,'B2CS' TABLE_TYPE FROM GETGSTR1_B2CS_HEADER HDR ");
		build.append("INNER JOIN GETGSTR1_B2CS_ITEM ITM ON HDR.ID = ITM.HEADER_ID ");
		build.append("AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD ");
		build.append("LEFT JOIN GETANX1_BATCH_TABLE BT ON HDR.BATCH_ID = BT.ID ");
		build.append("WHERE HDR.IS_DELETE=FALSE AND BT.IS_DELETE=FALSE  AND ");
		build.append(buildQuery);
	//	and HDR.GSTIN='33GSPTN0481G1ZA'
	//	and HDR.return_period ='102020'
		build.append("GROUP BY HDR.GSTIN,");
		build.append("HDR.RETURN_PERIOD, SUPPLY_TYPE, POS, ITM.TAX_RATE, ");
		build.append("HDR.DIFF_PERCENT, HDR.ECOM_TYPE,");
		build.append("HDR.ETIN,BT.IS_FILED,HDR.DERIVED_RET_PERIOD ) ,DIGI AS ");
		build.append("( SELECT  SUPPLIER_GSTIN,RETURN_PERIOD,DERIVED_RET_PERIOD, ");
		build.append("DOC_KEY,TRAN_TYPE, '' MONTH, NULL ORG_POS, 0 ORG_HSNORSAC,");
		build.append("'' ORG_UOM, 0 ORG_QNT, NULL ORG_RATE,");
		build.append("SUM(ORG_TAXABLE_VALUE)ORG_TAXABLE_VALUE,");
		build.append("NULL ORG_ECOM_GSTIN, SUM(ORG_ECOM_SUP_VAL)ORG_ECOM_SUP_VAL,"); 
		build.append("NEW_POS,null as NEW_HSNORSAC, '' NEW_UOM, NEW_QNT,NEW_RATE,");
		build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN NEW_TAXABLE_VALUE END), 0) ");
		build.append("-IFNULL(SUM(CASE WHEN DOC_TYPE IN ('CR') THEN NEW_TAXABLE_VALUE END), 0) AS NEW_TAXABLE_VALUE,");
		build.append("NEW_ECOM_GSTIN, SUM(NEW_ECOM_SUP_VAL)NEW_ECOM_SUP_VAL ,");
		build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN IGST_AMT END),0) ");
		build.append("-IFNULL(SUM(CASE WHEN DOC_TYPE IN ('CR') THEN IGST_AMT END), 0) AS IGST_AMT,");
		build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN CGST_AMT END), 0) "); 
		build.append("-IFNULL(SUM(CASE WHEN DOC_TYPE IN ('CR') THEN CGST_AMT END), 0) AS CGST_AMT,");
		build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV','DR') THEN SGST_AMT END), 0) ");
		build.append("-IFNULL(SUM(CASE WHEN DOC_TYPE IN ('CR') THEN SGST_AMT END), 0) AS SGST_AMT,");
		build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV', 'DR') THEN CESS_AMT END), 0) ");
		build.append("-IFNULL(SUM(CASE WHEN DOC_TYPE IN ('CR') THEN CESS_AMT END),0) AS CESS_AMT,");
		build.append("IFNULL(SUM(CASE WHEN DOC_TYPE IN ('INV','DR') THEN TOT_VAL END), 0) ");
		build.append("-IFNULL(SUM(CASE WHEN DOC_TYPE IN ('CR') THEN TOT_VAL END),0) AS TOT_VAL,");
		...
		build.append("'' USERDEFINED_FIELD3, '' INFO_ERROR_CODE_ASP, '' INFO_ERROR_DESCRIPTION_ASP,");
		build.append("MAX(SAVE_STATUS)SAVE_STATUS, MAX(GSTIN_REF_ID)GSTIN_REF_ID , ");
		build.append("MAX(GSTIN_REF_ID_TIME )GSTIN_REF_ID_TIME, '' GSTN_ERROR_CODE , ");
		build.append("'' GSTN_ERROR_DESCRIPTION,'' SOURCE_ID,'' FILE_NAME, '' ASP_DATE_TIME  ");
		build.append("FROM  ( SELECT ");
		build.append("'INV' AS DOC_TYPE,(IFNULL(B2CS.SUPPLIER_GSTIN,'') ||'|'|| IFNULL(B2CS.RETURN_PERIOD,");
		build.append("'') ||'|'||IFNULL(TRAN_TYPE,'') ||'|'||IFNULL(NEW_POS, 9999)||'|'||IFNULL(NEW_ECOM_GSTIN,");
		build.append("'')||'|'||IFNULL(NEW_RATE, 9999) ) DOC_KEY, B2CS.SUPPLIER_GSTIN, B2CS.RETURN_PERIOD,");
		build.append("B2CS.DERIVED_RET_PERIOD, TRAN_TYPE, '' MONTH,NULL AS ORG_POS, 0 ORG_HSNORSAC,");
		build.append("''ORG_UOM, 0 ORG_QNT, NULL AS ORG_RATE, ORG_TAXABLE_VALUE, NULL AS ORG_ECOM_GSTIN,");
		build.append("ORG_ECOM_SUP_VAL, NEW_POS, NEW_HSNORSAC, ''NEW_UOM, NEW_QNT, NEW_RATE,");
		build.append("NEW_TAXABLE_VALUE, NEW_ECOM_GSTIN, NEW_ECOM_SUP_VAL, IGST_AMT, CGST_AMT,");
		build.append("SGST_AMT, CESS_AMT,IFNULL(NEW_TAXABLE_VALUE,0)+IFNULL(IGST_AMT,0) +IFNULL(CGST_AMT,0)+IFNULL(SGST_AMT,");
		build.append("0)+IFNULL(CESS_AMT, 0) AS TOT_VAL,PROFIT_CENTRE,PLANT_CODE,DIVISION,");
		build.append("LOCATION, SALES_ORGANIZATION, DISTRIBUTION_CHANNEL, USERACCESS1, USERACCESS2,");
		build.append("USERACCESS3, USERACCESS4, USERACCESS5, USERACCESS6, ''USERDEFINED_FIELD1,");
		build.append("''USERDEFINED_FIELD2,''USERDEFINED_FIELD3, ''INFO_ERROR_CODE_ASP, ''INFO_ERROR_DESCRIPTION_ASP,");
		build.append("(CASE WHEN B2CS.IS_SAVED_TO_GSTN = TRUE AND B2CS.IS_DELETE = FALSE THEN 'IS_SAVED' ");
		build.append("ELSE 'NOT_SAVED' END) AS SAVE_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, ");
		build.append("GSTNBATCH.GSTN_RESP_DATE AS GSTIN_REF_ID_TIME, '' AS GSTN_ERROR_CODE,");
		build.append("'' AS GSTN_ERROR_DESCRIPTION,'' SOURCE_ID,'' FILE_NAME,");
		build.append("'' ASP_DATE_TIME FROM GSTR1_PROCESSED_B2CS B2CS LEFT OUTER JOIN (SELECT ");
		build.append("COMMON_ID,VAL_TYPE,FILE_ID, INV_KEY,TABLE_TYPE,");
		build.append("STRING_AGG(INFO_ERROR_CODE_ASP,',') INFO_ERROR_CODE_ASP,");
		build.append("STRING_AGG(INFO_ERROR_DESCRIPTION_ASP,',') INFO_ERROR_DESCRIPTION_ASP ");
		build.append("FROM (SELECT COMMON_ID, VAL_TYPE, FILE_ID,INV_KEY, TABLE_TYPE,");
		build.append("CASE WHEN ERROR_TYPE='INFO' THEN ERROR_CODE END AS INFO_ERROR_CODE_ASP,");
		build.append("CASE WHEN ERROR_TYPE='INFO' THEN ERROR_DESCRIPTION END AS INFO_ERROR_DESCRIPTION_ASP ");
		build.append("FROM ANX_VERTICAL_ERROR) GROUP BY COMMON_ID, VAL_TYPE, FILE_ID, INV_KEY,TABLE_TYPE) ");
		build.append("GSTRERR ON B2CS.AS_ENTERED_ID= GSTRERR.COMMON_ID ");
		build.append("AND B2CS.FILE_ID=GSTRERR.FILE_ID AND GSTRERR.TABLE_TYPE='B2CS' AND RETURN_TYPE='GSTR1' ");
		build.append("LEFT OUTER JOIN FILE_STATUS FIL ON B2CS.FILE_ID=FIL.ID AND GSTRERR.FILE_ID=FIL.ID ");
		build.append("AND GSTRERR.INV_KEY=B2CS.B2CS_INVKEY LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH  ");
		build.append("GSTNBATCH ON GSTNBATCH.ID = B2CS.BATCH_ID ");
		build.append("WHERE B2CS.IS_DELETE=FALSE AND IS_AMENDMENT=FALSE  ");
		build.append(" UNION ALL ");
		build.append("SELECT ");
		build.append("DOC_TYPE,(IFNULL(B2CS.SUPPLIER_GSTIN,'') ||'|'|| IFNULL(B2CS.RETURN_PERIOD,");
		build.append("'') ||'|'||IFNULL(B2CS.DIFF_PERCENT,'') ||'|'||IFNULL(B2CS.POS,");
		build.append("9999)||'|'||IFNULL(ITM.ECOM_GSTIN,'')||'|'||IFNULL(ITM.TAX_RATE,9999) ) DOC_KEY,");
		build.append("B2CS.SUPPLIER_GSTIN,B2CS.RETURN_PERIOD,B2CS.DERIVED_RET_PERIOD,ITM.DIFF_PERCENT AS TRAN_TYPE,");
		build.append("'' AS MONTH, NULL AS ORG_POS, 0 AS ORG_HSNORSAC, '' AS ORG_UOM,");
		build.append("0 AS ORG_QNT, NULL AS ORG_RATE,0 AS ORG_TAXABLE_VALUE, NULL AS ORG_ECOM_GSTIN,");
		build.append("0 AS ORG_ECOM_SUP_VAL, B2CS.POS AS NEW_POS, ITM_HSNSAC AS NEW_HSNORSAC,'' NEW_UOM,");
		build.append("ITM_QTY AS NEW_QNT, ITM.TAX_RATE AS NEW_RATE, ITM.TAXABLE_VALUE AS NEW_TAXABLE_VALUE,");
		build.append("ITM.ECOM_GSTIN AS NEW_ECOM_GSTIN, 0 NEW_ECOM_SUP_VAL,ITM.IGST_AMT AS IGST_AMT,");
		build.append("ITM.CGST_AMT AS CGST_AMT, ITM.SGST_AMT AS SGST_AMT,(ITM.CESS_AMT_SPECIFIC+ITM.CESS_AMT_ADVALOREM) AS CESS_AMT,");
		build.append("ITM.ONB_LINE_ITEM_AMT AS TOT_VAL, B2CS.PROFIT_CENTRE, B2CS.PLANT_CODE,B2CS.DIVISION,");
		build.append("B2CS.LOCATION, B2CS.SALES_ORGANIZATION, B2CS.DISTRIBUTION_CHANNEL,");
		build.append("B2CS.USERACCESS1, B2CS.USERACCESS2, B2CS.USERACCESS3,B2CS.USERACCESS4,");
		build.append("B2CS.USERACCESS5, B2CS.USERACCESS6, '' USERDEFINED_FIELD1, '' USERDEFINED_FIELD2,");
		build.append("'' USERDEFINED_FIELD3, '' AS INFO_ERROR_CODE_ASP,'' AS INFO_ERROR_DESCRIPTION_ASP,");
		build.append("( CASE WHEN B2CS.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' ELSE 'NOT_SAVED' ");
		build.append("END) AS SAVE_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, ");
		build.append("GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME,'' GSTN_ERROR_CODE,'' GSTN_ERROR_DESCRIPTION,");
		build.append("'' SOURCE_ID, '' FILE_NAME,'' ASP_DATE_TIME ");
		build.append("FROM ANX_OUTWARD_DOC_HEADER B2CS INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON B2CS.ID = ITM.DOC_HEADER_ID ");
		build.append("AND B2CS.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD ");
		build.append("LEFT OUTER JOIN FILE_STATUS FIL ON B2CS.FILE_ID=FIL.ID ");
		build.append("LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = B2CS.BATCH_ID ");
		build.append("WHERE B2CS.RETURN_TYPE = 'GSTR1' AND B2CS.ASP_INVOICE_STATUS = 2 ");
		build.append("AND B2CS.COMPLIANCE_APPLICABLE=TRUE AND B2CS.IS_DELETE=FALSE ");
		build.append("AND B2CS.TAX_DOC_TYPE='B2CS' ) WHERE ");
		build.append(buildQueryb2cs);
		build.append("GROUP BY SUPPLIER_GSTIN, RETURN_PERIOD, ");
		build.append("DERIVED_RET_PERIOD, TRAN_TYPE, NEW_POS, NEW_ECOM_GSTIN, NEW_RATE,");
		build.append("PROFIT_CENTRE, PLANT_CODE, DIVISION,LOCATION, SALES_ORGANIZATION,");
		build.append("DISTRIBUTION_CHANNEL,USERACCESS1,USERACCESS2,USERACCESS3,USERACCESS4,USERACCESS5,");
		build.append("USERACCESS6,NEW_QNT,DOC_KEY 	)  ");
		build.append("SELECT IFNULL(GSTN.SUPPLIER_GSTIN, DIGI.SUPPLIER_GSTIN),");
		...
		build.append("GSTN.SUPPLY_TYPE, GSTN.STATE_NAME, GSTN.POS, GSTN.TAX_RATE,");
		build.append("GSTN.TAXABLE_VALUE, GSTN.GSTN_IGST_AMT, GSTN.GSTN_CGST_AMT, GSTN.GSTN_SGST_AMT,");
		build.append("GSTN.GSTN_CESS_AMT, GSTN.DIFF_PERCENT, GSTN.ECOM_TYPE, GSTN.ECOM_GSTIN,");
		build.append("GSTN.IS_FILED, DIGI.TRAN_TYPE, DIGI.MONTH,DIGI.ORG_POS, DIGI.ORG_HSNORSAC,");
		build.append("DIGI.ORG_UOM,	DIGI.ORG_QNT, DIGI.ORG_RATE,DIGI.ORG_TAXABLE_VALUE,");
		build.append("DIGI.ORG_ECOM_GSTIN, DIGI.ORG_ECOM_SUP_VAL, DIGI.NEW_POS, DIGI.NEW_HSNORSAC,");
		build.append("DIGI.NEW_UOM, DIGI.NEW_QNT, DIGI.NEW_RATE,DIGI.NEW_TAXABLE_VALUE,");
		build.append("DIGI.NEW_ECOM_GSTIN,DIGI.NEW_ECOM_SUP_VAL, DIGI.IGST_AMT,");
		build.append("DIGI.CGST_AMT, DIGI.SGST_AMT, DIGI.CESS_AMT, DIGI.TOT_VAL, DIGI.PROFIT_CENTRE,");
		build.append("DIGI.PLANT_CODE, DIVISION,LOCATION, SALES_ORGANIZATION, DISTRIBUTION_CHANNEL,");
		build.append("USERACCESS1, USERACCESS2, USERACCESS3,USERACCESS4, USERACCESS5,USERACCESS6,");
		build.append("USERDEFINED_FIELD1, USERDEFINED_FIELD2,USERDEFINED_FIELD3, INFO_ERROR_CODE_ASP,");
		build.append("INFO_ERROR_DESCRIPTION_ASP,SAVE_STATUS, GSTIN_REF_ID , GSTIN_REF_ID_TIME,");
		build.append("GSTN_ERROR_CODE , GSTN_ERROR_DESCRIPTION, SOURCE_ID,FILE_NAME,");
		build.append("ASP_DATE_TIME,STATE_CODE ,ROW_NUMBER () OVER ( "); 
