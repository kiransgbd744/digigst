...
import com.ey.advisory.app.data.entities.simplified.client.GetAnx2DeInvoicesItemEntity;
 ...
 *
 */
@Service("Anx2DeDataParserImpl")
@Slf4j
public class Anx2DeDataParserImpl implements Anx2DeDataParser {
	...
	public Set<GetAnx2DeInvoicesHeaderEntity> parseDeData(
		...
		Set<GetAnx2DeInvoicesHeaderEntity> invoiceList = new HashSet<>();
		...
		try {
			...
			for (Anx2Data eachInv : baseEntity) {
				for (Anx2DocumentData eachInvData : eachInv.getInvoiceData()) {
					List<GetAnx2DeInvoicesItemEntity> itemList = new ArrayList<>();
					GetAnx2DeInvoicesHeaderEntity invoice = setInvoiceData(
					...
					for (Anx2ItemDetails lineItem : eachInvData.getItems()) {
						GetAnx2DeInvoicesItemEntity item = setItemData(
								lineItem);
						itemList.add(item);
	...
	private GetAnx2DeInvoicesHeaderEntity setInvoiceData(Anx2Data eachInv,
			Anx2DocumentData eachInvData, Anx2GetInvoicesReqDto dto,
			Long batchId) {
		GetAnx2DeInvoicesHeaderEntity invoice = new GetAnx2DeInvoicesHeaderEntity();
		...
		invoice.setTableSection(APIConstants.DE.toUpperCase());
		/**
		 * setting B2BInvoices
		 */
		invoice.setCfs(cfs);
		...
		invoice.setDeBatchIdAnx2(batchId);
	...
	private GetAnx2DeInvoicesItemEntity setItemData(Anx2ItemDetails lineItem) {
		GetAnx2DeInvoicesItemEntity item = new GetAnx2DeInvoicesItemEntity();
