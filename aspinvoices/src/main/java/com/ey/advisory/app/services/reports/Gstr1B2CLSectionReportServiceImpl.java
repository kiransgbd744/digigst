...
@Service("Gstr1B2CLSectionReportServiceImpl")
public class Gstr1B2CLSectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1B2CLSectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1B2CLSectionReportDaoImpl")
	...
	public Workbook findGstr1ReviewSummRecords(SearchCriteria criteria,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Gstr1 B2CL processed data item level response {} ",
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 B2CL isActiveFieldSel response"
			...
			if (responseFromView != null && responseFromView.size() > 0) {
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					String msg = String
							.format("B2BCL Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 B2CL custom file name -"
			...
			LOGGER.error(
					"Exception Occuered in B2CL Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
