...
@Component("Anx1ASPIMPSSavableTransDaoImpl")
public class Anx1ASPIMPSSavableTransDaoImpl
	...
	public List<Object> getAnx1IMPSSavableReports(SearchCriteria criteria) {
		...
		String queryStr = createImpsQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertImpsTrans(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1AspImpsTransLevDto convertImpsTrans(Object[] arr) {
		Anx1AspImpsTransLevDto obj = new Anx1AspImpsTransLevDto();
	...
	private String createImpsQueryString(String buildQuery) {
		...
		return "SELECT HDR.ID AS HID,HDR.USER_ID,"
				...
				+ "HDR.PURCHASE_ORGANIZATION,HDR.BILL_OF_ENTRY,"
				+ "HDR.BILL_OF_ENTRY_DATE,HDR.ITC_ENTITLEMENT,"
				...
				+ "HDR.CRDR_PRE_GST,HDR.CUST_GSTIN,HDR.CUST_SUPP_TYPE,"
				+ "HDR.DIFF_PERCENT,HDR.ORIG_SUPPLIER_GSTIN,"
				+ "HDR.CUST_SUPP_NAME,HDR.CUST_SUPP_CODE,"
				...
				+ "HDR.SECTION7_OF_IGST_FLAG,HDR.DOC_AMT AS INV_VALUE,"
				+ "HDR.REVERSE_CHARGE,HDR.POSTING_DATE,"
				+ "HDR.ITC_REVERSAL_IDENTIFER,HDR.CLAIM_REFUND_FLAG,"
				+ "HDR.AUTOPOPULATE_TO_REFUND,HDR.EWAY_BILL_NUM,"
				+ "HDR.EWAY_BILL_DATE,ITM.DOC_HEADER_ID,ITM.GLCODE_TAXABLEVALUE,"
				...
				+ "ITM.STATECESS_AMT,ITM.ADJ_TAXABLE_VALUE,ITM.AVAILABLE_IGST,"
				...
				+ "AS GSTIN_REF_ID_TIME,HDR.CREATED_ON AS ASP_DATE_TIME,"
				...
				+ "HDR.AN_RETURN_TYPE IN ('ANX1') AND AN_TABLE_SECTION = '3I' "
				+ buildQuery
