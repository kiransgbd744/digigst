...
@Service("DefaultDocErrorSaveService")
@Slf4j
public class DefaultDocErrorSaveService implements DocErrorSaveService {
	...
	@Autowired
	@Qualifier("OutwardDocSvErrSaveResp")
	private OutwardDocSvErrSaveResp outwardDocSvErrSaveResp;
	...
	public void saveErrorRecord(
		...
		try {
			...
			Map<String, Anx1OutWardErrHeader> errDocMap = new HashMap<>();
			...
			PerfUtil.logEventToFile(PerfamanceEventConstants.FILE_PROCESSING,
					...
					null);
			List<Anx1OutWardErrHeader> anx1OutWardErrHeaders = anx1OutWardErrHeaderRepository
	...
	private void populateErrorCodesAndDescription(
			Map<String, List<ProcessingResult>> processingResults,
			String docKey, Anx1OutWardErrHeader errDoc) {
		...
		if (aitemErrorMap.size() > 0) {
			IntStream.range(0, errDoc.getLineItems().size()).forEach(idx -> {
				AnxOutwardTransDocLineItemError item = errDoc.getLineItems()
						.get(idx);
				if (LOGGER.isDebugEnabled()) {
