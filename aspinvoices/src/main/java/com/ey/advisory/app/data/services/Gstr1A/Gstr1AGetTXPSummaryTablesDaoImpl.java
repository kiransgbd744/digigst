...
import com.ey.advisory.app.data.views.client.Gstr1GstnAdvanceAdjustedTXPDto;
...
@Component("Gstr1AGetTXPSummaryTablesDaoImpl")
@Slf4j
public class Gstr1AGetTXPSummaryTablesDaoImpl implements Gstr1AaGetDao {
	...
	public List<Object> getGstnConsolidatedReports(SearchCriteria criteria) {
		...
		String queryStr = creategstnTXPQueryString(
				buildQuery.toString());
	...
	private Gstr1GstnAdvanceAdjustedTXPDto convertgstnTXPSummary(Object[] arr) {
		Gstr1GstnAdvanceAdjustedTXPDto obj = new Gstr1GstnAdvanceAdjustedTXPDto();
		...
		obj.setGrossAdvanceAdjusted(arr[7] != null ? arr[7].toString() : null);
	...
	private String creategstnTXPQueryString(String buildQuery) {

		return "select ID,GSTIN,RETURN_PERIOD,SUPPLY_TYPE,STATE_NAME,"
				+ "POS,TAX_RATE,TAXABLE_VALUE,IGST_AMT,CGST_AMT,SGST_AMT,"
				+ "CESS_AMT,DIFF_PERCENT,IS_FILED,DERIVED_RET_PERIOD,"
				+ "TABLE_TYPE,(ROW_NUMBER () OVER (ORDER BY GSTIN)) AS  SNO "
				+ " from (SELECT HDR.ID,HDR.GSTIN,HDR.RETURN_PERIOD,"
				+ "SUPPLY_TYPE,"
				+ "MT.STATE_NAME,POS,ITM.TAX_RATE,SUM(ITM.ADVADJ_AMT) "
				+ "TAXABLE_VALUE,SUM(ITM.IGST_AMT) IGST_AMT,"
				+ "SUM(ITM.CGST_AMT) CGST_AMT,SUM(ITM.SGST_AMT) SGST_AMT,"
				+ "SUM(ITM.CESS_AMT) CESS_AMT,HDR.DIFF_PERCENT,BT.IS_FILED,"
				+ "HDR.DERIVED_RET_PERIOD,'ADV ADJ' AS TABLE_TYPE "
				+ "FROM GETGSTR1A_TXP_HEADER HDR INNER JOIN "
				+ "GETGSTR1A_TXP_ITEM ITM ON HDR.ID = ITM.HEADER_ID "
