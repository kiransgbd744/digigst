 ...
 *
 */
@Service("SRFileToGstr1AComprehensiveOutwardTransDocConvertion")
@Slf4j
public class SRFileToGstr1AComprehensiveOutwardTransDocConvertion {

	public List<Gstr1AOutwardTransDocument> convertSRFileToOutwardTransDoc(
			Map<String, List<Object[]>> documentMap,
			DocumentKeyBuilder documentKeyBuilder,
			Gstr1FileStatusEntity fileStatus, String userName) {
		List<Gstr1AOutwardTransDocument> documents = new ArrayList<>();

		documentMap.entrySet().forEach(entry -> {
			String key = entry.getKey();
			List<Object[]> value = entry.getValue();
			Gstr1AOutwardTransDocument document = new Gstr1AOutwardTransDocument();
			List<Gstr1AOutwardTransDocLineItem> lineItems = new ArrayList<>();
			for (Object[] obj : value) {
				...
				Gstr1AOutwardTransDocLineItem lineItem = new Gstr1AOutwardTransDocLineItem();
				...
				String uom = (obj[61] != null
						&& !obj[61].toString().trim().isEmpty())
								? String.valueOf(obj[61]).trim() : null;
