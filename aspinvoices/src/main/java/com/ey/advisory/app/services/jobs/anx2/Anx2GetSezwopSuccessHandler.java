 ...
 *
 */
@Service("Anx2GetSezwopSuccessHandler")
@Slf4j
public class Anx2GetSezwopSuccessHandler  implements SuccessHandler {

	@Autowired
	@Qualifier("Anx2SezwopDataParserImpl")
	private Anx2SezwopDataParser anx2DataParser;
	...
	@Autowired
	private GetAnx2SezwopInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx2GetSezwopSuccessHandler.java",
			...
			resultIds.forEach(id -> {
				...
				if (!invoices.isEmpty()) {
					...
					reqDto.setBatchId(ctxParamsObj.get("batchId").getAsLong());
					reqDto.setSection(ctxParamsObj.get("type").getAsString());
