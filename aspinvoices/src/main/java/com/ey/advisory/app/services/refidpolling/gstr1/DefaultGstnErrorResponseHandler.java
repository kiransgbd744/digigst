 ...
 */
@Slf4j
@Component("DefaultGstnErrorResponseHandler")
public class DefaultGstnErrorResponseHandler implements Gstr1GstnResponseHandler {
	...
	public void handleResponse(JsonElement root, Gstr1SaveBatchEntity batch) {
		...
		if (batch.getSection() != null && batch.getReturnType() != null) {
			...
			if (SECTIONS.contains(batch.getSection().toLowerCase())) {
				...
				hsnProcRepository.updateIserrorfalg(batch.getId());
				...
				b2csProcRepository.updateIserrorfalg(batch.getId(),now);
				docRepo.markDocsAsErrorForBatch(batch.getId(), now);
				b2csB2csaRepo.updateIserrorfalg(batch.getId());

			} else if (batch.getSection().equalsIgnoreCase(APIConstants.TXPD)
					|| batch.getSection()
							.equalsIgnoreCase(APIConstants.TXPDA)) {

				txpTxpaProcRepository.updateIserrorfalg(batch.getId());
				docRepo.markDocsAsErrorForBatch(batch.getId(), now);
