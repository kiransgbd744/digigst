 ...
 *
 */
@Service("anx2DeInvoicesAtGstnImpl")
@Slf4j
public class Anx2DeInvoicesAtGstnImpl implements Anx2InvoicesAtGstn {
	...
	public ResponseEntity<String> findInvFromGstn(String jsonReq,
		...
		try {
			...
			dtos.forEach(dto ->{
			 ...
			 type, json);
			// static data
			//String apiResp = "{'de':[{'ctin':'32SSSKL8363S1ZF','docs':[{'doctyp':'I','diffprcnt':0.65,'sec7act':'Y','rfndelg':'Y','action':'A','pos':'03','chksum':'0dbdc754fbb79bf3eba1207a9bd0e21d27d3bf0935eaf8d58a29f0db32cef279','clmrfnd':'Y','doc':{'num':'INV200','dt':'25-04-2019','val':25000},'items':[{'hsn':'223236','txval':1000,'rate':8,'igst':0,'sgst':0,'cgst':0,'cess':0}]}]}]}";
			...
			/*if (apiResp != null) {
				...
				Set<GetAnx2DeInvoicesHeaderEntity> invoices = anx2DataParser
						.parseDeData(dto, apiResp, batch.getId());
				...
				if (!invoices.isEmpty()) {
					...
					headerRepo.softlyDeleteDEHeader(dto.getGstin(),
							dto.getReturnPeriod());
					// Save new Header
					headerRepo.saveAll(invoices);
					
					GetGstr2aBatchEntity batch = batchUtil.makeBatch(dto, type);
					batch.setAnx2DeInvoices(invoices);
					invoices.forEach(invoice -> {
						invoice.setDeBatchIdAnx2(batch);
					});
	...
	/*private Long createAnx2ReconJob2a(String jsonParam) {
		String groupCode = TenantContext.getTenantId();
		LOGGER.debug("Anx2 Recon2a job Entry is insrted with param {} .", jsonParam);
