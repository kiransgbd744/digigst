 ...
 */
@Slf4j
@Service("anx1SaveSezwpAndSezwopBatchProcessImpl")
public class Anx1SaveSezwpAndSezwopBatchProcessImpl
		implements Anx1SectionWiseSaveBatchProcess {
	...
	private Pair<List<Long>, SaveAnx1> setChunkCtinData(SaveAnx1 anx1,
			List<Anx1SezwpAndSezwopData> cTinList, String section) {
		...
		for (Anx1SezwpAndSezwopData cData : cTinList) {
			for (Anx1SezwpAndSezwopDocumentData iData : cData.getDocs()) {
	...
	private Pair<List<Long>, SaveAnx1> setChunkInvData(SaveAnx1 anx1,
			List<Anx1SezwpAndSezwopDocumentData> invList, String section) {

		SaveAnx1 anx = new SaveAnx1();
		List<Long> idsList = new ArrayList<>();
		List<Anx1SezwpAndSezwopData> cTinList = new ArrayList<>();

		anx.setSgstin(anx1.getSgstin());
		anx.setTaxperiod(anx1.getTaxperiod());

		Anx1SezwpAndSezwopData cTin = null;

		if (APIConstants.SEZWP.equals(section)) {
			anx.setSezwpInvoice(cTinList);
			cTin = anx1.getSezwpInvoice().get(0);
		} else if (APIConstants.SEZWOP.equals(section)) {
			anx.setSezwopInvoice(cTinList);
			cTin = anx1.getSezwopInvoice().get(0);
		}
		cTin.setDocs(invList);
		cTinList.add(cTin);

		for (Anx1SezwpAndSezwopDocumentData data : invList) {
	...
	private List<Triplet<List<Long>, SaveAnx1, String>> batchChunk(
		...
		int ctinSize = 0;
		if (APIConstants.SEZWP.equals(section)) {
			ctinSize = anx1.getSezwpInvoice().size();
		} else if (APIConstants.SEZWOP.equals(section)) {
			ctinSize = anx1.getSezwopInvoice().size();
		}
		...
		if (ctinSize > 1) {
			List<Anx1SezwpAndSezwopData> cTinList1 = null;
			List<Anx1SezwpAndSezwopData> cTinList2 = null;
			if (APIConstants.SEZWP.equals(section)) {
				cTinList1 = anx1.getSezwpInvoice().subList(0, ctinSize / 2);
				cTinList2 = anx1.getSezwpInvoice().subList(ctinSize / 2, ctinSize);
			} else if (APIConstants.SEZWOP.equals(section)) {
				cTinList1 = anx1.getSezwopInvoice().subList(0, ctinSize / 2);
				cTinList2 = anx1.getSezwopInvoice().subList(ctinSize / 2, ctinSize);
			}
			...
			Anx1SezwpAndSezwopData cTin1 = null;
			
			if (APIConstants.SEZWP.equals(section)) {
				cTin1 = anx1.getSezwpInvoice().get(0);
			} else if (APIConstants.SEZWOP.equals(section)) {
				cTin1 = anx1.getSezwopInvoice().get(0);
			}
			int invSize = cTin1.getDocs().size();
			List<Anx1SezwpAndSezwopDocumentData> invList1 = cTin1.getDocs().subList(0,
					invSize / 2);
			List<Anx1SezwpAndSezwopDocumentData> invList2 = cTin1.getDocs()
