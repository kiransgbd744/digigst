...
@Component("Gstr1GstnHsnSummaryTablesDaoImpl")
public class Gstr1GstnHsnSummaryTablesDaoImpl
		implements Gstr1GstnSummaryTablesDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1GstnHsnSummaryTablesDaoImpl.class);
	...
	public List<Object> getGstr1GstnSummaryTablesReport(
		...
		String queryStr = creategstnHsnQueryString(
				buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertgstnhsnSummary(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1GstnHsnSummaryTableDto convertgstnhsnSummary(Object[] arr) {
		...
		obj.setSerialNumber(arr[14] != null ? arr[14].toString() : null);
		return obj;
	}

	private String creategstnHsnQueryString(String buildQuery) {

		return "SELECT HDR.ID,HDR.GSTIN,HDR.RETURN_PERIOD,HSNORSAC,"
				...
				+ "SUM(TOT_VAL) TOT_VAL,BT.IS_FILED, "
				+ "(ROW_NUMBER () OVER (ORDER BY HDR.GSTIN)) AS  SNO "
				+ "FROM GETGSTR1_HSNORSAC HDR LEFT JOIN GETANX1_BATCH_TABLE BT "
				+ "ON HDR.BATCH_ID = BT.ID WHERE HDR.IS_DELETE=FALSE " + buildQuery
				+ " GROUP BY HDR.ID,HDR.GSTIN,HDR.RETURN_PERIOD,HSNORSAC,"
				+ "ITM_DESC,ITM_UQC,ITM_QTY,BT.IS_FILED";
