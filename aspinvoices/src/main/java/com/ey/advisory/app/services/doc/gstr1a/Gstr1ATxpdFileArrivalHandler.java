...
import com.ey.advisory.app.services.gstr1afileupload.Gstr1aTxpdStructuralProcess;
...
@Service("Gstr1ATxpdFileArrivalHandler")
@Slf4j
public class Gstr1ATxpdFileArrivalHandler {
	...
	@Autowired
	@Qualifier("Gstr1aTxpdStructuralProcess")
	private Gstr1aTxpdStructuralProcess gstr1TxpdStructuralProcess;
	...
	@Autowired
	@Qualifier("TxpdHeaderCheckService")
	private Gstr1HeaderCheckService gstr1HeaderCheckService;
	
	public void processAtaFile(Message message, AppExecContext context) {
		...
		try {
			...
			LOGGER.error("Exception occured in ATA File Arrival Processor", e);
		}
		...
		List<Object[]> listOfAtas = ((FileUploadDocRowHandler<?>) rowHandler)
				.getFileUploadList();
		...
		if (checkHeaderFormat.getValue0()) {
			gstr1TxpdStructuralProcess.txpdStructureProcessData(listOfAtas,
					updateFileStatus);
		} else {
