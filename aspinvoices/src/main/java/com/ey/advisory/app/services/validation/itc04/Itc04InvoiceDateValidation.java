 ...
 *
 */
public class Itc04InvoiceDateValidation
	...
	public List<ProcessingResult> validate(Itc04HeaderEntity document,
		...
		String tbleNumber = document.getTableNumber().trim();
		LocalDate invDate = document.getInvDate();

		if (tbleNumber != null
				&& GSTConstants.TABLE_NUMBER_5C.equalsIgnoreCase(tbleNumber)) {
			if (invDate == null) {
				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(GSTConstants.INVOICE_DATE);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						null, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER5820",
						"InvoiceDate cannot be left blank.", location));
				return errors;
			}
		}
		...
		if (document.getRetPeriod() != null
			...
			if (n <= 0) {
				...
				LocalDate invDates = document.getInvDate();
				...
				if (invDates != null && invDates.compareTo(retPer) > 0) {
					Set<String> errorLocations = new HashSet<>();
					errorLocations.add(GSTConstants.INVOICE_DATE);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION,"ER5861",
							"InvoiceDate cannot be greater than last date of return period.",
							location));
					return errors;
				}
