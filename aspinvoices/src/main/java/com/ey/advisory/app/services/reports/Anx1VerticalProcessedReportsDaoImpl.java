...
import com.ey.advisory.app.data.views.client.Anx13H3IVerticalProcessedDto;
import com.ey.advisory.app.data.views.client.Anx1B2CVerticalProcessedDto;
import com.ey.advisory.app.data.views.client.Anx1Table4VerticalProcessedDto;
 ...
 *
 */
@Component("Anx1VerticalProcessedReportsDaoImpl")
public class Anx1VerticalProcessedReportsDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1VerticalProcessedReportsDaoImpl.class);
	...
	private Object convertVerticalProcessed(Object[] arr) {
		...
		Anx1B2CVerticalProcessedDto obj1 = new Anx1B2CVerticalProcessedDto();
		Anx13H3IVerticalProcessedDto obj2 = new Anx13H3IVerticalProcessedDto();
		Anx1Table4VerticalProcessedDto obj3 = new Anx1Table4VerticalProcessedDto();
	...
	private String createVerticalProcessedQueryString(String buildb2cQuery,
		...
		if ((DownloadReportsConstant.B2C).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "ECOM_VAL_SUPRET,ECOM_NETVAL_SUP,IGST_AMT,"
					+ "CGST_AMT,SGST_AMT,CESS_AMT,TCS_AMT,STATE_CESS_AMT,"
					+ "TOTAL_VALUE,PROFIT_CENTER,PLANT,DIVISION,LOCATION,"
					+ "SALES_ORG,DISTRIBUTION_CHANNEL,USER_ACCESS1,USER_ACCESS2,"
					+ "USER_ACCESS3,USER_ACCESS4,USER_ACCESS5,USER_ACCESS6,"
					...
					+ "AS A_INFO_DESCRIPTION FROM ANX_PROCESSED_B2C "
					+ "HI LEFT OUTER JOIN ANX_VERTICAL_ERROR ERR ON "
					+ "HI.AS_ENTERED_ID= ERR.COMMON_ID AND HI.FILE_ID=ERR.FILE_ID "
					+ "LEFT OUTER JOIN FILE_STATUS FIL ON HI.FILE_ID=FIL.ID AND "
					+ "ERR.FILE_ID=FIL.ID AND ERR.INV_KEY=HI.B2C_INVKEY WHERE "
					+ buildb2cQuery + ")"
					+ " GROUP BY "
		...
		} else if ((DownloadReportsConstant.TABLE3H3I).equalsIgnoreCase(fileType)) {
			...
			queryStr = "SELECT RETURN_TYPE,CUST_GSTIN,RETURN_PERIOD,TRAN_FLAG,"
					...
					+ "USER_ACCESS3,USER_ACCESS4,USER_ACCESS5,"
					+ "USER_ACCESS6,USERDEFINED1,USERDEFINED2,USERDEFINED3,"
		...
		} else if ((DownloadReportsConstant.TABLE4).equalsIgnoreCase(fileType)) {

			queryStr = "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					+ "ECOM_GSTIN,ECOM_VAL_SUPMADE,ECOM_VAL_SUPRET,ECOM_NETVAL_SUP,"
	                ...
	                + "TRIM(', ' FROM IFNULL(STRING_AGG(A_INFO_DESCRIPTION,',' ),'') ) "
	                + "AS ASP_INFO_DESCRIPTION "
					+ "FROM (SELECT HI.ID,RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					+ "ECOM_GSTIN,ECOM_VAL_SUPMADE,ECOM_VAL_SUPRET,ECOM_NETVAL_SUP,"
					+ "IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT,PROFIT_CENTER,PLANT,"
					+ "DIVISION,LOCATION,SALES_ORG,DISTRIBUTION_CHANNEL,"
					+ "USER_ACCESS1,USER_ACCESS2,USER_ACCESS3,USER_ACCESS4,"
