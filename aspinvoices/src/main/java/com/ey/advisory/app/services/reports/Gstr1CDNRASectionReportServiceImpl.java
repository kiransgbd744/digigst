...
@Service("Gstr1CDNRASectionReportServiceImpl")
public class Gstr1CDNRASectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1CDNRASectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1CDNRASectionReportDaoImpl")
	...
	public Workbook findGstr1ReviewSummRecords(SearchCriteria criteria,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 CDNRA processed data item level response"
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 CDNRA isActiveFieldSel response"
			...
			if (responseFromView != null && responseFromView.size() > 0) {
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					String msg = String
							.format("CDNR-A Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 CDNRA custom file name -"
			...
			LOGGER.error(
					"Exception Occuered in CDNRA Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
