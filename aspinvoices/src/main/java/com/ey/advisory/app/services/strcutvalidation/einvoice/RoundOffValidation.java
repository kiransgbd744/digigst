...
public class RoundOffValidation implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errorLocations.add(GSTConstants.ROUND_OFF);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER15018",
					"Invalid round off Value", location));
			// if it's not a number, then we can return the errors
			// immediately
			return errors;
		}
		boolean isValid = NumberFomatUtil.is2And2digValidDec(obj.toString().trim());
		if (!isValid) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.ROUND_OFF);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
