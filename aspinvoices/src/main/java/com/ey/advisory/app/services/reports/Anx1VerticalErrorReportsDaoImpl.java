...
import com.ey.advisory.app.data.views.client.Anx13H3IVerticalErrorDto;
import com.ey.advisory.app.data.views.client.Anx1B2CVerticalErrorDto;
import com.ey.advisory.app.data.views.client.Anx1Table4VerticalErrorDto;
 ...
 *
 */
@Component("Anx1VerticalErrorReportsDaoImpl")
public class Anx1VerticalErrorReportsDaoImpl implements Anx1VerticalReportsDao {

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1VerticalErrorReportsDaoImpl.class);
	...
	public List<Object> getVerticalReports(
		...
		String queryStr = createVerticalErrorQueryString(
				buildb2cQuery.toString(), build3h3iQuery.toString(),
	...
	private Object convertVerticalError(Object[] arr) {
		...
		Anx1B2CVerticalErrorDto obj1 = new Anx1B2CVerticalErrorDto();
		Anx13H3IVerticalErrorDto obj2 = new Anx13H3IVerticalErrorDto();
		Anx1Table4VerticalErrorDto obj3 = new Anx1Table4VerticalErrorDto();
	...
	private String createVerticalErrorQueryString(String buildb2cQuery,
		...
		if ((DownloadReportsConstant.B2C).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "AUTOPOPULATE_TO_REFUND,POS,HSNORSAC,UOM,"
					+ "QUANTITY,TAX_RATE,STATE_APPLYING_CESS,"
					+ "STATE_CESS_RATE,TAXABLE_VALUE,TCS_FLAG,ECOM_GSTIN,"
					+ "ECOM_VAL_SUPMADE,ECOM_VAL_SUPRET,ECOM_NETVAL_SUP,IGST_AMT,"
					+ "CGST_AMT,SGST_AMT,CESS_AMT,TCS_AMT,STATE_CESS_AMT,"
					+ "TOTAL_VALUE,PROFIT_CENTER,PLANT,DIVISION,LOCATION,"
					...
					+ "AS A_ERROR_CODE,"
					+ "(CASE WHEN ERROR_TYPE='ERROR' THEN ERROR_DESCRIPTION END) "
					+ "AS A_ERROR_DESCRIPTION,"
					...
					+ "ANX_AS_ENTERED_B2C HI LEFT OUTER JOIN ANX_VERTICAL_ERROR "
					+ "ERR ON  HI.ID= ERR.COMMON_ID AND ERR.TABLE_TYPE= 'B2C' "
					+ "LEFT OUTER JOIN FILE_STATUS "
					+ "FIL ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID AND "
					+ "ERR.INV_KEY=HI.B2C_INVKEY WHERE IS_ERROR= TRUE "
					+ "AND "
					+ buildb2cQuery + ")"
					+ " GROUP BY "
			...
			queryStr = "SELECT RETURN_TYPE,CUST_GSTIN,RETURN_PERIOD,TRAN_FLAG,"
					...
					+ "TOTAL_VALUE,ELGBL_INDICATOR,AVAIL_IGST,AVAIL_CGST,AVAIL_SGST,"
					...
					+ "AUTOPOPULATE_TO_REFUND,POS,HSNORSAC,"
					+ "TAXABLE_VALUE,TAX_RATE,IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT,"
					...
					+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_CODE END) "
					+ "AS A_INFO_CODE,(CASE WHEN ERROR_TYPE='INFO' THEN "
					+ "ERROR_DESCRIPTION END)AS A_INFO_DESCRIPTION "
					+ "FROM ANX_AS_ENTERED_3H_3I HI LEFT OUTER JOIN "
					+ "ANX_VERTICAL_ERROR ERR ON  HI.ID= ERR.COMMON_ID "
					+ "AND ERR.TABLE_TYPE= 'TABLE3H3I' "
					+ "LEFT OUTER JOIN FILE_STATUS FIL ON HI.FILE_ID=FIL.ID "
					+ "AND ERR.FILE_ID=FIL.ID AND  ERR.INV_KEY=HI.INVKEY_3H_3I "
			...
			queryStr = "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "SGST_AMT,CESS_AMT,PROFIT_CENTER,PLANT,DIVISION,"
					...
					+ "USER_DEFINED3,(CASE WHEN ERROR_TYPE='ERROR' "
					+ "THEN ERROR_CODE END) AS A_ERROR_CODE,"
					+ "(CASE WHEN ERROR_TYPE='ERROR' THEN ERROR_DESCRIPTION END) "
					+ "AS A_ERROR_DESCRIPTION,(CASE WHEN ERROR_TYPE='INFO' "
					+ "THEN ERROR_CODE END) AS A_INFO_CODE,"
					+ "(CASE WHEN ERROR_TYPE='INFO' "
