...
@Slf4j
@Component("Gstr1CDNRSectionReportHandler")
public class Gstr1CDNRSectionReportHandler {

	@Autowired
	@Qualifier("Gstr1CDNRSectionReportServiceImpl")
		...
		try {
			...
			if (workbook != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 CDNR template file name is : {}",
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						...
						+ "_CDNR_SECTION9B" + ".xlsx";

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 CDNR file name is : {}", fileName);
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Workbook has been generated successfully with the"
									+ " Gstr1 CDNR response list in the directory : {}",
				...
				if (id != null) {
					try {
						...
						String msg = String.format(
								"Gstr1CDNRSectionReportHandler - "
										+ "Exception occured while Uploading into DocRepo, %s",
								e);
						LOGGER.error(msg);
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 CDNR No Data found for report id : {} ",
							id);
				}
