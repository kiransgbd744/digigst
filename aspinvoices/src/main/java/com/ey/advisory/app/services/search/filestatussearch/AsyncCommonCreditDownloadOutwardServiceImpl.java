 ...
 *
 */
@Component("AsyncCommonCreditDownloadOutwardServiceImpl")
@Slf4j
public class AsyncCommonCreditDownloadOutwardServiceImpl
	...
	public void generateReports(Long id) {
		...
		try {
			...
			String fullPath = tempDir.getAbsolutePath() + File.separator
					+ "Transactional_Outward_ITC_Reversal_Rule_42_Report_"
			...
			StatefulBeanToCsv<ItcOutwardDto> beanWriter = getBeanWriterInwardRecords(
					reportConfig, writer);
...
*/
	...
	private void WritetoCsvInwardrecords(Integer chunkNo, Long id,
			...
			StatefulBeanToCsv<ItcOutwardDto> beanWriter, String reportCateg) {

		List<Object[]> list = null;
		List<ItcOutwardDto> responseFromDao = null;
			...
			for (int i = 1; i <= chunkNo; i++) {
				...
				StoredProcedureQuery reportDataProc = entityManager
						.createStoredProcedureQuery(
								"USP_DISP_OUTWARD_CHUNK_CREDIT_REVERSAL_RPT");
				...
				if (list.size() > 0) {
					...
					responseFromDao = list.stream()
							.map(o -> (ItcOutwardDto) reportConvertor.convert(o,
									reportType))
	...
	private Integer getChunkCount(Long id) {
		StoredProcedureQuery storedProc = entityManager
				.createStoredProcedureQuery(
						"USP_INS_OUTWARD_CHUNK_CREDIT_REVERSAL_RPT");
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
					"Executing chunking proc"
							+ " USP_INS_OUTWARD_CHUNK_CREDIT_REVERSAL_RPT: '%s'",
					id.toString());
			LOGGER.debug(msg);
		}
	...
	private StatefulBeanToCsv<ItcOutwardDto> getBeanWriterInwardRecords(
		...
		ColumnPositionMappingStrategy<ItcOutwardDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
		mappingStrategy.setType(ItcOutwardDto.class);
		mappingStrategy.setColumnMapping(
				env.getProperty(config.getColumnMappingsKey()).split(","));

		StatefulBeanToCsvBuilder<ItcOutwardDto> builder = new StatefulBeanToCsvBuilder<>(
				writer);
		StatefulBeanToCsv<ItcOutwardDto> beanWriter = builder
