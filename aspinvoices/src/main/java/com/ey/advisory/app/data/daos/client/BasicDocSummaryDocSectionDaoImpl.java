 ...
 *
 */
@Component("BasicDocSummaryDocSectionDaoImpl")
public class BasicDocSummaryDocSectionDaoImpl 
				implements BasicDocSummaryDocSectionDao {
	...
	@Override
	public List<Gstr1DocIssuedSummarySectionDto> loadBasicSummarySection(
		...
		List<Gstr1DocIssuedSummarySectionDto> retList = list.parallelStream()
				.map(o -> convert(o))
	...
	private Gstr1DocIssuedSummarySectionDto convert(Object[] arr) {
	//	BaseGstr1SummaryEntity obj = new BaseGstr1SummaryEntity();
		
		Gstr1DocIssuedSummarySectionDto obj = new Gstr1DocIssuedSummarySectionDto();		
		obj.setRecords((Integer) arr[0]);
		obj.setTableSection((String) arr[1]);
		obj.setTotalIssued((Integer) arr[2]);
		obj.setNetIssued((Integer) arr[3]);
		obj.setCancelled((Integer) arr[4]);
		return obj;
	}
...
*/
		String queryStr = "SELECT RECORD_COUNT,TABLE_SECTION, "
				+ "TOT_DOC_ISSUED,NET_DOC_ISSUED, "
				+ "TOT_DOC_CANCELLED  FROM \"" + viewName 
				+ "\" WHERE " + condition +" ORDER BY TABLE_SECTION";
