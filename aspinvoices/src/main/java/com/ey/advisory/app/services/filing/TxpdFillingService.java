...
@Component("TxpdFillingService")
/**
 * 
 * @author Siva.Nandam
 *
 */
public class TxpdFillingService implements FilingService {
	...
	public HsnFillingDto getFilingData(String gstin, String returnPeriod) {
		...
		List<Object[]> doc = txpTxpaProcRepository.getTxpd(returnPeriod, gstin);
		Object[] arr = doc.get(0);
		HsnFillingDto txpd = new HsnFillingDto();
		...
		txpd.setTtl_rec((Long) arr[0]);
		txpd.setTtl_tax((BigDecimal) arr[1]);
		txpd.setTtl_igst((BigDecimal) arr[2]);
		txpd.setTtl_cgst((BigDecimal) arr[3]);
		txpd.setTtl_sgst((BigDecimal) arr[4]);
		txpd.setTtl_cess((BigDecimal) arr[5]);
