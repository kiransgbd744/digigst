	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		try {
			if (!isPresent(obj)) {
			 ...
			 LOGGER.debug("inside doc space -Validating Document Number is: '{}'", docNo);  
			    // Check if the document number contains any space
			    if (docNo.contains(" ")) {
			    	 LOGGER.debug("inside doc space -Validating Document Number: '{}'", docNo);
			        Set<String> errorLocations = new HashSet<>();
			        errorLocations.add(GSTConstants.DOC_NO);
			...
			if (!matcher.matches()) {
				 LOGGER.debug("inside Regex -Validating Document Number: '{}'", docNo);
				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(GSTConstants.DOC_NO);
				TransDocProcessingResultLoc location =
						new TransDocProcessingResultLoc(
