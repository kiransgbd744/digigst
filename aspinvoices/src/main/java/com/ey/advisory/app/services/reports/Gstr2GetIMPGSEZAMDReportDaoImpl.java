 ...
 * 
 */
	@Component("Gstr2GetIMPGSEZAMDReportDaoImpl")
	@Slf4j
	public class Gstr2GetIMPGSEZAMDReportDaoImpl implements Gstr1aGetDao {
		...
		public List<Object> getGstnConsolidatedReports(SearchCriteria criteria) {
			...
			if (StringUtils.isNotBlank(gstin)) {
				buildQuery.append(" WHERE CGSTIN IN :gstin");
			}
		...
		private String creategstnB2CSSummaryQueryString(String buildQuery) {
			...
			return "SELECT RETURN_PERIOD,CGSTIN,SGSTIN,LEGAL_NAME,TRADE_NAME,"
					...
					+ " '' DOC_TYPE,'' SUPPLY_TYPE,'IMPG_SEZ_AMD' TABLE_TYPE,"
					+ " '' DOC_NUM,'' DOC_DATE, SUM(IFNULL(ITM.TAXABLE_VALUE,0)) "
					+ "TAXABLE_VALUE,0 TAX_RATE, SUM(IFNULL(ITM.IGST_AMT,0)) "
					+ "IGST_AMT, 0 CGST_AMT, 0 SGST_AMT, SUM(IFNULL(ITM.CESS_AMT,0)) "
					...
					+ " '' ITC_ELIGIBLE,0 DIFF_PERCENT,0 ITEM_NUMBER,'' ECOM_GSTIN,"
					+ " '' MERCHANT_ID ,"
					+ "TO_CHAR(HDR.CREATED_ON,'DD-MM-YYYY') INITIATED_DATE,"
					+ "SUBSTR(HDR.CREATED_ON,12) INITIATED_TIME,HDR.DERIVED_RET_PERIOD "
					+ "FROM GETGSTR2A_AMDHIST_HEADER HDR INNER JOIN GETGSTR2A_AMDHIST_ITEM "
					...
					+ "FROM TBL_VENDOR_MASTER_CONFIG WHERE GSTIN_STATUS ='Active' "
					+ "AND IS_ACTIVE = TRUE AND IS_FETCHED = TRUE) VD ON HDR.SGSTIN = VD.GSTIN "
					+ " WHERE HDR.IS_DELETE = FALSE  "
					+ " GROUP BY RET_PERIOD,HDR.GSTIN,"
					+ "SGSTIN,LEGAL_NAME_BS,VD.TRADE_NAME,ITM.TAXABLE_VALUE,"
					+ "ITM.IGST_AMT,ITM.CESS_AMT,"
					+ "PORT_CODE,BOE_NUM,BOE_CREATED_DATE,"
					+ "BOE_REF_DATE ,HDR.CREATED_ON,HDR.DERIVED_RET_PERIOD) "
					+ buildQuery;
