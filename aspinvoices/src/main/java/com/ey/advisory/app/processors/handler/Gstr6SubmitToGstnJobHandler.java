 ...
 */
@Slf4j
@Service("Gstr6SubmitToGstnJobHandler")
public class Gstr6SubmitToGstnJobHandler {
	...
	public APIResponse submitGstinAndPeriod(String jsonString, String groupCode, String userName) {
		...
		try {
			Gstr1SubmitGstnDto dto = gson.fromJson(requestObject, Gstr1SubmitGstnDto.class);
			String gstin = dto.getGstin();
			String ret_period = dto.getRet_period();
			TenantContext.setTenantId(groupCode);
			Optional<GstnSubmitEntity> saveEntity = gstr6SubmitRepo.findById(dto.getBatchId());

			if (saveEntity.isPresent()) {
				entity = saveEntity(saveEntity.get(), groupCode, userName);
			} else {
				LOGGER.error("No Valid Request found in the the GSTR6_SUBMIT job.");
				return null;
			}
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GSTR6_SUBMIT Gstn Call initiated");
			}
			resp = hitGstnServer.gstr6SubmitApiCall(groupCode, data, gstin, ret_period);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GSTR6_SUBMIT Gstn Call Completed");
			}
			...
			gstr6SubmitRepo.save(entity);
			throw new AppException(msg, e);
		}
		return resp;
	}

	private String updateRefIdAndTxnId(String gstin, String ret_period, String groupCode, Long id, APIResponse resp,
			String userName) {
		String refId = null;
		...
		if (groupCode != null && resp != null) {
			if (resp.isSuccess()) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("GSTR6_SUBMIT ref_id: {} and txn_id: {} ", refId, txnId);
				}

				if (refId != null && !refId.isEmpty()) {
					TenantContext.setTenantId(groupCode);
					Optional<GstnSubmitEntity> OpEntity = gstr6SubmitRepo.findById(id);
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Error thrown by Gstn ErrorCode: {} and " + "ErrorDesc: {}", errorCode, errorDesc);
				}
				...
				if (OpEntity.isPresent()) {
					...
					if ("RET12521".equalsIgnoreCase(errorCode) || "RTN_17".equalsIgnoreCase(errorCode)) {
						entity.setStatus(APIConstants.SUBMIT_COMPLETED);
						entity.setGstnStatus("P");
						entity.setRefId(APIConstants.ALREADY_SUBMITTED);
						inwardTransDocRepository.markSubmittedAsTrue(ret_period, gstin, modifiedDate, now);
						distributionRepository.markSubmittedAsTrue(ret_period, gstin, modifiedDate, now);
					} else {
						entity.setStatus(APIConstants.SUBMIT_FAILED);
					}
					gstr6SubmitRepo.save(entity);
	...
	private GstnSubmitEntity saveEntity(GstnSubmitEntity entity, String groupCode, String userName) {
		LocalDateTime now = EYDateUtil.toUTCDateTimeFromLocal(LocalDateTime.now());
		TenantContext.setTenantId(groupCode);
		entity.setModifiedBy(userName);
		entity.setModifiedOn(now);
		entity.setStatus(APIConstants.SUBMIT_INPROGRESS);
		GstnSubmitEntity save = gstr6SubmitRepo.save(entity);
		return save;
