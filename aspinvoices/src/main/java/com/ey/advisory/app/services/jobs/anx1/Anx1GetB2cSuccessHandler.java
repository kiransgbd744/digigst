...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1B2cInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetB2cSuccessHandler")
@Slf4j
public class Anx1GetB2cSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("anx1B2cDataParserImpl")
	private Anx1B2cDataParser anx1DataParser;

	@Autowired
	@Qualifier("GetAnx1B2cInvoicesRepository")
	private GetAnx1B2cInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetB2cSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1B2cInvoicesHeaderEntity> b2cEntities = anx1DataParser
						.parseB2cData(dto, apiResp);
				if (!b2cEntities.isEmpty()) {
					headerRepo.softlyDeleteB2cHeader(dto.getGstin(),
							dto.getReturnPeriod());
