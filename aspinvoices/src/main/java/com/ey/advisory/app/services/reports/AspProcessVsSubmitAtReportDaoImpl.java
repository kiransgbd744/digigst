...
@Component("AspProcessVsSubmitAtReportDaoImpl")
public class AspProcessVsSubmitAtReportDaoImpl
	...
	private String createApiProcessedQueryString(String buildHeader,String buildHeaderForGet, String buildHeader3) {
		...
		return "SELECT SUPPLIER_GSTIN, RETURN_PERIOD, DERIVED_RET_PERIOD,"
				+ "SUPPLY_TYPE, STATE_NAME, POS,  TAX_RATE,"
				+ "SUM(TAXABLE_VALUE) GROSS_ADV_RECEIVED,"
				+ "SUM(GSTN_IGST_AMT)GSTN_IGST_AMT,"
				+ "SUM(GSTN_CGST_AMT)GSTN_CGST_AMT,"
				+ "SUM(GSTN_SGST_AMT)GSTN_SGST_AMT,"
				+ "SUM(GSTN_CESS_AMT)GSTN_CESS_AMT,DIFF_PERCENT,IS_FILED,"
				+ "TRAN_TYPE,MONTH,ORG_POS,ORG_RATE,"
				...
				+ "SUPPLY_TYPE,STATE_CODE,STATE_NAME,POS,TAX_RATE,TAXABLE_VALUE,"
				+ "GSTN_IGST_AMT,GSTN_CGST_AMT,GSTN_SGST_AMT,GSTN_CESS_AMT,"
				+ "DIFF_PERCENT,IS_FILED,NULL TRAN_TYPE,NULL MONTH,"
				+ "NULL ORG_POS, NULL ORG_RATE,0 ORG_TAXABLE_VALUE,"
				+ "NULL NEW_POS,NULL NEW_RATE,0 NEW_TAXABLE_VALUE, "
				...
				+ "NULL GSTIN_REF_ID , NULL GSTIN_REF_ID_TIME, "
				+ "NULL GSTN_ERROR_CODE , NULL GSTN_ERROR_DESCRIPTION, "
				+ "NULL SOURCE_ID,NULL FILE_NAME, NULL ASP_DATE_TIME "
				+ "from "
				+ "(select SUPPLIER_GSTIN, RETURN_PERIOD, DERIVED_RET_PERIOD,"
				+ "SUPPLY_TYPE,STATE_NAME,POS,TAX_RATE,TAXABLE_VALUE,"
				...
				+ "HDR.DERIVED_RET_PERIOD  DERIVED_RET_PERIOD,SUPPLY_TYPE,"
				+ "SUBSTR(HDR.GSTIN,1,2) AS STATE_CODE,'' STATE_NAME,POS,ITM.TAX_RATE,"
				...
				+ "'ADV REC' AS TABLE_TYPE FROM  GETGSTR1_AT_HEADER HDR INNER"
				+ " JOIN  GETGSTR1_AT_ITEM ITM ON HDR.ID = ITM.HEADER_ID AND "
				...
				+ "SUPPLY_TYPE, POS,ITM.TAX_RATE,HDR.DIFF_PERCENT,BT.IS_FILED)) "
				+ "UNION ALL "
				+ "SELECT SUPPLIER_GSTIN, RETURN_PERIOD, "
				+ "DERIVED_RET_PERIOD, NULL SUPPLY_TYPE,NULL STATE_CODE,"
				+ "NULL STATE_NAME,NULL POS, NULL TAX_RATE,"
				+ "0 TAXABLE_VALUE,0 GSTN_IGST_AMT,0 GSTN_CGST_AMT,"
				+ "0 GSTN_SGST_AMT,0 GSTN_CESS_AMT,"
				+ "NULL DIFF_PERCENT,NULL IS_FILED,TRAN_TYPE,MONTH,"
				+ "ORG_POS,ORG_RATE,ORG_TAXABLE_VALUE,NEW_POS,NEW_RATE,"
				+ "NEW_TAXABLE_VALUE,"
				+ "IGST_AMT AS ASP_IGST_AMT,CGST_AMT ASP_CGST_AMT,"
				+ "SGST_AMT ASP_SGST_AMT,CESS_AMT ASP_CESS_AMT, "
				+ "PROFIT_CENTRE,PLANT_CODE,DIVISION, LOCATION, SALES_ORGANIZATION, "
				+ "DISTRIBUTION_CHANNEL,USERACCESS1,USERACCESS2,USERACCESS3,"
				+ "USERACCESS4,USERACCESS5,USERACCESS6,USERDEFINED_FIELD1,"
				+ "USERDEFINED_FIELD2,USERDEFINED_FIELD3,INFO_ERROR_CODE_ASP, "
				...
				+ "SELECT SUPPLIER_GSTIN, RETURN_PERIOD, DERIVED_RET_PERIOD,"
				+ "(CASE WHEN TRAN_TYPE IN ('ZL65','L65') "
				+ "THEN 'L65' WHEN (TRAN_TYPE IN ('Z','N','') OR "
				+ "TRAN_TYPE IS NULL) THEN 'N' END) TRAN_TYPE,MAX(MONTH)MONTH,"
				+ " MAX(ORG_POS)ORG_POS,MAX(ORG_RATE)ORG_RATE, "
				+ "MAX(ORG_GROSS_ADV_RECEIVED)ORG_TAXABLE_VALUE,"
				+ " NEW_POS,MAX(NEW_RATE)NEW_RATE,"
				+ " SUM(NEW_GROSS_ADV_RECEIVED)NEW_TAXABLE_VALUE, "
				+ " SUM(IGST_AMT)IGST_AMT,SUM(CGST_AMT)CGST_AMT,"
				+ " SUM(SGST_AMT)SGST_AMT, SUM(CESS_AMT)CESS_AMT, "
				+ "MAX(PROFIT_CENTRE)PROFIT_CENTRE, MAX(PLANT_CODE)PLANT_CODE, "
				+ "MAX(DIVISION)DIVISION,MAX(LOCATION)LOCATION, "
				+ "MAX(SALES_ORGANIZATION)SALES_ORGANIZATION, "
				+ " MAX(DISTRIBUTION_CHANNEL)DISTRIBUTION_CHANNEL, "
				+ "MAX(USERACCESS1)USERACCESS1,"
				+ "MAX(USERACCESS2)USERACCESS2,MAX(USERACCESS3)USERACCESS3,"
				+ " MAX(USERACCESS4)USERACCESS4, MAX(USERACCESS5)USERACCESS5,"
				+ "  MAX(USERACCESS6)USERACCESS6, "
				...
				+ "REC.DERIVED_RET_PERIOD, TRAN_TYPE,MONTH,"
				+ "NULL ORG_POS,NULL ORG_RATE,ORG_GROSS_ADV_RECEIVED, "
				+ "NEW_POS,NULL NEW_RATE,"
				+ " NEW_GROSS_ADV_RECEIVED,IGST_AMT,CGST_AMT, SGST_AMT,"
				+ "CESS_AMT, '' PROFIT_CENTRE,''PLANT_CODE,'' DIVISION,''LOCATION,"
				...
				+ "REC.IS_SAVED_TO_GSTN = TRUE AND REC.IS_DELETE=FALSE "
				+ "THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) AS SAVE_STATUS,"
				...
				+ "  '' ASP_DATE_TIME FROM  GSTR1_PROCESSED_ADV_RECEIVED REC  "
				...
				+ "INFO_ERROR_DESCRIPTION_ASP,SAVE_STATUS,GSTIN_REF_ID ,"
				+ " GSTIN_REF_ID_TIME, GSTN_ERROR_CODE , GSTN_ERROR_DESCRIPTION,"
				+ "SOURCE_ID,FILE_NAME, ASP_DATE_TIME,SUPPLY_TYPE,  STATE_CODE, "
				+ "STATE_NAME, POS,  TAX_RATE,DIFF_PERCENT, IS_FILED";
*/
	}
