 ...
 *
 */
public class Gstr7GrossAmountValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row, TabularDataLayout layout) {
		...
		if (!isPresent(obj)) {
			...
			errorLocations.add(GSTConstants.GROSS_AMOUNT);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2013", "Invalid Gross Amount.", location));
		...
		if (result.compareTo(BigDecimal.ZERO) < 0) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2014", "Only Positive amount allowed.", location));
			return errors;
		}
		return errors;
	}
