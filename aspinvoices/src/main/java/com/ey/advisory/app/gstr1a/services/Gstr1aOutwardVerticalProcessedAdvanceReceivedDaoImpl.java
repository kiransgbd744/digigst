...
import com.ey.advisory.app.services.reports.Gstr1OutwardVerticalProcessedAdvanceReceivedDao;
...
@Component("Gstr1aOutwardVerticalProcessedAdvanceReceivedDaoImpl")
public class Gstr1aOutwardVerticalProcessedAdvanceReceivedDaoImpl
		implements Gstr1OutwardVerticalProcessedAdvanceReceivedDao {

	private static final Logger LOGGER = LoggerFactory.getLogger(
			Gstr1aOutwardVerticalProcessedAdvanceReceivedDaoImpl.class);
	...
	public List<Object> getGstr1RSReports(SearchCriteria criteria) {
		...
		return list.parallelStream().map(o -> convertProcessedAdvRec(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1AspVerticalAdvRecDto convertProcessedAdvRec(Object[] arr) {
	...
	private String createB2CSSavableSummaryQueryString(String buildQuery) {
		...
		return "SELECT REC.SUPPLIER_GSTIN,REC.RETURN_PERIOD,TRAN_TYPE,"
				...
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON REC.FILE_ID=FIL.ID "
				+ "AND GSTRERR.FILE_ID=FIL.ID AND "
				+ "GSTRERR.INV_KEY=REC.AT_INVKEY LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = REC.BATCH_ID "
				+ "WHERE REC.IS_DELETE=FALSE"
				+ buildQuery;
	}
}
