...
@Component("Gstr1APEB2bResponserHandler")
public class Gstr1APEB2bResponserHandler implements Gstr1AResponseHandler {
	...
	public Triplet<List<String>, Map<String, OutwardTransDocErrorGstr1A>, Integer> response(
		...
		if (APIConstants.B2B.equalsIgnoreCase(batch.getSection())
				&& gsterrors != null) {
			for (B2BInvoices invoice : gsterrors.getB2bInvoice()) {
				for (B2BInvoiceData docData : invoice.getB2bInvoiceData()) {

					String docNo = docData.getInvoiceNumber();

					String idt = docData.getInvoiceDate();
					String[] arr = idt.split(DASH);

					LocalDate docDate = deserialize(
							arr[2] + DASH + arr[1] + DASH + arr[0]);
				...
				n = invoice.getB2bInvoiceData().size();
			}
		} else if (APIConstants.B2BA.equalsIgnoreCase(batch.getSection())
				&& gsterrors != null) {
			for (B2BInvoices invoice : gsterrors.getB2baInvoice()) {
				for (B2BInvoiceData docData : invoice.getB2bInvoiceData()) {
