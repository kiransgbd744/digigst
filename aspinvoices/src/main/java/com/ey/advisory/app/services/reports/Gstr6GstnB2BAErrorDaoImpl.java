...
import com.ey.advisory.app.data.views.client.Gstr6B2BAGstnErrorDto;
...
@Component("Gstr6GstnB2BAErrorDaoImpl")
public class Gstr6GstnB2BAErrorDaoImpl implements Gstr6GstnErrorDao {
	...
	public List<Object> getGstr6GstnErrorReport(SearchCriteria criteria) {
		...
		return list.parallelStream().map(o -> convertgstnB2BAError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr6B2BAGstnErrorDto convertgstnB2BAError(Object[] arr) {
		Gstr6B2BAGstnErrorDto obj = new Gstr6B2BAGstnErrorDto();
		...
		obj.setLineNo(arr[5] != null ? arr[5].toString() : null);
		...
		obj.setRefIdDateTime(arr[15] != null ? arr[15].toString() : null);
		obj.setErrorCode(arr[16] != null ? arr[16].toString() : null);
		obj.setErrorMessage(arr[17] != null ? arr[17].toString() : null);
	...
	private String creategstnB2BErrorQueryString(String buildQuery) {
		...
		return "select HDR.SUPPLIER_GSTIN,HDR.DOC_NUM,HDR.DOC_DATE,"
				+ "HDR.ORIGINAL_DOC_NUM,HDR.ORIGINAL_DOC_DATE, "
				+ "NULL AS ITM_NO, ITM.TAX_RATE,HDR.POS,ifnull(SUM(ITM.TAXABLE_VALUE),0),"
				...
				+ "as CESS_AMT, ifnull(SUM(HDR.DOC_AMT),0),BT.GSTN_SAVE_REF_ID,"
				+ "BT.CREATED_ON, HDR.GSTN_ERROR_CODE AS GSTIN_ERROR_CODE, "
				+ "HDR.GSTN_ERROR_DESC AS GSTN_ERROR_DESC FROM "
				+ "ANX_INWARD_DOC_HEADER HDR INNER JOIN ANX_INWARD_DOC_ITEM ITM "
				+ "ON HDR.ID = ITM.DOC_HEADER_ID AND "
				+ "HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				+ "AND RETURN_TYPE = 'GSTR6' AND HDR.TAX_DOC_TYPE = 'B2BA' "
				+ "AND GSTN_ERROR = TRUE LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH BT ON HDR.BATCH_ID = BT.ID "
				+ buildQuery + " GROUP BY "
				+ "HDR.SUPPLIER_GSTIN,HDR.DOC_NUM,HDR.DOC_DATE, "
				+ "HDR.ORIGINAL_DOC_NUM,HDR.ORIGINAL_DOC_DATE,ITM.TAX_RATE,"
				+ "HDR.POS, BT.GSTN_SAVE_REF_ID,BT.CREATED_ON, "
