...
@Component("Gstr1SecSectionReportDaoImpl")
public class Gstr1SecSectionReportDaoImpl
		implements Gstr1ReviewSummaryCustomizedReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1SecSectionReportDaoImpl.class);
	...
	public Pair<List<Object[]>, List<Object>> getGstr1RSReports(SearchCriteria criteria) {
		...
		if (APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
			queryStr = createB2CSSavableTransactionalQueryStringForGstr1a(
					buildQueryheader.toString());

		} else {
			queryStr = createB2CSSavableTransactionalQueryString(
					buildQueryheader.toString());
	...
	private static String createB2CSSavableTransactionalQueryString(
		...
		/*return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, "
				...
				+ "AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1' AND HDR.TAX_DOC_TYPE IN ('B2CS') ";*/
		return  "SELECT '' AS ERROR_CODE_ASP "
				...
				+ "	,HDR.CANCEL_REASON "
				+ "	,HDR.CANCEL_REMARKS "
				+ "	,ITM.SUPPLY_TYPE AS SUPPLY_TYPE "
				+ "	,HDR.DOC_CATEGORY "
				+ "	,HDR.DOC_TYPE "
				+ "	,HDR.DOC_NUM "
				+ "	,HDR.DOC_DATE "
				...
				+ "	WHERE CONFG_QUESTION_ID = ( "
				+ "			SELECT ID "
				+ "			FROM CONFG_QUESTION "
				+ "			WHERE QUESTION_DESCRIPTION = 'Do you want to bifurcate TAX/NIL/EXT/NON to Tables 4 and 8 separately basis the supply type?' "
				+ "			) "
				+ "		AND GCP.IS_DELETE = FALSE "
				+ "	) A ON A.GSTIN = HDR.SUPPLIER_GSTIN "
	...
	private String createB2CSSavableTransactionalQueryStringForGstr1a(
			String buildQueryheader) {
		...
		return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, "
				...
				+ "ITM.TCS_AMT AS Tcsamount, HDR.CUST_SUPP_ADDRESS3, "
				+ "HDR.TAX_DOC_TYPE AS DATA_CATEGORY,ITM.ITM_UQC,ITM.ITM_QTY  "
				+ "FROM ANX_OUTWARD_DOC_HEADER_1A HDR INNER JOIN "
				+ "ANX_OUTWARD_DOC_ITEM_1A ITM ON HDR.ID = ITM.DOC_HEADER_ID "
				+ "AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH "
				+ "ON GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN "
				...
				+ "HDR.ASP_INVOICE_STATUS = 2 "
				+ "AND COMPLIANCE_APPLICABLE=TRUE " + buildQueryheader
				+ "AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE IN ('B2CS') ";
