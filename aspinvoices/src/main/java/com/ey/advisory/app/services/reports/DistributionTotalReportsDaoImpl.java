...
@Component("DistributionTotalReportsDaoImpl")
public class DistributionTotalReportsDaoImpl implements DistributionReportsDao {
	...
	private DistributionTotalDto convertProcessed(Object[] arr) {
		...
		obj.setRecordStatus(arr[28] != null ? arr[28].toString() : null);
		return obj;
	}
	...
	private String createProcessedQueryString(String buildQuery) {
		return "SELECT TAX_PERIOD,ISD_GSTIN,CUST_GSTIN,STATE_CODE,"
				...
				+ "DOC_NUM,DOC_DATE,ORG_DOC_NUM,ORG_DOC_DATE,ORG_CR_NUM,"
				+ "ORG_CR_DATE,ELIGIBLE_INDICATOR, IGST_AMT_AS_IGST ,"
				+ "IGST_AMT_AS_SGST,IGST_AMT_AS_CGST,SGST_AMT_AS_SGST,"
				+ "SGST_AMT_AS_IGST, CGST_AMT_AS_CGST,CGST_AMT_AS_IGST,CESS_AMT,"
				+ "(CASE WHEN DOC_TYPE = 'INV' AND ELIGIBLE_INDICATOR IN('IS','E') "
				+ "THEN 'DISTRIBUTION_INV_ELIGIBLE' WHEN DOC_TYPE = 'INV' "
				+ "AND ELIGIBLE_INDICATOR IN( 'NO','IE') THEN "
				+ " 'DISTRIBUTION_INV_INELIGIBLE' WHEN DOC_TYPE = 'RNV' "
				+ "AND ELIGIBLE_INDICATOR IN('IS','E') THEN "
				+ " 'RE_DISTRIBUTION_INV_ELIGIBLE' WHEN DOC_TYPE = 'RNV' "
				+ "AND ELIGIBLE_INDICATOR IN('NO','IE') THEN "
				+ " 'RE_DISTRIBUTION_INV_INELIGIBLE' WHEN DOC_TYPE = 'CR' "
				+ "AND ELIGIBLE_INDICATOR IN('IS','E') THEN "
				+ " 'DISTRIBUTION_CR_ELIGIBLE' WHEN DOC_TYPE = 'CR' "
				+ "AND ELIGIBLE_INDICATOR IN('NO','IE') THEN "
				+ "'DISTRIBUTION_CR_INELIGIBLE' WHEN DOC_TYPE = 'RCR' "
				+ "AND ELIGIBLE_INDICATOR IN('IS','E') THEN "
				+ " 'RE_DISTRIBUTION_CR_ELIGIBLE' WHEN DOC_TYPE = 'RCR' "
				+ "AND ELIGIBLE_INDICATOR IN('NO','IE') THEN "
				+ " 'RE_DISTRIBUTION_CR_INELIGIBLE' END) CATEGORY,"
				...
				+ "GSTR6_ISD_DISTRIBUTION_AS_ENTERED GID "
				+ "LEFT JOIN UTF_GSTR6_ISD_DISTRIBUTION_ERROR_INFO() "
