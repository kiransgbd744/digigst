...
import static com.ey.advisory.common.GSTConstants.TAXABLE_VALUE;
...
public class HsnTaxableValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER5709",
		...
		if (obj != null) {
			...
			if (!isValid /* || number.compareTo(BigDecimal.ZERO) < 0 */) {
				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(TAXABLE_VALUE);
