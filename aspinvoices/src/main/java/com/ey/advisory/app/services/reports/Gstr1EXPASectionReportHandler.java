...
@Slf4j
@Component("Gstr1EXPASectionReportHandler")
public class Gstr1EXPASectionReportHandler {

	@Autowired
	@Qualifier("Gstr1EXPASectionReportServiceImpl")
		...
		try {
			...
			if (workbook != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 EXPORTSA template file name is : {}",
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						...
						+ criteria.getTaxperiod() + "_EXPORTS_Amended_SECTION9A"
						+ ".xlsx";

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 EXPORTSA file name is : {}", fileName);
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Workbook has been generated successfully with the"
									+ " Gstr1 EXPORTSA response list in the directory : {}",
				...
				if (id != null) {
					try {
						...
						String msg = String
								.format("Gstr1EXPASectionReportHandler - "
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 EXPORTSA No Data found for report id : {} ",
							id);
				}
			...
			String msg = String.format("Gstr1EXPASectionReportHandler - "
