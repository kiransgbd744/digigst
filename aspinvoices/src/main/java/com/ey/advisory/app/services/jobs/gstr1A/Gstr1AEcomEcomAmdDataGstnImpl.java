 ...
 *
 */
@Component("Gstr1AEcomEcomAmdDataGstnImpl")
@Slf4j
public class Gstr1AEcomEcomAmdDataGstnImpl implements Gstr1AEcomEcomAmdDataGstn {
	...
	public Long findEcomDataGstn(Gstr1GetInvoicesReqDto dto, String groupCode,
		...
		if (APIConstants.ECOM.equalsIgnoreCase(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_ECOM);
			}
		} else {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1A_GET_ECOM_AMD, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_ECOM_AMD);
			}
		}
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1AEcomEcomAmdSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
