 ...
 *
 */
public class Gstr7DivisionValidator implements BusinessRuleValidator<Gstr7AsEnteredTdsEntity> {
	...
	public List<ProcessingResult> validate(Gstr7AsEnteredTdsEntity document, ProcessingContext context) {
		...
		EntityAtConfigKey entityAtConfigKey = new EntityAtConfigKey(document.getEntityId(),
		...
		entitAtConfigInnerMap.entrySet().forEach(entitAtConfMap -> {
			...
			if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
				boolean isAttrValid = onboardAttributesChecking.isAttrValid(entityAtValMap, OnboardingConstant.DIVISION,
						division, document.getEntityId());
				if (division == null || division.isEmpty()) {
					...
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5082",
							" Division is not as per master " + "provided during On-Boarding.", location));
				}
				if (!isAttrValid) {
			...
			if (OnboardingConstant.AT_O.equalsIgnoreCase(atOutward)) {
				...
				if (!isAttrValid) {
					...
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(null,
							errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, ProcessingResultType.INFO, "IN5115",
							" Division is not as per master " + "provided during On-Boarding.", location));
		...
		if (division != null && !division.isEmpty()) {
			...
			if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name().equalsIgnoreCase(paramterValue12)) {
				boolean isAttrUserValid = onboardAttributesChecking.isAttrValid(OnboardingConstant.PC, division,
						document.getEntityId(), document.getCreatedBy());
				if (!isAttrUserValid) {
					errorLocations.add(GSTConstants.DIVISION);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(null,
							errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5059",
							"Division is not as per user access level" + "  provided in during On-Boarding.",
							location));
				}
			}
		}
