 ...
 *
 */
@Component("Gstr1AOutwardDocSvErrSaveResp")
public class Gstr1AOutwardDocSvErrSaveResp {

	public List<OutwardDocSvErrSaveRespDto> createOutwardDocSvErrSaveResponse(
			List<Long> oldDocIds, List<Gstr1AAnx1OutWardErrHeader> savedDocs,
			List<OutwardTransDocErrorGstr1A> outErrors,
		...
		if (oldDocIds.isEmpty()) {
			savedDocs.forEach(savedDocument -> {
				...
				OutwardDocSvErrSaveRespDto res = new OutwardDocSvErrSaveRespDto();
			...
			IntStream.range(0, savedDocs.size()).forEach(i -> {
				Gstr1AAnx1OutWardErrHeader savedDocument = savedDocs.get(i);
				// errorsResp list is to show errors in response
				List<DocErrorDto> errorsResp = new ArrayList<>();
				OutwardDocSvErrSaveRespDto res = new OutwardDocSvErrSaveRespDto();
				res.setId(savedDocument.getId());
