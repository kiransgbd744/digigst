 ...
 *
 */
@Service("CrossItcTotalReportsServiceImpl")
@Slf4j
public class CrossItcTotalReportsServiceImpl
	...
	@Autowired
	@Qualifier("CrossItcTotalReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Cross Itc total response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.CROSS_ITC).equalsIgnoreCase(request.getFileType())) { 
				...
				String[] invoiceHeaders = commonUtility
						.getProp("crossitc.vertical.total.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
