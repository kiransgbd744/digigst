 ...
 *
 */
public class EinvoiceArithmaticValidation
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		for(OutwardTransDocLineItem item:items){
			itemtaxableValue=	itemtaxableValue.add(item.getTaxableValue() == null
					? BigDecimal.ZERO : item.getTaxableValue());
			itemIgst=	itemIgst.add(item.getIgstAmount() == null ? BigDecimal.ZERO
					: item.getIgstAmount());
			itemCgst = itemCgst.add(item.getCgstAmount() == null ? BigDecimal.ZERO
					: item.getCgstAmount());
			itemSgst =	itemSgst.add(item.getSgstAmount() == null ? BigDecimal.ZERO
					: item.getSgstAmount());
			cessAdv = cessAdv.add(item.getCessAmountAdvalorem() == null ? BigDecimal.ZERO
					: item.getCessAmountAdvalorem());
			cessSpe = 	cessSpe.add(item.getCessAmountSpecific() == null ? BigDecimal.ZERO
					: item.getCessAmountSpecific());
			cessState = cessState.add(item.getStateCessAmount() == null ? BigDecimal.ZERO
					: item.getStateCessAmount());
		}
		
		
	/*	items.forEach(item -> {
			itemtaxableValue.add(item.getTaxableValue() == null
					? BigDecimal.ZERO : item.getTaxableValue());
		});
		items.forEach(item -> {
			itemIgst.add(item.getIgstAmount() == null ? BigDecimal.ZERO
					: item.getIgstAmount());
		});
		items.forEach(item -> {
...
*/
		if (invoiceCessAdvaloremAmount.compareTo(cessAdv) != 0) {
			String[] errorLocations = new String[] { GSTConstants.INV_ADV_CESSAMOUNT };
		...
		if (invoiceCessSpecificAmount.compareTo(cessSpe) != 0) {
			String[] errorLocations = new String[] {GSTConstants.INVSPECESSAMOUNT };
			TransDocProcessingResultLoc location 
			            = new TransDocProcessingResultLoc(
					null, errorLocations);
			errors.add(new ProcessingResult(APP_VALIDATION,
					ProcessingResultType.INFO, "IN11016",
					"Invoice Cess Specific is incorrectly computed",
					location));
		}
		if (invoiceStateCessAmount.compareTo(cessState) != 0) {
			String[] errorLocations = new String[] {GSTConstants.INVSTATECESSAMOUNT };
			TransDocProcessingResultLoc location 
			            = new TransDocProcessingResultLoc(
					null, errorLocations);
			errors.add(new ProcessingResult(APP_VALIDATION,
					ProcessingResultType.INFO, "IN11017",
					"Invoice State Cess is incorrectly computed",
					location));
		}
/*for(OutwardTransDocLineItem item:items){
...
/*
		BigDecimal sumInvamount=invoiceCessSpecificAmount
				...
				//.add(invoiceStateCessAmount)
				//.add(igstAmount)
				//.add(cgstAmount)
				//.add(sgstAmount)
				//.add(invoiceAssessableAmount)
				.add(invoiceCessAdvaloremAmount);
		...
		if(taxAmount.compareTo(BigDecimal.ZERO) > 2){
			String[] errorLocations = new String[] { GSTConstants.INVOICE_VALUE };
			TransDocProcessingResultLoc location 
			            = new TransDocProcessingResultLoc(
					null, errorLocations);
			errors.add(new ProcessingResult(APP_VALIDATION,
					ProcessingResultType.INFO, "IN11018",
