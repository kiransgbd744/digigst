...
@Service("ITC0404ReviewsummaryImpl")
public class ITC0404ReviewsummaryImpl {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ITC0404ReviewsummaryImpl.class);
	...
	public Workbook findReviewSummary(Gstr6SummaryRequestDto setDataSecurity,
			PageRequest pageReq) {
		...
		responseFromView = iTC04EntityLevelSummReportsDaoImpl
				.getEntityLevelSummReports(request);
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ITC04-Review Summary.xlsx");
		...
		Optional<ITC04EntitylevelDto> fromTaxPeriod = responseFromView.stream()
				.findFirst();
		if (fromTaxPeriod.isPresent()) {
			String fromYear = fromTaxPeriod.get().getQreturnPeriod()
					.substring(2);
			String toYear = String.valueOf(Integer.parseInt(fromYear) + 1);
			taxPeriod = fromTaxPeriod.get().getReturnPeriod() + " " + fromYear
					+ "-" + toYear;
		}

		ITC04ReviewSummaryUtility.buildAndGenerateItc04ReviewReport(workbook, responseFromView,
