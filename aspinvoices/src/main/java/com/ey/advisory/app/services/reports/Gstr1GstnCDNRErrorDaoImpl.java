...
import com.ey.advisory.app.data.views.client.Gstr1GstnCDNRErrorDto;
...
@Component("Gstr1GstnCDNRErrorDaoImpl")
public class Gstr1GstnCDNRErrorDaoImpl implements Gstr1GstnErrorDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1GstnCDNRErrorDaoImpl.class);
...
String queryStr = null;
		...
		{
			queryStr = creategstnGstr1ACDNRErrorQueryString(buildQuery.toString());
		}
		else
		{

		queryStr = creategstnCDNRErrorQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertgstnCDNRError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1GstnCDNRErrorDto convertgstnCDNRError(Object[] arr) {
		Gstr1GstnCDNRErrorDto obj = new Gstr1GstnCDNRErrorDto();

		obj.setOriginalPeriod(arr[0] != null ? arr[0].toString() : null);
		obj.setCustomerGSTINUI(arr[1] != null ? arr[1].toString() : null);
		obj.setCreditdebitNoteNumber(arr[2] != null ? arr[2].toString() : null);
		obj.setCreditdebitNoteDate(arr[3] != null ? arr[3].toString() : null);
		obj.setPreceedNum(arr[4] != null ? arr[4].toString() : null);
		obj.setPreceedDate(arr[5] != null ? arr[5].toString() : null);
		obj.setDifferentialPercentage(
				arr[6] != null ? arr[6].toString() : null);
		obj.setTaxableValue(arr[7] != null ? arr[7].toString() : null);
		obj.setRate(arr[8] != null ? arr[8].toString() : null);
		obj.setIntegratedAmount(arr[9] != null ? arr[9].toString() : null);
		obj.setCessAmount(arr[10] != null ? arr[10].toString() : null);
		obj.setInvoiceValue(arr[11] != null ? arr[11].toString() : null);
		obj.setNoteType(arr[12] != null ? arr[12].toString() : null);
		obj.setPregstFlag(arr[13] != null ? arr[13].toString() : null);
		obj.setReasonforNote(arr[14] != null ? arr[14].toString() : null);
		obj.setErrorCode(arr[15] != null ? arr[15].toString() : null);
		obj.setGstnErrorMessage(arr[16] != null ? arr[16].toString() : null);
		obj.setRefId(arr[17] != null ? arr[17].toString() : null);
		obj.setPos(arr[18] != null ? arr[18].toString() : null);
		obj.setReverseChargeFlag(arr[19] != null ? arr[19].toString() : null);
		obj.setSupplyType(arr[20] != null ? arr[20].toString() : null);

		return obj;
	}

	private String creategstnCDNRErrorQueryString(String buildQuery) {

		return "SELECT HDR.ORIGINAL_DOC_PERIOD,HDR.CUST_GSTIN AS GSTIN,"
				+ "HDR.DOC_NUM NOTE_NUM,HDR.DOC_DATE NOTE_DATE,"
                + "HDR.PRECEEDING_INV_NUM,HDR.PRECEEDING_INV_DATE,"
				+ "HDR.DIFF_PERCENT,ITM.TAXABLE_VALUE,"
				+ "ITM.TAX_RATE,ITM.IGST_AMT,(ITM.CESS_AMT_SPECIFIC+ITM.CESS_AMT_ADVALOREM) "
				+ "CESS_AMT, ITM.LINE_ITEM_AMT,HDR.DOC_TYPE AS NOTE_TYPE,"
				+ "ITM.CRDR_PRE_GST, ITM.CRDR_REASON, TRIM(', ' FROM "
				...
				+ "HDR.POS,HDR.REVERSE_CHARGE,HDR.SUPPLY_TYPE "
				...
				+ "WHERE HDR.RETURN_TYPE='GSTR1'  "
				+ "AND HDR.GSTN_ERROR=TRUE AND HDR.TAX_DOC_TYPE='CDNR' "
				+ "AND HDR.IS_DELETE=FALSE "
				+ "AND GSTN_SAVE_STATUS='PE' " + buildQuery;
	}
	
	private String creategstnGstr1ACDNRErrorQueryString(String buildQuery) {
		...
		return "SELECT HDR.ORIGINAL_DOC_PERIOD,HDR.CUST_GSTIN AS GSTIN,"
				...
				+ "LEFT OUTER JOIN TF_OUTWARD_ITEM_GSTIN_ASP_ERROR_INFO_1A () "
				+ "ERRI ON ITM.DOC_HEADER_ID = ERRI.DOC_HEADER_ID "
				+ "AND IFNULL(ITM.ITEM_INDEX,'-1') = IFNULL(ERRI.ITEM_INDEX,'-1') "
				+ "AND IFNULL(ITM.ITM_NO,'0') = IFNULL(ERRI.ITM_NO,'0') "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH "
				+ "GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID "
				+ "WHERE HDR.RETURN_TYPE='GSTR1A'  "
				+ "AND HDR.GSTN_ERROR=TRUE AND HDR.TAX_DOC_TYPE='CDNR' "
				+ "AND HDR.IS_DELETE=FALSE "
				+ "AND GSTN_SAVE_STATUS='PE' " + buildQuery;
	}
