...
import static com.ey.advisory.common.GSTConstants.OrgHsnOrSac;
 ...
 *
 */
public class Gstr1AOrgHsnOrSac
	...
	public List<ProcessingResult> validate(Gstr1AAsEnteredB2csEntity document,
		...
		if (document.getOrgHsnOrSac() != null
				&& !document.getOrgHsnOrSac().isEmpty()) {
			String hsnOrSac = document.getOrgHsnOrSac().trim();
			...
			if (getHsnOrSac <= 0) {
				...
				if (findByHsnOrSac == 0) {
					...
					errorLocations.add(OrgHsnOrSac);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5012",
							"Invalid OrgHSNorSAC", location));
				}
			}
		}
		return errors;
