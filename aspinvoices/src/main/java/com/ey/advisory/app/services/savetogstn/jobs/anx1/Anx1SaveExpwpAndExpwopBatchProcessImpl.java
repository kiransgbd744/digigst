...
@Slf4j
@Service("anx1SaveExpwpAndExpwopBatchProcessImpl")
public class Anx1SaveExpwpAndExpwopBatchProcessImpl implements Anx1SectionWiseSaveBatchProcess {
	...
	/*private Pair<List<Long>, SaveAnx1> setChunkCtinData(SaveAnx1 anx1,
			List<Anx1ExpwpAndExpwopData> cTinList, String section) {
		...
		if (APIConstants.EXPWP.equals(section)) {
			anx.setExpwpInvoice(cTinList);
		} else if (APIConstants.EXPWOP.equals(section)) {
			anx.setExpwopInvoice(cTinList);
		}

		for (Anx1ExpwpAndExpwopData cData : cTinList) {
			for (Anx1ExpwpAndExpwopDocumentData iData : cData.getDocs()) {
	...
	private Pair<List<Long>, SaveAnx1> setChunkInvData(SaveAnx1 anx1,
			List<Anx1ExpwpAndExpwopDocumentData> invList, String section) {

		SaveAnx1 anx = new SaveAnx1();
		List<Long> idsList = new ArrayList<>();
		List<Anx1ExpwpAndExpwopData> cTinList = new ArrayList<>();

		anx.setSgstin(anx1.getSgstin());
		anx.setTaxperiod(anx1.getTaxperiod());

		Anx1ExpwpAndExpwopData cTin = null;

		if (APIConstants.EXPWP.equals(section)) {
			anx.setExpwpInvoice(cTinList);
			cTin = anx1.getExpwpInvoice().get(0);
		} else if (APIConstants.EXPWOP.equals(section)) {
			anx.setExpwopInvoice(cTinList);
			cTin = anx1.getExpwopInvoice().get(0);
		}
		cTin.setDocs(invList);
		cTinList.add(cTin);

		for (Anx1ExpwpAndExpwopDocumentData data : invList) {
	...
	private List<Triplet<List<Long>, SaveAnx1, String>> batchChunk(
		...
		int ctinSize = 0;
		if (APIConstants.EXPWP.equals(section)) {
			ctinSize = anx1.getExpwpInvoice().size();
		} else if (APIConstants.EXPWOP.equals(section)) {
			ctinSize = anx1.getExpwopInvoice().size();
		}
		...
		if (ctinSize > 1) {
			List<Anx1ExpwpAndExpwopData> cTinList1 = null;
			List<Anx1ExpwpAndExpwopData> cTinList2 = null;
			if (APIConstants.EXPWP.equals(section)) {
				cTinList1 = anx1.getExpwpInvoice().subList(0, ctinSize / 2);
				cTinList2 = anx1.getExpwpInvoice().subList(ctinSize / 2, ctinSize);
			} else if (APIConstants.EXPWOP.equals(section)) {
				cTinList1 = anx1.getExpwopInvoice().subList(0, ctinSize / 2);
				cTinList2 = anx1.getExpwopInvoice().subList(ctinSize / 2, ctinSize);
			}
			...
			Anx1ExpwpAndExpwopData cTin1 = null;
			
			if (APIConstants.EXPWP.equals(section)) {
				cTin1 = anx1.getExpwpInvoice().get(0);
			} else if (APIConstants.EXPWOP.equals(section)) {
				cTin1 = anx1.getExpwopInvoice().get(0);
			}
			int invSize = cTin1.getDocs().size();
			List<Anx1ExpwpAndExpwopDocumentData> invList1 = cTin1.getDocs().subList(0,
					invSize / 2);
			List<Anx1ExpwpAndExpwopDocumentData> invList2 = cTin1.getDocs()
