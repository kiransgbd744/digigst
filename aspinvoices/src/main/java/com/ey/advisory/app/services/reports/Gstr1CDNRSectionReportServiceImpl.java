...
@Service("Gstr1CDNRSectionReportServiceImpl")
public class Gstr1CDNRSectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1CDNRSectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1CDNRSectionReportDaoImpl")
		...
		try {
			...
			if (responseFromView != null && responseFromView.size() > 0) {
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					String msg = String
							.format("CDNR Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(" time taken to create the workbook {} ",
								dbLoadTimeDiff);
					}
				} else {
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 CDNR custom file name -"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(
								" time taken to convert the list {} in else block ",
								dbLoadTimeDiff);
					}
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(
								" time taken to generate the workbook in else block  {} ",
								dbLoadTimeDiff);
					}
			...
			LOGGER.error(
					"Exception Occuered in CDNR Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
