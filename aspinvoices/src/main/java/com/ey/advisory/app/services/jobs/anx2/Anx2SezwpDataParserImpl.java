...
import com.ey.advisory.app.data.entities.simplified.client.GetAnx2SezwpInvoicesItemEntity;
 ...
 *
 */
@Service("Anx2SezwpDataParserImpl")
@Slf4j
public class Anx2SezwpDataParserImpl implements Anx2SezwpDataParser {
	...
	public Set<GetAnx2SezwpInvoicesHeaderEntity> parseSezwpData(
		...
		Set<GetAnx2SezwpInvoicesHeaderEntity> invoiceList = new HashSet<>();
		...
		try {
			...
			for (Anx2Data eachInv : baseEntity) {
				for (Anx2DocumentData eachInvData : eachInv.getInvoiceData()) {
					List<GetAnx2SezwpInvoicesItemEntity> itemList = new ArrayList<>();
					GetAnx2SezwpInvoicesHeaderEntity invoice = setInvoiceData(
					...
					for (Anx2ItemDetails lineItem : eachInvData.getItems()) {
						GetAnx2SezwpInvoicesItemEntity item = setItemData(
								lineItem);
						itemList.add(item);
	...
	private GetAnx2SezwpInvoicesHeaderEntity setInvoiceData(Anx2Data eachInv,
			Anx2DocumentData eachInvData, Anx2GetInvoicesReqDto dto,
			Long batchId) {
		GetAnx2SezwpInvoicesHeaderEntity invoice = new GetAnx2SezwpInvoicesHeaderEntity();
		...
		String claimRfd = eachInvData.getClaimRfnd();

		invoice.setTableSection(APIConstants.SEZWP.toUpperCase());
		/**
		 * setting B2BInvoices
		 */
		invoice.setCfs(cfs);
		...
		invoice.setClaimRfnd(claimRfd);

		invoice.setSezwpBatchIdAnx2(batchId);
	...
	private GetAnx2SezwpInvoicesItemEntity setItemData(
			Anx2ItemDetails lineItem) {
		GetAnx2SezwpInvoicesItemEntity item = new GetAnx2SezwpInvoicesItemEntity();
