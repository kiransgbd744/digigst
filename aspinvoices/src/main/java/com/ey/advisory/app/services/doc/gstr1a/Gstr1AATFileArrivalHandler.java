...
import com.ey.advisory.app.services.gstr1afileupload.Gstr1AARStructuralProcess;
import com.ey.advisory.app.services.gstr1fileupload.Gstr1ARStructuralProcess;
...
@Service("Gstr1AATFileArrivalHandler")
@Slf4j
public class Gstr1AATFileArrivalHandler {

	@Autowired
	@Qualifier("AtHeaderCheckService")
	private Gstr1HeaderCheckService gstr1HeaderCheckService;

	@Autowired
	@Qualifier("Gstr1AARStructuralProcess")
	private Gstr1AARStructuralProcess gstr1ARStructuralProcess;
	...
	public void processATsFile(Message message, AppExecContext context) {
		...
		try {
			...
			LOGGER.error(
					"Exception occured in Advance Received File Arrival Processor",
					e);
		}
		...
		List<Object[]> listOfAt = ((FileUploadDocRowHandler<?>) rowHandler)
				.getFileUploadList();
		...
		if (checkHeaderFormat.getValue0()) {
			gstr1ARStructuralProcess.processData(listOfAt, updateFileStatus);
		} else {
