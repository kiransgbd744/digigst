...
import com.ey.advisory.app.data.views.client.Gstr7ProcessedUploadedRecords;
...
import jakarta.persistence.Query;
	...
	@Component("Gstr7RefidReportDaoImpl")
	public class Gstr7RefidReportDaoImpl implements Gstr7GstnErrorReportsDao {

		private static final Logger LOGGER = LoggerFactory.getLogger(Gstr7RefidReportDaoImpl.class);
		...
		@Override
		public List<Object> getGstr7GstnReports(SearchCriteria criteria) {
			Gstr1ReviwSummReportsReqDto request = (Gstr1ReviwSummReportsReqDto) criteria;

			// String dataType = request.getDataType();
			String taxperiod = request.getTaxperiod();
			String refId = request.getGstnRefId();
...
*/
			...
			if (refId != null) {// Reference Id
				buildQuery.append(" AND GSTN_SAVE_REF_ID  = :refId");
			}
		...
		private Gstr7ProcessedUploadedRecords convertProcessed(Object[] arr) {
			Gstr7ProcessedUploadedRecords obj = new Gstr7ProcessedUploadedRecords();
			...
			if (arr[2] == null || arr[2] == "null") {
				...
				obj.setGstnRefidDateTime(newdate);
			}
			/*if (arr[2] != null) {
	            Timestamp timeStamp = (Timestamp) arr[2];
			...
			obj.setSource(arr[33] != null ? arr[33].toString() : null);
			obj.setUserId(arr[34] != null ? arr[34].toString() : null);
			obj.setFileId(arr[35] != null ? arr[35].toString() : null);
			obj.setFileName(arr[36] != null ? arr[36].toString() : null);
			//obj.setUploadDataDateTime(arr[37] != null ? arr[37].toString() : null);
			/*if (arr[37] != null) {
	            Timestamp timeStamp = (Timestamp) arr[37];
	            LocalDateTime localDT = timeStamp.toLocalDateTime();
	            LocalDateTime convertref = EYDateUtil
	                    .toISTDateTimeFromUTC(localDT);
	            obj.setUploadDataDateTime(convertref.toString());
	        } else {
	            obj.setUploadDataDateTime(null);
	        }*/

			
			if (arr[37] == null || arr[37] == "null") {
				obj.setUploadDataDateTime("");
			} else {
				Timestamp date = (Timestamp) arr[37];
				LocalDateTime dt = date.toLocalDateTime();
				LocalDateTime dateTimeFormatter = EYDateUtil
						.toISTDateTimeFromUTC(dt);
				DateTimeFormatter FOMATTER = DateTimeFormatter
		...
		private String createApiProcessedQueryString(String buildQuery) {
			...
			return "SELECT (CASE WHEN TDS.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' "
					+ "WHEN TDS.IS_SAVED_TO_GSTN = FALSE AND "
					+ "TDS.GSTN_ERROR = TRUE THEN 'IS_ERROR' "
					+ "WHEN TDS.IS_SAVED_TO_GSTN = FALSE AND TDS.GSTN_ERROR = FALSE "
					...
					+ "TRIM(', ' FROM IFNULL(GSTN_ERROR_CODE,'')) AS GSTN_ERROR_CODE, "
					+ "TRIM(', ' FROM IFNULL(GSTN_ERROR_DESCRIPTION,'')) "
					+ "AS GSTN_ERROR_DESCRIPTION, TDS.TABLE_NUM,"
					+ "TDS.RETURN_PERIOD,ACTION_TYPE, TDS_DEDUCTOR_GSTIN,"
					+ "ORG_TDS_DEDUCTEE_GSTIN,ORG_RETURN_PERIOD,"
					+ "ORG_GROSS_AMT, NEW_TDS_DEDUCTEE_GSTIN,"
					+ "NEW_GROSS_AMT,IGST_AMT,"
					+ "CGST_AMT,SGST_AMT, "
					+ "CONTRACT_NUMBER,CONTRACT_DATE,CONTRACT_VALUE,"
					+ "PAYMENT_ADV_NUM,PAYMENT_ADV_DATE, DOC_NUM,DOC_DATE,"
					+ "INVOICE_VALUE, PLANT_CODE,DIVISION,"
					+ "PURCHASE_ORGANIZATION ,PROFIT_CENTRE1,PROFIT_CENTRE2, "
					+ "USERDEFINED_FIELD1,USERDEFINED_FIELD2,USERDEFINED_FIELD3, "
					+ "FIL.SOURCE SOURCE,FIL.CREATED_BY USER_ID, "
					+ "FIL.ID FILE_ID,FIL.FILE_NAME FILE_NAME,"
					+ "FIL.UPLOAD_START_TIME FROM GSTR7_PROCESSED_TDS TDS "
					+ "LEFT OUTER JOIN FILE_STATUS FIL ON TDS.FILE_ID=FIL.ID "
					+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH "
					+ "ON GSTNBATCH.ID = TDS.BATCH_ID WHERE "
					+ "GSTN_ERROR=TRUE AND GSTN_SAVE_STATUS='PE' "
					+ "AND TDS.IS_DELETE=FALSE " + buildQuery;
