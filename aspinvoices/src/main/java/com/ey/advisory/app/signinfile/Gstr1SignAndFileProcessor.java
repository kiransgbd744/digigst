 ...
 */
@Slf4j
@Component("Gstr1SignAndFileProcessor")
public class Gstr1SignAndFileProcessor implements TaskProcessor {
	...
	@Autowired
	private Gstr1SignAndFileService gstr1SignAndFileService;
	...
	public void execute(Message message, AppExecContext context) {
		...
		LOGGER.debug(
				"GSTR1 Sign&File request received for groupcode {} and params {}",
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GSTR1 Sign&File GSTN payload about to send is {}",
						data);
			}
			APIResponse gstr1FileApiResp = hitGstnServer.gstr1FileApiCall(
			...
			if (gstr1FileApiResp.isSuccess()) {
				...
				gstr1SignAndFileService.updateGstr1Tables(taxPeriod, gstin,
						signId, ackNum, now);

			} else {
				LOGGER.error("GSTR1 Sign&File Failed with response {} ",
			...
			String msg = String.format(
					"Exception while GSTR1 Sign&File, jsonParams are '%s'",
					jsonString);
