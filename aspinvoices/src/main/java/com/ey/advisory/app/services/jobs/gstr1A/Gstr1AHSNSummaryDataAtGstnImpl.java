...
@Component("gstr1AHSNSummaryDataAtGstnImpl")
@Slf4j
public class Gstr1AHSNSummaryDataAtGstnImpl
		implements Gstr1AHSNSummaryDataAtGstn {
	...
	public Long gstr1HSNSummaryDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		APIParam para1 = new APIParam("gstin", dto.getGstin());
		APIParam para2 = new APIParam("ret_period", dto.getReturnPeriod());
		...
		if (APIConstants.HSN.equals(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1A_GET_HSN_SUMMARY, para1, para2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_HSN_SUMMARY);
			}
		}
		
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1AHsnSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
		...
		if (!response.isSuccess()) {
			String msg = "Failed to get Gstr1 HSN Details from Gstn";
