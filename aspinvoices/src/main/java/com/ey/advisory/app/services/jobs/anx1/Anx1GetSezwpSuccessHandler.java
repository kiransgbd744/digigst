...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1SezwpInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetSezwpSuccessHandler")
@Slf4j
public class Anx1GetSezwpSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Anx1SezwpDataParserImpl")
	private Anx1SezwpDataParser anx1SezwpDataParser;

	@Autowired
	@Qualifier("GetAnx1SezwpInvoicesRepository")
	private GetAnx1SezwpInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetSezwpSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1SezwpInvoicesHeaderEntity> b2cEntities = anx1SezwpDataParser
						.parseSezwpData(dto, apiResp);
