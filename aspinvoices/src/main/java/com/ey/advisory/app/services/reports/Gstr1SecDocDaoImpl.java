...
@Component("Gstr1SecDocDaoImpl")
public class Gstr1SecDocDaoImpl implements Gstr1ASPInvSavableDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1SecDocDaoImpl.class);
	...
	public List<Object> getGstr1InvSavableReports(SearchCriteria criteria) {
		...
		if (!Strings.isNullOrEmpty(request.getReturnType())
			...
			queryStr = createInvSavableSummaryQueryStringForGstr1a(
					buildQuery.toString());

		} else {
			queryStr = createInvSavableSummaryQueryString(
					buildQuery.toString());
	...
	private String createInvSavableSummaryQueryString(String buildQuery) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("gstr1 Invoice series buildQuery{}", buildQuery);
		}
		...
		return "select SUPPLIER_GSTIN, RETURN_PERIOD,SERIAL_NUM, NATURE_OF_DOC,"
				...
				+ "WHERE INV.IS_DELETE = FALSE " + buildQuery + " ) "
				+ " GROUP BY SUPPLIER_GSTIN, RETURN_PERIOD,SERIAL_NUM, "
				+ "NATURE_OF_DOC,DOC_SERIES_FROM,DOC_SERIES_TO ";
	}

	private String createInvSavableSummaryQueryStringForGstr1a(
			String buildQuery) {

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("gstr1a Invoice series buildQuery{}", buildQuery);
		}
