 ...
 *
 */
@Component("CrossItcProcessedInfoReportsDaoImpl")
@Slf4j
public class CrossItcProcessedInfoReportsDaoImpl
	...
	private String createVerticalProcessedInfoQueryString(String buildQuery) {
		...
		if ((GSTConstants.CROSS_ITC).equalsIgnoreCase(fileType)) {
			...
			builder.append(
					"FROM CROSS_ITC_PROCESSED HI LEFT OUTER JOIN TF_VERTICAL_ERROR_INFO() ERR ON ");
			builder.append(
					"HI.AS_ENTERED_ID= ERR.COMMON_ID AND HI.FILE_ID = ERR.FILE_ID ");
			builder.append(
					"LEFT OUTER JOIN CLIENT1_GST.FILE_STATUS FIL ON HI.FILE_ID = FIL.ID ");
			builder.append(
