...
@Component("ItcTotalReportDaoImpl")
public class ItcTotalReportDaoImpl implements ItcReportDao {

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ItcTotalReportDaoImpl.class);
	...
	public List<Object> getItcReports(
		...
		if (fileId != null) {
			buildQuery1.append(" WHERE HDR.FILE_ID= :fileId ");
		}
	...
	private String createVerticalTotalQueryString(String buildQuery, String buildQuery1) {
		return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| "
				...
				+ "TO_CHAR(JW_STATE_CODE ),TO_CHAR(JW_WORKER_TYPE),"
				+ "TO_CHAR(JW_ID),TO_CHAR(JW_NAME), TO_CHAR(GOODS_TYPE),"
				+ "TO_CHAR(ITM_SER_NO),TO_CHAR(PRODUCT_DESC),TO_CHAR(PRODUCT_CODE),"
				+ "TO_CHAR(NATURE_OF_JW), TO_CHAR(ITM_HSNSAC ),TO_CHAR(ITM_UQC),"
				+ "TO_CHAR(ITM_QTY ),TO_CHAR(LOSSES_UQC),TO_CHAR(LOSSES_QTY), "
				+ "TO_CHAR(ITM.TAXABLE_VALUE )ITM_ACCESSABLE_AMT ,TO_CHAR(IGST_RATE),"
				+ "TO_CHAR(ITM.IGST_AMT),TO_CHAR(CGST_RATE),TO_CHAR(ITM.CGST_AMT),"
				+ "TO_CHAR(SGST_RATE),TO_CHAR(ITM.SGST_AMT), TO_CHAR(CESS_RATE_ADVALOREM),"
				+ "TO_CHAR(ITM.CESS_AMT_ADVALOREM),TO_CHAR(CESS_RATE_SPECIFIC ),"
				+ "TO_CHAR(ITM.CESS_AMT_SPECIFIC), TO_CHAR(STATE_CESS_RATE_ADVALOREM),"
				+ "TO_CHAR(ITM.STATE_CESS_AMT_ADVALOREM),TO_CHAR(STATE_CESS_RATE_SPECIFIC),"
				...
				+ "FIL.SOURCE SOURCE, '' AS GSTN_SAVE_STATUS, '' AS GSTIN_REF_ID, "
				...
				+ "AND HDR.RETURN_PERIOD=ITM.RETURN_PERIOD AND "
				+ "HDR.FILE_ID = ITM.FILE_ID INNER JOIN FILE_STATUS FIL "
				...
				+ "ITM.TABLE_NUMBER ,ITM.ACTION_TYPE ,ITM.FI_YEAR,ITM.RETURN_PERIOD,"
				+ "ITM.SUPPLIER_GSTIN, ITM.DELIVERY_CHALLAN_NO,ITM.DELIVERY_CHALLAN_DATE,"
				+ "ITM.JW_DELIVERY_CHALLAN_NO,ITM.JW_DELIVERY_CHALLAN_DATE, "
				+ "ITM.GOODS_RECEIVING_DATE,ITM.INV_NUM ,ITM.INV_DATE,ITM.JW_GSTIN ,"
				+ "ITM.JW_STATE_CODE ,ITM.JW_WORKER_TYPE,ITM.JW_ID,ITM.JW_NAME, "
				+ "ITM.GOODS_TYPE,ITM.ITM_SER_NO,ITM.PRODUCT_DESC,ITM.PRODUCT_CODE,"
				+ "ITM.NATURE_OF_JW, ITM_HSNSAC , ITM_UQC, ITM_QTY , LOSSES_UQC, "
				+ "LOSSES_QTY, ITM.TAXABLE_VALUE ITM_ACCESSABLE_AMT , IGST_RATE, "
				...
				+ "ITM.STATE_CESS_AMT_SPECIFIC,ITM.TOTAL_VALUE, ITM.POSTING_DATE,"
				+ "ITM.USER_ID,ITM.COMPANY_CODE,ITM.SOURCE_IDENTIFIER, "
				+ "ITM.SOURCE_FILE_NAME , ITM.PLANT_CODE ,ITM.DIVISION,"
				+ "ITM.PROFIT_CENTRE1,ITM.PROFIT_CENTRE2, "
				+ "ITM.ACCOUNTING_VOUCHER_NUM,ITM.ACCOUNTING_VOUCHER_DATE, "
				+ "USERDEFINED_FIELD1,"
				+ "USERDEFINED_FIELD2,USERDEFINED_FIELD3,USERDEFINED_FIELD4,"
				+ "USERDEFINED_FIELD5, USERDEFINED_FIELD6,USERDEFINED_FIELD7,"
				+ "USERDEFINED_FIELD8,USERDEFINED_FIELD9,USERDEFINED_FIELD10, "
				+ "FIL.CREATED_BY USERID, FIL.ID FILE_ID,FIL.FILE_NAME FILE_NAME,"
				+ "FIL.SOURCE SOURCE, '' AS GSTN_SAVE_STATUS, '' AS GSTIN_REF_ID, "
				+ " '' AS GSTIN_REF_ID_TIME, '' AS GSTIN_ERROR_CODE, "
				+ " '' AS GSTIN_ERROR_DESCRIPTION_ASP FROM ITC04_ERR_HEADER "
				+ "HDR INNER JOIN ITC04_ERR_ITEM ITM ON HDR.ID=ITM.DOC_HEADER_ID "
