 ...
 *
 */
@Service("Gstr1SimpleDocGstnSummarySearchService")
public class Gstr1SimpleDocGstnSummarySearchService implements SearchService {

	@SuppressWarnings("unused")
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1SimpleDocGstnSummarySearchService.class);

	@Autowired
	@Qualifier("SimpleGstr1BasicGstnSummarySectionFetcher")
	SimpleGstr1BasicGstnSummarySectionFetcher fetcher;
		...
		if ("UPDATEGSTIN".equalsIgnoreCase(req.getAction())) {
			...
			if (gstinList != null && gstinList.size() > 0) {
				for (String gstn : gstinList) {
					...
					if(req.getReturnType()!=null && "GSTR1A".equalsIgnoreCase(req.getReturnType()))
					{
					apiResp = gstr1SummaryGstnData.getGstr1ASummary(reqDto,
							groupCode);
					}else{
						apiResp = gstr1SummaryGstnData.getGstr1Summary(reqDto,
								groupCode);
					}
	...
	/*
	 * @Override public <R> Stream<R> find(SearchCriteria criteria, Class<?
	 * extends R> retType) { // TODO Auto-generated method stub return null; }
	 */
}
