...
@Component("Gstr1ACdnurResponseHandler")
public class Gstr1ACdnurResponseHandler implements Gstr1AResponseHandler {
	...
	private DocumentTypeMapperFactory cdnura;
	...
	public Triplet<List<String>, Map<String, OutwardTransDocErrorGstr1A>, Integer> response(
		...
		if (APIConstants.CDNUR.equalsIgnoreCase(batch.getSection())
				&& gsterrors != null) {
			for (CDNURInvoices invoice : gsterrors.getCdnurInvoice()) {
				String docNo = invoice.getCredDebRefVoucherNum();

				String idt = invoice.getCredDebRefVoucherDate();
				...
				digiGstDocType = cdnur
						.mapDocType(invoice.getCredDebRefVoucher());
			...
			n = gsterrors.getCdnurInvoice().size();
		} else if (APIConstants.CDNURA.equalsIgnoreCase(batch.getSection())
				&& gsterrors != null) {
			for (CDNURInvoices invoice : gsterrors.getCdnuraInvoice()) {
				String docNo = invoice.getCredDebRefVoucherNum();
			...
			n = gsterrors.getCdnuraInvoice().size();
		}
