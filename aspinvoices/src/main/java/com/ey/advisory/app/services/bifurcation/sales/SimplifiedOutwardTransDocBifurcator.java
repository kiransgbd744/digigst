...
@Component("SimplifiedOutwardTransDocBifurcator")
public class SimplifiedOutwardTransDocBifurcator
		implements DocBifurcator<OutwardTransDocument> {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(SimplifiedOutwardTransDocBifurcator.class);
	...
	private OutwardTransDocument checkForB2b(OutwardTransDocument doc,
			ProcessingContext context) {
		...
		for (OutwardTransDocLineItem lineItem : doc.getLineItems()) {
		...
		OutwardTransDocLineItem firstLineItem = doc.getLineItems().get(0);
	...
	private OutwardTransDocument checkForB2c(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForExportsWithTax(
			OutwardTransDocument doc, ProcessingContext context) {
	...
	private OutwardTransDocument checkForExportsWithOutTax(
			OutwardTransDocument doc, ProcessingContext context) {
	...
	private OutwardTransDocument checkForSEZWithTax(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForSEZWithOutTax(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForDeemedExports(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForExemptedNil(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForNonRated(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForReverseCharge(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForSezToDTA(OutwardTransDocument doc,
			ProcessingContext context) {
	...
	private OutwardTransDocument checkForAdjustmentVouchers(
			OutwardTransDocument doc, ProcessingContext context) {
	...
	public boolean isBifurcated(OutwardTransDocument doc) {
		...
		String msg = String.format("Checking isBifurcated for Doc No : %s, "
				+ "Doc Date : %s, Supply Type : %s, Doc Type : %s, Cgstin %s ",
				doc.getDocNo(), doc.getDocDate().toString(), doc.getSupplyType()
				, doc.getDocType(), doc.getCgstin());
