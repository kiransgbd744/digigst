...
@Service("TXPDStructure")
public class TXPDStructure {
	...
	@Autowired
	@Qualifier("TXPDEYFinalStructure")
	private TXPDEYFinalStructure txpdEYFinalStructure;
	...
	public JsonElement txpdResp(
			List<Gstr1BasicSummarySectionDto> eySummaryListFromView,
			List<? extends Gstr1B2BGstnRespDto> gstnList) {

		Gson gson = GsonUtil.newSAPGsonInstance();

		List<Gstr1SummaryRespDto> defaultTXPDEYList = getDefaultTXPDEYStructure();

		List<Gstr1SummaryRespDto> txpdEYList = txpdEYFinalStructure
				.gettxpdEyList(defaultTXPDEYList, eySummaryListFromView);
		
		Gstr1SummaryRespDto eyTotalCount = eyTotalCount(txpdEYList);
		...
		JsonElement eyTypeDetailsRespBody = gson.toJsonTree(txpdEYList);
		...
		JsonElement txpdRespbody = gson.toJsonTree(combinedMap);
	...
	private List<Gstr1SummaryRespDto> getDefaultTXPDEYStructure() {
		// TODO Auto-generated method stub
		List<Gstr1SummaryRespDto> defaultTXPDEY = new ArrayList<>();

		Gstr1SummaryRespDto txpdEy11A = new Gstr1SummaryRespDto();
		txpdEy11A.setTableSection("11B");
		txpdEy11A = defaultStructureUtil.defaultStructure(txpdEy11A);

		defaultTXPDEY.add(txpdEy11A);
	...
	private Gstr1SummaryRespDto eyTotalCount(
			List<Gstr1SummaryRespDto> txpdEYList) {
		...
		List<Gstr1SummaryRespDto> txpdey = txpdEYList;
		for (Gstr1SummaryRespDto txpdeyreq : txpdey) {
			invValue = invValue.add(txpdeyreq.getInvValue());
			taxable = taxable.add(txpdeyreq.getTaxableValue());
			taxpayble = taxpayble.add(txpdeyreq.getTaxPayble());
			igst = igst.add(txpdeyreq.getIgst());
			cgst = cgst.add(txpdeyreq.getCgst());
			sgst = sgst.add(txpdeyreq.getSgst());
			cess = cess.add(txpdeyreq.getCess());
			records = records + (txpdeyreq.getRecords());
