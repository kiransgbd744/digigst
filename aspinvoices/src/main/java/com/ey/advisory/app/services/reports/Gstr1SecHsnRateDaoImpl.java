...
@Component("Gstr1SecHsnRateDaoImpl")
public class Gstr1SecHsnRateDaoImpl /* implements Gstr1AspHsnSummaryDao */ {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1SecHsnRateDaoImpl.class);
	...
	public List<Gstr1AspVerticalHsnDto> getGstr1RSReports(
		...
		if (supplyType == null || supplyType.isEmpty()) {
			buildSupplyType.append(
				...
				buildHdr1SupplyType.append(
						" OR ITM.SUPPLY_TYPE NOT IN ('NON','SCH3','NIL','EXT') ");
		...
		if ("A".equalsIgnoreCase(supplyType)) {
			buildSupplyType.append(" AND SUPPLY_TYPE NOT IN ('NIL','EXT') ");
				...
				buildHdr1SupplyType
				.append(" OR ITM.SUPPLY_TYPE NOT IN ('NIL','EXT') ");
			...
			buildSupplyType
				...
				buildHdr1SupplyType
				.append(" OR ITM.SUPPLY_TYPE NOT IN ('NON','SCH3','EXT') ");
			...
			buildSupplyType
				...
				buildHdr1SupplyType
				.append(" OR ITM.SUPPLY_TYPE NOT IN ('NON','SCH3','NIL') ");
		...
		if (!Strings.isNullOrEmpty(request.getReturnType()) && APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
			
			 queryStr = createHsnSummaryQueryStringForGstr1a(buildQuery.toString(),
						buildHeader.toString(), buildheader.toString(), supplyTypeQuery,
						buildHdrSupplyType, docTypeQuery);

		} else {
			queryStr = createHsnSummaryQueryString(buildQuery.toString(),
					buildHeader.toString(), buildheader.toString(), supplyTypeQuery,
					buildHdrSupplyType, docTypeQuery, multiSupplyTypeAns);	
	...
	/*	BigDecimal bigDecimalTotalQuantity = (BigDecimal) arr[6];
		...
		String hsnCheck = hsnCode != null
				&& hsnCode.trim().length() > 1
						? hsnCode.substring(0, 2) : hsnCode;
						
		if ("99".equalsIgnoreCase(hsnCheck)){
			obj.setUqc("NA");
			obj.setTotalQuantity(BigDecimal.ZERO);
		}else{
			BigDecimal bigDecimalTotalQuantity = (BigDecimal) arr[6];
			if (bigDecimalTotalQuantity != null) {
				BigDecimal total = new BigDecimal(
		...
		//	obj.setTotalQuantity(bigDecimalTotalQuantity);
			obj.setUqc(arr[5] != null ? arr[5].toString() : null);
		}
		...
		obj.setRate(arr[26] != null ? arr[26].toString() : null);
		obj.setRecordType(arr[27] != null ? arr[27].toString() : null);
	...
	public static void main(String[] args) {
		...
		System.out.println(createHsnSummaryQueryString(buildQuery.toString(),
				buildHeader.toString(), buildheader.toString(), supplyTypeQuery,
				buildHdrSupplyType, docTypeQuery, multiSupplyTypeAns));
		
//		System.out.println(createHsnSummaryQueryStringForGstr1a("buildQuery",
//				"buildHeader", "buildheader", "supplyTypeQuery",
//				"buildHdrSupplyType", "docTypeQuery"));
	}
	//  check here fro gstr1
	private static String createHsnSummaryQueryString(String buildQuery,
			String buildHeader, String buildheader, String supplyTypeQuery,
			String buildHdrSupplyType, String docTypeQuery, String multiSupplyTypeAns) {
		...
		bufferString.append("SUPPLIER_GSTIN) SNO,TAX_RATE,RecordType  ");
		...
		bufferString.append(" (CASE WHEN LEFT(ITM_HSNSAC, 2) = '99' then 'NA' else ITM_UQC end) AS ITM_UQC, ");
		bufferString.append("SUM(CASE WHEN LEFT(ITM_HSNSAC, 2) = '99' then 0 ELSE ASP_ITM_QTY END) AS ASP_ITM_QTY, ");
		bufferString.append("SUM(ASP_TOTAL_VALUE)ASP_TOTAL_VALUE, ");
		bufferString.append("SUM(ASP_TAXABLE_VALUE)ASP_TAXABLE_VALUE, ");
		bufferString.append("SUM(ASP_IGST)ASP_IGST,SUM(ASP_CGST)ASP_CGST, ");
		bufferString.append("SUM(ASP_SGST)ASP_SGST,SUM(ASP_CESS)ASP_CESS, ");
		...
		bufferString.append(
				"MAX(GSTIN_ERROR_DESCRIPTION)GSTIN_ERROR_DESCRIPTION,IFNULL(TAX_RATE,0) TAX_RATE,RecordType ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_REF_ID, GSTIN_REF_ID_TIME, GSTIN_ERROR_CODE, GSTIN_ERROR_DESCRIPTION,IFNULL(TAX_RATE,0) TAX_RATE,RecordType ");
		bufferString.append(
				" from ( SELECT SUPPLIER_GSTIN,RETURN_PERIOD,DERIVED_RET_PERIOD,");
		bufferString.append(
				"ITM_HSNSAC, ITM_UQC, ASP_ITM_QTY,ASP_TOTAL_VALUE, ASP_TAXABLE_VALUE, ASP_IGST, ASP_CGST, ASP_SGST,");
		bufferString.append(
				"ASP_CESS, SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME, GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION,IFNULL(TAX_RATE,0) TAX_RATE,RecordType ");
		bufferString.append(
		...
		/*
		 * bufferString
		 * .append("THEN GSTIN_REF_ID ELSE NULL END AS GSTIN_REF_ID, ");
		 * bufferString.append(
		 * "CASE WHEN SAVE_STATUS='IS_SAVED' THEN GSTIN_REF_ID_TIME ELSE NULL END AS GSTIN_REF_ID_TIME, "
		 * );
		 */
		bufferString.append(
				"GSTIN_ERROR_CODE, GSTIN_ERROR_DESCRIPTION,IFNULL(TAX_RATE,0) TAX_RATE,RecordType FROM ( SELECT SUPPLIER_GSTIN,RETURN_PERIOD,");
		bufferString.append(
		...
		bufferString.append(
				" GSTIN_REF_ID_TIME, '' GSTIN_ERROR_CODE, '' GSTIN_ERROR_DESCRIPTION,IFNULL(TAX_RATE,0) TAX_RATE,RecordType ");
		bufferString.append(
				" FROM ( SELECT SUPPLIER_GSTIN,RETURN_PERIOD, DERIVED_RET_PERIOD,ITM_HSNSAC, ");
		bufferString.append(
				"ITM_UQC,SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME,ASP_ITM_QTY,ASP_TOTAL_VALUE, ASP_TAXABLE_VALUE, ASP_IGST, ASP_CGST, ");
		bufferString.append(
				"ASP_SGST, ASP_CESS,IFNULL(TAX_RATE,0) TAX_RATE,RecordType FROM (SELECT  DOC_KEY,SUPPLIER_GSTIN,RETURN_PERIOD, ");
		bufferString.append(
		...
		bufferString.append(
				"SUM(IFNULL(CESS_A,0)- IFNULL(CESS_S,0)) AS ASP_CESS,IFNULL(TAX_RATE,0) TAX_RATE,RecordType ");
		bufferString.append(
		...
		bufferString.append(
				"CASE WHEN DOC_TYPE IN('CR') THEN CESS END CESS_S,TAX_RATE,RecordType ");
		bufferString.append(
		...
		bufferString.append(
				"END AS TOTAL_VALUE,TAXABLE_VALUE,IGST_AMT IGST,CGST_AMT CGST,SGST_AMT SGST,CESS_AMT CESS,TAX_RATE,RecordType ");
		...
		bufferString.append(
				"ELSE IFNULL(ITM_QTY,0) END AS ITM_QTY , IFNULL(ONB_LINE_ITEM_AMT,0) AS TOTAL_VALUE, ");
		...
		bufferString.append(
					" ITM.ITM_TAX_DOC_TYPE AS TAX_DOC_TYPE,  ITM.ITM_TABLE_SECTION AS TABLE_SECTION,ONB_LINE_ITEM_AMT ,ITM.OTHER_VALUES,TAX_RATE, ");
		
		bufferString.append(" CASE WHEN ITM.CUST_GSTIN IS NOT NULL AND LENGTH(ITM.CUST_GSTIN) = 15 THEN 'B2B' ");
		bufferString.append(" WHEN (LENGTH(ITM.CUST_GSTIN) < 15 OR ITM.CUST_GSTIN IS NULL)    THEN 'B2C' END AS RecordType ");
		...
		bufferString.append(
				"AND COMPLIANCE_APPLICABLE = TRUE AND HDR.RETURN_TYPE='GSTR1' ");
		

		bufferString.append(
					"AND HDR.IS_DELETE=false AND ( ITM.ITM_TABLE_SECTION NOT IN('8','8A','8B','8C','8D')  ");	
		...
		bufferString.append(
				"+IFNULL(CESS_AMT,0) ONB_LINE_ITEM_AMT,0.00 OTHER_VALUES,NEW_RATE TAX_RATE,'B2C' AS RecordType  FROM GSTR1_PROCESSED_B2CS ");
		bufferString.append(
		...
		bufferString.append("SELECT ( DERIVED_RET_PERIOD || '|' || SUPPLIER_GSTIN || '|' || ");
		bufferString.append("IFNULL(ITM_UQC, 'OTH') || '|' || IFNULL(ITM_HSNSAC, 'null') ) DOC_KEY,"); 
		bufferString.append("DERIVED_RET_PERIOD,SUPPLIER_GSTIN,RETURN_PERIOD,"); 
		bufferString.append("SUPPLY_TYPE,'INV' DOC_TYPE,ITM_HSNSAC,");
		bufferString.append("IFNULL(ITM_UQC, 'OTH') AS ITM_UQC,SAVE_STATUS,GSTIN_REF_ID,"); 
		bufferString.append("GSTIN_REF_ID_TIME,"); 
		bufferString.append("IFNULL( MAX(CASE WHEN RN = 1 THEN ITM_QTY ELSE 0 END) , 0) AS ITM_QTY, ");
		bufferString.append("IFNULL(SUM(TAXABLE_VALUE), 0) AS TOTAL_VALUE,"); 
		bufferString.append("IFNULL(SUM(TAXABLE_VALUE), 0) AS TAXABLE_VALUE,");
		bufferString.append("0 AS IGST_AMT,0 CGST_AMT,0 AS SGST_AMT,0 AS CESS_AMT,"); 
		bufferString.append("'NILEXTNON' AS TAX_DOC_TYPE,'8' TABLE_SECTION,");
		bufferString.append("IFNULL(SUM(TAXABLE_VALUE), 0) ONB_LINE_ITEM_AMT,"); 
		bufferString.append("0.00 OTHER_VALUES,0.00 TAX_RATE, RecordType ");
		bufferString.append("FROM ( SELECT HDR.*, ROW_NUMBER() OVER ");
		bufferString.append("( PARTITION BY (HDR.DERIVED_RET_PERIOD || '|' || ");
		bufferString.append("HDR.SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') ");
		bufferString.append("|| '|' || 0.00 || '|' || IFNULL(ITM_HSNSAC, 'null')) ");
		bufferString.append("ORDER BY (HDR.DERIVED_RET_PERIOD || '|' || ");
		bufferString.append("HDR.SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') ");
		bufferString.append("|| '|' || 0.00 || '|' || IFNULL(ITM_HSNSAC, 'null') ) ) AS RN,");
		bufferString.append("(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE ");
		bufferString.append("THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) AS SAVE_STATUS,");
		bufferString.append("(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE ");
		bufferString.append("THEN GSTNBATCH.GSTN_SAVE_REF_ID ELSE NULL END) AS GSTIN_REF_ID,");
		bufferString.append("(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE ");
		bufferString.append("THEN GSTNBATCH.BATCH_DATE ELSE NULL END) AS GSTIN_REF_ID_TIME, ");
		
		bufferString.append(" CASE WHEN TABLE_SECTION IN ( '8A', '8B') THEN 'B2B' ");
		bufferString.append(" WHEN TABLE_SECTION IN ( '8C', '8D') THEN 'B2C' ");
		bufferString.append(" END AS RecordType ");
		
		bufferString.append("FROM GSTR1_SUMMARY_NILEXTNON HDR ");
		bufferString.append("LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON ");
		bufferString.append("GSTNBATCH.ID = HDR.BATCH_ID WHERE HDR.IS_DELETE = FALSE "); 
		bufferString.append(supplyTypeQuery);
		bufferString.append(buildHeader);
		bufferString.append(" ) GROUP BY ( DERIVED_RET_PERIOD || '|' || ");
		bufferString.append("SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') ");
		bufferString.append("|| '|' || IFNULL(ITM_HSNSAC, 'null') ), ");
		bufferString.append("DERIVED_RET_PERIOD, SUPPLIER_GSTIN, RETURN_PERIOD, ");
		bufferString.append("SUPPLY_TYPE, ITM_HSNSAC, IFNULL(ITM_UQC, 'OTH'), SAVE_STATUS,");
		bufferString.append("GSTIN_REF_ID,GSTIN_REF_ID_TIME,RecordType ");
		...
		// bufferString.append("AND DOC_TYPE IN('INV','BOS','CR','DR') AND
		// SUPPLY_TYPE NOT IN('NIL'))) ");
		bufferString.append(
				" )) GROUP BY DOC_KEY,SUPPLIER_GSTIN,RETURN_PERIOD, DERIVED_RET_PERIOD,ITM_HSNSAC,ITM_UQC,SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME,TAX_RATE,RecordType) ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_ERROR_CODE, GSTIN_ERROR_DESCRIPTION,TAX_RATE,RecordType  FROM( SELECT DERIVED_RET_PERIOD, ");
		bufferString.append(
				"SUPPLIER_GSTIN,RETURN_PERIOD,ITM_HSNSAC, ITM_UQC,ROWNUM_DT, SAVE_STATUS, GSTIN_REF_ID,");
		bufferString.append(
				"GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION,TAX_RATE,RecordType from( SELECT DERIVED_RET_PERIOD, ");
		bufferString.append(
		...
		bufferString.append(
					"GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION,TAX_RATE,RecordType FROM ( SELECT  ITM.ITM_TAX_DOC_TYPE AS TAX_DOC_TYPE,HDR.DERIVED_RET_PERIOD,");
		...
		bufferString.append(
				" FROM ANX_OUTWARD_DOC_HEADER HDR INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID LEFT OUTER JOIN ");
		bufferString.append(
				"FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID ");
	
		bufferString.append(
				"WHERE ASP_INVOICE_STATUS = 2 AND COMPLIANCE_APPLICABLE=TRUE AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1' ");
		bufferString.append(
				"AND (TABLE_SECTION NOT IN ('4A','5A','6B','6C','7A(1)','7B(1)','8A','8B','8C','8D','9B','4B','6A')  ");
		bufferString.append(" ) ");

		bufferString.append(buildHeader);
		bufferString.append(" AND GSTNBATCH.RETURN_TYPE='GSTR1' ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,NEW_RATE TAX_RATE,'B2C' AS RecordType FROM GSTR1_PROCESSED_B2CS HDR LEFT OUTER JOIN FILE_STATUS FIL ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,0.00 TAX_RATE,  ");
		
		bufferString.append(" CASE WHEN TABLE_SECTION IN ( '8A', '8B') THEN 'B2B' ");
		bufferString.append(" WHEN TABLE_SECTION IN ( '8C', '8D') THEN 'B2C'  ");
		bufferString.append(" END AS RecordType  ");
		
		bufferString.append(" FROM GSTR1_SUMMARY_NILEXTNON HDR LEFT OUTER JOIN ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_REF_ID, GSTIN_REF_ID_TIME, GSTIN_ERROR_CODE, GSTIN_ERROR_DESCRIPTION,TAX_RATE, RecordType ) ");
		bufferString.append(
				" GROUP BY SUPPLIER_GSTIN,RETURN_PERIOD, DERIVED_RET_PERIOD,ITM_HSNSAC,ITM_UQC,TAX_RATE,RecordType ) ) ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_REF_ID,GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION_ASP,TAX_RATE,RecordType ");
		bufferString.append(
				" FROM ( SELECT SUPPLIER_GSTIN,RETURN_PERIOD,DERIVED_RET_PERIOD,ITM_HSNSAC, ");
		bufferString.append(
		...
		bufferString.append(
				"SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE, GSTIN_ERROR_DESCRIPTION_ASP,TAX_RATE,RecordType ");
		bufferString.append(
		...
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,TAX_RATE, Record_Type AS  RecordType  FROM GSTR1_USERINPUT_HSNSAC HSN LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ");
		bufferString.append(
		...
		bufferString.append(
				"IS_SAVED_TO_GSTN,HSN.IS_DELETE , GSTN_SAVE_REF_ID,BATCH_DATE,ERROR_CODE,ERROR_DESC,TAX_RATE,Record_Type ) ");
		bufferString.append(
				" GROUP BY SUPPLIER_GSTIN,RETURN_PERIOD,DERIVED_RET_PERIOD, ITM_HSNSAC, ITM_UQC , ");
		bufferString.append(
				"SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION_ASP,TAX_RATE,RecordType )) ");
		bufferString.append(
				" GROUP BY SUPPLIER_GSTIN,RETURN_PERIOD, DERIVED_RET_PERIOD,ITM_HSNSAC,ITM_DESCRIPTION,(CASE WHEN LEFT(ITM_HSNSAC, 2) = '99' then 'NA' else ITM_UQC end),TAX_RATE,RecordType  ) ");
		bufferString.append(
				" GROUP BY SUPPLIER_GSTIN,RETURN_PERIOD, DERIVED_RET_PERIOD,ITM_HSNSAC,ITM_DESCRIPTION,ITM_UQC, ");
		bufferString.append(
				"SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE, GSTIN_ERROR_DESCRIPTION ,TAX_RATE,RecordType ");
	...
	private static String createHsnSummaryQueryStringForGstr1a(String buildQuery,
			String buildHeader, String buildheader, String supplyTypeQuery,
		...
		bufferString.append(
				"IFNULL(ITM.CESS_AMT_SPECIFIC,0)+IFNULL(ITM.CESS_AMT_ADVALOREM,0) AS CESS_AMT,");
		bufferString.append(
				"HDR.TAX_DOC_TYPE,HDR.TABLE_SECTION,ONB_LINE_ITEM_AMT ,ITM.OTHER_VALUES,TAX_RATE, ");
		
		bufferString.append(" CASE WHEN ITM.CUST_GSTIN IS NOT NULL AND LENGTH(ITM.CUST_GSTIN) = 15 THEN 'B2B' ");
		bufferString.append(" WHEN (LENGTH(ITM.CUST_GSTIN) < 15 OR ITM.CUST_GSTIN IS NULL)    THEN 'B2C' END AS RecordType ");
		
		bufferString.append(" FROM ANX_OUTWARD_DOC_HEADER_1A ");
		bufferString.append(
		...
		bufferString.append(
				" '7' TABLE_SECTION,IFNULL(NEW_TAXABLE_VALUE,0)+IFNULL(IGST_AMT,0)+ IFNULL(CGST_AMT,0)+IFNULL(SGST_AMT,0) ");
		bufferString.append(
				"+IFNULL(CESS_AMT,0) ONB_LINE_ITEM_AMT,0.00 OTHER_VALUES,NEW_RATE TAX_RATE,'B2C' AS RecordType  FROM GSTR1A_PROCESSED_B2CS ");
		bufferString.append(
				"HDR LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID ");
		bufferString.append(
				"LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID ");
		...
		bufferString.append("FROM GSTR1A_SUMMARY_NILEXTNON HDR ");
		bufferString.append("LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON ");
		bufferString.append("GSTNBATCH.ID = HDR.BATCH_ID WHERE HDR.IS_DELETE = FALSE "); 
		bufferString.append(supplyTypeQuery);
		bufferString.append(buildHeader);
		...
		bufferString.append(
				"ITM_UQC ORDER BY CREATED_ON DESC) AS ROWNUM_DT, SAVE_STATUS, GSTIN_REF_ID,GSTIN_REF_ID_TIME,");
		bufferString.append(
				"GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION,TAX_RATE,RecordType FROM( SELECT HDR.TAX_DOC_TYPE,HDR.DERIVED_RET_PERIOD,");
		bufferString.append(
				" HDR.SUPPLIER_GSTIN, HDR.CREATED_ON,HDR.RETURN_PERIOD,ITM_HSNSAC, IFNULL(ITM_UQC,'OTH') ITM_UQC, ");
		bufferString.append(
				"(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) SAVE_STATUS, ");
		...
		bufferString.append(
				"GSTIN_ERROR_CODE, (CASE WHEN GSTN_ERROR = TRUE AND HDR.IS_DELETE = FALSE THEN IFNULL( GSTNBATCH.ERROR_DESC,'') ELSE NULL END) ");
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,NEW_RATE TAX_RATE,'B2C' AS RecordType FROM GSTR1A_PROCESSED_B2CS HDR LEFT OUTER JOIN FILE_STATUS FIL ");
		bufferString.append(
				"ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID ");
		bufferString
				.append(" WHERE HDR.IS_DELETE = FALSE AND IS_AMENDMENT=FALSE ");
		...
		bufferString.append(" FROM GSTR1A_SUMMARY_NILEXTNON HDR LEFT OUTER JOIN ");
		bufferString.append(
				"FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON ");
		bufferString.append(
				"GSTNBATCH.ID = HDR.BATCH_ID WHERE HDR.IS_DELETE = FALSE ");
		...
		bufferString.append(
				"(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HSN.IS_DELETE = FALSE THEN IFNULL( GSTNBATCH.ERROR_DESC,'') ELSE NULL END) ");
		bufferString.append(
				"GSTIN_ERROR_DESCRIPTION_ASP,TAX_RATE,Record_Type AS  RecordType  FROM GSTR1A_USERINPUT_HSNSAC HSN LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ");
		bufferString.append(
				"ON GSTNBATCH.ID = HSN.BATCH_ID WHERE HSN.IS_DELETE=FALSE ");
		bufferString.append(buildQuery);
		bufferString.append(
