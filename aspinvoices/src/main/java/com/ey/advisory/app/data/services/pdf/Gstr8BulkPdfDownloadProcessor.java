 ...
 */
@Slf4j
@Component("Gstr8BulkPdfDownloadProcessor")
public class Gstr8BulkPdfDownloadProcessor implements TaskProcessor {
	...
	public void execute(Message message, AppExecContext context) {
		...
		try {
			...
			try {
				...
				request.setRetunPeriod(taxPeriod);
				if (LOGGER.isDebugEnabled()) {
					String msg = "Request ";
				...
				try {
					...
					for (String gstn : gstinList) {
						...
						String fileName = "";
						if (isDigigst) {
							fileName = String.format("GSTR8_%s_%s_DigiGST", gstn,
									taxPeriod);
						} else {
							fileName = String.format("GSTR8_%s_%s_GSTN", gstn, taxPeriod);
						}
						...
						if (LOGGER.isDebugEnabled()) {
							String msg = String.format(
									"inside service impl before calling to Gstr8BulkPdfDownloadProcessor this method    id : %d",
									id);
							LOGGER.debug(msg);
						}
						
						JasperPrint jprint = gstr8ReportImpl
								.generatePdfGstr8Report(request, gstn);
						if (LOGGER.isDebugEnabled()) {
							String msg = String.format(
									"inside service impl before  after calling to Gstr8BulkPdfDownloadProcessor this method ",
									id);
							LOGGER.debug(msg);
						}
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"inside Gstr7BulkPdfDownloadProcessor  before uploading file to the temp folder file path ");
						LOGGER.debug(msg);
					}
					...
					if (LOGGER.isDebugEnabled()) {
						String msg = String.format(
								"inside Gstr8BulkPdfDownloadProcessor  after uploading file to the temp folder file path ");
						LOGGER.debug(msg);
					}
					...
					String msg = "Error while creating Gstr8 Summary bulk report";
					LOGGER.error(msg, ex);
			...
			String msg = String.format(
					"Exception occured during report generation for gstr7");
			LOGGER.error(msg, ex);
	...
	private String zipEinvoicePdfFiles(File tempDir, Long id) throws Exception {
		...
		String fileName = "Gstr8EntityLevelSummaryPDFReport_" + id;
