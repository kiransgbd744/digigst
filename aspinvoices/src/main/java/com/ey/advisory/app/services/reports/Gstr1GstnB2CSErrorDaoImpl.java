...
import com.ey.advisory.app.data.views.client.Gstr1GstnB2CSErrorDto;
...
@Component("Gstr1GstnB2CSErrorDaoImpl")
public class Gstr1GstnB2CSErrorDaoImpl implements Gstr1GstnErrorDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1GstnB2CSErrorDaoImpl.class);
...
String queryStr = null;
		...
		{
			queryStr = creategstnGstr1AB2CSErrorQueryString(buildQuery.toString());
		}
		else
		{
		queryStr = creategstnB2CSErrorQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertgstnB2CSError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1GstnB2CSErrorDto convertgstnB2CSError(Object[] arr) {
		Gstr1GstnB2CSErrorDto obj = new Gstr1GstnB2CSErrorDto();
		...
		obj.setEcommerceGSTIN(arr[9] != null ? arr[9].toString() : null);
		obj.setErrorCode(arr[10] != null ? arr[10].toString() : null);
		obj.setGstnErrorMessage(arr[11] != null ? arr[11].toString() : null);
		obj.setRefId(arr[12] != null ? arr[12].toString() : null);

		return obj;
	}

	private String creategstnB2CSErrorQueryString(String buildQuery) {

		return "SELECT HDR.POS PLACE_OF_SUPPLUY,HDR.SUPPLY_TYPE,"
				+ "HDR.ECOM_TYPE TYPE,HDR.DIFF_PERCENT,"
				+ "ITM.TAXABLE_VALUE,ITM.TAX_RATE,ITM.IGST_AMT,"
				+ "ITM.CGST_AMT,ITM.SGST_AMT,(ITM.CESS_AMT_SPECIFIC"
				+ " +ITM.CESS_AMT_ADVALOREM) CESS_AMT,"
				+ "HDR.ECOM_GSTIN E_COM_GSTIN,TRIM(', ' FROM "
				...
				+ "AND HDR.GSTN_ERROR=TRUE AND HDR.TAX_DOC_TYPE IN ('B2CS') "
				+ "AND HDR.IS_DELETE=FALSE "
				+ "AND GSTN_SAVE_STATUS='PE' " + buildQuery;
	}
	
	private String creategstnGstr1AB2CSErrorQueryString(String buildQuery) {
