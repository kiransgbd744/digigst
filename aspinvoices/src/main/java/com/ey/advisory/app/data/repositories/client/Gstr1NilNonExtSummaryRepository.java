 ...
 *
 */
@Repository("Gstr1NilNonExtSummaryRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr1NilNonExtSummaryRepository
		extends JpaRepository<Gstr1NilNonExmptSummaryEntity, Long>,
		JpaSpecificationExecutor<Gstr1NilNonExmptSummaryEntity> {

	@Modifying
	@Query("UPDATE Gstr1NilNonExmptSummaryEntity b SET b.isDelete= TRUE "
			+ "WHERE b.isDelete= FALSE AND  b.nKey IN (:entityKey) ")
	public void UpdateSameInvKey(@Param("entityKey") List<String> entityKey);

	@Query("SELECT doc FROM Gstr1NilNonExmptSummaryEntity doc "
			+ "WHERE doc.fileId = :id  ")
	public List<Gstr1NilNonExmptSummaryEntity> findByFileId(
			@Param("id") Long id);
	...
	public List<Gstr1NilNonExmptSummaryEntity> findByProcessId(
			@Param("id") Long id);

	@Modifying
	@Query("UPDATE Gstr1NilNonExmptSummaryEntity doc SET doc.isGstnError=true, "
			+ "doc.isSaved=false WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIserrorfalg(@Param("gstnBatchId") Long gstnBatchId);

	@Modifying
	@Query("UPDATE Gstr1NilNonExmptSummaryEntity doc SET doc.isSaved=true, "
			+ "doc.isGstnError =false WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIsSavedToGstn(@Param("gstnBatchId") Long gstnBatchId);

	@Modifying
	@Query("UPDATE Gstr1NilNonExmptSummaryEntity doc SET doc.gstnBatchId=:gstnBatchId,"
	...
	@Query("SELECT COUNT(*) FROM Gstr1NilNonExmptSummaryEntity  WHERE  "
