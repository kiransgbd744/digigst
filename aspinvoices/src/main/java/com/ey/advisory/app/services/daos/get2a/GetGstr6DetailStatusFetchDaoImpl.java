...
@Component("GetGstr6DetailStatusFetchDaoImpl")
public class GetGstr6DetailStatusFetchDaoImpl
		implements GetGstr6DetailStatusFetchDao {
	...
	public List<Object[]> getDataUploadedStatusDetails(String gstin,
		...
		buffer.append(
				"SELECT * FROM ( SELECT * FROM (SELECT GSTIN,RETURN_PERIOD,DENSE_RANK() "
						...
						+ "API_SECTION='GSTR6') A WHERE NUM = 1 AND GSTIN = :gstin");
		if (StringUtils.isNotBlank(taxPeriod)) {
			buffer.append(" AND RETURN_PERIOD"
					+ " = :taxPeriod");
		}
		buffer.append(" UNION "
				...
				+ " WHERE STATUS = 'SUCCESS' AND API_SECTION='GSTR6')"
