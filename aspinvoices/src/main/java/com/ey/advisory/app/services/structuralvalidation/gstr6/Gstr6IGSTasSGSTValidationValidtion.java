 ...
 *
 */
public class Gstr6IGSTasSGSTValidationValidtion implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx,Object obj, Object[] row,
		...
		if (!isPresent(obj)) return errors;
			...
			if (!isDecimal(obj.toString().trim())) {
				...
				errors.add(new ProcessingResult(APP_VALIDATION, "ER3033",
						"Invalid IGSTasSGST", location));
			...
			/*if (!isPresent(row[6])) return errors;
			if(GSTConstants.CR.equalsIgnoreCase(row[6].toString().trim())) return errors;
			...
			if (result.compareTo(BigDecimal.ZERO) < 0) {
				...
				errors.add(new ProcessingResult(APP_VALIDATION, "ER3034",
