...
@Slf4j
@Component("Gstr1AdvRecSectionReportDaoImpl")
public class Gstr1AdvRecSectionReportDaoImpl
	...
	public Pair<List<Object[]>, List<Object>> getGstr1RSReports(
		...
		String queryStr = createAdvRecSavableTransactionalQueryString(
		...
		List<Object> collect = list.stream().map(o -> convertB2CSSummary(o))
	...
	private String createAdvRecSavableTransactionalQueryString(
		...
		return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, "
				...
				+ "'' AS INFO_ERROR_DESCRIPTION_ASP, case when HDR.IRN_STATUS = 1 "
				+ "then 'Not Opted' when HDR.IRN_STATUS = 2 then 'Not Applicable' "
				+ "when HDR.IRN_STATUS = 3 then 'Pending (No Action)' when HDR.IRN_STATUS = 4 "
				+ "then 'Generation Error' when HDR.IRN_STATUS = 5 then "
				+ " 'Generated' when HDR.IRN_STATUS = 6 then 'Cancelled' "
				+ "when HDR.IRN_STATUS = 7 then 'ASP Error' "
				+ "when HDR.IRN_STATUS = 8 then 'Duplicate_Irn' "
				+ "when HDR.IRN_STATUS = 11 then 'Pending (Error)' "
				+ "when HDR.IRN_STATUS = 10 then 'Pushed to NIC' end "
				+ "as IRN_STATUS, HDR.IRN_RESPONSE AS IRN_NO, "
				+ "HDR.ACK_NUM AS IRN_ACK_NO, "
				+ "TO_CHAR(HDR.ACK_DATE,'DD-MM-YYYY HH24:MI:SS') "
				+ "AS IRN_ACK_DATE_TIME, EINV.SIGNED_QR AS SIGNED_QR_CODE, "
				+ "NULL AS SIGNED_INVOICE, HDR.EINV_ERROR_CODE AS IRP_ERROR_CODE, "
				+ "HDR.EINV_ERROR_DESC AS IRP_ERROR_DESCRIPTION, "
				+ "case when HDR.EWB_STATUS = 1 then 'Not Applicable' "
				+ "when HDR.EWB_STATUS = 2 then 'Pending (No Action)' "
				...
				+ "WHEN HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = FALSE "
				+ "THEN 'NOT_SAVED' END AS GSTIN_STATUS, "
				+ "GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, "
				+ "GSTNBATCH.BATCH_DATE AS GSTIN_REFID_DATE_TIME, "
				+ "NULL AS GSTIN_ERROR_CODE, NULL AS GSTIN_ERROR_DESCRIPTION_ASP, "
				+ "HDR.RETURN_TYPE AS RET_TYPE, HDR.TABLE_SECTION "
				+ "AS TABLE_NUMBER, HDR.IRN, HDR.IRN_DATE,HDR.TAX_SCHEME, "
				+ "HDR.CANCEL_REASON, HDR.CANCEL_REMARKS, "
				+ "ITM.SUPPLY_TYPE, HDR.DOC_CATEGORY, HDR.DOC_TYPE,"
				+ "HDR.DOC_NUM,HDR.DOC_DATE, HDR.REVERSE_CHARGE, "
				...
				+ "HDR.SHIP_TO_GSTIN ,HDR.SHIP_TO_TRADE_NAME, "
				...
				+ "ITM.ORIGIN_COUNTRY,ITM.ITM_UQC_USER,ITM.QTY_USER, "
				...
				+ "THEN ITM.LINE_ITEM_AMT END LINE_ITEM_AMT, "
				...
				+ "ITM.PRODUCT_CODE,ITM.ITM_TYPE, ITM.ITC_FLAG,"
				+ "HDR.STATE_APPLYING_CESS,ITM.FOB, ITM.EXPORT_DUTY,"
				+ "HDR.EXCHANGE_RATE,ITM.CRDR_REASON, HDR.TCS_FLAG ,"
				+ "ITM.TCS_AMT,TCS_CGST_AMT,TCS_SGST_AMT, "
				...
				+ "ITM.PLANT_CODE,HDR.DIVISION, SUB_DIVISION,"
				...
				+ "HDR.USERACCESS5,HDR.USERACCESS6, "
				...
				+ "FIL.FILE_NAME as FILE_NAME,HDR.INV_OTHER_CHARGES "
				...
				+ "as InvoiceCessSpecificAmountASP, HDR.INV_STATE_CESS_AMT "
				...
				+ "CASE WHEN HDR.IS_DELETE = TRUE THEN 'INACTIVE' "
				+ "WHEN HDR.IS_DELETE = FALSE THEN 'ACTIVE'END "
				+ "AS RECORD_STATUS, "
				...
				+ "AS EWB_CANCELLATION_DATE, CASE WHEN HDR.ASP_INVOICE_STATUS=1 "
				...
				+ "ON HDR.EWB_NO_RESP = EWB.EWB_NUM WHERE "
				+ "HDR.ASP_INVOICE_STATUS = 2 "
				+ "AND COMPLIANCE_APPLICABLE=TRUE " + buildQueryheader
				+ "AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1' AND HDR.TAX_DOC_TYPE IN ('AT') ";
