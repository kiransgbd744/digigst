...
import com.ey.advisory.app.services.gstr1fileupload.Gstr9InOutwardStructuralProcess;
...
@Service("Gstr9InOutwardFileArrivalHandler")
@Slf4j
public class Gstr9InOutwardFileArrivalHandler {
	@Autowired
	...
	@Autowired
	@Qualifier("Gstr9InOutwardStructuralProcess")
	private Gstr9InOutwardStructuralProcess gstr9InOutwardStructuralProcess;

	@Autowired
	@Qualifier("Gstr9InOutwardHeaderCheckService")
	private Gstr1HeaderCheckService gstr9InOutHeaderCheckService;
	...
	public void processTdsTcsFile(Message message, AppExecContext context) {
		...
		try {
			...
			LOGGER.error(
					"Exception occured in Gstr9 inward and outward File Arrival Processor",
					e);
		}
		...
		Pair<Boolean, String> checkHeaderFormat = gstr9InOutHeaderCheckService
				.validate(getHeaders);

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Header Checker Validations Gstr9 Inward and Outward"
					+ checkHeaderFormat.getValue0() + " ,  :"
					+ checkHeaderFormat.getValue1());
		}
		List<Object[]> listOfGstr9InOutward = ((FileUploadDocRowHandler<?>) rowHandler)
				.getFileUploadList();
		...
		if (checkHeaderFormat != null && checkHeaderFormat.getValue0()) {
			gstr9InOutwardStructuralProcess.gstr9InOutwardStructureProcessData(
					listOfGstr9InOutward, updateFileStatus);
		} else {
