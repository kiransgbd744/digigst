...
@Slf4j
@Component("Gstr1AsUploadedTnxSplitReportCommonService")
public class Gstr1AsUploadedTnxSplitReportCommonService {
	...
	private static final String GSTR1_CHUNK_ROWS = "gstr1.trnx.report.chunk.rows";
	...
	public Integer getChunkingSizes() {
		...
		String chunkSize = config != null ? config.getValue() !=null?config.getValue(): "500000":"500000";
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info(
					"Gstr1AsUploadedTnxSplitReportCommonService Directory is empty");
			return;
		}
		...
		try {
			...
			for (Pair<Long, List<File>> rptChunks : reportIds) {
				...
				if(reportType.startsWith("GSTR3B Table4 Transactional")){
				...
				reportTypeRepo.updateFilePath(uploadedDocName.getValue0(),
						rptChunks.getValue0(),
	...
	private String getIndexNumber(String fileName) {
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format("getIndexNumber File Name is %s :",
					fileName);
			LOGGER.debug(msg);
		}
		String index = "1";
		...
		if (fileName.startsWith("ReviewSummary_asUploaded_")) {
			index = name[2];
		} else if(fileName.startsWith("ProcessedSummary_asUploaded_")) {
			index = name[2];
		}else if (fileName.startsWith("GSTR3B_Table4_Transactional_Report_")) {
			index = name[5];
		}
		return index;
