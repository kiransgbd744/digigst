...
@Service("Anx1TotalRecReportsServiceImpl")
public class Anx1TotalRecReportsServiceImpl
		implements Anx1TotalRecReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1TotalRecReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1TotalRecReportsDaoImpl")
	private Anx1TotalRecReportsDao anx1TotalRecReportsDao;

	@Override
	public Workbook findTotalRecords(SearchCriteria criteria,
			PageRequest pageReq) {
		...
		List<Anx1TotalRecordsView> responseFromView = new ArrayList<>();
		responseFromView = anx1TotalRecReportsDao.getTotalRecReports(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.totrec.report.headers").split(",");
			
			LOGGER.debug("workbook properties reading starting ");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_TotalRecReport.xlsx");
