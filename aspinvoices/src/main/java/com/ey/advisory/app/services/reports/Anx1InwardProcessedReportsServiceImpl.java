...
@Service("Anx1InwardProcessedReportsServiceImpl")
public class Anx1InwardProcessedReportsServiceImpl 
          implements Anx1InwardProcessedReportsService {
	
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1InwardProcessedReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;
	
	@Autowired
	@Qualifier("Anx1InwardProcessedReportsDaoImpl")
	private Anx1InwardProcessedReportsDao anx1InwardProcessedReportsDao;
	...
	public Workbook findInwardApiProcesseRec(SearchCriteria criteria,
		...
		List<Anx1InwardProcessedRecordsDto> responseFromView = new ArrayList<>();
		responseFromView = anx1InwardProcessedReportsDao.getInwardProcessedReports(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.inward.processed.report.headers").split(",");
			LOGGER.debug("workbook properties reading starting ");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_InwardProcessedRecReport.xlsx");
