 ...
 *
 */
@Component("PrecedingDocDateSame")
public class PrecedingDocDateSame implements DocRulesValidator<OutwardTransDocument> {
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			if (!isValidTax(item, items.get(idx))) {
				...
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0043",
						"Preceeding Invoice Date should be same "
