...
import com.ey.advisory.app.data.gstr1A.repositories.client.Gstr1ASaveHsnPEResponseRepository;
...
@Slf4j
@Service("Gstr1AHsnResponseHandler")
public class Gstr1AHsnResponseHandler implements Gstr1AResponseHandler {

	@Autowired
	private Gstr1ASaveHsnPEResponseRepository hsnRepo;
	...
	public Integer SaveResponse(JsonElement root, Gstr1SaveBatchEntity batch,
		...
		n = temp.size();
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("GSTR1A SAVE HSNSUM PE error invoices count is {} ", n);
		}
	...
	{
		LOGGER.debug("GSTR1A SAVE HSNSUM PE error invoices SaveAll is Completed");
	}return n;
	 ...
	 * @return
	 */
	private List<Gstr1ASaveHsnPEResponseEntity> addSaveHsnPeEntity(
		...
		if (docDataList != null && !docDataList.isEmpty()) {
			for (HSNSummaryInvData docData : docDataList) {
				Gstr1ASaveHsnPEResponseEntity entity = new Gstr1ASaveHsnPEResponseEntity();
