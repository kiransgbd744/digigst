...
@Slf4j
@Component("DRC01CSignAndFileServiceImpl")
public class DRC01CSignAndFileServiceImpl implements DRC01CSignAndFileService {

	@Autowired
	private Drc01cGstnServiceImpl drcGstnGetCall;
	...
	public Pair<Boolean, String> getDrc01CGstnSummary(
		...
		try {
			...
			String msg = "Exception in DRC01CSignAndFileServiceImpl while "
					+ "DRC01C  api Call";
			LOGGER.error(msg, ex);
	...
	public void saveJsonAsRecords(String apiResp, Gstr3bGetInvoicesReqDto dto,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("DRC01C Summary GSTN GET call Json is {} ", apiResp);
		}
	...
	public void updateDrc01CTables(String taxperiod, String gstin,
		...
		try {
			...
			if (i == 0) {
				...
				returnStatusentity.setReturnType(APIIdentifiers.DRC01C);
			...
			String msg = "Exception occured in  updateDrc01CTables";
			LOGGER.error(msg, e);
	...
	private void saveOrUpdateGstnUserRequest(String gstin, String taxPeriod,
		...
		try {
			...
			if (gstnUserRequestEntity == null) {
				...
				gstnUserRequestEntity.setReturnType(APIIdentifiers.DRC01C);
				...
				gstnUserRequestRepo.updateGstnResponse(reqClob, 1, gstin,
						taxPeriod, APIIdentifiers.DRC01C, LocalDateTime.now());

			}
		} catch (Exception e) {
			String msg = "Exception occured in DRC01CSignAndFileServiceImpg "
		...
		try {
			...
			List<SignAndFileEntity> entityList = signAndFileRepo
					.findByGstinAndTaxPeriodAndReturnTypeOrderByCreatedOnDesc(
							gstin, taxPeriod, APIIdentifiers.DRC01C);
			...
			String msg = "Exception occured while getting DRC01C data";
			LOGGER.error(msg, e);
		...
		try {
			...
			if (opEntity.isPresent()) {
				...
				String errMsg = String.format(
						"DRC01C Sign & File is not initiated for id %s", id);
				LOGGER.error(errMsg);
				throw new AppException(errMsg);
			}
		} catch (Exception e) {
			String msg = "Exception occured while getting DRC01C Filing data";
			LOGGER.error(msg, e);
	...
	private void writeMsgToTxt(SignAndFileEntity entity,
		...
		try {
			...
			String fileName = "DRC01CFilingErrors_" + entity.getGstin() + "_"
					+ entity.getTaxPeriod();
				...
				try {
					...
					String msg = String.format(
							"Failed to remove the temp "
									+ "directory created for DRC01C errors:"
									+ " '%s'. This will "
	...
	public static File createTempDir() throws IOException {
		return Files.createTempDirectory("DRC01CErrors").toFile();
	}
