...
@Service("Anx1InwardProcessedInfoReportsServiceImpl")
public class Anx1InwardProcessedInfoReportsServiceImpl
		implements Anx1InwardProcessedInfoReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1InwardProcessedInfoReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1InwardProcessedInfoReportsDaoImpl")
	private Anx1InwardProcessedInfoReportsDao anx1InwardProcessedInfoReportsDao;
	...
	public Workbook findInwardApiProcesseInfoRec(SearchCriteria criteria,
		...
		List<Anx1InwardProcessedInfoRecordsDto> responseFromView = new ArrayList<>();
		responseFromView = anx1InwardProcessedInfoReportsDao
				.getInwardProcessedInfoReports(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.inward.processedinforec.report.headers")
					.split(",");
			LOGGER.debug("workbook properties reading starting ");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_InwardProcessedInfoRecReport.xlsx");
