 ...
 *
 */
@Component("gstr1B2bB2baDataAtGstnImpl")
@Slf4j
public class Gstr1B2bB2baDataAtGstnImpl implements Gstr1B2bB2baDataAtGstn {
	...
	public Long findB2bB2baDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.B2B.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_B2BA);

			}
		}
		
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1B2bB2baSuccessHandler", "Gstr1GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
