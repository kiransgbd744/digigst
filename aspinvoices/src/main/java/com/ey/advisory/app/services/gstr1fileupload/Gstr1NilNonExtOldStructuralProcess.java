 ...
 *
 */
@Component("Gstr1NilNonExtOldStructuralProcess")
@Slf4j
public class Gstr1NilNonExtOldStructuralProcess {

	@Autowired
	@Qualifier("Gstr1NilNonExmtBusinessProcessOld")
	private Gstr1NilNonExmtBusinessProcessOld gstr1NilNonExmtBusinessProcess;
	...
	public void nilNonExtStructureProcessData(List<Object[]> listOfNil,
		...
		List<Gstr1NilNonExemptedAsEnteredEntity> excelData = sRFileToNilDetailsConvertion
				.convertSRFileToNilExcelDocOld(listOfNil, updateFileStatus);
		...
		Map<String, List<ProcessingResult>> processingResults = nilNonExptStructValidationChain
				.validationOld(listOfNil, excelDataSave);
		...
		if (strErrorKeys.size() > 0 && !strErrorKeys.isEmpty()) {
			...
			gstr1NilNonExtAsRepository.invUpdateErrorInfo(strErrorKeys, ids);
