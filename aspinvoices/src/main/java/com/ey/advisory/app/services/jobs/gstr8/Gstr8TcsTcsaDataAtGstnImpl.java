 ...
 *
 */
@Component("Gstr8TcsTcsaDataAtGstnImpl")
@Slf4j
public class Gstr8TcsTcsaDataAtGstnImpl implements Gstr8TcsTcsaDataAtGstn {
	...
	public Long findTcsTcsaDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.TCS.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR8_GET_TCS);
			}
		}
		
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr8TcsTcsaSuccessHandler", "Gstr8GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
