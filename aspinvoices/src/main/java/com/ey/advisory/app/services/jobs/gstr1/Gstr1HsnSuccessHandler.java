...
@Service("Gstr1HsnSuccessHandler")
@Slf4j
public class Gstr1HsnSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("gstr1HSNSummaryDataParserImpl")
	private Gstr1HSNSummaryDataParser gstr1HSNSummaryDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				gstr1GetHSNOrSacGstnRepository.softlyDeleteHsnHeader(
						dto.getGstin(), dto.getReturnPeriod());
			}
			resultIds.forEach(id -> {
				String apiResp = APIInvokerUtil.getResultById(id);
				// Long batchId = null;
				List<GetGstr1HSNOrSACInvoicesEntity> hsnEnties = gstr1HSNSummaryDataParser
						.gstr1HSNSummaryDataParser(dto, apiResp);
				if (!hsnEnties.isEmpty()) {
					
					gstr1GetHSNOrSacGstnRepository.saveAll(hsnEnties);
				}
			});
