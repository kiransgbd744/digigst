...
@Service("SEZWEYFinalStructure")
public class SEZWEYFinalStructure {


	public List<Gstr1SummaryRespDto> getSEZWEyList(
			List<Gstr1SummaryRespDto> sezwEYList,
		...
		if (view6BFiltered != null & view6BFiltered.size() > 0) {
			...
			List<Gstr1SummaryRespDto> default6BFiltered = sezwEYList.stream()
					.filter(p -> p.getTableSection().equalsIgnoreCase("6B"))
					.collect(Collectors.toList());
			...
			view6BFiltered.forEach(view6B -> {
				...
				sezwEYList.add(summaryRespDto);
			});
		}
	
		// Sort the list in Ascending Order
		Collections.sort(sezwEYList, new Comparator<Gstr1SummaryRespDto>() {
			@Override
