...
@Component("Gstr1aAspNilRatedTransactionalDaoImpl")
public class Gstr1aAspNilRatedTransactionalDaoImpl
		implements Gstr1OutwardVerticalProcessNilDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1aAspNilRatedTransactionalDaoImpl.class);
	...
	private static String createApiProcessedQueryString(String buildQueryheader) {
		...
		return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, "
				...
				+ "when HDR.IRN_STATUS = 6 then 'Cancelled' when HDR.IRN_STATUS = 7 then 'DigiGST Error' "
				...
				+ "then 'DigiGST Error' WHEN HDR.EWB_STATUS = 12 then 'Not_Opted' end as EWBPartAStatusA, "
		 ...
		 * "-- AND HDR.SUPPLIER_GSTIN='33GSPTN0481G1ZA' AND HDR.DERIVED_RET_PERIOD=202001 "
		 * + "-- AND HDR.DOC_DATE BETWEEN '2017-09-01' AND '2020-10-30' " +
		 * "-- AND IRN_RESPONSE IS NULL AND EINV_STATUS IN (5) " +
		 * "-- AND IRN_RESPONSE IS NOT NULL AND EINV_STATUS IN (7,10,11) " +
		 * "-- AND HDR.DOC_TYPE IN ('INV','CR','DR','RNV','RCR','RDR','BOS') " +
