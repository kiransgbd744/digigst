...
@Component("Anx1ASPHsnOutwardSavableDaoImpl")
public class Anx1ASPHsnOutwardSavableDaoImpl
		implements Anx1AspHsnOutwardSavableSummaryDao {
	...
	@Override
	public List<Object> getAnx1HsnOutwardSavableReports(
		...
		String queryStr = createProcessedOutwardQueryString(
				buildQuery.toString());
	...
	private String createProcessedOutwardQueryString(String buildQuery) {

		return "SELECT HDR.USER_ID,HDR.SOURCE_FILENAME,"
				+ " ITM.PROFIT_CENTRE AS ITM_PROFIT_CENTRE, ITM.PLANT_CODE AS"
				+ " ITM_PLANT_CODE,HDR.DIVISION, ITM.LOCATION AS ITM_LOCATION,"
				...
				+ "ITM.GLCODE_STATE_CESS, HDR.RETURN_PERIOD,HDR.SUPPLIER_GSTIN,"
				+ "HDR.DOC_TYPE, HDR.SUPPLY_TYPE,HDR.DOC_NUM,HDR.DOC_DATE,"
				+ "ITM.ORIGINAL_DOC_TYPE, ITM.ORIGINAL_DOC_NUM AS"
				+ " ITM_ORIGINAL_DOC_NUM, ITM.ORIGINAL_DOC_DATE AS"
				+ " ITM_ORIGINAL_DOC_DATE, HDR.CRDR_PRE_GST, ITM.ITM_NO,"
				+ " HDR.CUST_GSTIN, HDR.CUST_SUPP_TYPE,HDR.DIFF_PERCENT,"
				+ "HDR.ORIGINAL_CUST_GSTIN, HDR.CUST_SUPP_NAME,"
				...
				+ " HDR.SHIP_BILL_NUM,HDR.SHIP_BILL_DATE,ITM.FOB,"
				+ " ITM.EXPORT_DUTY, '' AS ITM_HSNSAC,ITM.PRODUCT_CODE,"
				+ " ITM.ITM_DESCRIPTION,ITM.ITM_TYPE,ITM.ITM_UQC,ITM.ITM_QTY,"
				+ " HDR.SECTION7_OF_IGST_FLAG,SUM(ITM.TAXABLE_VALUE),"
				+ "ITM.IGST_RATE, SUM(ITM.IGST_AMT),ITM.CGST_RATE,"
				+ "SUM(ITM.CGST_AMT),ITM.SGST_RATE, SUM(ITM.SGST_AMT),"
				+ "ITM.CESS_RATE_ADVALOREM,SUM(ITM.CESS_AMT_ADVALOREM), "
				+ "ITM.CESS_RATE_SPECIFIC, SUM(ITM.CESS_AMT_SPECIFIC),"
				+ " ITM.STATECESS_RATE,SUM(ITM.STATECESS_AMT),SUM(ITM.OTHER_VALUES), "
				+ "SUM(ITM.LINE_ITEM_AMT) AS ITM_INV_VALUE,ITM.ADJ_REF_NO,"
				+ " ITM.ADJ_REF_DATE, SUM(ITM.ADJ_TAXABLE_VALUE),"
				...
				+ "SUM(ITM.TCS_AMT),ITM.ITC_FLAG, HDR.CLAIM_REFUND_FLAG,"
				...
				+ "SUM(ITM.MEMO_VALUE_IGST), SUM(ITM.MEMO_VALUE_CGST),"
				+ " SUM(ITM.MEMO_VALUE_SGST), SUM(ITM.IGST_RET1_IMPACT),"
				+ " SUM(ITM.CGST_RET1_IMPACT), SUM(ITM.SGST_RET1_IMPACT), "
				...
				+ " GSTIN_REF_ID_TIME, TRIM(', ' FROM IFNULL"
				+ "(ERRH.ERROR_CODE_GSTIN,'') ||','|| IFNULL"
				+ "(ERRI.ERROR_CODE_GSTN,'')) AS GSTIN_ERROR_CODE,"
				...
				+ " AS ASP_DATE_TIME FROM ANX_OUTWARD_DOC_HEADER HDR LEFT "
				+ "OUTER JOIN TF_OUTWARD_HEADER_GSTIN_ASP_ERROR_INFO ()ERRH "
				+ "ON HDR.ID = ERRH.DOC_HEADER_ID INNER JOIN"
				+ " ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID LEFT"
				+ " OUTER JOIN TF_OUTWARD_ITEM_GSTIN_ASP_ERROR_INFO ()ERRI ON"
				+ " ITM.DOC_HEADER_ID = ERRI.DOC_HEADER_ID AND ITM.ITEM_INDEX"
				+ " = ERRI.ITEM_INDEX AND ITM.ITM_NO = ERRI.ITM_NO INNER JOIN"
				+ " FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID = HDR.BATCH_ID WHERE "
				+ "IS_ERROR = FALSE AND AN_RETURN_TYPE = 'ANX1'" + buildQuery
				+ " GROUP BY HDR.ID ,HDR.USER_ID, HDR.AN_RETURN_TYPE,"
				+ "HDR.AN_TAX_DOC_TYPE, HDR.AN_TABLE_SECTION, HDR.SOURCE_FILENAME,"
				+ "ITM.TAX_RATE,HDR.DIVISION, HDR.SALES_ORGANIZATION,"
				...
				+ "HDR.DOC_TYPE, HDR.SUPPLY_TYPE,HDR.DOC_NUM,HDR.DOC_DATE,"
				+ "ITM.ORIGINAL_DOC_TYPE, HDR.CRDR_PRE_GST,HDR.CUST_GSTIN,"
				...
				+ "ITM.FOB,ITM.EXPORT_DUTY, HDR.DERIVED_RET_PERIOD,"
				+ "ITM.DERIVED_RET_PERIOD, ITM.USERDEFINED_FIELD1,"
				...
				+ "ITM.LINE_ITEM_AMT,ITM.PROFIT_CENTRE,ITM.PLANT_CODE,"
				+ " ITM.LOCATION,GSB.GSTN_SAVE_REF_ID,GSB.GSTN_RESP_DATE,"
				+ " FIL.SOURCE,FIL.CREATED_BY,FIL.FILE_NAME,ERRH.INFO_ERROR_CODE_ASP,"
				+ " ERRH.INFO_ERROR_DESCRIPTION_ASP,ERRI.INFO_ERROR_CODE_ASP,"
				+ " ERRI.INFO_ERROR_DESCRIPTION_ASP,ERRH.ITM_NO,"
				+ "ERRH.ERROR_CODE_GSTIN, ERRI.ERROR_CODE_GSTN,"
				+ "ERRI.ERROR_DESCRIPTION_GSTN, ERRH.ERROR_DESCRIPTION_GSTIN,"
				+ "GSB.GSTN_SAVE_REF_ID ,ERRH.CREATED_ON, ITM.IGST_RATE,ITM.CGST_RATE,"
				+ "ITM.SGST_RATE, ITM.CESS_RATE_SPECIFIC,ITM.CESS_RATE_ADVALOREM,"
				+ " ITM.STATECESS_RATE,HDR.CREATED_ON,HDR.IS_SAVED_TO_GSTN,"
				+ " HDR.GSTN_ERROR ORDER BY DOC_NUM,ITM_NO ";
