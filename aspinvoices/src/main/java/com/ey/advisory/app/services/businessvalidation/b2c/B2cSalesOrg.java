...
public class B2cSalesOrg implements 
	...
	public List<ProcessingResult> validate(OutwardB2cExcelEntity document,
		...
		Map<EntityAtConfigKey, Map<Long, String>> entAtConfMap = document
				...
				String so = document.getSalesOrganisation();
						...
						if (!isAttrValid) {
								...
								TransDocProcessingResultLoc location 
								      = new TransDocProcessingResultLoc(
										null, errorLocations.toArray());
								errors.add(new ProcessingResult(APP_VALIDATION,
										ProcessingResultType.INFO, "IN0205",
										"Sales Organisation is not as per "
										+ "master provided during On-Boarding.",
										location));
		...
		if (so != null && !so.isEmpty()) {
			...
			if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
				...
				boolean isAttrUserValid = onboardAttributesChecking.isAttrValid(
						OnboardingConstant.PC, so, document.getEntityId()
				...
				if (!isAttrUserValid) {
					...
					TransDocProcessingResultLoc location = 
							new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER0265",
