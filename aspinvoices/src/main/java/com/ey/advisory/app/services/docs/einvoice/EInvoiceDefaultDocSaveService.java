...
import com.ey.advisory.app.services.validation.eInvoice.EinvoiceSalesDocRulesValidatorService;
 ...
 *
 */
@Service("EInvoiceDefaultDocSaveService")
public class EInvoiceDefaultDocSaveService implements EInvoiceDocSaveService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(EInvoiceDefaultDocSaveService.class);
	private static final String CLASS_NAME = "EInvoiceDefaultDocSaveService";

	public static final String CONFI_KEY = "b2cs.threshold.value";

	public static final String MULTI_GROUP_LEVEL = "isMultiSupplyOpted";
	...
	@Autowired
	@Qualifier("EinvoiceSalesDocRulesValidatorService")
	private EinvoiceSalesDocRulesValidatorService salesDocRulesValSvc;
	...
	@Autowired
	@Qualifier("DefaultOutwardLineTransDocBifurcator")
	private DocBifurcator<OutwardTransDocument> defaultLineBifurcator;
	...
	@Autowired
	@Qualifier("EInvoiceOutwardDocSaveResp")
	private EInvoiceOutwardDocSaveResp outwardDocSaveResp;
	...
	@Autowired
	@Qualifier("EInvoiceOutwardDocSave")
	private EInvoiceOutwardDocSave outwardDocSave;

	@Autowired
	private EinvoiceDocSaveUtility docSaveUtility;
	...
	public EInvoiceOutwardDocSaveRespDto saveDocuments(
		...
		try {
			...
			/*
			 * Map<String, List<ProcessingResult>> isFreezeprocessingResults =
			 * isGstr1FreezeCheckBasedOnOnboarding( documents, context);
			 */

			Map<String, List<String>> isFreezeMap = isGstr1MapFreezeCheckBasedOnOnboarding(
					documents, context);

			context.seAttribute("freezeMap", isFreezeMap);
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						...
						groupCode);
				/*
				 * if (isFreezeprocessingResults != null) LOGGER.debug(
				 * "The size of the rule error list map is : {}" +
				 * " for group code : {}", isFreezeprocessingResults.size(),
				 * groupCode);
				 */
			}
			...
			/*
			 * isFreezeprocessingResults.forEach( (key, value) ->
			...
			for (OutwardTransDocument savedoc : savedDocs) {
			...
			PerfUtil.logEventToFile(PerfamanceEventConstants.FILE_PROCESSING,
					...
					null);
			List<OutwardTransDocument> updateSave = new ArrayList<>();

			if (!processingResults.isEmpty()) {
				for (OutwardTransDocument doc : savedDocs) {
			...
			if (!updateSave.isEmpty()) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("updateSave {}  ", updateSave);
				}
			...
			LOGGER.debug(" outError {} ");
			PerfUtil.logEventToFile(PerfamanceEventConstants.FILE_PROCESSING,
					"DOC_ERROR_SAVE_START", CLASS_NAME, "saveDocuments", null);
			if (!outError.isEmpty()) {

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("outError {}  ", outError.toArray());
				}
			...
			List<DocRateSummary> docRateSummaryList = createRateLevelSummary(
					savedDocs);
	...
	private void setItemCountInHeader(OutwardTransDocument doc) {
	...
	public List<DocRateSummary> createRateLevelSummary(
			List<OutwardTransDocument> documents) {
		Map<Long, List<DocRateSummary>> docRateMap = new HashMap<>();
		List<DocRateSummary> ratLevelList = new ArrayList<>();
		documents.forEach(doc -> {
			List<OutwardTransDocLineItem> lineItems = doc.getLineItems();
			lineItems.forEach(lineItem -> {
				DocRateSummary docRateSummary = new DocRateSummary();
		...
		docRateMap.forEach((key, value) -> {
			Map<String, List<DocRateSummary>> taxRateMap = value.stream()
					.collect(Collectors
							.groupingBy(obj -> taxRateBifurcationString(obj)));
			taxRateMap.forEach((rate, obj) -> {
				LOGGER.debug("taxRate Map {}", taxRateMap);
				LOGGER.debug("Rate {}", rate);
				DocRateSummary dto = obj.stream().reduce(new DocRateSummary(),
						(a, b) -> addDto(a, b));
	...
	private String taxRateBifurcationString(DocRateSummary rateSummary) {
	...
	private void bifurcateOutwardDoc(List<OutwardTransDocument> documents,
		...
		String configString = confiMap.get(CONFI_KEY) == null
				? "201704-202407|250000;202408-202812|100000"
				: confiMap.get(CONFI_KEY).getValue();

		String multiSupplyTypeAns = groupConfigPrmtRepository
				.findAnswerForMultiSupplyType();

		bifContext.seAttribute(MULTI_GROUP_LEVEL,
				Strings.isNullOrEmpty(multiSupplyTypeAns) ? "B"
						: multiSupplyTypeAns);
	...
	private int errorCount(List<OutwardTransDocument> savedDocs) {

		List<OutwardTransDocument> errorDocs = savedDocs.stream()
	...
	private boolean isIrnAvailable(OutwardTransDocument doc) {
		return !Strings.isNullOrEmpty(doc.getIrnResponse())
				|| (!Strings.isNullOrEmpty(doc.getIrn())
						&& doc.getIrn().length() == 64);
	}

	private void dynamicQrcodeSave(List<OutwardTransDocument> savedDocs) {
		try {
	...
	public EInvoiceOutwardDocSaveRespDto saveCanEwbDocuments(
		...
		try {
			...
			for (CancelEwbReqDto savedoc : documents) {
				...
				Optional<OutwardTransDocument> doc = docRepository
		...
		try {
			for (GenerateEWBByIrnERPReqDto reqDto : documents) {
				...
				if (einvRecord.isPresent()) {
					...
					Optional<OutwardTransDocument> outDoc = docRepository
	...
	private DocRateSummary addDto(DocRateSummary a, DocRateSummary b) {
		DocRateSummary dto = new DocRateSummary();
		dto.setCessAmt(
	...
	private Map<String, List<ProcessingResult>> isGstr1FreezeCheckBasedOnOnboarding(
		...
		try {
			...
			for (OutwardTransDocument document : documents) {
				...
				LOGGER.debug("entityOptedFreeze {} ", entityOptedFreeze);
	...
	private Map<String, List<String>> isGstr1MapFreezeCheckBasedOnOnboarding(
			List<OutwardTransDocument> documents, ProcessingContext context) {
		try {
			String paramkryId = CONFIG_PARAM_OUTWARD_QUE_KEY_ID.O36.name();
			OnboardingQuestionValidationsUtil util = StaticContextHolder
			...
			Map<String, List<String>> retResultMap = new HashMap<>();
