...
@Component("gstr1NilRatedSupDataATGstnImpl")
@Slf4j
public class GSTR1NilRatedSupDataATGstnImpl
		implements GSTR1NilRatedSupDataATGstn {
	...
	public Long findNilRatedSupDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.NIL.equals(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_NIL_RATED, param1, param2);
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1NilRatedSuccessHandler", "Gstr1GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
