...
@Service("CDNURAStructure")
public class CDNURAStructure {
	...
	@Autowired
	@Qualifier("CDNURAEYFinalStructure")
	private CDNURAEYFinalStructure cdnuraEYFinalStructure;
	...
	public JsonElement cdnuraResp(
			List<Gstr1BasicSummarySectionDto> eySummaryListFromView,
			List<? extends Gstr1B2BGstnRespDto> gstnList) {

		Gson gson = GsonUtil.newSAPGsonInstance();

		List<Gstr1SummaryRespDto> defaultcdnuraEYList = getDefaultCDNURAEYStructure();

		List<Gstr1SummaryRespDto> cdnuraEYList = cdnuraEYFinalStructure
				.getcdnuraEyList(defaultcdnuraEYList, eySummaryListFromView);
		
		Gstr1SummaryRespDto eyTotalCount = eyTotalCount(cdnuraEYList);
		...
		JsonElement eyTypeDetailsRespBody = gson.toJsonTree(cdnuraEYList);
	...
	private List<Gstr1SummaryRespDto> getDefaultCDNURAEYStructure() {
		// TODO Auto-generated method stub
		List<Gstr1SummaryRespDto> defaultCDNURAEY = new ArrayList<>();

		Gstr1SummaryRespDto cdnuraEy9B = new Gstr1SummaryRespDto();
		cdnuraEy9B.setTableSection("9B");
		//cdnuraEy9B.setTableSection("9C");
		cdnuraEy9B = defaultStructureUtil.defaultB2CLStructure(cdnuraEy9B);

		defaultCDNURAEY.add(cdnuraEy9B);
	...
	private Gstr1SummaryRespDto eyTotalCount(
			List<Gstr1SummaryRespDto> cdnuraEYList) {
	...
	//	BigDecimal sgst = new BigDecimal(0);
		...
		List<Gstr1SummaryRespDto> cdnuraey = cdnuraEYList;
		for (Gstr1SummaryRespDto cdnuraeyreq : cdnuraey) {
			invValue = invValue.add(cdnuraeyreq.getInvValue());
			taxable = taxable.add(cdnuraeyreq.getTaxableValue());
			taxpayble = taxpayble.add(cdnuraeyreq.getTaxPayble());
			igst = igst.add(cdnuraeyreq.getIgst());
		//	cgst = cgst.add(cdnuraeyreq.getCgst());
		//	sgst = sgst.add(cdnuraeyreq.getSgst());
			cess = cess.add(cdnuraeyreq.getCess());
			records = records + (cdnuraeyreq.getRecords());
