...
public class B2cQuantityValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(Object obj, Object[] row,
		...
		if (!isDecimal(quentity)) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER0215",
					"Invalid Quantity", location));
			// if it's not a number, then we can return the errors
			// immediately
			return errors;
		}
		boolean isValid = NumberFomatUtil.isValidDec(quentity);
		if (!isValid) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(QUANTITY);
			TransDocProcessingResultLoc location = 
