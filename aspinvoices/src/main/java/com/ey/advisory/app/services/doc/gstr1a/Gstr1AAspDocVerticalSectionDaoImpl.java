 ...
 *
 */
@Component("Gstr1AAspDocVerticalSectionDaoImpl")
public class Gstr1AAspDocVerticalSectionDaoImpl {
	...
	Gstr1AUserInputHsnSacRepository userHsnRepo;
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1AAspDocVerticalSectionDaoImpl.class);
	...
	private String createQueryString(String buildQuery2) {
		String queryStr = "SELECT SERIAL_NUM,NATURE_OF_DOC,"
				...
				+ "GSTR1A_PROCESSED_INV_SERIES " + "WHERE IS_DELETE=FALSE AND "
	...
	private String createHsnQueryString(String buildQuery1, String buildQuery2,
			String buildQueeryUser, String docTypeQuery, String supplyTypeQuery,
			String hdrSupplyTypeQuery, Boolean rateIncludedInHsn ,Boolean hsnFlag) {
		...
		if (rateIncludedInHsn && hsnFlag) {
			...
			builder.append("FROM ( ");
			builder.append(
					"SELECT ( HDR.SUPPLIER_GSTIN || '|' ||HDR.DERIVED_RET_PERIOD || '|' || CASE WHEN ITM.CUST_GSTIN IS NOT NULL AND LENGTH(ITM.CUST_GSTIN) = 15 THEN 'B2B' WHEN (LENGTH(ITM.CUST_GSTIN) < 15 OR ITM.CUST_GSTIN IS NULL) THEN 'B2C' END || '|' ||IFNULL(ITM_HSNSAC, 'null') || '|' || IFNULL(ITM_UQC, 'OTH')|| '|' || IFNULL(TAX_RATE, 0)) AS DOC_KEY,");
			builder.append(
			...
			builder.append(" FROM ANX_OUTWARD_DOC_HEADER_1A HDR INNER JOIN ");
			builder.append(
			...
			builder.append("FROM ANX_OUTWARD_DOC_ITEM_1A ITM1 ");
			builder.append("INNER JOIN ");
			builder.append(
					"ANX_OUTWARD_DOC_HEADER_1A HDR1 ON HDR1.ID = ITM1.DOC_HEADER_ID) ITM ON HDR.ID = ITM.DOC_HEADER_ID  ");
			builder.append(
			...
			builder.append("AND HDR.RETURN_TYPE='GSTR1A'  ");
			builder.append(
			...
			builder.append("FROM GSTR1A_PROCESSED_B2CS ");
			builder.append(
			...
			builder.append("FROM GSTR1A_SUMMARY_NILEXTNON ");
			...
			builder.append(
					"CESS_AMT as UI_CESS,TAX_RATE,RECORD_TYPE AS RECORDTYPE FROM GSTR1A_USERINPUT_HSNSAC  ");
			builder.append("WHERE IS_DELETE=FALSE AND ");
			...
			builder.append(
					"IGST_AMT as UI_IGST,CGST_AMT as UI_CGST,SGST_AMT as UI_SGST,");
			builder.append(
					"CESS_AMT as UI_CESS,TAX_RATE FROM GSTR1A_USERINPUT_HSNSAC  ");
			builder.append("WHERE IS_DELETE=FALSE AND ");
			...
			builder.append(
					"IGST_AMT as UI_IGST,CGST_AMT as UI_CGST,SGST_AMT as UI_SGST,");
			builder.append(
					"CESS_AMT as UI_CESS FROM GSTR1A_USERINPUT_HSNSAC  ");
			builder.append("WHERE IS_DELETE=FALSE AND ");
			builder.append(buildQueeryUser);
			builder.append(") GROUP BY DOC_KEY,SUPPLIER_GSTIN,RETURN_PERIOD,");
			builder.append("DERIVED_RET_PERIOD,ITM_HSNSAC,ITM_UQC )) ");
