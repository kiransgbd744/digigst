...
@Component("Gstr1aRSHsnRateLevelProcessedReportsDaoImpl")
public class Gstr1aRSHsnRateLevelProcessedReportsDaoImpl
		implements Gstr1ReviewSummaryReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1aRSHsnRateLevelProcessedReportsDaoImpl.class);
	...
	private String createApiProcessedQueryString(String buildHeader) {
		...
		return "SELECT MAX(ERROR_CODE_ASP)ERROR_CODE_ASP, MAX(ERROR_DESCRIPTION_ASP)ERROR_DESCRIPTION_ASP, "
				...
				+ " NULL AS EINVVSGSTR1RESPONSE  FROM ANX_OUTWARD_DOC_HEADER_1A HDR INNER JOIN "
				+ "ANX_OUTWARD_DOC_ITEM_1A ITM ON HDR.ID = ITM.DOC_HEADER_ID AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD LEFT OUTER JOIN "
				...
				+ " WHERE HDR.ASP_INVOICE_STATUS = 2 AND COMPLIANCE_APPLICABLE=TRUE AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1A' "
