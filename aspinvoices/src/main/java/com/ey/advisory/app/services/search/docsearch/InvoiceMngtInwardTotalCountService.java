...
@Service("InvoiceMngtInwardTotalCountService")
public class InvoiceMngtInwardTotalCountService {
	...
	public Integer getInvMngmtInwardCount(InwardListingReqDto criteria) {
		...
		//String accVoucherNo = request.getAccVoucherNo();
		...
		if (accVoucherNo != null && !accVoucherNo.isEmpty()) {
			if(accVoucherNo.size() > 1)
				buildQuery.append(" AND HDR.PURCHASE_VOUCHER_NUM IN :accVoucherNo ");
			else
				buildQuery.append(" AND HDR.PURCHASE_VOUCHER_NUM like :accVoucherNo ");
			}
		
		String queryStr = createInvdownloadQueryString(buildQuery.toString());
	...
	private String createInvdownloadQueryString(String buildQuery) {
