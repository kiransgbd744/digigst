...
@Component("Gstr1B2BASectionReportDaoImpl")
public class Gstr1B2BASectionReportDaoImpl
		implements Gstr1ReviewSummaryCustomizedReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1B2BASectionReportDaoImpl.class);
	...
	public Pair<List<Object[]>, List<Object>> getGstr1RSReports(SearchCriteria criteria) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inside Gstr1B2BASectionReportDaoImpl");
		}
		...
		if (arr[20] != null) {
			...
			if(timestamp.length() > 19){
			 ...
			 DateTimeFormatter utcFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
		        LocalDateTime localDateTime = LocalDateTime.parse(timestamp, utcFormatter);
	...
	private static String createApiProcessedQueryString(String buildHeader, String multiSupplyTypeAns) {
			...
			return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES, '') || ',' || IFNULL(ITM.ERROR_CODES, '')) AS ERROR_CODE_ASP "
					...
					+ "	AND  ITM.ITM_TAX_DOC_TYPE IN ('B2BA') "
					+ buildHeader
					+ " ORDER BY DOC_NUM "
					+ "	,ITM.ITM_NO";  
	...
	private static String createApiProcessedQueryStringForGstr1a(String buildHeader) {
		...
		return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| "
				...
				+ "NULL AS GSTIN_ERROR_DESCRIPTION_ASP, "
				+ "HDR.RETURN_TYPE AS RET_TYPE, "
				//+ "CASE WHEN A.ANSWER = 'B' OR HDR.IS_MULTI_SUPP_INV = FALSE THEN "
				+ "HDR.TABLE_SECTION "
				//+ " ELSE ITM.ITM_TABLE_SECTION END 
				 + "AS TABLE_NUMBER,HDR.IRN, "
				...
				+ "AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE IN ('B2BA') "
