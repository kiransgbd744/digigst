 ...
 * 
 */
@Component("Gstr2GetB2BAReportDaoImpl")
@Slf4j
public class Gstr2GetB2BAReportDaoImpl implements Gstr1aGetDao {
	...
	private String creategstnB2CSSummaryQueryString(String buildQuery) {
		...
		return "SELECT RETURN_PERIOD,CGSTIN,SGSTIN,LEGAL_NAME,TRADE_NAME,"
				+ "DOC_TYPE,SUPPLY_TYPE ,TABLE_TYPE,DOC_NUM,DOC_DATE,TAXABLE_VALUE,"
				+ "TAX_RATE,IGST_AMT,CGST_AMT,SGST_AMT ,CESS_AMT,TOTAL_TAX,"
				+ "INVOICE_VALUE,POS,STATE_NAME,PORT_CODE,BOE_NUM,"
				...
				+ "AS TOTAL_TAX, "
				+ "SUPPLIER_INV_VAL AS INVOICE_VALUE,"
				...
				*/+ " '' CDN_DELINKING_FLAG ,'' CR_DR_PRE_GST,"
				...
				+ "WHERE GSTIN_STATUS ='Active' AND IS_ACTIVE = TRUE AND IS_FETCHED = TRUE) VD "
				+ "ON HDR.SGSTIN = VD.GSTIN "
				+ " LEFT JOIN MASTER_STATE SC ON HDR.POS = SC.STATE_CODE "
				+ "WHERE HDR.IS_DELETE = FALSE GROUP BY TAX_PERIOD,"
