...
@Component("AspProcessVsSubmitB2csaReportDaoImpl")
public class AspProcessVsSubmitB2csaReportDaoImpl
	...
	private String createApiProcessedQueryString(String buildquery) {
		...
		build.append(
				"'B2CSA' TABLE_TYPE FROM GETGSTR1_B2CSA_HEADER HDR INNER ");
		build.append("JOIN GETGSTR1_B2CSA_ITEM ITM ON HDR.ID = ITM.HEADER_ID ");
		build.append(
		...
		build.append(
				" '' AS GSTN_ERROR_CODE,'' AS GSTN_ERROR_DESCRIPTION, '' SOURCE_ID,'' FILE_NAME, '' ASP_DATE_TIME FROM GSTR1_PROCESSED_B2CS B2CS");
		build.append(
		...
		build.append(
				"GSTRERR ON B2CS.AS_ENTERED_ID= GSTRERR.COMMON_ID AND  B2CS.FILE_ID=GSTRERR.FILE_ID AND GSTRERR.TABLE_TYPE='B2CS'  AND RETURN_TYPE='GSTR1' LEFT OUTER JOIN ");
		build.append(
