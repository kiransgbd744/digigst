 ...
 *
 */
@Service("Gstr9InOutwardProcessReportsServiceImpl")
@Slf4j
public class Gstr9InOutwardProcessReportsServiceImpl
	...
	@Autowired
	@Qualifier("Gstr9InOutwardProcessReportsDaoImpl")
	private Gstr1VerticalReportsDao gstr9InOutward; 
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		responseFromView = gstr9InOutward
				.getGstr1VerticalReports(request);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Gstr9 InOutward Process response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.INWARD_OUTWARD)
				...
				String[] b2csHeaders = commonUtility
						.getProp("gstr9.inoutward.vertical.process.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
				...
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"GSTR_9_Inward_Outward_Process.xlsx");
				if (LOGGER.isDebugEnabled()) {
