...
@Component("Gstr1AExpResponseHandler")
public class Gstr1AExpResponseHandler implements Gstr1AResponseHandler {

	@Autowired
	@Qualifier("ExpDefaultDocumentTypeFactoryImpl")
	private DocumentTypeMapperFactory exp;
	...
	public Triplet<List<String>, Map<String, OutwardTransDocErrorGstr1A>, Integer> response(
		...
		if (APIConstants.EXP.equalsIgnoreCase(batch.getSection())
				&& gsterrors != null) {
			// temp variable in place consider the dynamic response
			// flow(exp,expwop,expwp) from gstn PE case.
			List<EXPInvoices> temp = gsterrors.getExpInvoice() != null
					? gsterrors.getExpInvoice()
					: gsterrors.getExpwopInvoice() != null
							? gsterrors.getExpwopInvoice()
							: gsterrors.getExpwpInvoice();
			for (EXPInvoices invoice : temp) {
				for (EXPInvoiceData docData : invoice.getExpInvoiceData()) {
					...
					// digiGstDocType= exp.mapDocType(invoice.getExportType());
				...
				n = invoice.getExpInvoiceData().size();
			}
		} else if (APIConstants.EXPA.equalsIgnoreCase(batch.getSection())
				&& gsterrors != null) {
			for (EXPInvoices invoice : gsterrors.getExpaInvoice()) {
				for (EXPInvoiceData docData : invoice.getExpInvoiceData()) {
					String docNo = docData.getInvoiceNum();
					/*
