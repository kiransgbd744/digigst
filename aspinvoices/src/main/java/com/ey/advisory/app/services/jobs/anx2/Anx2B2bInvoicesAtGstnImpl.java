 ...
 *
 */
@Service("anx2B2bInvoicesAtGstnImpl")
@Slf4j
public class Anx2B2bInvoicesAtGstnImpl implements Anx2InvoicesAtGstn {
	...
	public ResponseEntity<String> findInvFromGstn(String jsonReq,
		...
		try {
			...
			dtos.forEach(dto ->{
			 ...
			 type, json);
			// static data
			//String apiResp = "{'b2b':[{'ctin':'32SSSKL8363S1ZF','docs':[{'doctyp':'I','diffprcnt':0.65,'sec7act':'Y','rfndelg':'Y','action':'R','cfs':'NF','pos':'03','itcent':'Y','uplddt':'25-05-2019','chksum':'0dbdc754fbb79bf3eba1207a9bd0e21d27d3bf0935eaf8d58a29f0db32cef279','doc':{'num':'INV200','dt':'25-04-2019','val':25000},'items':[{'hsn':'223456','txval':1000,'rate':8,'igst':80,'sgst':80,'cgst':80,'cess':100}]}]}]}";
			...
			/*	if (apiResp != null) {
				...
				Set<GetAnx2B2bInvoicesHeaderEntity> invoices = anx2DataParser
						.parseB2bData(dto, apiResp, batch.getId());
				...
				if (!invoices.isEmpty()) {
					...
					batch.setAnx2B2bInvoices(invoces);
					invoces.forEach(invoice -> {
						invoice.setB2bBatchIdAnx2(batch);
					});
					TenantContext.setTenantId(groupCode);
