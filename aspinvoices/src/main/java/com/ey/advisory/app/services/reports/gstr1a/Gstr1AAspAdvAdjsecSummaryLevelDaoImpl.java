...
import com.ey.advisory.core.search.SearchCriteria;
	...
	@Component("Gstr1AAspAdvAdjsecSummaryLevelDaoImpl")
	public class Gstr1AAspAdvAdjsecSummaryLevelDaoImpl
			implements Gstr1AASPAdvAdjSavableSummaryLevelDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr1AAspAdvAdjsecSummaryLevelDaoImpl.class);
		...
		private String createApiProcessedQueryString(String buildQuery) {
			...
			return "SELECT ADJ.SUPPLIER_GSTIN,ADJ.RETURN_PERIOD,"
					...
					+ " NEW_POS,NEW_RATE,NEW_GROSS_ADV_ADJUSTED,IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT,PROFIT_CENTRE,PLANT_CODE,DIVISION,"
					+ " LOCATION, SALES_ORGANIZATION, DISTRIBUTION_CHANNEL, USERACCESS1,USERACCESS2,USERACCESS3,USERACCESS4,"
					...
					+ " (CASE WHEN ADJ.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' WHEN ADJ.IS_SAVED_TO_GSTN = FALSE"
					+ " AND ADJ.GSTN_ERROR = TRUE THEN 'IS_ERROR' WHEN ADJ.IS_SAVED_TO_GSTN = FALSE AND"
					+ " ADJ.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) AS SAVE_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID,"
					+ " GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME, '' AS GSTN_ERROR_CODE,'' AS GSTN_ERROR_DESCRIPTION,"
					+ " FIL.CREATED_BY AS SOURCE_ID,FIL.FILE_NAME AS FILE_NAME, ADJ.CREATED_ON AS ASP_DATE_TIME"
					+ " FROM GSTR1A_PROCESSED_ADV_ADJUSTMENT ADJ LEFT OUTER JOIN TF_GSTR1A_ERROR_INFO() "
					...
					+ "WHERE ADJ.IS_DELETE=FALSE AND ADJ.IS_AMENDMENT=FALSE " 
			        + buildQuery;
		}
	}
