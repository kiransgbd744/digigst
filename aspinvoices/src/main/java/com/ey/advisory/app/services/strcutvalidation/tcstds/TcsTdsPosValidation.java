...
public class TcsTdsPosValidation implements ValidationRule {

	@Override
	public List<ProcessingResult> isValid(Object obj, Object[] row,
			TabularDataLayout layout) {
		List<ProcessingResult> errors = new ArrayList<>();
		//mandatory in case regular supplier gstin
		/*if (!isPresent(obj)) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.TDS_POS);
		...
		if(obj.toString().trim().length() > 2 ||  !matcher.matches()){
			...
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER1351",
					"Invalid POS", location));
			return errors;
		}
