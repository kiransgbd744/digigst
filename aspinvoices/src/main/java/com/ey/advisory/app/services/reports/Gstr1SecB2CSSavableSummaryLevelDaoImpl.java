 ...
 * 
 */
@Component("Gstr1SecB2CSSavableSummaryLevelDaoImpl")
	public class Gstr1SecB2CSSavableSummaryLevelDaoImpl
			implements Gstr1ASPB2CSSavableSummaryLevelDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr1SecB2CSSavableSummaryLevelDaoImpl.class);
		...
		public List<Object> getGstr1B2CSSavableReports(SearchCriteria criteria) {
			...
			if (APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
				queryStr = createB2CSSavableSummaryQueryStringForGstr1a(
						buildQuery.toString());

			} else {
				queryStr = createB2CSSavableSummaryQueryString(
						buildQuery.toString());
		...
		private Gstr1SavableB2CSSummaryLevelDto convertB2CSSummary(Object[] arr) {
			...
			obj.setAspInformationID(arr[40] != null ? DownloadReportsConstant.CSVCHARACTER
					.concat(arr[40].toString()) : null);
		...
		private String createB2CSSavableSummaryQueryString(String buildQuery) {
			...
			/*return "SELECT B2CS.SUPPLIER_GSTIN,B2CS.RETURN_PERIOD,"
					...
					+ "B2CS.CREATED_ON AS ASP_DATE_TIME FROM GSTR1_PROCESSED_B2CS "
					+ "B2CS LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() GSTRERR ON "
					...
					+ "FIL ON B2CS.FILE_ID=FIL.ID AND GSTRERR.FILE_ID=FIL.ID "
					+ "AND GSTRERR.INV_KEY=B2CS.B2CS_INVKEY LEFT OUTER JOIN "
					+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON "
					+ "GSTNBATCH.ID = B2CS.BATCH_ID WHERE B2CS.IS_DELETE=FALSE "
					+ "AND IS_AMENDMENT=FALSE " + buildQuery;*/
					
					return "SELECT B2CS.SUPPLIER_GSTIN,B2CS.RETURN_PERIOD,"
					+ "B2CS.TRAN_TYPE,MONTH,ORG_POS,ORG_HSNORSAC,"
					+ "ORG_UOM,ITM.ORG_QNT,ORG_RATE,ITM.ORG_TAXABLE_VALUE,"
					+ "ORG_ECOM_GSTIN,ITM.ORG_ECOM_SUP_VAL,ITM.NEW_POS,NEW_HSNORSAC,"
					+ "NEW_UOM,NEW_QNT,B2CS.NEW_RATE,ITM.NEW_TAXABLE_VALUE,"
					+ "NEW_ECOM_GSTIN,'' NEW_ECOM_SUP_VAL,ITM.IGST_AMT,ITM.CGST_AMT,ITM.SGST_AMT,"
					+ "ITM.CESS_AMT,ITM.TOT_VAL,PROFIT_CENTRE,PLANT_CODE,DIVISION,LOCATION,"
					+ "SALES_ORGANIZATION,DISTRIBUTION_CHANNEL,USERACCESS1,"
					+ "USERACCESS2,USERACCESS3,USERACCESS4,USERACCESS5,"
					+ "USERACCESS6,USERDEFINED_FIELD1,USERDEFINED_FIELD2,"
					+ "USERDEFINED_FIELD3, TRIM(', ' FROM "
					...
					+ "JOIN   (SELECT SUM(ORG_TAXABLE_VALUE) AS ORG_TAXABLE_VALUE,SUM(ORG_ECOM_SUP_VAL) AS ORG_ECOM_SUP_VAL,"
					+ "SUM(NEW_TAXABLE_VALUE) AS NEW_TAXABLE_VALUE,SUM(IGST_AMT) AS IGST_AMT,SUM(CGST_AMT) AS CGST_AMT,"
					+ "SUM(SGST_AMT) AS SGST_AMT,SUM(CESS_AMT) AS CESS_AMT,SUM(TOT_VAL) AS TOT_VAL,SUM(ORG_QNT) AS ORG_QNT,"
					+ "MAX(ID) AS ID,NEW_POS,SUPPLIER_GSTIN,DERIVED_RET_PERIOD,TRAN_TYPE,NEW_RATE "
					+ "FROM GSTR1_PROCESSED_B2CS "
					+ "GROUP BY NEW_POS,SUPPLIER_GSTIN,DERIVED_RET_PERIOD,TRAN_TYPE,NEW_RATE) ITM "
					+ "ON B2CS.ID=ITM.ID AND B2CS.SUPPLIER_GSTIN=ITM.SUPPLIER_GSTIN "
					+ "AND B2CS.DERIVED_RET_PERIOD=ITM.DERIVED_RET_PERIOD "
					+ "LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() GSTRERR "
					+ "ON B2CS.AS_ENTERED_ID= GSTRERR.COMMON_ID AND "
					+ "B2CS.FILE_ID=GSTRERR.FILE_ID AND GSTRERR.TABLE_TYPE='B2CS' "
					+ "AND RETURN_TYPE='GSTR1' "
					+ "LEFT OUTER JOIN FILE_STATUS "
					+ "FIL ON B2CS.FILE_ID=FIL.ID AND GSTRERR.FILE_ID=FIL.ID "
					...
					+ "WHERE B2CS.IS_DELETE=FALSE AND IS_AMENDMENT=FALSE" + buildQuery;
		}
		
		private String createB2CSSavableSummaryQueryStringForGstr1a(String buildQuery) {
					...
					+ "FIL.CREATED_BY AS SOURCE_ID, FIL.FILE_NAME AS FILE_NAME,"
					+ "B2CS.CREATED_ON AS ASP_DATE_TIME "
					+ "FROM GSTR1A_PROCESSED_B2CS  B2CS "
					+ "JOIN   (SELECT SUM(ORG_TAXABLE_VALUE) AS ORG_TAXABLE_VALUE,SUM(ORG_ECOM_SUP_VAL) AS ORG_ECOM_SUP_VAL,"
					+ "SUM(NEW_TAXABLE_VALUE) AS NEW_TAXABLE_VALUE,SUM(IGST_AMT) AS IGST_AMT,SUM(CGST_AMT) AS CGST_AMT,"
					+ "SUM(SGST_AMT) AS SGST_AMT,SUM(CESS_AMT) AS CESS_AMT,SUM(TOT_VAL) AS TOT_VAL,SUM(ORG_QNT) AS ORG_QNT,"
					+ "MAX(ID) AS ID,NEW_POS,SUPPLIER_GSTIN,DERIVED_RET_PERIOD,TRAN_TYPE,NEW_RATE "
