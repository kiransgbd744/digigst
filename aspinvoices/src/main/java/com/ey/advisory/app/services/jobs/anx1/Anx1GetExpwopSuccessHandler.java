...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1ExpwopInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetExpwopSuccessHandler")
@Slf4j
public class Anx1GetExpwopSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("anx1ExpwopDataParserImpl")
	private Anx1ExpwopDataParser anx1ExpwopDataParser;

	@Autowired
	@Qualifier("GetAnx1ExpwopInvoicesRepository")
	private GetAnx1ExpwopInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetExpwopSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1ExpwopInvoicesHeaderEntity> b2cEntities = anx1ExpwopDataParser
						.parseExpwopData(dto, apiResp);
				if (!b2cEntities.isEmpty()) {

					headerRepo.softlyDeleteExpwopHeader(dto.getGstin(),
							dto.getReturnPeriod());
