...
@Service("CewbProcessedInfoReportsServiceImpl")
@Slf4j
public class CewbProcessedInfoReportsServiceImpl
	...
	@Autowired
	@Qualifier("CewbProcessedInfoReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Cewb processedinfo response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.CEWB).equalsIgnoreCase(request.getFileType())) {
				...
				String[] invoiceHeaders = commonUtility
						.getProp("cewb.vertical.processinfo.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
