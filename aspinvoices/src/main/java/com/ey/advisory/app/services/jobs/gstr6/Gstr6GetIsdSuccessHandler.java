 ...
 *
 */
@Service("Gstr6GetIsdSuccessHandler")
@Slf4j
public class Gstr6GetIsdSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr6IsdDataParserImpl")
	private Gstr6IsdGetDataParser gstr6IsdGetDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GET Call is Success for the batchId {} inside Gstr6GetIsdSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				...
				isdHeaderRepo.softlyDeleteIsdHeader(dto.getGstin(), dto.getReturnPeriod(), now);
			...
			resultIds.forEach(id -> {
				...
				List<GetGstr6IsdDetailsEntity> isdEntities = gstr6IsdGetDataParser.parseIsdGetData(dto, apiResp,
						dto.getType(), ctxParamsObj.get("batchId").getAsLong(), now);

				if (isdEntities != null && !isdEntities.isEmpty()) {

					isdHeaderRepo.saveAll(isdEntities);
