...
public class GetGstr6ADetailStatusFetchDaoImpl {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(GetGstr6DetailStatusFetchDaoImpl.class);
	...
	public List<Object[]> getDataUploadedStatusDetails(String gstin,
		...
		buffer.append(
				"SELECT * FROM ( SELECT * FROM (SELECT GSTIN,RETURN_PERIOD,DENSE_RANK() "
						...
						+ "AS NUM, GET_TYPE,"
						+ "(CASE WHEN STATUS IN('FAILED','SUCCESS','SUCCESS_WITH_NO_DATA') THEN MAX(END_TIME) "
						+ " WHEN STATUS IN('INITIATED','INPROGRESS') THEN MAX(CREATED_ON) END) AS START_TIME,"
						+ " STATUS" + " FROM GETANX1_BATCH_TABLE GBT WHERE "
						+ "API_SECTION='GSTR6A' AND GSTIN = :gstin ");
						if (StringUtils.isNotBlank(taxPeriod)) {
							buffer.append(" AND RETURN_PERIOD = :taxPeriod");
						}
						buffer.append(" group by GSTIN,RETURN_PERIOD, " 
						+ "STATUS,GET_TYPE,START_TIME) A WHERE NUM = 1");
		...
		buffer.append(" UNION "
				...
				+ " WHERE STATUS = 'SUCCESS' AND API_SECTION='GSTR6A' AND GSTIN = :gstin ");
				if (StringUtils.isNotBlank(taxPeriod)) {
					buffer.append(" AND RETURN_PERIOD = :taxPeriod");
				}
				buffer.append(" ) A WHERE NUM = 1");
