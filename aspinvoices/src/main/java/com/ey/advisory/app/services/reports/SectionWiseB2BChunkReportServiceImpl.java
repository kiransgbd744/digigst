...
@Slf4j
@Component("SectionWiseB2BChunkReportServiceImpl")
public class SectionWiseB2BChunkReportServiceImpl
		implements SectionWiseB2BChunkReportService {
	...
	private static final String SECTION_B2B_REPORT_CHUNK_ROWS = "section.b2b.report.chunk.rows";
	private static final String SECTION_B2B_REPORT_CHUNK_FILES = "section.b2b.report.chunk.files";

	@Override
	public Pair<Integer, Integer> getChunkingSizes() {
		Map<String, Config> configMap = configManager.getConfigs(
				"SECTION_B2B_REPORTS",
				"section.b2b.report.chunk");
		String fileSize = configMap.get(SECTION_B2B_REPORT_CHUNK_ROWS) != null
				? configMap.get(SECTION_B2B_REPORT_CHUNK_ROWS).getValue()
				: "100000";
		String reportSize = configMap.get(SECTION_B2B_REPORT_CHUNK_FILES) != null
				? configMap.get(SECTION_B2B_REPORT_CHUNK_FILES).getValue() : "1";
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		String type = entity.getReportType();
		//String dataType = entity.getDataType();
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("B2B Section Directory is empty");
			return;
		}

		Set<File> fileNames = new TreeSet<File>();
		File[] files = tempDir.listFiles();
		LOGGER.debug("B2B Section Directory file size {} ", files.length);
		...
		try {
			...
			while (it.hasNext()) {
				if("GSTR1_B2B".equalsIgnoreCase(type)){
					
				 rptType = "GSTR1 B2B" + " " + index;

					
				}else{
					
				 rptType = "GSTR1A B2B" + " " + index;
