...
/*
 * This service responsible for read the Product data from excel and then write
 * into hana data base
 */

@Component("ItemWebUploadServiceImpl")
public class ItemWebUploadServiceImpl implements ItemWebUploadService {
	public static final Logger LOGGER = LoggerFactory
			.getLogger(ItemWebUploadServiceImpl.class);
	...
	public ResponseEntity<String> itemProcessingFile(MultipartFile[] files,
			String folderName, String uniqueName, String fileType,
			String userName, String dataType, String groupCode, Long entityId) {
		LOGGER.error("Inside item file upload method and uniqueName is {}  ",
				uniqueName);
		...
		try {
			...
			Pair<Boolean, String> checkHeaderFormat = itemHeaderCheckService
					.validate(getHeaders);
			if (checkHeaderFormat.getValue0()) {
				...
				for (MultipartFile file : files) {
					...
					if (!file.isEmpty()) {
						...
						if (uniqueName.equalsIgnoreCase(
								GSTConstants.MASTER_ITEM_FILE_NAME)) {
							String jobCategory = JobConstants.ITEMWEBUPLOAD;
						...
						for (Long jobId : findByJobIds) {
							...
							Gstr1FileStatusEntity fileStatus = gstr1FileUploadUtil
									.getMasterFileStatusEntity(fileName,
											...
											dataType, entityId);
							fileStatus.setFileType(GSTConstants.ITEM);
				...
				APIRespDto dto = new APIRespDto("Success",
						"Master Item file uploaded successfully");
			...
			APIRespDto dto = new APIRespDto("Failed",
					"Master Item file uploaded Failed");
			JsonObject resp = new JsonObject();
			JsonElement respBody = gson.toJsonTree(dto);
			String msg = "Unexpected error while uploading Master Item files";
