...
import com.ey.advisory.core.search.SearchCriteria;
	...
	@Service("Anx1InwardApiProcessedReportsServiceImpl")
	public class Anx1InwardApiProcessedReportsServiceImpl
	implements Anx1InwardApiProcessedReportsService {

private static final Logger LOGGER = LoggerFactory
		.getLogger(Anx1InwardApiProcessedReportsServiceImpl.class);

@Autowired
CommonUtility commonUtility;

@Autowired
@Qualifier("Anx1InwardApiProcessedReportsDaoImpl")
private Anx1InwardApiProcessedReportsDao anx1InwardApiProcessedReportsDao;

@Override
public Workbook findInwardApiProcesseRec(SearchCriteria criteria,
		PageRequest pageReq) {
	...
	List<Anx1InwardApiProcessedRecordsDto> responseFromView = new ArrayList<>();
	responseFromView = anx1InwardApiProcessedReportsDao
			.getInwardApiProcessedReports(request);

	if (responseFromView != null && responseFromView.size() > 0) {
		String[] invoiceHeaders = commonUtility
				.getProp("anx1.inward.api.processedrec.report.headers").split(",");
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ANX1_InwardAPIProcessedRecReport.xlsx");
