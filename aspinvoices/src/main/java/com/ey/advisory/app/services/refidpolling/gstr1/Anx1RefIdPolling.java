 ...
 *
 */
@Component("Anx1RefIdPolling")
public class Anx1RefIdPolling {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1RefIdPolling.class);

	@Autowired
	@Qualifier("Anx1DefaultBatchIdPollingManager")
	...
	public ResponseEntity<String> processRefIds(Anx1SaveToGstnReqDto reqDto) {
		...
		TenantContext.setTenantId(reqDto.getGroupcode());
		try {
			batches = gstr1BatchRepository.findanx1ReferenceIdByGstin(
					reqDto.getGstins(),reqDto.getReturnPeriod());

			// For each of the batch, perform the polling operation.
