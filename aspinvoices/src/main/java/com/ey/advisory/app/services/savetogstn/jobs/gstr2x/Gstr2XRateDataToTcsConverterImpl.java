 ...
 *
 */
@Service("Gstr2XRateDataToTcsConverterImpl")
@Slf4j
public class Gstr2XRateDataToTcsConverterImpl implements RateDataToGstr2XConverter{
	...
	public SaveBatchProcessDto convertToGstr2XObject(List<Object[]> objects, String section, String groupCode,
		...
		try {
			if (objects != null && !objects.isEmpty()) {
				...
				List<Gstr2XTcsDto> tcsList = new ArrayList<>();
				...
				for (int counter = 0; counter < totSize; counter++) {
					...
					if (isNewInvoice(arr1, arr2, idsList, totSize, counter2)) {
						Gstr2XTcsDto tcsData = setInv(arr1);
						tcsList.add(tcsData);
					} else {
						continue;
					}
					if (isNewBatch(idsList, totSize, counter2)) {
						Gstr2XDto gstr2x = setBatch(arr1, tcsList);
						if (LOGGER.isDebugEnabled()) {
	...
	private Gstr2XTcsDto setInv(Object[] arr1) {
		Gstr2XTcsDto data = new Gstr2XTcsDto();
	...
	private Gstr2XDto setBatch(Object[] arr1, List<Gstr2XTcsDto> tcsList) {
