...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1SezwpaInvoicesRepository;
...
@Service("Anx1GetSezwpaSuccessHandler")
@Slf4j
public class Anx1GetSezwpaSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Anx1SezwpaDataParserImpl")
	private Anx1SezwpaDataParser anx1SezwpaDataParser;

	@Autowired
	@Qualifier("GetAnx1SezwpaInvoicesRepository")
	private GetAnx1SezwpaInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetSezwpaSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1SezwpaInvoicesHeaderEntity> b2cEntities = anx1SezwpaDataParser
						.parseSezwpaData(dto, apiResp);
