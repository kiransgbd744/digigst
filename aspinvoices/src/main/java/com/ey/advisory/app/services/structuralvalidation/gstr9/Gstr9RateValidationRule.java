 ...
 *
 */
public class Gstr9RateValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(Object obj, Object[] row,
		...
		if (!isPresent(obj)) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER6174",
					"Rate cannot be left blank", location));
			return errors;
		}
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER6175",
		...
		boolean isValid = NumberFomatUtil
				.is7And3digValidDec(obj.toString().trim());
		if (!isValid) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.RATE);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
