...
@Service("CrossItcProcessedInfoReportsServiceImpl")
@Slf4j
public class CrossItcProcessedInfoReportsServiceImpl
	...
	@Autowired
	@Qualifier("CrossItcProcessedInfoReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Cross Itc processedinfo response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.CROSS_ITC).equalsIgnoreCase(request.getFileType())) {
				...
				String[] invoiceHeaders = commonUtility
						.getProp("crossitc.vertical.processinfo.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
				...
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Cross_ITC_ProcessedInfoReport.xlsx");
				if (LOGGER.isDebugEnabled()) {
