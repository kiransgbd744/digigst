...
import com.ey.advisory.app.data.views.client.Gstr1GstnEXPErrorDto;
...
@Component("Gstr1GstnEXPErrorDaoImpl")
public class Gstr1GstnEXPErrorDaoImpl implements Gstr1GstnErrorDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1GstnEXPErrorDaoImpl.class);
...
String queryStr = null;
		...
		{
			queryStr = creategstnGstr1AEXPErrorQueryString(buildQuery.toString());
		}
		else
		{
			queryStr = creategstnEXPErrorQueryString(buildQuery.toString());
		}
		...
		return list.parallelStream().map(o -> convertgstnEXPerror(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1GstnEXPErrorDto convertgstnEXPerror(Object[] arr) {
		Gstr1GstnEXPErrorDto obj = new Gstr1GstnEXPErrorDto();

		obj.setInvoiceNumber(arr[0] != null ? arr[0].toString() : null);
		obj.setInvoiceDate(arr[1] != null ? arr[1].toString() : null);
		obj.setShippingbillNumber(arr[2] != null ? arr[2].toString() : null);
		obj.setShippingbillDate(arr[3] != null ? arr[3].toString() : null);
		...
		obj.setIntegratedAmount(arr[8] != null ? arr[8].toString() : null);
		obj.setCess(arr[9] != null ? arr[9].toString() : null);
		obj.setInvoiceValue(arr[10] != null ? arr[10].toString() : null);
		obj.setExpType(arr[11] != null ? arr[11].toString() : null);
		obj.setErrorCode(arr[12] != null ? arr[12].toString() : null);
	...
	private String creategstnEXPErrorQueryString(String buildQuery) {

		return "SELECT HDR.DOC_NUM,HDR.DOC_DATE,HDR.SHIP_BILL_NUM,"
				+ "HDR.SHIP_BILL_DATE,HDR.SHIP_PORT_CODE,"
				+ "HDR.DIFF_PERCENT,ITM.TAXABLE_VALUE,"
				+ "ITM.TAX_RATE,ITM.IGST_AMT,(ITM.CESS_AMT_SPECIFIC+ITM.CESS_AMT_ADVALOREM) "
				+ "CESS_AMT, ITM.LINE_ITEM_AMT,HDR.TAX_DOC_TYPE AS EXPORT_TYPE,"
				...
				+ "IFNULL(ERRI.ERROR_DESCRIPTION_GSTN,'')) AS GSTN_ERROR_DESCRIPTION, "
				...
				+ "HDR.ID = ERRH.DOC_HEADER_ID INNER JOIN ANX_OUTWARD_DOC_ITEM ITM "
				...
				+ "AND HDR.GSTN_ERROR=TRUE AND "
				+ "HDR.TAX_DOC_TYPE IN ('EXPORTS') AND HDR.IS_DELETE=FALSE "
				+ "AND GSTN_SAVE_STATUS='PE' " + buildQuery;
	}
	
	private String creategstnGstr1AEXPErrorQueryString(String buildQuery) {
		...
		return "SELECT HDR.DOC_NUM,HDR.DOC_DATE,HDR.SHIP_BILL_NUM,"
				...
				+ "TF_OUTWARD_HEADER_GSTIN_ASP_ERROR_INFO_1A () ERRH ON "
				+ "HDR.ID = ERRH.DOC_HEADER_ID INNER JOIN ANX_OUTWARD_DOC_ITEM_1A ITM "
				+ "ON HDR.ID = ITM.DOC_HEADER_ID LEFT OUTER JOIN "
				+ "TF_OUTWARD_ITEM_GSTIN_ASP_ERROR_INFO_1A () ERRI ON "
				+ "ITM.DOC_HEADER_ID = ERRI.DOC_HEADER_ID AND "
				+ "IFNULL(ITM.ITEM_INDEX,'-1') = IFNULL(ERRI.ITEM_INDEX,'-1') "
				+ "AND IFNULL(ITM.ITM_NO,'0') = IFNULL(ERRI.ITM_NO,'0') "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH "
