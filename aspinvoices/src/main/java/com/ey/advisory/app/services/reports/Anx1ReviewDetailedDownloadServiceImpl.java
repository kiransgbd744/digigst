...
@Service("Anx1ReviewDetailedDownloadServiceImpl")
public class Anx1ReviewDetailedDownloadServiceImpl
		implements Anx1ReviewSummScreenDownloadDetailedService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ReviewDetailedDownloadServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1ReviewDetailedOutwardScreenDownloadDaoImpl")
	private Anx1ReviewSummaryScreenDetailedDownloadDao anx1ReviewSummaryScreenDetailedDownloadDao;

	@Autowired
	@Qualifier("Anx1ReviewDetailedInwardScreenDownloadDaoImpl")
	private Anx1ReviewSummaryInwardScreenDetailedDao anx1ReviewSummaryInwardScreenDetailedDao;
	...
	@Override
	public Workbook findReviewSumScreenDetailDownload(SearchCriteria criteria,
			PageRequest pageReq) {
		Annexure1SummaryReqDto request = (Annexure1SummaryReqDto) criteria;
		Workbook workbook = new Workbook();
		int outwardStartRow = 3;

		int inwardStartRow = 62;
		int ecomStartRow = 95;

		int startcolumn = 0;
		boolean isHeaderRequired = false;

		List<Anx1RSScreenDetailOutwarddownloadDto> outwardRSresponse = anx1ReviewSummaryScreenDetailedDownloadDao
				.getReviewDetailScreenDownload(request);

		List<Anx1RSScreenInwarddetailedDto> inwardRSresponse = anx1ReviewSummaryInwardScreenDetailedDao
				.getReviewSummInvScreenDetailDownload(request);
		...
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"ANX-1_ReviewSummary_Screen_Detailed_Download.xlsx");
