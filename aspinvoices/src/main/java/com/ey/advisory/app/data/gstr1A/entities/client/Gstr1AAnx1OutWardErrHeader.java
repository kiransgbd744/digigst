...
import com.ey.advisory.app.data.entities.client.TransErrorDocument;
...
@Entity
@Table(name = "ANX_OUTWARD_ERR_HEADER_1A")
@Setter
@Getter
@ToString
@Slf4j
public class Gstr1AAnx1OutWardErrHeader extends TransErrorDocument {

	@Expose
	@SerializedName("svErrDocId")
	@Id
	@SequenceGenerator(name = "sequence", sequenceName = "ANX_OUTWARD_ERR_HEADER_SEQ_1A", allocationSize = 100)
	...
	protected List<Gstr1AAnxOutwardTransDocLineItemError> lineItems = new ArrayList<>();
	...
	public Integer getItemNoForIndex(Integer itemIndex) {
		...
		if (itemIndex < lineItemCount && itemIndex >= 0) {
			Gstr1AAnxOutwardTransDocLineItemError lineItem = lineItems
					.get(itemIndex);
			try {
