 ...
 */
@Slf4j
@Service("RetGetInvoicesAtGstnImpl")
public class RetGetInvoicesAtGstnImpl implements RetInvoicesAtGstn {

	@Autowired
	@Qualifier("RetDataAtGstnImpl")
	private RetDataAtGstn retDataAtGstn;
	...
	public ResponseEntity<String> findInvFromGstn(RetGetInvoicesReqDto dto,
		...
		try {
			...
			batch = batchUtil.makeBatchRet(dto);
			TenantContext.setTenantId(groupCode);
			// InActivating Previous Batch Records
			batchRepo.softlyDelete("", APIConstants.RET.toUpperCase(),
			...
			apiResp = retDataAtGstn.findRetDataAtGstn(dto, groupCode, json);
			...
			throw new APIException(
					"Unexpected error while executing Ret Get Invoices");
