 ...
 */
@Slf4j
@Service("ProcsSubmitDocIssuedScreenSectionDaoImpl")
public class ProcsSubmitDocIssuedScreenSectionDaoImpl
	...
	public List<Gstr1SummaryDocSectionDto> loadBasicSummarySection(
		...
		if (StringUtils.isNotEmpty(taxPeriodFrom1) && StringUtils.isNotEmpty(taxPeriodTo1)) {

			build.append(" AND DERIVED_RET_PERIOD BETWEEN  :taxPeriodFrom AND :taxPeriodTo ");
			buildForGet.append(" AND DERIVED_RET_PERIOD BETWEEN  :taxPeriodFrom AND :taxPeriodTo ");
	 ...
	 * @return
	 */
	private static String createQueryString(String buildQuery,String buildQueryGet) {
		// TODO Auto-generated method stub
		...
		String queryStr = "SELECT TAX_DOC_TYPE,SUM(ASP_TOT_NUM) ASP_TOT_NUM,"
				...
				+ "0 GSTN_CANCELED,0 GSTN_NET_NUM "
				+ "FROM ( "
				+ "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,DERIVED_RET_PERIOD,"
				...
				+ "FROM GSTR1_PROCESSED_INV_SERIES "
				...
				+ "GSTN_TOT_NUM, GSTN_CANCELED, GSTN_NET_NUM "
				+ "FROM ( "
	...
	//	StringBuilder buildVertical = new StringBuilder();
		...
		if (StringUtils.isNotEmpty(taxPeriodFrom1) && StringUtils.isNotEmpty(taxPeriodTo1)) {
				...
				build.append(" AND HDR.DERIVED_RET_PERIOD BETWEEN  :taxPeriodFrom AND :taxPeriodTo ");
			
			
			
			buildHdr.append(" AND DERIVED_RET_PERIOD BETWEEN  :taxPeriodFrom AND :taxPeriodTo ");
	...
	//	String buildQueryVert = buildVertical.toString();
		if(LOGGER.isDebugEnabled()){
		LOGGER.debug(
				"Prepared where Condition and apply in "
				+ "Nil,Non Exempted Query BEGIN");
	}
		
		String queryStr = createQueryStringNil(buildQuery,buildQueryForGet, multiSupplyTypeAns);
		if(LOGGER.isDebugEnabled()) {
		LOGGER.debug(
				"Executing Nil,Non Exempted Query For Sections is -->" + queryStr);
		}
	...
	public static void main(String[] args) {
		...
		String buildQueryForGet = buildHdr.toString().substring(4);
		//String queryStr = createQueryStringNil(buildQuery,buildQueryForGet, multiSupplyTypeAns);
		String queryStr = createQueryString(buildQuery,buildQueryForGet);
		System.out.println(queryStr);
	}
	private static String createQueryStringNil(String buildQuery,String buildQueryForGet, String multiSupplyTypeAns) {
		// TODO Auto-generated method stub
		LOGGER.debug("Outward Query Execution BEGIN ");
