...
@Component("gstr1DocIssuesDataAtGstnImpl")
@Slf4j
public class Gstr1DocIssuesDataAtGstnImpl implements Gstr1DocIssuesDataAtGstn {
	...
	public Long findDocIssueDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.DOC_ISSUE.equals(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_DOC_ISSUED, param1, param2);
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1DocSuccessHandler", "Gstr1GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
