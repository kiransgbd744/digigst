...
public class RcValidator
	...
	public List<ProcessingResult> validate(InwardTable3I3HExcelEntity document,
		...
		if (transactionFlag != null && !transactionFlag.isEmpty()) {
			if (transactionFlag.equalsIgnoreCase(GSTConstants.RC)) {
				if (sec70fIGSTFLAG != null && !sec70fIGSTFLAG.isEmpty()) {
					if (sec70fIGSTFLAG.equalsIgnoreCase(GSTConstants.Y)) {
						...
						if (amount.compareTo(BigDecimal.ZERO) > 0) {
							...
							errors.add(new ProcessingResult(APP_VALIDATION,
									"ER0430",
									" In case of RC & Section7ofIGST is"
											+ " Y, then CGST and SGST tax "
