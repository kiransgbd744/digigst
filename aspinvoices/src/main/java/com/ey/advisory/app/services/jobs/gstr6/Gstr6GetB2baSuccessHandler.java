 ...
 *
 */
@Service("Gstr6GetB2baSuccessHandler")
@Slf4j
public class Gstr6GetB2baSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr6B2baDataParserImpl")
	private Gstr6B2baGetDataParser gstr6B2baGetDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GET Call is Success for the batchId {} inside Gstr6GetB2baSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				if (APIConstants.B2BA.equalsIgnoreCase(dto.getType())) {
					b2baHeaderRepo.softlyDeleteB2baHeader(dto.getGstin(), dto.getReturnPeriod(), now);
				}
			}
			...
			resultIds.forEach(id -> {
				...
				if (APIConstants.B2BA.equalsIgnoreCase(dto.getType())) {
				List<GetGstr6B2baHeaderEntity> b2baEntities = gstr6B2baGetDataParser.parseB2baGetData(dto, apiResp,
						dto.getType(), ctxParamsObj.get("batchId").getAsLong(), now);
				if (b2baEntities!=null && !b2baEntities.isEmpty()) {
					b2baHeaderRepo.saveAll(b2baEntities);
				}
				}
