...
public class ReverseChargeFlagValidationRule implements ValidationRule {

	private static final List<String> IMPORTS = ImmutableList.of("Y", "N", "n",
			"y");

	private static final List<String> IMPORTS_2_VERSION = ImmutableList.of("Y",
			"N", "n", "y", "L", "l");
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (CommonContext.getDelinkingFlagContext()) {
			flags.addAll(IMPORTS_2_VERSION);
		} else {
			flags.addAll(IMPORTS);
		}

		// First check if the input object is a valid decimal number.
		if (!flags.contains(obj.toString().trim())) {
