...
public class B2cArthimatics
	...
	public List<ProcessingResult> validate(OutwardB2cExcelEntity document,
		...
		String taxableValues = (document.getTaxableValue() != null)
				? document.getTaxableValue().trim() : null;
		if (taxableValues != null && !taxableValues.isEmpty()) {
			taxableValue = NumberFomatUtil.getBigDecimal(taxableValues);
		}
		BigDecimal rate = BigDecimal.ZERO;
		String rates = (document.getRate() != null) ? document.getRate().trim()
				: null;
		...
		String pos = (document.getPos() != null) ? document.getPos().trim()
				: null;
		...
		String igst = (document.getIgstAmt() != null)
		...
		if (document.getDiffPercent() != null
				...
				&& document.getDiffPercent().trim().equalsIgnoreCase("L65")) {
			if ((sgstin != null && pos != null)
				...
				if (sgstAmount.compareTo(intraL65taxAmount) != 0) {
					...
					errors.add(new ProcessingResult(APP_VALIDATION,
							ProcessingResultType.INFO, "IN0276",
				...
				if (cgstAmount.compareTo(intraL65taxAmount) != 0) {
					...
					TransDocProcessingResultLoc location = 
							...
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION,
							ProcessingResultType.INFO, "IN0277",
				...
				if (igstAmount.compareTo(interL65taxAmount) != 0) {
					...
					TransDocProcessingResultLoc location = 
							...
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION,
							ProcessingResultType.INFO, "IN0278",
