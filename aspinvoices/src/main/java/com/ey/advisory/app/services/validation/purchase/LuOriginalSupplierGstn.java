...
public class LuOriginalSupplierGstn implements DocRulesValidator<InwardTransDocument>{
	...
	public List<ProcessingResult> validate(InwardTransDocument document,
		...
		String s=document.getOrigSgstin().substring(0,1);
		
		int n=StatecodeRepository.findStateCode(s);
		if(n<=0){
		
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.OrgSgstin);
