 ...
 */
@Slf4j
@Component("DocSeriesJsonToCsvConverterAndUploaderService")
public class DocSeriesJsonToCsvConverterAndUploaderService {
	...
	@Autowired
	@Qualifier("ConvertDtoTocsv")
	private ConvertDtoTocsv convertDtoTocsv;

	public String convertAndUpload(InvDocSeriesHeaderReqDto dto, String cntrlPayloadId) {
		File tempDir = null;
		...
		try {
			...
			Gstr1FileStatusEntity fileStatus = gstr1FileUploadUtil
					.getErpFileStatus(filename, userName, null,
							GSTConstants.INVOICE, GSTConstants.GSTR1, null);
			...
			gstr1FileUploadJobInsertion.fileUploadJob(jsonParams,
					JobConstants.GSTR1INVOICEFILEUPLOAD, userName);
	...
	private static File createTempDir() throws IOException {
		return Files.createTempDirectory("DocSeriesFile").toFile();
	}
}
