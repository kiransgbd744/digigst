...
@Service("Gstr3BErrorReportsServiceImpl")
public class Gstr3BErrorReportsServiceImpl implements GSTR3BErrorReportService {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr3BErrorReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr3BErrorReportDaoImpl")
	private Gstr3BErrorReportDao gstr3BErrorReportDao;

	@Override
	public void generateFileStatusCsv(SearchCriteria criteria, PageRequest pageReq, String fullPath) {
		Anx1FileStatusReportsReqDto request = (Anx1FileStatusReportsReqDto) criteria;

		List<Gstr3bdownloadProcessedDto> responseFromDao = gstr3BErrorReportDao.generateErrorCsv(request);
		...
		if (responseFromDao != null && !responseFromDao.isEmpty()) {
			...
			try (Writer writer = new BufferedWriter(new FileWriter(fullPath), 8192)) {
				...
				invoiceHeadersTemplate = commonUtility.getProp("gstr3b.error.csv.report.headers");
				writer.append(invoiceHeadersTemplate);
				columnMappings = commonUtility.getProp("gstr3b.error.csv.report.headers.mapping").split(",");
