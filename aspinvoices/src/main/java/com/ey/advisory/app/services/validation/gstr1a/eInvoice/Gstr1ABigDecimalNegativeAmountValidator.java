...
@Component("Gstr1ABigDecimalNegativeAmountValidator")
public class Gstr1ABigDecimalNegativeAmountValidator
	...
	private static final List<String> ORGDOCNUM_REQUIRING_IMPORTS = ImmutableList
			.of(GSTConstants.CR, GSTConstants.RFV, GSTConstants.RCR,
					GSTConstants.RRFV, GSTConstants.AV, GSTConstants.RAV);
	...
	public List<ProcessingResult> validate(Gstr1AOutwardTransDocument document,
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			if (!BigDecimalNagativeValueUtil.validate(qty)) {
			...
			if (!BigDecimalNagativeValueUtil.validate(invoiceValue)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0084",
			...
			if (!BigDecimalNagativeValueUtil.validate(taxableValue)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0074",
						"Invalid Taxable Value", location));
			}
			...
			BigDecimal specificcessamountadj = item
					.getAdjustedCessAmtSpecific();
			...
			if (!BigDecimalNagativeValueUtil.validate(igstAmount)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0076",
			...
			if (!BigDecimalNagativeValueUtil.validate(cgstAmount)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0078",
			...
			if (!BigDecimalNagativeValueUtil.validate(sgstAmount)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0080",
			...
			if (!BigDecimalNagativeValueUtil.validate(advcessAmount)
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0082",
			...
			if (!BigDecimalNagativeValueUtil.validate(taxableValueadj)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0087",
						"Invalid Taxable Value Adjusted", location));
			}
			if (!BigDecimalNagativeValueUtil.validate(igstadj)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0088",
						"Invalid IGST Amount Adjusted", location));
			}
			if (!BigDecimalNagativeValueUtil.validate(cgstadj)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0089",
						"Invalid CGST Amount Adjusted", location));
			}
			if (!BigDecimalNagativeValueUtil.validate(sgstadj)) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0090",
						"Invalid SGST / UTGST Amount Adjusted", location));
			}
			if (!BigDecimalNagativeValueUtil.validate(advcessamountadj)) {
				List<String> errorLocations = new ArrayList<>();
				errorLocations.add(GSTConstants.AdvaloremCessAmountAdjusted);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0091",
						"Invalid Advalorem Cess Amount Adjusted", location));
			}
			if (!BigDecimalNagativeValueUtil.validate(specificcessamountadj)) {
				List<String> errorLocations = new ArrayList<>();
				errorLocations.add(GSTConstants.SpecificCessAmountAdjusted);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0092",
						"Invalid Specific Cess Amount Adjusted", location));
			}
			if (!BigDecimalNagativeValueUtil.validate(statecessamountadj)) {
				List<String> errorLocations = new ArrayList<>();
				errorLocations.add(GSTConstants.StateCessAmountAdjusted);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0093",
						"Invalid State Cess Amount Adjusted", location));
			}
