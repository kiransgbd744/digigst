...
@Service("ComprehensiveSRFileArrivalHandler")
@Slf4j
public class ComprehensiveSRFileArrivalHandler {
	...
	@Autowired
	@Qualifier("SRFileToComprehensiveOutwardTransDocConvertion")
	private SRFileToComprehensiveOutwardTransDocConvertion sRFileToComprehensiveTransDocConvertion;

	@Autowired
	@Qualifier("EInvoiceDefaultDocSaveService")
	private EInvoiceDocSaveService srFileSaveService;
	...
	@Autowired
	@Qualifier("SRFileToComprehensiveOutwardTransDocErrConvertion")
	private SRFileToComprehensiveOutwardTransDocErrConvertion srFileToOutwardTransDocErrConvertion;
...
*/
		...
		if (checkHeaderFormat.getValue0()) {
			...
			if (!documentMap.isEmpty()) {
				LOGGER.debug(
						...
						documentMap.size(), updateFileStatus.getId(),TenantContext.getTenantId());
			}
			PerfUtil.logEventToFile(PerfamanceEventConstants.FILE_PROCESSING,
	...
	private void saveDoc(DocumentKeyBuilder documentKeyBuilder,
		...
		try {
			List<OutwardTransDocument> documents = sRFileToComprehensiveTransDocConvertion
	...
	/*	asbProducerService.produceMessage("SAP_FILE_STATUS",
			...
			asbProducerService.produceMessage("SAP_SR_DOC", "DTLDSAPSRDOC",
					"DigiSAP", TenantContext.getTenantId(),
					updateFileStatus.getId(), null);
			asbProducerService.produceMessage("SAP_SR_ERROR", "DTLDSAPSRERROR",
					"DigiSAP", TenantContext.getTenantId(),
					updateFileStatus.getId(), null);*/
