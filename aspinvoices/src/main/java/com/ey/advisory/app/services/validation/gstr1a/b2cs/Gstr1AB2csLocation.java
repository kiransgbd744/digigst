...
public class Gstr1AB2csLocation
	...
	public List<ProcessingResult> validate(Gstr1AAsEnteredB2csEntity document,
		...
		EntityAtConfigKey entityAtConfigKey = new EntityAtConfigKey(
				document.getEntityId(), OnboardingConstant.LOCATION);
		...
		entitAtConfigInnerMap.entrySet().forEach(entitAtConfMap -> {
			...
			if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
				boolean isAttrValid = onboardAttributesChecking.isAttrValid(
						entityAtValMap, OnboardingConstant.LOCATION, loc,
						document.getEntityId());
				if (loc == null || loc.isEmpty()) {
					errorLocations.add(GSTConstants.LOCATION);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5040",
							" Location is not as per master provided "
									+ "during On-Boarding.",
							location));
				}
				if (!isAttrValid) {
			...
			if (OnboardingConstant.AT_O.equalsIgnoreCase(atOutward)) {
				...
				if (!isAttrValid) {
					...
					errors.add(new ProcessingResult(APP_VALIDATION,
							ProcessingResultType.INFO, "IN5004",
							" Location is not as per master "
									+ "provided during On-Boarding.",
							location));
		...
		if (loc != null && !loc.isEmpty()) {
			...
			if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
				...
				boolean isAttrUserValid = onboardAttributesChecking.isAttrValid(
						OnboardingConstant.PC, loc, document.getEntityId(),
						document.getCreatedBy());
				if (!isAttrUserValid) {
					errorLocations.add(GSTConstants.LOCATION);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5060",
							"Location is not as per user access level"
