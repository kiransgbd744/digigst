 ...
 *
 */
@Component("AsyncGstr8ProcessedRecordsServiceImpl")
@Slf4j
public class AsyncGstr8ProcessedRecordsServiceImpl
	...
	private static final String CHUNK_PROC = "USP_GSTR8_UPLOAD_PSD_INS_CHUNK";
	private static final String DISP_PROC = "USP_GSTR8_UPLOAD_PSD_DISP_CHUNK";
	...
	public void generateReports(Long id) {
		...
		try {
			...
			String fullPath = tempDir.getAbsolutePath() + File.separator
					+ "GSTR8_Processed_Records_" + entityName
			...
			StatefulBeanToCsv<Gstr8DownloadDto> beanWriter = getBeanWriterGstr8ProcRecords(
					reportConfig, writer);
			WritetoCsvGstr8Processedrecords(chunkCount, id, reportConvertor,
					reportType, beanWriter, reportCateg);
			...
			if (tempDir.list().length > 0) {
				...
				Pair<String, String> uploadedZipName = DocumentUtility
						.uploadFile(zipFile,
								ConfigConstants.GSTR8_DOWNLOAD_REPORT);
			...
			fileStatusDownloadReportRepo.updateStatus(id,
					...
					EYDateUtil.toUTCDateTimeFromLocal(LocalDateTime.now()));
			String msg = "Exception occued while while generating GSTR8 Processed File";
			LOGGER.error(msg, e);
			throw new AppException(msg, e);
	...
	private void WritetoCsvGstr8Processedrecords(Integer chunkNo, Long id,
			ReportConvertor reportConvertor, String reportType,
			StatefulBeanToCsv<Gstr8DownloadDto> beanWriter,
			String reportCateg) {

		List<Object[]> list = null;
		List<Gstr8DownloadDto> responseFromDao = null;
		...
		try {
			for (int i = 1; i <= chunkNo; i++) {
				...
				reportDataProc.registerStoredProcedureParameter("P_CHUNK_VAL",
						Integer.class, ParameterMode.IN);

				reportDataProc.setParameter("P_CHUNK_VAL", chunkNo);
				...
				if (list.size() > 0) {
					...
					responseFromDao = list.stream()
							.map(o -> (Gstr8DownloadDto) reportConvertor
	...
	private StatefulBeanToCsv<Gstr8DownloadDto> getBeanWriterGstr8ProcRecords(
		...
		ColumnPositionMappingStrategy<Gstr8DownloadDto> mappingStrategy = new ColumnPositionMappingStrategy<>();
		mappingStrategy.setType(Gstr8DownloadDto.class);
		mappingStrategy.setColumnMapping(
				env.getProperty(config.getColumnMappingsKey()).split(","));

		StatefulBeanToCsvBuilder<Gstr8DownloadDto> builder = new StatefulBeanToCsvBuilder<>(
				writer);
		StatefulBeanToCsv<Gstr8DownloadDto> beanWriter = builder
