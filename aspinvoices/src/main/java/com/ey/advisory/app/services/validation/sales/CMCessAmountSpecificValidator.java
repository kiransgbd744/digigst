...
@Component("CMCessAmountSpecificValidator")
public class CMCessAmountSpecificValidator implements 
DocRulesValidator<OutwardTransDocument>{
	private static final List<String> ORGDOCNUM_REQUIRING_IMPORTS = 
		ImmutableList.of(GSTConstants.CR, GSTConstants.RFV,GSTConstants.RCR);
	@Override
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		if(!ORGDOCNUM_REQUIRING_IMPORTS.contains(
			...
			if((BigDecimal.ZERO.compareTo(item.getCessAmountSpecific()) < 0)){
				...
				errors.add(new ProcessingResult(APP_VALIDATION, "ER083",
						"Invalid Cess Amount",
						location));
		}
			}				
