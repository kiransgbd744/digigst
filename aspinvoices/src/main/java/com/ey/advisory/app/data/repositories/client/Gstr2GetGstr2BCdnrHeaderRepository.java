...
@Repository("Gstr2GetGstr2BCdnrHeaderRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr2GetGstr2BCdnrHeaderRepository
		extends CrudRepository<Gstr2GetGstr2BCdnrHeaderEntity, Long>,
		JpaSpecificationExecutor<Gstr2GetGstr2BCdnrHeaderEntity> {

	@Modifying
	@Query("UPDATE Gstr2GetGstr2BCdnrHeaderEntity b SET b.isDelete = true, "
	...
	@Modifying
	@Query("UPDATE Gstr2GetGstr2BCdnrHeaderEntity b SET b.status = 'SAVED', "
			+ " b.modifiedOn =:modifiedOn WHERE b.invocationId =:invocationId"
			+ " AND b.status = 'ONHOLD'")
	int updateStatus(@Param("invocationId") Long invocationId,
			@Param("modifiedOn") LocalDateTime modifiedOn);
	
	@Query("from Gstr2GetGstr2BCdnrHeaderEntity where docKey=:docKey")
	List<Gstr2GetGstr2BCdnrHeaderEntity> findByInvoiceKey(
			@Param("docKey") String docKey);

	@Modifying
	@Query("UPDATE Gstr2GetGstr2BCdnrHeaderEntity b SET b.isDelete = true, b.modifiedOn =:modifiedOn  WHERE"
	...
	@Query("SELECT COUNT(1) FROM Gstr2GetGstr2BCdnrHeaderEntity where "
