 ...
 *
 */
@Service("Gstr6GetLateFeeSuccessHandler")
@Slf4j
public class Gstr6GetLateFeeSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr6LateFeeDataParserImpl")
	private Gstr6LateFeeDataParser gstr6LateFeeDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GET Call is Success for the batchId {} inside Gstr6GetLateFeeSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				if (APIConstants.LATEFEE.equalsIgnoreCase(dto.getType())) {
					lateFeeHeaderRepo.softlyDeleteLateFeeHeader(dto.getGstin(), dto.getReturnPeriod(), now);
				}
			}
			resultIds.forEach(id -> {
				...
				if (APIConstants.LATEFEE.equalsIgnoreCase(dto.getType())) {
					...
					List<GetGstr6LateFeeEntity> lateFeeEntities = gstr6LateFeeDataParser.parseLateFeeData(dto, apiResp,
							dto.getType(), ctxParamsObj.get("batchId").getAsLong(), now);
					if (lateFeeEntities != null && !lateFeeEntities.isEmpty()) {

						lateFeeHeaderRepo.saveAll(lateFeeEntities);
					}
				}
