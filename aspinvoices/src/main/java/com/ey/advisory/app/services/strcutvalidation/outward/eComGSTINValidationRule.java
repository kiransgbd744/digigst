...
public class eComGSTINValidationRule implements ValidationRule {
	 ...
	 private static final Logger LOGGER = LoggerFactory.getLogger(eComGSTINValidationRule.class);
	@Override
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isPresent(obj))
			return errors;
		if (GSTConstants.URP.equalsIgnoreCase(obj.toString().trim()))
			return errors;
		if (obj.toString().trim().length() != 15) {
			
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format("inside to the eComGSTINValidationRule and isValid mehod ");
				LOGGER.debug(msg);
			}
		...
		if (matcher.matches() || matcher1.matches()) {
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format("inside to the eComGSTINValidationRule and in isValid mehod and else condition  ");
				LOGGER.debug(msg);
			}
