...
@Component("gstr1ATxpTxpaDataAtGstnImpl")
@Slf4j
public class Gstr1ATxpTxpaDataAtGstnImpl implements Gstr1ATxpTxpaDataAtGstn {
	...
	public Long getGstr1TxpTxpaDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.TXP.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_TXPA);
			}
		}
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1ATxpTxpaSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
