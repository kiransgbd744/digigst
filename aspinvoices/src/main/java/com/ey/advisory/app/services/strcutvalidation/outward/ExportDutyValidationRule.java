...
public class ExportDutyValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx,Object obj, Object[] row,
		...
		if (!isPresent(obj)) return errors;
			...
			if (!isDecimal(obj.toString())) {
				...
				errorLocations.add(GSTConstants.ExportDuty);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0068",
						"Invalid Export Duty", location));
				// if it's not a number, then we can return the errors
				// immediately
				return errors;
			}
			boolean isValid = NumberFomatUtil.is12And2digValidDec(obj.toString());
			if (!isValid) {
				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(GSTConstants.ExportDuty);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
