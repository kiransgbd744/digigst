...
@Component("EinvoiceFileStatusProcessedReportDaoImpl")
public class EinvoiceFileStatusProcessedReportDaoImpl
		implements EinvoiceFilestatusDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(EinvoiceFileStatusProcessedReportDaoImpl.class);
	...
	@Override
	public List<DataStatusEinvoiceDto> getProcessedReports(
	...
	private DataStatusEinvoiceDto convertProcessed(Object[] arr) {
		...
		obj.setAspInformationId(arr[2] != null ? arr[2].toString() : null);
		obj.setAspInformationDesc(arr[3] != null ? arr[3].toString() : null);
		...
		/*
		 * if (arr[15] != null) { String strdate = arr[15].toString();
		 * DateTimeFormatter f = new DateTimeFormatterBuilder()
		 * .appendPattern(OLDFARMATTER).toFormatter(); LocalDate parsedDate =
		 * LocalDate.parse(strdate, f); DateTimeFormatter f2 =
		 * DateTimeFormatter.ofPattern(NEWFARMATTER); String newDate =
		 * parsedDate.format(f2); obj.setEwbDate(newDate); } else {
		 * obj.setEwbDate(null); }
		 */
		...
		obj.setSupplierStateCode(arr[43] != null ? arr[43].toString() : null);
		...
		obj.setCustomerStateCode(arr[53] != null ? arr[53].toString() : null);
		obj.setBillingPOS(arr[54] != null ? arr[54].toString() : null);
		...
		obj.setDispatcherStateCode(arr[63] != null ? arr[63].toString() : null);
		...
		// obj.setBatchExpiryDate(arr[94] != null ? arr[94].toString() : null);
		if (arr[80] != null) {
		...
		// obj.setWarrantyDate(arr[95] != null ? arr[95].toString() : null);
		if (arr[81] != null) {
		...
		// obj.setShippingBillDate(arr[150] != null ? arr[150].toString() :
		// null);
		if (arr[131] != null) {
		...
		// obj.setInvoicePeriodStartDate(arr[152] != null ? arr[152].toString()
		// : null);
		if (arr[133] != null) {
		...
		// obj.setInvoicePeriodEndDate(arr[153] != null ? arr[153].toString() :
		// null);
		if (arr[134] != null) {
		...
		// obj.setOriginalDocumentDate(arr[155] != null ? arr[155].toString() :
		// null);
		if (arr[136] != null) {
		...
		// obj.setPaymentDueDate(arr[179] != null ? arr[179].toString() : null);
		if (arr[156] != null) {
		...
		// obj.setTransportDocDate(arr[194] != null ? arr[194].toString() :
		// null);
		if (arr[170] != null) {
		...
		obj.setReturnPeriod(arr[174] != null ? arr[174].toString() : null);
		...
		// obj.setGlPostingDate(arr[248] != null ? arr[248].toString() : null);
		if (arr[226] != null) {
		...
		// obj.seteWBDate(arr[251] != null ? arr[251].toString() : null);
		if (arr[229] != null) {
			String strdate = arr[229].toString();
			DateTimeFormatter f = new DateTimeFormatterBuilder()
					.appendPattern(OLDFARMATTER).toFormatter();
			LocalDate parsedDate = LocalDate.parse(strdate, f);
			DateTimeFormatter f2 = DateTimeFormatter.ofPattern(NEWFARMATTER);
		...
		// obj.setAccountingVoucherDate(arr[253] != null ? arr[253].toString() :
		// null);
		if (arr[231] != null) {
	...
	private String createProcessedQueryString(String buildQuery) {
		return "SELECT TRIM(', ' FROM IFNULL(ERRH.ERROR_CODE_ASP,'') ||','|| "
				...
				+ "AS INFO_ERROR_DESCRIPTION_ASP,EINV.STATUS_DESCRIPTION "
				+ "AS IRN_STATUS, EINV.IRN AS IRN_NO,EINV.ACK_NUM AS IRN_ACK_NO,"
				+ "HDR.IRN_DATE AS IRN_ACK_DATE_TIME,"
				+ "EINV.SIGNED_QR AS SIGNED_QR_CODE, EINV.SIGNED_INV "
				+ "AS SIGNED_INVOICE, HDR.EINV_ERROR_CODE AS IRP_ERROR_CODE,"
				+ "HDR.EINV_ERROR_DESC AS IRP_ERROR_DESCRIPTION,"
				+ "HDR.EWB_STATUS AS EWBPartAStatusA,"
				+ " '' AS EWBValidUpTo, '' AS EWBNO, '' AS EWBDATE,"
				...
				+ "AS GSTIN_REFID_DATE_TIME, TRIM(', ' FROM "
				...
				+ " THEN HDR.AN_RETURN_TYPE ELSE HDR.RETURN_TYPE END "
				+ "AS RETURN_TYPE,"
				+ "CASE WHEN HDR.DERIVED_RET_PERIOD >= " + answer
				+ " THEN HDR.AN_TABLE_SECTION ELSE HDR.TABLE_SECTION END "
				...
				+ "HDR.DISPATCHER_STATE_CODE, '' AS SHIP_TO_FLOOR_NUM,"
				+ "HDR.SHIP_TO_TRADE_NAME,HDR.SHIP_TO_LEGAL_NAME,"
				+ "HDR.SHIP_TO_BUILDING_NUM,HDR.SHIP_TO_BUILDING_NAME,"
				+ "HDR.SHIP_TO_LOCATION,HDR.SHIP_TO_PINCODE,"
				+ "HDR.SHIP_TO_PINCODE AS SHIPTOSTATECODE,"
				...
				+ "ITM.STATE_CESS_SPECIFIC_AMOUNT,OTHER_VALUES,TOT_ITEM_AMT,"
				...
				+ "ITM.LINE_ITEM_AMT,HDR.ROUND_OFF,HDR.TOT_INV_VAL_WORLDS,"
				+ "HDR.TCS_FLAG_INCOME_TAX,ITM.TCS_RATE_INCOME_TAX,"
				+ "ITM.TCS_AMOUNT_INCOME_TAX,CUSTOMER_PAN_OR_AADHAAR,"
				...
				+ "HDR.CRDR_PRE_GST,HDR.CUST_SUPP_TYPE,"
				+ "HDR.CUST_SUPP_TYPE AS CUSTOMERCODE,"
				+ "ITM.PRODUCT_CODE,ITM.ITM_TYPE,ITM.ITC_FLAG,"
				+ "HDR.STATE_APPLYING_CESS,"
				+ "ITM.FOB,ITM.EXPORT_DUTY,HDR.EXCHANGE_RATE,ITM.CRDR_REASON,"
				...
				+ "HDR.EWAY_BILL_DATE,HDR.ACCOUNTING_VOUCHER_NUM,"
				...
				+ "HDR.MEMO_VALUE_CGST,"
				+ "HDR.MEMO_VALUE_SGST, "
				+ " '' AS CessAdvaloremAmount_ASP, '' AS StateCessAdvaloremAmount_ASP,"
				...
				+ "GSTR1_GSTN_SAVE_BATCH GSB ON "
				+ "GSB.ID = HDR.BATCH_ID LEFT OUTER JOIN "
