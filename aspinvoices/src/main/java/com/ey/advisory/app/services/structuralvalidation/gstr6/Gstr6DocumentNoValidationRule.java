...
public class Gstr6DocumentNoValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isPresent(obj)) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER3029",
		...
		if (obj.toString().matches("-?\\d+(\\.\\d+)?([E-e]-?\\d+)?")) {
			String mobileDecimalFormatStr = (String
					.valueOf(obj.toString().trim())).trim();
			...
			if (mobileNo.length() > 16) {
				...
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						null, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER3018",
