...
@Slf4j
@Component("PRSummarySplitReportCommonService")
public class PRSummarySplitReportCommonService {
	...
	private static final String GSTR2_6_CHUNK_ROWS = "gstr2.gstr6.report.chunk.rows";
	private static final String CONF_CATEG = "RECON_RESULT_REPORTS";

	public Integer getChunkingSizes() {

		Config config = configManager.getConfig(CONF_CATEG, GSTR2_6_CHUNK_ROWS);
	...
	public void chunkZipFiles(File tempDir, Long reportId, Long configId,
		...
		if (tempDir.isDirectory() && tempDir.list().length == 0) {
			LOGGER.info("PRSummarySplitReportCommonService Directory is empty");
			return;
		}
	...
	private String getIndexNumber(String fileName) {
		...
		if (fileName.startsWith("InvoiceMgt_InwardData")) {
			index = name[2];
		} else if (fileName.startsWith("InvoiceMgt_data")) {
			index = name[2];
		} else if (fileName.startsWith("GSTR2_Consolidated_ASP_Error_Report")) {
			index = name[5];
		} else if (fileName.startsWith("PR Records_Recon Type Tagging_")){//PR Records_Recon Type Tagging_id_time
			index = name[4];
		}
		return index;
