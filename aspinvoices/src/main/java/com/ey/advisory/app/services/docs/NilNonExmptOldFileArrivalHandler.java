...
import com.ey.advisory.app.services.gstr1fileupload.Gstr1NilNonExtOldStructuralProcess;
...
@Service("NilNonExmptOldFileArrivalHandler")
@Slf4j
public class NilNonExmptOldFileArrivalHandler {
	@Autowired
	...
	@Autowired
	@Qualifier("Gstr1NilNonExtOldStructuralProcess")
	private Gstr1NilNonExtOldStructuralProcess gstr1NilNonExtStructuralProcess;

	@Autowired
	@Qualifier("NilHeaderOldCheckService")
	...
	public void processNilFile(Message message, AppExecContext context) {
		/*
		...
		try {
			...
			LOGGER.debug("Exception occured in Nil File Arrival Processor", e);
		}

		TabularDataSourceTraverser traverser = traverserFactory
				.getTraverser(fileName);
		// TabularDataLayout layout = new DummyTabularDataLayout(1, 2, 0, 14);
		...
		for (Object[] obj : listOfNil) {
			obj[4] = obj[4] != null && !obj[4].toString().isEmpty()
						? String.valueOf(obj[4]).toUpperCase() : null;
		}
		...
		if (checkHeaderFormat.getValue0()) {
			gstr1NilNonExtStructuralProcess
					.nilNonExtStructureProcessData(listOfNil, updateFileStatus);
		} else {
