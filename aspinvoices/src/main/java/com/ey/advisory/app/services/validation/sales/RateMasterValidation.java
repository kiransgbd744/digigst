 ...
 *
 */
@Component("RateMasterValidation")
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		if (paramtrvalue.equalsIgnoreCase(
			...
			IntStream.range(0, items.size()).forEach(idx -> {
				...
				if (map == null) {
					...
					if (paramtrvalue9 != null && paramtrvalue9.equalsIgnoreCase(
						...
						errors.add(
								new ProcessingResult(APP_VALIDATION, "ER0147",
						...
						if(sgstRate.compareTo(BigDecimal.ZERO) > 0){
							...
							TransDocProcessingResultLoc location 
									...
									idx, errorLocations.toArray());
							errors.add(
									new ProcessingResult(APP_VALIDATION, "ER0149",
						...
						if(cgstRate.compareTo(BigDecimal.ZERO) > 0){
							...
							TransDocProcessingResultLoc location 
									...
									idx, errorLocations.toArray());
							errors.add(
									new ProcessingResult(APP_VALIDATION, "ER0148",
					...
					else if (paramtrvalue9 != null && paramtrvalue9.equalsIgnoreCase(
							CONFIG_PARAM_OUTWARD_ANS_KEY_ID.B.name())) {
						if(igstRate.compareTo(BigDecimal.ZERO) > 0){
						Set<String> errorLocations = new HashSet<>();
						errorLocations.add(GSTConstants.IGST_RATE);
			...
			Boolean cgstvalid=	ProductMasterRateAndHsnValUtil
			          .isValidRate(map,cgstRatemul,document.getSgstin());
			Boolean sgstvalid=	ProductMasterRateAndHsnValUtil
			          .isValidRate(map,sgstRatemul,document.getSgstin());
