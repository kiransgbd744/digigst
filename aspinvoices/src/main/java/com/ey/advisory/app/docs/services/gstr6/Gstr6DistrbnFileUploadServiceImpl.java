...
@Component("Gstr6DistrbnFileUploadServiceImpl")
public class Gstr6DistrbnFileUploadServiceImpl
		implements DistributionFileUploadService {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr6DistrbnFileUploadServiceImpl.class);
			...
			for (MultipartFile file : files) {
				...
				if (!file.isEmpty()) {
					...
					String paramJson = "{\"filePath\":\"" + createdFolder.getName() + "\","
			                  ...
			                  + "\"docId\":\"" + docId + "\"}";
					if (uniqueName
							.equalsIgnoreCase(GSTConstants.GSTR6_RAW_FILE_NAME)) {
						String jobCategory = JobConstants.GSTR6DISTRIBUTIONFILEUPLOAD;
					...
					for (Long jobId : findByJobIds) {
						...
						Gstr1FileStatusEntity fileStatus = gstr1FileUploadUtil
								.getGstr6FileStatus(fileName, userName,
										asyncExceJob, fileType, dataType, null, docId);
