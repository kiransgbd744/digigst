...
@Component("Gstr7ProcessedReportDaoImpl")
public class Gstr7ProcessedReportDaoImpl implements Gstr7ProcessedReportsDao {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr7ProcessedReportDaoImpl.class);
	...
	@Override
	public List<Object> getGstr7RSReports(SearchCriteria criteria) {
	...
	private String createApiProcessedQueryString(String buildQuery) {
		...
		return "SELECT (CASE WHEN TDS.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' "
				...
				+ "TDS.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) "
				...
				+ "TRIM(', ' FROM IFNULL(GSTN_ERROR_DESCRIPTION,'')) AS GSTN_ERROR_DESCRIPTION,"
				+ "TDS.TABLE_NUM,TDS.RETURN_PERIOD,ACTION_TYPE, "
				+ "TDS_DEDUCTOR_GSTIN,ORG_TDS_DEDUCTEE_GSTIN,ORG_RETURN_PERIOD,"
				+ "ORG_GROSS_AMT, NEW_TDS_DEDUCTEE_GSTIN," + "NEW_GROSS_AMT,IGST_AMT,"
				+ "CGST_AMT,SGST_AMT, " + "CONTRACT_NUMBER,CONTRACT_DATE,CONTRACT_VALUE,"
				+ "PAYMENT_ADV_NUM,PAYMENT_ADV_DATE, DOC_NUM,DOC_DATE,"
				+ "INVOICE_VALUE, PLANT_CODE,DIVISION,PURCHASE_ORGANIZATION ,"
				+ "PROFIT_CENTRE1,PROFIT_CENTRE2, USERDEFINED_FIELD1,USERDEFINED_FIELD2,"
				+ "USERDEFINED_FIELD3, FIL.SOURCE SOURCE,FIL.CREATED_BY USER_ID, "
				+ "FIL.ID FILE_ID,FIL.FILE_NAME FILE_NAME,FIL.UPLOAD_START_TIME "
				...
				+ "GSTNBATCH.ID = TDS.BATCH_ID WHERE TDS.IS_DELETE=FALSE "
				+ "AND ACTION_TYPE IS NULL " + buildQuery;
