...
@Service("Anx1ErrorRecordsServiceImpl")
public class Anx1ErrorReportsServiceImpl implements Anx1ErrorReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ErrorReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1ErrorReportsDaoImpl")
	private Anx1ErrorReportsDao anx1ErrorReportsDao;
	...
	public Workbook findError(SearchCriteria criteria, PageRequest pageReq) {
		...
		List<Anx1ErrorReportsView> responseFromView = new ArrayList<>();
		responseFromView = anx1ErrorReportsDao.getErrorReports(request);
		...
		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.error.report.headers").split(",");
			
			LOGGER.debug("workbook properties reading starting ");
			
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_ErrorsReport.xlsx");
