 ...
 *
 */
@Component("GSTR7TransOriginalInvoiceValue")
public class GSTR7TransOriginalInvoiceValue
	...
	public List<ProcessingResult> validate(Gstr7TransDocHeaderEntity document,
		...
		if (GSTConstants.RNV.equalsIgnoreCase(document.getDocType())) {
			BigDecimal originalInvoiceValue = document
					.getOriginalInvoiceValue();
			if (originalInvoiceValue == null) {
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						null, errorLocations.toArray());
				errorLocations.add(GSTConstants.ORIGINAL_INVOICE_VALUE);
				errors.add(new ProcessingResult(APP_VALIDATION, "ER63047",
						"Original Invoice Value cannot be left blank",
						location));
			}
		}
		return errors;
