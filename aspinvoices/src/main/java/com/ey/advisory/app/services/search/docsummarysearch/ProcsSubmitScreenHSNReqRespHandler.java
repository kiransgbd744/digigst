 ...
 */
@Slf4j
@Service("ProcsSubmitScreenHSNReqRespHandler")
public class ProcsSubmitScreenHSNReqRespHandler {

	
	public static final String hsnconst = "HSN";
	
	@Autowired
	@Qualifier("ProcesSubmitHSNSummarySearchService")
	ProcesSubmitHSNSummarySearchService service;
	...
	public List<Gstr1SummaryScreenRespDto> handleGstr1HsnReqAndResp(
		...
		for (Gstr1CompleteSummaryDto dto : list) {
			...
			if (hsnSummary != null) {
				hsnSummary.forEach(hsnEySummary -> {
					...
					summaryResp
							.setAspTaxableValue(hsnEySummary.getAspTaxableValue());
					summaryResp.setAspTaxPayble(hsnEySummary.getAspTaxPayble());
					summaryResp.setAspInvoiceValue(hsnEySummary.getAspInvoiceValue());
					...
					summaryResp.setGstnTaxableValue(hsnEySummary.getGstnTaxableValue());
					summaryResp.setGstnTaxPayble(hsnEySummary.getGstnTaxPayble());
					summaryResp.setGstnInvoiceValue(hsnEySummary.getGstnInvoiceValue());
		...
		List<Gstr1SummaryScreenRespDto> handleHSNResp = respHandler.pvsSubmitB2BResp(hsnSummaryRespList, "HSN");
