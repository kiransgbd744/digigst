...
public class AdjustementReferenceNoValidationRule implements ValidationRule {
...
if(obj.toString().matches("-?\\d+(\\.\\d+)?([E-e]-?\\d+)?")){
	...
	if(mobileNo.length() > 16){
		...
		errorLocations.add(GSTConstants.ADJREFNO);
		TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
				idx, errorLocations.toArray());
		errors.add(new ProcessingResult(APP_VALIDATION, "ER1089",
				"Invalid Adjustement Reference No", location));
		return errors;
