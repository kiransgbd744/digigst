...
import static com.ey.advisory.common.GSTConstants.AvailableSgstamount;
...
public class AvailableSgst implements DocRulesValidator<InwardTransDocument> {
	...
	if(document.getDocType()!=null && !document.getDocType().isEmpty()){
		if(document.getSupplyType()!=null 
			...
			if((!ORGDOCNUM_REQUIRING_IMPORTS.contains(
				...
				if(item.getAvailableSgst().compareTo(BigDecimal.ZERO) < 0){
					
			
			errorLocations.add(AvailableSgstamount);
			TransDocProcessingResultLoc location = 
					                  new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER108",
					"Invalid Available SGST",
					location));
		}
