...
@Service("Anx1ProcessedRecordsServiceImpl")
public class Anx1ProcessedRecordsServiceImpl
		implements Anx1ProcessedReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ProcessedRecordsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1ProcessedReportsDaoImpl")
	private Anx1ProcessedReportsDao anx1ProcessedReportsDao;
	...
	public Workbook findProcesseRec(SearchCriteria criteria,
		...
		List<Anx1ProcessedRecordsView> responseFromView = new ArrayList<>();
		responseFromView = anx1ProcessedReportsDao.getProcessedReports(request);
		
		LOGGER.debug("Outward file status processed response", responseFromView);
		
		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.processed.report.headers").split(",");
			
			LOGGER.debug("workbook properties reading starting ");
			
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_processedRecReport.xlsx");
