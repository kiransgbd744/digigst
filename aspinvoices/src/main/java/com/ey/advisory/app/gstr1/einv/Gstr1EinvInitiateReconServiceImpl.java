...
@Service("Gstr1EinvInitiateReconServiceImpl")
@Slf4j
public class Gstr1EinvInitiateReconServiceImpl
		implements Gstr1EinvInitiateReconService {
	...
	Gstr1SubmittedReconConfigRepository gstr1SubmReconrepo;
	...
	Gstr1PRvsSubReconGstinDetailsRepository gstr1PrSubmReconGstin;
	...
	public List<Gstr1EinvInitiateReconGstinDetailsDto> getGstins(
		...
		Map<String, Gstr1EinvoiceAndReconStatusDto> reconStatusMap = getReconstatus(
				listGstins, taxPeriod);
		List<Gstr1EinvInitiateReconGstinDetailsDto> gstnList = new ArrayList<>();

		if (!gstins.isEmpty()) {
			gstins.forEach(e -> {

				Gstr1EinvInitiateReconGstinDetailsDto dto = new Gstr1EinvInitiateReconGstinDetailsDto();
	...
	@Override
	public String initiatPRvsSubmRecon(List<String> gstinlist, String fromTaxPeriod,String toTaxPeriod,
			Long entityId){

		if (LOGGER.isDebugEnabled()) {
			String msg = "Inside InitiateReconserviceimpl"
					+ " method";
			LOGGER.debug(msg);
		}
		try {
			...
			Gstr1PrVsSubmittedReconConfigEntity entity = new Gstr1PrVsSubmittedReconConfigEntity();
			...
			Gstr1PrVsSubmittedReconConfigEntity obj = gstr1SubmReconrepo.save(entity);

			// saving in child table
						
			
			List<Gstr1PRvsSubmReconGstinDetailEntity> reconGstinObjList = gstinlist
					.stream()
					.map(o -> new Gstr1PRvsSubmReconGstinDetailEntity(o,
							obj.getReconConfigId(),fromReturnPeriod,toReturnPeriod,isActive))
					.collect(Collectors.toList());
			...
			String groupCode = TenantContext.getTenantId();
			asyncJobsService.createJob(groupCode,
					JobConstants.GSTR1_PR_SUBMITTED_INITIATE_RECON,
					jsonParams.toString(), userName, 1L, null, null);
		} catch (Exception ex) {
	...
	public List<Gstr1EinvRequesIdWiseDownloadTabDto> getDownloadData(
		...
		List<Gstr1EinvRequesIdWiseDownloadTabDto> respList = listObj.stream()
...
List<Gstr1EinvRequesIdWiseDownloadTabDto> list= new ArrayList<>();
		...
		gstr1ReqIdWiseDownloDto.setReportName("Processed Records Vs Submitted Data");
		if(filePath!=null)
	...
	private Gstr1EinvRequesIdWiseDownloadTabDto convertToDto(Object[] arr) {

		Gstr1EinvRequesIdWiseDownloadTabDto dto = new Gstr1EinvRequesIdWiseDownloadTabDto();
	...
	public Map<String, Gstr1EinvoiceAndReconStatusDto> getReconstatus(
		...
		Map<String, Gstr1EinvoiceAndReconStatusDto> recStatusMap = convertToMap(
				reconStatusList);
	...
	private Map<String, Gstr1EinvoiceAndReconStatusDto> convertToMap(
			List<Object[]> reconStatusList) {

		Map<String, Gstr1EinvoiceAndReconStatusDto> statusMap = new HashMap<>();
