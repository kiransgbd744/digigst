 ...
 *
 */
@Component("gstr1AtAtaDataAtGstnImpl")
@Slf4j
public class Gstr1AtAtaDataAtGstnImpl implements Gstr1AtAtaDataAtGstn {
	...
	public Long findAtAtaDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.AT.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_ATA);
			}
		}
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1AtAtaSuccessHandler", "Gstr1GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
		 ...
		 * "failed to get Gstr1 AT from Gstn"; if (LOGGER.isErrorEnabled()) {
