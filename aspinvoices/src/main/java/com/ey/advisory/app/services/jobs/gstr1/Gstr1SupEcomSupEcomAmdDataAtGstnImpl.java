 ...
 *
 */
@Component("Gstr1SupEcomSupEcomAmdDataAtGstnImpl")
@Slf4j
public class Gstr1SupEcomSupEcomAmdDataAtGstnImpl
		implements Gstr1SupEcomSupEcomAmdDataAtGstn {
	...
	@Override
	public Long findSupEcomDataATGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.SUPECO.equalsIgnoreCase(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_SUPECO, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_SUPECO);
			}
		} else {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_SUPECO_AMD, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_SUPECO_AMD);
			}
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1SupEcomSupEcomASuccessHandler",
				"Gstr1GetCommonFailureHandler", jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
