...
@Component("Anx1AspLineOutwardSavableTransDaoImpl")
public class Anx1AspLineOutwardSavableTransDaoImpl
		implements Anx1AspLineOutwardSavableSummaryDao {
	...
	@Override
	public List<Object> getLineOutwardSavableReports(SearchCriteria criteria) {
		...
		String queryStr = createinwardQueryString(buildQuery.toString());
	...
	private Anx1ProcessedRecordsAspOutwardDto convertProcessedInward(
			Object[] arr) {
	...
	private String createinwardQueryString(String buildQuery) {
		...
		return "SELECT HDR.USER_ID,HDR.SOURCE_FILENAME,"
				...
				+ "HDR.ORIGINAL_DOC_TYPE, ITM.ORIGINAL_DOC_NUM AS"
				...
				+ " ITM.EXPORT_DUTY, ITM.ITM_HSNSAC,ITM.PRODUCT_CODE,"
				...
				+ " ITM.LINE_ITEM_AMT AS ITM_INV_VALUE,ITM.ADJ_REF_NO, "
				...
				+ "HDR.EWAY_BILL_NUM, HDR.EWAY_BILL_DATE, HDR.AN_RETURN_TYPE AS"
				+ " RETURN_TYPE, HDR.AN_TAX_DOC_TYPE AS DATA_CATEGORY,"
				+ " HDR.AN_TABLE_SECTION AS TABLE_NUMBER, TRIM(', ' "
				+ "FROM IFNULL(ERRH.INFO_ERROR_CODE_ASP,'') ||','|| "
				...
				+ " INFO_ERROR_DESCRIPTION_ASP,HDR.DOC_AMT AS INV_VALUE_ASP,"
				+ " ITM.MEMO_VALUE_IGST, ITM.MEMO_VALUE_CGST, ITM.MEMO_VALUE_SGST,"
				+ " ITM.IGST_RET1_IMPACT, ITM.CGST_RET1_IMPACT,"
				+ " ITM.SGST_RET1_IMPACT, case when HDR.IS_SAVED_TO_GSTN"
				+ " = TRUE THEN 'IS_SAVED' WHEN HDR.IS_SAVED_TO_GSTN = FALSE "
				+ "AND HDR.GSTN_ERROR = TRUE THEN 'IS_ERROR' WHEN"
				+ " HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = "
				+ "FALSE THEN 'NOT_SAVED' END AS SAVE_STATUS,"
				+ " GSB.GSTN_SAVE_REF_ID AS GSTIN_REF_ID,GSB.GSTN_RESP_DATE "
				+ "AS GSTIN_REF_ID_TIME, TRIM(', ' FROM IFNULL"
				...
				+ " AS ASP_DATE_TIME FROM  ANX_OUTWARD_DOC_HEADER"
				...
				+ " ITM.ITEM_INDEX = ERRI.ITEM_INDEX AND ITM.ITM_NO = "
				+ "ERRI.ITM_NO INNER JOIN  FILE_STATUS FIL ON HDR.FILE_ID="
				+ "FIL.ID LEFT OUTER JOIN  GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID"
				+ " = HDR.BATCH_ID WHERE IS_ERROR = FALSE AND AN_RETURN_TYPE = 'ANX1' "
