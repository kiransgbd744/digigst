...
@Component("InwardRevhSummarysectionDaoImpl")
public class InwardRevhSummarysectionDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(InwardRevhSummarysectionDaoImpl.class);
	...
	public List<Annexure1SummarySectionDto> loadBasicSummarySection(
		...
		LOGGER.debug("Building Where Condition For ReverseCharge Section " + buildQuery);
		}

		String queryStr = createQueryString(buildQuery);
		if(LOGGER.isDebugEnabled()){
		LOGGER.debug("Vertical Query For Summary Level  Reverse Charge " + queryStr);
		}
		try {
			...
			LOGGER.debug("Converting to ResultSet To List ");
	...
	private String createQueryString(String buildQuery) {
		...
		LOGGER.debug(
				"Executing Query For ReverseCharge Vertical Upload BEGIN ");
		/*String queryStr = "SELECT 'SLF' DOC_TYPE,'3H' AS TABLE_SECTION,"
				...
				+ "COUNT(INVKEY_GSTN_3H_3I) AS RECORDS_COUNT "
				+ "FROM ANX_PROCESSED_3H_3I WHERE TRAN_FLAG='RC' "
		...
		String queryStr = "SELECT DOC_TYPE,TABLE_SECTION,"
				...
				+ "SUM(CESS) CESS,COUNT(*) RECORD_COUNT "
				+ "FROM ( SELECT 'SLF' DOC_TYPE,(CASE WHEN TRAN_FLAG='RC' "
				+ "THEN '3H' END) AS TABLE_SECTION,"
