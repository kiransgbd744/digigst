 ...
 *
 */
@Component("Gstr2aB2bFileUploadServiceImpl")
public class Gstr2aB2bFileUploadServiceImpl
		implements Gstr2aB2bFileUploadService {
	public static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr2aB2bFileUploadServiceImpl.class);
	...
	public ResponseEntity<String> uploadDocuments(MultipartFile[] files,
		...
		LOGGER.info(
				"Inside get gstr2a file upload method and uniqueName is {}  ",
				uniqueName);
		...
		try {
			...
			if (!anxList.isEmpty() && anxList.size() > 0) {
				{
					for (MultipartFile file : files) {
						...
						if (!file.isEmpty()) {
							...
							fileStatus.setFileType(GSTConstants.GSTR2A_B2B);
							...
							if (uniqueName.equalsIgnoreCase(
									GSTConstants.GSTR2A_B2B_FILE_NAME)) {
								String jobCategory = JobConstants.GSTR2AB2BFILEUPLOAD;
							 ...
							 * asyncExceJob =
							 * asyncExecJobRepository.findByJobDetails(jobId);
							 * 
					...
					/*
					 * Gstr1GetInvoicesReqDto dtos = new
					 * Gstr1GetInvoicesReqDto(); GSTNDetailEntity entity =
					 * gstnDetailRepo
					 * .findByGstinAndIsDeleteFalse(dtos.getGstin()); if
					 * (entity.getEntityId() == null) {
					 * 
					 * APIRespDto dto = new APIRespDto("Failed",
					 * "GSTR-2A file uploaded Failed"); JsonObject resp = new
					 * JsonObject(); JsonElement respBody =
					 * gson.toJsonTree(dto); String msg =
					 * " Gstin is not onboarded"; resp.add("hdr", new Gson()
					 * .toJsonTree(new APIRespDto("E", msg))); resp.add("resp",
					 * respBody); return new ResponseEntity<>(resp.toString(),
					 * HttpStatus.OK); }
					 */
					APIRespDto dto = new APIRespDto("Success",
							"GSTR-2A file uploaded successfully");
					JsonObject resp = new JsonObject();
					JsonElement respBody = gson.toJsonTree(dto);
					resp.add("hdr",
			...
			JsonObject resp = new JsonObject();
			JsonElement respBody = gson.toJsonTree(dto);
			String msg = "Unexpected error while uploading GSTR-2A file";
			resp.add("hdr",
	...
	private Gstr1GetInvoicesReqDto createBatchAndSave(
		...
		Map<String, Config> configMap = configManager.getConfigs("GSTR2A",
				"gstr2a.manual.upload");
		boolean isManualUplaod = configMap.get("gstr2a.manual.upload") == null
				? false
				: Boolean.valueOf(
						configMap.get("gstr2a.manual.upload").getValue());
		...
		if (isManualUplaod) {
			...
			if ("B".equals(paramValue)) {
				...
				// reconStatus.setMonthlyGet(Boolean.FALSE);
				reconStatus.setGetEvent("D");// Daily
		...
		batchRepo.softlyDelete(dto.getType().toUpperCase(),
				APIConstants.GSTR2A_UPLOAD.toUpperCase(), dto.getGstin(),
				dto.getReturnPeriod());
		try {
			GetAnx1BatchEntity batch = batchUtil.makeBatchGstr1(dto,
					dto.getType(), APIConstants.GSTR2A_UPLOAD.toUpperCase());
			// Save new Batch
