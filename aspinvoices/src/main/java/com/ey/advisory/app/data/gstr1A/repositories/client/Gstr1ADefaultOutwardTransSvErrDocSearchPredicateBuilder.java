 ...
 * search Structurally Validated Error OutwardTransDocument objects, based on
 * the SearchCriteria provided. Since there can be multiple null/empty fields in
 * the search criteria, it is highly inefficient to provide several combinations
 * of repository query methods to cover all scenarios. Instead, this class uses
 * the JPA crieria builder class to build dynamic predicates based on the usable
 * fields in the input criteria. The return predicate list will be used by the
 * repository method to implement the search.
 * 
 * @author Shashikant.Shukla
 *
 */
@Component("Gstr1ADefaultOutwardTransSvErrDocSearchPredicateBuilder")
public class Gstr1ADefaultOutwardTransSvErrDocSearchPredicateBuilder
		implements Gstr1AOutwardTransSvErrDocSearchPredicateBuilder {
	...
	public List<Predicate> build(DocSearchReqDto criteria,
		...
		Predicate docTypeNotEmptyPredicate = criteriaBuilder
		...
		Predicate docDateNotEmptyPredicate = criteriaBuilder
		...
		Predicate sgstinNotEmptyPredicate = criteriaBuilder
				.notLike(criteriaBuilder.trim(root.get("sgstin")), empty);
		...
		Predicate docNumNotEmptyPredicate = criteriaBuilder
				.notLike(criteriaBuilder.trim(root.get("docNo")), empty);
