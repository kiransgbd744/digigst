 ...
 *
 */
@Service("Gstr1VerticalErrorReportsServiceImpl")
public class Gstr1VerticalErrorReportsServiceImpl
		implements Gstr1VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1VerticalErrorReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1VerticalErrorReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		LOGGER.debug("Gstr1 vertical error response" + responseFromView);
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((DownloadReportsConstant.B2CS)
				...
				String[] invoiceHeaders = commonUtility
						.getProp("gstr1.vertical.b2cS.error.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Gstr1_B2csVerticalErrorReport.xlsx");
				...
				errorDumpCells.importCustomObjects(responseFromView,
					...
					.equalsIgnoreCase(request.getFileType())) {
				String[] invoiceHeaders = commonUtility
						.getProp("gstr1.vertical.advrec.error.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Gstr1_AdvRecVerticalErrorReport.xlsx");
				...
				errorDumpCells.importCustomObjects(responseFromView,
					...
					.equalsIgnoreCase(request.getFileType())) {
				String[] invoiceHeaders = commonUtility
						.getProp("gstr1.vertical.advadj.error.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Gstr1_AdvAdjVerticalErrorReport.xlsx");
				...
				errorDumpCells.importCustomObjects(responseFromView,
					...
					.equalsIgnoreCase(request.getFileType())) {
				String[] invoiceHeaders = commonUtility
						.getProp("gstr1.vertical.invoice.error.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Gstr1_InvoiceVerticalErrorReport.xlsx");
