...
@Component("GoodsPDFGenerationReportService")
@Slf4j
public class GoodsPDFGenerationReportService {

	@Autowired
	@Qualifier("GoodsServicesPDFDaoImpl")
	GoodsServicePdf goodsServicePdf;

	public String generateGoodsPdfZip(File tempDir,
		...
		for (PdfPrintReqDto req : request) {
			...
			try (OutputStream outStream = new BufferedOutputStream(
				...
				JasperPrint jasperPrint = goodsServicePdf.generatePdfReport(id,
						docNo, sgstin);
		...
		String zipFileName = zipGoodsPdfFiles(tempDir);
		return zipFileName;
	}

	private String zipGoodsPdfFiles(File tempDir) throws Exception {
