 ...
 *
 */
@Service("Anx1VerticalErrorReportsServiceImpl")
public class Anx1VerticalErrorReportsServiceImpl
		implements Anx1VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1VerticalErrorReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1VerticalErrorReportsDaoImpl")
	...
	public Workbook findReports(SearchCriteria criteria, PageRequest pageReq) {
		...
		LOGGER.debug("vertical error response" + responseFromView);
		...
		if (responseFromView != null && responseFromView.size() > 0) {
			...
			if ((DownloadReportsConstant.B2C)
				...
				String[] invoiceHeaders = commonUtility
						.getProp(
								"anx1.vertical.b2c.error.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");
				
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_B2CVerticalError.xlsx");
				...
				String[] invoiceHeaders = commonUtility
						.getProp(
								"anx1.vertical.3h3i.error.report.headers")
						.split(",");
				
				LOGGER.debug("workbook properties reading starting ");
				
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_3H3IVerticalError.xlsx");
				...
				String[] invoiceHeaders = commonUtility
						.getProp(
								"anx1.vertical.table4.error.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");
				
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_Table4VerticalError.xlsx");
