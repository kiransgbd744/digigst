...
@Component("Gstr1VerticalTotalReportsDaoImpl")
public class Gstr1VerticalTotalReportsDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1VerticalTotalReportsDaoImpl.class);
	...
	public List<Object> getGstr1VerticalReports(
		...
		if(!StringUtils.isEmpty(dataType) && !dataType.equalsIgnoreCase("GSTR1A"))
	...
	private String createVerticalTotalQueryString(String buildQuery) {
		...
		if ((DownloadReportsConstant.B2CS).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "NEW_TAXABLE_VALUE,NEW_ECOM_GSTIN,"
					+ "NEW_ECOM_SUP_VAL,IGST_AMT,CGST_AMT,SGST_AMT,"
					+ "CESS_AMT,TOT_VAL,PROFIT_CENTRE,PLANT_CODE,"
					+ "DIVISION,LOCATION,SALES_ORGANIZATION,"
					...
					+ "HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID "
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "ORG_GROSS_ADV_RECEIVED,NEW_POS,NEW_RATE,"
					+ "NEW_GROSS_ADV_RECEIVED,IGST_AMT,CGST_AMT,"
	                ...
	                + "TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) "
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM GSTR1_AS_ENTERED_ADV_RECEVIED HI "
					+ "LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() ERR "
					+ "ON HI.ID = ERR.COMMON_ID AND HI.FILE_ID=ERR.FILE_ID "
			...
			queryStr = "SELECT HI.ID,SUPPLIER_GSTIN,RETURN_PERIOD,"
					+ "TRAN_TYPE,MONTH,ORG_POS,ORG_RATE,"
					+ "ORG_GROSS_ADV_ADJUSTED,NEW_POS,NEW_RATE,"
					+ "NEW_GROSS_ADV_ADJUSTED,IGST_AMT,"
	                ...
	                + "TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) "
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM GSTR1_AS_ENTERED_ADV_ADJUSTMENT HI "
					+ "LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() ERR "
					+ "ON HI.ID = ERR.COMMON_ID AND "
					+ "HI.FILE_ID = ERR.FILE_ID LEFT OUTER JOIN "
					+ "FILE_STATUS FIL ON HI.FILE_ID=FIL.ID "
					+ "AND ERR.FILE_ID=FIL.ID AND ERR.INV_KEY=HI.TXPD_INVKEY "
					+ "WHERE " + buildQuery;

		}

		else if ((DownloadReportsConstant.INVOICE).equalsIgnoreCase(fileType)) {

			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,"
					+ "SERIAL_NUM,NATURE_OF_DOC,TOT_NUM,CANCELED,"
					+ "NET_NUM,DOC_SERIES_FROM,DOC_SERIES_TO,"
					+ "ERROR_CODE_ASP,ERROR_DESCRIPTION_ASP"
					+ " FROM GSTR1_AS_ENTERED_INV_SERIES HI "
					+ "LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() ERR "
	...
	private String createGstr1aVerticalTotalQueryString(String buildQuery) {
		...
		if ((DownloadReportsConstant.B2CS).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,"
	                ...
	                + "TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) "
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ " FROM GSTR1A_AS_ENTERED_B2CS HI "
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,"
	                ...
	                + "TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) "
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM GSTR1A_AS_ENTERED_ADV_RECEVIED HI "
					+ "LEFT OUTER JOIN TF_GSTR1A_ERROR_INFO() ERR "
					+ "ON HI.ID = ERR.COMMON_ID AND HI.FILE_ID=ERR.FILE_ID "
					+ "LEFT OUTER JOIN FILE_STATUS FIL "
					+ "ON HI.FILE_ID=FIL.ID " + "AND ERR.FILE_ID=FIL.ID "
			...
			queryStr = "SELECT HI.ID,SUPPLIER_GSTIN,RETURN_PERIOD,"
	                ...
	                + "TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) "
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM GSTR1A_AS_ENTERED_ADV_ADJUSTMENT HI "
					+ "LEFT OUTER JOIN TF_GSTR1A_ERROR_INFO() ERR "
					+ "ON HI.ID = ERR.COMMON_ID AND "
					+ "HI.FILE_ID = ERR.FILE_ID LEFT OUTER JOIN "
					+ "FILE_STATUS FIL ON HI.FILE_ID=FIL.ID "
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "NET_NUM,DOC_SERIES_FROM,DOC_SERIES_TO,"
					+ "ERROR_CODE_ASP,ERROR_DESCRIPTION_ASP"
					+ " FROM GSTR1A_AS_ENTERED_INV_SERIES HI "
					+ "LEFT OUTER JOIN TF_GSTR1A_ERROR_INFO() ERR "
					+ "ON HI.ID = ERR.COMMON_ID "
					+ "AND HI.FILE_ID = ERR.FILE_ID "
					+ "LEFT OUTER JOIN FILE_STATUS FIL "
