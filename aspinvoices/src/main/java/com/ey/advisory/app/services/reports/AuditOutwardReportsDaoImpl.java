 ...
 *
 */
@Component("AuditOutwardReportsDaoImpl")
public class AuditOutwardReportsDaoImpl implements AuditOutwardReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(AuditOutwardReportsDaoImpl.class);
	...
	public List<Object> getAudOutwardReports(SearchCriteria criteria) {
		...
		if (gstin != null && !gstin.isEmpty()) {
			buildHeader.append(" AND HDR.SUPPLIER_GSTIN  = :gstin ");
		}
	...
	private DataStatusEinvoiceDto convertProcessed(Object[] arr, int i) {
		...
		if (arr[26] != null) {
			Timestamp timeStamp1 = (Timestamp) arr[26];

			LocalDateTime irndate = timeStamp1.toLocalDateTime();
			String dateTime = formatter1.format(irndate).toString();

			obj.setIrnDate(dateTime);
		} else {
			obj.setIrnDate(null);
		}
		...
		if (arr[313] != null) {
			Timestamp timeStamp1 = (Timestamp) arr[313];
		...
		String createdBy = arr[267] != null ? arr[267].toString() : null;

		String dataOriginTypeCode = arr[315] != null ? arr[315].toString()
				: null;

		String userid = arr[200] != null ? arr[200].toString() : null;
		...
		obj.setProcessFreq(arr[314] != null ? arr[314].toString() : null);
	...
	private String createApiProcessedQueryString(String buildHeader) {
		...
		return "SELECT TRIM(',' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| IFNULL(ITM.ERROR_CODES,'')) AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, TRIM(',' FROM IFNULL(HDR.INFORMATION_CODES,'')||','|| "
				+ "IFNULL(ITM.INFORMATION_CODES,'')) AS INFO_ERROR_CODE_ASP, '' AS INFO_ERROR_DESCRIPTION_ASP, CASE WHEN HDR.IRN_STATUS = 1 "
				+ "THEN 'NOT OPTED' WHEN HDR.IRN_STATUS = 2 THEN 'NOT APPLICABLE' WHEN HDR.IRN_STATUS = 3 THEN 'PENDING' "
				+ "WHEN HDR.IRN_STATUS = 4 THEN 'GENERATION ERROR' WHEN HDR.IRN_STATUS = 5 THEN 'GENERATED' WHEN HDR.IRN_STATUS = 6 "
				+ "THEN 'CANCELLED' WHEN HDR.IRN_STATUS = 7 THEN 'ASP ERROR' WHEN HDR.IRN_STATUS = 8 THEN 'DUPLICATE_IRN' "
				...
				+ "CASE WHEN HDR.EWB_STATUS = 1 THEN 'NOT APPLICABLE' WHEN HDR.EWB_STATUS = 2 THEN 'PENDING' WHEN HDR.EWB_STATUS = 3 "
				...
				+ "WHEN HDR.EWB_STATUS = 11 THEN 'ASP ERROR' WHEN HDR.EWB_STATUS = 12 THEN 'NOT_OPTED' END AS EWBPARTASTATUSA, "
				...
				+ "THEN 'ASPERROR' WHEN HDR.ASP_INVOICE_STATUS=2 THEN 'ASPPROCESSED' END AS DIGI_GST_STATUS, HDR.INFO_ERROR_CODE "
				...
				+ "ITM.TCS_AMT AS TCSAMOUNT, HDR.CUST_SUPP_ADDRESS3, HDR.TAX_DOC_TYPE AS DATA_CATEGORY,ITM.ITM_UQC,ITM.ITM_QTY,HDR.ID,HDR.CREATED_ON,"
				+ "DENSE_RANK() OVER ( PARTITION BY HDR.DOC_NUM,DOC_KEY ORDER BY ITM.DOC_HEADER_ID ASC ) AS PROCESSING_FREQUENCY,DATAORIGINTYPECODE "
				+ "FROM ANX_OUTWARD_DOC_HEADER HDR INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID "
				...
				+ "EINV_MASTER EINV ON HDR.IRN_RESPONSE = EINV.IRN LEFT OUTER JOIN EWB_MASTER EWB ON HDR.EWB_NO_RESP = EWB.EWB_NUM "
