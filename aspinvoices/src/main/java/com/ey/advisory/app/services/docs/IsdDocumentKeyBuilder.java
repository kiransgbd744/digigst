 ...
 */
@Slf4j
public class IsdDocumentKeyBuilder implements DataBlockKeyBuilder<String> {
	...
	public String buildDataBlockKey(Object[] arr, TabularDataLayout config) {
		String isdGtin = (arr[0] != null && !arr[0].toString().isEmpty())
				? String.valueOf((arr[0])).trim() : "";

		String supplierGstin = (arr[1] != null && !arr[1].toString().isEmpty())
				? String.valueOf((arr[1])).trim() : "";

		String documentType = (arr[2] != null && !arr[2].toString().isEmpty())
				?String.valueOf(arr[2]).trim() : "";
		String documentNumber = (arr[3] != null && !arr[3].toString().isEmpty())
					?String.valueOf(arr[3]).trim() : "";
		Object documentDate = (arr[4] != null && !arr[4].toString().isEmpty())
					? arr[4] : "";
		String itemSerialNumber = (arr[5] != null && !arr[5].toString().isEmpty())
					? String.valueOf(arr[5]).trim() : "";
		String gstinforDistribution = (arr[6] != null && !arr[6].toString().isEmpty())
					? String.valueOf(arr[6]).trim() : "";


		LocalDate localDocDate = DateUtil.parseObjToDate(documentDate);
		return new StringJoiner(DOC_KEY_JOINER).add(isdGtin).add(supplierGstin)
				.add(documentType).add(documentNumber).
				add(localDocDate==null?null:localDocDate.toString()).add(itemSerialNumber)
				.add(gstinforDistribution).toString();
	}
