...
import com.ey.advisory.app.data.views.client.Gstr1GstnAdvanceReceivedAmendmentDto;
 ...
 */
	...
	@Component("Gstr1AGetATASummaryTablesDaoImpl")
	@Slf4j
	public class Gstr1AGetATASummaryTablesDaoImpl implements Gstr1AaGetDao {
		...
		public List<Object> getGstnConsolidatedReports(SearchCriteria criteria) {
			...
			String queryStr = creategstnATAQueryString(
					buildQuery.toString());
			...
			return list.parallelStream().map(o -> convertgstnATASummary(o))
					.collect(Collectors.toCollection(ArrayList::new));
		}
		private Gstr1GstnAdvanceReceivedAmendmentDto convertgstnATASummary(
				Object[] arr) {
			Gstr1GstnAdvanceReceivedAmendmentDto obj = new Gstr1GstnAdvanceReceivedAmendmentDto();
		...
		private String creategstnATAQueryString(String buildQuery) {

			return "select ID,GSTIN,RETURN_PERIOD,ORG_MONTH,"
					+ "SUPPLY_TYPE,ORG_SUPPLY_TYPE,STATE_NAME,POS,ORG_POS,"
					...
					+ "from (SELECT HDR.ID,HDR.GSTIN,HDR.RETURN_PERIOD,HDR.ORG_MONTH,"
					+ " '' SUPPLY_TYPE,SUPPLY_TYPE ORG_SUPPLY_TYPE,"
					...
					+ " 'ADV REC-A' AS TABLE_TYPE "
					+ "FROM GETGSTR1A_ATA_HEADER HDR INNER JOIN GETGSTR1A_ATA_ITEM ITM "
