...
@Service("Gstr1B2CLASectionReportServiceImpl")
public class Gstr1B2CLASectionReportServiceImpl
		implements Gstr1ReviewSummReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1B2CLASectionReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr1B2CLASectionReportDaoImpl")
	...
	public Workbook findGstr1ReviewSummRecords(SearchCriteria criteria,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 B2CLA processed data item level response"
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1 B2CLA isActiveFieldSel response"
			...
			if (responseFromView != null && responseFromView.size() > 0) {
				...
				if (isActiveFieldSel == null || isActiveFieldSel.isEmpty()) {
					...
					String msg = String
							.format("B2BCLA Gstr1_Asp_Processed_TransactionalReport :"
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug("Gstr1 B2CLA custom file name -"
			...
			LOGGER.error(
					"Exception Occuered in B2CLA Gstr1_Asp_Processed_TransactionalReport Report {}",
					ex);
