 ...
 * search Structurally Validated Error InwardTransDocument objects, 
 ...
 *
 */
@Component("DefaultInwardTransSvErrDocSearchPredicateBuilder")
public class DefaultInwardTransSvErrDocSearchPredicateBuilder
		implements InwardTransSvErrDocSearchPredicateBuilder {
	...
	public List<Predicate> build(DocSearchReqDto criteria,
		...
		//Select Records where Recipient GSTIN is not null and not empty
		Predicate cgstinNotNullPredicate = criteriaBuilder
				.isNotNull(root.get("cgstin"));
		Predicate cgstinNotEmptyPredicate = criteriaBuilder
				.notLike(criteriaBuilder.trim(root.get("cgstin")), empty);
		predicates.add(criteriaBuilder.and(cgstinNotNullPredicate,
				cgstinNotEmptyPredicate));
