...
@Component("Gstr1AUserAccess5")
public class Gstr1AUserAccess5
	...
	public List<ProcessingResult> validate(Gstr1AOutwardTransDocument document,
		...
		String ua5 = document.getUserAccess5();
		if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
			...
			EntityAtConfigKey entityAtConfigKey = new EntityAtConfigKey(
					document.getEntityId(), OnboardingConstant.UD5);
			...
			entitAtConfigInnerMap.entrySet().forEach(entitAtConfMap -> {
				...
				if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
					boolean isAttrValid = onboardAttributesChecking.isAttrValid(
							entityAtValMap, OnboardingConstant.UD5, ua5,
							document.getEntityId());
					if (ua5 == null || ua5.isEmpty()) {
						errorLocations.add(GSTConstants.USER_ACCESS5);
						TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
								null, errorLocations.toArray());
						errors.add(
								new ProcessingResult(APP_VALIDATION, "ER0136",
										"User Access 5 is not as per master "
				...
				if (OnboardingConstant.AT_O.equalsIgnoreCase(atOutward)) {
					...
					if (!isAttrValid) {
						...
						errors.add(new ProcessingResult(APP_VALIDATION,
								ProcessingResultType.INFO, "IN0020",
								"User Access 5 is not as per master "
										+ "provided during On-Boarding.",
								location));
		...
		if (ua5 != null && !ua5.isEmpty()) {
			...
			if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
				...
				boolean isAttrUserValid = onboardAttributesChecking.isAttrValid(
						OnboardingConstant.UD4, ua5, document.getEntityId(),
						document.getCreatedBy());
				if (!isAttrUserValid) {
					errorLocations.add(GSTConstants.USER_ACCESS5);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER0163",
							"User Access 5 is not as per user access level"
