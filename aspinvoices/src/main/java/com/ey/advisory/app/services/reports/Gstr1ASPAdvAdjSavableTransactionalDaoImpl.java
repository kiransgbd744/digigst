 ...
 */
@Slf4j
@Component("Gstr1ASPAdvAdjSavableTransactionalDaoImpl")
public class Gstr1ASPAdvAdjSavableTransactionalDaoImpl
		implements Gstr1ASPAdvAdjSavableTransactionalDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1ASPAdvAdjSavableTransactionalDaoImpl.class);
	...
	public List<Object> getGstr1AdjSavableReports(SearchCriteria criteria) {
		...
		if (taxperiod != null && !taxperiod.isEmpty()) {
			...
			buildQuery.append(
					" AND HDR.DERIVED_RET_PERIOD BETWEEN :fromTaxPeriod AND :toTaxPeriod ");
		}
		
		String queryStr = null; 

		if (APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
			 queryStr = createAdjSavableTransactionalQueryStringForGstr1a(
						buildQuery.toString());

		} else {
			queryStr = createAdjSavableTransactionalQueryString(
					buildQuery.toString());
		}
	...
	private String createAdjSavableTransactionalQueryString(
		...
		return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, "
				...
				+ " AND HDR.IS_DELETE=FALSE AND HDR.TAX_DOC_TYPE IN ('TXP') ";

	}
	
	private String createAdjSavableTransactionalQueryStringForGstr1a(
			String buildQueryheader) {
