 ...
 *
 */
@Service("ProcsSubmitScreenHsnSectionDaoImpl")
public class ProcsSubmitScreenHsnSectionDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ProcsSubmitScreenHsnSectionDaoImpl.class);
	...
	public List<Gstr1SummaryCDSectionDto> loadBasicSummarySection(
		...
		if (StringUtils.isNotEmpty(taxPeriodFrom1)
			...
			build2.append(" AND DERIVED_RET_PERIOD BETWEEN  :taxPeriodFrom AND :taxPeriodTo ");
	...
	/*	String queryStr = "SELECT TAX_DOC_TYPE,SUM(ASP_COUNT)ASP_COUNT,"
				...
				+ "COUNT(DISTINCT (HSNSAC || '-' || ITM_UQC)) as ASP_COUNT,"
				+ "SUM(TAXABLE_VALUE) AS ASP_TAXABLE_VALUE,"
				+ "SUM(TOTAL_TAX) AS ASP_TOTAL_TAX,"
				+ "SUM(TOTAL_VALUE) AS ASP_TOTAL_VALUE,"
				+ "SUM(IGST_AMT) AS ASP_IGST_AMT,"
				+ "SUM(CGST_AMT) AS ASP_CGST_AMT,"
				+ "SUM(SGST_AMT) AS ASP_SGST_AMT,"
				+ "SUM(CESS_AMT) AS ASP_CESS_AMT,"
				+ "0 AS GSTN_COUNT,0 AS GSTN_TAXABLE_VALUE,"
				+ "0 AS GSTN_TOTAL_TAX,0 AS GSTN_TOTAL_VALUE,"
				+ "0 AS GSTN_IGST_AMT,0 AS GSTN_CGST_AMT,"
				+ "0 AS GSTN_SGST_AMT,0 AS GSTN_CESS_AMT "
				+ "FROM GSTR1_SUBMITTED_PS_TRANS "
				+ "WHERE RECORD_TYPE ='ASP' AND TAX_DOC_TYPE ='HSN_DIGI' "
				+ "AND TABLE_NAME IN ('ANX_OUTWARD_DOC_HEADER','GSTR1_PROCESSED_B2CS') AND "
				+ buildQuery
				+ " UNION ALL "
				+ "SELECT 'HSN' AS TAX_DOC_TYPE,"
				+ "COUNT(DISTINCT (HSNSAC || '-' || ITM_UQC)) as ASP_COUNT,"
				...
				+ buildQuery2
				+ " UNION ALL "
				+ "SELECT 'HSN' AS TAX_DOC_TYPE,0 as ASP_COUNT,"
				+ "0 AS ASP_TAXABLE_VALUE,0 AS ASP_TOTAL_TAX,"
				+ "0 AS ASP_TOTAL_VALUE,0 AS ASP_IGST_AMT,"
				+ "0 AS ASP_CGST_AMT,0 AS ASP_SGST_AMT,"
				+ "0 AS ASP_CESS_AMT,"
				+ "COUNT(DISTINCT (HSNSAC || '-' || ITM_UQC)) AS GSTN_COUNT,"
...
*/
		String queryStr = "SELECT TAX_DOC_TYPE,SUM(ASP_COUNT)ASP_COUNT,"
                ...
                + "SUM(SGST_AMT) AS GSTN_SGST_AMT,"
                + "SUM(CESS_AMT) AS GSTN_CESS_AMT,"
                + "tax_rate FROM GSTR1_SUBMITTED_PS_TRANS "
                + "WHERE RECORD_TYPE ='GSTN' "
                + "AND TAX_DOC_TYPE ='HSN_GSTN' AND "
                + buildQuery2
                + "group by tax_rate,"
