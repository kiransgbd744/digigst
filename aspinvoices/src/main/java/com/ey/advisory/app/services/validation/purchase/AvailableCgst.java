...
import static com.ey.advisory.common.GSTConstants.AvailableCgstamount;
...
public class AvailableCgst  implements DocRulesValidator<InwardTransDocument> {
	...
	if(document.getDocType()!=null && !document.getDocType().isEmpty()){
		if(document.getSupplyType()!=null 
			...
			if((!ORGDOCNUM_REQUIRING_IMPORTS.contains(
				...
				if(item.getAvailableCgst().compareTo(BigDecimal.ZERO) < 0){
					
			
			errorLocations.add(AvailableCgstamount);
			TransDocProcessingResultLoc location = 
					                  new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER107",
					"Invalid Available CGST",
					location));
		}
