...
import com.ey.advisory.app.data.views.client.Anx1AspRevTotSumDto;
...
@Component("Anx1ASPRCSavableTotalSummaryDaoImpl")
public class Anx1ASPRCSavableTotalSummaryDaoImpl
		implements Anx1ASPRCSavableSummaryDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ASPRCSavableTotalSummaryDaoImpl.class);
	...
	public List<Object> getAnx1RCSavableReports(SearchCriteria criteria) {
		...
		String queryStr = createRCSavableTotalQueryString(
				buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertrcSummary(o))
				.collect(Collectors.toCollection(ArrayList::new));

	}

	private Anx1AspRevTotSumDto convertrcSummary(Object[] arr) {
		Anx1AspRevTotSumDto obj = new Anx1AspRevTotSumDto();
	...
	private String createRCSavableTotalQueryString(String buildQuery) {

		return "SELECT RETURN_TYPE,CUST_GSTIN,RETURN_PERIOD,"
				+ "TRAN_FLAG,SUPPLIER_GSTIN_PAN,SUPPLIER_NAME,DIFF_PERCENT,"
				+ "SEC7_OF_IGST_FLAG,AUTOPOPULATE_TO_REFUND,POS,HSNORSAC,"
				+ "SUM(TAXABLE_VALUE),TAX_RATE,SUM(IGST_AMT),SUM(TOTAL_VALUE),"
				+ "sum(CGST_AMT),SUM(SGST_AMT),SUM(CESS_AMT),ELGBL_INDICATOR,"
				+ "sum(AVAIL_IGST),sum(AVAIL_CGST),sum(AVAIL_SGST),"
				+ "sum(AVAIL_CESS),PROFIT_CENTRE,PLANT,DIVISION,LOCATION,"
				...
				+ "FROM ((SELECT HDR.AN_RETURN_TYPE AS RETURN_TYPE,"
				+ "HDR.CUST_GSTIN,HDR.RETURN_PERIOD,"
				+ " 'RC' AS TRAN_FLAG,HDR.SUPPLIER_GSTIN AS SUPPLIER_GSTIN_PAN,"
				+ "HDR.CUST_SUPP_NAME AS SUPPLIER_NAME,HDR.DIFF_PERCENT,"
				+ "HDR.SECTION7_OF_IGST_FLAG AS SEC7_OF_IGST_FLAG,"
				+ "HDR.AUTOPOPULATE_TO_REFUND,HDR.POS,ITM.ITM_HSNSAC AS HSNORSAC,"
				+ "ITM.TAXABLE_VALUE, ITM.TAX_RATE,ITM.IGST_AMT,ITM.CGST_AMT,"
				+ "ITM.SGST_AMT,SUM(IFNULL(ITM.CESS_AMT_SPECIFIC,'0')+ IFNULL"
				+ "(ITM.CESS_AMT_ADVALOREM,'0')) AS CESS_AMT,"
				+ "ITM.ELIGIBILITY_INDICATOR AS ELGBL_INDICATOR,"
				+ "ITM.AVAILABLE_IGST AS AVAIL_IGST,ITM.AVAILABLE_CGST "
				+ "AS AVAIL_CGST,ITM.AVAILABLE_SGST AS AVAIL_SGST,"
				+ "ITM.AVAILABLE_CESS AS AVAIL_CESS,ITM.PROFIT_CENTRE,"
				+ "ITM.PLANT_CODE AS PLANT, HDR.DIVISION,ITM.LOCATION,"
				+ "HDR.PURCHASE_ORGANIZATION AS PURCHAGE_ORG,HDR.USERACCESS1 "
				+ "AS USER_ACCESS1, HDR.USERACCESS2 AS USER_ACCESS2,"
				+ "HDR.USERACCESS3 AS USER_ACCESS3,HDR.USERACCESS4 "
				+ "AS USER_ACCESS4, HDR.USERACCESS5 AS USER_ACCESS5,"
				+ "HDR.USERACCESS6 AS USER_ACCESS6,ITM.USERDEFINED_FIELD1 AS "
				+ "USERDEFINED1, ITM.USERDEFINED_FIELD2 AS USERDEFINED2,"
				...
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSB ON "
				+ "GSB.ID = HDR.BATCH_ID WHERE IS_ERROR=FALSE AND "
				+ "HDR.AN_RETURN_TYPE IN ('ANX1') AND AN_TABLE_SECTION = '3H' "
				+ "group by HDR.AN_RETURN_TYPE ,HDR.CUST_GSTIN,"
				+ "HDR.RETURN_PERIOD,HDR.DOC_TYPE,HDR.CUST_SUPP_NAME,"
				+ "HDR.DERIVED_RET_PERIOD,"
				+ "HDR.DIFF_PERCENT,HDR.SECTION7_OF_IGST_FLAG,HDR.SUPPLIER_GSTIN,"
				+ "HDR.AUTOPOPULATE_TO_REFUND,ITM.TAXABLE_VALUE,"
				+ "ITM.TAX_RATE,ITM.IGST_AMT,ITM.CGST_AMT,ITM.SGST_AMT,"
				+ "ITM.ELIGIBILITY_INDICATOR,ITM.AVAILABLE_IGST,"
				+ "ITM.AVAILABLE_CGST,ITM.AVAILABLE_SGST,"
				+ "ITM.AVAILABLE_CESS,ITM.PROFIT_CENTRE,ITM.PLANT_CODE,"
				...
				+ "IS_SAVED_TO_GSTN,GSTN_ERROR,ITM.USERDEFINED_FIELD3,"
				+ "ERRH.INFO_ERROR_CODE_ASP, ERRI.INFO_ERROR_CODE_ASP,"
				+ "ERRH.INFO_ERROR_DESCRIPTION_ASP,ERRI.INFO_ERROR_DESCRIPTION_ASP,"
				+ "GSB.GSTN_SAVE_REF_ID , GSB.GSTN_RESP_DATE,"
				+ "ERRH.ERROR_CODE_GSTIN,ERRI.ERROR_CODE_GSTIN,"
				...
				+ "HI.SGST_AMT,HI.CESS_AMT,HI.ELGBL_INDICATOR,"
				+ "HI.AVAIL_IGST,HI.AVAIL_CGST,AVAIL_SGST,HI.AVAIL_CESS,"
				+ "HI.PROFIT_CENTER,HI.PLANT,HI.DIVISION,HI.LOCATION,"
				+ "HI.PURCHAGE_ORG, HI.USER_ACCESS1,HI.USER_ACCESS2,"
				+ "HI.USER_ACCESS3, HI.USER_ACCESS4,HI.USER_ACCESS5,"
				+ "HI.USER_ACCESS6, HI.USERDEFINED1,HI.USERDEFINED2,"
				+ "HI.USERDEFINED3, ERR.INFO_ERROR_CODE_ASP AS INFO_CODE_ASP,"
				...
				+ "AND HI.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END AS SAVE_STATUS,"
				+ "BT.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, BT.GSTN_RESP_DATE "
				+ "AS GSTIN_REF_ID_TIME, '' AS ERROR_CODE_GSTIN,'' "
				+ "AS ERROR_DESCRIPTION_GSTIN,FIL.SOURCE AS UPLOAD_SOURCE,"
				+ "HI.CREATED_BY AS SOURCE_ID,FIL.FILE_NAME,HI.CREATED_ON "
				+ "AS ASP_DATE_TIME ,HI.TOTAL_VALUE,"
				+ "HI.DERIVED_RET_PERIOD FROM ANX_PROCESSED_3H_3I "
				+ "HI LEFT OUTER JOIN TF_GSTR1_ERROR_INFO () ERR "
				+ "ON HI.AS_ENTERED_ID= ERR.COMMON_ID AND HI.FILE_ID=ERR.FILE_ID "
				+ "AND ERR.INV_KEY = HI.INVKEY_3H_3I LEFT OUTER JOIN "
				+ "FILE_STATUS FIL ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH BT "
				+ "ON HI.BATCH_ID = BT.ID AND HI.FILE_ID = BT.ID AND "
				+ "HI.DERIVED_RET_PERIOD = BT.DERIVED_RET_PERIOD "
				+ "WHERE HI.IS_DELETE = FALSE AND HI.TRAN_FLAG ='RC')) "
				+ buildQuery + " GROUP BY "
				...
				+ "HSNORSAC,TAX_RATE,ELGBL_INDICATOR,PROFIT_CENTRE,"
				+ "PLANT,DIVISION,LOCATION,PURCHAGE_ORG,USER_ACCESS1,USER_ACCESS2,"
				...
				+ "SAVE_STATUS,GSTIN_REF_ID,GSTIN_REF_ID_TIME,ERROR_CODE_GSTIN,"
				+ "ERROR_DESCRIPTION_GSTIN,UPLOAD_SOURCE, SOURCE_ID,"
