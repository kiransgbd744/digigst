...
@Component("GetGstr2aSuccessStatusFetchDaoImpl")
public class GetGstr2aSuccessStatusFetchDaoImpl
		implements GetGstr2aSuccessStatusFetchDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(GetGstr2aSuccessStatusFetchDaoImpl.class);
	...
	public List<Object[]> getDataUploadedStatusDetails(String gstin,
		...
		buffer.append(
				"SELECT * FROM ( SELECT * FROM (SELECT GSTIN,RETURN_PERIOD,DENSE_RANK() "
						...
						+ "API_SECTION='GSTR2A' AND IS_DELETE=FALSE AND GSTIN = :gstin");
		...
		buffer.append(" ) A WHERE NUM = 1 UNION "
				...
				+ " WHERE API_SECTION='GSTR2A' AND IS_DELETE=FALSE AND STATUS IN ('SUCCESS','SUCCESS_WITH_NO_DATA') AND GSTIN = :gstin");
		if (StringUtils.isNotBlank(taxPeriod)) {
			buffer.append(" AND RETURN_PERIOD = :taxPeriod");
		}
