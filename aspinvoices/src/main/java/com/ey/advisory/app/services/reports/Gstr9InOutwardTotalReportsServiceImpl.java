 ...
 *
 */
@Service("Gstr9InOutwardTotalReportsServiceImpl")
@Slf4j
public class Gstr9InOutwardTotalReportsServiceImpl
	...
	@Autowired
	@Qualifier("Gstr9InOutwardTotalReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Gstr9 InOutward total response" + responseFromView);
		}

		if (responseFromView != null && !responseFromView.isEmpty()) {

			if ((GSTConstants.INWARD_OUTWARD).equalsIgnoreCase(request.getFileType())) {  

				String[] invoiceHeaders = commonUtility
						.getProp("gstr9.inoutward.vertical.total.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
				...
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"GSTR_9_Inward_Outward_Total.xlsx");
				if (LOGGER.isDebugEnabled()) {
