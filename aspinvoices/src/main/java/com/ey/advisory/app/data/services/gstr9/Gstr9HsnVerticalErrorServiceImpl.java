 ...
 *
 */
@Service("Gstr9HsnVerticalErrorServiceImpl")
public class Gstr9HsnVerticalErrorServiceImpl
		implements Gstr7VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr9HsnVerticalErrorServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr9HsnVerticalErrorReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("gstr9.asp.error.hsn.report.headers").split(",");
