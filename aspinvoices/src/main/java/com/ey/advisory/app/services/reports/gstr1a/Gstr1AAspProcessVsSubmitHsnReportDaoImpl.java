 ...
 * 
 */
@Component("Gstr1AAspProcessVsSubmitHsnReportDaoImpl")
@Slf4j
public class Gstr1AAspProcessVsSubmitHsnReportDaoImpl
	...
	public List<Object> aspProcessVsSubmitDaoReports(SearchCriteria criteria) {
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(list)) {
				...
				for (Object arr[] : list) {
					verticalHsnList.add(convertProcessed(arr, hsnMap,
							rateIncludedInHsn, context));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private HsnProcessSubmitdto convertProcessed(Object[] arr,
			Map<String, String> hsnMap, Boolean rateIncludedInHsn,
		...
		if (rateIncludedInHsn) {
			...
			obj.setIsFiled(isGstinTaxperiodFiled(context, arr[1].toString(),
					arr[2].toString()));
			/*
			 * obj.setGstnSupplierGSTIN(arr[15] != null ? arr[15].toString() :
			 * null); obj.setGstnReturnPeriod(arr[16] != null ?
			...
			obj.setGstnTotalQuantity(
					arr[19] != null ? arr[19].toString() : null);
			obj.setGstnTaxableValue(
	...
	private String createApiProcessedQueryString(String buildHeader,
		...
		if (rateIncludedInHsn) {
			...
			build.append(
					"A.GSTN_ITM_DESCRIPTION,A.GSTN_TAX_RATE,A.GSTN_ITM_UQC,");
			build.append(
					"sum(A.GSTN_ITM_QTY),sum(A.GSTN_TAXABLE_VALUE),sum(A.GSTN_IGST_AMT),");
			build.append(
					"sum(A.GSTN_CGST_AMT),sum(A.GSTN_SGST_AMT),sum(A.GSTN_CESS_AMT),");
			build.append(
					"sum(A.GSTN_TOT_VAL),A.IS_FILED,A.DG_ITM_HSNSAC,A.DG_ITM_DESCRIPTION,");
			build.append(
					"A.DG_TAX_RATE,A.DG_ITM_UQC,sum(A.DG_ITM_QTY),sum(A.DG_TAXABLE_VALUE),");
			build.append(
					"sum(A.DG_IGST_AMT),sum(A.DG_CGST_AMT),sum(A.DG_SGST_AMT),");
			build.append(
					"sum(A.DG_CESS_AMT),sum(A.DG_TOT_VAL), A.UI_ITM_HSNSAC,A.UI_ITM_DESCRIPTION,");
			build.append(
					"A.UI_TAX_RATE,A.UI_ITM_UQC,sum(A.UI_ITM_QTY),sum(A.UI_TAXABLE_VALUE),");
			build.append(
					"sum(A.UI_IGST_AMT),sum(A.UI_CGST_AMT),sum(A.UI_SGST_AMT),sum(A.UI_CESS_AMT),");
			build.append(
					"sum(A.UI_TOT_VAL),sum(A.DIFF_TAXABLE_VALUE),sum(A.DIFF_IGST_AMT),");
			build.append(
					"sum(A.DIFF_CGST_AMT),sum(A.DIFF_SGST_AMT),sum(A.DIFF_CESS_AMT),");
			build.append("max(A.SAVE_STATUS),Max( ");
			build.append(
					"CASE WHEN A.SAVE_STATUS = 'IS_SAVED' THEN GSTN_SAVE_REF_ID ELSE NULL END ");
			build.append(
					") AS GSTIN_REF_ID, max( CASE WHEN A.SAVE_STATUS = 'IS_SAVED' THEN TO_CHAR( ");
			build.append(
					"ADD_SECONDS(BATCH_DATE, 19800), 'DD-MM-YYYY HH24:MI:SS' ) ELSE NULL END ");
			build.append(
					") AS GSTIN_REF_ID_TIME,ERROR_CODE,ERROR_DESC,A.DERIVED_RET_PERIOD ");
			build.append(
					"from ( with GSTN as ( SELECT PS.SUPPLIER_GSTIN,PS.RETURN_PERIOD, ");
			build.append("PS.DERIVED_RET_PERIOD,HSNSAC AS GSTN_ITM_HSNSAC,");
			build.append(
					"HSN_DESCRIPTION AS GSTN_ITM_DESCRIPTION, TAX_RATE GSTN_TAX_RATE,");
			build.append(
					"(CASE WHEN LEFT(HSNSAC, 2) = '99' then 'NA' else ITM_UQC end ");
			build.append(") AS GSTN_ITM_UQC,SUM( ");
			build.append(
					"( CASE WHEN LEFT(HSNSAC, 2) = '99' then 0 else ifnull(ITM_QTY, 0) end ");
			build.append(
					") ) AS GSTN_ITM_QTY,sum(IFNULL(TAXABLE_VALUE, 0) ) AS GSTN_TAXABLE_VALUE,");
			...
			build.append("FROM GSTR1A_SUBMITTED_PS_TRANS PS ");
			build.append(
					"LEFT JOIN GETANX1_BATCH_TABLE BT ON PS.BATCH_ID = BT.ID ");
			build.append("WHERE TAX_DOC_TYPE = 'HSN_GSTN' group by ");
			build.append(
					"PS.SUPPLIER_GSTIN,PS.RETURN_PERIOD,PS.DERIVED_RET_PERIOD,");
			build.append(
					"HSNSAC,HSN_DESCRIPTION,TAX_RATE,ITM_UQC,BT.IS_FILED ), ");
			build.append(
					"digi as ( SELECT PS.SUPPLIER_GSTIN,PS.RETURN_PERIOD,PS.DERIVED_RET_PERIOD,");
			build.append(
					"HSNSAC AS DG_ITM_HSNSAC, HSN_DESCRIPTION AS DG_ITM_DESCRIPTION,");
			...
			build.append(
					"CASE WHEN LEFT(HSNSAC, 2) = '99' then 0 else ifnull(ITM_QTY, 0) end ");
			build.append(
					") ) AS DG_ITM_QTY,sum( (IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(TAXABLE_VALUE,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(TAXABLE_VALUE,0) END),0)) ");
			build.append(
					") AS DG_TAXABLE_VALUE, sum( (IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(IGST_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(IGST_AMT,0) END),0)) ) AS DG_IGST_AMT,");
			build.append(
					"sum( (IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(CGST_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(CGST_AMT,0) END),0)) ) AS DG_CGST_AMT, ");
			build.append(
					"sum( (IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(SGST_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(SGST_AMT,0) END),0)) ) AS DG_SGST_AMT, ");
			build.append(
					"sum( (IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(CESS_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(CESS_AMT,0) END),0)) ");
			build.append(
					") AS DG_CESS_AMT,sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(TOTAL_VALUE,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(TOTAL_VALUE,0) END),0))  ) AS DG_TOT_VAL, ");
			build.append(
					"MAX( CASE WHEN IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' ");
			build.append(
					"ELSE 'NOT_SAVED' END ) DG_SAVE_STATUS, max(BATCH_ID) dg_batch_id ");
			build.append("FROM GSTR1A_SUBMITTED_PS_TRANS PS ");
			build.append(
					"WHERE TAX_DOC_TYPE = 'HSN_DIGI' group by PS.SUPPLIER_GSTIN, ");
			build.append(
					"PS.RETURN_PERIOD,PS.DERIVED_RET_PERIOD,HSNSAC,HSN_DESCRIPTION,");
			build.append(
					"TAX_RATE,ITM_UQC ), ui as ( SELECT PS.SUPPLIER_GSTIN,PS.RETURN_PERIOD, ");
			build.append("PS.DERIVED_RET_PERIOD,HSNSAC AS UI_ITM_HSNSAC,");
			build.append(
					"HSN_DESCRIPTION AS UI_ITM_DESCRIPTION,TAX_RATE AS UI_TAX_RATE, ");
			build.append(
					"( CASE WHEN LEFT(HSNSAC, 2) = '99' then 'NA' else ITM_UQC end ");
			build.append(
					") AS UI_ITM_UQC, SUM( ( CASE WHEN LEFT(HSNSAC, 2) = '99' ");
			build.append(
					"then 0 else ifnull(ITM_QTY, 0) end ) ) AS UI_ITM_QTY, ");
			build.append(
					"sum( IFNULL(TAXABLE_VALUE, 0) ) AS UI_TAXABLE_VALUE,");
			build.append("sum( IFNULL(IGST_AMT, 0) ) AS UI_IGST_AMT, ");
			build.append(
					"sum( IFNULL(CGST_AMT, 0) ) AS UI_CGST_AMT,sum( IFNULL(SGST_AMT, 0) ");
			build.append(
					") AS UI_SGST_AMT,sum( IFNULL(CESS_AMT, 0) ) AS UI_CESS_AMT, ");
			build.append("sum( IFNULL(TOTAL_VALUE, 0) ) AS UI_TOT_VAL, ");
			build.append(
					"MAX(CASE WHEN IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' ");
			build.append("ELSE 'NOT_SAVED' END  ) UI_SAVE_STATUS, ");
			build.append(
					"max(BATCH_ID) UI_BATCH_ID FROM GSTR1A_SUBMITTED_PS_TRANS PS ");
			build.append(
					"WHERE TAX_DOC_TYPE = 'HSN_UI' group by PS.SUPPLIER_GSTIN,");
			build.append(
					"PS.RETURN_PERIOD,PS.DERIVED_RET_PERIOD,HSNSAC,HSN_DESCRIPTION,");
			build.append("TAX_RATE,ITM_UQC ) select IFNULL( ifnull( ");
			build.append(
					"GSTN.SUPPLIER_GSTIN, UI.SUPPLIER_GSTIN ),DIGI.SUPPLIER_GSTIN ");
			build.append(
					") AS SUPPLIER_GSTIN,IFNULL(ifnull( GSTN.RETURN_PERIOD, ");
			build.append(
					"UI.RETURN_PERIOD ),DIGI.RETURN_PERIOD ) AS RETURN_PERIOD, ");
			build.append(
					"IFNULL( ifnull( GSTN.DERIVED_RET_PERIOD, UI.DERIVED_RET_PERIOD ");
			...
			build.append(
					"GSTN_ITM_UQC,GSTN_ITM_QTY,GSTN_TAXABLE_VALUE,GSTN_IGST_AMT,");
			build.append("GSTN_CGST_AMT,GSTN_SGST_AMT,GSTN_CESS_AMT, ");
			build.append(
					"GSTN_TOT_VAL,IS_FILED,DG_ITM_HSNSAC,DG_ITM_DESCRIPTION,");
			build.append("DG_TAX_RATE,DG_ITM_UQC,DG_ITM_QTY,DG_TAXABLE_VALUE,");
			build.append(
					"DG_IGST_AMT,DG_CGST_AMT,DG_SGST_AMT,DG_CESS_AMT,DG_TOT_VAL,");
			build.append(
					"UI_ITM_HSNSAC,UI_ITM_DESCRIPTION,UI_TAX_RATE,UI_ITM_UQC,UI_ITM_QTY,");
			build.append(
					"UI_TAXABLE_VALUE,UI_IGST_AMT,UI_CGST_AMT,UI_SGST_AMT,");
			build.append(
					"UI_CESS_AMT,UI_TOT_VAL,( ifnull(DG_TAXABLE_VALUE, 0)- ");
			build.append(
					"ifnull(GSTN_TAXABLE_VALUE, 0) ) AS DIFF_TAXABLE_VALUE,");
			build.append("( ifnull(DG_IGST_AMT, 0)- ifnull(GSTN_IGST_AMT, 0) ");
			build.append(
					") AS DIFF_IGST_AMT, ( ifnull(DG_CGST_AMT, 0)- ifnull(GSTN_CGST_AMT, 0) ");
			build.append(
					") AS DIFF_CGST_AMT, ( ifnull(DG_SGST_AMT, 0)- ifnull(GSTN_SGST_AMT, 0) ");
			build.append(
					") AS DIFF_SGST_AMT,( ifnull(DG_CESS_AMT, 0)- ifnull(GSTN_CESS_AMT, 0) ");
			build.append(
					") AS DIFF_CESS_AMT,( case when UI_BATCH_ID > DG_BATCH_ID ");
			build.append(
					"then UI_BATCH_ID else DG_BATCH_ID end ) as batch_id, ");
			build.append(
					"(case when ifnull(UI_SAVE_STATUS,'') > IFNULL(DG_SAVE_STATUS,'') ");
			build.append(
					"then UI_SAVE_STATUS else DG_SAVE_STATUS end ) as SAVE_STATUS ");
			build.append("from gstn full outer join ui on ");
			build.append("gstn.supplier_gstin = UI.supplier_gstin ");
			build.append(
					"and gstn.derived_ret_period = UI.derived_ret_period ");
			...
			build.append(
					" ) and DIGI.DG_ITM_UQC = IFNULL( UI.UI_ITM_UQC, gstn.GSTN_ITM_UQC ");
			build.append(
					") and DIGI.DG_TAX_RATE = IFNULL( UI.UI_TAX_RATE, gstn.GSTN_TAX_RATE ");
			build.append(
					") ) A left join ( select ID,GSTN_SAVE_STATUS,GSTN_SAVE_REF_ID, ");
			build.append(
					"BATCH_DATE, ERROR_CODE,ERROR_DESC from GSTR1_GSTN_SAVE_BATCH ");
			build.append(
					"where is_delete = FALSE ) batch on A.BATCH_ID = batch.id ");
			build.append(buildHeader);
			build.append(
					"group by A.SUPPLIER_GSTIN,A.RETURN_PERIOD,A.GSTN_ITM_HSNSAC,");
			build.append(
					"A.GSTN_ITM_DESCRIPTION,A.GSTN_TAX_RATE,A.GSTN_ITM_UQC,A.IS_FILED,");
			build.append(
					"A.DG_ITM_HSNSAC,A.DG_ITM_DESCRIPTION,A.DG_TAX_RATE,A.DG_ITM_UQC,");
			build.append(
					"A.UI_ITM_HSNSAC,A.UI_ITM_DESCRIPTION,A.UI_TAX_RATE,A.UI_ITM_UQC,");
			...
			build.append(
					"SELECT ROW_NUMBER () OVER (ORDER BY A.SUPPLIER_GSTIN) SNO,");
			build.append(
					"A.SUPPLIER_GSTIN,	A.RETURN_PERIOD,A.GSTN_ITM_HSNSAC,");
			build.append(
					"A.GSTN_ITM_DESCRIPTION,A.GSTN_ITM_UQC,A.GSTN_ITM_QTY,");
			build.append(
					"A.GSTN_TAXABLE_VALUE,	A.GSTN_IGST_AMT,A.GSTN_CGST_AMT,");
			build.append(
					"A.GSTN_SGST_AMT,A.GSTN_CESS_AMT,A.GSTN_TOT_VAL,A.IS_FILED,");
			build.append(
					"A.DG_ITM_HSNSAC,A.DG_ITM_DESCRIPTION,	A.DG_ITM_UQC,A.DG_ITM_QTY,");
			build.append(
					"A.DG_TAXABLE_VALUE,A.DG_IGST_AMT,A.DG_CGST_AMT,A.DG_SGST_AMT,");
			build.append(
					"A.DG_CESS_AMT,A.DG_TOT_VAL,A.UI_ITM_HSNSAC,A.UI_ITM_DESCRIPTION,");
			build.append(
					"A.UI_ITM_UQC,	A.UI_ITM_QTY,A.UI_TAXABLE_VALUE,A.UI_IGST_AMT,");
			build.append(
					"A.UI_CGST_AMT,A.UI_SGST_AMT,A.UI_CESS_AMT,A.UI_TOT_VAL,");
			build.append(
					"A.DIFF_TAXABLE_VALUE,A.DIFF_IGST_AMT,	A.DIFF_CGST_AMT,");
			build.append("A.DIFF_SGST_AMT,A.DIFF_CESS_AMT,A.SAVE_STATUS,");
			build.append(
					"(CASE WHEN A.SAVE_STATUS='IS_SAVED' THEN GSTN_SAVE_REF_ID ELSE NULL END) AS GSTIN_REF_ID,");
			build.append(
					"(CASE WHEN A.SAVE_STATUS='IS_SAVED' THEN ADD_SECONDS(BATCH_DATE,19800) ELSE NULL END) AS GSTIN_REF_ID_TIME, ");
			build.append("ERROR_CODE, ERROR_DESC,A.DERIVED_RET_PERIOD from ");
			build.append("( with GSTN as ( ");
			build.append(
					"SELECT PS.SUPPLIER_GSTIN,PS.RETURN_PERIOD,PS.DERIVED_RET_PERIOD,");
			build.append(
					"HSNSAC AS GSTN_ITM_HSNSAC,HSN_DESCRIPTION AS GSTN_ITM_DESCRIPTION,");
			build.append(
					"ITM_UQC AS GSTN_ITM_UQC,SUM(ifnull(ITM_QTY,0)) AS GSTN_ITM_QTY,");
			...
			build.append("sum(IFNULL(TOTAL_VALUE,0)) AS GSTN_TOT_VAL,");
			build.append(
					"ifnull(BT.IS_FILED,FALSE) IS_FILED FROM  GSTR1A_SUBMITTED_PS_TRANS PS ");
			build.append(
					"LEFT JOIN GETANX1_BATCH_TABLE BT  ON PS.BATCH_ID = BT.ID	");
			build.append(
					"WHERE TAX_DOC_TYPE ='HSN_GSTN'  group by PS.SUPPLIER_GSTIN,");
			build.append(
					"PS.RETURN_PERIOD,	PS.DERIVED_RET_PERIOD,HSNSAC, HSN_DESCRIPTION ,");
			...
			build.append(
					"HSN_DESCRIPTION AS DG_ITM_DESCRIPTION,ITM_UQC AS DG_ITM_UQC,");
			build.append("SUM(ifnull(ITM_QTY,0)) AS DG_ITM_QTY,");
			build.append(
					"sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(TAXABLE_VALUE,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(TAXABLE_VALUE,0) END),0)) ) AS DG_TAXABLE_VALUE,");
			build.append(
					"sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(IGST_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(IGST_AMT,0) END),0))) AS DG_IGST_AMT,");
			build.append(
					"sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(CGST_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(CGST_AMT,0) END),0))) AS DG_CGST_AMT,");
			build.append(
					"sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(SGST_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(SGST_AMT,0) END),0))) AS DG_SGST_AMT,");
			build.append(
					"sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(CESS_AMT,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(CESS_AMT,0) END),0))) AS DG_CESS_AMT,");
			build.append(
					"sum((IFNULL((CASE WHEN DOC_TYPE IN('INV','BOS','DR') THEN IFNULL(TOTAL_VALUE,0) END),0)- IFNULL((CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(TOTAL_VALUE,0) END),0))) AS DG_TOT_VAL,");
			build.append(
					"MAX(CASE WHEN IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) DG_SAVE_STATUS,");
			build.append(
					"max(BATCH_ID) dg_batch_id FROM GSTR1A_SUBMITTED_PS_TRANS PS ");
			...
			build.append(
					"SELECT PS.SUPPLIER_GSTIN, PS.RETURN_PERIOD, PS.DERIVED_RET_PERIOD,");
			build.append(
					"HSNSAC AS UI_ITM_HSNSAC, HSN_DESCRIPTION AS UI_ITM_DESCRIPTION,");
			build.append(
					"ITM_UQC AS UI_ITM_UQC, SUM(ifnull(ITM_QTY,0)) AS UI_ITM_QTY,");
			...
			build.append("sum(IFNULL(TOTAL_VALUE,0)) AS UI_TOT_VAL,");
			build.append(
					"MAX(CASE WHEN IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) UI_SAVE_STATUS,");
			build.append(
					"max(BATCH_ID) UI_BATCH_ID FROM GSTR1A_SUBMITTED_PS_TRANS PS ");
			...
			build.append(
					"GSTN_ITM_HSNSAC,GSTN_ITM_DESCRIPTION,GSTN_ITM_UQC,GSTN_ITM_QTY,GSTN_TAXABLE_VALUE,");
			build.append(
					"GSTN_IGST_AMT,GSTN_CGST_AMT,GSTN_SGST_AMT,GSTN_CESS_AMT,GSTN_TOT_VAL,IS_FILED,");
			build.append(
					"DG_ITM_HSNSAC,DG_ITM_DESCRIPTION,DG_ITM_UQC,DG_ITM_QTY,");
			build.append(
					"DG_TAXABLE_VALUE,DG_IGST_AMT,DG_CGST_AMT,DG_SGST_AMT,");
			build.append(
					"DG_CESS_AMT,DG_TOT_VAL,UI_ITM_HSNSAC,UI_ITM_DESCRIPTION,");
			...
			build.append(
					"(ifnull(DG_TAXABLE_VALUE,0)-ifnull(GSTN_TAXABLE_VALUE,0)) AS DIFF_TAXABLE_VALUE,");
			build.append(
					"(ifnull(DG_IGST_AMT,0)-ifnull(GSTN_IGST_AMT,0)) AS DIFF_IGST_AMT,");
			build.append(
					"(ifnull(DG_CGST_AMT,0)-ifnull(GSTN_CGST_AMT,0)) AS DIFF_CGST_AMT,");
			build.append(
					"(ifnull(DG_SGST_AMT,0)-ifnull(GSTN_SGST_AMT,0)) AS DIFF_SGST_AMT,");
			build.append(
					"(ifnull(DG_CESS_AMT,0)-ifnull(GSTN_CESS_AMT,0)) AS DIFF_CESS_AMT,");
			build.append(
					"(case when UI_BATCH_ID> DG_BATCH_ID then UI_BATCH_ID ");
			build.append("else DG_BATCH_ID  end) as batch_id,");
			build.append(
					"(case when ifnull(UI_SAVE_STATUS,'') > IFNULL(DG_SAVE_STATUS,'') then UI_SAVE_STATUS ");
			...
			build.append(
					"and gstn.GSTN_ITM_HSNSAC = UI.UI_ITM_HSNSAC and gstn.GSTN_ITM_UQC = UI.UI_ITM_UQC ");
			build.append(
					"FULL OUTER JOIN DIGI ON DIGI.supplier_gstin = IFNULL(UI.supplier_gstin,gstn.supplier_gstin) ");
			build.append(
					"and DIGI.derived_ret_period =IFNULL(UI.derived_ret_period,gstn.derived_ret_period) ");
			build.append(
					"and DIGI.DG_ITM_HSNSAC = IFNULL(UI.UI_ITM_HSNSAC,gstn.GSTN_ITM_HSNSAC) ");
			build.append(
					"and DIGI.DG_ITM_UQC = IFNULL(UI.UI_ITM_UQC,gstn.GSTN_ITM_UQC) ");
