...
@Slf4j
@Component("Gstr1B2CSSectionReportHandler")
public class Gstr1B2CSSectionReportHandler {

	@Autowired
	@Qualifier("Gstr1B2CSSectionReportServiceImpl")
		...
		try {
			...
			if (workbook != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2CS template file name is : {}",
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						...
						+ criteria.getTaxperiod() + "_B2CS_SECTION7" + ".xlsx";

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2CS file name is : {}", fileName);
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Workbook has been generated successfully with the"
									+ " Gstr1 B2CS response list in the directory : {}",
				...
				if (id != null) {
					try {
						...
						String msg = String
								.format("Gstr1B2CSSectionReportHandler - "
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2CS No Data found for report id : {} ",
							id);
				}
			...
			String msg = String.format("Gstr1B2CSSectionReportHandler - "
					+ "Exception occured while "
