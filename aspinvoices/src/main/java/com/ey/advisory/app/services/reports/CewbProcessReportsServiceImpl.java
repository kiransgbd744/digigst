 ...
 *
 */
@Service("CewbProcessReportsServiceImpl")
@Slf4j
public class CewbProcessReportsServiceImpl
	...
	@Autowired
	@Qualifier("CewbProcessReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("CEWB Process response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.CEWB).equalsIgnoreCase(request.getFileType())) {
				...
				String[] b2csHeaders = commonUtility
						.getProp("cewb.vertical.process.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
