 ...
 *
 */
@Service("Gstr1AEInvoiceDefaultOutwardSupplyTypeResolver")
public class Gstr1AEInvoiceDefaultOutwardSupplyTypeResolver
		implements Gstr1AEInvoiceOutwardSupplyTypeResolver {
	...
	public String resolve(Gstr1AOutwardTransDocument document) {
		...
		if (map.containsKey(GSTConstants.TAX))
			return GSTConstants.TAX;

		// Then check if 'DTA' is present. If so, choose 'DTA' as the supply
		// type. This falls in the DTA, EXT, NIL, NON category.
		if (map.containsKey(GSTConstants.DTA))
			return GSTConstants.DTA;

		if (map.containsKey(GSTConstants.EXPT))
			return GSTConstants.EXPT;
