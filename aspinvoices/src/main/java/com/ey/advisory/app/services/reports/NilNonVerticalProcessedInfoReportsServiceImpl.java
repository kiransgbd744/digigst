 ...
 */
@Slf4j
@Service("NilNonVerticalProcessedInfoReportsServiceImpl")
public class NilNonVerticalProcessedInfoReportsServiceImpl implements NilNonVerticalReportsService {

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("NilNonVerticalProcessedInfoReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria, PageRequest pageReq) {
		...
		workbook = createWorkbookWithExcelTemplate("ReportTemplates", "InfoRecords-NIL NON EXT.xlsx");

		LOGGER.debug("Nil non data response" + responseFromView);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility.getProp("nil.processedInfo.report.headers").split(",");
