 ...
 *
 */
@Component("Anx1ShipplingBillDate")
public class Anx1ShipplingBillDate 
                implements DocRulesValidator<OutwardTransDocument> {

	
	private boolean isValidTax(OutwardTransDocLineItem firstType,
			OutwardTransDocLineItem curType) {
		if (firstType.getShippingBillDate() == null 
				&& curType.getShippingBillDate()!=null 
				|| firstType.getShippingBillDate() != null 
				&& curType.getShippingBillDate()==null) {
			
			return false;
		}
		if(firstType.getShippingBillDate() != null){
			if (firstType.getShippingBillDate()
					.compareTo(curType.getShippingBillDate()) != 0) {
				return false;
			}
		}
		return true;
...
if(!SUPPLY_TYPE_IMPORTS.contains(document.getSupplyType().trim())) return errors;
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			if (!isValidTax(item, items.get(idx))) {
				...
				errors.add(new ProcessingResult(APP_VALIDATION, "ER0065",
						" Shipping Bill Date should be same across"
