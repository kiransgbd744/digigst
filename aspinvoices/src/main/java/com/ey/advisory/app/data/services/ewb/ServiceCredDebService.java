...
@Component("ServiceCredDebService")
@Slf4j
public class ServiceCredDebService {

	@Autowired
	@Qualifier("ServiceCredDebPDFDaoImpl")
	ServiceCredDebPdf serviceCredDebPdf;

	public String generateServicedebPdfZip(File tempDir,
		...
		for (PdfPrintReqDto req : request) {
			...
			try (OutputStream outStream = new BufferedOutputStream(
				...
				JasperPrint jasperPrint = serviceCredDebPdf
						.generateServiceCredPdfReport(id, docNo, sgstin);
	...
	private String zipEinvoicePdfFiles(File tempDir) throws Exception {
		...
		String fileName = "ServiceCreditDebitPDF";
