...
import com.google.common.base.Strings;
	...
	@Component("Gstr1AAspSecHsnSummaryDaoImpl")
	public class Gstr1AAspSecHsnSummaryDaoImpl implements Gstr1AAspHsnSummaryDao {

		private static final Logger LOGGER = LoggerFactory.getLogger(Gstr1AAspSecHsnSummaryDaoImpl.class);
		...
		private String createHsnSummaryQueryString(String buildQuery,Boolean rateIncludedInHsn) {
			...
			return "SELECT HSN.SUPPLIER_GSTIN,HSN.RETURN_PERIOD,"
					...
					+ "SUM(CASE WHEN LEFT(HSNSAC, 2) = 99 then 0 ELSE IFNULL(ITM_QTY,0) END) AS ITM_QTY,"
					 ...
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(TAXABLE_VALUE, 0) END), 0) AS TAXABLE_VALUE, "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('INV', 'BOS', 'DR') THEN IFNULL(IGST_AMT, 0) END), 0) - "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(IGST_AMT, 0) END), 0) AS IGST, "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('INV', 'BOS', 'DR') THEN IFNULL(CGST_AMT, 0) END), 0) - "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(CGST_AMT, 0) END), 0) AS CGST, "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('INV', 'BOS', 'DR') THEN IFNULL(SGST_AMT, 0) END), 0) - "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(SGST_AMT, 0) END), 0) AS SGST, "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('INV', 'BOS', 'DR') THEN IFNULL(CESS_AMT, 0) END), 0) - "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(CESS_AMT, 0) END), 0) AS CESS, "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('INV', 'BOS', 'DR') THEN IFNULL(TOTAL_VALUE, 0) END), 0) - "
					 + " IFNULL(SUM(CASE WHEN DOC_TYPE IN('CR') THEN IFNULL(TOTAL_VALUE, 0) END), 0) AS TOTAL_VALUE, "
					
					+ "MAX(CASE WHEN IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' ELSE 'NOT_SAVED' END) SAVE_STATUS,"
					+ "MAX(BATCH_ID) BATCH_ID  FROM GSTR1A_SUBMITTED_PS_TRANS HSN "
