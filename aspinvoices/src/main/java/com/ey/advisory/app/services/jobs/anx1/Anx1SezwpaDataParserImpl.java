...
import com.ey.advisory.app.data.entities.simplified.client.GetAnx1SezwpaInvoicesItemEntity;
import com.ey.advisory.app.docs.dto.anx1.Anx1B2bItemDetails;
import com.ey.advisory.app.docs.dto.anx1.Anx1B2baDocumentData;
import com.ey.advisory.app.docs.dto.anx1.Anx1SezwpaData;
...
@Service("Anx1SezwpaDataParserImpl")
@Slf4j
public class Anx1SezwpaDataParserImpl implements Anx1SezwpaDataParser {
	...
	public Set<GetAnx1SezwpaInvoicesHeaderEntity> parseSezwpaData(
		...
		Set<GetAnx1SezwpaInvoicesHeaderEntity> invoiceList = new HashSet<>();

		JsonArray respObject = null;
		try {
			respObject = (new JsonParser()).parse(apiResp).getAsJsonObject()
					.get(APIConstants.SEZWPA).getAsJsonArray();

			Type listType = new TypeToken<List<Anx1SezwpaData>>() {
			}.getType();
			List<Anx1SezwpaData> baseEntity = gson.fromJson(respObject,
					listType);
			for (Anx1SezwpaData eachInv : baseEntity) {
				for (Anx1B2baDocumentData eachInvData : eachInv.getDocs()) {
					List<GetAnx1SezwpaInvoicesItemEntity> itemList = new ArrayList<>();
					GetAnx1SezwpaInvoicesHeaderEntity invoice = setInvoiceData(
			...
			String msg = "failed to parse Anx1 SEZWPA response";
			LOGGER.error(msg, ex);
	...
	private GetAnx1SezwpaInvoicesHeaderEntity setInvoiceData(
			Anx1SezwpaData eachInv, Anx1B2baDocumentData eachInvData,
			Anx1GetInvoicesReqDto dto, Long batchId) {
		GetAnx1SezwpaInvoicesHeaderEntity invoice = new GetAnx1SezwpaInvoicesHeaderEntity();
		...
		invoice.setSezwpaBatchId(batchId);
	...
	private GetAnx1SezwpaInvoicesItemEntity setItemData(
			Anx1B2bItemDetails lineItem) {
		GetAnx1SezwpaInvoicesItemEntity item = new GetAnx1SezwpaInvoicesItemEntity();
	...
	private void setDockey(GetAnx1SezwpaInvoicesHeaderEntity invoice,
