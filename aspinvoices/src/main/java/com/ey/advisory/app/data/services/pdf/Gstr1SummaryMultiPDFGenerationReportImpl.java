...
@Slf4j
@Component("Gstr1SummaryMultiPDFGenerationReportImpl")
public class Gstr1SummaryMultiPDFGenerationReportImpl
		implements Gstr1SummaryMultiPDFGenerationReport {
	...
	public JasperPrint generateGstr1SummaryPdfReport(
		...
		List<Long> entityId = annexure1SummaryRequest.getEntityId();
		...
		try {
			...
			for (Gstr1SummaryScreenRespDto b2ba : b2balist) {
				...
				b2baObj.setAspCess(GenUtil.formatCurrency(
						isDigigst ? b2ba.getAspCess() : b2ba.getGstnSgst()));
			...
			for (Gstr1SummaryScreenNilRespDto nilAsp : nilAspList) {
				 ...
				 * nilAsp.getAspNitRated() : nilAsp.getAspNitRated());
				 */
				BigDecimal nilRated;
				if (isDigigst) {
					nilRated = (nilAsp.getAspNitRated() != null
							? nilAsp.getAspNitRated() : BigDecimal.ZERO);
				} else {
					nilRated = (nilAsp.getGstnNitRated() != null
							? nilAsp.getGstnNitRated() : BigDecimal.ZERO);

				}
				nilAspObj.setAspNitRated(GenUtil.formatCurrency(nilRated));
				BigDecimal aspExmptd;
				if (isDigigst) {
					aspExmptd = nilAsp.getAspExempted() != null
							? nilAsp.getAspExempted() : BigDecimal.ZERO;

				} else {
					aspExmptd = nilAsp.getGstnExempted() != null
							? nilAsp.getGstnExempted() : BigDecimal.ZERO;
				}

				nilAspObj.setAspExempted(GenUtil.formatCurrency(aspExmptd));
				BigDecimal aspNonGst;
				if (isDigigst) {
					aspNonGst = nilAsp.getAspNonGst() != null
							? nilAsp.getAspNonGst() : BigDecimal.ZERO;
				} else {
					aspNonGst = nilAsp.getGstnNonGst() != null
							? nilAsp.getGstnNonGst() : BigDecimal.ZERO;
				}

				nilAspObj.setAspNonGst(GenUtil.formatCurrency(aspNonGst));
			...
			for (Gstr1SummaryScreenNilRespDto nilUI : nilUIList) {
				...
				BigDecimal nilRated;
				if (isDigigst) {
					nilRated = nilUI.getAspNitRated() != null
							? nilUI.getAspNitRated() : BigDecimal.ZERO;
				} else {
					nilRated = nilUI.getGstnNitRated() != null
							? nilUI.getGstnNitRated() : BigDecimal.ZERO;
				}

				nilUIObj.setAspNitRated(GenUtil.formatCurrency(nilRated));
				BigDecimal aspExmptd;
				if (isDigigst) {
					aspExmptd = nilUI.getAspExempted() != null
							? nilUI.getAspExempted() : BigDecimal.ZERO;
				} else {
					aspExmptd = nilUI.getGstnExempted() != null
							? nilUI.getGstnExempted() : BigDecimal.ZERO;
				}
				nilUIObj.setAspExempted(GenUtil.formatCurrency(aspExmptd));
				BigDecimal aspNonGst;
				if (isDigigst) {
					aspNonGst = nilUI.getAspNonGst() != null
							? nilUI.getAspNonGst() : BigDecimal.ZERO;
				} else {
					aspNonGst = nilUI.getGstnNonGst() != null
							? nilUI.getGstnNonGst() : BigDecimal.ZERO;
				}

				nilUIObj.setAspNonGst(GenUtil.formatCurrency(aspNonGst));
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = "B2B section values got added to the parameters";
				LOGGER.debug(msg);
			}
			parameters.put("B2BA",
