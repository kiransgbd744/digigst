...
@Component("Gstr1aOutwardVerticalProcessAdvAdjDaoImpl")
public class Gstr1aOutwardVerticalProcessAdvAdjDaoImpl
		implements Gstr1OutwardVerticalProcessAdvAdjDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1aOutwardVerticalProcessAdvAdjDaoImpl.class);
	...
	public List<Object> getGstr1RSReports(SearchCriteria criteria) {
		...
		if (taxperiod != null && !taxperiod.isEmpty()) {
			buildQuery.append(" AND ADJ.DERIVED_RET_PERIOD = :taxperiod ");
		}
		...
		return list.parallelStream().map(o -> convertProcessedAdvAdj(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1AspVerticalAdvAdjDto convertProcessedAdvAdj(Object[] arr) {
	...
	private String createB2CSSavableSummaryQueryString(String buildQuery) {
		...
		return "SELECT ADJ.SUPPLIER_GSTIN,ADJ.RETURN_PERIOD,"
				...
				+ "ON ADJ.FILE_ID=FIL.ID AND GSTRERR.FILE_ID=FIL.ID AND "
				+ "GSTRERR.INV_KEY=ADJ.TXPD_INVKEY LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = ADJ.BATCH_ID "
				+ "WHERE ADJ.IS_DELETE=FALSE " 
		        + buildQuery;
	}
}
