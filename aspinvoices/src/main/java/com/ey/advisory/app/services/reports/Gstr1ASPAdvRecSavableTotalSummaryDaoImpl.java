 ...
 *
 */
@Component("Gstr1ASPAdvRecSavableTotalSummaryDaoImpl")
public class Gstr1ASPAdvRecSavableTotalSummaryDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1ASPAdvRecSavableTotalSummaryDaoImpl.class);
	...
	private static String createApiProcessedQueryString(String buildQueryAdv,
		...
		return "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MAX(MONTH)MONTH, "
				...
				+ "NULL ORG_POS,NULL ORG_RATE,NULL ORG_GROSS_ADV_RECEIVED, NEW_POS,NULL NEW_RATE, "
				...
				+ "'' AS MONTH, NULL ORG_POS, NULL ORG_RATE, NULL AS ORG_TAXABLE_VALUE,"
				...
				+ "GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME, '' GSTN_ERROR_CODE, '' GSTN_ERROR_DESCRIPTION, '' SOURCE_ID, '' FILE_NAME, '' ASP_DATE_TIME FROM ANX_OUTWARD_DOC_HEADER HDR "
				+ "INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID = FIL.ID LEFT "
				...
				+ "HDR.RETURN_TYPE = 'GSTR1' AND HDR.IS_DELETE = FALSE AND HDR.IS_PROCESSED = TRUE "
				+ "AND HDR.TAX_DOC_TYPE='AT'" + buildQueryHdr + " ) "
				+ " GROUP BY "
				+ "SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,NEW_POS "
				+ " UNION ALL "
				+ "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MONTH, "
				+ "MAX(ORG_POS)ORG_POS, MAX(ORG_RATE)ORG_RATE,"
				+ "MAX(ORG_GROSS_ADV_RECEIVED)ORG_GROSS_ADV_RECEIVED, "
				+ "NEW_POS,MAX(NEW_RATE)NEW_RATE, "
				+ "SUM(NEW_GROSS_ADV_RECEIVED)NEW_GROSS_ADV_RECEIVED, "
				+ "SUM(IGST_AMT)IGST_AMT,SUM(CGST_AMT)CGST_AMT, SUM(SGST_AMT)SGST_AMT,"
				+ "SUM(CESS_AMT)CESS_AMT, MAX(PROFIT_CENTRE)PROFIT_CENTRE,"
				+ "MAX(PLANT_CODE)PLANT_CODE,MAX(DIVISION)DIVISION, MAX(LOCATION)LOCATION, "
				+ "MAX(SALES_ORGANIZATION)SALES_ORGANIZATION, "
				...
				+ "MAX(SOURCE_ID)SOURCE_ID, MAX(FILE_NAME)FILE_NAME,"
				+ "MAX(ASP_DATE_TIME)ASP_DATE_TIME "
				+ "FROM ( SELECT REC.SUPPLIER_GSTIN,REC.RETURN_PERIOD, TRAN_TYPE,"
				...
				+ "(CASE WHEN REC.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' "
				+ "WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = TRUE "
				+ "THEN 'IS_ERROR' WHEN REC.IS_SAVED_TO_GSTN = FALSE "
				+ "AND REC.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) AS SAVE_STATUS, "
				+ "GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, "
				...
				+ "AND REC.FILE_ID=GSTRERR.FILE_ID AND GSTRERR.TABLE_TYPE='ADV REC' "
				+ "AND RETURN_TYPE='GSTR1' LEFT OUTER JOIN FILE_STATUS FIL "
				+ "ON REC.FILE_ID=FIL.ID AND GSTRERR.FILE_ID=FIL.ID "
				+ "AND GSTRERR.INV_KEY=REC.AT_INVKEY LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = REC.BATCH_ID "
				+ "WHERE REC.IS_DELETE=FALSE AND IS_AMENDMENT=TRUE "
				+ buildQueryAdv + " ) " + " GROUP BY "
				+ "SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,NEW_POS,MONTH ";
	}
}
