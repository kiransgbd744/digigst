...
@Service("Anx1InwardApitotalReportsServiceImpl")
public class Anx1InwardApitotalReportsServiceImpl
		implements Anx1InwardApiTotalReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1InwardApitotalReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1InwardApiTotalReportsDaoImpl")
	private Anx1InwardApiTotalReportsDao anx1InwardApiTotalReportsDao;
	...
	public Workbook findInwardApiTotalRec(SearchCriteria criteria,
		...
		List<Anx1InwardApiTotalRecordsDto> responseFromView = new ArrayList<>();
		responseFromView = anx1InwardApiTotalReportsDao
				.getInwardApiTotalReports(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.inward.api.totrec.report.headers")
					.split(",");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_InwardAPITotalRecReport.xlsx");
