...
@Slf4j
@Component("Gstr1AProcessedRecordsFetchDaoImpl")
public class Gstr1AProcessedRecordsFetchDaoImpl
		implements Gstr1AProcessedRecordsFetchDao {
	...
	@Autowired
	private Gstr1AProcessedRecordsCommonUtil gstr1AProcessedRecordsCommonUtil;
	...
	public List<Gstr1ProcessedRecordsRespDto> loadGstr1ProcessedRecords(
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Gstr1AProcessedRecordsFetchDaoImpl->"
		...
		/*	String multiSupplyTypeAns = groupConfigPrmtRepository
			...
			if (!combinedGstinList.isEmpty() && combinedGstinList.size() > 0) {
				...
				return gstr1AProcessedRecordsCommonUtil
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Final list from dao is ->" + finalDtoList.size());
			}
		} catch (Exception e) {
	...
	public String createQueryString(List<Long> entityId, List<String> gstinList,
		...
		if (tabStatus != null && tabStatus.equals(1)) {
				...
				queryStr = "WITH HDR AS"
						...
						+ "    AND HDR.IS_DELETE = FALSE AND "
						+ "    ( HDR.RETURN_TYPE = 'GSTR1A' AND HDR.TAX_DOC_TYPE IN ( 'B2B', "
						+ "        'B2BA', "
						+ "        'B2CL', "
						+ "        'B2CLA','EXPORTS', "
						+ "        'EXPORTS-A', "
						...
						+ "        'CDNUR-EXPORTS', "
						+ "        'CDNUR-B2CL', "
						+ "        'CDNURA' ,'NILEXTNON','AT','TXP','B2CS','Supecom','Ecomsup') "
						+ "    ) "
						+ hdrCondition
						+ "),"
						+ "ITM AS "
						...
						+ "        FROM HDR"
						+ "        INNER JOIN"
						+ "        ANX_OUTWARD_DOC_ITEM_1A ITM"
						+ "        ON ITM.DOC_HEADER_ID = HDR.ID"
						+ ")"
						+ "SELECT "
						+ "SUPPLIER_GSTIN, "
						+ "RETURN_PERIOD, "
						...
						+ "CASE WHEN TAX_DOC_TYPE='NILEXTNON' THEN 0 ELSE COUNT(DISTINCT KEY) END AS TOT_COUNT, "
						...
						+ "		HDR.SUPPLIER_GSTIN,"
						+ "		HDR.RETURN_PERIOD, "
						+ "		CASE WHEN HDR.TAX_DOC_TYPE = 'AT' THEN 'ADV REC'"
						+ "		     WHEN HDR.TAX_DOC_TYPE = 'TXP' THEN 'ADV ADJ' "
						+ "		     ELSE HDR.TAX_DOC_TYPE "
						+ "		END AS TAX_DOC_TYPE, "
						+ "		CASE WHEN HDR.TAX_DOC_TYPE IN ('AT','TXP') THEN (Ifnull(HDR.SUPPLIER_GSTIN,'')||'|'||Ifnull(HDR.RETURN_PERIOD,'')||'|'||Ifnull(HDR.POS,9999)) ELSE HDR.DOC_KEY END AS KEY, "
						+ "		( Ifnull("
						+ "		        (CASE WHEN HDR.DOC_TYPE IN ('INV','DR','BOS') "
						+ "		                AND HDR.TAX_DOC_TYPE = 'NILEXTNON' "
						+ "		                AND HDR.SUPPLY_TYPE = 'NIL' THEN TAXABLE_VALUE "
						+ "		              WHEN HDR.DOC_TYPE IN ('CR','RCR') "
						+ "		                AND HDR.TAX_DOC_TYPE IN ('CDNR','CDNRA','CDNUR','CDNUR-EXPORTS','CDNUR-B2CL', 'CDNURA' ) THEN -1*TAXABLE_VALUE "
						+ "		              WHEN HDR.DOC_TYPE IN ('DR','RDR') "
						+ "		                AND HDR.TAX_DOC_TYPE IN ('CDNR','CDNRA','CDNUR','CDNUR-EXPORTS','CDNUR-B2CL', 'CDNURA' ) THEN TAXABLE_VALUE "
						+ "		              WHEN HDR.TAX_DOC_TYPE IN ('B2B','B2BA','B2CL','B2CLA','EXPORTS','EXPORTS-A' ,'AT','TXP') THEN TAXABLE_VALUE "
						+ "		         END), 0) "
						+ "		 - Ifnull((CASE WHEN HDR.DOC_TYPE = 'CR' AND HDR.TAX_DOC_TYPE = 'NILEXTNON' AND HDR.SUPPLY_TYPE = 'NIL' THEN TAXABLE_VALUE END), 0) ) "
						+ "		 + ( Ifnull( (CASE WHEN HDR.DOC_TYPE IN ('INV','DR','BOS') AND HDR.TAX_DOC_TYPE = 'NILEXTNON' AND HDR.SUPPLY_TYPE = 'EXT' THEN TAXABLE_VALUE END), 0) "
						+ "		 - Ifnull( (CASE WHEN HDR.DOC_TYPE = 'CR' AND HDR.TAX_DOC_TYPE = 'NILEXTNON' AND HDR.SUPPLY_TYPE = 'EXT' THEN TAXABLE_VALUE END), 0) ) "
						+ "		 + ( Ifnull( (CASE WHEN HDR.DOC_TYPE IN ('INV','DR','BOS') AND HDR.TAX_DOC_TYPE = 'NILEXTNON' AND HDR.SUPPLY_TYPE IN ('NON','SCH3') THEN TAXABLE_VALUE END), 0) "
						+ "		 - Ifnull( (CASE WHEN HDR.DOC_TYPE = 'CR' AND HDR.TAX_DOC_TYPE = 'NILEXTNON' AND HDR.SUPPLY_TYPE IN ('NON','SCH3') THEN TAXABLE_VALUE END), 0) "
						+ "		     "
						+ "		 ) AS SUPPLIES, "
						+ "        CASE WHEN HDR.TAX_DOC_TYPE = 'NILEXTNON' THEN 0 ELSE(CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*IFNULL(IGST_AMT,0) ELSE 1*IFNULL(IGST_AMT,0) END) END AS IGST, "
						+ "        CASE WHEN HDR.TAX_DOC_TYPE = 'NILEXTNON' THEN 0 ELSE(CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*IFNULL(CGST_AMT,0) ELSE 1*IFNULL(CGST_AMT,0) END) END AS CGST, "
						+ "        CASE WHEN HDR.TAX_DOC_TYPE = 'NILEXTNON' THEN 0 ELSE(CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*IFNULL(SGST_AMT,0) ELSE 1*IFNULL(SGST_AMT,0) END) END AS SGST, "
						+ "        CASE WHEN HDR.TAX_DOC_TYPE = 'NILEXTNON' THEN 0 ELSE(CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*(IFNULL(CESS_AMT_SPECIFIC,0)+IFNULL(CESS_AMT_ADVALOREM,0)) ELSE 1*IFNULL(CESS_AMT_SPECIFIC,0)+IFNULL(CESS_AMT_ADVALOREM,0) END) END AS CESS, "
						+ "        (CASE WHEN IS_SENT_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_NOT_SENT_COUNT, "
						+ "        (CASE WHEN IS_SAVED_TO_GSTN = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_SAVED_COUNT, "
						+ "        (CASE WHEN IS_SAVED_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_NOT_SAVED_COUNT, "
						+ "        (CASE WHEN GSTN_ERROR = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_ERROR_COUNT, "
						+ "        (CASE WHEN IS_DELETE = FALSE THEN 1 ELSE NULL END) TOTAL_COUNT_IN_ASP, "
						+ "        HDR.MODIFIED_ON "
						+ "    FROM  HDR "
						+ "    WHERE"
						+ "        HDR.TAX_DOC_TYPE IN ( 'B2B', "
						+ "        'B2BA', "
						+ "        'B2CL', "
						+ "        'B2CLA','EXPORTS', "
						+ "        'EXPORTS-A', "
						...
						+ "        'CDNUR-EXPORTS', "
						+ "        'CDNUR-B2CL', "
						+ "        'CDNURA' ,'NILEXTNON','AT','TXP') "
						+ "    UNION ALL "
						+ "	SELECT"
						+ "	    SUPPLIER_GSTIN,"
						+ "	    RETURN_PERIOD, "
						+ "        CASE WHEN IS_AMENDMENT = FALSE THEN 'B2CS' ELSE 'B2CSA' END AS TAX_DOC_TYPE, "
						...
						+ "        (CASE WHEN IS_DELETE = FALSE THEN 1 ELSE NULL END) TOTAL_COUNT_IN_ASP, "
						+ "        MODIFIED_ON"
						+ "    FROM GSTR1A_PROCESSED_B2CS WHERE IS_DELETE = FALSE "
						+ conditionVertical
						+ "    UNION ALL "
						+ "	SELECT "
						+ "    	SUPPLIER_GSTIN, "
						+ "        HDR.RETURN_PERIOD, "
						+ "        HDR.TAX_DOC_TYPE as TAX_DOC_TYPE, "
						...
						+ "        CASE WHEN HDR.DOC_TYPE IN ('CR','RCR') THEN -1*IFNULL(ITM.TAXABLE_VALUE,0) ELSE IFNULL(ITM.TAXABLE_VALUE,0) END AS SUPPLIES, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*IFNULL(ITM.IGST_AMT,0) ELSE 1*IFNULL(ITM.IGST_AMT,0) END AS IGST, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*IFNULL(ITM.CGST_AMT,0) ELSE 1*IFNULL(ITM.CGST_AMT,0) END AS CGST, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*IFNULL(ITM.SGST_AMT,0) ELSE 1*IFNULL(ITM.SGST_AMT,0) END AS SGST, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('C','CR','RCR') THEN -1*(IFNULL(ITM.CESS_AMT_SPECIFIC,0)+IFNULL(ITM.CESS_AMT_ADVALOREM,0)) ELSE 1*IFNULL(ITM.CESS_AMT_SPECIFIC,0)+IFNULL(ITM.CESS_AMT_ADVALOREM,0) END AS CESS, "
						+ "        CASE WHEN IS_SENT_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE NULL END AS GSTN_NOT_SENT_COUNT, "
						+ "        CASE WHEN IS_SAVED_TO_GSTN = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END AS GSTN_SAVED_COUNT, "
						+ "        CASE WHEN IS_SAVED_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE NULL END AS GSTN_NOT_SAVED_COUNT, "
						+ "        CASE WHEN GSTN_ERROR = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END AS GSTN_ERROR_COUNT, "
						+ "        CASE WHEN IS_DELETE = FALSE THEN 1 ELSE NULL END AS TOTAL_COUNT_IN_ASP, "
						+ "        HDR.MODIFIED_ON AS MODIFIED_ON "
						+ "    FROM HDR  "
						+ "    INNER JOIN  ITM "
						+ "    ON HDR.ID = ITM.DOC_HEADER_ID "
						+ "    WHERE"
						+ "    HDR.TAX_DOC_TYPE IN ('B2CS') "
						+ "    UNION ALL "
						+ "	 SELECT"
						+ "	    SUPPLIER_GSTIN, "
						+ "        RETURN_PERIOD, "
						+ "        CASE WHEN IS_AMENDMENT = FALSE THEN 'ADV REC' ELSE 'ADV REC-A' END AS TAX_DOC_TYPE, "
						+ "        ( Ifnull(SUPPLIER_GSTIN,'')||'|'||Ifnull(RETURN_PERIOD,'')||'|'||"
						+ "        ( CASE WHEN TRAN_TYPE IN ( 'ZL65', 'L65', 'zl65','l65','zL65','Zl65' ) THEN 'L65' "
						+ "               WHEN ( TRAN_TYPE IN ('Z','N','','z','n') OR TRAN_TYPE IS NULL ) THEN 'N' "
						+ "        END )||'|'||Ifnull(NEW_POS,9999)) AS KEY, "
						...
						+ "	    SUPPLIER_GSTIN, "
						+ "        RETURN_PERIOD, "
						+ "        CASE WHEN IS_AMENDMENT = FALSE THEN 'ADV ADJ' ELSE 'ADV ADJ-A' END AS TAX_DOC_TYPE, "
						+ "        ( Ifnull(SUPPLIER_GSTIN,'')||'|'||Ifnull(RETURN_PERIOD,'')||'|'||"
						+ "        ( CASE WHEN TRAN_TYPE IN ( 'ZL65', 'L65','zl65','l65','zL65', 'Zl65' ) THEN 'L65'"
						+ "               WHEN ( TRAN_TYPE IN ('Z','N','','z','n') OR TRAN_TYPE IS NULL ) THEN 'N' "
						+ "        END)"
						+ "        ||'|'||Ifnull(NEW_POS,9999)) AS KEY, "
						...
						+ "        (CASE WHEN IS_DELETE = FALSE THEN 1 ELSE NULL END) TOTAL_COUNT_IN_ASP, "
						+ "        MODIFIED_ON "
						+ "    FROM GSTR1A_PROCESSED_ADV_ADJUSTMENT WHERE IS_DELETE = FALSE"
						+ conditionVertical
						+ "    UNION ALL"
						+ "    SELECT "
						+ "        SUPPLIER_GSTIN, "
						...
						+ "        NULL AS KEY, "
						+ "        (IFNULL(( CASE WHEN SUPPLY_TYPE = 'NIL' THEN TAXABLE_VALUE END ), "
						+ "        0)) + (IFNULL(( CASE WHEN SUPPLY_TYPE = 'EXT' THEN TAXABLE_VALUE END), "
						+ "        0)) + ( IFNULL(( CASE WHEN SUPPLY_TYPE IN ('NON') THEN TAXABLE_VALUE END ), "
						+ "        0)) AS SUPPLIES, "
						+ "        0 AS IGST, "
						+ "        0 AS CGST, "
						+ "        0 AS SGST, "
						+ "        0 AS CESS, "
						+ "        CASE WHEN IS_SENT_TO_GSTN = FALSE AND HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END GSTN_NOT_SENT_COUNT, "
						+ "        CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END GSTN_SAVED_COUNT, "
						+ "        CASE WHEN IS_SAVED_TO_GSTN = FALSE AND HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END GSTN_NOT_SAVED_COUNT, "
						+ "        CASE WHEN GSTN_ERROR = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END GSTN_ERROR_COUNT, "
						+ "        CASE WHEN HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END TOTAL_COUNT_IN_ASP, "
						+ "        HDR.MODIFIED_ON MODIFIED_ON "
						+ "    FROM GSTR1A_SUMMARY_NILEXTNON HDR WHERE IS_DELETE = FALSE"
						+ nillCondition
						+ "    UNION ALL "
						+ "    SELECT "
						+ "        SUPPLIER_GSTIN, "
						...
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*IFNULL(ITM.TAXABLE_VALUE, 0)"
						+ "            WHEN HDR.DOC_TYPE in ('DR','INV') THEN IFNULL(ITM.TAXABLE_VALUE,0) "
						+ "        END AS SUPPLIES, "
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*IFNULL(ITM.IGST_AMT, 0) "
						+ "             WHEN HDR.DOC_TYPE in ('DR','INV') THEN IFNULL(ITM.IGST_AMT,0) "
						+ "        END AS IGST, "
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*IFNULL(ITM.CGST_AMT,0) "
						+ "             WHEN HDR.DOC_TYPE in ('DR','INV') THEN IFNULL(ITM.CGST_AMT,0) "
						+ "        END AS CGST, "
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*IFNULL(ITM.SGST_AMT,0) "
						+ "            WHEN HDR.DOC_TYPE in ('DR','INV') THEN IFNULL(ITM.SGST_AMT,0) "
						+ "        END AS SGST, "
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*(IFNULL(ITM.CESS_AMT_SPECIFIC, 0)+ IFNULL(ITM.CESS_AMT_ADVALOREM, 0)) "
						+ "            WHEN HDR.DOC_TYPE in ('DR','INV') THEN IFNULL(ITM.CESS_AMT_SPECIFIC,0)+ IFNULL(ITM.CESS_AMT_ADVALOREM,0) "
						+ "        END AS CESS, "
						+ "        (CASE WHEN IS_SENT_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE 0 END) AS GSTN_NOT_SENT_COUNT, "
						+ "        (CASE WHEN IS_SAVED_TO_GSTN = TRUE AND IS_DELETE = FALSE THEN 1 ELSE 0 END) AS GSTN_SAVED_COUNT, "
						+ "        (CASE WHEN IS_SAVED_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE 0 END) AS GSTN_NOT_SAVED_COUNT, "
						+ "        (CASE WHEN GSTN_ERROR = TRUE AND IS_DELETE = FALSE THEN 1 ELSE 0 END) AS GSTN_ERROR_COUNT, "
						+ "        (CASE WHEN IS_DELETE = FALSE THEN 1 ELSE 0 END) AS TOTAL_COUNT_IN_ASP, "
						...
						+ "    aND (HDR.TABLE_SECTION IN ('14(ii)') AND HDR.TCS_FLAG = 'E' ) "
						+ "    UNION ALL "
						+ "    SELECT "
						+ "        SUPPLIER_GSTIN, "
						+ "        RETURN_PERIOD, "
						...
						+ "        HDR.TAX_DOC_TYPE, "
						+ "        TABLE_SECTION, "
						+ "        CASE WHEN HDR.TABLE_SECTION in ('15(i)','15(iii)') THEN DOC_KEY "
						+ "            WHEN HDR.TABLE_SECTION = '15(ii)' THEN IFNULL(HDR.ECOM_GSTIN,'')||'|'|| IFNULL(HDR.POS,'')||'|'|| IFNULL(ITM.TAX_RATE,9999) "
						+ "            WHEN HDR.TABLE_SECTION = '15(iv)' THEN IFNULL(HDR.POS, '')||'|'|| IFNULL(ITM.TAX_RATE, 9999) "
						+ "        ELSE HDR.ECOM_GSTIN END AS DOC_KEY_15, "
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*IFNULL(ITM.TAXABLE_VALUE, 0) "
						+ "             WHEN HDR.DOC_TYPE in ('DR','INV') THEN IFNULL(ITM.TAXABLE_VALUE,0) "
						+ "        END AS SUPPLIES, "
						+ "        CASE WHEN HDR.DOC_TYPE='CR' THEN -1*IFNULL(ITM.IGST_AMT,0) "
						...
						+ "    WHERE "
						+ "     IS_PROCESSED = TRUE "
						+ "    AND HDR.TABLE_SECTION IN ( '15(i)', '15(ii)', '15(iii)', '15(iv)')  "
						+ "    )"
						+ ")"
						+  groupByCondition
						+ " GROUP BY SUPPLIER_GSTIN,RETURN_PERIOD,TAX_DOC_TYPE ORDER BY LENGTH(TAX_DOC_TYPE)";
				...
				queryStr = "WITH HDR AS"
						...
						+ "             ELSE HDR.TAX_DOC_TYPE "
						+ "        END AS TAX_DOC_TYPE, "
						+ "        CASE WHEN HDR.TAX_DOC_TYPE = 'B2CS' THEN ( Ifnull(HDR.SUPPLIER_GSTIN,'')||'|'||Ifnull(HDR.RETURN_PERIOD,'')||'|'||Ifnull(HDR.DIFF_PERCENT,'')||'|'||Ifnull(HDR.POS,9999)||'|'||Ifnull(HDR.ECOM_GSTIN,'')||'|'|| Ifnull(TAX_RATE,9999) ) "
						+ "             WHEN HDR.TAX_DOC_TYPE IN ('AT','TXP') THEN ( Ifnull(HDR.SUPPLIER_GSTIN,'')||'|'||Ifnull(HDR.RETURN_PERIOD,'')||'|'||Ifnull(HDR.DIFF_PERCENT,'')||'|'||Ifnull(HDR.POS,9999)) "
						+ "        ELSE HDR.DOC_KEY END AS KEY, "
						+ "        CASE WHEN DOC_TYPE IN ('CR','C','RCR') THEN -1*IFNULL(ITM.TAXABLE_VALUE,0) ELSE IFNULL (ITM.TAXABLE_VALUE,0) END AS SUPPLIES , "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('CR','C','RCR') THEN -1*IFNULL(ITM.IGST_AMT,0) ELSE IFNULL (ITM.IGST_AMT,0) END AS IGST, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('CR','C','RCR') THEN -1*IFNULL(ITM.CGST_AMT,0) ELSE IFNULL (ITM.CGST_AMT,0) END AS CGST, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('CR','C','RCR') THEN -1*IFNULL(ITM.SGST_AMT,0) ELSE IFNULL (ITM.SGST_AMT,0) END AS SGST, "
						+ "        CASE WHEN HDR.DOC_TYPE IN ('CR','C','RCR') THEN -1*(IFNULL(ITM.CESS_AMT_SPECIFIC,0)+ifnull(ITM.CESS_AMT_ADVALOREM,0)) ELSE (IFNULL(ITM.CESS_AMT_SPECIFIC,0)+ifnull(ITM.CESS_AMT_ADVALOREM,0)) END AS CESS, "
						+ "        (CASE WHEN IS_SENT_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_NOT_SENT_COUNT, "
						+ "        (CASE WHEN IS_SAVED_TO_GSTN = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_SAVED_COUNT, "
						+ "        (CASE WHEN IS_SAVED_TO_GSTN = FALSE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_NOT_SAVED_COUNT, "
						+ "        (CASE WHEN GSTN_ERROR = TRUE AND IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_ERROR_COUNT, "
						...
						+ "        0 AS SGST, "
						+ "        0 AS CESS, "
						+ "        COUNT(CASE WHEN IS_SENT_TO_GSTN = FALSE AND HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_NOT_SENT_COUNT, "
						+ "        COUNT(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_SAVED_COUNT, "
						+ "        COUNT(CASE WHEN IS_SAVED_TO_GSTN = FALSE AND HDR.IS_DELETE = FALSE THEN 1 ELSE NULL END) GSTN_NOT_SAVED_COUNT, "
						...
						+ "    FROM GSTR1A_USERINPUT_NILEXTNON HDR WHERE IS_DELETE = FALSE"
						+ nillCondition
						+ "	GROUP BY  SUPPLIER_GSTIN,RETURN_PERIOD,DOC_KEY"
						+ "	UNION ALL "
						+ "	SELECT "
						+ "	    SUPPLIER_GSTIN, "
						+ "        HDR.RETURN_PERIOD, "
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Gstr1A Query 6 from database is-->" + bufferString);
	...
	public Integer getTabStatus(List<String> gstinList, int taxPeriod) {
		...
		String queryStr = "SELECT (CASE WHEN IS_NIL_UI = FALSE THEN 1 ELSE 2 END) "
				+ "FROM GSTN_USER_REQUEST WHERE RETURN_TYPE='GSTR1A' "
