 ...
 *
 */
@Repository("Gstr1AATARepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr1AATARepository
		extends JpaRepository<Gstr1AAdvanceAdjustmentFileUploadEntity, Long>,
		JpaSpecificationExecutor<Gstr1AAdvanceAdjustmentFileUploadEntity> {

	@Modifying
	@Query("UPDATE Gstr1AAdvanceAdjustmentFileUploadEntity txpd SET "
			+ "txpd.isDelete= TRUE WHERE txpd.txpdInvKey IN (:txpdKey) ")
	public void updateKey(@Param("txpdKey") String txpdKey);

	@Modifying
	@Query("UPDATE Gstr1AAdvanceAdjustmentFileUploadEntity doc SET "
	...
	@Query("SELECT COUNT(*) FROM Gstr1AAdvanceAdjustmentFileUploadEntity "
			+ "WHERE sgstin=:gstin AND returnPeriod = :taxperiod ")
	public int gstinCount(@Param("gstin") String gstin,
			@Param("taxperiod") String taxperiod);

	@Modifying
	@Query("UPDATE Gstr1AAdvanceAdjustmentFileUploadEntity doc SET doc.isSaved"
	...
	@Modifying
	@Query("UPDATE Gstr1AAdvanceAdjustmentFileUploadEntity b SET b.isDelete= TRUE "
			+ "WHERE b.isDelete= FALSE AND  b.id IN (:id) ")
	public int UpdateId(@Param("id") List<Long> id);

	@Query("SELECT doc.id FROM Gstr1AAdvanceAdjustmentFileUploadEntity doc "
	...
	@Modifying
	@Query("UPDATE Gstr1AAdvanceAdjustmentFileUploadEntity doc SET doc.isDelete=true,"
	...
	@Query("SELECT COUNT(*) FROM Gstr1AAdvanceAdjustmentFileUploadEntity  WHERE  "
