...
public class B2cLocation implements 
                                BusinessRuleValidator<OutwardB2cExcelEntity> {
		...
		Map<EntityAtConfigKey, Map<Long, String>> entAtConfMap = document
						...
						if (!isAttrValid) {
								...
								TransDocProcessingResultLoc location 
								      = new TransDocProcessingResultLoc(
										null, errorLocations.toArray());
								errors.add(new ProcessingResult(APP_VALIDATION,
										ProcessingResultType.INFO, "IN0204",
										" Location is not as per master "
										+ "provided during On-Boarding.",
										location));
				...
				if(loc!=null && !loc.isEmpty()){
					...
					boolean isAttrUserValid = onboardAttributesChecking
							...
							loc,document.getEntityId(),document.getCreatedBy()); 
					if(!isAttrUserValid){
						errorLocations.add(GSTConstants.LOCATION);
						TransDocProcessingResultLoc location 
						      = new TransDocProcessingResultLoc(
