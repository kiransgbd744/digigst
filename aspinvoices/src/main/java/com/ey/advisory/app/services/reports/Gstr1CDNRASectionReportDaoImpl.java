...
@Component("Gstr1CDNRASectionReportDaoImpl")
public class Gstr1CDNRASectionReportDaoImpl
		implements Gstr1ReviewSummaryCustomizedReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1CDNRASectionReportDaoImpl.class);
	...
	public static void main(String[] args) {
		...
		String queryStr = createApiProcessedQueryString(buildHeader.toString(),
				...
				String timestamp = "2024-09-12 10:19:44.18";
				DateTimeFormatter utcFormatter = DateTimeFormatter
	...
	private static String createApiProcessedQueryString(String buildHeader, String multiSupplyTypeAns) {
			...
			return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES, '') || ',' || IFNULL(ITM.ERROR_CODES, '')) AS ERROR_CODE_ASP "
					...
					+ "	,LEFT(TO_VARCHAR(GSTNBATCH.BATCH_DATE,'YYYY-MM-DD HH24:MI:SS.FF7'),23) AS GSTIN_REFID_DATE_TIME "
	...
	private static String createApiProcessedQueryStringForGstr1a(String buildHeader) {
		...
		+ "ON HDR.IRN_RESPONSE = EINV.IRN LEFT OUTER JOIN "
		+ "EWB_MASTER EWB ON HDR.EWB_NO_RESP = EWB.EWB_NUM "
		+ "WHERE HDR.ASP_INVOICE_STATUS = 2 "
		+ "AND COMPLIANCE_APPLICABLE=TRUE AND HDR.IS_DELETE=FALSE "
		+ "AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE IN ('CDNRA') "
