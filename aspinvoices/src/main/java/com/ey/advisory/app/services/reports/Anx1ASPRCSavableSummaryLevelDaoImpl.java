...
import com.ey.advisory.app.data.views.client.Anx1AspRevSumLevelDto;
...
@Component("Anx1ASPRCSavableSummaryLevelDaoImpl")
public class Anx1ASPRCSavableSummaryLevelDaoImpl
		implements Anx1ASPRCSavableSummaryDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ASPRCSavableSummaryLevelDaoImpl.class);
	...
	public List<Object> getAnx1RCSavableReports(SearchCriteria criteria) {
		...
		String queryStr = createRCSavableSummaryQueryString(
				buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertrevSummary(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1AspRevSumLevelDto convertrevSummary(Object[] arr) {
		Anx1AspRevSumLevelDto obj = new Anx1AspRevSumLevelDto();
	...
	private String createRCSavableSummaryQueryString(String buildQuery) {
		...
		return "SELECT HI.RETURN_TYPE,HI.CUST_GSTIN,HI.RETURN_PERIOD,"
				...
				+ "BT.GSTN_SAVE_REF_ID AS GSTIN_REF_ID,BT.GSTN_RESP_DATE AS "
				+ "GSTIN_REF_ID_TIME, ERROR_CODE_ASP,ERROR_DESCRIPTION_ASP,"
				+ "FIL.SOURCE AS UPLOAD_SOURCE, HI.CREATED_BY AS SOURCE_ID,"
				+ "FIL.FILE_NAME,HI.CREATED_ON AS ASP_DATE_TIME,"
				+ "HI.TOTAL_VALUE FROM ANX_PROCESSED_3H_3I HI LEFT OUTER JOIN "
				...
				+ "ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH BT ON HI.BATCH_ID = BT.ID AND "
				+ "HI.FILE_ID = BT.ID AND HI.DERIVED_RET_PERIOD = BT.DERIVED_RET_PERIOD "
