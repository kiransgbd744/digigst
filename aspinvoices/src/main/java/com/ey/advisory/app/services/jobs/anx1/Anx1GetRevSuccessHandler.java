...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1RevInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetRevSuccessHandler")
@Slf4j
public class Anx1GetRevSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("anx1RevDataParserImpl")
	private Anx1RevDataParser anx1RevDataParser;

	@Autowired
	@Qualifier("GetAnx1RevInvoicesRepository")
	private GetAnx1RevInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetRevSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1RevInvoicesHeaderEntity> b2cEntities = anx1RevDataParser
						.parseRevData(dto, apiResp);
				if (!b2cEntities.isEmpty()) {
					headerRepo.softlyDeleteRevHeader(dto.getGstin(),
							dto.getReturnPeriod());
