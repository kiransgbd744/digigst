 ...
 *
 */
public class Gstr7TdsCgstValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errorLocations.add(GSTConstants.TDS_CGST);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2017",
					"Invalid TDS CGST", location));
		...
		if (result.compareTo(BigDecimal.ZERO) < 0) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2018",
