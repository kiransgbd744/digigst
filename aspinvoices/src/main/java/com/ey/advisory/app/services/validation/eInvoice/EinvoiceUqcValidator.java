 ...
 *
 */
@Component("EinvoiceUqcValidator")
public class EinvoiceUqcValidator
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			if (uom != null && !uom.isEmpty()) {
				...
				if (item.getHsnSac() != null && !item.getHsnSac().isEmpty()
					...
					if (m == 1 || n == 1 || o == 1 ) {
						if(n==1){
							item.setUom(uomCache.uQcDescAndCodemap().get(trimAndConvToUpperCase(uom)));
						}
						if(o==1){
							 
							item.setUom(uomCache.uQcDesc().get(trimAndConvToUpperCase(uom)));
						}
					}else{
						...
						if(CONFIG_PARAM_OUTWARD_ANS_KEY_ID.B.name().equalsIgnoreCase(paramtrvalue)){
					...
					item.setUom(GSTConstants.OTH);
						...
						errors.add(new ProcessingResult(APP_VALIDATION,
								...
								"Unit Of Measurement is mandatory as per "
									+ "On-Boarding  parameter.", location));
				}
