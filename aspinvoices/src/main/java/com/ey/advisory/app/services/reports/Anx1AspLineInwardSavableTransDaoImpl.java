...
@Component("Anx1AspLineInwardSavableTransDaoImpl")
public class Anx1AspLineInwardSavableTransDaoImpl
		implements Anx1AspLineInwardSavableSummaryDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ASPIMPSSavableTransDaoImpl.class);

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	@Override
	public List<Object> getLineInwardSavableReports(SearchCriteria criteria) {
		...
		String queryStr = createLineInwardQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertProcessedLineInward(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1ProcessedRecordsAspInwardDto convertProcessedLineInward(
			Object[] arr) {
	...
	private String createLineInwardQueryString(String buildQuery) {
		...
		return "SELECT  HDR.USER_ID,HDR.SOURCE_FILENAME,"
				...
				+ "HDR.DOC_DATE,ITM.ORIGINAL_DOC_NUM, ITM.ORIGINAL_DOC_DATE,"
				...
				+ "ITM.CIF_VALUE,ITM.CUSTOM_DUTY, ITM.ITM_HSNSAC, ITM.PRODUCT_CODE,"
				...
				+ " ITM.STATECESS_AMT,ITM.OTHER_VALUES, TO_CHAR(ITM.LINE_ITEM_AMT) "
				...
				+ "HDR.REVERSE_CHARGE, ITM.ELIGIBILITY_INDICATOR AS"
				...
				+ "HDR.AN_TAX_DOC_TYPE AS DATA_CATEGORY,HDR.AN_TABLE_SECTION AS"
				+ " TABLE_NUMBER,TRIM(', ' FROM IFNULL(ERRH.INFO_ERROR_CODE_ASP"
				+ ",'') ||','|| IFNULL(ERRI.INFO_ERROR_CODE_ASP,'')) AS "
				+ "INFO_ERROR_CODE_ASP, TRIM(', ' FROM IFNULL"
				+ "(ERRH.INFO_ERROR_DESCRIPTION_ASP,'') ||','|| "
				+ "IFNULL(ERRI.INFO_ERROR_DESCRIPTION_ASP,'')) AS "
				+ "INFO_ERROR_DESCRIPTION_ASP, HDR.DOC_AMT AS INV_VALUE, "
				+ "case when HDR.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED'"
				+ " WHEN HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = "
				+ "TRUE THEN 'IS_ERROR' WHEN HDR.IS_SAVED_TO_GSTN = FALSE "
				+ "AND HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END AS "
				+ "SAVE_STATUS, GSB.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, "
				+ "GSB.GSTN_RESP_DATE AS GSTIN_REF_ID_TIME, TRIM(', ' "
				+ "FROM IFNULL(ERRH.ERROR_CODE_GSTIN,'') ||','|| IFNULL"
				+ "(ERRI.ERROR_CODE_GSTIN,'')) AS GSTIN_ERROR_CODE_ASP, "
				...
				+ " AS ASP_DATE_TIME FROM  ANX_INWARD_DOC_HEADER HDR LEFT"
				+ " OUTER JOIN  TF_INWARD_HEADER_GSTIN_ASP_ERROR_INFO () "
				...
				+ "ERRI ON ITM.DOC_HEADER_ID= ERRI.DOC_HEADER_ID AND IFNULL"
				+ "(ITM.ITEM_INDEX,'-1' ) = IFNULL(ERRI.ITEM_INDEX,'-1' ) AND"
				+ " IFNULL(ITM.ITM_NO,'0' ) = IFNULL(ERRI.ITM_NO,'0' ) INNER "
				+ "JOIN  FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ " GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID = HDR.BATCH_ID WHERE "
				+ "IS_ERROR=FALSE AND HDR.AN_RETURN_TYPE IN ('ANX1') "
