 ...
 *
 */
@Component("Gstr9InOutwardProcessedInfoReportsDaoImpl")
@Slf4j
public class Gstr9InOutwardProcessedInfoReportsDaoImpl
	...
	public List<Object> getGstr1VerticalReports(
		...
		String queryStr = createVerticalProcessedInfoQueryString(
				buildQuery.toString());
	...
	private String createVerticalProcessedInfoQueryString(String buildQuery) {
		...
		if ((GSTConstants.INWARD_OUTWARD).equalsIgnoreCase(fileType)) {
			...
			builder.append(
					"FROM TBL_GSTR9_USER_INPUT HI LEFT OUTER JOIN TF_VERTICAL_ERROR_INFO() ERR ON ");
			builder.append(
					"HI.AS_ENTERED_ID= ERR.COMMON_ID AND HI.FILE_ID = ERR.FILE_ID ");
			builder.append(
					"LEFT OUTER JOIN FILE_STATUS FIL ON HI.FILE_ID = FIL.ID AND ERR.FILE_ID = FIL.ID ");
			builder.append("AND ERR.INV_KEY=HI.DOC_KEY ");
			builder.append("WHERE HI.IS_INFORMATION = TRUE And ");
