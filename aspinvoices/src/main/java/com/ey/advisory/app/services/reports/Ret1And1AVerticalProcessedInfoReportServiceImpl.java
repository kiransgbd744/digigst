...
@Service("Ret1And1AVerticalProcessedInfoReportServiceImpl")
public class Ret1And1AVerticalProcessedInfoReportServiceImpl
		implements Ret1And1AVerticalProcessedReportService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Ret1And1AVerticalProcessedInfoReportServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Ret1And1AVerticalProcessedInfoReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		LOGGER.debug(
				"Ret1And1A vertical ProcessInfo response" + responseFromView);
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((DownloadReportsConstant.RET1AND1A)
				...
				String[] userinputsHeaders = commonUtility
						.getProp(
								"ret1and1a.userinputs.processinfo.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"RET1And1AUSERINPUTSProcessedInfo.xlsx");
				...
				String[] interestHeaders = commonUtility
						.getProp(
								"ret1and1a.interestandlatefee.processinfo.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"INTERESTAndLATEFEEProcessedInfo.xlsx");
				...
				String[] setoffHeaders = commonUtility
						.getProp(
								"ret1and1a.setoffutil.processinfo.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"SETOFFUTILIZATIONProcessedInfo.xlsx");
				...
				String[] refundsHeaders = commonUtility
						.getProp("ret1and1a.refunds.processinfo.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"REFUNDSProcessedInfo.xlsx");
