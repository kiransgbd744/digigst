...
@Slf4j
@Component("Gstr7TransRSProcessedRptDwnldServiceImpl")
public class Gstr7TransRSProcessedRptDwnldServiceImpl
	...
	private static final String CHUNK_PROC = "USP_INS_CHUNK_OD_PS_GSTR7_ASP_PROCESS_UPLOADED";
	private static final String DISP_PROC = "USP_DY_CHUNK_DISP_OD_PS_GSTR7_ASP_PROCESS";
	...
	public void generateReports(Long id, String orgReportType) {
		...
		try {
			...
			String finalGstinList = String.join(",", gstins);
			
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format("finalGstinList : %s",finalGstinList);
				LOGGER.debug(msg);
			}
			...
			fileName = "GSTR7_"+finalGstinList+"_ProcessedAsUploadedRecords_";
			...
			fullPath = tempDir.getAbsolutePath() + File.separator + fileName
					+ entityName+"_" + format.format(convertISDDate) + ".csv";
			...
			while (j < chunkCount) {
				j++;
				List<Object[]> records = getProcData(DISP_PROC, id, j);
