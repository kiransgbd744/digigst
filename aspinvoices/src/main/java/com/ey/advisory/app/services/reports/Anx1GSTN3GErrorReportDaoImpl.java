...
import com.ey.advisory.app.data.views.client.Anx1GSTN3GErrorReportsView;
import com.ey.advisory.app.util.OnboardingConstant;
import com.ey.advisory.common.GenUtil;
import com.ey.advisory.core.dto.Gstr1ReviwSummReportsReqDto;
import com.ey.advisory.core.search.SearchCriteria;

@Component("Anx1GSTN3GErrorReportDaoImpl")
public class Anx1GSTN3GErrorReportDaoImpl implements Anx1GstnErrorDao {
	...
	public List<Object> getAnx1GstnErrorReport(SearchCriteria criteria) {
		...
		String queryStr = GSTN3GErrorQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertGSTN3GError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1GSTN3GErrorReportsView convertGSTN3GError(Object[] arr) {
		Anx1GSTN3GErrorReportsView obj = new Anx1GSTN3GErrorReportsView();
		...
		obj.setClaimRefundFlag(arr[17] != null ? arr[17].toString() : null);
		obj.setgSTNErrorCode(arr[18] != null ? arr[18].toString() : null);
		obj.setgSTNErrorMessage(arr[19] != null ? arr[19].toString() : null);
		obj.setRefID(arr[20] != null ? arr[20].toString() : null);
		obj.setRefDate(arr[21] != null ? arr[21].toString() : null);

		return obj;
	}

	private String GSTN3GErrorQueryString(String buildQuery) {
		return "SELECT HT.SUPPLIER_GSTIN,HT.CUST_GSTIN,DOC_TYPE,"
				+ "DOC_NUM,HT.DOC_DATE,ITM_HSNSAC,TAX_RATE,"
				...
				+ "SUM(DOC_AMT) INVVAL,HT.POS,HT.DIFF_PERCENT,"
				+ "HT.AUTOPOPULATE_TO_REFUND,"
				+ "HT.SECTION7_OF_IGST_FLAG,HT.CLAIM_REFUND_FLAG,"
				...
				+ "AND UCASE(HT.AN_TAX_DOC_TYPE) = UCASE('DXP') "
				+ "AND HT.GSTN_ERROR = TRUE "
				+ "INNER JOIN ANX_OUTWARD_DOC_ERROR ERT ON HT.ID = ERT.DOC_HEADER_ID "
				+ "AND IT.DOC_HEADER_ID = ERT.DOC_HEADER_ID INNER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID = HT.BATCH_ID "
				+ buildQuery + " GROUP BY "
				+ "HT.SUPPLIER_GSTIN,HT.CUST_GSTIN,DOC_TYPE,DOC_NUM,"
				+ "HT.DOC_DATE,ITM_HSNSAC,TAX_RATE,HT.POS,HT.DIFF_PERCENT,"
				+ "HT.AUTOPOPULATE_TO_REFUND,HT.SECTION7_OF_IGST_FLAG,"
				+ "HT.CLAIM_REFUND_FLAG,GSTN_SAVE_REF_ID,GSTN_RESP_DATE"
