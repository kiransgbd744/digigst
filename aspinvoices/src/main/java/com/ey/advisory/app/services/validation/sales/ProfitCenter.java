...
public class ProfitCenter implements DocRulesValidator<OutwardTransDocument> {
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		IntStream.range(0, items.size()).forEach(idx -> {
						...
						if (!isAttrValid) {
							...
							TransDocProcessingResultLoc location 
							  = new TransDocProcessingResultLoc(
									idx, errorLocations.toArray());
							errors.add(new ProcessingResult(APP_VALIDATION,
									ProcessingResultType.INFO, "IN0010",
									" Profit Centre is not as per master "
											+ "provided during On-Boarding.",
									location));
			...
			if (profitCentre != null && !profitCentre.isEmpty()) {
				String paramkeyId12 = CONFIG_PARAM_OUTWARD_QUE_KEY_ID.O12
						.name();
				...
				if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
					...
					if (!isAttrUserValid) {
						...
						= new TransDocProcessingResultLoc(
								idx, errorLocations.toArray());
						errors.add(
								new ProcessingResult(APP_VALIDATION, "ER0153",
										"Profit Centre is not as "
