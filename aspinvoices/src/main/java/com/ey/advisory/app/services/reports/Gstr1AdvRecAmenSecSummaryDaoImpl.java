 ...
 * 
 */
@Component("Gstr1AdvRecAmenSecSummaryDaoImpl")
	public class Gstr1AdvRecAmenSecSummaryDaoImpl
			implements Gstr1AdvRecSavableDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr1AdvRecAmenSecSummaryDaoImpl.class);
		...
		public List<Object> getGstr1AdvRecSavableReports(SearchCriteria criteria) {
			...
			if (GSTIN != null && !GSTIN.isEmpty()) {
				if (gstinList != null && gstinList.size() > 0) {
					buildQuery.append(" AND REC.SUPPLIER_GSTIN IN :gstinList");
				}
			}
			if (ProfitCenter != null && !ProfitCenter.isEmpty()) {
				if (pcList != null && pcList.size() > 0) {
					buildQuery.append(" AND REC.PROFIT_CENTRE IN :pcList");
				}
			}
			if (plant != null && !plant.isEmpty()) {
				if (plantList != null && plantList.size() > 0) {
					buildQuery.append(" AND REC.PLANT_CODE IN :plantList");
				}
			}
			if (sales != null && !sales.isEmpty()) {
				if (salesList != null && salesList.size() > 0) {
					buildQuery.append(" AND REC.SALES_ORGANIZATION IN :salesList");
				}
			}
			if (distChannel != null && !distChannel.isEmpty()) {
				if (distList != null && distList.size() > 0) {
					buildQuery.append(" AND REC.DISTRIBUTION_CHANNEL IN :distList");
				}
			}
			if (division != null && !division.isEmpty()) {
				if (divisionList != null && divisionList.size() > 0) {
					buildQuery.append(" AND REC.DIVISION IN :divisionList");
				}
			}
			if (location != null && !location.isEmpty()) {
				if (locationList != null && locationList.size() > 0) {
					buildQuery.append(" AND REC.LOCATION IN :locationList");
				}
			}
			if (ud1 != null && !ud1.isEmpty()) {
				if (ud1List != null && ud1List.size() > 0) {
					buildQuery.append(" AND REC.USERACCESS1 IN :ud1List");
				}
			}
			if (ud2 != null && !ud2.isEmpty()) {
				if (ud2List != null && ud2List.size() > 0) {
					buildQuery.append(" AND REC.USERACCESS2 IN :ud2List");
				}
			}
			if (ud3 != null && !ud3.isEmpty()) {
				if (ud3List != null && ud3List.size() > 0) {
					buildQuery.append(" AND REC.USERACCESS3 IN :ud3List");
				}
			}
			if (ud4 != null && !ud4.isEmpty()) {
				if (ud4List != null && ud4List.size() > 0) {
					buildQuery.append(" AND REC.USERACCESS4 IN :ud4List");
				}
			}
			if (ud5 != null && !ud5.isEmpty()) {
				if (ud5List != null && ud5List.size() > 0) {
					buildQuery.append(" AND REC.USERACCESS5 IN :ud5List");
				}
			}
			if (ud6 != null && !ud6.isEmpty()) {
				if (ud6List != null && ud6List.size() > 0) {
					buildQuery.append(" AND REC.USERACCESS6 IN :ud6List");
				}
			}

			if (taxperiod != null && !taxperiod.isEmpty()) {
				buildQuery.append(" AND REC.DERIVED_RET_PERIOD = :taxperiod ");
			...
			if (APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
				...
				queryStr = createApiProcessedQueryString(buildQuery.toString());
		...
		private String createApiProcessedQueryString(String buildQuery) {
			...
			return "SELECT REC.SUPPLIER_GSTIN,REC.RETURN_PERIOD,TRAN_TYPE,"
					+ "MONTH, ORG_POS,ORG_RATE,ORG_GROSS_ADV_RECEIVED, NEW_POS,NEW_RATE, NEW_GROSS_ADV_RECEIVED, IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT, PROFIT_CENTRE,PLANT_CODE,DIVISION,LOCATION,SALES_ORGANIZATION, DISTRIBUTION_CHANNEL, USERACCESS1,USERACCESS2,USERACCESS3, USERACCESS4,USERACCESS5,USERACCESS6, USERDEFINED_FIELD1, USERDEFINED_FIELD2,USERDEFINED_FIELD3, TRIM(', ' FROM IFNULL(INFO_ERROR_CODE_ASP,'') ) AS INFO_ERROR_CODE_ASP, TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) AS INFO_ERROR_DESCRIPTION_ASP, (CASE WHEN REC.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = TRUE THEN 'IS_ERROR' WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) AS SAVE_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME, '' AS GSTN_ERROR_CODE, '' AS GSTN_ERROR_DESCRIPTION, FIL.CREATED_BY AS SOURCE_ID, FIL.FILE_NAME AS FILE_NAME, REC.CREATED_ON AS ASP_DATE_TIME FROM GSTR1_PROCESSED_ADV_RECEIVED REC LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() GSTRERR ON REC.AS_ENTERED_ID= GSTRERR.COMMON_ID AND REC.FILE_ID=GSTRERR.FILE_ID "
					+ "AND GSTRERR.TABLE_TYPE='ADV REC' AND RETURN_TYPE='GSTR1' "
					+ "LEFT OUTER JOIN FILE_STATUS FIL ON "
					+ "(REC.FILE_ID=FIL.ID OR GSTRERR.FILE_ID=FIL.ID)"
					+ " LEFT OUTER JOIN "
					+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = REC.BATCH_ID "
					+ "WHERE REC.IS_DELETE=FALSE AND REC.IS_AMENDMENT=TRUE "
					+ buildQuery;
		}
		
		private String createApiProcessedQueryStringForGstr1a(String buildQuery) {

			return "SELECT REC.SUPPLIER_GSTIN,REC.RETURN_PERIOD,TRAN_TYPE,"
					+ "MONTH, ORG_POS,ORG_RATE,ORG_GROSS_ADV_RECEIVED, NEW_POS,NEW_RATE, NEW_GROSS_ADV_RECEIVED, IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT, PROFIT_CENTRE,PLANT_CODE,DIVISION,LOCATION,SALES_ORGANIZATION, DISTRIBUTION_CHANNEL, USERACCESS1,USERACCESS2,USERACCESS3, USERACCESS4,USERACCESS5,USERACCESS6, USERDEFINED_FIELD1, USERDEFINED_FIELD2,USERDEFINED_FIELD3, TRIM(', ' FROM IFNULL(INFO_ERROR_CODE_ASP,'') ) AS INFO_ERROR_CODE_ASP, TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) AS INFO_ERROR_DESCRIPTION_ASP, (CASE WHEN REC.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = TRUE THEN 'IS_ERROR' WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) AS SAVE_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME, '' AS GSTN_ERROR_CODE, '' AS GSTN_ERROR_DESCRIPTION, FIL.CREATED_BY AS SOURCE_ID, FIL.FILE_NAME AS FILE_NAME, REC.CREATED_ON AS ASP_DATE_TIME FROM GSTR1A_PROCESSED_ADV_RECEIVED REC LEFT OUTER JOIN TF_GSTR1A_ERROR_INFO() GSTRERR ON REC.AS_ENTERED_ID= GSTRERR.COMMON_ID AND REC.FILE_ID=GSTRERR.FILE_ID "
					+ "AND GSTRERR.TABLE_TYPE='ADV REC' AND RETURN_TYPE='GSTR1A' "
					+ "LEFT OUTER JOIN FILE_STATUS FIL ON "
					+ "(REC.FILE_ID=FIL.ID OR GSTRERR.FILE_ID=FIL.ID)"
					+ " LEFT OUTER JOIN "
					+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = REC.BATCH_ID "
