...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1SezwopaInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetSezwopaSuccessHandler")
@Slf4j
public class Anx1GetSezwopaSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Anx1SezwopaDataParserImpl")
	private Anx1SezwopaDataParser anx1SezwopaDataParser;

	@Autowired
	@Qualifier("GetAnx1SezwopaInvoicesRepository")
	private GetAnx1SezwopaInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetSezwopaSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1SezwopaInvoicesHeaderEntity> b2cEntities = anx1SezwopaDataParser
						.parseSezwopaData(dto, apiResp);
				if (!b2cEntities.isEmpty()) {
					headerRepo.softlyDeleteSezwopaHeader(dto.getGstin(),
							dto.getReturnPeriod());
