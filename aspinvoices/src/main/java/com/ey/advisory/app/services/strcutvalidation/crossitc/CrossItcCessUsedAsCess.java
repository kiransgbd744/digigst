...
public class CrossItcCessUsedAsCess implements ValidationRule {
	...
	public List<ProcessingResult> isValid(Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errorLocations.add(GSTConstants.CESS_USED_AS_CESS);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER6146",
					"Invalid CessusedasCess.", location));
			// if it's not a number, then we can return the errors
			// immediately
			return errors;
		}
		boolean isValid = NumberFomatUtil.is3digValidDec(obj.toString().trim());
