...
import com.ey.advisory.app.services.jobs.gstr1.Gstr1B2csB2csaDataAtGstn;
 ...
 *
 */
@Component("gstr1AB2csB2csaDataAtGstnImpl")
@Slf4j
public class Gstr1AB2csB2csaDataAtGstnImpl implements Gstr1B2csB2csaDataAtGstn {
	...
	public Long findB2CSDataATGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.B2CS.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_B2CS);
			}
		} else {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_B2CSA);
		}
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1AB2csB2csaSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
