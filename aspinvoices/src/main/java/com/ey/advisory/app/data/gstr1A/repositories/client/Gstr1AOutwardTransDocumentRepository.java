...
@Repository("Gstr1AOutwardTransDocumentRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr1AOutwardTransDocumentRepository
		extends CrudRepository<Gstr1AOutwardTransDocument, Long> {
	 ...
	 *//**
		 ...
		 */
	@Modifying
	@Query("UPDATE Gstr1AOutwardTransDocument outWardEntity SET "
	...
	Optional<Gstr1AOutwardTransDocument> findByEwbNorespAndIsDeleted(Long ewbNo,
			boolean b);

	// @Query("SELECT doc from Gstr1AOutwardTransDocument doc WHERE id = :id")
	public Optional<Gstr1AOutwardTransDocument> findById(/* @Param ("id") */ Long id);

	Optional<Gstr1AOutwardTransDocument> findByIdAndIsDeleted(Long docHeaderId,
			boolean b);
	...
	public List<Gstr1AOutwardTransDocument> findInactiveRecordsByDocKey(
			@Param("docKeys") String docKeys);
	
	@Query("SELECT doc FROM Gstr1AOutwardTransDocument doc "
			+ "WHERE doc.ewbNoresp IN (:ewbNo) "
			+ "AND doc.isDeleted = false AND doc.aspInvoiceStatus=2 ")
	List<Gstr1AOutwardTransDocument> findByEwayBillNo(@Param("ewbNo") List<Long> ewbNo);
