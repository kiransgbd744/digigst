 ...
 *
 */
@Service("Gstr6RateDataToCdnConverter")
@Slf4j
public class Gstr6RateDataToCdnConverter implements RateDataToGstr6Converter {
		...
		try {
			if (objects != null && !objects.isEmpty()) {
				...
				LOGGER.debug("{} Batch seperation is started with {} docs",
						section, totSize);
				List<Gstr6CdnDto> cdnList = new ArrayList<>();
				List<Gstr6CdnNtData> invList = new ArrayList<>();
				...
				for (int counter = 0; counter < totSize; counter++) {
					...
					if (isNewInvoice(arr1, arr2, idsList, totSize, counter2)) {
						idsList.add(id);
						Gstr6CdnNtData inv = setInvData(arr1, itmsList,
								taxDocType);
						invList.add(inv);
					...
					if (isNewCtin(arr1, arr2, idsList, totSize, counter2)) {
						Gstr6CdnDto b2b = setInv(arr1, invList);
						cdnList.add(b2b);
						itmsList = new ArrayList<>();
					...
					if (isNewBatch(idsList, totSize, counter2)) {
						SaveGstr6 gstr6 = setBatch(arr1, section, cdnList);
	...
	private Gstr6CdnDto setInv(Object[] arr1, List<Gstr6CdnNtData> invList) {
		Gstr6CdnDto data = new Gstr6CdnDto();
	...
	private Gstr6CdnNtData setInvData(Object[] arr1, List<Gstr6Items> itmsList,
			String taxDocType) {
		Gstr6CdnNtData data = new Gstr6CdnNtData();
		...
		if(ntty != null){
			data.setNtty(ntty.equalsIgnoreCase("CR") ? "C" : "D");
		}
		data.setVal(arr1[9] != null
				? new BigDecimal(String.valueOf(arr1[9])).abs() : BigDecimal.ZERO);		
		if (gstnapi.isDelinkingEligible(APIConstants.GSTR6.toUpperCase())) {
	    	data.setPos(arr1[17] != null ? String.valueOf(arr1[17]) : null);
	    }else{
	...
	private Gstr6Items setItemDetail(Object[] arr1, int itemNumber) {
		...
		String pos = arr1[17] != null
				? (String.valueOf(arr1[17]).trim().length() == 1
						? "0".concat(String.valueOf(arr1[17]).trim())
						: String.valueOf(arr1[17]))
				: null;
		String cGstin = arr1[3] != null ? String.valueOf(arr1[3]) : null;
		...
		details.setRate(taxRate.stripTrailingZeros());
