...
@Slf4j
@Service("Gstr1AProcessedRecordsCommonUtil")
public class Gstr1AProcessedRecordsCommonUtil {
	...
	@Autowired
	private Gstr1AEinvSeriesCompRepo gstr1EinvSeriesCompRepo;
		...
		sortedGstinDtoList.stream().forEach(dto -> {
			...
			if (isAvailable.isPresent()) {
				if ("C".equalsIgnoreCase(
					...
					Optional<Gstr1AEinvSeriesCompEntity> isCompDtlsAva = gstr1EinvSeriesCompRepo
	...
	private int checkTheCountByGstinTaxperiod(String gstin, String taxPeriod) {
		...
		int b2csCount = gstr1AB2CSRepository.gstinCount(gstin, taxPeriod);
		int arCount = gstr1AARRepository.gstinCount(gstin, taxPeriod);
		int ataCount = gstr1AATARepository.gstinCount(gstin, taxPeriod);
