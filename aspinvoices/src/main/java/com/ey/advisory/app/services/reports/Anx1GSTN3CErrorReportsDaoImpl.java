...
import com.ey.advisory.app.data.views.client.Anx1GSTN3CErrorReportsView;
...
@Component("Anx1GSTN3CErrorReportsDaoImpl")
public class Anx1GSTN3CErrorReportsDaoImpl implements Anx1GstnErrorDao {
	...
	public List<Object> getAnx1GstnErrorReport(SearchCriteria criteria) {
		...
		String queryStr = GSTN3CErrorQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertgstn3CError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1GSTN3CErrorReportsView convertgstn3CError(Object[] arr) {
		Anx1GSTN3CErrorReportsView obj = new Anx1GSTN3CErrorReportsView();
		...
		obj.setIntegTaxAmt(arr[10] != null ? arr[10].toString() : null);
	...
	private String GSTN3CErrorQueryString(String buildQuery) {
		return "SELECT HT.SUPPLIER_GSTIN,DOC_TYPE,DOC_NUM,"
				+ "HT.DOC_DATE,HT.SHIP_PORT_CODE,HT.SHIP_BILL_NUM,"
				+ "HT.SHIP_BILL_DATE,ITM_HSNSAC,TAX_RATE,"
				+ "SUM(IT.TAXABLE_VALUE) TAXABLE_VALUE,"
				+ "SUM(IT.IGST_AMT) IGST_AMT,SUM(IT.CESS_AMT_SPECIFIC) CESS_AMT,"
				+ "SUM(DOC_AMT) INVVAL,"
				...
				+ "AND UCASE(HT.AN_TAX_DOC_TYPE) = UCASE('EXPT') "
				...
				+ buildQuery
				+ "GROUP BY HT.SUPPLIER_GSTIN,DOC_TYPE,DOC_NUM,HT.DOC_DATE,"
				+ "HT.SHIP_PORT_CODE,HT.SHIP_BILL_NUM,HT.SHIP_BILL_DATE,"
				+ "ITM_HSNSAC,TAX_RATE,HT.DIFF_PERCENT,"
