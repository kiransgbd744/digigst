 ...
 */
@Slf4j
@Service("Gstr9HsnFileArrivalHandler")
public class Gstr9HsnFileArrivalHandler {
	...
	@Autowired
	@Qualifier("Gstr9HsnFileProcessService")
	private Gstr9HsnFileProcessService gstr9HsnFileProcessService;

	@Transactional(value = "clientTransactionManager")
	public void processGstr9HsnFile(Message message, AppExecContext context) {
		...
		try {
			...
			String msg1 = "Exception occured in Gstr9 hsn"
					+ "File Arrival Processor";
			e.printStackTrace();
			LOGGER.error(msg1, e);
		...
		List<Object[]> gstr9HsnList = ((FileUploadDocRowHandler<?>) rowHandler)
				.getFileUploadList();
		...
		if (checkHeaderFormat.getValue0()) {
			gstr9HsnFileProcessService.validateAndProcessGstr9HsnFileData(
					gstr9HsnList, updateFileStatus, gstr1FileStatusRepository);
		} else {
