...
public class IntraSateRegistered implements B2csBusinessRuleValidator<Gstr1NilDetailsEntity> {
	 private static DecimalFormat df = new DecimalFormat("0.00");
	@Override
	public List<ProcessingResult> validate(Gstr1NilDetailsEntity document, ProcessingContext context) {
		List<ProcessingResult> errors = new ArrayList<>();
		if(document.getIntraStateRegistered()!=null){
			document.setIntraStateRegistered(new BigDecimal(df.format(document.getIntraStateRegistered())));
		}
		return errors;
	}
