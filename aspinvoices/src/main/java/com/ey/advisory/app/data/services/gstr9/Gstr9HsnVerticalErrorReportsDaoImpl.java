 ...
 *
 */
@Component("Gstr9HsnVerticalErrorReportsDaoImpl")
public class Gstr9HsnVerticalErrorReportsDaoImpl implements Gstr7VerticalDao {

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr9HsnVerticalErrorReportsDaoImpl.class);
	...
	private Gstr9HsnTotalRecords convertVerticalTotal(Object[] arr) {
		...
		/*
		 * String errDesc = null; String errCode = (arr[0] != null ?
		 * arr[0].toString() : null);
		 */
		 ...
		 * ErrorMasterUtil.getErrorCodeWithoutIndex(errCodeList, "GSTR9"); }
		...
		obj.setTotalQuantity(arr[9] != null ? arr[9].toString() : null);
		/*
		...
		obj.setIgst(arr[12] != null ? arr[12].toString() : null);
		obj.setCgst(arr[13] != null ? arr[13].toString() : null);
		obj.setSgst(arr[14] != null ? arr[14].toString() : null);
		obj.setCess(arr[15] != null ? arr[15].toString() : null);
	...
	private String createVerticalTotalQueryString(String buildQuery) {
		...
		builder.append(
				"SELECT ERROR_CODE,ERROR_DESCRIPTION, HI.GSTIN,FY,TABLE_NUMBER,HSN,DESCRIPTION,RATE_OF_TAX,UQC, ");
		builder.append(
		...
		builder.append(
				"FROM GSTR9_HSN_AS_ENTERED HI INNER JOIN VERTICAL_ERROR ERR ");
		builder.append(
				"ON HI.ID = ERR.COMMON_ID AND HI.FILE_ID=ERR.FILE_ID LEFT JOIN FILE_STATUS FIL ");
		builder.append(
				"ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID AND ERR.INV_KEY=HI.DOC_KEY  WHERE IS_ERROR = TRUE ");
		builder.append(buildQuery);
		builder.append("ORDER BY HSN ");
