 ...
 *
 */
@Repository("Gstr1AARRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr1AARRepository
		extends JpaRepository<Gstr1AARDetailsEntity, Long>,
		JpaSpecificationExecutor<Gstr1AARDetailsEntity> {

	@Query("SELECT doc.id FROM Gstr1AARDetailsEntity doc "
			+ "WHERE doc.invAtKey IN (:docKeys) AND doc.isDelete = false ")
	List<Long> findActiveDocsByDocKeys(@Param("docKeys") List<String> docKeys);

	
	@Modifying
	@Query("UPDATE Gstr1AARDetailsEntity doc SET doc.isGstnError=true, "
			+ "doc.isSaved=false WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIserrorfalg(@Param("gstnBatchId") Long gstnBatchId);
	
	@Modifying
	@Query("UPDATE Gstr1AARDetailsEntity doc SET doc.isSaved=true, "
			+ "doc.isGstnError =false WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIsSavedToGstn(@Param("gstnBatchId") Long gstnBatchId);

	
	@Modifying
	@Query("UPDATE Gstr1AARDetailsEntity doc SET doc.gstnBatchId=:gstnBatchId,"
	...
	@Query("SELECT COUNT(*) FROM Gstr1AARDetailsEntity "
			+ "WHERE sgstin=:gstin AND returnPeriod = :taxperiod ")
	public int gstinCount(@Param("gstin") String gstin,
			@Param("taxperiod") String taxperiod);

	@Modifying
	@Query("UPDATE Gstr1AARDetailsEntity b SET b.isDelete= TRUE "
			+ "WHERE b.isDelete= FALSE AND  b.id IN (:id) ")
	public int UpdateId(
			@Param("id") List<Long> id);

	@Modifying
	@Query("UPDATE Gstr1AARDetailsEntity doc SET doc.isDelete=true,"
	...
	@Query("SELECT COUNT(*) FROM Gstr1AARDetailsEntity  WHERE  "
