...
import jakarta.persistence.Query;
	...
	@Component("Gstr1AAspAdvRecAmenSecSummaryDaoImpl")
	public class Gstr1AAspAdvRecAmenSecSummaryDaoImpl
			implements Gstr1AAdvRecSavableDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr1AAspAdvRecAmenSecSummaryDaoImpl.class);
		...
		public List<Object> getGstr1AdvRecSavableReports(SearchCriteria criteria) {
			...
			if (taxPeriodFrom != null && taxPeriodTo != null) {
				buildQuery.append(" AND REC.DERIVED_RET_PERIOD BETWEEN ");
		...
		private String createApiProcessedQueryString(String buildQuery) {
			...
			return "SELECT REC.SUPPLIER_GSTIN,REC.RETURN_PERIOD,TRAN_TYPE,"
					+ "MONTH, ORG_POS,ORG_RATE,ORG_GROSS_ADV_RECEIVED, NEW_POS,NEW_RATE, NEW_GROSS_ADV_RECEIVED,"
					+ " IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT, PROFIT_CENTRE,PLANT_CODE,DIVISION,LOCATION,SALES_ORGANIZATION,"
					+ " DISTRIBUTION_CHANNEL, USERACCESS1,USERACCESS2,USERACCESS3, USERACCESS4,USERACCESS5,USERACCESS6,"
					+ " USERDEFINED_FIELD1, USERDEFINED_FIELD2,USERDEFINED_FIELD3, TRIM(', ' FROM IFNULL(INFO_ERROR_CODE_ASP,'') ) AS INFO_ERROR_CODE_ASP,"
					+ " TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) AS INFO_ERROR_DESCRIPTION_ASP,"
					+ " (CASE WHEN REC.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = TRUE THEN 'IS_ERROR'"
					+ " WHEN REC.IS_SAVED_TO_GSTN = FALSE AND REC.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) AS SAVE_STATUS,"
					+ " GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME, '' AS GSTN_ERROR_CODE,"
					+ " '' AS GSTN_ERROR_DESCRIPTION, FIL.CREATED_BY AS SOURCE_ID, FIL.FILE_NAME AS FILE_NAME,"
					+ " REC.CREATED_ON AS ASP_DATE_TIME FROM GSTR1A_PROCESSED_ADV_RECEIVED REC LEFT OUTER JOIN TF_GSTR1A_ERROR_INFO()"
					+ " GSTRERR ON REC.AS_ENTERED_ID= GSTRERR.COMMON_ID AND REC.FILE_ID=GSTRERR.FILE_ID "
