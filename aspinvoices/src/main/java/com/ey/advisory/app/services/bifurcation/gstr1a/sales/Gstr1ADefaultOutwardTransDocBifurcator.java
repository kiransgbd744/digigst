...
@Component("Gstr1ADefaultOutwardTransDocBifurcator")
@Slf4j
public class Gstr1ADefaultOutwardTransDocBifurcator
	...
	private Gstr1AOutwardTransDocument checkBasedOnOnboardingAnswer(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
			...
			if (docType1.contains(document.getDocType())) {
				if (GSTConstants.Y
					...
					document.setGstrReturnType(
							BifurcationConstants.GSTR1A_RETURN_TYPE);
				} else {
	...
	private Gstr1AOutwardTransDocument checkForTable4(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
		String docNum = document.getDocNo();
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
		...
		document.setGstnBifurcation(GSTConstants.GSTR1_B2B);
		document.setGstrReturnType(BifurcationConstants.GSTR1A_RETURN_TYPE);
	...
	private Gstr1AOutwardTransDocument checkForTable5(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {

		Integer derivedTaxPeriod = document.getDerivedTaxperiod();
		Object attribute = context.getAttribute(CONFI_KEY);

		BigDecimal threshold = getThresholdForTaxPeriod((String) attribute,
				derivedTaxPeriod);
	...
	private Gstr1AOutwardTransDocument checkForTable6(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
	...
	private Gstr1AOutwardTransDocument checkForTable7(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
		...
		if (isDocExist) {
			...
			if (docAmount == null || threshold.compareTo(docAmount) < 0)
				return document;
	...
	private Gstr1AOutwardTransDocument checkForTable8(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
	...
	private Gstr1AOutwardTransDocument checkForTable9(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
	...
	private Gstr1AOutwardTransDocument checkForTable10(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
	...
	private Gstr1AOutwardTransDocument checkForRefundVouchers(
		...
		for (Gstr1AOutwardTransDocLineItem lineItem : document.getLineItems()) {
	...
	private Gstr1AOutwardTransDocument checkForAdvanceVouchers(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
	...
	private Gstr1AOutwardTransDocument checkForAdvanceAdjustments(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
		String docNum = document.getDocNo();
		if (LOGGER.isDebugEnabled()) {
			String msg = String
	...
	private Gstr1AOutwardTransDocument checkForTable14II(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
		String docNum = document.getDocNo();
		if (LOGGER.isDebugEnabled()) {
			String msg = String.format(
	...
	private Gstr1AOutwardTransDocument checkForTable15(
			Gstr1AOutwardTransDocument document, ProcessingContext context) {
	...
	/*
	 * public static void main(String[] args) { String configString =
	 * "201704-202408|250000;202409-202812|100000"; int derivedTaxPeriod =
	 * 202409; // Example derived tax period
	 * 
	 * BigDecimal threshold = getThresholdForTaxPeriod(configString,
	 * derivedTaxPeriod); System.out.println(threshold);
	 * 
	 * }
	 */
	public static BigDecimal getThresholdForTaxPeriod(String configString,
		...
		for (String entry : entries) {
			...
			if (derivedTaxPeriod >= startPeriod
					&& derivedTaxPeriod <= endPeriod) {
				return threshold;
			}
		}
