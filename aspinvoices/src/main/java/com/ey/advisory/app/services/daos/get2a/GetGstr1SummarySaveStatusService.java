...
@Component("GetGstr1SummarySaveStatusService")
public class GetGstr1SummarySaveStatusService {

	@Autowired
	@Qualifier("GetGstr1DetailStatusFetchDaoImpl")
	private GetGstr1DetailStatusFetchDao gstr1DetailStatusFetchDao;
	...
	public List<Gstr1SummarySaveStatusRespDto> findByCriteria(
		...
		List<Object[]> savedDataList = null;
		if (reqDto.getReturntype() != null
				&& "GSTR1A".equalsIgnoreCase(reqDto.getReturntype()))

		{
			savedDataList = gstr1DetailStatusFetchDao
					.getGstr1ADataUploadedStatusDetails(GstinList,
							criteria.getTaxPeriod(), null);
		}else {
	...
	public List<Gstr1SummarySaveStatusRespDto> findByCriteria(
			List<Gstr1GetInvoicesReqDto> criteria) {
		List<Gstr1SummarySaveStatusRespDto> summaryList = new ArrayList<Gstr1SummarySaveStatusRespDto>();
		Set<String> uniqueGstnReturnPeriod = new HashSet<>();

		for (Gstr1GetInvoicesReqDto reqDto : criteria) {
			String gstin = reqDto.getGstin();
			String returnPeriod = reqDto.getReturnPeriod();
			String uniqueIdentifier = gstin + "_" + returnPeriod;
			uniqueGstnReturnPeriod.add(uniqueIdentifier);
		}

		for (String uniqueCombination : uniqueGstnReturnPeriod) {
			String[] parts = uniqueCombination.split("_");
			String gstin = parts[0];
			String returnPeriod = parts[1];
			List<Object[]> savedDataList = gstr1DetailStatusFetchDao
					.getDataUploadedStatusDetails(Arrays.asList(gstin),
							returnPeriod, APIConstants.INVMGMT);
			if (!savedDataList.isEmpty() && savedDataList.size() > 0) {
				for (Object[] data : savedDataList) {
					Gstr1SummarySaveStatusRespDto dto = new Gstr1SummarySaveStatusRespDto();
					dto.setGstin(gstin);
					dto.setRetPeriod(returnPeriod);
					if (data[1] == null) {
						dto.setDate("");
						dto.setTime("");
					} else {
	...
	public List<Gstr1SummarySaveStatusRespDto> findByGstr8Criteria(
			List<Gstr1GetInvoicesReqDto> criteria) {
		List<Gstr1SummarySaveStatusRespDto> summaryList = new ArrayList<Gstr1SummarySaveStatusRespDto>();
		Set<String> uniqueGstnReturnPeriod = new HashSet<>();
		...
		for (String uniqueCombination : uniqueGstnReturnPeriod) {
			...
			String returnPeriod = parts[1];
			List<Object[]> savedDataList = gstr1DetailStatusFetchDao
					.getGstr8SaveStatusDetails(Arrays.asList(gstin),
							returnPeriod);
			if (!savedDataList.isEmpty() && savedDataList.size() > 0) {
				for (Object[] data : savedDataList) {
					Gstr1SummarySaveStatusRespDto dto = new Gstr1SummarySaveStatusRespDto();
