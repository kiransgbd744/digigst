...
import com.ey.advisory.app.data.entities.client.NilProcEntity;
/**
 * 
 * @author Hemasundar.J
 *
 */
@Repository("NilProcRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface NilProcRepository extends CrudRepository<NilProcEntity, Long> {
	@Modifying
	@Query("UPDATE NilProcEntity doc SET doc.gstnBatchId=:gstnBatchId,"
	...
	@Modifying
	@Query("UPDATE NilProcEntity doc SET doc.isSaved=true, doc.isError =false "
			+ "WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIsSavedToGstn(@Param("gstnBatchId") Long gstnBatchId);
	
	@Modifying
	@Query("UPDATE NilProcEntity doc SET doc.isError=true, doc.isSaved=false "
			+ "WHERE doc.gstnBatchId=:gstnBatchId")
