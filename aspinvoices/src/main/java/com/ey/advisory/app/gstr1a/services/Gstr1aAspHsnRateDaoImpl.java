...
@Component("Gstr1aAspHsnRateDaoImpl")
public class Gstr1aAspHsnRateDaoImpl  {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1aAspHsnRateDaoImpl.class);
	...
	private Gstr1AspVerticalHsnDto convertProcessedHsn(Object[] arr,
		...
		// obj.setRate(arr[7] != null ? arr[7].toString() : null);
		...
		// obj.setUrate(arr[14] != null ? arr[14].toString() : null);
	...
	private static String createHsnSummaryQueryString(String buildQuery,
		...
		bufferString.append("SUM((CASE WHEN LEFT(ITM_HSNSAC, 2) = '99' then 0 else ASP_ITM_QTY end))ASP_ITM_QTY, ");
		bufferString.append("SUM(ASP_TOTAL_VALUE)ASP_TOTAL_VALUE, ");
		bufferString.append("SUM(ASP_TAXABLE_VALUE)ASP_TAXABLE_VALUE, ");
		bufferString.append("SUM(ASP_IGST)ASP_IGST,SUM(ASP_CGST)ASP_CGST, ");
		bufferString.append("SUM(ASP_SGST)ASP_SGST,SUM(ASP_CESS)ASP_CESS, ");
		...
		bufferString.append(" SELECT ( DERIVED_RET_PERIOD || '|' || "); 
		bufferString.append(" SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') || '|' || IFNULL(ITM_HSNSAC, 'null') ) DOC_KEY, ");
		...
		bufferString.append("(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND ");
		bufferString.append("HDR.IS_DELETE = FALSE THEN 'IS_SAVED' ");
		bufferString.append("ELSE 'NOT_SAVED' END) AS SAVE_STATUS,");
		bufferString.append("(CASE WHEN IS_SAVED_TO_GSTN = TRUE AND HDR.IS_DELETE = FALSE THEN ");
		bufferString.append("GSTNBATCH.GSTN_SAVE_REF_ID ELSE NULL END) AS GSTIN_REF_ID,");
		...
		bufferString.append("LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID ");
		bufferString.append(" WHERE HDR.IS_DELETE = FALSE ");
...
public static void main(String[] args) {
	...
	System.out.println(createHsnSummaryQueryString("buildQuery",
