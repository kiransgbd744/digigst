...
@Component("gstr1ACdnrCdnraDataAtGstnImpl")
@Slf4j
public class Gstr1ACdnrCdnraDataAtGstnImpl implements Gstr1ACdnrCdnraDataAtGstn {
		...
		if (APIConstants.CDNR.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_CDNR);
			}
		} else {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_CDNRA);
			}
		}
		
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1ACdnrCdnraSuccessHandler", "Gstr1AGetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
