...
@Service("GstinApiCallServiceImpl")
@Slf4j
public class GstinApiCallServiceImpl implements GstinApiCallService {
	...
	public List<GstnGetStatusEntity> fetchGetStatus(List<String> gstinDtoList,
		...
		return gstinApiCallRepository.getStatus(gstinDtoList,
	...
	public List<ApiGstinDetailsDto> getTaxPeriodDetails(
		...
		Collector<GstnGetStatusEntity, ?, TaxPeriodDetailsDto> collector3 = Collectors
				.reducing(new TaxPeriodDetailsDto(), cpi -> convertDto(cpi),
						(cpt1, cpt2) -> merge(cpt1, cpt2));

		Collector<GstnGetStatusEntity, ?, Map<String, TaxPeriodDetailsDto>> collector2 = Collectors
				.groupingBy(obj -> obj.getTaxPeriod(), collector3);

		Collector<GstnGetStatusEntity, ?, Map<String, Map<String, TaxPeriodDetailsDto>>> collector1 = Collectors
		...
		List<String> combinataionsZipReady = new ArrayList<>();

		listZipstatus.forEach(e -> combinataionsZipReady
				.add(String.valueOf(e[0]) + "|" + String.valueOf(e[1])));
		...
		getTaxperiodDetails.forEach(e -> {
			...
			taxPrdDetails.forEach(taxObj -> {
				...
				/*
	             * commented to hide report status from API call Dashboard
	             * 

				if ("SUCCESS".equalsIgnoreCase(taxObj.getApiStatus())) {
					if (combinataionsZipReady.contains(gstinTaxPeriodKey))
						taxObj.setReportStatus("Generated");
					else
						taxObj.setApiStatus("Report generation in progres");
				}
				*/
	...
	public List<ApiFyGstinDetailsDto> getFyPeriodDetails(
			List<GstnGetStatusEntity> getBatchEntityDetails,
			Map<String, String> gstnRegMap) {

		List<ApiFyGstinDetailsDto> fyDetailsDtoList = new ArrayList<>();
		for (int i = 0; i < getBatchEntityDetails.size(); i++) {

			String gstin = getBatchEntityDetails.get(i).getGstin();
			ApiFyGstinDetailsDto fyDetailsDto = new ApiFyGstinDetailsDto();
			fyDetailsDto.setGstin(gstin);
			fyDetailsDto
					.setTaxPeriod(getBatchEntityDetails.get(i).getTaxPeriod());
			String status = getBatchEntityDetails.get(i).getStatus();
			Boolean csvGenFlag = getBatchEntityDetails.get(i)
					.getCsvGenerationFlag();
			if ("SUCCESS".equalsIgnoreCase(status) && csvGenFlag == true) {
				fyDetailsDto.setApiStatus("SUCCESS");
			} else if ("SUCCESS".equalsIgnoreCase(status)
					&& csvGenFlag == false) {
				fyDetailsDto.setApiStatus("Report generation in progress");
			}
			else if("INITIATED".equalsIgnoreCase(status))
			{
				fyDetailsDto.setApiStatus("INPROGRESS");
			}
			else if("SUCCESS_WITH_NO_DATA".equalsIgnoreCase(status))
			{
				fyDetailsDto.setApiStatus("SUCCESS_WITH_NO_DATA");
			}else {
			...
			fyDetailsDto.setGetCallDate(EYDateUtil
			...
			fyDetailsDtoList.add(fyDetailsDto);
		}
		return fyDetailsDtoList;
	}

	private TaxPeriodDetailsDto convertDto(GstnGetStatusEntity cpi) {
