 ...
 *
 */
@Service("Itc04RateDataToM2jwConverter")
@Slf4j
public class Itc04RateDataToM2jwConverter implements RateDataToItc04Converter {
	...
	public SaveBatchProcessDto convertToItc04Object(List<Object[]> objects, String section, String groupCode,
		...
		try {
			if (objects != null && !objects.isEmpty()) {
				...
				List<Itc04M2jwDto> m2jwList = new ArrayList<>();
				...
				for (int counter = 0; counter < totSize; counter++) {
					...
					if (isNewInvoice(arr1, arr2, idsList, totSize, counter2)) {
						Itc04M2jwDto m2jw = setInv(arr1,itmsList,taxDocType);
						m2jwList.add(m2jw);
	...
	private Itc04Dto setBatch(Object[] arr1, String section, List<Itc04M2jwDto> m2jwList) {
		...
		dto.setM2jw(m2jwList);
		return dto;
	}

	private Itc04M2jwDto setInv(Object[] arr1, List<Itc04ItemsDto> itmsList, String taxDocType) {
		Itc04M2jwDto data = new Itc04M2jwDto();
		...
		data.setChNum(arr1[5] != null ? String.valueOf(arr1[5]) : null);
		data.setChDate(arr1[6] != null ? String.valueOf(arr1[6]) : null);
		if (taxDocType != null && GSTConstants.CAN.equalsIgnoreCase(taxDocType)) {
			data.setFlag(APIConstants.D);
		}else{
			String saveFlag = arr1[17] != null ? String.valueOf(arr1[17]) : null;
	...
	private Itc04ItemsDto setItemDetail(Object[] arr1) {
		...
		items.setTxval(arr1[7] != null ? new BigDecimal(String.valueOf(arr1[7])).abs() : BigDecimal.ZERO);
		items.setGoodsType(arr1[8] != null ? String.valueOf(arr1[8]) : null);
		items.setUqc(arr1[9] != null ? String.valueOf(arr1[9]) : "");
		items.setQty(arr1[10] != null ? new BigDecimal(String.valueOf(arr1[10])).abs() : BigDecimal.ZERO);
		items.setTxi(arr1[11] != null ? new BigDecimal(String.valueOf(arr1[11])).abs() : BigDecimal.ZERO);
		items.setTxc(arr1[12] != null ? new BigDecimal(String.valueOf(arr1[12])).abs() : BigDecimal.ZERO);
		items.setTxs(arr1[13] != null ? new BigDecimal(String.valueOf(arr1[13])).abs() : BigDecimal.ZERO);
		items.setTxcs(arr1[14] != null ? new BigDecimal(String.valueOf(arr1[14])).abs() : BigDecimal.ZERO);
		String desc = arr1[16] != null ? String.valueOf(arr1[16]) : null;
