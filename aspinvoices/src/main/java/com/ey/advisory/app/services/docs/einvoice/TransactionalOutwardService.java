...
import com.ey.advisory.app.data.repositories.client.TransactionalOutwardRepository;
 ...
 */
@Slf4j
@Service("TransactionalOutwardService")
public class TransactionalOutwardService {

	@Autowired
	private TransactionalOutwardRepository transactionalOutwardRepo;
	...
	public void persistData(LocalDate reporDate, List<TransactionalDto> payload) {
		...
		List<TransactionalOutwardEntity> saveEntity = new ArrayList<>();
		...
		for (TransactionalDto obj : payload) {
			...
			supType = obj.getSupplyType();
			status = obj.getStatus();
			entityId = gstinRepo.findEntityIdByGstin(gstin);
			TransactionalOutwardEntity entity = new TransactionalOutwardEntity();
			...
			entity.setSupplyType(obj.getSupplyType());
			entity.setErpNumOfInvoices(obj.getErpNumOfInvs());
			entity.setErpTotalInvValue(obj.getErpTotalInvValue() != null
					? new BigDecimal(obj.getErpTotalInvValue()) : null);
			entity.setErpAssessableValue(obj.getErpAccValue() != null
					? new BigDecimal(obj.getErpAccValue()) : null);
			entity.setErpIgstAmt(obj.getErpIgst() != null
					? new BigDecimal(obj.getErpIgst()) : null);
			entity.setErpSgstAmt(obj.getErpSgst() != null
					? new BigDecimal(obj.getErpSgst()) : null);
			entity.setErpCgstAmt(obj.getErpCgst() != null
					? new BigDecimal(obj.getErpCgst()) : null);
			entity.setErpCessAmt(obj.getErpCess() != null
					? new BigDecimal(obj.getErpCess()) : null);
			entity.setCreatedBy(user.getUserPrincipalName());
			entity.setCreatedOn(convertNow);
			entity.setIsDelete(false);
			saveEntity.add(entity);
			transactionalOutwardRepo.softDelete(gstin, drvdRetPeriod, sourceId);
		...
		//transactionalOutwardRepo.softDelete(gstin, drvdRetPeriod, sourceId);
		transactionalOutwardRepo.saveAll(saveEntity);
		transactionalOutwardRepo.setCloudValues(gstin, drvdRetPeriod, sourceId);
		//transactionalOutwardRepo.setCloudValuesForPushPendingVerification(gstin, drvdRetPeriod, sourceId);
	...
	public TransactionalReportingDto getData(
		...
		List<TransactionalOutwardEntity> enities = findAllDataByCondition(
	...
	private List<TransactionalOutwardEntity> findAllDataByCondition(
		...
		String sql = "SELECT doc FROM TransactionalOutwardEntity doc WHERE "
