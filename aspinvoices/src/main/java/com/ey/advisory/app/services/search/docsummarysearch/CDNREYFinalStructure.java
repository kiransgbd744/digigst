...
@Service("CDNREYFinalStructure")
public class CDNREYFinalStructure {
	

	public List<Gstr1SummaryRespDto> getcdnrEyList(
		...
		if (view9BFiltered != null & view9BFiltered.size() > 0) {
			...
			List<Gstr1SummaryRespDto> default9BFiltered = cdnrEYList.stream()
					.filter(p -> p.getTableSection().equalsIgnoreCase("9B"))
					.collect(Collectors.toList());
		...
		Collections.sort(cdnrEYList, new Comparator<Gstr1SummaryRespDto>() {
			@Override
