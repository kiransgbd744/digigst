 ...
 *
 */
@Service("SRFileToComprehensiveOutwardTransDocErrConvertion")
@Slf4j
public class SRFileToComprehensiveOutwardTransDocErrConvertion {
		...
		try {
			errDocMapObj.entrySet().forEach(entry -> {
				...
				for (Object[] obj : objs) {
					...
					lineItem.setInvoiceStateCessAmount(invoiceStateCessAdvaloremAmount);
					...
					errorDocument.setInvoiceCessSpecificAmount(
							invoiceCessSpecificAmount);
					errorDocument
							.setInvoiceStateCessAmount(invoiceStateCessAdvaloremAmount);
