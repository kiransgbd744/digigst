 ...
 *
 */
@Service("DefaultItc04RefIdPollingManager")
public class DefaultItc04RefIdPollingManager implements Itc04RefIdPollingManger {

	private static final Logger LOGGER = LoggerFactory.getLogger(DefaultItc04RefIdPollingManager.class);

	@Autowired
	@Qualifier("DefaultItc04BatchIdPollingManager")
	...
	@Override
	public ResponseEntity<String> processItc04RefIds(PollingMessage reqDto, String groupcode) {
		...
		try {
			batches = gstr1BatchRepository.findReferenceIdForPooling(reqDto.getReturnType().toUpperCase());
		} catch (Exception ex) {
