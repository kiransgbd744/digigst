...
@Service("Gstr7VerticalErrorServiceImpl")
public class Gstr7VerticalErrorServiceImpl implements Gstr7VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr7VerticalErrorServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr7VerticalErrorReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria, PageRequest pageReq) {
		...
		workbook = createWorkbookWithExcelTemplate("ReportTemplates", "Gstr7_Error_Records.xlsx");

		LOGGER.debug("gstr6 PR Summary data response" + responseFromView);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility.getProp("gstr7.asp.error.report.headers").split(",");
