...
@Component("Anx1ASPEcomTotalSummaryDaoImpl")
public class Anx1ASPEcomTotalSummaryDaoImpl implements Anx1ASPEcomSavableDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ASPEcomTotalSummaryDaoImpl.class);
	...
	public List<Object> getAnx1EcomSavableReports(SearchCriteria criteria) {
		...
		String queryStr = createEcomSavableTotalQueryString(
				buildQuery.toString());
	...
	private Anx1AspEcomTotSumDto convertEcomSummary(Object[] arr) {
		...
		obj.setGstnErrorCode(arr[31] != null ? arr[31].toString() : null);
		obj.setGstnErrorDescription(
				arr[32] != null ? arr[32].toString() : null);
		obj.setSourceID(arr[33] != null ? arr[33].toString() : null);
		obj.setFileName(arr[34] != null ? arr[34].toString() : null);
		obj.setAspDateTime(arr[35] != null ? arr[35].toString() : null);

		return obj;
	}

	private String createEcomSavableTotalQueryString(String buildQuery) {

		return "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,ECOM_GSTIN,"
				+ "SUM(ECOM_VAL_SUPMADE) ECOM_VAL_SUPMADE,"
				+ "SUM(ECOM_VAL_SUPRET) ECOM_VAL_SUPRET,"
				+ "SUM(ECOM_NETVAL_SUP) ECOM_NETVAL_SUP,"
				...
				+ "USER_DEFINED2,USER_DEFINED3,INFO_CODE_ASP,"
				+ "INFO_DESCRIPTION_ASP,SAVE_STATUS,GSTIN_REF_ID,"
				+ "GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE,GSTIN_ERROR_DESCRIPTION,"
				+ "SOURCE_ID,FILE_NAME,ASP_DATE_TIME FROM ( (SELECT HDR.RETURN_TYPE "
				+ "AS RETURN_TYPE, HDR.SUPPLIER_GSTIN AS SUPPLIER_GSTIN,"
				+ "HDR.RETURN_PERIOD as RETURN_PERIOD,ECOM_GSTIN,"
				+ "ECOM_VAL_SUPMADE,ECOM_VAL_SUPRET,ECOM_NETVAL_SUP,"
				+ "IGST_AMT,CGST_AMT,SGST_AMT,CESS_AMT,PROFIT_CENTER,PLANT,"
				...
				+ "USER_DEFINED2,USER_DEFINED3,INFO_ERROR_CODE_ASP INFO_CODE_ASP,"
				...
				+ "BT.GSTN_SAVE_REF_ID AS GSTIN_REF_ID,BT.CREATED_ON "
				+ "AS GSTIN_REF_ID_TIME, '' AS GSTIN_ERROR_CODE,"
				+ " '' AS GSTIN_ERROR_DESCRIPTION, FIL.CREATED_BY "
				+ "AS SOURCE_ID,FIL.FILE_NAME AS FILE_NAME,HDR.CREATED_ON "
				+ "AS ASP_DATE_TIME,HDR.DERIVED_RET_PERIOD "
				+ "FROM ANX_PROCESSED_B2C HDR "
				+ "LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() ERR "
				+ "ON HDR.AS_ENTERED_ID = ERR.COMMON_ID AND "
				+ "HDR.FILE_ID = ERR.FILE_ID AND ERR.INV_KEY = HDR.B2C_INVKEY "
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID "
				+ "AND ERR.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH BT ON HDR.BATCH_ID = BT.ID "
				+ "AND HDR.FILE_ID = BT.ID AND "
				+ "HDR.DERIVED_RET_PERIOD = BT.DERIVED_RET_PERIOD "
				+ "WHERE HDR.IS_DELETE = FALSE ) "
				+ " UNION ALL ( SELECT HDR.AN_RETURN_TYPE AS RETURN_TYPE,"
				+ "HDR.SUPPLIER_GSTIN SUPPLIER_GSTIN,HDR.RETURN_PERIOD "
				+ "as RETURN_PERIOD, HDR.ECOM_GSTIN AS ECOM_GSTIN, "
				+ "(IFNULL((CASE WHEN DOC_TYPE='INV' THEN "
				+ "ITM.TAXABLE_VALUE END),0) + IFNULL((CASE WHEN DOC_TYPE='DR' "
				+ "THEN ITM.TAXABLE_VALUE END),0)) AS ECOM_VAL_SUPMADE,"
				+ "(IFNULL((CASE WHEN DOC_TYPE='CR' THEN ITM.TAXABLE_VALUE END),0)) "
				+ "AS ECOM_VAL_SUPRET, (IFNULL((CASE WHEN DOC_TYPE='INV' "
				+ "THEN ITM.TAXABLE_VALUE END),0) + IFNULL((CASE "
				+ "WHEN DOC_TYPE='DR' THEN ITM.TAXABLE_VALUE END),0) - IFNULL((CASE "
				+ "WHEN DOC_TYPE='CR' THEN ITM.TAXABLE_VALUE END),0)) "
				+ "AS ECOM_NETVAL_SUP, ITM.IGST_AMT AS IGST_AMT,ITM.CGST_AMT "
				+ "AS CGST_AMT,ITM.SGST_AMT AS SGST_AMT,"
				+ "(ITM.CESS_AMT_SPECIFIC+ITM.CESS_AMT_ADVALOREM) "
				+ "AS CESS_AMT, ITM.PROFIT_CENTRE AS PROFIT_CENTER,"
				+ "ITM.PLANT_CODE AS PLANT,HDR.DIVISION as DIVISION,"
				+ "ITM.LOCATION AS LOCATION,HDR.SALES_ORGANIZATION "
				+ "AS SALES_ORG,HDR.DISTRIBUTION_CHANNEL AS DISTRIBUTION_CHANNEL,"
				+ "HDR.USERACCESS1 AS USER_ACCESS1,HDR.USERACCESS2 "
				+ "AS USER_ACCESS2,HDR.USERACCESS3 AS USER_ACCESS3,"
				+ "HDR.USERACCESS4 AS USER_ACCESS4,HDR.USERACCESS5 AS USER_ACCESS5,"
				+ "HDR.USERACCESS6 AS USER_ACCESS6,ITM.USERDEFINED_FIELD1 "
				+ "AS USER_DEFINED1,ITM.USERDEFINED_FIELD2 AS USER_DEFINED2,"
				+ "ITM.USERDEFINED_FIELD3 AS USER_DEFINED3,"
				+ "TRIM(', ' FROM IFNULL(ERRH.INFO_ERROR_CODE_ASP,'') ||','|| "
				+ "IFNULL(ERRI.INFO_ERROR_CODE_ASP,'')) AS INFO_CODE_ASP,"
				+ "TRIM(', ' FROM IFNULL(ERRH.INFO_ERROR_DESCRIPTION_ASP,'') ||','|| "
				+ "IFNULL(ERRI.INFO_ERROR_DESCRIPTION_ASP,'')) "
				+ "AS INFO_DESCRIPTION_ASP, CASE WHEN HDR.IS_SAVED_TO_GSTN = TRUE "
				+ "THEN 'IS_SAVED' WHEN HDR.IS_SAVED_TO_GSTN = FALSE "
				+ "AND HDR.GSTN_ERROR = TRUE THEN 'IS_ERROR' WHEN "
				+ "HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = FALSE "
				+ "THEN 'NOT_SAVED' END AS SAVE_STATUS,GSB.GSTN_SAVE_REF_ID "
				...
				+ "FIL.FILE_NAME AS FILE_NAME, HDR.CREATED_ON AS ASP_DATE_TIME,HDR.DERIVED_RET_PERIOD "
				...
				+ "ITM.ITEM_INDEX = ERRI.ITEM_INDEX AND "
				+ "ITM.ITM_NO = ERRI.ITM_NO INNER JOIN FILE_STATUS FIL "
				+ "ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSB ON GSB.ID = HDR.BATCH_ID "
				+ "WHERE IS_ERROR = FALSE AND HDR.AN_RETURN_TYPE = 'ANX1' AND "
				+ "AN_TAX_DOC_TYPE = 'B2C' GROUP BY HDR.AN_RETURN_TYPE,"
				+ "HDR.SUPPLIER_GSTIN,HDR.RETURN_PERIOD,HDR.ECOM_GSTIN,"
				+ "ITM.TAXABLE_VALUE,DOC_TYPE,ITM.IGST_AMT,ITM.CGST_AMT,"
				+ "ITM.SGST_AMT,ITM.CESS_AMT_SPECIFIC,ITM.CESS_AMT_ADVALOREM,"
				+ "ITM.PROFIT_CENTRE, ITM.PLANT_CODE,HDR.DIVISION,"
				+ "ITM.LOCATION,HDR.SALES_ORGANIZATION,"
				+ "HDR.DISTRIBUTION_CHANNEL,HDR.USERACCESS1,HDR.USERACCESS2,"
				+ "HDR.USERACCESS3, HDR.USERACCESS4,HDR.USERACCESS5,"
				+ "ERRI.INFO_ERROR_CODE_ASP,HDR.USERACCESS6,"
				+ "ITM.USERDEFINED_FIELD1,ITM.USERDEFINED_FIELD2,"
				+ "ITM.USERDEFINED_FIELD3,ERRH.INFO_ERROR_CODE_ASP,"
				+ "ERRH.INFO_ERROR_DESCRIPTION_ASP,"
				+ "ERRI.INFO_ERROR_DESCRIPTION_ASP,HDR.IS_SAVED_TO_GSTN,"
				+ "HDR.GSTN_ERROR, GSB.GSTN_SAVE_REF_ID,GSB.GSTN_RESP_DATE,"
				+ "ERRH.ERROR_CODE_GSTIN,ERRI.ERROR_CODE_GSTN,"
				+ "ERRH.ERROR_DESCRIPTION_GSTIN,ERRI.ERROR_DESCRIPTION_GSTN,"
				+ "HDR.DERIVED_RET_PERIOD,"
				+ "FIL.CREATED_BY, FIL.FILE_NAME,HDR.CREATED_ON ) "
				+ " UNION ALL ( SELECT HI.RETURN_TYPE,HI.SUPPLIER_GSTIN,"
				+ "HI.RETURN_PERIOD,HI.ECOM_GSTIN,HI.ECOM_VAL_SUPMADE,"
				+ "HI.ECOM_VAL_SUPRET,HI.ECOM_NETVAL_SUP,HI.IGST_AMT,"
				+ "HI.CGST_AMT,HI.SGST_AMT,HI.CESS_AMT,"
				+ "HI.PROFIT_CENTER,HI.PLANT,DIVISION,HI.LOCATION,"
				+ "HI.SALES_ORG,HI.DISTRIBUTION_CHANNEL,HI.USER_ACCESS1,"
				+ "HI.USER_ACCESS2,HI.USER_ACCESS3,HI.USER_ACCESS4,"
				+ "HI.USER_ACCESS5,HI.USER_ACCESS6,HI.USER_DEFINED1,"
				+ "HI.USER_DEFINED2,HI.USER_DEFINED3,ERR.INFO_ERROR_CODE_ASP "
				+ "AS INFO_CODE_ASP, ERR.INFO_ERROR_DESCRIPTION_ASP "
				+ "AS INFO_DESCRIPTION_ASP, CASE WHEN HI.IS_SAVED_TO_GSTN = TRUE "
				+ "THEN 'IS_SAVED' WHEN HI.IS_SAVED_TO_GSTN = FALSE "
				+ "AND HI.GSTN_ERROR = TRUE THEN 'IS_ERROR' "
				+ "WHEN HI.IS_SAVED_TO_GSTN = FALSE THEN 'NOT_SAVED' "
				...
				+ "HI.CREATED_BY AS SOURCE_ID,FIL.FILE_NAME AS FILE_NAME,"
				+ "HI.CREATED_ON AS ASP_DATE_TIME,"
				+ "HI.DERIVED_RET_PERIOD FROM ANX_PROCESSED_TABLE4 "
				...
				+ "WHERE HI.IS_DELETE = FALSE ) ) " + buildQuery + " GROUP BY "
				+ "RETURN_TYPE, SUPPLIER_GSTIN, RETURN_PERIOD,ECOM_GSTIN,"
				+ "PROFIT_CENTER,PLANT,DIVISION,LOCATION,SALES_ORG,"
				+ "DISTRIBUTION_CHANNEL,USER_ACCESS1,USER_ACCESS2,USER_ACCESS3,"
				+ "USER_ACCESS4,USER_ACCESS5,USER_ACCESS6,USER_DEFINED1,"
				+ "USER_DEFINED2,USER_DEFINED3,INFO_CODE_ASP,"
				+ "INFO_DESCRIPTION_ASP, SAVE_STATUS,GSTIN_REF_ID,"
				+ "GSTIN_REF_ID_TIME,GSTIN_ERROR_CODE,"
				+ "GSTIN_ERROR_DESCRIPTION,SOURCE_ID,"
				+ "FILE_NAME,ASP_DATE_TIME ";
	}
}
