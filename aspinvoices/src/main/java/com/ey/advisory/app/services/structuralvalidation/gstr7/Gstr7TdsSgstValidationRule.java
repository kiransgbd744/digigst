 ...
 *
 */
public class Gstr7TdsSgstValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errorLocations.add(GSTConstants.TDS_SGST);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2019",
					"Invalid TDS SGST", location));
		...
		if (result.compareTo(BigDecimal.ZERO) < 0) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER2020",
