...
import com.ey.advisory.app.data.gstr1A.repositories.client.Gstr1AAdditionalDocDetailsRepository;
import com.ey.advisory.app.data.gstr1A.repositories.client.Gstr1AContractDetailsRepository;
import com.ey.advisory.app.data.gstr1A.repositories.client.Gstr1APreceedingDocDetailsRepository;
 ...
 *
 */
@Component("Gstr1AEInvoiceOutwardDocSave")
public class Gstr1AEInvoiceOutwardDocSave {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1AEInvoiceOutwardDocSave.class);
	private static final String CLASS_NAME = "Gstr1AEInvoiceOutwardDocSave";

	@Autowired
	@Qualifier("DocRepositoryGstr1A")
	private DocRepositoryGstr1A docHeaderRepository;

	@Autowired
	@Qualifier("Gstr1AEInvoiceDefaultDuplicateDocCheckServiceImpl")
	private Gstr1AEInvoiceDuplicateDocCheckService duplicateDocCheckService;

	@Autowired
	@Qualifier("Gstr1AEInvoiceDefaultOutwardSupplyTypeResolver")
	Gstr1AEInvoiceOutwardSupplyTypeResolver outwardSupplyTypeResolver;

	@Autowired
	@Qualifier("Gstr1AEInvoiceOutwardFieldTruncation")
	private Gstr1AEInvoiceOutwardFieldTruncation outwardFieldTruncation;

	@Autowired
	@Qualifier("Gstr1AEInvoiceOutwardDocMemoValCalculation")
	private Gstr1AEInvoiceOutwardDocMemoValCalculation outwardDocMemoValCalc;
	...
	@Autowired
	@Qualifier("Gstr1AEinvoiceOutwardConfigParams")
	private Gstr1AEinvoiceOutwardConfigParams einvoiceOutwardConfigParams;
	...
	@Autowired
	@Qualifier("Gstr1AContractDetailsRepository")
	private Gstr1AContractDetailsRepository contractDetailsRepository;

	@Autowired
	@Qualifier("Gstr1AAdditionalDocDetailsRepository")
	private Gstr1AAdditionalDocDetailsRepository additionalDocDetailsRepository;
	...
	@Autowired
	@Qualifier("Gstr1APreceedingDocDetailsRepository")
	private Gstr1APreceedingDocDetailsRepository preceedingDocDetailsRepository;
		...
		documents.forEach(document -> {
			...
			if (document.getPreDocDtls() != null
				...
				document.getPreDocDtls().forEach(preDetls -> {
					...
					Gstr1APreceedingDocDetails preFirstRec = document
							.getPreDocDtls().get(0);
					if (preFirstRec != null) {
			...
			if (document.getContrDtls() != null
				...
				document.getContrDtls().forEach(contrDetails -> {
					...
					Gstr1AContractDetails contractDetFirstRec = document
							.getContrDtls().get(0);
			...
			if (document.getAddlDocDtls() != null
				...
				document.getAddlDocDtls().forEach(addlDetails -> {
					...
					Gstr1AAdditionalDocDetails additionalDetFirstRec = document
			...
			if (document.getLineItems() != null) {
				document.getLineItems().forEach(item -> {
					if (item.getAttribDtls() != null
						...
						Gstr1AAttributeDetails attDetFirstRecord = item
								.getAttribDtls().get(0);
	...
	private void trimForTaxScheme(Gstr1AOutwardTransDocument document) {
	...
	private void setEwbInvDetails(Gstr1AOutwardTransDocument document) {
	...
	private void setDerivedSourceIdAndPayloadId(
			Gstr1AOutwardTransDocument document, String sourceId,
			String headerPayloadId) {
	...
	private void setLineItemAmtByItemAmt(Gstr1AOutwardTransDocLineItem item) {
	...
	public List<Gstr1AOutwardTransDocument> saveDocs(
			List<Gstr1AOutwardTransDocument> docs,
			DocKeyGenerator<Gstr1AOutwardTransDocument, String> docKeyGen) {
		...
		Gstr1AOutwardTransDocument firstOutwardDoc = docs.get(0);
		...
		List<Gstr1AOutwardTransDocument> activeDocList = docs.parallelStream()
		...
		List<Gstr1AOutwardTransDocument> outWardTransList = docHeaderRepository
				.saveAll(docs);
		PerfUtil.logEventToFile(PerfamanceEventConstants.FILE_PROCESSING,
	...
	private void setDataOriginTypeCode(Gstr1AOutwardTransDocument document) {
		/*
	...
	private void setPosAndStateCodeValues(Gstr1AOutwardTransDocument document) {
	...
	private void setItemAmountsToZero(Gstr1AOutwardTransDocument document,
	...
	private void convertToUpperCaseAndSetDocValues(
		...
		if (index != null) {
			Gstr1AOutwardTransDocLineItem outwardTransDocLineItem = document
	...
	private void setAbsoluteValuesHeaderLevel(
			Gstr1AOutwardTransDocument document) {
	...
	private void setAbsoluteValuesItemLevel(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
		String docType = document.getDocType();
		String supplyType = document.getSupplyType();
		final List<String> docTypes = ImmutableList.of("CR", "RCR", "RFV",
	...
	private void setFormReturnType(Gstr1AOutwardTransDocument document,
	...
	private void calculateAndSetDocAmount(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
	...
	private void calculateAndSetTotalTaxAmt(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
	...
	private void calculateAndSetItemTaxRate(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
		// Calculate Tax Rate
	...
	private void calculateAndSetTaxPayble(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
	...
	private void calculateAndSetTaxableValue(
			Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
	...
	private void calculateInvAmounts(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
	...
	private void setHeaderValuesToItem(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item) {
	...
	public void setHeaderPrecInvAmtTaxableValue(
		...
		List<Gstr1AOutwardTransDocument> orgDocDetails = new ArrayList<>();
		...
		List<Gstr1AOutwardTransDocument> crDrDocs = documents.stream()
		...
		Map<String, Gstr1AOutwardTransDocument> orgDocDetailsMap = orgDocDetails
				.stream()
				.collect(Collectors.toMap(Gstr1AOutwardTransDocument::getDocKey,
						Function.identity()));
		LOGGER.debug("orgDocDetails  map {} ", orgDocDetailsMap);

		crDrDocs.forEach(doc -> {
			List<Gstr1AOutwardTransDocLineItem> lineItem = doc.getLineItems();
			String orgDocKey = docKeyGen.generateOrgKey(doc);
			LOGGER.debug("orgDocKey {}", orgDocKey);
			if (orgDocDetailsMap.containsKey(orgDocKey)) {
				Gstr1AOutwardTransDocument obj = orgDocDetailsMap
						.get(orgDocKey);
				BigDecimal docAmt = obj.getDocAmount();
				BigDecimal taxableVal = obj.getTaxableValue();
				List<Gstr1AOutwardTransDocLineItem> originalLineitem = obj
						.getLineItems();
	...
	public void setItemPrecInvAmtTaxableValue(
			List<Gstr1AOutwardTransDocLineItem> lineItem,
			List<Gstr1AOutwardTransDocLineItem> originalLineitem) {
		BigDecimal lineItemAmt = BigDecimal.ZERO;
		BigDecimal taxbleValue = BigDecimal.ZERO;

		for (Gstr1AOutwardTransDocLineItem item : originalLineitem) {
			if (item.getLineItemAmt() != null) {
	...
	private void setValuesToPreceeding(Gstr1AOutwardTransDocument document,
			Gstr1APreceedingDocDetails preDetls) {
	...
	private void setValuesToContr(Gstr1AOutwardTransDocument document,
			Gstr1AContractDetails contrDetails) {
	...
	private void setValuestoAddlDetails(Gstr1AOutwardTransDocument document,
			Gstr1AAdditionalDocDetails addlDetails) {
	...
	private void setValuestoAttDetails(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item,
			Gstr1AAttributeDetails attDetails) {
	...
	private void setInvRefValToItem(Gstr1AOutwardTransDocument document,
			Gstr1AOutwardTransDocLineItem item,
			Gstr1APreceedingDocDetails preFirstRec) {
		item.setInvoiceReference(preFirstRec.getInvoiceReference());
	}

	private void setContractDetailsToItem(Gstr1AOutwardTransDocLineItem item,
			Gstr1AContractDetails contractDetFirstRec) {
	...
	private void setAdditionalDetailsToItem(Gstr1AOutwardTransDocLineItem item,
			Gstr1AAdditionalDocDetails additionalDetFirstRec) {
	...
	private void setAttrinutesToItem(Gstr1AOutwardTransDocLineItem item,
			Gstr1AAttributeDetails attDetFirstRecord) {
		item.setAttributeName(attDetFirstRecord.getAttributeName());
		item.setAttributeValue(attDetFirstRecord.getAttributeValue());
	}

	private Pair<Integer, Integer> getPreference(
			Gstr1AOutwardTransDocument document,
	...
	private void UQCConversion(List<Gstr1AOutwardTransDocument> documents) {
		...
		for (Gstr1AOutwardTransDocument doc : documents) {
			...
			List<Gstr1AOutwardTransDocLineItem> items = doc.getLineItems();
