 ...
 *
 */
@Service("anx2SezwopInvoicesAtGstnImpl")
public class Anx2SezwopInvoicesAtGstnImpl implements Anx2InvoicesAtGstn {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx2SezwopInvoicesAtGstnImpl.class);
	...
	public ResponseEntity<String> findInvFromGstn(String jsonReq,
		...
		try {
			...
			dtos.forEach(dto ->{
			 ...
			 type, json);
			// static data
			//String apiResp = "{'sezwop':[{'ctin':'32SSSKL8363S1ZF','docs':[{'doctyp':'I','rfndelg':'Y','pos':'03','action':'R','cfs':'NF','chksum':'0dbdc754fbb79bf3eba1207a9bd0e21d27d3bf0935eaf8d58a29f0db32cef279','itcent':'Y','upldt':'25-04-2019','doc':{'num':'INV200','dt':'25-04-2019','val':25000},'items':[{'hsn':'223456','txval':1000,'rate':8}]}]}]}";
			...
			/*if (apiResp != null) {
				...
				Set<GetAnx2SezwopInvoicesHeaderEntity> invoices = anx2DataParser
						.parseSezwopData(dto, apiResp, batch.getId());
				...
				if (!invoices.isEmpty()) {
					...
					headerRepo.softlyDeleteSezwopHeader(dto.getGstin(),
							dto.getReturnPeriod());
					// Save new Header
					headerRepo.saveAll(invoices);
					GetGstr2aBatchEntity batch = batchUtil.makeBatch(dto, type);
					batch.setAnx2SezwopInvoices(invoces);
					invoces.forEach(invoice -> {
						invoice.setSezwopBatchIdAnx2(batch);
					});
