...
@Service("Gstr2xProcessedReportServiceImpl")
public class Gstr2xProcessedReportServiceImpl implements Gstr2xReportService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr2xProcessedReportServiceImpl.class);
	...
	@Autowired
	@Qualifier("Gstr2xProcessedReportDaoImpl")
	private Gstr2xReportDao gstr2xProcessedReportDao;
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		//responseFromView = gstr2xProcessedReportDao.getGstr2xReports(request);

		/*workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"TDS _TDSA _TCS_ TCSA_GSTR-2X.xlsx");
		
		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("tds.processed.report.headers").split(",");
