 ...
 */
@Slf4j
@Service("anx1SaveDeemedExportsBatchProcessImpl")
public class Anx1SaveDeemedExportsBatchProcessImpl implements Anx1SectionWiseSaveBatchProcess {
	...
	private SaveToGstnBatchRefIds process(
		...
		try {
			...
			Gstr1SaveBatchEntity saveBatch = batchHandler.saveBatch(
					anx1.getSgstin(), anx1.getTaxperiod(), section, groupCode,
					APIConstants.ANX1, idsList.size(), null, 0l, 0l);
	...
	private Pair<List<Long>, SaveAnx1> setChunkCtinData(SaveAnx1 anx1,
			List<Anx1DeemadExportsData> cTinList, String section) {
		...
		for (Anx1DeemadExportsData cData : cTinList) {
			for (Anx1DeemadExportsDocumentData iData : cData.getDocs()) {
	...
	private Pair<List<Long>, SaveAnx1> setChunkInvData(SaveAnx1 anx1,
			List<Anx1DeemadExportsDocumentData> invList, String section) {

		SaveAnx1 anx = new SaveAnx1();
		List<Long> idsList = new ArrayList<>();
		List<Anx1DeemadExportsData> cTinList = new ArrayList<>();

		anx.setSgstin(anx1.getSgstin());
		anx.setTaxperiod(anx1.getTaxperiod());

		anx.setDeInvoice(cTinList);
		Anx1DeemadExportsData cTin = anx1.getDeInvoice().get(0);

		cTin.setDocs(invList);
		cTinList.add(cTin);

		for (Anx1DeemadExportsDocumentData data : invList) {
	...
	private List<Triplet<List<Long>, SaveAnx1, String>> batchChunk(
		...
		int ctinSize = anx1.getDeInvoice().size();

		Pair<List<Long>, SaveAnx1> pair1 = null;
		Pair<List<Long>, SaveAnx1> pair2 = null;

		if (ctinSize > 1) {
			List<Anx1DeemadExportsData> cTinList1 = anx1.getDeInvoice().subList(0,
					ctinSize / 2);
			List<Anx1DeemadExportsData> cTinList2 = anx1.getDeInvoice()
			...
			Anx1DeemadExportsData cTin1 = anx1.getDeInvoice().get(0);
			int invSize = cTin1.getDocs().size();
			List<Anx1DeemadExportsDocumentData> invList1 = cTin1.getDocs().subList(0,
					invSize / 2);
			List<Anx1DeemadExportsDocumentData> invList2 = cTin1.getDocs()
