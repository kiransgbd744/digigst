 ...
 */
@Slf4j
@Component("DefaultGstr2XBatchIdPollingManager")
public class DefaultGstr2XBatchIdPollingManager implements SaveBatchIdPollingManager{
	...
	@Autowired
	@Qualifier("DefaultGstr2XGstnResponseHandler")
	private Gstr2XGstnResponseHandler gstr2XGstnResponseHandler;
	...
	public ReturnStatusRefIdDto processBatch(Gstr1SaveBatchEntity batch) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Pooling Gstr2X ReturnStatus- Sandbox response is {} ",
					resp);
		}
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Pooling Gstr2X ReturnStatus Sandbox status is {} ",
						gstnStatus);
			}
			...
			if (P.equalsIgnoreCase(gstnStatus)) {
				...
				gstr2XGstnResponseHandler.phandleResponse(null, batch);
			} else if (PE.equalsIgnoreCase(gstnStatus)) {
				LOGGER.info("PE is processed.");
				saveToGstnEventStatus.EventEntry(batch.getReturnPeriod(),
						batch.getSgstin(), 50, batch.getGroupCode());
				gstr2XGstnResponseHandler.pEhandleResponse(jsonRoot, batch);
				...
				gstr2XGstnResponseHandler.ehandleResponse(null, batch);
			}else {				
				LOGGER.error("Pooling Gstr2X ReturnStatus- intermediate status"
						+ gstnStatus);
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Pooling Gstr2X ReturnStatus- IP/REC response handling started for {} ",
							batch.getSection());
				}
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Pooling Gstr2X ReturnStatus Processing Completed.");
			}
		} catch (Exception ex) {			
			batchSaveStatusRepository.updateStatusById(batch.getId(),
					APIConstants.POLLING_FAILED,now);
