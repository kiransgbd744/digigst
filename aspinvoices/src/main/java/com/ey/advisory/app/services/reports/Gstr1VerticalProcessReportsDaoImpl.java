 ...
 *
 */
@Component("Gstr1VerticalProcessReportsDaoImpl")
public class Gstr1VerticalProcessReportsDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1VerticalProcessReportsDaoImpl.class);
	...
	public List<Object> getGstr1VerticalReports(
		...
		if(dataType!=null && !dataType.equalsIgnoreCase("GSTR1A"))
		 queryStr = createVerticalErrorQueryString(buildQuery.toString());
			 ...
			 queryStr = createGstr1aVerticalErrorQueryString(buildQuery.toString());
	...
	private Object convertVerticalError(Object[] arr) {
		...
		if ((DownloadReportsConstant.INVOICE).equalsIgnoreCase(fileType)) {
			...
			obj4.setFrom(arr[4] != null ? arr[4].toString() : null);
			obj4.setTo(arr[5] != null ? arr[5].toString() : null);
			obj4.setTotalNumber(arr[6] != null ? arr[6].toString() : null);
			obj4.setCancelled(arr[7] != null ? arr[7].toString() : null);
			obj4.setNetNumber(arr[8] != null ? arr[8].toString() : null);
			obj = obj4;
		}
		return obj;
	}
	...
	private String createVerticalErrorQueryString(String buildQuery) {
		...
		if ((DownloadReportsConstant.B2CS).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MONTH, "
					...
					+ "FROM  GSTR1_PROCESSED_B2CS HI LEFT OUTER JOIN "
					+ "  TF_GSTR1_ERROR_INFO() ERR ON "
					+ "HI.AS_ENTERED_ID= ERR.COMMON_ID AND HI.FILE_ID =  "
					+ "ERR.FILE_ID LEFT OUTER JOIN  FILE_STATUS FIL "
					+ " ON HI.FILE_ID = FIL.ID AND ERR.FILE_ID = FIL.ID "
					+ "AND ERR.INV_KEY=HI.B2CS_INVKEY  " + " WHERE "
					+ buildQuery;
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MONTH, "
					+ "ORG_POS,ORG_RATE,ORG_GROSS_ADV_RECEIVED, "
					...
					+ " ON HI.AS_ENTERED_ID= ERR.COMMON_ID AND "
					+ "HI.FILE_ID=ERR.FILE_ID LEFT OUTER JOIN  "
					+ " FILE_STATUS FIL  ON HI.FILE_ID=FIL.ID "
					+ "  AND ERR.FILE_ID=FIL.ID AND ERR.INV_KEY=HI.AT_INVKEY "
					+ "  WHERE " + buildQuery;
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MONTH, "
					+ "ORG_POS,ORG_RATE,ORG_GROSS_ADV_ADJUSTED,  NEW_POS, "
	                ...
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM "
					+ " GSTR1_PROCESSED_ADV_ADJUSTMENT HI "
					+ "LEFT OUTER JOIN  TF_GSTR1_ERROR_INFO() ERR "
					+ " ON HI.AS_ENTERED_ID= ERR.COMMON_ID AND  "
					+ "HI.FILE_ID=ERR.FILE_ID LEFT OUTER JOIN "
					+ " FILE_STATUS FIL  ON HI.FILE_ID=FIL.ID "
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,SERIAL_NUM, "
					+ " NATURE_OF_DOC, DOC_SERIES_FROM, DOC_SERIES_TO, TOT_NUM, "
					+ "CANCELED, NET_NUM FROM GSTR1_PROCESSED_INV_SERIES HI "
					+ "WHERE " + buildQuery;
		}
		return queryStr;
	}
	...
	private String createGstr1aVerticalErrorQueryString(String buildQuery) {
		...
		if ((DownloadReportsConstant.B2CS).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MONTH, "
	                ...
	                + "TRIM(', ' FROM IFNULL(INFO_ERROR_DESCRIPTION_ASP,'') ) "
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM  GSTR1A_PROCESSED_B2CS HI LEFT OUTER JOIN "
					+ "  TF_GSTR1A_ERROR_INFO() ERR ON "
					+ "HI.AS_ENTERED_ID= ERR.COMMON_ID AND HI.FILE_ID =  "
					+ "ERR.FILE_ID LEFT OUTER JOIN  FILE_STATUS FIL "
					+ " ON HI.FILE_ID = FIL.ID AND ERR.FILE_ID = FIL.ID "
					+ "AND ERR.INV_KEY=HI.B2CS_INVKEY  " + " WHERE "
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MONTH, "
	                ...
	                + "AS INFO_ERROR_DESCRIPTION_ASP " 
					+ "FROM "
					+ " GSTR1A_PROCESSED_ADV_ADJUSTMENT HI "
					+ "LEFT OUTER JOIN  TF_GSTR1A_ERROR_INFO() ERR "
					+ " ON HI.AS_ENTERED_ID= ERR.COMMON_ID AND  "
					+ "HI.FILE_ID=ERR.FILE_ID LEFT OUTER JOIN "
					+ " FILE_STATUS FIL  ON HI.FILE_ID=FIL.ID "
			...
			queryStr = "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,SERIAL_NUM, "
					+ " NATURE_OF_DOC, DOC_SERIES_FROM, DOC_SERIES_TO, TOT_NUM, "
					+ "CANCELED, NET_NUM FROM GSTR1A_PROCESSED_INV_SERIES HI "
					+ "WHERE " + buildQuery;
		}
		return queryStr;
	}
