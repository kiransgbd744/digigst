...
@Component("Gstr1NilRatedTransactionalDaoImpl")
public class Gstr1NilRatedTransactionalDaoImpl
		implements Gstr1OutwardVerticalProcessNilDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1NilRatedTransactionalDaoImpl.class);
	...
	private Gstr1ProcessedReviewSummDto convertProcessed(Object[] arr) {
		Gstr1ProcessedReviewSummDto obj = new Gstr1ProcessedReviewSummDto();
		...
		// obj.setPrecedDate(arr[29] != null ? arr[29].toString() : null);
		if (arr[29] != null) {
	...
	private String createApiProcessedQueryString(String buildQuery) {
		...
		return "SELECT NEN.USER_ID,NEN.SOURCE_FILENAME,ITM.PROFIT_CENTRE,"
				+ "ITM.PLANT_CODE,NEN.DIVISION,"
				+ "ITM.LOCATION, NEN.SALES_ORGANIZATION,NEN.DISTRIBUTION_CHANNEL,"
				+ "NEN.USERACCESS1,NEN.USERACCESS2,NEN.USERACCESS3,"
				+ "NEN.USERACCESS4,NEN.USERACCESS5,NEN.USERACCESS6,"
				...
				+ "ITM.RETURN_PERIOD,NEN.SUPPLIER_GSTIN,NEN.DOC_TYPE,NEN.SUPPLY_TYPE,"
				+ "NEN.DOC_NUM,NEN.DOC_DATE,NEN.ORIGINAL_DOC_TYPE,"
				+ "NEN.PRECEEDING_INV_NUM,NEN.PRECEEDING_INV_DATE,"
				+ "ITM.CRDR_PRE_GST,ITM.ITM_NO, NEN.CUST_GSTIN,NEN.DIFF_PERCENT,"
				+ "NEN.ORIGINAL_CUST_GSTIN, NEN.CUST_SUPP_TYPE,NEN.CUST_SUPP_NAME,"
				+ "NEN.CUST_SUPP_CODE,NEN.CUST_SUPP_ADDRESS1,"
				+ "NEN.CUST_SUPP_ADDRESS2,NEN.CUST_SUPP_ADDRESS3,"
				+ "NEN.CUST_SUPP_ADDRESS4,NEN.BILL_TO_STATE,NEN.SHIP_TO_STATE,"
				+ "NEN.POS,NEN.STATE_APPLYING_CESS,NEN.SHIP_PORT_CODE,"
				+ "NEN.SHIP_BILL_NUM,NEN.SHIP_BILL_DATE,ITM.FOB,ITM.EXPORT_DUTY,"
				+ "ITM.ITM_HSNSAC,ITM.PRODUCT_CODE, ITM.ITM_DESCRIPTION,"
				+ "ITM.ITM_TYPE,ITM.ITM_UQC,ITM.ITM_QTY,ITM.SECTION7_OF_IGST_FLAG,"
				+ "ITM.TAXABLE_VALUE,ITM.IGST_AMT AS ITM_IGST_AMT,ITM.IGST_RATE,"
				+ "ITM.CGST_AMT AS ITM_CGST_AMT,ITM.CGST_RATE,ITM.SGST_AMT "
				+ "AS ITM_SGST_AMT, ITM.SGST_RATE, ITM.CESS_RATE_SPECIFIC,"
				...
				+ "NEN.RETURN_TYPE,NEN.TAX_DOC_TYPE,NEN.TABLE_SECTION,"
				+ "TRIM(', ' FROM IFNULL(ASPH_ER.ERROR_CODE_ASP,'') ||','|| "
				+ "IFNULL(ASPI_ER.ERROR_CODE_ASP,'')) AS INFO_ERROR_CODE_ASP,"
				+ "TRIM(', ' FROM IFNULL(ASPH_ER.ERROR_DESCRIPTION_ASP,'') ||','|| "
				+ "IFNULL(ASPI_ER.ERROR_DESCRIPTION_ASP,'')) "
				+ "AS INFO_ERROR_DESCRIPTION_ASP,DOC_AMT,NEN.IGST_AMT,"
				+ "NEN.CGST_AMT,NEN.SGST_AMT,(NEN.IGST_AMT-ITM.IGST_AMT) "
				+ "AS DIFF_IGST_AMT,(NEN.CGST_AMT-ITM.CGST_AMT) "
				+ "AS DIFF_CGST_AMT, (NEN.SGST_AMT-ITM.SGST_AMT) "
				+ "AS DIFF_SGST_AMT, (CASE WHEN NEN.IS_SAVED_TO_GSTN = TRUE "
				+ "THEN 'IS_SAVED' WHEN NEN.IS_SAVED_TO_GSTN = FALSE "
				+ "AND NEN.GSTN_ERROR = TRUE THEN 'IS_ERROR' "
				+ "WHEN NEN.IS_SAVED_TO_GSTN = FALSE AND NEN.GSTN_ERROR = FALSE "
				...
				+ "IFNULL(GSTNH_ER.ERROR_CODE_ASP,'') ||','|| "
				+ "IFNULL(GSTNI_ER.ERROR_CODE_ASP,'') AS GSTIN_ERROR_CODE,"
				+ "IFNULL(GSTNH_ER.ERROR_DESCRIPTION_ASP,'') ||','|| "
				...
				+ "NEN.CREATED_ON AS ASP_DATE_TIME FROM ANX_OUTWARD_DOC_HEADER "
				+ "NEN LEFT OUTER JOIN TF_OUTWARD_HEADER_ERROR_INFO () ASPH_ER "
				+ "ON NEN.ID = ASPH_ER.DOC_HEADER_ID INNER JOIN "
				+ "ANX_OUTWARD_DOC_ITEM ITM ON NEN.ID = ITM.DOC_HEADER_ID "
				+ "LEFT OUTER JOIN TF_OUTWARD_ITEM_ERROR_INFO() ASPI_ER "
				+ "ON ITM.DOC_HEADER_ID = ASPI_ER.DOC_HEADER_ID AND "
				+ "ITM.ITM_NO = ASPI_ER.ITM_NO INNER JOIN FILE_STATUS FIL "
				+ "ON NEN.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ "TF_OUTWARD_HEADER_GSTIN_ASP_ERROR_INFO () GSTNH_ER "
				+ "ON NEN.ID=GSTNH_ER .DOC_HEADER_ID LEFT OUTER JOIN "
				+ "TF_OUTWARD_ITEM_GSTIN_ASP_ERROR_INFO () GSTNI_ER "
				+ "ON ITM.DOC_HEADER_ID= GSTNI_ER.DOC_HEADER_ID "
				+ "AND ITM.ITM_NO=GSTNI_ER.ITM_NO LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = NEN.BATCH_ID "
				+ "WHERE NEN.RETURN_TYPE = 'GSTR1' AND NEN.IS_PROCESSED=TRUE "
				+ "AND NEN.IS_DELETE=false AND NEN.TAX_DOC_TYPE='NILEXTNON' "
				+ buildQuery;
	}
