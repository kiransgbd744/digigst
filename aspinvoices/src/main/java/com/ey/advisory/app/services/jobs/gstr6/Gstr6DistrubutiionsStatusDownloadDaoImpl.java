...
@Component("Gstr6DistrubutiionsStatusDownloadDaoImpl")
public class Gstr6DistrubutiionsStatusDownloadDaoImpl {
	...
	private List<Gstr6DistrubutionStatusReportsRespDto> fetchSaveSectionsDataByReq(
		...
		if (CollectionUtils.isNotEmpty(gstr6aSections)) {
			buildQuery1.append(" CATEGORY IN :gstr6aSections");
		}
	...
	private String creategstnTransQueryString(String buildQuery,
		...
		return " SELECT Category,TAX_PERIOD, ISD_GSTIN, REC_GSTIN, STATE_CODE, ORG_REC_GSTIN, ORG_STATE_CODE,ISD_DOC_TYPE, SUPPLY_TYPE,DOC_NUMBER,DOC_DATE, ORG_DOC_NUMBER, ORG_DOC_DATE, ORG_CRDR_DOC_NUMBER, ORG_CRDR_DOC_DATE,ELIGIBLE_INDICATOR, IGST_AMT_AS_IGST,IGST_AMT_AS_SGST,IGST_AMT_AS_CGST, SGST_AMT_AS_SGST,SGST_AMT_AS_IGST,CGST_AMT_AS_CGST, CGST_AMT_AS_IGST,CESS_AMT FROM (select CASE WHEN ISD_DOC_TYPE IN ('ISD','ISDUR','ISDCN','ISDCNUR') THEN 'DISTRIBUTION' END as Category, TAX_PERIOD,GSTIN as ISD_GSTIN,CPTY AS REC_GSTIN, STATE_CODE, NULL AS ORG_REC_GSTIN,null as ORG_STATE_CODE,ISD_DOC_TYPE, NULL AS SUPPLY_TYPE,DOC_NUMBER,DOC_DATE, NULL AS ORG_DOC_NUMBER, NULL AS ORG_DOC_DATE,CRDR_DOC_NUMBER as ORG_CRDR_DOC_NUMBER, CRDR_DOC_DATE AS ORG_CRDR_DOC_DATE,ELIGIBLE_INDICATOR, IGST_AMT_AS_IGST,IGST_AMT_AS_SGST,IGST_AMT_AS_CGST, SGST_AMT_AS_SGST,SGST_AMT_AS_IGST,CGST_AMT_AS_CGST, CGST_AMT_AS_IGST,CESS_AMT FROM GETGSTR6_ISD_DETAILS WHERE IS_DELETE = FALSE "
				+ buildQuery
				+ " union all select CASE WHEN ISD_DOC_TYPE IN ('ISDA','ISDURA','ISDCNA','ISDCNURA') THEN 'REDISTRIBUTION' END as Category,TAX_PERIOD,GSTIN as ISD_GSTIN,CPTY AS REC_GSTIN, STATE_CODE, NULL AS ORG_REC_GSTIN,null as ORG_STATE_CODE,ISD_DOC_TYPE, NULL AS SUPPLY_TYPE,DOC_NUMBER,DOC_DATE, ORG_DOC_NUMBER, ORG_DOC_DATE,O_CRDR_DOC_NUMBER as ORG_CRDR_DOC_NUMBER, O_CRDR_DOC_DATE AS ORG_CRDR_DOC_DATE,ELIGIBLE_INDICATOR, IGST_AMT_AS_IGST,IGST_AMT_AS_SGST,IGST_AMT_AS_CGST, SGST_AMT_AS_SGST,SGST_AMT_AS_IGST,CGST_AMT_AS_CGST, CGST_AMT_AS_IGST,CESS_AMT FROM GETGSTR6_ISDA_DETAILS WHERE IS_DELETE = FALSE "
				+ buildQuery + " ) WHERE" + buildQuery1;
