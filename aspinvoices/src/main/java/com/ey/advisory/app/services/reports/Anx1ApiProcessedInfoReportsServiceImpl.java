...
import com.ey.advisory.core.search.PageRequest;
	...
	@Service("Anx1ApiProcessedInfoReportsServiceImpl")
	 public class Anx1ApiProcessedInfoReportsServiceImpl
	 implements Anx1ApiProcessedInfoReportsService {
		 
		 private static final Logger LOGGER = LoggerFactory
					.getLogger(Anx1ApiProcessedInfoReportsServiceImpl.class);
		 @Autowired
			CommonUtility commonUtility;

			@Autowired
			@Qualifier("Anx1ApiProcessedInfoReportsDaoImpl")
			private Anx1ApiProcessedInfoDao anx1ApiProcessedInfoDao;

			@Override
			public Workbook findApiProcesseInfoRec(SearchCriteria criteria,
					PageRequest pageReq) {
				...
				List<Anx1ApiProcessedInfoDto> responseFromView = new ArrayList<>();
				responseFromView = anx1ApiProcessedInfoDao
						.getApiProcessedInfoReports(request);

				if (responseFromView != null && responseFromView.size() > 0) {
					String[] invoiceHeaders = commonUtility
							.getProp("anx1.api.processed.info.report.headers").split(",");
					workbook = createWorkbookWithExcelTemplate("ReportTemplates",
							"ANX1_APIProcessedInfoReport.xlsx");
