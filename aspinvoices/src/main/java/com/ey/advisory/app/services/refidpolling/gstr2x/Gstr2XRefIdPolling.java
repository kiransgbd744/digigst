...
@Slf4j
@Component("Gstr2XRefIdPolling")
public class Gstr2XRefIdPolling {
	@Autowired
	@Qualifier("DefaultGstr2XBatchIdPollingManager")
	private SaveBatchIdPollingManager batchManager;

	@Transactional(value = "clientTransactionManager")
	public ResponseEntity<String> processRefIds(PollingMessage reqDto, String groupcode) {
		Gson gson = new Gson();
		...
		if (reqDto != null) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Pooling Gstr2X ReturnStatus- batch is {} ", batch);
			}
