...
import com.ey.advisory.app.data.services.einvseries.InvoiceBrkUpSeriesGenerator;
import com.ey.advisory.app.data.services.einvseries.InvoiceSeries;
import com.ey.advisory.app.data.services.einvseries.InvoiceSeriesDTO;
import com.ey.advisory.app.data.services.einvseries.InvoiceSeriesGenerator;
 ...
 */
@Slf4j
@Component("GSTR1AEinvSeriesCompServiceImpl")
public class GSTR1AEinvSeriesCompServiceImpl
		implements GSTR1AEinvSeriesCompService {
	...
	public void compandperstSeriesData(Long configId, String gstin,
		...
		try {
			...
			if (implType.equalsIgnoreCase("A")
				...
				List<Gstr1AInvoiceFileUploadEntity> entList = convertInvSerToEntity(
						serDto);
				...
				serDto = callAdvanceCompProcandGetData(gstin, retPeriod,
						optionOpted, parts.length == 2 ? parts[1] : "");
				LOGGER.debug(
						"invSeries List size {} for gstin and taxperiod {}, {} ",
	...
	private List<InvoiceSeries> callProcandGetData(String gstin,
		...
		StoredProcedureQuery dispProc = entityManager
				.createStoredProcedureQuery("USP_COMPUTE_INOVICE_SERIES_GSTR1A");
	...
	private List<InvoiceSeriesDTO> callAdvanceCompProcandGetData(String gstin,
		...
		StoredProcedureQuery dispProc = entityManager
				.createStoredProcedureQuery("USP_INVOICE_SERIES_ADVANCED_GSTR1A");
		...
		if ("C".equalsIgnoreCase(optionOpted)) {
...
//		obj.setPattern(arr[10] != null ? arr[10].toString() : null);
	...
	public List<Gstr1AInvoiceFileUploadEntity> convertInvSerToEntity(
			List<InvoiceSeriesDTO> busProcessRecords) {
		List<Gstr1AInvoiceFileUploadEntity> invoices = new ArrayList<>();
		Gstr1AInvoiceFileUploadEntity invoice = null;
		for (InvoiceSeriesDTO obj : busProcessRecords) {
			invoice = new Gstr1AInvoiceFileUploadEntity();
	...
	private void persistInvSeriesData(String gstin, String retPeriod,
			List<Gstr1AInvoiceFileUploadEntity> invoiceDoc) {
		try {
			gstr1AInvoiceRepository.updateExistingEntries(gstin, retPeriod,
					"COMPUTE");
			gstr1AInvoiceRepository.saveAll(invoiceDoc);
		} catch (Exception e) {
