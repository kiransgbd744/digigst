package com.ey.advisory.app.services.validation.crossitc;
 ...
 *
 */
@Component("CrossItcValidationChain")
@Slf4j
public class CrossItcValidationChain
		implements DocRulesValidatorChain<CrossItcAsEnteredEntity> {
	...
	private static final B2csBusinessRuleValidator[] VALIDATORS = {
			new CrossItcGstin(), new CrossItcReturnPeriodValidator() };

	@Override
	public List<ProcessingResult> validate(CrossItcAsEnteredEntity cewb,
			ProcessingContext context) {
		List<ProcessingResult> results = new ArrayList<>();
		for (B2csBusinessRuleValidator<CrossItcAsEnteredEntity> validator : VALIDATORS) {
			if (validator == null) {
