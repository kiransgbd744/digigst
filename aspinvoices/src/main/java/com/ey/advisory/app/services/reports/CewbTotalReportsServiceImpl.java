 ...
 *
 */
@Service("CewbTotalReportsServiceImpl")
@Slf4j
public class CewbTotalReportsServiceImpl
	...
	@Autowired
	@Qualifier("CewbTotalReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Cewb total response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.CEWB).equalsIgnoreCase(request.getFileType())) {
				...
				String[] invoiceHeaders = commonUtility
						.getProp("cewb.vertical.total.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
