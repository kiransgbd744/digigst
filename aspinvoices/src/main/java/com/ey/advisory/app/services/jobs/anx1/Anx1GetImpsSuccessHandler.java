...
import com.ey.advisory.app.data.repositories.client.gstr2.GetAnx1ImpsInvoicesRepository;
 ...
 *
 */
@Service("Anx1GetImpsSuccessHandler")
@Slf4j
public class Anx1GetImpsSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Anx1ImpsDataParserImpl")
	private Anx1ImpsDataParser anx1ImpsDataParser;

	@Autowired
	@Qualifier("GetAnx1ImpsInvoicesRepository")
	private GetAnx1ImpsInvoicesRepository headerRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Anx1GetImpsSuccessHandler.class",
			...
			resultIds.forEach(id -> {
				...
				Set<GetAnx1ImpsInvoicesHeaderEntity> b2cEntities = anx1ImpsDataParser
						.parseImpsData(dto, apiResp);
				if (!b2cEntities.isEmpty()) {
					headerRepo.softlyDeleteImpsHeader(dto.getGstin(),
							dto.getReturnPeriod());
