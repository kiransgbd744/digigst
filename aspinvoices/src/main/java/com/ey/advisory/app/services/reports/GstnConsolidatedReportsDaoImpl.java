 ...
 *
 */
@Component("GstnConsolidatedReportsDaoImpl")
@Slf4j
public class GstnConsolidatedReportsDaoImpl
	...
	public List<DataStatusEinvoiceDto> getGstnConsolidatedReports(
		...
		if (gstin != null && !gstin.isEmpty()) {
			buildQuery.append(" AND HDR.SUPPLIER_GSTIN  = :gstin ");
		}
	...
	private String createGstr1AGstnErrorQueryString(String buildQuery,
		...
		return " SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') "
				...
				+ "AND HDR.GSTN_ERROR = TRUE AND GSTN_SAVE_STATUS='PE' AND "
				+ "HDR.RETURN_TYPE='GSTR1A' " 
                + buildQuery
                + "ORDER BY DOC_NUM,ITM.ITM_NO " ;
