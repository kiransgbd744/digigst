...
import com.ey.advisory.app.data.views.client.Gstr1GstnB2CSSummaryTablesDto;
...
@Component("Gstr1AGetB2CSSummaryTablesDaoImpl")
@Slf4j
public class Gstr1AGetB2CSSummaryTablesDaoImpl implements Gstr1AaGetDao {
	...
	private Gstr1GstnB2CSSummaryTablesDto convertTransactionalLevel(
			Object[] arr) {
		Gstr1GstnB2CSSummaryTablesDto obj = new Gstr1GstnB2CSSummaryTablesDto();
		...
		obj.setType(arr[13] != null ? arr[13].toString() : null);
		obj.setEcomGSTIN(arr[14] != null ? arr[14].toString() : null);
		//obj.setIsFiled(arr[15] != null ? arr[15].toString() : null);
		obj.setIsFiled(isDecimal(arr[15]));
		obj.setSerialNo(arr[18] != null ? arr[18].toString() : null);
		return obj;
	}
	...
	private String creategstnB2CSSummaryQueryString(String buildQuery) {
		...
		return "select ID,GSTIN,RETURN_PERIOD,STATE_NAME,SUPPLY_TYPE,"
				+ "POS,TAX_RATE,TAXABLE_VALUE,IGST_AMT,CGST_AMT,SGST_AMT,"
				+ "CESS_AMT,DIFF_PERCENT,ECOM_TYPE,ECOM_GSTIN,IS_FILED,"
				+ "DERIVED_RET_PERIOD,TABLE_TYPE,"
				+ "(ROW_NUMBER () OVER (ORDER BY GSTIN)) AS  SNO from (SELECT HDR.ID,HDR.GSTIN,"
				+ "HDR.RETURN_PERIOD,MT.STATE_NAME,"
				+ "SUPPLY_TYPE,POS,ITM.TAX_RATE,"
				...
				+ "BT.IS_FILED,HDR.DERIVED_RET_PERIOD,'B2CS' TABLE_TYPE "
				...
				+ "SUBSTR(HDR.GSTIN,1,2) = MT.STATE_CODE WHERE HDR.IS_DELETE=FALSE "
				+ " GROUP BY HDR.ID,HDR.GSTIN,HDR.RETURN_PERIOD,"
				+ "MT.STATE_NAME,SUPPLY_TYPE,POS,ITM.TAX_RATE,HDR.DIFF_PERCENT,"
				+ "HDR.ECOM_TYPE,HDR.ETIN,BT.IS_FILED,HDR.DERIVED_RET_PERIOD)"
				+ buildQuery;
