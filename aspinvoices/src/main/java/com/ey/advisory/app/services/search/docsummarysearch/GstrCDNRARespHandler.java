...
@Service("GstrCDNRARespHandler")
public class GstrCDNRARespHandler {
	...
	public JsonElement handleCDNRAResp(List<? extends Gstr1SummaryDto> list,
			Gstr1SummaryReqDto gstr1SummaryRequest) {
		
		List<Gstr1BasicSummarySectionDto> cdnraEySummary = new ArrayList<>();
		for (Gstr1SummaryDto gstr1Summary : list) {
			cdnraEySummary = gstr1Summary.getCdnra().getEySummary();
		}
		...
		//TO-DO Get GSTN Live Data and add it to gstnList
				 ...
				 */
		SearchResult<Gstr1SummaryDto> cdnraGstnResult = 
				(SearchResult<Gstr1SummaryDto>) gstrSummarySectService
				.find(gstr1SummaryRequest, null, Gstr1SummaryDto.class);
		
		List<? extends Gstr1SummaryDto> gstnListResp = cdnraGstnResult.getResult();
...
*/				
				...
				JsonElement cdnraRespBody = gstr1SummaryStructure
				.cdnraStructure(cdnraEySummary, gstnList);
