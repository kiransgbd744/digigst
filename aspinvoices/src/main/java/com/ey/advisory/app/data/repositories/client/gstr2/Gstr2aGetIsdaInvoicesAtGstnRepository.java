 ...
 *
 */
@Repository("Gstr2aGetIsdaInvoicesAtGstnRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr2aGetIsdaInvoicesAtGstnRepository
        extends CrudRepository<GetGstr2aIsdaInvoicesHeaderEntity, Long> {

    // Curently isDelete is not added in table
    @Modifying
    @Query("UPDATE GetGstr2aIsdaInvoicesHeaderEntity b SET b.isDelete = true ,"
    ...
    @Query("SELECT COUNT(*) FROM GetGstr2aIsdaInvoicesHeaderEntity "
            + "WHERE cgstin=:gstin AND returnPeriod = :taxperiod and isDelete =false")
    public int gstinCount(@Param("gstin") String gstin,
            @Param("taxperiod") String taxperiod);

    @Query("from GetGstr2aIsdaInvoicesHeaderEntity where gstin=:sgstin and cgstin=:cgstin and documentDate=:invDate and documentNumber=:invNum and isdDocumentType=:invType")
    List<GetGstr2aIsdaInvoicesHeaderEntity> findByInvoiceKey(
    ...
    @Modifying
    @Query("UPDATE GetGstr2aIsdaInvoicesHeaderEntity b SET b.isDelete = true, b.modifiedOn=:modifiedOn WHERE"
