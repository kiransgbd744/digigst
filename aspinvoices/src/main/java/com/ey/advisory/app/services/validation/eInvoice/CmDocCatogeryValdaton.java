 ...
 *
 */
@Component("CmDocCatogeryValdaton")
public class CmDocCatogeryValdaton
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		/*if (!BigDecimalNagativeValueUtil.valid(docCategory, CustAddress1,
				C)) {
		...
		if (!BigDecimalNagativeValueUtil.valid(docCategory, custLocation,
				C)) {
			List<String> errorLocations = new ArrayList<>();
		...
		/*if (!BigDecimalNagativeValueUtil.valid(docCategory, SuppAddress1,
				R)) {
			List<String> errorLocations = new ArrayList<>();
		...
		if (!BigDecimalNagativeValueUtil.valid(docCategory, dispatcherAddr2,
				D)) {
			List<String> errorLocations = new ArrayList<>();
			errorLocations.add(GSTConstants.DIS_PATCHER_ADDR2);
			TransDocProcessingResultLoc location 
			                = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
		...
		if (!BigDecimalNagativeValueUtil.valid(docCategory, shipToAddr2, S)) {
			List<String> errorLocations = new ArrayList<>();
		...
		if (!BigDecimalNagativeValueUtil.valid(docCategory, shipToLocation, S)) {
			List<String> errorLocations = new ArrayList<>();
