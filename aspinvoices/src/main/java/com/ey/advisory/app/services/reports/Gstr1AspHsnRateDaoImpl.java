...
@Component("Gstr1AspHsnRateDaoImpl")
public class Gstr1AspHsnRateDaoImpl /* implements Gstr1AspHsnSummaryDao */ {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1AspHsnRateDaoImpl.class);
	...
	private static String createHsnSummaryQueryString(String buildQuery,
		...
		bufferString.append(
				"SELECT SUPPLIER_GSTIN "
						...
						+ "	,ITM_DESCRIPTION "
						+ "	,ITM_UQC "
						+ "	,SUM(ASP_ITM_QTY) ASP_ITM_QTY "
						+ "	,SUM(ASP_TOTAL_VALUE) ASP_TOTAL_VALUE "
						+ "	,SUM(ASP_TAXABLE_VALUE) ASP_TAXABLE_VALUE "
						+ "	,SUM(ASP_IGST) ASP_IGST "
						+ "	,SUM(ASP_CGST) ASP_CGST "
						+ "	,SUM(ASP_SGST) ASP_SGST "
						+ "	,SUM(ASP_CESS) ASP_CESS "
						+ "	,SUM(UI_ITM_QTY) UI_ITM_QTY "
						+ "	,SUM(UI_TOTAL_VALUE) UI_TOTAL_VALUE "
						+ "	,SUM(UI_TAXABLE_VALUE) UI_TAXABLE_VALUE "
						+ "	,SUM(UI_IGST) UI_IGST "
						+ "	,SUM(UI_CGST) UI_CGST "
						+ "	,SUM(UI_SGST) UI_SGST "
						+ "	,SUM(UI_CESS) UI_CESS "
						+ "	,SAVE_STATUS "
						+ "	,CASE "
						+ "		WHEN SAVE_STATUS = 'IS_SAVED' "
						+ "			THEN GSTIN_REF_ID "
						+ "		ELSE NULL "
						+ "		END AS GSTIN_REF_ID "
						+ "	,CASE "
						+ "		WHEN SAVE_STATUS = 'IS_SAVED' "
						+ "			THEN GSTIN_REF_ID_TIME "
						+ "		ELSE NULL "
						+ "		END AS GSTIN_REF_ID_TIME "
						+ "	,GSTIN_ERROR_CODE "
						+ "	,GSTIN_ERROR_DESCRIPTION "
						+ "	,ROW_NUMBER() OVER ( "
						...
						+ "		,MAX(GSTIN_ERROR_DESCRIPTION) GSTIN_ERROR_DESCRIPTION "
						+ "		,IFNULL(TAX_RATE, 0) TAX_RATE,RecordType "
						+ "	FROM ( "
						+ "		SELECT SUPPLIER_GSTIN "
						+ "			,RETURN_PERIOD "
						+ "			,DERIVED_RET_PERIOD "
						...
						+ "															) "
						+ "														) "
						+ "													THEN IFNULL(ONB_LINE_ITEM_AMT, 0) "
						+ "												WHEN ( "
						+ "														TABLE_SECTION IN ( "
						+ "															'4A' "
						+ "															,'5A' "
						...
						+ "												WHEN TABLE_SECTION = '4B' "
						+ "													THEN IFNULL(TAXABLE_VALUE, 0) "
						+ "												WHEN ( "
						+ "														ECP.QUESTION_CODE = 'O21' "
						+ "														AND ECP.ANSWER IN ('B') "
						+ "														AND SUPPLY_TYPE IN ( "
						+ "															'EXPT' "
						+ "															,'EXPWT' "
						...
						+ "												,DOC_TYPE "
						+ "												,ITM.ITM_HSNSAC "
						+ "												,IFNULL(ITM.ITM_UQC, 'OTH') AS ITM_UQC "
						+ "												,( "
						+ "													CASE "
						...
						+ "													WHEN 'B' = :configAnswer "
						+ "														AND DOC_TYPE = 'CR' "
						+ "														AND IFNULL(CRDR_REASON, 'NA') <> 'SR' "
						+ "														THEN 0 "
						+ "													WHEN 'B' = :configAnswer "
						+ "														AND DOC_TYPE = 'DR' "
						+ "														AND IFNULL(CRDR_REASON, 'NA') <> 'SR' "
						+ "														THEN 0 "
						+ "													ELSE IFNULL(ITM_QTY, 0) "
						+ "													END AS ITM_QTY "
						+ "												,IFNULL(ONB_LINE_ITEM_AMT, 0) AS TOTAL_VALUE "
						...
						+ "													CASE "
						+ "														WHEN ITM.CUST_GSTIN IS NOT NULL AND LENGTH(ITM.CUST_GSTIN) = 15 THEN 'B2B' "
						+ "														WHEN (LENGTH(ITM.CUST_GSTIN) < 15 OR ITM.CUST_GSTIN IS NULL)    THEN 'B2C' END AS RecordType "
						...
						+ "											UNION ALL "
						+ "											 "
						+ "											SELECT (HDR.DERIVED_RET_PERIOD || '|' || HDR.SUPPLIER_GSTIN || '|' || IFNULL(NEW_UOM, 'OTH') || '|' || IFNULL(NEW_RATE, 0) || '|' || IFNULL(NEW_HSNORSAC, 'null')) DOC_KEY "
						+ "												,HDR.DERIVED_RET_PERIOD "
						+ "												,HDR.SUPPLIER_GSTIN "
						+ "												,HDR.RETURN_PERIOD "
						+ "												,'TAX' SUPPLY_TYPE "
						+ "												,'INV' DOC_TYPE "
						+ "												,NEW_HSNORSAC AS ITM_HSNSAC "
						+ "												,IFNULL(NEW_UOM, 'OTH') AS ITM_UQC "
						+ "												,( "
						+ "													CASE "
						+ "														WHEN IS_SAVED_TO_GSTN = TRUE "
						+ "															AND HDR.IS_DELETE = FALSE "
						...
						+ "														END "
						+ "													) GSTIN_REF_ID_TIME "
						+ "												,IFNULL(NEW_QNT, 0) AS ITM_QTY "
						+ "												,IFNULL(NEW_TAXABLE_VALUE, 0) + IFNULL(IGST_AMT, 0) + IFNULL(CGST_AMT, 0) + IFNULL(SGST_AMT, 0) + IFNULL(CESS_AMT, 0) AS TOTAL_VALUE "
						+ "												,IFNULL(NEW_TAXABLE_VALUE, 0) AS TAXABLE_VALUE "
						+ "												,IFNULL(IGST_AMT, 0) AS IGST_AMT "
						+ "												,IFNULL(CGST_AMT, 0) AS CGST_AMT "
						+ "												,IFNULL(SGST_AMT, 0) AS SGST_AMT "
						+ "												,IFNULL(CESS_AMT, 0) AS CESS_AMT "
						+ "												,'B2CS' AS TAX_DOC_TYPE "
						+ "												,'7' TABLE_SECTION "
						+ "												,IFNULL(NEW_TAXABLE_VALUE, 0) + IFNULL(IGST_AMT, 0) + IFNULL(CGST_AMT, 0) + IFNULL(SGST_AMT, 0) + IFNULL(CESS_AMT, 0) ONB_LINE_ITEM_AMT "
						+ "												,0.00 OTHER_VALUES "
						+ "												,NEW_RATE TAX_RATE, 'B2C' AS RecordType"
						...
						+ "											UNION ALL "
						+ "											 "
						+ "											SELECT (DERIVED_RET_PERIOD || '|' || SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') || '|' || IFNULL(ITM_HSNSAC, 'null')) DOC_KEY "
						+ "												,DERIVED_RET_PERIOD "
						+ "												,SUPPLIER_GSTIN "
						+ "												,RETURN_PERIOD "
						+ "												,SUPPLY_TYPE "
						...
						+ "															ELSE 0 "
						+ "															END), 0) AS ITM_QTY "
						+ "												,IFNULL(SUM(TAXABLE_VALUE), 0) AS TOTAL_VALUE "
						...
						+ "												,'8' TABLE_SECTION "
						+ "												,IFNULL(SUM(TAXABLE_VALUE), 0) ONB_LINE_ITEM_AMT "
						+ "												,0.00 OTHER_VALUES "
						+ "												,0.00 TAX_RATE, RecordType "
						
						+ "											FROM ( "
						+ "												SELECT HDR.* "
						+ "													,ROW_NUMBER() OVER ( "
						+ "														PARTITION BY (HDR.DERIVED_RET_PERIOD || '|' || HDR.SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') || '|' || 0.00 || '|' || IFNULL(ITM_HSNSAC, 'null')) ORDER BY (HDR.DERIVED_RET_PERIOD || '|' || HDR.SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') || '|' || 0.00 || '|' || IFNULL(ITM_HSNSAC, 'null')) "
						+ "														) AS RN "
						+ "													,( "
						+ "														CASE "
						+ "															WHEN IS_SAVED_TO_GSTN = TRUE "
						...
						+ "														CASE "
						+ "                                                        WHEN TABLE_SECTION IN ( '8A', '8B') THEN 'B2B' "
						+ "													       WHEN TABLE_SECTION IN ( '8C', '8D') THEN 'B2C' "
						+ "														END AS RecordType "
						...
						+ buildHeader
						+ "												) "
						+ "											GROUP BY (DERIVED_RET_PERIOD || '|' || SUPPLIER_GSTIN || '|' || IFNULL(ITM_UQC, 'OTH') || '|' || IFNULL(ITM_HSNSAC, 'null')) "
						+ "												,DERIVED_RET_PERIOD "
						+ "												,SUPPLIER_GSTIN "
						+ "												,RETURN_PERIOD "
						+ "												,SUPPLY_TYPE "
						...
						+ "											AND ECP.IS_DELETE = FALSE "
						+ "											AND ECP.QUESTION_CODE IN ('O21') "
						+ "											AND DOC_TYPE IN ( "
						+ "												'INV' "
						+ "												,'BOS' "
						+ "												,'CR' "
						...
						+ "									ITM_HSNSAC != '0' "
						+ "									OR ITM_UQC != '0' "
						+ "									OR ASP_TOTAL_VALUE != '0' "
						+ "									OR ASP_TAXABLE_VALUE != '0' "
						+ "									OR ASP_IGST != '0' "
						+ "									OR ASP_CGST != '0' "
						+ "									OR ASP_SGST != '0' "
						+ "									OR ASP_CESS != '0' "
						...
						+ "									,ITM_UQC "
						+ "									,ROW_NUMBER() OVER ( "
						+ "										PARTITION BY ITM_HSNSAC "
						+ "										,ITM_UQC ORDER BY CREATED_ON DESC "
						+ "										) AS ROWNUM_DT "
						+ "									,SAVE_STATUS "
						+ "									,GSTIN_REF_ID "
						+ "									,GSTIN_REF_ID_TIME "
						...
						+ "											) "
						+ buildHeader
						+ "										AND GSTNBATCH.RETURN_TYPE = 'GSTR1' "
						+ "									 "
						+ "									UNION ALL "
						+ "									 "
						...
						+ "									UNION ALL "
						+ "									 "
						+ "									SELECT 'NILEXTNON' AS TAX_DOC_TYPE "
						+ "										,HDR.DERIVED_RET_PERIOD "
						+ "										,HDR.SUPPLIER_GSTIN "
						+ "										,HDR.CREATED_ON "
						+ "										,HDR.RETURN_PERIOD "
						...
						+ "				,ITM_HSNSAC "
						+ "				,IFNULL(ITM_UQC, 'OTH') ITM_UQC "
						+ "				,SUM(ITM_QTY) AS UI_ITM_QTY "
						+ "				,SUM(TOTAL_VALUE) AS UI_TOTAL_VALUE "
						+ "				,SUM(TAXABLE_VALUE) AS UI_TAXABLE_VALUE "
						+ "				,SUM(IGST) AS UI_IGST "
						+ "				,SUM(CGST) AS UI_CGST "
						+ "				,SUM(SGST) AS UI_SGST "
						+ "				,SUM(CESS) AS UI_CESS "
						...
						+ "				,TAX_RATE, RecordType "
						+ "			FROM ( "
						+ "				SELECT HSN.SUPPLIER_GSTIN "
						+ "					,HSN.RETURN_PERIOD "
						+ "					,HSN.DERIVED_RET_PERIOD "
						+ "					,HSNSAC AS ITM_HSNSAC "
						+ "					,IFNULL(UQC, 'OTH') ITM_UQC "
						+ "					,IFNULL(SUM(ITM_QTY), 0) ITM_QTY "
						+ "					,IFNULL(SUM(TOTAL_VALUE), 0) TOTAL_VALUE "
						+ "					,IFNULL(SUM(TAXABLE_VALUE), 0) TAXABLE_VALUE "
						+ "					,IFNULL(SUM(IGST_AMT), 0) IGST "
						+ "					,IFNULL(SUM(CGST_AMT), 0) CGST "
						+ "					,IFNULL(SUM(SGST_AMT), 0) SGST "
						+ "					,IFNULL(SUM(CESS_AMT), 0) CESS "
						+ "					,( "
						+ "						CASE "
						+ "							WHEN IS_SAVED_TO_GSTN = TRUE "
						+ "								AND HSN.IS_DELETE = FALSE "
						+ "								THEN 'IS_SAVED' "
						+ "							ELSE 'NOT_SAVED' "
						+ "							END "
						+ "						) SAVE_STATUS "
						...
						+ "							END "
						+ "						) GSTIN_ERROR_DESCRIPTION_ASP "
						+ "					,TAX_RATE,Record_Type AS  RecordType "
						+ "				FROM GSTR1_USERINPUT_HSNSAC HSN "
						+ "				LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON GSTNBATCH.ID = HSN.BATCH_ID "
						+ "				WHERE HSN.IS_DELETE = FALSE "
						+ buildQuery
						+ "				GROUP BY HSN.SUPPLIER_GSTIN "
						+ "					,HSN.RETURN_PERIOD "
						+ "					,HSN.DERIVED_RET_PERIOD "
						+ "					,HSNSAC "
						+ "					,UQC "
