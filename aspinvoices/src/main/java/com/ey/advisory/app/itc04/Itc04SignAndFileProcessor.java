 ...
 */
@Slf4j
@Component("Itc04SignAndFileProcessor")
public class Itc04SignAndFileProcessor implements TaskProcessor {
	...
	public void execute(Message message, AppExecContext context) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("ITC04 Sign&File GSTN payload about to send is {}",
						data);
			}
			APIResponse itc04FileApiResp = hitGstnServer.itc04FileApiCall(
					TenantContext.getTenantId(), data, gstin, taxPeriod);
			if (itc04FileApiResp.isSuccess()) {

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("SignAndFie Sandbox api success {} ",
							itc04FileApiResp.getResponse());
				}
				JsonObject responseParams = (new JsonParser())
						.parse(itc04FileApiResp.getResponse())
						.getAsJsonObject();

				String ackNum = responseParams.get("ack_num").getAsString();
				itc04SignAndFileService.updateItc04Tables(taxPeriod, gstin,
						signId, ackNum, now);

			} else {
				LOGGER.error("ITC04 Sign&File Failed with response {} ",
						itc04FileApiResp.getError());
				signAndFileRepo.updateStatus(null, "Failed",
						itc04FileApiResp.getError().getErrorDesc(),
						Long.valueOf(signId));
			}
		} catch (Exception ex) {
			String msg = String.format(
					"Exception while ITC04 Sign&File, jsonParams are '%s'",
					jsonString);
