 ...
 *
 */
public class OriginalDocNumSame
	...
	private boolean isValidTax(OutwardTransDocLineItem firstType,
		...
		if(isPresent(firstType.getOrigDocNo()) 
				&& !isPresent(curType.getOrigDocNo())){
			return false;
		}
		...
		if (firstType.getOrigDocNo() != null
				&& !firstType.getOrigDocNo().isEmpty()){
			if (!firstType.getOrigDocNo()
					.equalsIgnoreCase(curType.getOrigDocNo())) {
				return false;
			}
		}
		return true;
