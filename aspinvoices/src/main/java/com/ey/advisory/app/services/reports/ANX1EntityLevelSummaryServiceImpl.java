...
@Service("ANX1EntityLevelSummaryServiceImpl")
public class ANX1EntityLevelSummaryServiceImpl
		implements ANX1EntityLevelSummaryService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ANX1EntityLevelSummaryServiceImpl.class);

	@Autowired
	private CommonUtility commonUtility;

	@Autowired
	@Qualifier("ANX1EntityLevelSummaryDaoImpl")
	private ANX1EntityLevelSummaryDao aNX1EntityLevelSummaryDao;
	...
	public Workbook findEntityLevelSummary(SearchCriteria criteria,
		...
		List<Anx1EntityLevelSummaryDto> responseFromView = new ArrayList<>();
		responseFromView = aNX1EntityLevelSummaryDao
				.getEntityLevelSummary(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.entitylevel.report.headers").split(",");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"Anx1_EntityLevelSummary.xlsx");
