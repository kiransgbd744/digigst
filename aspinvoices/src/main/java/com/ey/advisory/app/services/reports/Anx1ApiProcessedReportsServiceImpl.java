...
@Service("Anx1ApiProcessedReportsServiceImpl")
public class Anx1ApiProcessedReportsServiceImpl
		implements Anx1ApiProcessedReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1ApiProcessedReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1ApiProcessedReportsDaoImpl")
	private Anx1ApiProcessedReportsDao anx1ApiProcessedReportsDao;
	...
	public Workbook findApiProcesseRec(SearchCriteria criteria,
		...
		List<Anx1ApiProcessedRecordsDto> responseFromView = new ArrayList<>();
		responseFromView = anx1ApiProcessedReportsDao
				.getApiProcessedReports(request);

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("anx1.api.processed.report.headers").split(",");
			workbook = createWorkbookWithExcelTemplate("ReportTemplates",
					"ANX1_APIprocessedRecReport.xlsx");
