...
@Service("Gstr2ConsolidatedAspErrorReportsServiceImpl")
public class Gstr2ConsolidatedAspErrorReportsServiceImpl
		implements Gstr2AspErrorReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr2ConsolidatedAspErrorReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr2ConsolidatedAspErrorReportsDaoImpl")
	private Gstr2ConsolidatedAspErrorReportsDaoImpl gstr2AspErrorReportsDaoImpl;
	...
	public Workbook findError(SearchCriteria searchParams, PageRequest pageReq) {
		...
		responseFromView = gstr2AspErrorReportsDaoImpl.getErrorReports(request);
		workbook = createWorkbookWithExcelTemplate("ReportTemplates",
				"GSTR2_APIErrorReport.xlsx");

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility
					.getProp("gstr2.asp.error.report.headers").split(",");
