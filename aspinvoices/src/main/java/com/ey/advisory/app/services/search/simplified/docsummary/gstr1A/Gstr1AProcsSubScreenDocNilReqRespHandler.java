...
import com.ey.advisory.app.services.search.docsummarysearch.ProcSubmitNilSummaryStructure;
import com.ey.advisory.app.services.search.docsummarysearch.ProcsSubmitDocSummaryStructure;
 ...
 */
@Slf4j
@Service("Gstr1AProcsSubScreenDocNilReqRespHandler")
public class Gstr1AProcsSubScreenDocNilReqRespHandler {

	public static final String docissued = "DOC_ISSUED";
	public static final String nilNoN = "NILEXTNON";

	@Autowired
	@Qualifier("Gstr1AProcsSubmitDocIssueSummarySearchService")
	Gstr1AProcsSubmitDocIssueSummarySearchService service;

	@Autowired
	@Qualifier("Gstr1AProcsSubmitNilExtNonSearchService")
	Gstr1AProcsSubmitNilExtNonSearchService nilService;
	...
	public List<Gstr1SummaryScreenDocRespDto> handleGstr1DocReqAndResp(
		...
		for (Gstr1CompleteSummaryDto dto : list) {
			...
			if (docSummary != null) {
				docSummary.forEach(docEySummary -> {
					...
					summaryResp
							.setGstnCancelled(docEySummary.getGstnCancelled());
					summaryResp
							.setGstnNetIssued(docEySummary.getGstnNetIssued());
		...
		List<Gstr1SummaryScreenDocRespDto> handleDocResp = docRespHandler
				.gstr1DocResp(docSummaryRespList, docissued);
	...
	public List<Gstr1SummaryScreenNilRespDto> handleGstr1NilReqAndResp(
		...
		for (Gstr1CompleteSummaryDto dto : list) {
			...
			// nilSummary =
			// gstnApiNilCalculation.addNilaspGstnDocTypes(nilSummaryDetail);

			Gstr1SummaryScreenNilRespDto summaryResp = new Gstr1SummaryScreenNilRespDto();
			if (nilSummary != null) {
		...
		/*
		 * Gson gson = GsonUtil.newSAPGsonInstance();
		 * List<Gstr1SummaryScreenNilRespDto> handleNilResp =
		 * gstr1NilRespHandler .handleNilResp(nilSummaryRespList);
		 */

		List<Gstr1SummaryScreenNilRespDto> gstr1NilResp = reqRespDefault
				.gstr1NilResp(nilSummaryRespList, nilNoN);
