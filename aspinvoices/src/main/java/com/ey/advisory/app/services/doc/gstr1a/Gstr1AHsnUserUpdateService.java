...
@Service("Gstr1AHsnUserUpdateService")
public class Gstr1AHsnUserUpdateService {

	@Autowired
	@Qualifier("Gstr1AUserInputHsnSacRepository")
	Gstr1AUserInputHsnSacRepository repo;
	...
	Gstr1AHsnValidationChain hsnBusinessChain;
	...
	public JsonObject updateVerticalData(List<Gstr1HsnSummarySectionDto> list) {
		...
		for (Gstr1HsnSummarySectionDto dto : list) {
			...
			Gstr1AUserInputHsnSacEntity entity = new Gstr1AUserInputHsnSacEntity();
			 ...
			 * 
			 * String hsn = dto.getHsn(); //hsn.substring(0, 1); if
			 * ("99".equalsIgnoreCase(hsn.substring(0, 2)) ||
			 * "OTH".equalsIgnoreCase(entity.getUqc()) ||
			 * com.google.common.base.Strings.isNullOrEmpty(entity.getUqc())){
			 * 
			 * entity.setUqc("NA"); entity.setUsrQunty(BigDecimal.ZERO);
			 * 
			 * } }else{ entity.setUqc(dto.getUqc());
			...
			/*
			 * if (dto.getDocKey() != null) { // hsnData =
			 * repo.getHsnData(dto.getDocKey()); //
			 * entity.setDesc(dto.getUiDesc()); repo.UpdateId(dto.getDocKey());
			 * entity.setDocKey(dto.getDocKey()); } else { String docKey ;
			 * if(rateIncludedInHsn){ docKey = getProcessedKey(dto); }else{
			 * docKey = getProcessedKeywR(dto); } //
			 * entity.setDesc(dto.getUiDesc()); // hsnData =
			 * repo.getHsnData(docKey); entity.setDocKey(docKey); }
			 */
			...
			if (structureValidation.isEmpty()) {
				...
				Gstr1AAsEnteredHsnEntity convertBusinessEntityToChain = convertBusinessEntityToChain(
						dto);
			...
			if (!errorList.isEmpty()) {
				...
				Gstr1AUserInputHsnSacEntity hsnData = repo
				...
				Gstr1AUserInputHsnSacEntity save = repo.save(entity);
	...
	private void aggregateHsn(Gstr1AUserInputHsnSacEntity hsnData,
			Gstr1AUserInputHsnSacEntity entity) {
	...
	public Gstr1AAsEnteredHsnEntity convertBusinessEntityToChain(
			Gstr1HsnSummarySectionDto list) {
		Gstr1AAsEnteredHsnEntity entity = new Gstr1AAsEnteredHsnEntity();
		if (list != null) {
	...
	public Gstr1HsnSummarySectionDto convertEntityToDto(
			Gstr1AUserInputHsnSacEntity save) {
	...
	public String getProcessedKey(Gstr1AUserInputHsnSacEntity obj) {
	...
	public String getProcessedKeywR(Gstr1AUserInputHsnSacEntity obj) {
		...
		return new StringJoiner(GSTConstants.WEB_UPLOAD_KEY).add(taxPeriod)
				.add(sgstn).add(uqc).add(hsn).toString();
	}
