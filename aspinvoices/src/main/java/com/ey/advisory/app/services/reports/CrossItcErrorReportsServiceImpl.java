 ...
 *
 */
@Service("CrossItcErrorReportsServiceImpl")
@Slf4j
public class CrossItcErrorReportsServiceImpl
	...
	@Autowired
	@Qualifier("CrossItcErrorReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Cross ITC error response" + responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.CROSS_ITC).equalsIgnoreCase(request.getFileType())) { 
				...
				String[] invoiceHeaders = commonUtility
						.getProp("crossitc.vertical.error.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
