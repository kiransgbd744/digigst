...
@Component("CMOriginalDocumentRnvNumberValidator")
public class CMOriginalDocumentRnvNumberValidator
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		if (document.getDocType() != null && !document.getDocType().isEmpty()) {
			if (document.getSupplyType() != null
				...
				if (SUPPLYTYPE1.contains(trimAndConvToUpperCase(document.getSupplyType()))
						&& DOCTYPE.contains(trimAndConvToUpperCase(document.getDocType()))) {
					IntStream.range(0, items.size()).forEach(idx -> {
						...
						if (item.getOrigDocNo() == null
							...
							errors.add(new ProcessingResult(APP_VALIDATION,
									"ER0039",
									"Original Document Number cannot be left balnk.",
									location));
						}
					});
				}
