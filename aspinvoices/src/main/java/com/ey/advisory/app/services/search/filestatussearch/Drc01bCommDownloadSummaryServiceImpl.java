 ...
 *
 */
@Component("Drc01bCommDownloadSummaryServiceImpl")
@Slf4j
public class Drc01bCommDownloadSummaryServiceImpl
	...
	public void generateReports(Long requestId, Long entryId) {
		...
		try {
			...
			if (gstinList.size() > 1) {
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						+ "DetailedReport_" + commType + "_" + gstinString + "_"
						+ format.format(convertISDDate) + ".xlsx";
			}
	...
	public Workbook getDrcData(List<String> gstin, String taxPeriod,
			Long requestId, List<TblDrcDetails> tblDrcDetails, Long entryId) {
		...
		try {
			...
			String msg = String.format(
					"Error occurred in Drc01bCommDownloadSummaryServiceImpl.getDrcData()",
					e);
			drc01RequestCommDetailsRepo.updateStatus(requestId,
	...
	private Workbook writeToExcel(List<Gstr3BDrc01DownloadDto> respDrc01List,
		...
		try {
			if (respDrc01List != null && !respDrc01List.isEmpty()) {
				...
				if (LOGGER.isDebugEnabled()) {
					String msg = "Drc01bCommDownloadSummaryServiceImpl.writeToExcel "
