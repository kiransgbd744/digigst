...
public class Itc04JWDeliveryChallanDateValidation
	...
	public List<ProcessingResult> validate(Itc04HeaderEntity document,
		...
		if (document.getRetPeriod() != null
			...
			if (n <= 0) {
				...
				LocalDate jwDelChaDate = document.getJwDeliveryChallanaDate();
				...
				if (jwDelChaDate != null
						&& jwDelChaDate.compareTo(retPer) > 0) {
					Set<String> errorLocations = new HashSet<>();
					errorLocations.add(GSTConstants.JW_DELIVERY_CHALLAN_DATE);
					TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
							null, errorLocations.toArray());
					errors.add(new ProcessingResult(APP_VALIDATION, "ER5860",
							"JWDeliveryChallanDate cannot be greater than last date of return period.",
							location));
					return errors;
				}
