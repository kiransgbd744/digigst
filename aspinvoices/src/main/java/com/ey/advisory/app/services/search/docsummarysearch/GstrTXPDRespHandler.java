...
@Service("GstrTXPDRespHandler")
public class GstrTXPDRespHandler {
	...
	public JsonElement handleTXPDResp(List<? extends Gstr1SummaryDto> list,
			Gstr1SummaryReqDto gstr1SummaryRequest) {
		
		List<Gstr1BasicSummarySectionDto> txpdEySummary = new ArrayList<>();
		for (Gstr1SummaryDto gstr1Summary : list) {
			txpdEySummary = gstr1Summary.getTxpd().getEySummary();
		}
		...
		//TO-DO Get GSTN Live Data and add it to gstnList
				 ...
				 */
		SearchResult<Gstr1SummaryDto> txpdGstnResult = 
				(SearchResult<Gstr1SummaryDto>) gstrSummarySectService
				.find(gstr1SummaryRequest, null, Gstr1SummaryDto.class);
		
		List<? extends Gstr1SummaryDto> gstnListResp = txpdGstnResult.getResult();
...
*/				/**
				...
				JsonElement txpdRespBody = gstr1SummaryStructure
				.txpdStructure(txpdEySummary,gstnList);
