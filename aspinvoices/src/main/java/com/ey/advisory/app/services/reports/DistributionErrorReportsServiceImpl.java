...
import com.opencsv.bean.StatefulBeanToCsvBuilder;
	...
	@Service("DistributionErrorReportsServiceImpl")
	public class DistributionErrorReportsServiceImpl
			implements DistributionReportsService {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(DistributionErrorReportsServiceImpl.class);

		@Autowired
		CommonUtility commonUtility;

		@Autowired
		@Qualifier("DistributionErrorReportsDaoImpl")
		...
		public void generateFileStatusCsv(SearchCriteria criteria,
			...
			List<DistributionTotalDto> responseErrorDao = distributionReportsDao
					.generateFileStatusCsv(request);

			if (responseErrorDao != null && !responseErrorDao.isEmpty()) {
				...
				try (Writer writer = new BufferedWriter(new FileWriter(fullPath),
					...
					invoiceHeadersTemplate = commonUtility
							.getProp("dist.error.csv.report.headers");
					writer.append(invoiceHeadersTemplate);
					columnMappings = commonUtility
							.getProp("dist.error.csv.report.headers.mapping")
							.split(",");
					...
					beanWriter.write(responseErrorDao);
