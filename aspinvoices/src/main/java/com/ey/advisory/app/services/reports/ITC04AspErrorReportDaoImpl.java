...
@Component("ITC04AspErrorReportDaoImpl")
public class ITC04AspErrorReportDaoImpl {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ITC04AspErrorReportDaoImpl.class);

	@PersistenceContext(unitName = "clientDataUnit")
	private EntityManager entityManager;

	public List<ItcTotalRecords> getAspErrorReport(
		...
		if (division != null && !division.isEmpty()) {
			if (divisionList != null && divisionList.size() > 0) {
				...
				buildQuery1.append(" AND HDR.DIVISION IN :divisionList");
			}
		}
		...
		String queryStr = AspErrorQueryString(buildQuery.toString(),
	...
	private ItcTotalRecords convertVerticalTotal(Object[] arr) {
		...
		obj.setjWDeliveryChallanDate(fmtDate(arr[12]));
		...
		obj.setItemAssessableAmount(appendDoubleDecimalDigit(arr[31]));
		...
		obj.setStateCessAdvaloremRate(appendDoubleDecimalDigit(arr[42]));
		obj.setStateCessAdvaloremAmount(appendDoubleDecimalDigit(arr[43]));
		obj.setStateCessSpecificRate(appendDoubleDecimalDigit(arr[44]));
		obj.setStateCessSpecificAmount(appendDoubleDecimalDigit(arr[45]));
	...
	private String AspErrorQueryString(String buildQuery, String buildQuery1) {
		return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| "
				...
				+ "TO_CHAR(ACTION_TYPE ),TO_CHAR(FI_YEAR),"
				+ "TO_CHAR(HDR.RETURN_PERIOD),TO_CHAR(HDR.SUPPLIER_GSTIN), "
				+ "TO_CHAR(DELIVERY_CHALLAN_NO),TO_CHAR(DELIVERY_CHALLAN_DATE),"
				+ "TO_CHAR(JW_DELIVERY_CHALLAN_NO),TO_CHAR(JW_DELIVERY_CHALLAN_DATE), "
				+ "TO_CHAR(GOODS_RECEIVING_DATE),TO_CHAR(INV_NUM ),"
				+ "TO_CHAR(INV_DATE),TO_CHAR(JW_GSTIN ),TO_CHAR(JW_STATE_CODE ),"
				+ "TO_CHAR(JW_WORKER_TYPE),TO_CHAR(JW_ID),TO_CHAR(JW_NAME), "
				+ "TO_CHAR(GOODS_TYPE),TO_CHAR(ITM_SER_NO),"
				+ "TO_CHAR(PRODUCT_DESC),TO_CHAR(PRODUCT_CODE),"
				+ "TO_CHAR(NATURE_OF_JW), TO_CHAR(ITM_HSNSAC ),"
				+ "TO_CHAR(ITM_UQC),TO_CHAR(ITM_QTY ),TO_CHAR(LOSSES_UQC),"
				+ "TO_CHAR(LOSSES_QTY), TO_CHAR(ITM.TAXABLE_VALUE )ITM_ACCESSABLE_AMT ,"
				...
				+ "TO_CHAR(STATE_CESS_RATE_SPECIFIC),TO_CHAR(ITM.STATE_CESS_AMT_SPECIFIC),"
				+ "TO_CHAR(ITM.TOTAL_VALUE), TO_CHAR(POSTING_DATE),"
				+ "TO_CHAR(USER_ID),TO_CHAR(COMPANY_CODE),TO_CHAR(SOURCE_IDENTIFIER), "
				+ "TO_CHAR(SOURCE_FILE_NAME) , TO_CHAR(PLANT_CODE ),"
				+ "TO_CHAR(DIVISION),TO_CHAR(PROFIT_CENTRE1),"
				+ "TO_CHAR(PROFIT_CENTRE2), TO_CHAR(ACCOUNTING_VOUCHER_NUM),"
				+ "TO_CHAR(ACCOUNTING_VOUCHER_DATE), TO_CHAR(USERDEFINED_FIELD1),"
				+ "TO_CHAR(USERDEFINED_FIELD2),TO_CHAR(USERDEFINED_FIELD3),"
				+ "TO_CHAR(USERDEFINED_FIELD4),TO_CHAR(USERDEFINED_FIELD5), "
				+ "TO_CHAR(USERDEFINED_FIELD6),TO_CHAR(USERDEFINED_FIELD7),"
				+ "TO_CHAR(USERDEFINED_FIELD8),TO_CHAR(USERDEFINED_FIELD9),"
				+ "TO_CHAR(USERDEFINED_FIELD10), FIL.CREATED_BY USERID, "
				+ "FIL.ID FILE_ID,FIL.FILE_NAME FILE_NAME,"
				...
				+ " '' AS GSTIN_ERROR_DESCRIPTION_ASP,TO_CHAR(HDR.QRETURN_PERIOD) "
				+ "FROM ITC04_HEADER "
				...
				+ "WHERE HDR.IS_DELETE=FALSE AND IS_ERROR=TRUE " + buildQuery
				+ " UNION ALL "
				...
				+ "IFNULL(HDR.INFORMATION_CODES,'') ||','|| "
				+ "IFNULL(ITM.INFORMATION_CODES,'')) AS INFO_ERROR_CODE_ASP, "
				+ " '' AS INFO_ERROR_DESCRIPTION_ASP, HDR.TABLE_NUMBER ,"
				+ "HDR.ACTION_TYPE ,HDR.FI_YEAR,HDR.RETURN_PERIOD,"
				+ "HDR.SUPPLIER_GSTIN, HDR.DELIVERY_CHALLAN_NO,"
				+ "HDR.DELIVERY_CHALLAN_DATE,HDR.JW_DELIVERY_CHALLAN_NO,"
				+ "HDR.JW_DELIVERY_CHALLAN_DATE, ITM.GOODS_RECEIVING_DATE,"
				+ "HDR.INV_NUM ,HDR.INV_DATE,HDR.JW_GSTIN ,HDR.JW_STATE_CODE ,"
				+ "HDR.JW_WORKER_TYPE,HDR.JW_ID,HDR.JW_NAME, "
				+ "ITM.GOODS_TYPE,ITM.ITM_SER_NO,ITM.PRODUCT_DESC,"
				+ "ITM.PRODUCT_CODE,ITM.NATURE_OF_JW, ITM_HSNSAC , "
				+ "ITM_UQC, ITM_QTY , LOSSES_UQC, LOSSES_QTY, "
				...
				+ "ITM.TOTAL_VALUE, HDR.POSTING_DATE,HDR.USER_ID,"
				+ "HDR.COMPANY_CODE,HDR.SOURCE_IDENTIFIER, HDR.SOURCE_FILE_NAME , "
				+ "ITM.PLANT_CODE ,HDR.DIVISION,"
				+ "HDR.PROFIT_CENTRE1,HDR.PROFIT_CENTRE2, "
				...
				+ "FROM ITC04_ERR_HEADER "
				+ "HDR INNER JOIN ITC04_ERR_ITEM ITM ON "
				+ "HDR.ID=ITM.DOC_HEADER_ID AND "
				+ "HDR.RETURN_PERIOD=ITM.RETURN_PERIOD "
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID "
				+ "WHERE HDR.IS_DELETE='false' AND HDR.IS_ERROR='true' "
				+ buildQuery1;
