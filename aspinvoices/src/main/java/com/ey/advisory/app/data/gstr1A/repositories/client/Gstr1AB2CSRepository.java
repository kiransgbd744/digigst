...
@Repository("Gstr1AB2CSRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr1AB2CSRepository
		extends JpaRepository<Gstr1AB2csDetailsEntity, Long>,
		JpaSpecificationExecutor<Gstr1AB2csDetailsEntity> {

	@Modifying
	@Query("UPDATE Gstr1AB2csDetailsEntity doc SET doc.isGstnError=true, "
			+ "doc.isSaved=false WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIserrorfalg(@Param("gstnBatchId") Long gstnBatchId);

	@Modifying
	@Query("UPDATE Gstr1AB2csDetailsEntity doc SET doc.isSaved=true, "
			+ "doc.isGstnError =false WHERE doc.gstnBatchId=:gstnBatchId")
	void updateIsSavedToGstn(@Param("gstnBatchId") Long gstnBatchId);

	@Modifying
	@Query("UPDATE Gstr1AB2csDetailsEntity doc SET doc.gstnBatchId=:gstnBatchId,"
	...
	@Query("SELECT COUNT(*) FROM Gstr1AB2csDetailsEntity "
			+ "WHERE sgstin=:gstin AND returnPeriod = :taxperiod ")
	public int gstinCount(@Param("gstin") String gstin,
			@Param("taxperiod") String taxperiod);

	@Modifying
	@Query("UPDATE Gstr1AB2csDetailsEntity b SET b.isDelete= TRUE "
	...
	@Query("UPDATE Gstr1AB2csDetailsEntity b SET b.isDelete= TRUE "
			+ "WHERE b.isDelete= FALSE AND  b.id IN (:id) ")
	public int UpdateId(@Param("id") Long id);

	@Query("SELECT doc.id FROM Gstr1AB2csDetailsEntity doc "
	...
	@Modifying
	@Query("UPDATE Gstr1AB2csDetailsEntity doc SET doc.isDelete=true,"
	 ...
	 * @Query("SELECT doc.id FROM Gstr1AB2csDetailsEntity doc " +
	 * "WHERE doc.invB2csKey IN (:docKeys) AND doc.isDelete = false ")
	 * List<Long> findActiveDocsByDocKeys(@Param("docKeys") List<String>
	 * docKeys);
	 * 
	 * 
	 * @Modifying
	 ...
	 * @Query("UPDATE Gstr1AB2csDetailsEntity doc SET doc.isDelete=true," +
	 * "doc.modifiedOn=:updatedDate,doc.modifiedBy=:updatedBy " +
	 * "WHERE doc.id IN (:ids) ") public void
	 * updateDuplicateDocDeletionByDocKeys(
	 * 
	 * @Param("ids") List<Long> ids,
	 * 
	 * @Param("updatedDate") LocalDateTime updatedDate,
	...
	@Query("SELECT COUNT(*) FROM Gstr1AB2csDetailsEntity  WHERE  "
