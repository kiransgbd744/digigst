 ...
 *
 *//*
public class EwayAvailCgstValidator
	...
	public List<ProcessingResult> validate(OutwardTransDocument document,
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			BigDecimal acgst = item.getAvailableCgst();
			BigDecimal cgst = item.getCgstAmount();
			if (acgst == null) {
				acgst = BigDecimal.ZERO;
			}
			if (cgst == null) {
				cgst = BigDecimal.ZERO;
			}
			if (acgst.compareTo(cgst) > 0) {

				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(GSTConstants.AvailableCgstamount);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						idx, errorLocations.toArray());
				errors.add(new ProcessingResult(APP_VALIDATION, "ER1106",
