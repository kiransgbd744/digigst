 ...
 *
 */
@Component("EInvManagmtInwardScreenDownloadDaoImpl")
@Slf4j
public class EInvManagmtInwardScreenDownloadDaoImpl
	...
	public List<EInvMangmntScreenDownloadResponseDto> getEInvMngmtScreendwnld(
		...
		criteria.getGstReturnsStatus().forEach(gstReturnStatus -> {
			if (notApplicable.equalsIgnoreCase(gstReturnStatus)) {
				buildQuery.append(" AND IS_PROCESSED = FALE");
				buildQuery.append(" AND IS_ERROR = FALE");
			}
			...
			if (aspProcessedNotSaved.equalsIgnoreCase(gstReturnStatus)) {
				buildQuery.append(" AND IS_PROCESSED = TRUE");
				buildQuery.append(" AND IS_SAVED_TO_GSTN = FALE");
			}
		...
		if (criteria.getProcessingStatus() != null
			...
			if (criteria.getProcessingStatus()
				...
				buildQuery.append(" AND HDR.RETURN_TYPE = :gstReturn");
			}
		} else {
			if (gstReturn != null) {
				buildQuery.append(" AND HDR.RETURN_TYPE = :gstReturn");
		...
		if (criteria.getProcessingStatus() != null
			...
			if (!criteria.getProcessingStatus()
					.equalsIgnoreCase(ProcessingStatus.ERROR.getStatus())) {
				if (gstReturn != null) {
					q.setParameter("gstReturn", gstReturn);
				}
			}
		} else {
			if (gstReturn != null) {
	...
	private EInvMangmntScreenDownloadResponseDto convertInwwardListing(
			Object[] arr) {
	...
	private String createEInvdownloadQueryString(String buildQuery) {
		...
		return "SELECT DISTINCT HDR.ID,HDR.CUST_GSTIN,HDR.DOC_TYPE,HDR.DOC_NUM,"
				+ "HDR.DOC_DATE,HDR.SUPPLIER_GSTIN,TRIM(', ' FROM "
				+ "IFNULL(HDR.ERROR_CODES,''||','|| IFNULL(ITM.ERROR_CODES,''))) "
				+ "AS ERROR_CODE_ASP FROM ANX_INWARD_DOC_HEADER HDR INNER JOIN "
				+ "ANX_INWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID AND "
				+ "HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD INNER JOIN "
				+ "GSTIN_INFO GIN ON HDR.CUST_GSTIN = GIN.GSTIN "
