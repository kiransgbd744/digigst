...
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AEcomSupAmdItemEntity;
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AEcomSupHeaderEntity;
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AEcomSupItemEntity;
 ...
 *
 */
@Component("Gstr1AEcomEcomAmdDataParserImpl")
@Slf4j
public class Gstr1AEcomEcomAmdDataParserImpl
		implements Gstr1AEcomEcomAmdDataParser {
	...
	public List<GetGstr1AEcomSupHeaderEntity> parseEcomData(
		...
		List<GetGstr1AEcomSupHeaderEntity> headerList = new ArrayList<>();
		...
		try {
			...
			for (B2BEcomInvoices eachInv : b2bEcomInv) {
				for (B2BInvoiceData eachInvData : eachInv.getB2bInvoiceData()) {
					GetGstr1AEcomSupHeaderEntity header = new GetGstr1AEcomSupHeaderEntity();
	...
	public List<GetGstr1AEcomSupAmdHeaderEntity> parseEcomAmdData(
		...
		Gson gson = GsonUtil.newSAPGsonInstance();
		JsonArray respObject = null;
		List<GetGstr1AEcomSupAmdHeaderEntity> headerList = new ArrayList<>();
		...
		try {
			...
			for (B2BEcomInvoices eachInv : b2bEcomInv) {
				for (B2BInvoiceData eachInvData : eachInv.getB2bInvoiceData()) {
					GetGstr1AEcomSupAmdHeaderEntity header = new GetGstr1AEcomSupAmdHeaderEntity();
	...
	private void setEcomTrans(GetGstr1AEcomSupHeaderEntity header,
		...
		List<GetGstr1AEcomSupItemEntity> lineItems = new ArrayList<>();
		...
		if (eachInvData.getLineItems() != null) {
			for (B2bLineItem b2cItems : eachInvData.getLineItems()) {
				...
				// New Item Entity
				GetGstr1AEcomSupItemEntity item = new GetGstr1AEcomSupItemEntity();
	...
	private void setEcomAmdTrans(GetGstr1AEcomSupAmdHeaderEntity header,
			B2BEcomInvoices eachInv, B2BInvoiceData eachInvData,
			Gstr1GetInvoicesReqDto dto, String type) {
		BigDecimal taxRate = BigDecimal.ZERO;
		BigDecimal taxValue = BigDecimal.ZERO;
		...
		BigDecimal sgstAmt = BigDecimal.ZERO;
		BigDecimal cessAmt = BigDecimal.ZERO;
		List<GetGstr1AEcomSupAmdItemEntity> lineItems = new ArrayList<>();
		...
		if (eachInvData.getLineItems() != null) {
			...
			for (B2bLineItem b2bAItems : eachInvData.getLineItems()) {
				...
				// New Item Entity
				GetGstr1AEcomSupAmdItemEntity item = new GetGstr1AEcomSupAmdItemEntity();
	...
	private void setEcomSumm(Gstr1GetInvoicesReqDto dto,
			B2CSInvoices b2csInvoices, GetGstr1AEcomSupHeaderEntity header,
			String type) {
		header.setGstin(dto.getGstin());
		header.setReturnPeriod(dto.getReturnPeriod());
		if (dto.getReturnPeriod() != null
	...
	private void setEcomAmdSumm(Gstr1GetInvoicesReqDto dto,
			B2CSInvoices eachInv, GetGstr1AEcomSupAmdHeaderEntity header,
			String type) {
