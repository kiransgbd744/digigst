 ...
 */
@Slf4j
@Component("Itc04SignAndFileServiceImpl")
public class Itc04SignAndFileServiceImpl implements Itc04SignAndFileService {
	...
	@Override
	public Pair<Boolean, String> getItc04GstnSummary(
			Itc04GetInvoicesReqDto itc04SummaryDto) {
		String gstin = itc04SummaryDto.getGstin();
		String taxPeriod = itc04SummaryDto.getReturnPeriod();
		String gstnResponse = null;
		try {
			APIResponse resp = getItc04Summary(taxPeriod, gstin);
			if (resp.isSuccess()) {
				...
				LOGGER.error(
						"we have received Error Response from GSTN"
								+ " for GETItc04Summ GSTIN {}, Taxperiod {}, Response is {} ",
						itc04SummaryDto.getGstin(),
						itc04SummaryDto.getReturnPeriod(), resp.getErrors());
				return new Pair<>(Boolean.FALSE,
						resp.getError().getErrorDesc());
			}

		} catch (Exception ex) {
			String msg = "Exception while GetItc04 api Call";
			LOGGER.error(msg, ex);
	...
	@Override
	public void updateItc04Tables(String taxperiod, String gstin, String signId,
		...
		try {
			...
			if (i == 0) {
				...
				returnStatusentity
						.setReturnType(APIConstants.ITC04_RETURN_TYPE);
			...
			String msg = "Exception occured in updateItc04Tables";
			LOGGER.error(msg, e);
	...
	private void saveOrUpdateGstnUserRequest(String gstin, String taxPeriod,
		...
		gstnUserRequestEntity = gstnUserRequestRepo
				.findTop1ByGstinAndTaxPeriodAndReturnTypeAndRequestTypeAndIsDeleteFalseOrderByIdDesc(
						gstin, taxPeriod, APIConstants.ITC04_RETURN_TYPE,
						"GET");
		Clob reqClob;
		try {
			...
			if (gstnUserRequestEntity == null) {
				...
				gstnUserRequestRepo.updateGstnResponse(reqClob, 1, gstin,
						taxPeriod, APIConstants.ITC04_RETURN_TYPE,
						LocalDateTime.now());
			}
		} catch (Exception e) {
	...
	public APIResponse getItc04Summary(String taxPeriod, String gstin) {
		...
		try {
			...
			String msg = String.format(
					...
					APIIdentifiers.ITC04_GETSUM, gstin, taxPeriod);
			LOGGER.error(msg, ex);
			throw ex;
		}
	...
	@Override
	public List<Gstr3BFilingDetailsDTO> getItc04FilingDetails(String gstin,
			String taxPeriod) {
		...
		try {
			List<SignAndFileEntity> entityList = signAndFileRepo
					.findByGstinAndTaxPeriodAndReturnTypeOrderByCreatedOnDesc(
							gstin, taxPeriod, APIConstants.ITC04.toUpperCase());
			...
			String msg = "Exception occured while getting itc04Filing data";
			LOGGER.error(msg, e);
			throw new AppException(msg, e);
		}
	...
	@Override
	public void downloadItc04ErrorResp(String id,
		...
		try {
			...
			if (opEntity.isPresent()) {
				...
				String errMsg = String.format(
						"GSTR1 Sign & File is not initiated for id %s", id);
				LOGGER.error(errMsg);
	...
	private void writeMsgToTxt(SignAndFileEntity entity,
		...
		try {
			...
			String fileName = "ITC04FilingErrors_" + entity.getGstin() + "_"
					+ entity.getTaxPeriod();
			...
			String msg = "Error while writing Itc04 errors to txt";
			LOGGER.error(msg, e);
			throw new AppException(msg, e);
		} finally {
				...
				try {
					...
					String msg = String.format(
							"Failed to remove the temp "
									+ "directory created for Itc04 errors: '%s'. This will "
	...
	public static File createTempDir() throws IOException {
		return Files.createTempDirectory("Itc04Errors").toFile();
	}
}
