...
@Slf4j
@Component("Gstr1B2BASectionReportHandler")
public class Gstr1B2BASectionReportHandler {

	@Autowired
	@Qualifier("Gstr1B2BASectionReportServiceImpl")
		...
		try {
			...
			if (workbook != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2BA template file name is : {}",
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						...
						+ "_B2BA_SECTION9A" + ".xlsx";

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2BA file name is : {}", fileName);
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Workbook has been generated successfully with the"
									+ " Gstr1 B2BA response list in the directory : {}",
				...
				if (id != null) {
					try {
						...
						String msg = String
								.format("Gstr1B2BASectionReportHandler - "
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2BA No Data found for report id : {} ",
							id);
				}
			...
			String msg = String.format("Gstr1B2BASectionReportHandler - "
