...
import com.ey.advisory.app.downloadreports.DownloadReportsConstant;
	...
	@Component("DistributionProcessedInfoReportsDaoImpl")
	public class DistributionProcessedInfoReportsDaoImpl implements DistributionReportsDao {

			private static final Logger LOGGER = LoggerFactory
					.getLogger(DistributionProcessedInfoReportsDaoImpl.class);
			...
			private String createProcessedQueryString(String buildQuery) {
				return "SELECT TAX_PERIOD,ISD_GSTIN,CUST_GSTIN,"
						...
						+ "ORG_DOC_NUM,ORG_DOC_DATE,ORG_CR_NUM,"
						+ "ORG_CR_DATE,ELIGIBLE_INDICATOR,"
						+ "IGST_AMT_AS_IGST ,IGST_AMT_AS_SGST,"
						+ "IGST_AMT_AS_CGST,SGST_AMT_AS_SGST,"
						+ "SGST_AMT_AS_IGST, CGST_AMT_AS_CGST,"
						+ "CGST_AMT_AS_IGST,CESS_AMT,"
						+ "(CASE WHEN DOC_TYPE = 'INV' "
						+ "AND ELIGIBLE_INDICATOR IN('IS','E') "
						+ "THEN 'DISTRIBUTION_INV_ELIGIBLE' "
						+ "WHEN DOC_TYPE = 'INV' AND "
						...
						+ "WHEN DOC_TYPE = 'RNV' AND "
						+ "ELIGIBLE_INDICATOR IN('NO','IE') "
						+ "THEN 'RE_DISTRIBUTION_INV_INELIGIBLE' "
						...
						+ "WHEN DOC_TYPE = 'RCR' AND "
						+ "ELIGIBLE_INDICATOR IN('NO','IE') "
						+ "THEN 'RE_DISTRIBUTION_CR_INELIGIBLE' END) "
						+ "CATEGORY, TRIM(', ' FROM "
						+ "IFNULL(GFN.INFO_ERROR_CODE_ASP,'')) "
						+ "INFO_CODE_ASP, TRIM(', ' FROM "
						+ "IFNULL(GFN.INFO_ERROR_DESCRIPTION_ASP,'')) "
						+ "INFO_DESCRIPTION_ASP, CASE "
						+ "WHEN IS_DELETE = FALSE "
						+ "THEN 'ACTIVE' WHEN IS_DELETE = TRUE "
						+ "THEN 'INACTIVE' END RECORD_STATUS "
						+ "FROM GSTR6_ISD_DISTRIBUTION GID LEFT JOIN "
