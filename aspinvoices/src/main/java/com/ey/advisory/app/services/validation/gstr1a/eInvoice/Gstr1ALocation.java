 ...
 *
 */
@Component("Gstr1ALocation")
public class Gstr1ALocation implements DocRulesValidator<Gstr1AOutwardTransDocument> {
					...
					if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
						boolean isAttrValid = onboardAttributesChecking
								.isAttrValid(entityAtValMap,
										OnboardingConstant.LOCATION, loc,
										document.getEntityId());
					...
					if (OnboardingConstant.AT_O.equalsIgnoreCase(atOutward)) {
						...
						if (!isAttrValid) {
							...
							TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
									idx, errorLocations.toArray());
							errors.add(new ProcessingResult(APP_VALIDATION,
									ProcessingResultType.INFO, "IN0013",
									" Location is not as per master "
											+ "provided during On-Boarding.",
									location));
			...
			if (loc != null && !loc.isEmpty()) {
				...
				if (CONFIG_PARAM_OUTWARD_ANS_KEY_ID.A.name()
					...
					boolean isAttrUserValid = onboardAttributesChecking
							.isAttrValid(OnboardingConstant.LOCATION, loc,
									document.getEntityId(),
					...
					if (!isAttrUserValid) {
						...
						TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
								idx, errorLocations.toArray());
						errors.add(
								new ProcessingResult(APP_VALIDATION, "ER0156",
