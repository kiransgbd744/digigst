...
@Component("gstr1HSNSummaryDataAtGstnImpl")
@Slf4j
public class Gstr1HSNSummaryDataAtGstnImpl
		implements Gstr1HSNSummaryDataAtGstn {
	...
	public Long gstr1HSNSummaryDataAtGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.HSN.equals(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_HSN_SUMMARY, para1, para2);
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1HsnSuccessHandler", "Gstr1GetCommonFailureHandler",
				jsonReq);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
