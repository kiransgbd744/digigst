 ...
 */
@Slf4j
@Service("saveCdnurCdnuraProcessImpl")
public class Gstr1SaveCdnurCdnuraProcessImpl implements Gstr1SectionWiseSaveBatchProcess {
...
String origin = batchDto.getOrigin();
	...
	private SaveToGstnBatchRefIds process(
		...
		try {
			...
			if (resp.getResponse() != null) {
				...
				/**
				 * update the RefId and txnId in Gstr1_save_batch table
				 */
				if (returnType
						.equalsIgnoreCase(APIConstants.GSTR1A.toUpperCase())) {
	...
	private List<Triplet<List<Long>, SaveGstr1, String>> batchChunk(
		...
		List<CDNURInvoices> invListList = null;
		if (APIConstants.CDNUR.equals(section)) {
			invListList = gstr1.getCdnurInvoice();
		} else {
			invListList = gstr1.getCdnuraInvoice();
		}
		int invSize = invListList.size();
		List<CDNURInvoices> invList1 = invListList.subList(0, invSize / 2);
		List<CDNURInvoices> invList2 = invListList.subList(invSize / 2,
				invSize);
	...
	private Pair<List<Long>, SaveGstr1> setChunkInvData(SaveGstr1 gstr1,
			List<CDNURInvoices> invList, String section) {
		...
		if (APIConstants.CDNUR.equals(section)) {
			gstr.setCdnurInvoice(gstr1.getCdnurInvoice());
		} else {
			gstr.setCdnuraInvoice(gstr1.getCdnuraInvoice());
		}
		for (CDNURInvoices data : invList) {
