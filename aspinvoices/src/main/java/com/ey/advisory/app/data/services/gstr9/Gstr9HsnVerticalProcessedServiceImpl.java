...
import com.ey.advisory.app.services.reports.Gstr7VerticalProcessedServiceImpl;
 ...
 *
 */
@Service("Gstr9HsnVerticalProcessedServiceImpl")
public class Gstr9HsnVerticalProcessedServiceImpl implements Gstr7VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr9HsnVerticalProcessedServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr9HsnVerticalProcessedReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria, PageRequest pageReq) {
		...
		workbook = createWorkbookWithExcelTemplate("ReportTemplates", "GSTR9_Processed_HSN_Records.xlsx");
        
		if(LOGGER.isDebugEnabled()){
		LOGGER.debug("gstr9 hsn data response" + responseFromView);
		}

		if (responseFromView != null && responseFromView.size() > 0) {
			String[] invoiceHeaders = commonUtility.getProp("gstr9.asp.process.hsn.report.headers").split(",");
