...
import com.ey.advisory.app.data.views.client.Anx1GSTN3FErrorReportsView;
...
@Component("Anx1GSTN3FErrorReportDaoImpl")
public class Anx1GSTN3FErrorReportDaoImpl implements Anx1GstnErrorDao {
	...
	public List<Object> getAnx1GstnErrorReport(SearchCriteria criteria) {
		...
		String queryStr = GSTN3FErrorQueryString(buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertGSTN3FError(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1GSTN3FErrorReportsView convertGSTN3FError(Object[] arr) {
		Anx1GSTN3FErrorReportsView obj = new Anx1GSTN3FErrorReportsView();
		...
		obj.setAutoPopltToRefund(arr[10] != null ? arr[10].toString() : null);
	...
	private String GSTN3FErrorQueryString(String buildQuery) {
		return "SELECT HT.SUPPLIER_GSTIN,HT.CUST_GSTIN,DOC_TYPE,"
				...
				+ "SUM(DOC_AMT) INVVAL,HT.POS,HT.AUTOPOPULATE_TO_REFUND,"
				...
				+ "AND UCASE(HT.AN_TAX_DOC_TYPE) = UCASE('SEZWT') "
				...
				+ "HT.DOC_DATE,ITM_HSNSAC,TAX_RATE,"
				+ "HT.POS,HT.AUTOPOPULATE_TO_REFUND,"
