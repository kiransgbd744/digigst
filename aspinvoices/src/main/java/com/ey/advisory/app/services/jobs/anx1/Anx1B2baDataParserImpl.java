...
import com.ey.advisory.app.data.entities.simplified.client.GetAnx1B2baInvoicesItemEntity;
import com.ey.advisory.app.docs.dto.anx1.Anx1B2bItemDetails;
import com.ey.advisory.app.docs.dto.anx1.Anx1B2baData;
...
@Service("anx1B2baDataParserImpl")
@Slf4j
public class Anx1B2baDataParserImpl implements Anx1B2baDataParser {
	...
	@Override
	public Set<GetAnx1B2baInvoicesHeaderEntity> parseB2baData(
			Anx1GetInvoicesReqDto dto, String apiResp) {
		Gson gson = GsonUtil.newSAPGsonInstance();
		Set<GetAnx1B2baInvoicesHeaderEntity> invoiceList = new HashSet<>();
		...
		try {
			...
			Type listType = new TypeToken<List<Anx1B2baData>>() {
			}.getType();
			List<Anx1B2baData> baseEntity = gson.fromJson(respObject, listType);
			for (Anx1B2baData eachInv : baseEntity) {
				for (Anx1B2baDocumentData eachInvData : eachInv.getDocs()) {
					List<GetAnx1B2baInvoicesItemEntity> itemList = new ArrayList<>();
					GetAnx1B2baInvoicesHeaderEntity invoice = setInvoiceData(
			...
			String msg = "failed to parse Anx1 B2BA response";
			LOGGER.error(msg, ex);
	...
	private GetAnx1B2baInvoicesHeaderEntity setInvoiceData(Anx1B2baData eachInv,
			Anx1B2baDocumentData eachInvData, Anx1GetInvoicesReqDto dto,
			Long batchId) {
		GetAnx1B2baInvoicesHeaderEntity invoice = new GetAnx1B2baInvoicesHeaderEntity();
		...
		invoice.setB2baBatchId(batchId);
	...
	private GetAnx1B2baInvoicesItemEntity setItemData(
			Anx1B2bItemDetails lineItem) {
		GetAnx1B2baInvoicesItemEntity item = new GetAnx1B2baInvoicesItemEntity();
	...
	private void setDockey(GetAnx1B2baInvoicesHeaderEntity invoice,
