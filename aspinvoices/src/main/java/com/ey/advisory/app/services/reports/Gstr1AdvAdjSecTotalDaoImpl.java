 ...
 * 
 */
@Component("Gstr1AdvAdjSecTotalDaoImpl")
public class Gstr1AdvAdjSecTotalDaoImpl
	...
	public List<Object> getGstr1AdjSavableReports(SearchCriteria criteria) {
		...
		if (APIConstants.GSTR1A.equalsIgnoreCase(request.getReturnType())) {
			queryStr = createAdjSavableTotalQueryStringForGstr1a(
					buildQueryadj.toString(), buildQueryHdr.toString());

		} else {
			queryStr = createAdjSavableTotalQueryString(
					buildQueryadj.toString(), buildQueryHdr.toString());
	...
	private Gstr1AspVerticalAdvAdjDto convertAdjtotal(Object[] arr) {
		...
		if (arr[33] != null) {
			String timestamp = removeQuotes(arr[33].toString());
	...
	private static String createAdjSavableTotalQueryString(String buildQueryadj,
		...
		return "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,(CASE WHEN "
				+ "TRAN_TYPE IN ('ZL65','L65') THEN 'L65' "
				+ "WHEN (TRAN_TYPE IN ('Z','N','') OR TRAN_TYPE IS NULL) "
				+ "THEN 'N' END) TRAN_TYPE,MAX(MONTH)MONTH, "
				...
				+ "SUPPLIER_GSTIN,RETURN_PERIOD,(CASE WHEN "
				+ "TRAN_TYPE IN ('ZL65','L65') THEN 'L65' "
				+ "WHEN (TRAN_TYPE IN ('Z','N','') OR TRAN_TYPE IS NULL) "
				+ "THEN 'N' END),NEW_POS  ";

	}
	private static String createAdjSavableTotalQueryStringForGstr1a(String buildQueryadj,
			String buildQueryHdr) {
