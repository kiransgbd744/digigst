...
@Service("Gstr9InOutwardProcessedInfoReportsServiceImpl")
@Slf4j
public class Gstr9InOutwardProcessedInfoReportsServiceImpl
	...
	@Autowired
	@Qualifier("Gstr9InOutwardProcessedInfoReportsDaoImpl")
	...
	public Workbook downloadReports(SearchCriteria criteria,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Gstr9 Inoutward processedinfo response"
					+ responseFromView);
		}
		...
		if (responseFromView != null && !responseFromView.isEmpty()) {
			...
			if ((GSTConstants.INWARD_OUTWARD)
				...
				String[] invoiceHeaders = commonUtility
						.getProp(
								"gstr9.inoutward.vertical.processinfo.report.headers")
						.split(",");
				if (LOGGER.isDebugEnabled()) {
				...
				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"GSTR_9_Inward_Outward_ProcessedInfoReport.xlsx");
				if (LOGGER.isDebugEnabled()) {
