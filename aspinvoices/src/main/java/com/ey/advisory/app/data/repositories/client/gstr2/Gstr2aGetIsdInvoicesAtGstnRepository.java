 ...
 *
 */
@Repository("Gstr2aGetIsdInvoicesAtGstnRepository")
@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
public interface Gstr2aGetIsdInvoicesAtGstnRepository
        extends CrudRepository<GetGstr2aIsdInvoicesHeaderEntity, Long> {

    // Curently isDelete is not added in table
    @Modifying
    @Query("UPDATE GetGstr2aIsdInvoicesHeaderEntity b SET b.isDelete = true,"
    ...
    @Query("from GetGstr2aIsdInvoicesHeaderEntity where gstin=:sgstin and cgstin=:cgstin and documentDate=:invDate and documentNumber=:invNum and isdDocumentType=:invType")
    List<GetGstr2aIsdInvoicesHeaderEntity> findByInvoiceKey(
    ...
    @Query("SELECT COUNT(*) FROM GetGstr2aIsdInvoicesHeaderEntity "
            + "WHERE cgstin=:gstin AND returnPeriod = :taxperiod and isDelete =false")
    public int gstinCount(@Param("gstin") String gstin,
            @Param("taxperiod") String taxperiod);

    @Modifying
    @Query("UPDATE GetGstr2aIsdInvoicesHeaderEntity b SET b.isDelete = true, b.modifiedOn=:modifiedOn WHERE"
