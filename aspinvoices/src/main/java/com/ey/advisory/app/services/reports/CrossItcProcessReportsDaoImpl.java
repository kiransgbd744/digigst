 ...
 *
 */
@Component("CrossItcProcessReportsDaoImpl")
@Slf4j
public class CrossItcProcessReportsDaoImpl implements Gstr1VerticalReportsDao {
	...
	private String createVerticalErrorQueryString(String buildQuery) {
		...
		if ((GSTConstants.CROSS_ITC).equalsIgnoreCase(fileType)) {
			...
			builder.append("FROM  CROSS_ITC_PROCESSED HI LEFT OUTER JOIN  ");
			...
			builder.append(
					"AND ERR.FILE_ID = FIL.ID  AND ERR.INV_KEY=HI.DOC_KEY  WHERE ");
