 ...
 *
 */
@Service("Anx1VerticalTotalReportsServiceImpl")
public class Anx1VerticalTotalReportsServiceImpl
		implements Anx1VerticalReportsService {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1VerticalTotalReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Anx1VerticalTotalReportsDaoImpl")
	private Anx1VerticalReportsDao anx1VerticalOutwardReportsDao;
	...
	public Workbook findReports(SearchCriteria criteria, PageRequest pageReq) {
		...
		responseFromView = anx1VerticalOutwardReportsDao
				.getVerticalReports(request);

		LOGGER.debug("vertical total response" + responseFromView);
		...
		if (responseFromView != null && responseFromView.size() > 0) {
			...
			if ((DownloadReportsConstant.B2C)
				...
				String[] invoiceHeaders = commonUtility
						.getProp("anx1.vertical.b2c.total.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_B2CVerticalTotal.xlsx");
				...
				errorDumpCells.importCustomObjects(responseFromView,
					...
					.equalsIgnoreCase(request.getFileType())) {
				String[] invoiceHeaders = commonUtility
						.getProp("anx1.vertical.3h3i.total.report.headers")
						.split(",");
				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_3H3IVerticalTotal.xlsx");
				...
				errorDumpCells.importCustomObjects(responseFromView,
					...
					.equalsIgnoreCase(request.getFileType())) {
				String[] invoiceHeaders = commonUtility
						.getProp("anx1.vertical.table4.total.report.headers")
						.split(",");

				LOGGER.debug("workbook properties reading starting ");

				workbook = createWorkbookWithExcelTemplate("ReportTemplates",
						"Anx1_Table4VerticalTotal.xlsx");
				LOGGER.debug("workbook reading ending");
