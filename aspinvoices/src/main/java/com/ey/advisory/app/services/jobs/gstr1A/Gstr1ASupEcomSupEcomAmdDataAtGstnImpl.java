 ...
 *
 */
@Component("Gstr1ASupEcomSupEcomAmdDataAtGstnImpl")
@Slf4j
public class Gstr1ASupEcomSupEcomAmdDataAtGstnImpl
		implements Gstr1ASupEcomSupEcomAmdDataAtGstn {
	...
	public Long findSupEcomDataATGstn(Gstr1GetInvoicesReqDto dto,
		...
		if (APIConstants.SUPECO.equalsIgnoreCase(type)) {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1A_GET_SUPECO, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_SUPECO);
			}
		} else {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1A_GET_SUPECO_AMD, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1A_GET_SUPECO_AMD);
			}
		...
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1ASupEcomSupEcomASuccessHandler",
