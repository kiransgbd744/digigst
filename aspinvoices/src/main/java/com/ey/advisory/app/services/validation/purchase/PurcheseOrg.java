 ...
 *
 */
public class PurcheseOrg implements DocRulesValidator<InwardTransDocument> {
	...
	public List<ProcessingResult> validate(InwardTransDocument document,
		...
		Map<EntityAtConfigKey, Map<Long, String>> entAtConfMap = document
				...
				String po = document.getPurchaseOrganization();
				...
				EntityAtConfigKey entityAtConfigKey = new EntityAtConfigKey(
						document.getEntityId(), OnboardingConstant.PO);
					...
					if (OnboardingConstant.AT_M.equalsIgnoreCase(atOutward)) {
						...
						if(po==null || po.isEmpty()){
							...
							errors.add(new ProcessingResult(APP_VALIDATION,
									"ER1125",
									"Purchase Organisation not as per "
						...
						if (!isAttrValid) {
								...
								TransDocProcessingResultLoc location 
								      = new TransDocProcessingResultLoc(
										null, errorLocations.toArray());
								errors.add(new ProcessingResult(APP_VALIDATION,
										ProcessingResultType.INFO, "IN1007",
										"Purchase Organisation not as per "
										+ "configurable parameters "
										+ "selected by user.",
										location));
          ...
          if(po!=null && !po.isEmpty()){
					...
					if(!isAttrUserValid){
						...
						errors.add(new ProcessingResult(APP_VALIDATION,
								 "ER1141",
								"Purchase Organisation is not as per "
								+ "user access level"
