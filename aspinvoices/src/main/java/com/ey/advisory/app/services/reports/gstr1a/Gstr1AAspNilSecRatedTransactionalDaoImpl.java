...
import com.google.common.base.Strings;
	...
	@Component("Gstr1AAspNilSecRatedTransactionalDaoImpl")
	public class Gstr1AAspNilSecRatedTransactionalDaoImpl
			implements Gstr1AOutwardVerticalProcessNilDao {

		private static final Logger LOGGER = LoggerFactory
				.getLogger(Gstr1AAspNilSecRatedTransactionalDaoImpl.class);
		...
		public List<Object> getGstr1RSReports(SearchCriteria criteria) {
			...
			if (ProfitCenter != null && !ProfitCenter.isEmpty()) {
				if (pcList != null && pcList.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.PROFIT_CENTRE IN :pcList");
				}
			}
			if (plant != null && !plant.isEmpty()) {
				if (plantList != null && plantList.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.PLANT_CODE IN :plantList");
				}
			}
			...
			if (division != null && !division.isEmpty()) {
				if (divisionList != null && divisionList.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.DIVISION IN :divisionList");
				}
			}
			if (location != null && !location.isEmpty()) {
				if (locationList != null && locationList.size() > 0) {
					buildQueryheader
					.append(" AND HDR.LOCATION IN :locationList");
				}
			}
			if (ud1 != null && !ud1.isEmpty()) {
				if (ud1List != null && ud1List.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.USERACCESS1 IN :ud1List");
				}
			}
			if (ud2 != null && !ud2.isEmpty()) {
				if (ud2List != null && ud2List.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.USERACCESS2 IN :ud2List");
				}
			}
			if (ud3 != null && !ud3.isEmpty()) {
				if (ud3List != null && ud3List.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.USERACCESS3 IN :ud3List");
				}
			}
			if (ud4 != null && !ud4.isEmpty()) {
				if (ud4List != null && ud4List.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.USERACCESS4 IN :ud4List");
				}
			}
			if (ud5 != null && !ud5.isEmpty()) {
				if (ud5List != null && ud5List.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.USERACCESS5 IN :ud5List");
				}
			}
			if (ud6 != null && !ud6.isEmpty()) {
				if (ud6List != null && ud6List.size() > 0) {
					...
					buildQueryheader
					.append(" AND HDR.USERACCESS6 IN :ud6List");
				}
			}

			if (taxPeriodFrom != null && taxPeriodTo != null) {
				buildQueryheader.append(" AND HDR.DERIVED_RET_PERIOD BETWEEN ");
				buildQueryheader.append(":taxPeriodFrom AND :taxPeriodTo");
...
//					.findAnswerForMultiSupplyType();
		...
		private static String createApiProcessedQueryString(String buildQueryheader, String multiSupplyTypeAns) {
				...
				+ "when HDR.IRN_STATUS = 3 then 'Pending' when HDR.IRN_STATUS = 4 "
				+ "then 'Generation Error' when HDR.IRN_STATUS = 5 "
				+ "then 'Generated' when HDR.IRN_STATUS = 6 then 'Cancelled' "
				...
				+ "then 'Part A Generated' when HDR.EWB_STATUS = 5 then 'EWB Active' "
				+ "when HDR.EWB_STATUS = 6 then 'Cancelled' when HDR.EWB_STATUS = 7 "
				+ "then 'Discarded' when HDR.EWB_STATUS = 8 then 'Rejected' "
				+ "when HDR.EWB_STATUS = 9 then 'Expired' when HDR.EWB_STATUS = 10 "
				+ "then 'Pushed to NIC' when HDR.EWB_STATUS = 11 then 'ASP Error' "
				+ "WHEN HDR.EWB_STATUS = 12 then 'Not_Opted' end as EWBPartAStatusA, "
				+ "TO_CHAR(EWB.VALID_UPTO,'DD-MM-YYYY HH24:MI:SS') AS EWBValidUpTo, "
				+ "HDR.EWAY_BILL_NUM AS EWBNO, CAST(HDR.EWAY_BILL_DATE AS DATE) "
				...
				+ "AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE AS GSTIN_REFID_DATE_TIME, "
				+ "NULL AS GSTIN_ERROR_CODE, NULL AS GSTIN_ERROR_DESCRIPTION_ASP, "
				+ "HDR.RETURN_TYPE AS RET_TYPE, HDR.TABLE_SECTION AS TABLE_NUMBER, "
				...
				+ "HDR.SUPP_BUILDING_NAME AS SUPPLIERADDRESS2,HDR.SUPP_LOCATION, "
				...
				+ "HDR.CUST_PINCODE,HDR.BILL_TO_STATE, HDR.POS,HDR.CUST_PHONE,"
				+ "HDR.CUST_EMAIL,HDR.DISPATCHER_GSTIN, HDR.DISPATCHER_TRADE_NAME,"
				+ "HDR.DISPATCHER_BUILDING_NUM, HDR.DISPATCHER_BUILDING_NAME,"
				+ "HDR.DISPATCHER_LOCATION, HDR.DISPATCHER_PINCODE,"
				+ "HDR.DISPATCHER_STATE_CODE ,HDR.SHIP_TO_GSTIN ,"
				+ "HDR.SHIP_TO_TRADE_NAME, HDR.SHIP_TO_LEGAL_NAME,"
				+ "HDR.SHIP_TO_BUILDING_NUM, HDR.SHIP_TO_BUILDING_NAME,"
				+ "HDR.SHIP_TO_LOCATION, ITM.SHIP_TO_PINCODE,ITM.SHIP_TO_STATE,"
				...
				+ "HDR.INFO_ERROR_CODE AS EWB_INFO_CODE, HDR.INFO_ERROR_MSG "
				+ "AS EWB_INFO_ERROR_MSG, HDR.EWB_NO_RESP AS EWB_NUM, "
				...
				+ "LEFT OUTER JOIN EINV_MASTER EINV ON "
				+ "HDR.IRN_RESPONSE = EINV.IRN LEFT OUTER JOIN "
				...
				+ " AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE IN ('NILEXTNON') ";
