 ...
 *
 */
@Service("InwardDocSave")
public class InwardDocSave {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(InwardDocSave.class);
	...
	@Autowired
	@Qualifier("InwardFieldTruncation")
	private InwardFieldTruncation inwardFieldTruncation;
	...
	public void convertCalcConfigAndSetValues(
		...
		documents.forEach(document -> {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("cgstin " + cgstin + " - entityId " + entityId);
			}
	 ...
	 */
	/*
	 * private void calculateAndSetLineItemAmount(InwardTransDocLineItem item) {
	 * 
	 * BigDecimal lineItemAmt = BigDecimal.ZERO;
	 * 
	 * if (item.getTaxableValue() != null) { lineItemAmt =
	 * lineItemAmt.add(item.getTaxableValue()); } if (item.getOtherValues() !=
	 * null) { lineItemAmt = lineItemAmt.add(item.getOtherValues()); } if
	 * (item.getStateCessAmount() != null) { lineItemAmt =
	 * lineItemAmt.add(item.getStateCessAmount()); } if (item.getIgstAmount() !=
	 * null) { lineItemAmt = lineItemAmt.add(item.getIgstAmount()); } if
	 * (item.getCgstAmount() != null) { lineItemAmt =
	 * lineItemAmt.add(item.getCgstAmount()); } if (item.getSgstAmount() !=
	 * null) { lineItemAmt = lineItemAmt.add(item.getSgstAmount()); } if
	 * (item.getCessAmountSpecific() != null) { lineItemAmt =
	 * lineItemAmt.add(item.getCessAmountSpecific()); } if
	 * (item.getCessAmountAdvalorem() != null) { lineItemAmt =
	 * lineItemAmt.add(item.getCessAmountAdvalorem()); }
	 * item.setLineItemAmt(lineItemAmt); }
	 */
	...
	public List<InwardTransDocument> saveDocs(List<InwardTransDocument> docs,
		...
		docs.forEach(document -> {
			...
			inwardFieldTruncation.truncateHeaderFields(document);
			...
			if (null != document.getLineItems()) {
				document.getLineItems().forEach(item -> {
					...
					inwardFieldTruncation.truncateLineItemFields(item);
	...
	private void setHeaderValuesToItem(InwardTransDocument document,
		...
		if (document.getDataOriginTypeCode() != null
				&& (GSTConstants.DataOriginTypeCodes.ERP_API
						.getDataOriginTypeCode()
						.equalsIgnoreCase(document.getDataOriginTypeCode())
