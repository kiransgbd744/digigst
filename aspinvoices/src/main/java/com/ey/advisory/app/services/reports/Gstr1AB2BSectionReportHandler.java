...
@Slf4j
@Component("Gstr1AB2BSectionReportHandler")
public class Gstr1AB2BSectionReportHandler {

	@Autowired
	@Qualifier("Gstr1B2BSectionReportServiceImpl")
		...
		try {
			...
			if (workbook != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2B template file name is : {}",
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						...
						+ "_B2B_SECTION4_SECTION6B_SECTION6C" + ".xlsx";

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2B file name is : {}", fileName);
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Workbook has been generated successfully with the"
									+ " Gstr1 B2B response list in the directory : {}",
				...
				if (id != null) {
					try {
						...
						String msg = String
								.format("Gstr1B2BSectionReportHandler - "
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2B No Data found for report id : {} ",
							id);
				}
			...
			String msg = String.format("Gstr1B2BSectionReportHandler - "
					+ "Exception occured while "
