 ...
 *
 */
@Component("GetGstr2XSummarySaveStatusService")
public class GetGstr2XSummarySaveStatusService {

	@Autowired
	@Qualifier("GetGstr2XDetailStatusFetchDaoImpl")
	private GetGstr2XDetailStatusFetchDao gstr2xDetailStatusFetchDao;

	@Autowired
	@Qualifier("ProcessedRecordsCommonSecParam")
	private ProcessedRecordsCommonSecParam processedRecordsCommonSecParam;

	public List<Gstr1SummarySaveStatusRespDto> findByCriteria(Gstr1SummarySaveStatusReqDto criteria) throws Exception {
		Gstr1SummarySaveStatusReqDto reqDto = processedRecordsCommonSecParam.setgstr2DataSecuritySearchParams(criteria);
		...
		List<Object[]> savedDataList = gstr2xDetailStatusFetchDao.getDataUploadedStatusDetails(GstinList,
				criteria.getTaxPeriod(),criteria.getType());
		if (!savedDataList.isEmpty() && savedDataList.size() > 0) {
			for (Object[] data : savedDataList) {
				...
				if (data[1] == null) {
					...
					LocalDateTime localDateTime = EYDateUtil.toISTDateTimeFromUTC(dt);
