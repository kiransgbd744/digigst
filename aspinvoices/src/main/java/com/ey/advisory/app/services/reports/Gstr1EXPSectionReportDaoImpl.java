...
@Component("Gstr1EXPSectionReportDaoImpl")
public class Gstr1EXPSectionReportDaoImpl
		implements Gstr1ReviewSummaryCustomizedReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1EXPSectionReportDaoImpl.class);
	...
	private String createApiProcessedQueryString(String buildHeader) {
		...
		return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| "
				...
				+ "HDR.TAX_DOC_TYPE AS DATA_CATEGORY,ITM.ITM_UQC,ITM.ITM_QTY, CASE WHEN PSD.USER_RESPONSE='S' THEN 'S-Save to GSTN' WHEN PSD.USER_RESPONSE='N' THEN 'N-Do Not save to GSTN'  ELSE '' END AS EINVVSGSTR1RESPONSE "
				...
				+ "ON HDR.IRN_RESPONSE = EINV.IRN  LEFT OUTER JOIN TBL_EINV_RECON_RESP_PSD PSD ON PSD.DOC_HEADER_ID = ITM.DOC_HEADER_ID AND PSD.IS_DELETE=FALSE AND PSD.IS_PROCESSED=TRUE LEFT OUTER JOIN "
				...
				+ "AND HDR.RETURN_TYPE='GSTR1' AND HDR.TAX_DOC_TYPE IN ('EXPORTS') "
				+ buildHeader + " ORDER BY DOC_NUM,ITM.ITM_NO ";
	}
/*public static void main(String[] args) {
	String s="2023-10-10 06:28:37.637";
	...
	private String createApiProcessedQueryStringForGstr1a(String buildHeader) {
		...
		return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| "
				...
				+ "WHERE HDR.ASP_INVOICE_STATUS = 2 "
				+ "AND COMPLIANCE_APPLICABLE=TRUE AND HDR.IS_DELETE=FALSE "
				+ "AND HDR.RETURN_TYPE='GSTR1A' AND HDR.TAX_DOC_TYPE IN ('EXPORTS') "
				+ buildHeader + " ORDER BY DOC_NUM,ITM.ITM_NO ";
