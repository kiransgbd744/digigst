...
@Slf4j
@Component("Gstr1B2CSASectionReportHandler")
public class Gstr1B2CSASectionReportHandler {

	@Autowired
	@Qualifier("Gstr1B2CSASectionReportServiceImpl")
	...
	public Workbook downloadRSProcessedReport(
		...
		try {
			...
			}if (criteria.getReturnType().equalsIgnoreCase(APIConstants.GSTR1A)) {
			...
			if (workbook != null) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2CSA template file name is : {}",
				...
				fileName = tempDir.getAbsolutePath() + File.separator
						...
						+ "_B2CSA_SECTION10" + ".xlsx";

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2CSA file name is : {}", fileName);
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"Workbook has been generated successfully with the"
									+ " Gstr1 B2CSA response list in the directory : {}",
				...
				if (id != null) {
					try {
						...
						String msg = String
								.format("Gstr1B2CSASectionReportHandler - "
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Gstr1 B2CSA No Data found for report id : {} ",
							id);
				}
			...
			String msg = String.format("Gstr1B2CSASectionReportHandler - "
					+ "Exception occured while " + "invoking report service, %s",
					e);
			LOGGER.error(msg);
			fileStatusDownloadReportRepo.updateStatus(id,
