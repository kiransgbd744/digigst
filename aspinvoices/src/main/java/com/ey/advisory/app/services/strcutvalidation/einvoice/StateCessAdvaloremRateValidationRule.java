...
public class StateCessAdvaloremRateValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isDecimal(obj.toString().trim())) {
			...
			errorLocations.add(GSTConstants.ADVVALOREM_CESS_RATE_STATE);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					idx, errorLocations.toArray());
			errors.add(new ProcessingResult(APP_VALIDATION, "ER15020",
					"Invalid State cess Advalorem", location));
			// if it's not a number, then we can return the errors
			// immediately
			return errors;
		}
		boolean isValid = NumberFomatUtil.is3And3digValidDec(obj.toString().trim());
		if (!isValid) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.ADVVALOREM_CESS_RATE_STATE);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
