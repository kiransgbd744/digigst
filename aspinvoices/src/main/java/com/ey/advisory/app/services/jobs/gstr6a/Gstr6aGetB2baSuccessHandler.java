 ...
 *
 */
@Service("Gstr6aGetB2baSuccessHandler")
@Slf4j
public class Gstr6aGetB2baSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr6aB2baDataParserImpl")
	private Gstr6aB2baGetDataParser gstr6aB2baGetDataParser;

	@Autowired
	@Qualifier("GetGstr6aStagingB2baHeaderRepository")
	private GetGstr6aStagingB2baHeaderRepository b2baStagingRepo;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			Gstr6aGetInvoicesReqDto dto = gson.fromJson(ctxParamsObj, Gstr6aGetInvoicesReqDto.class);
			batchId = dto.getBatchId();

			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("GET Call is Success for the batchId {} inside Gstr6aGetB2baSuccessHandler.class",
			...
			if (resultIds != null && !resultIds.isEmpty()) {
				if (APIConstants.B2BA.equalsIgnoreCase(dto.getType())) {
					b2baStagingRepo.softlyDeleteB2bHeader(dto.getGstin(),
			...
			resultIds.forEach(id -> {
				...
				// Long batchId = null;
				if (APIConstants.B2BA.equalsIgnoreCase(dto.getType())) {
					List<GetGstr6aStagingB2baHeaderEntity> atEnties = gstr6aB2baGetDataParser
							.parseB2baGetData(dto, apiResp, dto.getType(),
									ctxParamsObj.get("batchId").getAsLong(),
									now);
					if (!atEnties.isEmpty()) {
						b2baStagingRepo.saveAll(atEnties);
			...
			String msg = "exception occured while GSTR6A get call B2BA success handler";
			LOGGER.error(msg, e);
