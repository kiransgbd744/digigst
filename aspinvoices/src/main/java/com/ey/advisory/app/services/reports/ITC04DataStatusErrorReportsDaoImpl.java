...
@Slf4j
@Component("ITC04DataStatusErrorReportsDaoImpl")
public class ITC04DataStatusErrorReportsDaoImpl {
...
*/
	...
	private String createItc04ErrorQueryString(String buildQuery) {
		return "SELECT TRIM(', ' FROM IFNULL(HDR.ERROR_CODES,'') ||','|| IFNULL(ITM.ERROR_CODES,'')) "
				...
				+ "CGST_RATE,ITM.CGST_AMT,SGST_RATE,ITM.SGST_AMT,"
				...
				+ "PROFIT_CENTRE2, ACCOUNTING_VOUCHER_NUM,ACCOUNTING_VOUCHER_DATE,"
				...
				+ "WHEN IS_SAVED_TO_GSTN = FALSE AND "
				+ "GSTN_ERROR = FALSE THEN 'NOT_SAVED' END) AS GSTN_SAVE_STATUS,"
				...
				+ "WHERE  "
				+  buildQuery
				+ " AND IS_ERROR = TRUE AND HDR.DATAORIGINTYPECODE IN ('A','AI') "; 
					}
