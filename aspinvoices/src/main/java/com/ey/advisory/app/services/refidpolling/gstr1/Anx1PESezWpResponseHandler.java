 ...
 *
 */
@Component("Anx1PESezWpResponseHandler")
public class Anx1PESezWpResponseHandler implements Anx1ResponseHandler{
	...
	public Triplet<List<String>, Map<String, OutwardTransDocError>,Integer> response(
		...
		Anx1ErrorData anx1ErrorReportData = anx1.getAnx1ErrorReport()
				.getAnx1ErrorData();

		Map<String, OutwardTransDocError> docErrorMap = new HashMap<>();
		for (Anx1SezwpAndSezwopData invoice : anx1ErrorReportData
				.getSezwpInvoice()) {
			for (Anx1SezwpAndSezwopDocumentData docData : invoice.getDocs()) {
