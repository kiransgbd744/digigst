...
@Component("AspProcessVsSubmitTable15ReportDaoImpl")
public class AspProcessVsSubmitTable15ReportDaoImpl
	...
	private String createApiProcessedQueryString(String buildHeaderHdr,String buildHeader) {
		...
		build.append("SELECT   S_NO, "
				...
				+ "		AND IS_DELETE = FALSE "
				+ "		AND HDR.TCS_FLAG = 'O' AND HDR.RETURN_TYPE='GSTR1' AND "
				+ buildHeaderHdr
				+ "		GROUP BY  HDR.SUPPLIER_GSTIN, "
				+ "                  HDR.RETURN_PERIOD, "
				+ "			      HDR.ECOM_GSTIN, "
				...
				+ "AND TABLE_SECTION IN ('15(ii)') AND "
				+ buildHeader
				+ "GROUP BY HDR.TAX_RATE "
				+ " "
				+ "UNION ALL "
				+ " "
				+ "SELECT 'Table 15A(ii).a' AS S_NO, "
				...
				+ "AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				+ "WHERE HDR.IS_DELETE = FALSE "
				+ "AND TABLE_SECTION IN ('15A(ii)') AND "
				+ buildHeader
				+ "GROUP BY HDR.TAX_RATE "
				+ ") "
				+ ") "
