...
public class CewbModeOfOtherRoadDocNo
	...
	public List<ProcessingResult> validate(CewbExcelEntity document,
		...
		if (transMode != null) {
			String transNo = document.getTransDocNo() != null
					&& !document.getTransDocNo().isEmpty()
							? document.getTransDocNo() : null;
			if (transNo == null &&  !GSTConstants.ROAD.equalsIgnoreCase(transMode)) {
				Set<String> errorLocations = new HashSet<>();
				errorLocations.add(GSTConstants.TRANS_PORT_DOCNO);
				TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
						null, errorLocations.toArray());
				errors.add(new ProcessingResult(GSTConstants.APP_VALIDATION,
						"ER6131",
						"In case of mode other than Road Transport Document No is Mandatory.",
						location));
				return errors;
			}
		}
