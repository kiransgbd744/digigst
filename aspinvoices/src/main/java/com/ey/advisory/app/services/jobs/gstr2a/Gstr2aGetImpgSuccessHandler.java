 ...
 *
 */
@Service("Gstr2aGetImpgSuccessHandler")
@Slf4j
public class Gstr2aGetImpgSuccessHandler implements SuccessHandler {

	@Autowired
	@Qualifier("Gstr2aImpgDataParserImpl")
	private Gstr2aImpgDataParser gstr2aImpgDataParser;
	...
	public void handleSuccess(SuccessResult result, String apiParams) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"GET Call is Success for the batchId {} inside Gstr2aGetImpgSuccessHandler.class",
						batchId);
			}
			...
			if (APIConstants.GSTR2A.equalsIgnoreCase(apiSection)) {
				...
				if (!isFDeltaGetData) {
					...
					List<String> uniqueKeys = getGstr2aImpgRepository
							.findInvKeyByBatchId(batchId);

					uniqueKeys.forEach(uniqueKey -> {
						amdUtil.createAsyncjob(uniqueKey, APIConstants.IMPG,
								dto);
					});
	...
	private void handleUserGetGstr2a(List<Long> resultIds,
		...
		if (resultIds != null && !resultIds.isEmpty()) {
			if (APIConstants.IMPG.equalsIgnoreCase(dto.getType())) {
				getGstr2aImpgRepository.softlyDeleteByGstnRetPeriod(
		...
		resultIds.forEach(id -> {
			...
			if (APIConstants.IMPG.equalsIgnoreCase(dto.getType())) {
				Pair<List<GetGstr2aStagingImpgHeaderEntity>, Set<String>> parseImpgData = gstr2aImpgDataParser
						.parseImpgData(dto, apiResp, dto.getType(), batchId,
								now);
				List<GetGstr2aStagingImpgHeaderEntity> impgEntities = parseImpgData
						.getValue0();
				Type listType = new TypeToken<List<GetGstr2aImpgHeaderEntity>>() {
				}.getType();
				ModelMapper modelmapper = new ModelMapper();
				List<GetGstr2aImpgHeaderEntity> postDtoList = modelmapper
						.map(impgEntities, listType);
				postDtoList.forEach(postDto -> {
					postDto.setDeltaInStatus(APIConstants.N);
				});
				if (!impgEntities.isEmpty()) {
					getGstr2aImpgRepository.saveAll(postDtoList);
				}
				Set<String> uniqueKeys = parseImpgData.getValue1();
				uniqueKeys.forEach(uniqueKey -> {
					amdUtil.createAsyncjob(uniqueKey, APIConstants.IMPG, dto);
				});
	...
	private void handleStagingGetGstr2a(List<Long> resultIds,
		...
		if (resultIds != null && !resultIds.isEmpty()) {
			if (APIConstants.IMPG.equalsIgnoreCase(dto.getType())) {
				getGstr2aStagingImpgRepository.softlyDeleteByGstnRetPeriod(
						dto.getGstin(), dto.getReturnPeriod(), now);
			}
		}
		resultIds.forEach(id -> {
			...
			if (APIConstants.IMPG.equalsIgnoreCase(dto.getType())) {
				...
				if (!impgEntities.isEmpty()) {
					getGstr2aStagingImpgRepository.saveAll(impgEntities);
				}
				/*
				 * Set<String> uniqueKeys = parseImpgData.getValue1();
				 * uniqueKeys.forEach(uniqueKey -> {
