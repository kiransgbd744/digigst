...
public class Gstr6SuppliergstinValidationRule implements ValidationRule {
	...
	public List<ProcessingResult> isValid(int idx, Object obj, Object[] row,
		...
		if (!isPresent(obj)) {
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER3005",
					"ISD GSTIN cannot be left balnk.", location));
			return errors;
		}
		...
		if (obj.toString().trim().length() == 15
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER3004",
					"Invalid ISD GSTIN.", location));
			return errors;
		}
		...
		if (obj.toString().trim().length() == 10 && !matcher1.matches()) {
			Set<String> errorLocations = new HashSet<>();
			errorLocations.add(GSTConstants.SGSTIN);
			TransDocProcessingResultLoc location = new TransDocProcessingResultLoc(
					null, errorLocations.toArray());
