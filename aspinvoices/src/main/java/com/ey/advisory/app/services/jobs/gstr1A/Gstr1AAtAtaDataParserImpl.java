...
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AAtItemEntity;
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AAtaHeaderEntity;
import com.ey.advisory.app.data.gstr1A.entities.client.GetGstr1AAtaItemEntity;
 ...
 *
 */
@Component("gstr1AAtAtaDataParserImpl")
@Slf4j
public class Gstr1AAtAtaDataParserImpl implements Gstr1AAtAtaDataParser {
	...
	public List<GetGstr1AAtHeaderEntity> parseAtData(Gstr1GetInvoicesReqDto dto, String apiResp) {
		...
		List<GetGstr1AAtHeaderEntity> headerList = new ArrayList<>();
		...
		//try {
			...
			for (ATInvoices eachInv : baseEntity) {
				...
				List<GetGstr1AAtItemEntity> lineItems = new ArrayList<>();
				if (eachInv.getItems() != null) {
					for (ATItemDetails eachInvData : eachInv.getItems()) {

						// New Item Entity
						GetGstr1AAtItemEntity item = new GetGstr1AAtItemEntity();
						/**
						 * ATItemDetails data
						 */
					...
					// New Header Entity
					GetGstr1AAtHeaderEntity header = new GetGstr1AAtHeaderEntity();
					/**
					 * Input data
					 */
	...
	public List<GetGstr1AAtaHeaderEntity> parseAtaData(Gstr1GetInvoicesReqDto dto, String apiResp) {
		...
		Gson gson = GsonUtil.newSAPGsonInstance();
		JsonArray respObject = null;
		List<GetGstr1AAtaHeaderEntity> headerList = new ArrayList<>();
		...
		//try {
			...
			for (ATInvoices eachInv : baseEntity) {
				...
				List<GetGstr1AAtaItemEntity> lineItems = new ArrayList<>();
				...
				if (eachInv.getItems() != null) {
					...
					for (ATItemDetails eachInvData : eachInv.getItems()) {
						...
						// New Item Entity
						GetGstr1AAtaItemEntity item = new GetGstr1AAtaItemEntity();
						/**
						 * ATaItemDetails data
						 */
						item.setReturnPeriod(dto.getReturnPeriod());
					...
					// New Header Entity
					GetGstr1AAtaHeaderEntity header = new GetGstr1AAtaHeaderEntity();
