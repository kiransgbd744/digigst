...
@Component("Gstr1GstnInvSeriesTablesDaoImpl")
public class Gstr1GstnInvSeriesTablesDaoImpl
		implements Gstr1GstnSummaryTablesDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1GstnInvSeriesTablesDaoImpl.class);
	...
	public List<Object> getGstr1GstnSummaryTablesReport(
		...
		String queryStr = creategstnInvQueryString(
				buildQuery.toString());
		...
		return list.parallelStream().map(o -> convertgstnInvoiceSeries(o))
	...
	private Gstr1GstnInvoiceSeriesTableDto convertgstnInvoiceSeries(
		...
		obj.setFromserialNumber(arr[4] != null ? arr[4].toString() : null);
		obj.setToserialNumber(arr[5] != null ? arr[5].toString() : null);
		obj.setTotalNumber(arr[6] != null ? arr[6].toString() : null);
		obj.setCancelled(arr[7] != null ? arr[7].toString() : null);
		obj.setNetIssued(arr[8] != null ? arr[8].toString() : null);
		obj.setSerialNumber(arr[9] != null ? arr[9].toString() : null);
		return obj;
	}

	private String creategstnInvQueryString(String buildQuery) {

		return "SELECT HDR.ID,GSTIN,RETURN_PERIOD,MD.NATURE_DOC_TYPE DOC_TYPE,"
				+ "FROM_SERIAL_NUM,TO_SERIAL_NUM,TOT_NUM,CANCEL,NET_ISSUE, "
				+ "(ROW_NUMBER () OVER (ORDER BY GSTIN)) AS  SNO "
				+ "FROM GETGSTR1_DOC_ISSUED HDR LEFT OUTER JOIN "
				+ "MASTER_NATURE_OF_DOC MD ON HDR.DOC_NUM = MD.ID "
				+ " WHERE HDR.IS_DELETE=FALSE "
				+ buildQuery
				+ " GROUP BY HDR.ID,GSTIN,RETURN_PERIOD,MD.NATURE_DOC_TYPE,"
				+ "FROM_SERIAL_NUM,TO_SERIAL_NUM,TOT_NUM,CANCEL,NET_ISSUE";
	}
}
