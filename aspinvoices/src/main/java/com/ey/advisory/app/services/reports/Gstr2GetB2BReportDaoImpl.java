...
@Component("Gstr2GetB2BReportDaoImpl")
@Slf4j
public class Gstr2GetB2BReportDaoImpl implements Gstr1aGetDao {
	...
	private String creategstnB2CSSummaryQueryString(String buildQuery) {
		...
		return "SELECT RETURN_PERIOD,CGSTIN,SGSTIN,LEGAL_NAME,TRADE_NAME,"
			...
			*/	+ " '' CDN_DELINKING_FLAG ,'' CR_DR_PRE_GST,'' ITC_ELIGIBLE,DIFF_PERCENT,ITEM_NUMBER,"
				...
				+ "WHERE GSTIN_STATUS ='Active' AND IS_ACTIVE = TRUE AND IS_FETCHED = TRUE) VD ON HDR.SGSTIN = VD.GSTIN "
				+ " LEFT JOIN MASTER_STATE SC ON HDR.POS = SC.STATE_CODE "
				+ "WHERE HDR.IS_DELETE = FALSE "
				+ " GROUP BY "
				+ "TAX_PERIOD,CFS,CGSTIN,SGSTIN,LEGAL_NAME_BS,TRADE_NAME,SUPPLIER_INV_NUM, "
				+ "SUPPLIER_INV_DATE,SUPPLIER_INV_VAL,TAX_RATE,"
