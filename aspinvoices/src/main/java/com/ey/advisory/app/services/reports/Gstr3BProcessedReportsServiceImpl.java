...
@Service("Gstr3BProcessedReportsServiceImpl")
public class Gstr3BProcessedReportsServiceImpl implements GSTR3BProcessedReportService {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr3BProcessedReportsServiceImpl.class);

	@Autowired
	CommonUtility commonUtility;

	@Autowired
	@Qualifier("Gstr3BProcessedReportDaoImpl")
	private Gstr3bdownloadProcessedDao gstr3bdownloadProcessedDao;

	@Override
	public void generateProcessedCsv(SearchCriteria criteria, PageRequest pageReq, String fullPath) {
		Anx1FileStatusReportsReqDto request = (Anx1FileStatusReportsReqDto) criteria;

		List<Gstr3bdownloadProcessedDto> responseFromDao = gstr3bdownloadProcessedDao.generatProcessedCsv(request);
		...
		if (responseFromDao != null && !responseFromDao.isEmpty()) {
			...
			try (Writer writer = new BufferedWriter(new FileWriter(fullPath), 8192)) {
				...
				invoiceHeadersTemplate = commonUtility.getProp("gstr3b.processed.csv.report.headers");
				writer.append(invoiceHeadersTemplate);
				columnMappings = commonUtility.getProp("gstr3b.processed.csv.report.headers.mapping").split(",");
