...
@Slf4j
@Component("Gstr7TransRSAspErrorRptDwnldServiceImpl")
public class Gstr7TransRSAspErrorRptDwnldServiceImpl
	...
	private static final String CHUNK_PROC = "USP_INS_CHUNK_GSTR7_OD_PS_CONS_ASP_ERROR";
	private static final String DISP_PROC = "USP_CHUNK_DISP_GSTR7_OD_PS_CONS_ASP_ERROR";
	...
	public void generateReports(Long id, String orgReportType) {
		...
		try {
			...
			invoiceHeadersTemplate = commonUtility
					.getProp("gstr7.trans.con.digigst.error.report.headers");

			columnMappings = commonUtility
					.getProp("gstr7.trans.con.digigst.error.report.columns")
					.split(",");
			...
			while (j < chunkCount) {
				...
				if (records != null && !records.isEmpty()) {
					...
					LOGGER.error("reconData List {} ", reconDataList);
