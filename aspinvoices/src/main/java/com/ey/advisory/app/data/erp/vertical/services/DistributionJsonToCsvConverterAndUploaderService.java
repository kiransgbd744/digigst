 ...
 */
@Slf4j
@Component("DistributionJsonToCsvConverterAndUploaderService")
public class DistributionJsonToCsvConverterAndUploaderService {
	...
	@Autowired
	@Qualifier("DistributionConvertDtoTocsv")
	private DistributionConvertDtoTocsv convertDtoTocsv;

	public String convertAndUpload(DistributionHeaderReqDto dto, String cntrlPayloadId) {
		File tempDir = null;
		String filename = null;
		try {
			String gstin = dto.getIsdGstin();
...
//			filename = DocumentUtility.uploadCsvFile(csvDir, folderName);
			...
			Gstr1FileStatusEntity fileStatus = gstr1FileUploadUtil
					.getErpFileStatus(filename, userName, null,
							GSTConstants.DISTRIBUTION, GSTConstants.INWARD, null);
			...
			gstr1FileUploadJobInsertion.fileUploadJob(jsonParams,
					JobConstants.GSTR6DISTRIBUTIONFILEUPLOAD, userName);
	...
	private static File createTempDir() throws IOException {
		return Files.createTempDirectory("DistributionFile").toFile();
	}
