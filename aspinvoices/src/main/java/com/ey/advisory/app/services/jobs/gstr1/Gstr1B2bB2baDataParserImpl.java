 ...
 *
 */
@Component("gstr1B2bB2baDataParserImpl")
@Slf4j
public class Gstr1B2bB2baDataParserImpl implements Gstr1B2bB2baDataParser {

	@Autowired
	private Anx1DocKeyGenerator docKeyGenerator;
	
	@Override
	public List<GetGstr1B2bHeaderEntity> parseB2bData(Gstr1GetInvoicesReqDto dto, String apiResp) {

		Gson gson = GsonUtil.newSAPGsonInstance();
		JsonArray respObject = null;
		List<GetGstr1B2bHeaderEntity> headerList = new ArrayList<>();
		...
		for (B2BInvoices eachInv : baseEntity) {
			...
			for (B2BInvoiceData eachInvData : eachInv.getB2bInvoiceData()) {
				...
				List<GetGstr1B2bItemEntity> lineItems = new ArrayList<>();
				...
				if (eachInvData.getLineItems() != null) {
					...
					for (B2bLineItem b2cItems : eachInvData.getLineItems()) {
						...
						// New Item Entity
						GetGstr1B2bItemEntity item = new GetGstr1B2bItemEntity();
						/**
						 * B2BItemDetails data
						 */
					...
					// New Header Entity
					GetGstr1B2bHeaderEntity header = new GetGstr1B2bHeaderEntity();
					/**
					 * Input data
					 */
	...
	@Override
	public List<GetGstr1B2baHeaderEntity> parseB2baData(Gstr1GetInvoicesReqDto dto, String apiResp) {

		Gson gson = GsonUtil.newSAPGsonInstance();
		JsonArray respObject = null;
		List<GetGstr1B2baHeaderEntity> headerList = new ArrayList<>();
		...
		for (B2BInvoices eachInv : baseEntity) {
			...
			for (B2BInvoiceData eachInvData : eachInv.getB2bInvoiceData()) {
				...
				BigDecimal sgstAmt = BigDecimal.ZERO;
				BigDecimal cessAmt = BigDecimal.ZERO;
				List<GetGstr1B2baItemEntity> lineItems = new ArrayList<>();
				...
				if (eachInvData.getLineItems() != null) {
					...
					for (B2bLineItem b2cItems : eachInvData.getLineItems()) {
						...
						B2bLineItemDetail b2cItem = b2cItems.getItemDetail();
						// New Item Entity
						GetGstr1B2baItemEntity item = new GetGstr1B2baItemEntity();
						/**
					...
					// New Header Entity
					GetGstr1B2baHeaderEntity header = new GetGstr1B2baHeaderEntity();
					/**
					 * Input data
					 */
					header.setAction(APIConstants.N);
