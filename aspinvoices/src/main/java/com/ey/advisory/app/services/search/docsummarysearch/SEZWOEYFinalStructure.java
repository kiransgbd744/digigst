...
@Service("SEZWOEYFinalStructure")
public class SEZWOEYFinalStructure {
	

	public List<Gstr1SummaryRespDto> getSEZWOEyList(
			List<Gstr1SummaryRespDto> sezwoEYList,
		...
		// If 6B filtered list is not null
		if (view6BFiltered != null & view6BFiltered.size() > 0) {
			// then filter default List for 6B
			List<Gstr1SummaryRespDto> default6BFiltered = sezwoEYList.stream()
					.filter(p -> p.getTableSection().equalsIgnoreCase("6B"))
					.collect(Collectors.toList());
			...
			view6BFiltered.forEach(view6B -> {
				...
				sezwoEYList.add(summaryRespDto);
			});
		}
	
		// Sort the list in Ascending Order
		Collections.sort(sezwoEYList, new Comparator<Gstr1SummaryRespDto>() {
			@Override
