...
@Component("AsyncFileStatusReportHandlerImpl")
public class AsyncReportHandlerImpl implements AsyncReportHandler {
	...
	public void setDataToEntity(FileStatusDownloadReportEntity entity,
		...
		if (ReportTypeConstants.FILE_STATUS.equalsIgnoreCase(reportCateg)
			...
			entity.setRequestFromDate(
					setDataSecurity.getReceivFromDate() != null
							? EYDateUtil.toUTCDateTimeFromLocal(setDataSecurity
									.getReceivFromDate().atStartOfDay())
							: null);
			entity.setRequestToDate(setDataSecurity.getReceivToDate() != null
					? EYDateUtil.toUTCDateTimeFromLocal(
							setDataSecurity.getReceivToDate().atStartOfDay())
					: null);

			Long derivedTaxFrom = reqDto.getReturnFrom() != null
					? getDerivedTaxPeriod(reqDto.getReturnFrom()) : null;
			Long derivedTaxTo = reqDto.getReturnTo() != null
					? getDerivedTaxPeriod(reqDto.getReturnTo()) : null;

			entity.setDerivedRetPeriodFrom(derivedTaxFrom);
			entity.setDerivedRetPeriodFromTo(derivedTaxTo);
			
			entity.setAccVoucherDateFrom(
					setDataSecurity.getAccVoucherDateFrom() != null
							? EYDateUtil.toUTCDateTimeFromLocal(setDataSecurity
									.getAccVoucherDateFrom().atStartOfDay())
							: null);
			entity.setAccVoucherDateTo(setDataSecurity.getAccVoucherDateTo() != null
					? EYDateUtil.toUTCDateTimeFromLocal(
							setDataSecurity.getAccVoucherDateTo().atStartOfDay())
					: null);
			
			entity.setDocumentDateFrom(
					setDataSecurity.getDocumentDateFrom() != null
							? EYDateUtil.toUTCDateTimeFromLocal(setDataSecurity
									.getDocumentDateFrom().atStartOfDay())
							: null);
			entity.setDocumentDateTo(
					setDataSecurity.getDocumentDateTo() != null
					? EYDateUtil.toUTCDateTimeFromLocal(
							setDataSecurity.getDocumentDateTo().atStartOfDay())
					: null);
			...
			if("gstr7txn".equalsIgnoreCase(entity.getDataType())) {
				entity.setReportType(reqDto.getType());
				entity.setReportCateg("GSTR7");
				entity.setDataType("GSTR7");
				/*
				entity.setRequestFromDate(
						reqDto.getReceivFromDate() != null
								? EYDateUtil.toUTCDateTimeFromLocal(reqDto
										.getReceivFromDate().atStartOfDay())
								: null);
				entity.setRequestToDate(reqDto.getReceivToDate() != null
						? EYDateUtil.toUTCDateTimeFromLocal(
								reqDto.getReceivToDate().atStartOfDay())
						: null);
				*/
			}
		} else if (ReportTypeConstants.GSTR3B_ENTITY_REPORT
				.equalsIgnoreCase(reportCateg)) {
			...
			Gstr1FileStatusEntity fileEntity = fileStatusEntity.get();
			entity.setUpldFileName(fileEntity.getFileName());
		} else if (ReportTypeConstants.EINVSUMMARY
				.equalsIgnoreCase(reportCateg)) {
			entity.setGstins(GenUtil.convertStringToClob(
					convertToQueryFormat(reqDto.getGstins())));
			entity.setTaxPeriod(reqDto.getTaxPeriod());
