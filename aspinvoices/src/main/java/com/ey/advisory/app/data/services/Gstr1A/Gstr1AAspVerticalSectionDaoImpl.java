 ...
 *
 */
@Component("Gstr1AAspVerticalSectionDaoImpl")
public class Gstr1AAspVerticalSectionDaoImpl
		implements Gstr1AAspVerticalSectionDao {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1AAspVerticalSectionDaoImpl.class);
	...
	private String createQueryString(String buildQuery, String buildQuery2,
		...
		if ("B2CS".equalsIgnoreCase(docType)) {
			queryStr = "WITH TRANS AS (" + "     SELECT " + "          ("
					+ "            IFNULL(HDR.SUPPLIER_GSTIN, '') || '|' || IFNULL(HDR.RETURN_PERIOD, '') || '|' || "
					+ "	IFNULL(HDR.DIFF_PERCENT, '') || '|' || IFNULL(HDR.POS, 9999)|| '|' || IFNULL(HDR.ECOM_GSTIN, '') || '|' || IFNULL(ITM.TAX_RATE, 9999)"
					+ "          ) KEY, " + "          IFNULL("
					+ "            SUM("
					...
					+ "            ), " + "            0"
					+ "          ) AS CESS_AMT " + "        FROM "
					+ "          ANX_OUTWARD_DOC_HEADER_1A HDR "
					+ "          INNER JOIN ANX_OUTWARD_DOC_ITEM_1A ITM ON HDR.ID = ITM.DOC_HEADER_ID "
					+ "        WHERE " + buildQuery +
					/*
					 * "          HDR.DERIVED_RET_PERIOD = :taxPeriod " +
					 * "          AND HDR.SUPPLIER_GSTIN IN (:gstinList) " +
					 */
					...
					+ "        GROUP BY " + "          HDR.SUPPLIER_GSTIN, "
					+ "          HDR.RETURN_PERIOD, "
					+ "          HDR.DIFF_PERCENT, " + "          HDR.POS, "
					+ "          HDR.ECOM_GSTIN, " + "          ITM.TAX_RATE "
					+ "), " + "VER AS (" + "        SELECT " + "          ("
					+ "            IFNULL(SUPPLIER_GSTIN, '') || '|' || IFNULL(RETURN_PERIOD, '') || '|' || "
					+ " IFNULL(TRAN_TYPE, '') || '|' || IFNULL(NEW_POS, 9999)|| '|' || IFNULL(NEW_ECOM_GSTIN, '')|| '|' || IFNULL(NEW_RATE, 9999)"
					+ "          ) KEY, " + "          SUM("
					+ "            IFNULL(NEW_TAXABLE_VALUE, 0)+ IFNULL(IGST_AMT, 0)+ IFNULL(CGST_AMT, 0)+ "
					+ " IFNULL(SGST_AMT, 0)+ IFNULL(CESS_AMT, 0)"
					+ "          ) AS DOC_AMT, " + "          IFNULL("
					+ "            SUM(NEW_TAXABLE_VALUE), " + "            0"
					+ "          ) AS TAXABLE_VALUE, " + "          IFNULL("
					+ "            SUM(IGST_AMT), " + "            0"
					+ "          ) AS IGST_AMT, " + "          IFNULL("
					+ "            SUM(CGST_AMT), " + "            0"
					...
					+ "      SUM(CESS_AMT) CESS_AMT " + "      FROM TRANS "
					+ "UNION ALL " + "SELECT " + "      'TOTAL' AS TYPE, "
					+ "      COUNT(DISTINCT KEY) KEYCOUNT, "
					+ "      SUM(DOC_AMT) DOC_AMT, "
					+ "      SUM(TAXABLE_VALUE) TAXABLE_VALUE, "
					+ "      SUM(IGST_AMT) IGST_AMT, "
					+ "      SUM(CGST_AMT) CGST_AMT, "
					+ "      SUM(SGST_AMT) SGST_AMT, "
					+ "      SUM(CESS_AMT) CESS_AMT " + "      FROM VER "
					+ "UNION ALL " + "SELECT "
	...
	private String createGstnQueryString(String buildQuery, String docType,
		...
		if ("B2CS".equalsIgnoreCase(docType)) {
			...
			queryString = "WITH HDR AS (" + "    SELECT " + "        ID, "
					+ "        DERIVED_RET_PERIOD, "
					+ "        SUPPLIER_GSTIN, " + "        RETURN_PERIOD, "
					+ "        POS, " + "        DOC_TYPE " + "    FROM "
					+ "        ANX_OUTWARD_DOC_HEADER_1A B2CS" + "    WHERE "
					+ buildQuery +
					/*
					 * "        DERIVED_RET_PERIOD = 202405 " +
					 * "        AND SUPPLIER_GSTIN IN ('33GSPTN0482G1Z9') " +
					 */
					"        AND TAX_DOC_TYPE = 'B2CS' "
					+ "        AND RETURN_TYPE = 'GSTR1A' "
					+ "        AND ASP_INVOICE_STATUS = 2 "
					+ "        AND COMPLIANCE_APPLICABLE = TRUE "
					+ "        AND IS_DELETE = FALSE " + ") " + "SELECT "
					+ "    SUPPLIER_GSTIN, " + "    RETURN_PERIOD, "
					+ "    POS, " + "    TRAN_TYPE, " + "    RATE, "
					...
					+ "            SGST_AMT, " + "            CESS_AMT "
					+ "        FROM "
					+ "            GSTR1A_PROCESSED_B2CS B2CS "
					+ "        WHERE " + buildQuery
					+ "           AND B2CS.IS_DELETE = FALSE "
					+ "            AND IS_AMENDMENT = FALSE " +
					/*
					 * "            AND B2CS.SUPPLIER_GSTIN IN ('33GSPTN0482G1Z9') "
					 * + "            AND B2CS.DERIVED_RET_PERIOD = 202405 " +
					 */
					"        UNION ALL " + "        SELECT "
					+ "            B2CS.SUPPLIER_GSTIN, "
					+ "            B2CS.RETURN_PERIOD, "
					...
					+ "            ) AS CESS_AMT " + "        FROM "
					+ "            HDR B2CS "
					+ "            INNER JOIN ANX_OUTWARD_DOC_ITEM_1A ITM ON B2CS.ID = ITM.DOC_HEADER_ID "
					+ "            AND B2CS.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
					+ "        GROUP BY " + "            B2CS.SUPPLIER_GSTIN, "
					+ "            B2CS.RETURN_PERIOD, "
					+ "            B2CS.POS, "
					+ "            ITM.DIFF_PERCENT, "
					+ "            ITM.ECOM_GSTIN, "
					+ "            ITM.TAX_RATE " + "    ) " + "GROUP BY "
					+ "    SUPPLIER_GSTIN, " + "    RETURN_PERIOD, "
					+ "    POS, " + "    TRAN_TYPE, " + "    RATE ";
