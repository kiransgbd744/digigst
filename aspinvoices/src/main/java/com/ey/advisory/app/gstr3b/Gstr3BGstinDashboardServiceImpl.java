 ...
 *
 */
@Component("Gstr3BGstinDashboardServiceImpl")
@Slf4j
public class Gstr3BGstinDashboardServiceImpl
	...
	@Autowired
	Gstr3BAllOtherItcSumService gstr3BAllOtherItcSumService;
	...
	public Gstr3bDashboardDto getgstr3bgstnDashBoardList(String taxPeriod,
		...
		Table4d1ValidationStatusDto get4d1ValidationStatusDto=null;
		try {
			...
			/* Logic for all other ITC 
			 * 4(a)(5) = 4(a)(5)(a/b/c/d) + 4(a)(5)(e) + 4(a)(5)(f)
			
			Logic for Inward supplies from ISD 
				
			4(a)(4) = 4(a)(4)(4.1) + 4(a)(4)(4.2) + 4(a)(4)(4.3)
			
			 */
			
			if (userInputDtoList != null && !userInputDtoList.isEmpty()) {
				gstr3BAllOtherItcSumService.sumOfITcSection(userInputDtoList,
						gstin, taxPeriod, entityId);

			}

			userInputDtoList = gstnDao
					.getGstinUserInputDtoList(gstin, taxPeriod);
			...
			SignAndFileEntity signAndFileEntity = signAndFileRepo
					.findFirstByGstinAndTaxPeriodAndReturnTypeOrderByIdDesc(
							gstin, taxPeriod, APIConstants.GSTR3B);
			/*
			...
			if (gstnDtoList.isEmpty()) {
				...
				if (authTokenService.getAuthTokenStatusForGstin(gstin)
					...
					// Invoking GSTN API
					APIResponse gstnApiResponse = getGSTR3BSummary(taxPeriod,
							gstin);

					if (LOGGER.isDebugEnabled()) {
						LOGGER.info("Gstr3BGstinDashboardServiceImpl"
								+ ".getgstr3bgstnDashBoardList parsing the "
								+ "json response");
					}
					...
					if (gstnApiResponse.isSuccess()) {
						...
						List<Gstr3BGstinsDto> resultList = dashBoardService
								.getGstrDtoList(gstnApiResponse);

						// Saving Response to DB Table.
						aspService.saveGstnResponse(gstin, taxPeriod,
						...
						/*
						 * the below code will take the hard coded map for
						 * gstr3b and set the values (if available) from user
						 * input list, compute list, AutoCal and GSTN response
						 * into thegstr3b list
						 */
			...
			if (saveEntity != null) {
				...
				LocalDateTime recentSaveDate = EYDateUtil
						.toISTDateTimeFromUTC(saveEntity.getCreatedOn());
				gstr3bResp.setRecentSavedDate(formatter.format(recentSaveDate));
			} else {
			...
			if (updateEntity != null && !updateEntity.isEmpty()) {
				...
				LocalDateTime recentUpdatetoGstn = EYDateUtil
						.toISTDateTimeFromUTC(entity.getCreateDate());
				gstr3bResp.setRecentUpdatedDate(
						formatter.format(recentUpdatetoGstn));
			...
			if (signAndFileEntity != null) {
				DateTimeFormatter formatter = DateTimeFormatter
						.ofPattern("yyyy-MM-dd'T'HH:mm:ss");
				if (signAndFileEntity.getModifiedOn() != null) {
					LocalDateTime modifiedOnist = EYDateUtil
							.toISTDateTimeFromUTC(
									signAndFileEntity.getModifiedOn());
					String statusAndTime = signAndFileEntity.getStatus() + ": "
							+ formatter.format(modifiedOnist);
					gstr3bResp.setRecentFilingDate(statusAndTime);
				} else {
					gstr3bResp.setRecentFilingDate("");
				}
			...
			get4d1ValidationStatusDto = validationService
					.get4D1ValidationStatus(gstin, taxPeriod);
			
			// logic to check zero value on 4(d)(1)
			List<Gstr3BGstinAspUserInputEntity> entity4d2 = userRepo
					.findByGstinAndTaxPeriodAndSectionNameAndIsActiveTrue(gstin,
							taxPeriod, "4(d)(1)");
			
			if (entity4d2 == null || entity4d2.isEmpty()) {
				gstr3bResp.setErrorFlag(false);
				gstr3bResp.setValidationFlag(true);
			} else {
				BigDecimal cess = entity4d2.get(0).getCess() != null
						? entity4d2.get(0).getCess()
						: BigDecimal.ZERO;
				BigDecimal cgst = entity4d2.get(0).getCgst() != null
						? entity4d2.get(0).getCgst()
						: BigDecimal.ZERO;
				BigDecimal igst = entity4d2.get(0).getIgst() != null
						? entity4d2.get(0).getIgst()
						: BigDecimal.ZERO;
				BigDecimal sgst = entity4d2.get(0).getSgst() != null
						? entity4d2.get(0).getSgst()
						: BigDecimal.ZERO;

				if (cess.add(cgst.add(igst.add(sgst)))
						.compareTo(BigDecimal.ZERO) == 0) {
					gstr3bResp.setErrorFlag(false);
					gstr3bResp.setValidationFlag(true);
				}
				else {
					gstr3bResp.setValidationFlag(get4d1ValidationStatusDto
							.getValidationFlag());
					gstr3bResp.setErrorFlag(
							get4d1ValidationStatusDto.getErrorFlag());
				}
			}
			
			gstr3bResp.setActive(get4d1ValidationStatusDto.getIsActive());
	...
	private Gstr3BGstinAspUserInputDto addInternalDto(Gstr3BGstinAspUserInputDto a,
			Gstr3BGstinAspUserInputDto b) {
		Gstr3BGstinAspUserInputDto dto = new Gstr3BGstinAspUserInputDto();
		dto.setCess(a.getCess().add(b.getCess()));
		dto.setInterState(a.getInterState().add(b.getInterState()));
		dto.setIntraState(a.getIntraState().add(b.getIntraState()));
		dto.setCgst(a.getCgst().add(b.getCgst()));
		dto.setIgst(a.getIgst().add(b.getIgst()));
		dto.setSgst(a.getSgst().add(b.getSgst()));
		dto.setTaxableVal(a.getTaxableVal().add(b.getTaxableVal()));
	...
	public APIResponse getGSTR3BSummary(String taxPeriod, String gstin) {
		...
		saveOrUpdateGSTR3B3BPeriodWiseTable(gstin, taxPeriod);

		saveOrUpdateGSTR3BFyTable(gstin, fy);
	...
	public List<Gstr3BExcemptNilNonGstnDto> getExcemptNilNonGstIS(
		...
		try {
			...
			Map<String, Gstr3BGstinAspUserInputDto> userMap = respUser.stream()
					.collect(Collectors.groupingBy(o -> o.getSectionName(),
							Collectors.collectingAndThen(
									Collectors.reducing((a, b) -> addInternalDto(a, b)),
									Optional::get)));
	...
	private void setGstinvalues(Gstr3BGstinsDto gstr3bgstnDto,
		...
		if (jsonObject.has("ret_period"))
			gstr3bgstnDto.setUserRetPeriod(
					jsonObject.get("ret_period").getAsString());
	...
	private void saveOrUpdateGSTR3B3BPeriodWiseTable(String gstin,
			String taxPeriod) {

		int rowsEffected = 0;

		try {
			rowsEffected = gstr9PeriodWiseRepo.updateGstr3BGetStatus(gstin,
			...
			if (rowsEffected == 0) {
				...
				if (LOGGER.isDebugEnabled()) {
					...
					LOGGER.debug(
							"Creating new entry for GSTR3B Period wise with "
				...
				try {
					...
					String errMsg = String
							.format("Trying to insert duplicate entry in GSTR3B period wise "
				...
				if (LOGGER.isDebugEnabled()) {
					String msg1 = String.format(
							"After Resetting GSTR3B Period wise table for"
									+ "GSTIN: '%s', TaxPeriod: '%s'",
							gstin, taxPeriod);
					LOGGER.debug(msg1);
				}
			...
			// to insert into the AnnualReturnController table.
			String errMsg = String
					.format("Trying to update entry in GSTR3B Period wise "
							+ "table. Ignoring the error");
			// Don't throw back the error.
			LOGGER.error(errMsg, ex);
		}
	...
	private void saveOrUpdateGSTR3BFyTable(String gstin, Integer fy) {

		int rowsEffected = 0;

		try {
			rowsEffected = gstr9FyRepo.updateGstr3BGetStatus(
			...
			if (rowsEffected == 0) {
				...
				if (LOGGER.isDebugEnabled()) {
					...
					LOGGER.debug("Creating new entry for GSTR3B Fy table for "
							+ "gstin : " + gstin + ", fy : " + fy);
				}
				...
				try {
					...
					// to insert into the AnnualReturnController table.
					String errMsg = String
							.format("Trying to insert duplicate entry in GSTR3B Fy "
									+ "table. Ignoring the error");
					// Don't throw back the error.
					LOGGER.error(errMsg, ex);
				}
			...
			// happen if a race condition occurs when 2 threads try
			// to insert into the AnnualReturnController table.
			String errMsg = String.format("Trying to update entry in GSTR3B Fy "
					+ "table. Ignoring the error");
			// Don't throw back the error.
			LOGGER.error(errMsg, ex);
		}
	...
	@Override
	public List<Gstr3BGstinAspUserInputDto> getPastLiabDtls(String taxPeriod,
			String gstin) {

		final List<String> sections = ImmutableList
				.of(Gstr3BConstants.Table7_1);

		List<Gstr3BGstinAspUserInputDto> respUserList = new ArrayList<>();

		try {
			respUserList = gstnDao.getUserInputDtoforSavePst(taxPeriod, gstin,
					sections);

		} catch (Exception ex) {
			String msg = String.format(
					"error occured while invoking " + "getPastLiabDtls() ", ex);
			throw new AppException(msg);
		}
