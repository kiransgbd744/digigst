...
public class AvailableTaxRates18 implements DocRulesValidator<InwardTransDocument>{
	...
	public List<ProcessingResult> validate(InwardTransDocument document,
		...
		IntStream.range(0, items.size()).forEach(idx -> {
			...
			if(item.getAvailableIgst().compareTo(item.getIgstAmount()) > 0 
				|| item.getAvailableCgst().compareTo(item.getCgstAmount()) > 0 
				|| item.getAvailableSgst().compareTo(item.getSgstAmount()) > 0 
					 ){
			...
			errors.add(new ProcessingResult(APP_VALIDATION, "ER404",
					"Total tax available as ITC is more than the Total "
