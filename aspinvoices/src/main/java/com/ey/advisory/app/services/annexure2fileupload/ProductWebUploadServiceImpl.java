...
@Component("ProductWebUploadServiceImpl")
public class ProductWebUploadServiceImpl implements ProductWebUploadService {
	public static final Logger LOGGER = LoggerFactory
			.getLogger(ProductWebUploadServiceImpl.class);
	...
	public ResponseEntity<String> productProcessingFile(MultipartFile[] files,
			String folderName, String uniqueName, String fileType,
			String userName, String dataType,String groupCode,
			Long entityId) {
		LOGGER.error("Inside product file upload method and uniqueName is {}  ",
				uniqueName);
		...
		try {
			...
			if (checkHeaderFormat.getValue0()) {
				...
				for (MultipartFile file : files) {
					...
					if (!file.isEmpty()) {
						...
						if (uniqueName.equalsIgnoreCase(
								GSTConstants.MASTER_PRODUCT_FILE_NAME)) {
							String jobCategory = JobConstants.PRODUCTWEBUPLOAD;
						...
						for (Long jobId : findByJobIds) {
							...
							Gstr1FileStatusEntity fileStatus = gstr1FileUploadUtil
									.getMasterFileStatusEntity(fileName, userName,
											asyncExceJob, fileType, dataType, entityId);
							fileStatus.setFileType(GSTConstants.PRODUCT);
				...
				APIRespDto dto = new APIRespDto("Success",
						"Master Product file uploaded successfully");
