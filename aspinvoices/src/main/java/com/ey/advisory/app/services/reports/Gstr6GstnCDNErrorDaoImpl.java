...
import com.ey.advisory.app.data.views.client.Gstr6CDNGstnErrorDto;
...
@Component("Gstr6GstnCDNErrorDaoImpl")
public class Gstr6GstnCDNErrorDaoImpl implements Gstr6GstnErrorDao {
	...
	public List<Object> getGstr6GstnErrorReport(SearchCriteria criteria) {
		...
		return list.parallelStream().map(o -> convertgstnCDNError(o)).collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr6CDNGstnErrorDto convertgstnCDNError(Object[] arr) {
		Gstr6CDNGstnErrorDto obj = new Gstr6CDNGstnErrorDto();
		...
		obj.setInvoiceNumber(arr[4] != null ? arr[4].toString() : null);
		obj.setInvoiceDate(arr[5] != null ? arr[5].toString() : null);
		obj.setLineNo(arr[6] != null ? arr[6].toString() : null);
		obj.setRate(arr[7] != null ? arr[7].toString() : null);
		obj.setTaxableValue(arr[8] != null ? arr[8].toString() : null);
		obj.setIgst(arr[9] != null ? arr[9].toString() : null);
		obj.setCgst(arr[10] != null ? arr[10].toString() : null);
		obj.setSgst(arr[11] != null ? arr[11].toString() : null);
		obj.setCess(arr[12] != null ? arr[12].toString() : null);
		obj.setRefId(arr[13] != null ? arr[13].toString() : null);
		obj.setRefIdDateTime(arr[14] != null ? arr[14].toString() : null);
		obj.setErrorCode(arr[15] != null ? arr[15].toString() : null);
		obj.setErrorMessage(arr[16] != null ? arr[16].toString() : null);
	...
	private String creategstnB2BErrorQueryString(String buildQuery) {
		...
		return "select HDR.SUPPLIER_GSTIN,HDR.DOC_TYPE,HDR.DOC_NUM,"
				...
				+ "as CESS_AMT, BT.GSTN_SAVE_REF_ID,BT.CREATED_ON, " + "HDR.GSTN_ERROR_CODE AS GSTIN_ERROR_CODE, "
				+ "HDR.GSTN_ERROR_DESC AS GSTN_ERROR_DESC,HDR.POS " + "FROM ANX_INWARD_DOC_HEADER HDR INNER JOIN "
				+ "ANX_INWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID "
				+ "AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				+ "AND RETURN_TYPE = 'GSTR6' AND HDR.TAX_DOC_TYPE = 'CDN' "
				+ "AND GSTN_ERROR = TRUE LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH BT " + "ON HDR.BATCH_ID = BT.ID "
				+ buildQuery + " GROUP BY " + "HDR.SUPPLIER_GSTIN,HDR.DOC_TYPE,HDR.DOC_NUM,HDR.DOC_DATE, "
				+ "HDR.ORIGINAL_DOC_NUM,HDR.ORIGINAL_DOC_DATE,ITM.TAX_RATE," + "BT.GSTN_SAVE_REF_ID,BT.CREATED_ON, "
				+ "HDR.GSTN_ERROR_CODE,HDR.GSTN_ERROR_DESC,HDR.POS ";
	}
}
