...
@Component("Gstr1RSummErrorAspReportsDaoImpl")
public class Gstr1RSummErrorAspReportsDaoImpl
		implements Gstr1ReviewSummaryReportsDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1RSummErrorAspReportsDaoImpl.class);
	...
	public List<Object> getGstr1RSReports(SearchCriteria criteria) {
		...
		if (ud2 != null && !ud2.isEmpty()) {
			if (ud2List != null && ud2List.size() > 0) {
				...
				buildHeader.append(" AND HDR.USERACCESS2 IN :ud1List");
			}
		}
	...
	private DataStatusEinvoiceDto convertError(Object[] arr) {
		...
		obj.setDocumentDate(arr[34] != null ? arr[34].toString() : null);
		...
		obj.setBatchExpiryDate(arr[80] != null ? arr[80].toString() : null);
		obj.setWarrantyDate(arr[81] != null ? arr[81].toString() : null);
	...
	private String createErrorQueryString(String buildQuery, String buildHeader) {
		return " SELECT TRIM(', ' FROM "
				+ "IFNULL(ASPH_ER.ERROR_CODE_ASP,'') ||','|| "
				+ "IFNULL(ASPI_ER.ERROR_CODE_ASP,'')) "
				+ "AS ERROR_CODE_ASP, TRIM(', ' FROM "
				+ "IFNULL(ASPH_ER.ERROR_DESCRIPTION_ASP,'') ||','|| "
				+ "IFNULL(ASPI_ER.ERROR_DESCRIPTION_ASP,'')) "
				+ "AS ERROR_DESCRIPTION_ASP, TRIM(', ' FROM "
				+ "IFNULL(ASPH_ER.INFO_ERROR_CODE_ASP,'') ||','|| "
				+ "IFNULL(ASPI_ER.INFO_ERROR_CODE_ASP,'')) "
				+ "AS INFO_ERROR_CODE_ASP, TRIM(', ' FROM "
				+ "IFNULL(ASPH_ER.INFO_ERROR_DESCRIPTION_ASP,'') ||','|| "
				+ "IFNULL(ASPI_ER.INFO_ERROR_DESCRIPTION_ASP,'')) "
				+ "AS INFO_ERROR_DESCRIPTION_ASP, "
				+ "EINV.STATUS_DESCRIPTION AS IRN_STATUS, "
				+ "EINV.IRN AS IRN_NO, EINV.ACK_NUM AS IRN_ACK_NO, "
				+ "TO_CHAR(HDR.IRN_DATE) AS IRN_ACK_DATE_TIME,"
				+ "EINV.SIGNED_QR AS SIGNED_QR_CODE,"
				+ " '' AS SIGNED_INVOICE,"
				+ "TO_CHAR(HDR.EINV_ERROR_CODE) "
				+ "AS IRP_ERROR_CODE,TO_CHAR(HDR.EINV_ERROR_DESC) "
				+ "AS IRP_ERROR_DESCRIPTION, "
				+ "case when TO_CHAR(HDR.EWB_STATUS) = 1 "
				+ "then 'Not Applicable' when "
				+ "TO_CHAR(HDR.EWB_STATUS) = 2 "
				+ "then 'Pending' when TO_CHAR(HDR.EWB_STATUS) = 3 "
				+ "then 'Generation Error' when "
				+ "TO_CHAR(HDR.EWB_STATUS) = 4 then "
				+ " 'Part A Generated' when "
				+ "TO_CHAR(HDR.EWB_STATUS) = 5 then 'EWB Active' "
				+ "when TO_CHAR(HDR.EWB_STATUS) = 6 "
				+ "then 'Cancelled' when TO_CHAR(HDR.EWB_STATUS) = 7 "
				+ "then 'Discarded' when TO_CHAR(HDR.EWB_STATUS) = 8 "
				+ "then 'Rejected' when TO_CHAR(HDR.EWB_STATUS) = 9 "
				+ "then 'Expired' end as EWBPartAStatusA, "
				+ " '' AS EWBValidUpTo, TO_CHAR(HDR.EWAY_BILL_NUM) "
				+ "AS EWBNO, TO_CHAR(HDR.EWAY_BILL_DATE) "
				+ "AS EWBDATE, TO_CHAR(HDR.EWB_ERROR_CODE) "
				+ "AS EWB_ERROR_CODE, TO_CHAR(HDR.EWB_ERROR_DESC) "
				+ "AS EWB_ERROR_DESCRIPTION, "
				+ " '' AS SAVE_GSTIN_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID "
				+ "AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE "
				+ "AS GSTIN_REFID_DATE_TIME, '' AS GSTIN_ERROR_CODE,"
				+ " '' GSTIN_ERROR_DESCRIPTION_ASP,"
				+ "TO_CHAR(HDR.RETURN_TYPE), TO_CHAR(HDR.TABLE_SECTION) "
				+ "AS TABLE_NUMBER,TO_CHAR(HDR.IRN),"
				+ "TO_CHAR(HDR.IRN_DATE),TO_CHAR(HDR.TAX_SCHEME),"
				+ "TO_CHAR(HDR.CANCEL_REASON),"
				+ "TO_CHAR(HDR.CANCEL_REMARKS),TO_CHAR(ITM.SUPPLY_TYPE),"
				+ "TO_CHAR(HDR.DOC_CATEGORY),"
				+ "TO_CHAR(HDR.DOC_TYPE),TO_CHAR(HDR.DOC_NUM),"
				+ "TO_CHAR(HDR.DOC_DATE),"
				+ "TO_CHAR(HDR.REVERSE_CHARGE),"
				+ "TO_CHAR(HDR.SUPPLIER_GSTIN),TO_CHAR(HDR.SUPP_TRADE_NAME),"
				+ "TO_CHAR(HDR.SUPP_LEGAL_NAME),"
				+ "TO_CHAR(HDR.SUPP_BUILDING_NUM),"
				+ "TO_CHAR(HDR.CUST_SUPP_ADDRESS2) AS SUPPLIERADDRESS2,"
				+ "TO_CHAR(HDR.SUPP_LOCATION), TO_CHAR(HDR.SUPP_PINCODE),"
				+ "TO_CHAR(HDR.SUPP_STATE_CODE),TO_CHAR(HDR.SUPP_PHONE),"
				+ "TO_CHAR(HDR.SUPP_EMAIL),TO_CHAR(HDR.CUST_GSTIN),"
				+ "TO_CHAR(HDR.CUST_TRADE_NAME),TO_CHAR(HDR.CUST_SUPP_NAME),"
				+ "TO_CHAR(HDR.CUST_SUPP_ADDRESS1),"
				+ "TO_CHAR(HDR.CUST_SUPP_ADDRESS2),TO_CHAR(HDR.CUST_SUPP_ADDRESS4),"
				+ "TO_CHAR(HDR.CUST_PINCODE),"
				+ "TO_CHAR(HDR.BILL_TO_STATE),TO_CHAR(HDR.POS),"
				+ "TO_CHAR(HDR.CUST_PHONE),TO_CHAR(HDR.CUST_EMAIL),"
				+ "TO_CHAR(HDR.DISPATCHER_GSTIN),"
				+ "TO_CHAR(HDR.DISPATCHER_TRADE_NAME),"
				+ "TO_CHAR(HDR.DISPATCHER_BUILDING_NUM),"
				+ "TO_CHAR(HDR.DISPATCHER_BUILDING_NAME),"
				+ "TO_CHAR(HDR.DISPATCHER_LOCATION),"
				+ "TO_CHAR(HDR.DISPATCHER_PINCODE),"
				+ "TO_CHAR(HDR.DISPATCHER_STATE_CODE), "
				+ " '' AS SHIP_TO_FLOOR_NUM,TO_CHAR(HDR.SHIP_TO_TRADE_NAME),"
				+ "TO_CHAR(HDR.SHIP_TO_LEGAL_NAME),"
				+ "TO_CHAR(HDR.SHIP_TO_BUILDING_NUM),"
				+ "TO_CHAR(HDR.SHIP_TO_BUILDING_NAME),"
				+ "TO_CHAR(HDR.SHIP_TO_LOCATION),"
				+ "TO_CHAR(ITM.SHIP_TO_PINCODE),"
				+ "TO_CHAR(ITM.SHIP_TO_STATE) AS SHIPTOSTATECODE,"
				+ "TO_CHAR(ITM.ITM_NO), TO_CHAR(ITM.SERIAL_NUM2),"
				+ "TO_CHAR(ITM.PRODUCT_NAME),"
				+ "TO_CHAR(ITM.ITM_DESCRIPTION),"
				+ "TO_CHAR(ITM.IS_SERVICE),TO_CHAR(ITM.ITM_HSNSAC),"
				+ "TO_CHAR(ITM.BAR_CODE),TO_CHAR(ITM.BATCH_NAME_OR_NUM),"
				+ "TO_CHAR(ITM.BATCH_EXPIRY_DATE),"
				+ "TO_CHAR(ITM.WARRANTY_DATE),"
				+ "TO_CHAR(ITM.ORDER_ITEM_REFERENCE),"
				+ "TO_CHAR(ITM.ATTRIBUTE_NAME),"
				+ "TO_CHAR(ITM.ATTRIBUTE_VALUE),"
				+ "TO_CHAR(ITM.ORIGIN_COUNTRY),TO_CHAR(ITM.ITM_UQC),"
				+ "TO_CHAR(ITM.ITM_QTY),TO_CHAR(ITM.FREE_QTY),"
				+ "TO_CHAR(ITM.UNIT_PRICE),TO_CHAR(ITM.ITEM_AMT_UP_QTY),"
				+ "TO_CHAR(ITM.ITEM_DISCOUNT),TO_CHAR(ITM.PRE_TAX_AMOUNT),"
				+ "TO_CHAR(ITM.TAXABLE_VALUE),TO_CHAR(ITM.IGST_RATE),"
				+ "TO_CHAR(ITM.IGST_AMT),TO_CHAR(ITM.CGST_RATE),"
				+ "TO_CHAR(ITM.CGST_AMT),TO_CHAR(ITM.SGST_RATE),"
				+ "TO_CHAR(ITM.SGST_AMT),TO_CHAR(ITM.CESS_RATE_ADVALOREM),"
				+ "TO_CHAR(ITM.CESS_AMT_ADVALOREM),"
				+ "TO_CHAR(ITM.CESS_RATE_SPECIFIC),"
				+ "TO_CHAR(ITM.CESS_AMT_SPECIFIC),"
				...
				+ "TO_CHAR(ITM.INV_ASSESSABLE_AMT),"
				+ "TO_CHAR(ITM.INV_IGST_AMT),TO_CHAR(ITM.INV_CGST_AMT),"
				+ "TO_CHAR(ITM.INV_SGST_AMT),"
				+ "TO_CHAR(ITM.INV_CESS_ADVLRM_AMT),"
				+ "TO_CHAR(ITM.INV_CESS_SPECIFIC_AMT),"
				+ "TO_CHAR(ITM.INV_STATE_CESS_AMT),"
				+ "TO_CHAR(ITM.INV_STATE_CESS_SPECIFIC_AMOUNT),"
				+ "TO_CHAR(ITM.LINE_ITEM_AMT),TO_CHAR(HDR.ROUND_OFF),"
				+ "TO_CHAR(HDR.TOT_INV_VAL_WORLDS),"
				+ "TO_CHAR(HDR.TCS_FLAG_INCOME_TAX), "
				+ "TO_CHAR(ITM.TCS_RATE_INCOME_TAX),"
				+ "TO_CHAR(ITM.TCS_AMOUNT_INCOME_TAX), "
				+ "TO_CHAR(CUSTOMER_PAN_OR_AADHAAR), "
				+ "TO_CHAR(HDR.FOREIGN_CURRENCY), "
				+ "TO_CHAR(HDR.COUNTRY_CODE), "
				+ "TO_CHAR(HDR.INV_VAL_FC), "
				+ "TO_CHAR(HDR.SHIP_PORT_CODE), "
				+ "TO_CHAR(HDR.SHIP_BILL_NUM),TO_CHAR(HDR.SHIP_BILL_DATE), "
				+ "TO_CHAR(HDR.INV_REMARKS), "
				+ "TO_CHAR(HDR.INV_PERIOD_START_DATE),"
				+ "TO_CHAR(HDR.INV_PERIOD_END_DATE),"
				+ "TO_CHAR(ITM.PRECEEDING_INV_NUM),"
				+ "TO_CHAR(ITM.PRECEEDING_INV_DATE),"
				+ "TO_CHAR(ITM.INV_REFERENCE), "
				+ "TO_CHAR(ITM.RECEIPT_ADVICE_REFERENCE), "
				+ "TO_CHAR(ITM.RECEIPT_ADVICE_DATE), "
				+ "TO_CHAR(ITM.TENDER_REFERENCE) , "
				+ "TO_CHAR(ITM.CONTRACT_REFERENCE),"
				+ "TO_CHAR(ITM.EXTERNAL_REFERENCE), "
				+ "TO_CHAR(ITM.PROJECT_REFERENCE),"
				+ "TO_CHAR(ITM.CUST_PO_REF_NUM),"
				+ "TO_CHAR(ITM.CUST_PO_REF_DATE), "
				+ "TO_CHAR(HDR.PAYEE_NAME),TO_CHAR(HDR.MODE_OF_PAYMENT),"
				+ "TO_CHAR(HDR.BRANCH_IFSC_CODE), "
				+ "TO_CHAR(HDR.PAYMENT_TERMS),"
				+ "TO_CHAR(HDR.PAYMENT_INSTRUCTION),"
				+ "TO_CHAR(HDR.CR_TRANSFER), TO_CHAR(HDR.DB_DIRECT),"
				+ "TO_CHAR(HDR.CR_DAYS),TO_CHAR(ITM.PAID_AMT),"
				+ "TO_CHAR(ITM.BAL_AMT),"
				+ "TO_CHAR(HDR.PAYMENT_DUE_DATE),"
				+ "TO_CHAR(HDR.ACCOUNT_DETAIL),"
				+ "TO_CHAR(HDR.ECOM_GSTIN),TO_CHAR(HDR.ECOM_TRANS_ID),"
				+ "TO_CHAR(ITM.SUPPORTING_DOC_URL),"
				+ "TO_CHAR(ITM.SUPPORTING_DOC_BASE64),"
				+ "TO_CHAR(ITM.ADDITIONAL_INFORMATION),"
				+ "TO_CHAR(HDR.TRANS_TYPE),TO_CHAR(HDR.SUB_SUPP_TYPE),"
				+ "TO_CHAR(HDR.OTHER_SUPP_TYPE_DESC),"
				+ "TO_CHAR(HDR.TRANSPORTER_ID),"
				+ "TO_CHAR(HDR.TRANSPORTER_NAME), "
				+ "TO_CHAR(HDR.TRANSPORT_MODE),"
				+ "TO_CHAR(HDR.TRANSPORT_DOC_NUM),"
				+ "TO_CHAR(HDR.TRANSPORT_DOC_DATE),"
				+ "TO_CHAR(HDR.DISTANCE ),TO_CHAR(HDR.VEHICLE_NUM),"
				+ "TO_CHAR(HDR.VEHICLE_TYPE), TO_CHAR(HDR.RETURN_PERIOD),"
				+ "TO_CHAR(HDR.ORIGINAL_DOC_TYPE),"
				+ "TO_CHAR(HDR.ORIGINAL_CUST_GSTIN), "
				+ "TO_CHAR(HDR.DIFF_PERCENT),"
				+ "TO_CHAR(HDR.SECTION7_OF_IGST_FLAG),"
				+ "TO_CHAR(HDR.CLAIM_REFUND_FLAG),"
				+ "TO_CHAR(HDR.AUTOPOPULATE_TO_REFUND),"
				+ "TO_CHAR(HDR.CRDR_PRE_GST),TO_CHAR(HDR.CUST_SUPP_TYPE),"
				+ "TO_CHAR(HDR.CUST_SUPP_CODE) "
				+ "AS CUSTOMERCODE,TO_CHAR(ITM.PRODUCT_CODE),"
				+ "TO_CHAR(ITM.ITM_TYPE), TO_CHAR(ITM.ITC_FLAG), "
				...
				+ "TO_CHAR(ITM.TCS_AMT),TO_CHAR(TCS_CGST_AMT),"
				+ "TO_CHAR(TCS_SGST_AMT), TO_CHAR(HDR.TDS_FLAG), "
				+ "TO_CHAR(TDS_IGST_AMT),TO_CHAR(TDS_CGST_AMT),"
				+ "TO_CHAR(TDS_SGST_AMT), TO_CHAR(HDR.USER_ID) "
				+ "AS userid, TO_CHAR(HDR.COMPANY_CODE),"
				+ "TO_CHAR(HDR.SOURCE_IDENTIFIER), "
				+ "TO_CHAR(HDR.SOURCE_FILENAME), "
				+ "TO_CHAR(ITM.PLANT_CODE),TO_CHAR(HDR.DIVISION), "
				+ "TO_CHAR(SUB_DIVISION),TO_CHAR(ITM.LOCATION), "
				+ "TO_CHAR(HDR.SALES_ORGANIZATION),"
				+ "TO_CHAR(HDR.DISTRIBUTION_CHANNEL),"
				+ "TO_CHAR(HDR.PROFIT_CENTRE),"
				+ "TO_CHAR(HDR.PROFIT_CENTRE2),"
				+ "TO_CHAR(HDR.USERACCESS1),"
				+ "TO_CHAR(HDR.USERACCESS2),TO_CHAR(HDR.USERACCESS3), "
				+ "TO_CHAR(HDR.USERACCESS4),TO_CHAR(HDR.USERACCESS5), "
				+ "TO_CHAR(HDR.USERACCESS6), "
				+ "TO_CHAR(ITM.GLCODE_TAXABLEVALUE),"
				+ "TO_CHAR(ITM.GLCODE_IGST), "
				+ "TO_CHAR(ITM.GLCODE_CGST), "
				+ "TO_CHAR(ITM.GLCODE_SGST),TO_CHAR(ITM.GLCODE_ADV_CESS),"
				+ "TO_CHAR(ITM.GLCODE_SP_CESS), "
				+ "TO_CHAR(ITM.GLCODE_STATE_CESS),"
				+ "TO_CHAR(HDR.GL_STATE_CESS_SPECIFIC), "
				+ "TO_CHAR(HDR.GL_POSTING_DATE),"
				+ "TO_CHAR(HDR.SALES_ORD_NUM),"
				+ "TO_CHAR(HDR.EWAY_BILL_NUM),"
				+ "TO_CHAR(HDR.EWAY_BILL_DATE) AS DATE,"
				+ "TO_CHAR(HDR.ACCOUNTING_VOUCHER_NUM),"
				+ "TO_CHAR(HDR.ACCOUNTING_VOUCHER_DATE),"
				+ "TO_CHAR(ITM.DOCUMENT_REFERENCE_NUMBER),"
				+ "TO_CHAR(HDR.CUST_TAN), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD1), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD2), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD3),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD4), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD5),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD6), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD7), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD8), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD9),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD10), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD11),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD12), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD13), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD14), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD15),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD16), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD17),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD18), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD19), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD20), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD21),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD22), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD23),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD24), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD25), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD26), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD27),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD28), "
				+ "TO_CHAR(ITM.USERDEFINED_FIELD29),"
				+ "TO_CHAR(ITM.USERDEFINED_FIELD30), "
				+ "TO_CHAR(HDR.SUPPLY_TYPE) AS SUPPLYTYPEASP, "
				+ " '' AS ApproximateDistance_ASP, '' AS DistanceSavedtoEWB, "
				+ "TO_CHAR(HDR.CREATED_BY) as USER_ID, HDR.FILE_ID, "
				+ "FIL.FILE_NAME as FILE_NAME, "
				+ "TO_CHAR(HDR.INV_OTHER_CHARGES) "
				+ "as InvoiceOtherChargesASP, "
				+ "TO_CHAR(HDR.INV_ASSESSABLE_AMT) as "
				+ "InvoiceAssessableAmountASP, TO_CHAR(HDR.INV_IGST_AMT) "
				+ "as InvoiceIGSTAmountASP, TO_CHAR(HDR.INV_CGST_AMT) "
				+ "as InvoiceCGSTAmountASP, TO_CHAR(HDR.INV_SGST_AMT) "
				+ "as InvoiceSGSTAmountASP, TO_CHAR(HDR.INV_CESS_ADVLRM_AMT) "
				+ "as InvoiceCessAdvaloremAmountASP, "
				+ "TO_CHAR(HDR.INV_CESS_SPECIFIC_AMT) "
				+ "as InvoiceCessSpecificAmountASP, "
				+ "TO_CHAR(HDR.INV_STATE_CESS_AMT) "
				+ "as InvoiceStateCessAdvaloremAmountASP, "
				+ "TO_CHAR(HDR.INV_STATE_CESS_SPECIFIC_AMOUNT) "
				+ "as InvoiceStateCessSpecificAmountASP, "
				+ "TO_CHAR(HDR.DOC_AMT), TO_CHAR(ITM.MEMO_VALUE_IGST),"
				+ "TO_CHAR(HDR.MEMO_VALUE_CGST),TO_CHAR(HDR.MEMO_VALUE_SGST), "
				+ "TO_CHAR(ITM.MEMO_VALUE_CESS_ADV) AS "
				+ "CessAdvaloremAmount_ASP, "
				+ "TO_CHAR(ITM.MEMO_VALUE_STATE_CESS_ADV) "
				+ "AS StateCessAdvaloremAmount_ASP, "
				+ "TO_CHAR(ITM.IGST_RET1_IMPACT),"
				+ "TO_CHAR(ITM.CGST_RET1_IMPACT), "
				+ "TO_CHAR(ITM.SGST_RET1_IMPACT), "
				+ "TO_CHAR(ITM.CESS_ADV_RET1_IMPACT) "
				+ "AS CessAdvaloremAmount_Difference, "
				+ "TO_CHAR(ITM.STATE_CESS_ADV_RET1_IMPACT) "
				+ "AS StateCessAdvaloremAmount_Difference, "
				+ " '' AS RECORD_STATUS, TO_CHAR(ITM.ADJ_REF_NO),"
				+ "TO_CHAR(ITM.ADJ_REF_DATE), TO_CHAR(ITM.ADJ_TAXABLE_VALUE),"
				...
				+ "TO_CHAR(HDR.CUST_SUPP_ADDRESS3) FROM "
				+ "ANX_OUTWARD_DOC_HEADER HDR LEFT OUTER JOIN "
				+ "( SELECT DOC_HEADER_ID , ITEM_INDEX , "
				+ "ITM_NO , STRING_AGG(ERROR_CODE_GSTIN,',') "
				+ "ERROR_CODE_GSTIN , STRING_AGG(ERROR_DESCRIPTION_GSTIN,',') "
				+ "ERROR_DESCRIPTION_GSTIN , "
				+ "STRING_AGG(INFO_ERROR_CODE_GSTIN,',') "
				+ "INFO_ERROR_CODE_GSTIN , "
				+ "STRING_AGG(INFO_ERROR_DESCRIPTION_GSTIN,',') "
				+ "INFO_ERROR_DESCRIPTION_GSTIN , "
				+ "STRING_AGG(ERROR_CODE_ASP,',') ERROR_CODE_ASP , "
				+ "STRING_AGG(ERROR_DESCRIPTION_ASP,',') "
				+ "ERROR_DESCRIPTION_ASP , "
				+ "STRING_AGG(INFO_ERROR_CODE_ASP,',') "
				+ "INFO_ERROR_CODE_ASP , "
				+ "STRING_AGG(INFO_ERROR_DESCRIPTION_ASP,',') "
				+ "INFO_ERROR_DESCRIPTION_ASP ,SUPPLIER_GSTIN, "
				+ "FILE_ID, DERIVED_RET_PERIOD FROM "
				+ "( SELECT DOC_HEADER_ID, ITEM_INDEX, ITM_NO, "
				+ " ' 'AS ERROR_CODE_GSTIN,' ' AS ERROR_DESCRIPTION_GSTIN, "
				+ " ' ' AS INFO_ERROR_CODE_GSTIN,' ' AS INFO_ERROR_DESCRIPTION_GSTIN, "
				...
				+ "(CASE WHEN ERROR_TYPE='INFO' THEN "
				+ "ERROR_CODE END) AS INFO_ERROR_CODE_ASP, "
				+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_DESCRIPTION END) "
				+ "AS INFO_ERROR_DESCRIPTION_ASP ,SUPPLIER_GSTIN, "
				+ "FILE_ID, DERIVED_RET_PERIOD FROM ANX_OUTWARD_DOC_ERROR "
				+ "WHERE ITEM_INDEX IS NULL AND ERROR_SOURCE='ASP' "
				+ buildQuery +" ) "
				+ " GROUP BY "
				+ "DOC_HEADER_ID, ITEM_INDEX, ITM_NO,SUPPLIER_GSTIN, "
				+ "FILE_ID, DERIVED_RET_PERIOD ) ASPH_ER "
				+ "ON HDR.ID = ASPH_ER.DOC_HEADER_ID AND "
				+ "ASPH_ER.FILE_ID = HDR.FILE_ID AND "
				+ "ASPH_ER.SUPPLIER_GSTIN = HDR.SUPPLIER_GSTIN "
				+ "AND ASPH_ER.DERIVED_RET_PERIOD = HDR.DERIVED_RET_PERIOD "
				+ "INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON "
				+ "HDR.ID = ITM.DOC_HEADER_ID AND "
				+ "HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD "
				+ "LEFT OUTER JOIN (SELECT DOC_HEADER_ID , "
				+ "ITEM_INDEX , ITM_NO , STRING_AGG(ERROR_CODE_GSTN,',') "
				+ "ERROR_CODE_GSTN , STRING_AGG(ERROR_DESCRIPTION_GSTN,',') "
				+ "ERROR_DESCRIPTION_GSTN , "
				+ "STRING_AGG(INFO_ERROR_CODE_GSTN,',') "
				+ "INFO_ERROR_CODE_GSTN , STRING_AGG(INFO_ERROR_DESCRIPTION_GSTN,',') "
				+ "INFO_ERROR_DESCRIPTION_GSTN , "
				+ "STRING_AGG(ERROR_CODE_ASP,',') ERROR_CODE_ASP , "
				+ "STRING_AGG(ERROR_DESCRIPTION_ASP,',') "
				+ "ERROR_DESCRIPTION_ASP , "
				+ "STRING_AGG(INFO_ERROR_CODE_ASP,',') "
				...
				+ "FROM ( SELECT DOC_HEADER_ID, "
				+ "ITEM_INDEX, ITM_NO, ' 'AS ERROR_CODE_GSTN,"
				+ " ' ' AS ERROR_DESCRIPTION_GSTN, ' ' AS INFO_ERROR_CODE_GSTN,"
				+ " ' ' AS INFO_ERROR_DESCRIPTION_GSTN, (CASE WHEN ERROR_TYPE='ERR' "
				+ "THEN ERROR_CODE END) AS ERROR_CODE_ASP, "
				+ "(CASE WHEN ERROR_TYPE='ERR' THEN ERROR_DESCRIPTION END) "
				+ "AS ERROR_DESCRIPTION_ASP , (CASE WHEN ERROR_TYPE='INFO' "
				+ "THEN ERROR_CODE END) AS INFO_ERROR_CODE_ASP, "
				+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_DESCRIPTION END) "
				+ "AS INFO_ERROR_DESCRIPTION_ASP , SUPPLIER_GSTIN, "
				+ "FILE_ID, DERIVED_RET_PERIOD FROM ANX_OUTWARD_DOC_ERROR "
				+ "WHERE ITEM_INDEX IS NOT NULL AND "
				+ "ERROR_SOURCE='ASP' "
				+ buildQuery +" ) "
				+ " GROUP BY "
				+ "DOC_HEADER_ID, ITEM_INDEX, ITM_NO,SUPPLIER_GSTIN, "
				+ "FILE_ID, DERIVED_RET_PERIOD ) ASPI_ER "
				+ "ON ASPI_ER.FILE_ID = HDR.FILE_ID AND "
				+ "ASPH_ER.FILE_ID = ASPI_ER.FILE_ID "
				+ "AND ASPI_ER.DERIVED_RET_PERIOD= HDR.DERIVED_RET_PERIOD "
				+ "AND HDR.ID = ASPI_ER.DOC_HEADER_ID "
				+ "AND IFNULL(ITM.ITEM_INDEX,'-1') = IFNULL(ASPI_ER.ITEM_INDEX,'-1') "
				+ "AND IFNULL(ITM.ITM_NO,'0') = IFNULL(ASPI_ER.ITM_NO,'0') "
				+ "INNER JOIN FILE_STATUS FIL ON "
				+ "HDR.FILE_ID=FIL.ID LEFT OUTER JOIN "
				+ "GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON "
				+ "GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN "
				+ "EINV_MASTER EINV ON EINV.DOC_HEADER_ID = HDR.ID "
				+ "AND ITM.DOC_HEADER_ID = EINV.DOC_HEADER_ID "
				+ "WHERE HDR.IS_ERROR = TRUE AND "
				+ "HDR.IS_DELETE = FALSE AND HDR.RETURN_TYPE = 'GSTR1' "
				+ "AND ITM.DOC_HEADER_ID IN (SELECT DOC_HEADER_ID "
				+ "FROM ANX_OUTWARD_DOC_ERROR WHERE VAL_TYPE='BV') "
				+ buildHeader
				+ " UNION ALL "
				+ "SELECT TRIM(', ' FROM "
				+ "IFNULL(ASPH_ER.ERROR_CODE_ASP,'') ||','|| "
				...
				+ "EINV.STATUS_DESCRIPTION AS IRN_STATUS, "
				+ "EINV.IRN AS IRN_NO, EINV.ACK_NUM AS IRN_ACK_NO, "
				+ "HDR.IRN_DATE AS IRN_ACK_DATE_TIME, "
				+ "EINV.SIGNED_QR AS SIGNED_QR_CODE, "
				+ " '' AS SIGNED_INVOICE, HDR.EINV_ERROR_CODE "
				+ "AS IRP_ERROR_CODE, HDR.EINV_ERROR_DESC "
				+ "AS IRP_ERROR_DESCRIPTION, case when "
				+ "HDR.EWB_STATUS = 1 then 'Not Applicable' "
				...
				+ "AS EWB_ERROR_CODE, HDR.EWB_ERROR_DESC "
				+ "AS EWB_ERROR_DESCRIPTION, "
				+ " '' AS SAVE_GSTIN_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID "
				+ "AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE "
				+ "AS GSTIN_REFID_DATE_TIME, "
				+ " '' AS GSTIN_ERROR_CODE,'' AS GSTIN_ERROR_DESCRIPTION_ASP, "
				+ " '' RETURN_TYPE, '' TABLE_NUMBER,HDR.IRN, "
				+ "HDR.IRN_DATE,HDR.TAX_SCHEME, HDR.CANCEL_REASON, "
				+ "HDR.CANCEL_REMARKS, ITM.SUPPLY_TYPE, "
				+ "HDR.DOC_CATEGORY, HDR.DOC_TYPE,HDR.DOC_NUM, "
				...
				+ "ITM.LINE_ITEM_AMT, HDR.ROUND_OFF,"
				+ "HDR.TOT_INV_VAL_WORLDS,HDR.TCS_FLAG_INCOME_TAX,"
				...
				+ "HDR.SHIP_BILL_NUM,HDR.SHIP_BILL_DATE,"
				+ "HDR.INV_REMARKS, HDR.INV_PERIOD_START_DATE,"
				...
				+ "HDR.USER_ID AS userid, HDR.COMPANY_CODE,"
				+ "HDR.SOURCE_IDENTIFIER, HDR.SOURCE_FILENAME, "
				+ "ITM.PLANT_CODE,HDR.DIVISION, "
				...
				+ "AS StateCessAdvaloremAmount_Difference, "
				+ "'' AS RECORD_STATUS, ITM.ADJ_REF_NO,ITM.ADJ_REF_DATE, "
				...
				+ "LEFT OUTER JOIN ( SELECT DOC_HEADER_ID , "
				+ "ITEM_INDEX , ITM_NO , STRING_AGG(ERROR_CODE_GSTIN,',') "
				+ "ERROR_CODE_GSTIN , STRING_AGG(ERROR_DESCRIPTION_GSTIN,',') "
				+ "ERROR_DESCRIPTION_GSTIN , STRING_AGG(INFO_ERROR_CODE_GSTIN,',') "
				+ "INFO_ERROR_CODE_GSTIN , "
				+ "STRING_AGG(INFO_ERROR_DESCRIPTION_GSTIN,',') "
				+ "INFO_ERROR_DESCRIPTION_GSTIN , STRING_AGG(ERROR_CODE_ASP,',') "
				...
				+ "INFO_ERROR_DESCRIPTION_ASP ,SUPPLIER_GSTIN, FILE_ID, "
				+ "DERIVED_RET_PERIOD FROM ( SELECT DOC_HEADER_ID, "
				+ "ITEM_INDEX, ITM_NO, ' 'AS ERROR_CODE_GSTIN,' ' "
				+ "AS ERROR_DESCRIPTION_GSTIN, "
				+ " ' ' AS INFO_ERROR_CODE_GSTIN,"
				+ " ' ' AS INFO_ERROR_DESCRIPTION_GSTIN, "
				+ "(CASE WHEN ERROR_TYPE='ERR' THEN ERROR_CODE END) "
				+ "AS ERROR_CODE_ASP, (CASE WHEN ERROR_TYPE='ERR' "
				+ "THEN ERROR_DESCRIPTION END) "
				...
				+ "SUPPLIER_GSTIN, FILE_ID, DERIVED_RET_PERIOD "
				+ "FROM ANX_OUTWARD_DOC_ERROR WHERE ITEM_INDEX IS NULL "
				+ "AND ERROR_SOURCE='ASP' and VAL_TYPE='SV' "
				+ buildQuery +" ) "
				+ " GROUP BY "
				+ "DOC_HEADER_ID, ITEM_INDEX, ITM_NO,SUPPLIER_GSTIN, "
				+ "FILE_ID, DERIVED_RET_PERIOD ) ASPH_ER "
				+ "ON HDR.ID = ASPH_ER.DOC_HEADER_ID "
				+ "AND ASPH_ER.FILE_ID = HDR.FILE_ID "
				+ "AND ASPH_ER.SUPPLIER_GSTIN = HDR.SUPPLIER_GSTIN "
				+ "AND ASPH_ER.DERIVED_RET_PERIOD = HDR.DERIVED_RET_PERIOD "
				+ "INNER JOIN ANX_OUTWARD_ERR_ITEM ITM "
				...
				+ "INFO_ERROR_CODE_GSTN , "
				+ "STRING_AGG(INFO_ERROR_DESCRIPTION_GSTN,',') "
				+ "INFO_ERROR_DESCRIPTION_GSTN , STRING_AGG(ERROR_CODE_ASP,',') "
				+ "ERROR_CODE_ASP , STRING_AGG(ERROR_DESCRIPTION_ASP,',') "
				+ "ERROR_DESCRIPTION_ASP , STRING_AGG(INFO_ERROR_CODE_ASP,',') "
				+ "INFO_ERROR_CODE_ASP , STRING_AGG(INFO_ERROR_DESCRIPTION_ASP,',') "
				+ "INFO_ERROR_DESCRIPTION_ASP , SUPPLIER_GSTIN, FILE_ID, "
				+ "DERIVED_RET_PERIOD FROM ( SELECT DOC_HEADER_ID, ITEM_INDEX, ITM_NO, "
				+ " ' 'AS ERROR_CODE_GSTN,' ' AS ERROR_DESCRIPTION_GSTN, "
				+ " ' ' AS INFO_ERROR_CODE_GSTN,' ' AS INFO_ERROR_DESCRIPTION_GSTN, "
				+ "(CASE WHEN ERROR_TYPE='ERR' "
				+ "THEN ERROR_CODE END) AS ERROR_CODE_ASP, "
				+ "(CASE WHEN ERROR_TYPE='ERR' THEN ERROR_DESCRIPTION END) "
				+ "AS ERROR_DESCRIPTION_ASP , (CASE WHEN ERROR_TYPE='INFO' "
				+ "THEN ERROR_CODE END) AS INFO_ERROR_CODE_ASP, "
				+ "(CASE WHEN ERROR_TYPE='INFO' THEN "
				+ "ERROR_DESCRIPTION END) AS INFO_ERROR_DESCRIPTION_ASP , "
				+ "SUPPLIER_GSTIN, FILE_ID, DERIVED_RET_PERIOD FROM "
				+ "ANX_OUTWARD_DOC_ERROR WHERE ITEM_INDEX IS NOT NULL "
				+ "AND ERROR_SOURCE='ASP' and VAL_TYPE='SV' "
				+ buildQuery +" ) "
				+ "GROUP BY DOC_HEADER_ID, ITEM_INDEX, "
				+ "ITM_NO,SUPPLIER_GSTIN, FILE_ID, DERIVED_RET_PERIOD ) "
				+ "ASPI_ER ON ASPI_ER.FILE_ID = HDR.FILE_ID AND "
				+ "ASPH_ER.FILE_ID = ASPI_ER.FILE_ID AND "
				+ "ASPI_ER.DERIVED_RET_PERIOD= HDR.DERIVED_RET_PERIOD "
				+ "AND ITM.DOC_HEADER_ID = ASPI_ER.DOC_HEADER_ID "
				+ "AND IFNULL(ITM.ITEM_INDEX,'-1') = IFNULL(ASPI_ER.ITEM_INDEX,'-1') "
				+ "AND IFNULL(ITM.ITM_NO,'0') = IFNULL(ASPI_ER.ITM_NO,'0') "
				+ "INNER JOIN FILE_STATUS FIL ON HDR.FILE_ID=FIL.ID "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH "
				+ "ON GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN "
				+ "EINV_MASTER EINV ON EINV.DOC_HEADER_ID = HDR.ID "
				+ "AND ITM.DOC_HEADER_ID = EINV.DOC_HEADER_ID "
				+ "WHERE HDR.IS_ERROR = 'true' AND HDR.IS_DELETE = 'false' "
				+ "AND ITM.DOC_HEADER_ID IN (SELECT DOC_HEADER_ID "
				+ "FROM ANX_OUTWARD_DOC_ERROR WHERE VAL_TYPE='SV') "
				+ buildHeader ;
