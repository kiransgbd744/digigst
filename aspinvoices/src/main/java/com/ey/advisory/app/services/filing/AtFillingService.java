...
@Component("AtFillingService")
/**
 * 
 * @author Siva.Nandam
 *
 */
public class AtFillingService implements FilingService {
	...
	public HsnFillingDto getFilingData(String gstin, String returnPeriod) {
		...
		List<Object[]> doc = atAtaProcRepository.getAt(returnPeriod, gstin);
		Object[] arr = doc.get(0);
		HsnFillingDto at = new HsnFillingDto();
		...
		if (arr[2] == null || arr[2].toString().isEmpty()) {
			...
			at.setTtl_rec((Long) arr[0]);
			at.setTtl_tax((BigDecimal) arr[1]);
			at.setTtl_igst((BigDecimal) arr[2]);
			at.setTtl_cgst((BigDecimal) arr[3]);
			at.setTtl_sgst((BigDecimal) arr[4]);
			at.setTtl_cess((BigDecimal) arr[5]);
