...
@Component("gstr1CdnurCdnuraDataAtGstnImpl")
@Slf4j
public class Gstr1CdnurCdnuraDataAtGstnImpl
		implements Gstr1CdnurCdnuraDataAtGstn {
		...
		if (APIConstants.CDNUR.equals(type)) {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_CDNUR);
			}
		} else {
			params = new APIParams(groupCode, APIProviderEnum.GSTN,
					APIIdentifiers.GSTR1_GET_CDNURA, param1, param2);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("{} section GET API frame work call is initiated",
						APIIdentifiers.GSTR1_GET_CDNURA);
			}

		}
		APIInvocationResult result = apiInvoker.invoke(params, null,
				"Gstr1CdnurCdnuraSuccessHandler",
