...
@Component("Anx1InwardProcessedInfoReportsDaoImpl")
public class Anx1InwardProcessedInfoReportsDaoImpl
		implements Anx1InwardProcessedInfoReportsDao {
	...
	@Override
	public List<Anx1InwardProcessedInfoRecordsDto> getInwardProcessedInfoReports(
		...
		String queryStr = createInwardProcessedInfoQueryString(
				buildQuery.toString());
		...
		return list.stream().map(o -> convertInwardProcessedInfoReports(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Anx1InwardProcessedInfoRecordsDto convertInwardProcessedInfoReports(
			Object[] arr) {
		Anx1InwardProcessedInfoRecordsDto obj = new Anx1InwardProcessedInfoRecordsDto();
	...
	private String createInwardProcessedInfoQueryString(String buildQuery) {
		return "SELECT HDR.ID AS HID,HDR.USER_ID,"
				...
				+ "WHERE IS_ERROR=FALSE " + "AND IS_INFORMATION = TRUE "
				+ buildQuery 
				+ " ORDER BY HDR.ID ";
