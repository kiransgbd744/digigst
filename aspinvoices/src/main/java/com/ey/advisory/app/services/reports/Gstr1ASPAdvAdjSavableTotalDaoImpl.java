...
@Component("Gstr1ASPAdvAdjSavableTotalDaoImpl")
public class Gstr1ASPAdvAdjSavableTotalDaoImpl
		implements Gstr1ASPAdvAdjSavableTotalDao {
	...
	public static void main(String[] args) {
		...
		if (true) {
			buildQueryAdv.append(" AND ADJ.SUPPLIER_GSTIN IN :gstinList");
			buildQueryAdvHdr.append(" AND HDR.SUPPLIER_GSTIN IN :gstinList");

		}

		if (true) {

			buildQueryAdv.append(" AND ADJ.DERIVED_RET_PERIOD = :taxperiod ");
			buildQueryAdvHdr
					.append(" AND HDR.DERIVED_RET_PERIOD = :taxperiod ");

		}

		String queryStr = createAdjSavableTotalQueryString(
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1ASPAdvAdjSavableTotalDaoImpl.class);
	...
	public List<Object> getGstr1AdjSavableReports(SearchCriteria criteria) {
		...
		StringBuilder buildQueryadj = new StringBuilder();
		StringBuilder buildQueryHdr = new StringBuilder();

		if (GSTIN != null && !GSTIN.isEmpty()) {
			if (gstinList != null && gstinList.size() > 0) {
				buildQueryadj.append(" AND ADJ.SUPPLIER_GSTIN IN :gstinList");
				buildQueryHdr.append(" AND HDR.SUPPLIER_GSTIN IN :gstinList");

			}
		}
		if (ProfitCenter != null && !ProfitCenter.isEmpty()) {
			if (pcList != null && pcList.size() > 0) {
				buildQueryadj.append(" AND ADJ.PROFIT_CENTRE IN :pcList");
				buildQueryHdr.append(" AND HDR.PROFIT_CENTRE IN :pcList");
		...
		if (plant != null && !plant.isEmpty()) {
			if (plantList != null && plantList.size() > 0) {
				buildQueryadj.append(" AND ADJ.PLANT_CODE IN :plantList");
				buildQueryHdr.append(" AND HDR.PLANT_CODE IN :plantList");
		...
		if (sales != null && !sales.isEmpty()) {
			if (salesList != null && salesList.size() > 0) {
				buildQueryadj
						.append(" AND ADJ.SALES_ORGANIZATION IN :salesList");
				buildQueryHdr
		...
		if (distChannel != null && !distChannel.isEmpty()) {
			if (distList != null && distList.size() > 0) {
				buildQueryadj
						.append(" AND ADJ.DISTRIBUTION_CHANNEL IN :distList");
				buildQueryHdr
		...
		if (division != null && !division.isEmpty()) {
			if (divisionList != null && divisionList.size() > 0) {
				buildQueryadj.append(" AND ADJ.DIVISION IN :divisionList");
				buildQueryHdr.append(" AND HDR.DIVISION IN :divisionList");

			}
		}
		if (location != null && !location.isEmpty()) {
			if (locationList != null && locationList.size() > 0) {
				buildQueryadj.append(" AND ADJ.LOCATION IN :locationList");
				buildQueryHdr.append(" AND HDR.LOCATION IN :locationList");

			}
		}
		if (ud1 != null && !ud1.isEmpty()) {
			if (ud1List != null && ud1List.size() > 0) {
				buildQueryadj.append(" AND ADJ.USERACCESS1 IN :ud1List");
				buildQueryHdr.append(" AND HDR.USERACCESS1 IN :ud1List");
			}
		}
		if (ud2 != null && !ud2.isEmpty()) {
			if (ud2List != null && ud2List.size() > 0) {
				buildQueryadj.append(" AND ADJ.USERACCESS2 IN :ud2List");
				buildQueryHdr.append(" AND HDR.USERACCESS2 IN :ud2List");
			}
		}
		if (ud3 != null && !ud3.isEmpty()) {
			if (ud3List != null && ud3List.size() > 0) {
				buildQueryadj.append(" AND ADJ.USERACCESS3 IN :ud3List");
				buildQueryHdr.append(" AND HDR.USERACCESS3 IN :ud3List");
			}
		}
		if (ud4 != null && !ud4.isEmpty()) {
			if (ud4List != null && ud4List.size() > 0) {
				buildQueryadj.append(" AND ADJ.USERACCESS4 IN :ud4List");
				buildQueryHdr.append(" AND HDR.USERACCESS4 IN :ud4List");

			}
		}
		if (ud5 != null && !ud5.isEmpty()) {
			if (ud5List != null && ud5List.size() > 0) {
				buildQueryadj.append(" AND ADJ.USERACCESS5 IN :ud5List");
				buildQueryHdr.append(" AND HDR.USERACCESS5 IN :ud4List");

			}
		}
		if (ud6 != null && !ud6.isEmpty()) {
			if (ud6List != null && ud6List.size() > 0) {
				buildQueryadj.append(" AND ADJ.USERACCESS6 IN :ud6List");
				buildQueryHdr.append(" AND HDR.USERACCESS6 IN :ud6List");
			}
		}
		if (taxperiod != null && !taxperiod.isEmpty()) {
			buildQueryadj.append(" AND ADJ.DERIVED_RET_PERIOD = :taxperiod ");
			buildQueryHdr.append(" AND HDR.DERIVED_RET_PERIOD = :taxperiod ");
		}

		String queryStr = createAdjSavableTotalQueryString(
				buildQueryadj.toString(), buildQueryHdr.toString());
		...
		return list.parallelStream().map(o -> convertAdjtotal(o))
				.collect(Collectors.toCollection(ArrayList::new));
	}

	private Gstr1AspVerticalAdvAdjDto convertAdjtotal(Object[] arr) {
	...
	private static String createAdjSavableTotalQueryString(String buildQueryadj,
		...
		return "SELECT SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,MAX(MONTH)MONTH, "
				...
				+ "MAX(ORG_GROSS_ADV_ADJUSTED)ORG_GROSS_ADV_ADJUSTED, "
				+ "NEW_POS,MAX(NEW_RATE)NEW_RATE, "
				+ "SUM(NEW_GROSS_ADV_ADJUSTED)NEW_GROSS_ADV_ADJUSTED, "
				...
				+ "MAX(USERACCESS2)USERACCESS2,MAX(USERACCESS3)USERACCESS3, MAX(USERACCESS4)USERACCESS4, "
				...
				+ "MAX(INFO_ERROR_CODE_ASP)INFO_ERROR_CODE_ASP, MAX(INFO_ERROR_DESCRIPTION_ASP), "
				...
				+ "FROM ( SELECT ADJ.SUPPLIER_GSTIN,ADJ.RETURN_PERIOD, TRAN_TYPE,MONTH,"
				+ "NULL ORG_POS,NULL ORG_RATE, NULL ORG_GROSS_ADV_ADJUSTED, "
				+ "NEW_POS,NULL NEW_RATE,NEW_GROSS_ADV_ADJUSTED,IGST_AMT,CGST_AMT, "
				...
				+ " ''USERDEFINED_FIELD3, '' AS INFO_ERROR_CODE_ASP, "
				+ " '' AS INFO_ERROR_DESCRIPTION_ASP, (CASE WHEN ADJ.IS_SAVED_TO_GSTN = TRUE "
				+ "THEN 'IS_SAVED' WHEN ADJ.IS_SAVED_TO_GSTN = FALSE AND ADJ.GSTN_ERROR = TRUE "
				+ "THEN 'IS_ERROR' WHEN ADJ.IS_SAVED_TO_GSTN = FALSE AND ADJ.GSTN_ERROR = FALSE "
				+ "THEN 'NOT_SAVED' END) AS SAVE_STATUS, GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, "
				+ "GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME, '' AS GSTN_ERROR_CODE, "
				+ " '' AS GSTN_ERROR_DESCRIPTION, ''SOURCE_ID,''FILE_NAME, '' ASP_DATE_TIME "
				+ "FROM GSTR1_PROCESSED_ADV_ADJUSTMENT ADJ LEFT OUTER JOIN "
				+ "TF_GSTR1_ERROR_INFO() GSTRERR ON "
				+ "ADJ.AS_ENTERED_ID= GSTRERR.COMMON_ID AND ADJ.FILE_ID=GSTRERR.FILE_ID "
				+ "AND GSTRERR.TABLE_TYPE='ADV ADJ' AND RETURN_TYPE='GSTR1' "
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON ADJ.FILE_ID=FIL.ID AND "
				+ "GSTRERR.FILE_ID=FIL.ID AND GSTRERR.INV_KEY=ADJ.TXPD_INVKEY "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH ON "
				+ "GSTNBATCH.ID = ADJ.BATCH_ID WHERE ADJ.IS_DELETE=FALSE "
				+ "AND IS_AMENDMENT=FALSE " + buildQueryadj + " UNION ALL "
				+ "SELECT HDR.SUPPLIER_GSTIN, HDR.RETURN_PERIOD, ITM.DIFF_PERCENT AS TRAN_TYPE,"
				+ " '' AS MONTH, NULL ORG_POS, NULL ORG_RATE, NULL AS ORG_TAXABLE_VALUE, HDR.POS AS NEW_POS, "
				+ "NULL NEW_RATE, ITM.TAXABLE_VALUE AS NEW_TAXABLE_VALUE, ITM.IGST_AMT AS IGST_AMT, "
				+ "ITM.CGST_AMT AS CGST_AMT, ITM.SGST_AMT AS SGST_AMT, "
				+ "( ITM.CESS_AMT_SPECIFIC + ITM.CESS_AMT_ADVALOREM ) AS CESS_AMT, "
				+ "HDR.PROFIT_CENTRE, HDR.PLANT_CODE, HDR.DIVISION, HDR.LOCATION, HDR.SALES_ORGANIZATION, "
				...
				+ "HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END ) AS SAVE_STATUS, "
				+ "GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE AS GSTIN_REF_ID_TIME,"
				+ " '' GSTN_ERROR_CODE, '' GSTN_ERROR_DESCRIPTION, '' SOURCE_ID, '' FILE_NAME, '' ASP_DATE_TIME FROM "
				+ "ANX_OUTWARD_DOC_HEADER HDR INNER JOIN ANX_OUTWARD_DOC_ITEM ITM ON HDR.ID = ITM.DOC_HEADER_ID "
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON HDR.FILE_ID = FIL.ID LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH "
				+ "GSTNBATCH ON GSTNBATCH.ID = HDR.BATCH_ID WHERE ASP_INVOICE_STATUS = 2 AND "
				+ "COMPLIANCE_APPLICABLE = TRUE AND HDR.RETURN_TYPE = 'GSTR1' AND HDR.IS_DELETE = FALSE AND "
				+ "HDR.IS_PROCESSED = TRUE AND HDR.TAX_DOC_TYPE='TXP'"
				+ buildQueryHdr + " ) " + " GROUP BY "
				+ "SUPPLIER_GSTIN,RETURN_PERIOD,TRAN_TYPE,NEW_POS "
				+ " UNION ALL "
				+ "SELECT SUPPLIER_GSTIN,RETURN_PERIOD, TRAN_TYPE,MONTH, MAX(ORG_POS)ORG_POS,"
				+ "MAX(ORG_RATE)ORG_RATE, MAX(ORG_GROSS_ADV_ADJUSTED)ORG_GROSS_ADV_ADJUSTED, "
				+ "NEW_POS,MAX(NEW_RATE)NEW_RATE, SUM(NEW_GROSS_ADV_ADJUSTED)NEW_GROSS_ADV_ADJUSTED,"
				+ "SUM(IGST_AMT)IGST_AMT,SUM(CGST_AMT)CGST_AMT, SUM(SGST_AMT)SGST_AMT,"
				+ "SUM(CESS_AMT)CESS_AMT, MAX(PROFIT_CENTRE)PROFIT_CENTRE, "
				+ "MAX(PLANT_CODE)PLANT_CODE,MAX(DIVISION)DIVISION,MAX(LOCATION)LOCATION, "
				+ "MAX(SALES_ORGANIZATION)SALES_ORGANIZATION, "
				...
				+ "MAX(SOURCE_ID)SOURCE_ID, MAX(FILE_NAME)FILE_NAME,"
				+ "MAX(ASP_DATE_TIME)ASP_DATE_TIME "
				+ "FROM ( SELECT ADJ.SUPPLIER_GSTIN, ADJ.RETURN_PERIOD, TRAN_TYPE,"
				+ "MONTH,NULL ORG_POS,NULL ORG_RATE,ORG_GROSS_ADV_ADJUSTED, NEW_POS, "
				+ "NULL NEW_RATE,NEW_GROSS_ADV_ADJUSTED,IGST_AMT,CGST_AMT, SGST_AMT,CESS_AMT, "
				...
				+ " '' FILE_NAME, '' ASP_DATE_TIME FROM GSTR1_PROCESSED_ADV_ADJUSTMENT ADJ "
				+ "LEFT OUTER JOIN TF_GSTR1_ERROR_INFO() GSTRERR "
				+ "ON ADJ.AS_ENTERED_ID= GSTRERR.COMMON_ID AND ADJ.FILE_ID=GSTRERR.FILE_ID "
				+ "AND GSTRERR.TABLE_TYPE='ADV ADJ' AND RETURN_TYPE='GSTR1' "
				+ "LEFT OUTER JOIN FILE_STATUS FIL ON ADJ.FILE_ID=FIL.ID "
				+ "AND GSTRERR.FILE_ID=FIL.ID AND GSTRERR.INV_KEY=ADJ.TXPD_INVKEY "
				+ "LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH "
				+ "ON GSTNBATCH.ID = ADJ.BATCH_ID WHERE ADJ.IS_DELETE=FALSE "
				+ "AND IS_AMENDMENT=TRUE " + buildQueryadj + " ) "
