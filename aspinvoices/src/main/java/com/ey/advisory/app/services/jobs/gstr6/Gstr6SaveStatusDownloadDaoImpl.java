...
@Component("Gstr6SaveStatusDownloadDaoImpl")
public class Gstr6SaveStatusDownloadDaoImpl {
	...
	private List<Gstr6SaveStatusReportsRespDto> fetchSaveSectionsDataByReq(
		...
		if (CollectionUtils.isNotEmpty(gstr6Sections)) {
			buildQuery1.append(" AND CATEGORY IN :gstr6aSections");
		}
	...
	private Gstr6SaveStatusReportsRespDto convertTransactionalLevel(
		...
		obj.setOriginalInvoiceNo(arr[9] != null ? arr[9].toString() : null);
		obj.setOriginalInvoiceDate(arr[10] != null ? arr[10].toString() : null);
		obj.setCRDRPreGST(arr[11] != null ? arr[11].toString() : null);

		obj.setLineNumber(arr[12] != null ? arr[12].toString() : null);
		obj.setSupplierGstin(arr[13] != null ? arr[13].toString() : null);
		obj.setTaxableValue(arr[14] != null ? arr[14].toString() : null);

		obj.setTaxRate(arr[15] != null ? arr[15].toString() : null);
		obj.setIntegratedTaxAmount(arr[16] != null ? arr[16].toString() : null);
		obj.setCentralTaxAmount(arr[17] != null ? arr[17].toString() : null);

		obj.setStateUTTaxAmount(arr[18] != null ? arr[18].toString() : null);
		...
		obj.setReverseChargeFlag(arr[22] != null ? arr[22].toString() : null);
	...
	private String creategstnTransQueryString(String buildQuery,
		...
		return " select CATEGORY, CFS, TAX_PERIOD, REC_GSTIN, DOC_TYPE, DOC_NUMBER, "
				+ "DOC_DATE, ORG_DOC_NUM, ORG_DOC_DATE, ORG_INV_NUM, ORG_INV_DATE, "
				+ "CRDR_PERGST, ITM_NO, SUP_GSTIN, TAXABLE_VALUE, TAX_RATE, IGST_AMT, "
				+ "CGST_AMT, SGST_AMT, CESS_AMT, DOC_AMT, REASON_CR_DE, REV_CHARGE, DIFF_PER "
				+ "from ( SELECT 'B2B' AS CATEGORY,HDR.CFS,TAX_PERIOD,GSTIN AS REC_GSTIN,"
				+ " 'INV' AS DOC_TYPE, HDR.DOC_NUMBER,HDR.DOC_DATE,NULL AS ORG_DOC_NUM,NULL AS ORG_DOC_DATE, "
				+ "NULL AS ORG_INV_NUM,NULL AS ORG_INV_DATE,NULL AS CRDR_PERGST, ITM.ITM_NO,"
				+ "HDR.CTIN AS SUP_GSTIN,ITM.TAXABLE_VALUE,ITM.TAX_RATE, ITM.IGST_AMT,ITM.CGST_AMT,"
				+ "ITM.SGST_AMT,ITM.CESS_AMT, HDR.DOC_AMT, NULL AS REASON_CR_DE, NULL AS REV_CHARGE, "
				+ "NULL AS DIFF_PER FROM GETGSTR6_B2B_HEADER HDR INNER JOIN "
				+ "GETGSTR6_B2B_ITEM ITM ON HDR.ID = ITM.HEADER_ID WHERE IS_DELETE = FALSE "
				+ buildQuery
				+ " UNION SELECT 'B2BA' AS CATEGORY,HDR.CFS,TAX_PERIOD,GSTIN AS REC_GSTIN,"
				+ " 'RNV' AS DOC_TYPE, HDR.DOC_NUMBER,HDR.DOC_DATE, ORG_DOC_NUMBER AS ORG_DOC_NUM , "
				+ "ORG_DOC_DATE, NULL AS ORG_INV_NUM,NULL AS ORG_INV_DATE,NULL AS CRDR_PERGST, "
				+ "ITM.ITM_NO,HDR.CTIN AS SUP_GSTIN,ITM.TAXABLE_VALUE,ITM.TAX_RATE, "
				+ "ITM.IGST_AMT,ITM.CGST_AMT,ITM.SGST_AMT,ITM.CESS_AMT, HDR.DOC_AMT, "
				+ "NULL AS REASON_CR_DE, NULL AS REV_CHARGE, NULL AS DIFF_PER FROM "
				+ " GETGSTR6_B2BA_HEADER HDR INNER JOIN GETGSTR6_B2BA_ITEM ITM "
				+ "ON HDR.ID = ITM.HEADER_ID WHERE IS_DELETE = FALSE "
				+ buildQuery
				+ " UNION SELECT 'CDN' AS CATEGORY,NULL AS CFS,TAX_PERIOD,"
				+ "GSTIN AS REC_GSTIN,HDR.NOTE_TYPE AS DOC_TYPE, "
				+ "HDR.NOTE_NUM AS DOC_NUMBER,HDR.NOTE_DATE AS DOC_DATE, "
				+ "NULL AS ORG_DOC_NUM , NULL AS ORG_DOC_DATE, "
				...
				+ "ITM.SGST_AMT,ITM.CESS_AMT, HDR.DOC_AMT, NULL AS REASON_CR_DE, "
				+ "NULL AS REV_CHARGE, NULL AS DIFF_PER FROM "
				+ " GETGSTR6_CDN_HEADER HDR INNER JOIN "
				+ " GETGSTR6_CDN_ITEM ITM ON HDR.ID = ITM.HEADER_ID "
				+ "WHERE IS_DELETE = FALSE " + buildQuery
				+ " UNION SELECT 'CDNA' AS CATEGORY,NULL AS CFS,TAX_PERIOD,"
				+ "GSTIN AS REC_GSTIN,HDR.NOTE_TYPE AS DOC_TYPE, "
				+ "HDR.NOTE_NUM AS DOC_NUMBER,HDR.NOTE_DATE AS DOC_DATE, "
				+ "HDR.ORG_NOTE_NUM AS ORG_DOC_NUM , "
				+ "HDR.ORG_NOTE_DATE AS ORG_DOC_DATE, DOC_NUMBER AS ORG_INV_NUM,"
				+ "DOC_DATE AS ORG_INV_DATE,NULL AS CRDR_PERGST, ITM.ITM_NO,"
				+ "HDR.CTIN AS SUP_GSTIN,ITM.TAXABLE_VALUE,ITM.TAX_RATE, "
				+ "ITM.IGST_AMT,ITM.CGST_AMT,ITM.SGST_AMT,ITM.CESS_AMT, "
				+ "HDR.DOC_AMT, NULL AS REASON_CR_DE, NULL AS REV_CHARGE, "
				+ "NULL AS DIFF_PER FROM GETGSTR6_CDNA_HEADER HDR INNER JOIN "
				+ "GETGSTR6_CDNA_ITEM ITM ON HDR.ID = ITM.HEADER_ID "
				+ "WHERE IS_DELETE = FALSE " + buildQuery + ")" + " WHERE"
				+ buildQuery1;
