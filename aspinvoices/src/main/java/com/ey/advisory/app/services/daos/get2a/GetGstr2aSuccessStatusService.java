...
@Component("GetGstr2aSuccessStatusService")
public class GetGstr2aSuccessStatusService {

	private static final List<String> HEADER_LIST = Arrays.asList("B2B", "B2BA",
			"CDN", "CDNA", "ISD", "ISDA", "IMPG", "IMPGSEZ", "AMDIMPG","ECOM", "ECOMA");

	@Autowired
	@Qualifier("GetGstr2aSuccessStatusFetchDaoImpl")
	private GetGstr2aSuccessStatusFetchDao getGstr2aSuccessStatusFetchDao;
	...
	public GetGstr6DetailStatusMainDetailsRespDto findByCriteria(
		...
		List<Object[]> gstinList = getGstr2aSuccessStatusFetchDao
		...
		if (!gstinList.isEmpty() && gstinList.size() > 0) {
			for (Object obj : gstinList) {
				...
				List<Object[]> itemsList = getGstr2aSuccessStatusFetchDao
	...
	private void identifyAndEnrichData(GetAnx2DetailsStatusItemsDto dto,
		...
		if (dto != null) {
			...
			case "IMPG": {
				call.setImpgTimeStamp(dto.getCallDatetime());
				call.setImpgStatus(dto.getCallStatus());
				success.setImpgTimeStamp(dto.getSuccessDatetime());
				success.setImpgStatus(dto.getSuccessStatus());
				break;
			}
			case "IMPGSEZ": {
				call.setImpgSezTimeStamp(dto.getCallDatetime());
				call.setImpgSezStatus(dto.getCallStatus());
				success.setImpgSezTimeStamp(dto.getSuccessDatetime());
				success.setImpgSezStatus(dto.getSuccessStatus());
				break;
			}
			case "AMDIMPG": {
				call.setAmdImpgTimeStamp(dto.getCallDatetime());
				call.setAmdImpgStatus(dto.getCallStatus());
				success.setAmdImpgTimeStamp(dto.getSuccessDatetime());
				success.setAmdImpgStatus(dto.getSuccessStatus());
				break;
			}
			case "ECOM": {
				call.setEcomTimeStamp(dto.getCallDatetime());
				call.setEcomStatus(dto.getCallStatus());
				success.setEcomTimeStamp(dto.getSuccessDatetime());
				success.setEcomStatus(dto.getSuccessStatus());
				break;
			}
			case "ECOMA": {
				call.setEcomaTimeStamp(dto.getCallDatetime());
				call.setEcomaStatus(dto.getCallStatus());
				success.setEcomaTimeStamp(dto.getSuccessDatetime());
				success.setEcomaStatus(dto.getSuccessStatus());
				break;
			}
			default: {
