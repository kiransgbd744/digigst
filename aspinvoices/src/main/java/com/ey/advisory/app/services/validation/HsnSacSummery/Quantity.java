...
public class Quantity implements B2csBusinessRuleValidator<Gstr1HsnFileUploadEntity> {
	 private static DecimalFormat df = new DecimalFormat("0.00");
	@Override
	public List<ProcessingResult> validate(Gstr1HsnFileUploadEntity document, ProcessingContext context) {
		
		List<ProcessingResult> errors = new ArrayList<>();
		if(document.getQuentity()!=null){
			
		document.setQuentity(new BigDecimal(df.format(document.getQuentity())));
