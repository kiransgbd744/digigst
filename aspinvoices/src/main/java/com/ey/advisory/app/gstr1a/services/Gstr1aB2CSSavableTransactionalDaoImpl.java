...
import com.ey.advisory.app.services.reports.Gstr1ASPB2CSSavableTransactionalDao;
...
@Component("Gstr1aB2CSSavableTransactionalDaoImpl")
public class Gstr1aB2CSSavableTransactionalDaoImpl
		implements Gstr1ASPB2CSSavableTransactionalDao {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1aB2CSSavableTransactionalDaoImpl.class);
	...
	public List<Object> getGstr1B2CSSavableReports(SearchCriteria criteria) {
		...
		if (ProfitCenter != null && !ProfitCenter.isEmpty()) {
			if (pcList != null && pcList.size() > 0) {
				...
				buildQueryheader.append(" AND ITM.PROFIT_CENTRE IN :pcList");
			}
		}
		if (plant != null && !plant.isEmpty()) {
			if (plantList != null && plantList.size() > 0) {

				buildQueryheader.append(" AND ITM.PLANT_CODE IN :plantList");
			}
		}
		...
		if (division != null && !division.isEmpty()) {
			if (divisionList != null && divisionList.size() > 0) {
				...
				buildQueryheader.append(" AND HDR.DIVISION IN :divisionList");
			}
		}
		if (location != null && !location.isEmpty()) {
			if (locationList != null && locationList.size() > 0) {
				buildQueryheader.append(" AND ITM.LOCATION IN :locationList");
			}
		}
		if (ud1 != null && !ud1.isEmpty()) {
			if (ud1List != null && ud1List.size() > 0) {

				buildQueryheader.append(" AND HDR.USERACCESS1 IN :ud1List");
			}
		}
		if (ud2 != null && !ud2.isEmpty()) {
			if (ud2List != null && ud2List.size() > 0) {

				buildQueryheader.append(" AND HDR.USERACCESS2 IN :ud2List");
			}
		}
		if (ud3 != null && !ud3.isEmpty()) {
			if (ud3List != null && ud3List.size() > 0) {

				buildQueryheader.append(" AND HDR.USERACCESS3 IN :ud3List");
			}
		}
		if (ud4 != null && !ud4.isEmpty()) {
			if (ud4List != null && ud4List.size() > 0) {

				buildQueryheader.append(" AND HDR.USERACCESS4 IN :ud4List");
			}
		}
		if (ud5 != null && !ud5.isEmpty()) {
			if (ud5List != null && ud5List.size() > 0) {

				buildQueryheader.append(" AND HDR.USERACCESS5 IN :ud5List");
			}
		}
		if (ud6 != null && !ud6.isEmpty()) {
			if (ud6List != null && ud6List.size() > 0) {

				buildQueryheader.append(" AND HDR.USERACCESS6 IN :ud6List");
			}
		}
		...
		if (docFromDate != null && docToDate != null) {
			buildQueryheader.append(
					" AND HDR.DOC_DATE BETWEEN :docFromDate AND :docToDate ");
		}

		if (ewbResp != null && ewbResp.equalsIgnoreCase("YES")) {
			buildQueryheader.append(" AND EWB_NO_RESP IS NOT NULL ");
		}
		if (ewbResp != null && ewbResp.equalsIgnoreCase("NO")) {
			buildQueryheader.append(" AND EWB_NO_RESP IS NULL ");

		}
		if (einvGen != null && einvGen.equalsIgnoreCase("YES")) {
			buildQueryheader.append(" AND IRN_RESPONSE IS NOT NULL ");

		}
		if (einvGen != null && einvGen.equalsIgnoreCase("NO")) {
			buildQueryheader.append(" AND IRN_RESPONSE IS NULL ");
		}

		if (tableType != null && tableType.size() > 0) {
			buildQueryheader.append(" AND HDR.TAX_DOC_TYPE IN (:tableType) ");
		}

		if (docType != null && docType.size() > 0) {
			buildQueryheader.append(" AND HDR.DOC_TYPE IN (:docType) ");
	...
	private DataStatusEinvoiceDto convertB2CSSummary(Object[] arr) {
		...
		if ("GENERATED".equalsIgnoreCase(itnStatus)) {
			...
			if (CommonUtility.deriveEinvStatus(irnDate)) {
				...
				obj.setIrnStatus("Generated – Available for cancellation ");
			}
	...
	private String createB2CSSavableTransactionalQueryString(
			String buildQueryheader) {

			return "SELECT '' AS ERROR_CODE_ASP, '' AS ERROR_DESCRIPTION_ASP, TRIM(',' FROM "
			+ "IFNULL(HDR.INFORMATION_CODES,'')||','|| IFNULL(ITM.INFORMATION_CODES,'')) "
			+ "AS INFO_ERROR_CODE_ASP, '' AS INFO_ERROR_DESCRIPTION_ASP, "
			+ "case when HDR.IRN_STATUS = 1 then 'Not Opted' when HDR.IRN_STATUS = 2 then 'Not Applicable' "
			+ "when HDR.IRN_STATUS = 3 then 'Pending (No Action)' when HDR.IRN_STATUS = 4 then 'Generation Error' "
			+ "when HDR.IRN_STATUS = 5 then 'Generated' when HDR.IRN_STATUS = 6 then 'Cancelled' "
			+ "when HDR.IRN_STATUS = 7 then 'DigiGST Error' when HDR.IRN_STATUS = 8 then 'Duplicate_Irn' "
			+ "when HDR.IRN_STATUS = 13 then 'Already Generated by User' "
			+ "when HDR.IRN_STATUS = 11 then 'Pending (Error)' "
			+ "when HDR.IRN_STATUS = 10 "
			+ "then 'Pushed to NIC' end as IRN_STATUS, HDR.IRN_RESPONSE AS IRN_NO, HDR.ACK_NUM AS IRN_ACK_NO, "
			+ "TO_CHAR(HDR.ACK_DATE,'DD-MM-YYYY HH24:MI:SS') AS IRN_ACK_DATE_TIME, EINV.SIGNED_QR AS SIGNED_QR_CODE, "
			+ "NULL AS SIGNED_INVOICE, HDR.EINV_ERROR_CODE AS IRP_ERROR_CODE, HDR.EINV_ERROR_DESC AS IRP_ERROR_DESCRIPTION, "
			+ "case when HDR.EWB_STATUS = 1 then 'Not Applicable' when HDR.EWB_STATUS = 2 then 'Pending (No Action)' when HDR.EWB_STATUS = 3 "
			+ "then 'Generation Error' when HDR.EWB_STATUS = 4 then 'Part A Generated' when HDR.EWB_STATUS = 5 then 'EWB Active' "
			+ "when HDR.EWB_STATUS = 6 then 'Cancelled' when HDR.EWB_STATUS = 7 then 'Discarded' when HDR.EWB_STATUS = 8 "
			+ "then 'Rejected' when HDR.EWB_STATUS = 9 then 'Expired' when HDR.EWB_STATUS = 10 then 'Pushed to NIC' "
			+ "WHEN HDR.EWB_STATUS = 13 then  'Generated as provided by user' "
			+ "WHEN HDR.EWB_STATUS = 14 THEN 'Pending (Error)' "
			+ "when HDR.EWB_STATUS = 11 then 'DigiGST Error' WHEN HDR.EWB_STATUS = 12 then 'Not_Opted' end as EWBPartAStatusA, "
			+ "TO_CHAR(EWB.VALID_UPTO,'DD-MM-YYYY HH24:MI:SS') AS EWBValidUpTo, HDR.EWAY_BILL_NUM AS EWBNO, "
			+ "CAST(HDR.EWAY_BILL_DATE AS DATE) AS EWBDATE, HDR.EWB_ERROR_CODE AS EWB_ERROR_CODE, HDR.EWB_ERROR_DESC AS EWB_ERROR_DESCRIPTION, "
			+ "case when HDR.IS_SAVED_TO_GSTN = TRUE THEN 'IS_SAVED' WHEN HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = TRUE "
			+ "THEN 'IS_ERROR' WHEN HDR.IS_SAVED_TO_GSTN = FALSE AND HDR.GSTN_ERROR = FALSE THEN 'NOT_SAVED' END AS GSTIN_STATUS, "
			+ "GSTNBATCH.GSTN_SAVE_REF_ID AS GSTIN_REF_ID, GSTNBATCH.BATCH_DATE AS GSTIN_REFID_DATE_TIME, NULL AS GSTIN_ERROR_CODE, "
			+ "NULL AS GSTIN_ERROR_DESCRIPTION_ASP, HDR.RETURN_TYPE AS RET_TYPE, HDR.TABLE_SECTION AS TABLE_NUMBER, HDR.IRN, "
			+ "HDR.IRN_DATE,HDR.TAX_SCHEME, HDR.CANCEL_REASON, HDR.CANCEL_REMARKS, ITM.SUPPLY_TYPE, HDR.DOC_CATEGORY, HDR.DOC_TYPE, "
			+ "HDR.DOC_NUM,HDR.DOC_DATE, HDR.REVERSE_CHARGE, HDR.SUPPLIER_GSTIN,HDR.SUPP_TRADE_NAME, HDR.SUPP_LEGAL_NAME, "
			+ "HDR.SUPP_BUILDING_NUM, HDR.SUPP_BUILDING_NAME AS SUPPLIERADDRESS2, HDR.SUPP_LOCATION, HDR.SUPP_PINCODE, HDR.SUPP_STATE_CODE,"
			+ "HDR.SUPP_PHONE, HDR.SUPP_EMAIL, HDR.CUST_GSTIN,HDR.CUST_TRADE_NAME, HDR.CUST_SUPP_NAME, HDR.CUST_SUPP_ADDRESS1,"
			+ " HDR.CUST_SUPP_ADDRESS2, HDR.CUST_SUPP_ADDRESS4, HDR.CUST_PINCODE,HDR.BILL_TO_STATE, HDR.POS, HDR.CUST_PHONE,HDR.CUST_EMAIL,"
			+ "HDR.DISPATCHER_GSTIN, HDR.DISPATCHER_TRADE_NAME,HDR.DISPATCHER_BUILDING_NUM, HDR.DISPATCHER_BUILDING_NAME,"
			+ "HDR.DISPATCHER_LOCATION, HDR.DISPATCHER_PINCODE,HDR.DISPATCHER_STATE_CODE , HDR.SHIP_TO_GSTIN ,HDR.SHIP_TO_TRADE_NAME, "
			+ "HDR.SHIP_TO_LEGAL_NAME,HDR.SHIP_TO_BUILDING_NUM, HDR.SHIP_TO_BUILDING_NAME,HDR.SHIP_TO_LOCATION, ITM.SHIP_TO_PINCODE,"
			+ "ITM.SHIP_TO_STATE,ITM.ITM_NO, ITM.SERIAL_NUM2,ITM.PRODUCT_NAME,ITM.ITM_DESCRIPTION, ITM.IS_SERVICE,ITM.ITM_HSNSAC,ITM.BAR_CODE, "
			+ "ITM.BATCH_NAME_OR_NUM,ITM.BATCH_EXPIRY_DATE, ITM.WARRANTY_DATE, ITM.ORDER_ITEM_REFERENCE, ITM.ATTRIBUTE_NAME, "
			+ "ITM.ATTRIBUTE_VALUE, ITM.ORIGIN_COUNTRY,ITM.ITM_UQC_USER,ITM.QTY_USER, ITM.FREE_QTY, ITM.UNIT_PRICE, ITM.ITEM_AMT_UP_QTY, "
			+ "ITM.ITEM_DISCOUNT,ITM.PRE_TAX_AMOUNT,ITM.TAXABLE_VALUE, ITM.IGST_RATE, ITM.IGST_AMT,ITM.CGST_RATE,ITM.CGST_AMT, "
			+ "ITM.SGST_RATE,ITM.SGST_AMT,ITM.CESS_RATE_ADVALOREM, ITM.CESS_AMT_ADVALOREM, ITM.CESS_RATE_SPECIFIC, ITM.CESS_AMT_SPECIFIC, "
			+ "ITM.STATECESS_RATE,ITM.STATECESS_AMT, ITM.STATE_CESS_SPECIFIC_RATE, ITM.STATE_CESS_SPECIFIC_AMOUNT, ITM.OTHER_VALUES,"
			+ "TOT_ITEM_AMT, ITM.INV_OTHER_CHARGES, ITM.INV_ASSESSABLE_AMT,ITM.INV_IGST_AMT,ITM.INV_CGST_AMT, ITM.INV_SGST_AMT,"
			+ "ITM.INV_CESS_ADVLRM_AMT, ITM.INV_CESS_SPECIFIC_AMT,ITM.INV_STATE_CESS_AMT, ITM.INV_STATE_CESS_SPECIFIC_AMOUNT, "
			+ "CASE WHEN HDR.DATAORIGINTYPECODE IN ('A','AI') THEN HDR.DOC_AMT WHEN HDR.DATAORIGINTYPECODE IN ('E','EI') "
			+ "THEN ITM.LINE_ITEM_AMT END LINE_ITEM_AMT, HDR.ROUND_OFF,HDR.TOT_INV_VAL_WORLDS,HDR.TCS_FLAG_INCOME_TAX, "
			+ "ITM.TCS_RATE_INCOME_TAX,ITM.TCS_AMOUNT_INCOME_TAX, HDR.CUSTOMER_PAN_OR_AADHAAR, HDR.FOREIGN_CURRENCY, "
			+ "HDR.COUNTRY_CODE, HDR.INV_VAL_FC, HDR.SHIP_PORT_CODE, HDR.SHIP_BILL_NUM, HDR.SHIP_BILL_DATE, HDR.INV_REMARKS, "
			+ "HDR.INV_PERIOD_START_DATE, HDR.INV_PERIOD_END_DATE, ITM.PRECEEDING_INV_NUM, ITM.PRECEEDING_INV_DATE, "
			+ "ITM.INV_REFERENCE, ITM.RECEIPT_ADVICE_REFERENCE, ITM.RECEIPT_ADVICE_DATE, ITM.TENDER_REFERENCE , "
			+ "ITM.CONTRACT_REFERENCE, ITM.EXTERNAL_REFERENCE, ITM.PROJECT_REFERENCE, ITM.CUST_PO_REF_NUM, ITM.CUST_PO_REF_DATE,"
			+ "HDR.PAYEE_NAME, HDR.MODE_OF_PAYMENT,HDR.BRANCH_IFSC_CODE, HDR.PAYMENT_TERMS, HDR.PAYMENT_INSTRUCTION,"
			+ "HDR.CR_TRANSFER, HDR.DB_DIRECT, HDR.CR_DAYS,ITM.PAID_AMT,ITM.BAL_AMT, HDR.PAYMENT_DUE_DATE, "
			+ "HDR.ACCOUNT_DETAIL,HDR.ECOM_GSTIN, HDR.ECOM_TRANS_ID, ITM.SUPPORTING_DOC_URL, ITM.SUPPORTING_DOC_BASE64, "
			+ "ITM.ADDITIONAL_INFORMATION, HDR.TRANS_TYPE,HDR.SUB_SUPP_TYPE, HDR.OTHER_SUPP_TYPE_DESC,HDR.TRANSPORTER_ID, "
			+ "HDR.TRANSPORTER_NAME, HDR.TRANSPORT_MODE,HDR.TRANSPORT_DOC_NUM, HDR.TRANSPORT_DOC_DATE, "
			+ "HDR.DISTANCE ,HDR.VEHICLE_NUM, HDR.VEHICLE_TYPE, HDR.RETURN_PERIOD,HDR.ORIGINAL_DOC_TYPE, "
			+ "HDR.ORIGINAL_CUST_GSTIN, HDR.DIFF_PERCENT, HDR.SECTION7_OF_IGST_FLAG,HDR.CLAIM_REFUND_FLAG, "
			+ "HDR.AUTOPOPULATE_TO_REFUND,HDR.CRDR_PRE_GST, HDR.CUST_SUPP_TYPE, HDR.CUST_SUPP_CODE AS CUSTOMERCODE, "
			+ "ITM.PRODUCT_CODE,ITM.ITM_TYPE, ITM.ITC_FLAG, HDR.STATE_APPLYING_CESS,ITM.FOB, ITM.EXPORT_DUTY, "
			+ "HDR.EXCHANGE_RATE,ITM.CRDR_REASON, HDR.TCS_FLAG , ITM.TCS_AMT,TCS_CGST_AMT,TCS_SGST_AMT, "
			+ "HDR.TDS_FLAG,TDS_IGST_AMT,TDS_CGST_AMT,TDS_SGST_AMT, HDR.USER_ID AS userid,HDR.COMPANY_CODE, HDR.SOURCE_IDENTIFIER, "
			+ "HDR.SOURCE_FILENAME, ITM.PLANT_CODE,HDR.DIVISION, SUB_DIVISION, ITM.LOCATION,HDR.SALES_ORGANIZATION, "
			+ "HDR.DISTRIBUTION_CHANNEL, ITM.PROFIT_CENTRE,HDR.PROFIT_CENTRE2, HDR.USERACCESS1, HDR.USERACCESS2,HDR.USERACCESS3, "
			+ "HDR.USERACCESS4, HDR.USERACCESS5,HDR.USERACCESS6, ITM.GLCODE_TAXABLEVALUE,ITM.GLCODE_IGST,ITM.GLCODE_CGST, ITM.GLCODE_SGST,"
			+ "ITM.GLCODE_ADV_CESS,ITM.GLCODE_SP_CESS, ITM.GLCODE_STATE_CESS, HDR.GL_STATE_CESS_SPECIFIC, HDR.GL_POSTING_DATE,"
			+ "HDR.SALES_ORD_NUM,HDR.EWAY_BILL_NUM, CAST(HDR.EWAY_BILL_DATE AS DATE), HDR.ACCOUNTING_VOUCHER_NUM, "
			+ "HDR.ACCOUNTING_VOUCHER_DATE, ITM.DOCUMENT_REFERENCE_NUMBER, HDR.CUST_TAN, ITM.USERDEFINED_FIELD1,ITM.USERDEFINED_FIELD2, "
			+ "ITM.USERDEFINED_FIELD3,ITM.USERDEFINED_FIELD4, ITM.USERDEFINED_FIELD5,ITM.USERDEFINED_FIELD6, ITM.USERDEFINED_FIELD7,"
			+ "ITM.USERDEFINED_FIELD8, ITM.USERDEFINED_FIELD9,ITM.USERDEFINED_FIELD10, ITM.USERDEFINED_FIELD11,ITM.USERDEFINED_FIELD12, "
			+ "ITM.USERDEFINED_FIELD13,ITM.USERDEFINED_FIELD14, ITM.USERDEFINED_FIELD15,ITM.USERDEFINED_FIELD16, "
			+ "ITM.USERDEFINED_FIELD17,ITM.USERDEFINED_FIELD18, ITM.USERDEFINED_FIELD19, ITM.USERDEFINED_FIELD20, ITM.USERDEFINED_FIELD21,"
			+ "ITM.USERDEFINED_FIELD22, ITM.USERDEFINED_FIELD23,ITM.USERDEFINED_FIELD24, ITM.USERDEFINED_FIELD25, ITM.USERDEFINED_FIELD26, "
			+ "ITM.USERDEFINED_FIELD27,ITM.USERDEFINED_FIELD28, ITM.USERDEFINED_FIELD29,ITM.USERDEFINED_FIELD30, HDR.SUPPLY_TYPE "
			+ "AS SUPPLYTYPEASP, EWB. ASP_DISTANCE AS ApproximateDistance_ASP, EWB.REMAINING_DISTANCE AS DistanceSavedtoEWB, "
			+ "HDR.CREATED_BY as USER_ID,HDR.FILE_ID, FIL.FILE_NAME as FILE_NAME,HDR.INV_OTHER_CHARGES as InvoiceOtherChargesASP, "
			+ "HDR.INV_ASSESSABLE_AMT as InvoiceAssessableAmountASP, HDR.INV_IGST_AMT AS InvoiceIGSTAmountASP,HDR.INV_CGST_AMT "
			+ "as InvoiceCGSTAmountASP, HDR.INV_SGST_AMT as InvoiceSGSTAmountASP,HDR.INV_CESS_ADVLRM_AMT "
			+ "as InvoiceCessAdvaloremAmountASP, HDR.INV_CESS_SPECIFIC_AMT as InvoiceCessSpecificAmountASP, HDR.INV_STATE_CESS_AMT "
			+ "as InvoiceStateCessAdvaloremAmountASP, HDR.INV_STATE_CESS_SPECIFIC_AMOUNT AS InvoiceStateCessSpecificAmountASP, "
			+ "HDR.DOC_AMT, HDR.MEMO_VALUE_IGST,HDR.MEMO_VALUE_CGST, HDR.MEMO_VALUE_SGST, ITM.MEMO_VALUE_CESS_ADV "
			+ "AS CessAdvaloremAmount_ASP, ITM.MEMO_VALUE_STATE_CESS_ADV AS StateCessAdvaloremAmount_ASP, ITM.IGST_RET1_IMPACT, "
			+ "ITM.CGST_RET1_IMPACT,ITM.SGST_RET1_IMPACT, ITM.CESS_ADV_RET1_IMPACT AS CessAdvaloremAmount_Difference, "
			+ "ITM.STATE_CESS_ADV_RET1_IMPACT AS StateCessAdvaloremAmount_Difference, CASE WHEN HDR.IS_DELETE = TRUE "
			+ "THEN 'INACTIVE' WHEN HDR.IS_DELETE = FALSE THEN 'ACTIVE'END AS RECORD_STATUS, "
			+ "TO_CHAR(EINV.CANCELLATION_DATE,'DD-MM-YYYY HH24:MI:SS') AS IRN_CANCELLATION_DATE, "
			+ "TO_CHAR(EWB.CANCELLATION_DATE,'DD-MM-YYYY HH24:MI:SS') AS EWB_CANCELLATION_DATE, CASE WHEN HDR.ASP_INVOICE_STATUS=1 "
			+ "THEN 'DigiGST Error' WHEN HDR.ASP_INVOICE_STATUS=2 THEN 'DigiGST Processed' END AS DIGI_GST_STATUS, HDR.INFO_ERROR_CODE "
			+ "AS EWB_INFO_CODE, HDR.INFO_ERROR_MSG AS EWB_INFO_ERROR_MSG, HDR.EWB_NO_RESP AS EWB_NUM, "
			+ "TO_CHAR(HDR.EWB_DATE_RESP,'DD-MM-YYYY HH24:MI:SS') AS EWB_DATE_TIME, ITM.ADJ_REF_NO,ITM.ADJ_REF_DATE, "
			+ "ITM.ADJ_TAXABLE_VALUE, ITM.ADJ_IGST_AMT,ITM.ADJ_CGST_AMT, ITM.ADJ_SGST_AMT, ITM.ADJ_CESS_AMT_ADVALOREM, "
			+ "ITM.ADJ_CESS_AMT_SPECIFIC, ITM.ADJ_STATECESS_AMT, ITM.TCS_AMT AS Tcsamount, HDR.CUST_SUPP_ADDRESS3, "
			+ "HDR.TAX_DOC_TYPE AS DATA_CATEGORY,ITM.ITM_UQC,ITM.ITM_QTY FROM ANX_OUTWARD_DOC_HEADER_1A HDR INNER JOIN ANX_OUTWARD_DOC_ITEM_1A ITM "
			+ "ON HDR.ID = ITM.DOC_HEADER_ID AND HDR.DERIVED_RET_PERIOD = ITM.DERIVED_RET_PERIOD LEFT OUTER JOIN FILE_STATUS FIL "
			+ "ON HDR.FILE_ID=FIL.ID LEFT OUTER JOIN GSTR1_GSTN_SAVE_BATCH GSTNBATCH "
			+ "ON GSTNBATCH.ID = HDR.BATCH_ID LEFT OUTER JOIN EINV_MASTER EINV "
			+ "ON HDR.IRN_RESPONSE = EINV.IRN LEFT OUTER JOIN EWB_MASTER EWB ON HDR.EWB_NO_RESP = EWB.EWB_NUM "
			+ " WHERE HDR.ASP_INVOICE_STATUS = 2 AND COMPLIANCE_APPLICABLE=TRUE AND HDR.TAX_DOC_TYPE IN ('B2CS') "
			+ buildQueryheader
			+ "AND HDR.IS_DELETE=FALSE AND HDR.RETURN_TYPE='GSTR1A'";
		}
