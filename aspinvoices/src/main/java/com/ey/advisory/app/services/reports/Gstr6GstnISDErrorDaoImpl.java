...
import com.ey.advisory.repositories.client.Gstr1BatchRepository;
	...
	@Component("Gstr6GstnISDErrorDaoImpl")
	public class Gstr6GstnISDErrorDaoImpl implements Gstr6GstnErrorDao {

		private static final Logger LOGGER = LoggerFactory.getLogger(Gstr6GstnISDErrorDaoImpl.class);
		...
		public List<Object> getGstr6GstnErrorReport(SearchCriteria criteria) {
			...
			return list.parallelStream().map(o -> convertgstnISDError(o)).collect(Collectors.toCollection(ArrayList::new));
		}

		private Gstr6ISDGstnErrorDto convertgstnISDError(Object[] arr) {
			Gstr6ISDGstnErrorDto obj = new Gstr6ISDGstnErrorDto();
			...
			obj.setRecipeintGSTIN(arr[5] != null ? arr[5].toString() : null);
			obj.setStateCode(arr[6] != null ? arr[6].toString() : null);
			obj.setiGSTasIGST(arr[7] != null ? arr[7].toString() : null);
			obj.setiGSTasCGST(arr[8] != null ? arr[8].toString() : null);
			obj.setiGSTasSGST(arr[9] != null ? arr[9].toString() : null);
			obj.setcGSTasIGST(arr[10] != null ? arr[10].toString() : null);
			obj.setcGSTasCGST(arr[11] != null ? arr[11].toString() : null);
			obj.setsGSTasIGST(arr[12] != null ? arr[12].toString() : null);
			obj.setsGSTasSGST(arr[13] != null ? arr[13].toString() : null);
		...
		private String creategstnB2BErrorQueryString(String buildQuery) {
			...
			return "select DOC_TYPE,DOC_NUM,DOC_DATE,ORG_DOC_NUM,ORG_DOC_DATE,"
					+ "CUST_GSTIN ,STATE_CODE, "
					+ "SUM(IFNULL(IGST_AMT_AS_IGST,0)+IFNULL(CGST_AMT_AS_IGST,0)"
					+ " +IFNULL(SGST_AMT_AS_IGST,0)) AS IGST_AMT_AS_IGST, "
					+ "0 AS IGST_AMT_AS_CGST, "
					+ "0 AS IGST_AMT_AS_SGST, "
					+ "0 AS CGST_AMT_AS_IGST, "
					+ "SUM( IFNULL(IGST_AMT_AS_CGST,0)+IFNULL(CGST_AMT_AS_CGST,0)) "
					+ "AS CGST_AMT_AS_CGST, "
					+ "0 AS SGST_AMT_AS_IGST, "
					...
					+ "GSTR1_GSTN_SAVE_BATCH BT ON "
					+ "HDR.BATCH_ID = BT.ID WHERE HDR.IS_ERROR = TRUE "
					+ "AND HDR. DOC_TYPE IN ('INV','CR') " +buildQuery
					+ "GROUP BY "
					+ "DOC_TYPE,DOC_NUM,DOC_DATE,ORG_DOC_NUM,ORG_DOC_DATE,"
					+ "CUST_GSTIN ,STATE_CODE,BT.GSTN_SAVE_REF_ID,BT.CREATED_ON, "
