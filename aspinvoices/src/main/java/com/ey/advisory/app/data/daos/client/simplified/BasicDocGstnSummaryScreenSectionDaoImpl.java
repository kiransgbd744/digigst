 ...
 *
 */
@Component("BasicDocGstnSummaryScreenSectionDaoImpl")
public class BasicDocGstnSummaryScreenSectionDaoImpl
		implements BasicGstr1DocGstnSummaryScreenSectionDao {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(BasicDocGstnSummaryScreenSectionDaoImpl.class);
	...
	private Gstr1SummarySectionDto convert(Object[] arr) {
		...
		// obj.setRecords(((BigInteger) arr[1]).intValue());
	...
	private String createQueryString(String buildQuery) {
		...
		String queryStr = "SELECT TAX_DOC_TYPE,SUM(RECORD_COUNT) RECORD_COUNT,"
				...
				+ "FROM GETGSTR1_RATE_SUMMARY WHERE "
	...
	public List<Gstr1SummaryNilSectionDto> loadBasicSummarySectionNil(
		...
		if (!Strings.isNullOrEmpty(request.getReturnType())
				&& APIConstants.GSTR1A
						.equalsIgnoreCase(request.getReturnType())) {
			queryStr = createQueryStringNilForGstr1a(buildQuery1);

		} else {
			queryStr = createQueryStringNil(buildQuery1);
	 ...
	 * @return
	 */
	private String createQueryStringNil(String buildQuery1) {
		// TODO Auto-generated method stub
		...
		String queryStr = "SELECT 'NILEXTNON' SECTION_NAME, "
				...
				+ "FROM GETGSTR1_NIL_SUMMARY WHERE IS_DELETE = FALSE AND "
				+ buildQuery1;
		return queryStr;
	}
	...
	public List<Gstr1SummaryDocSectionDto> loadBasicSummaryDocSection(
		...
		if (!Strings.isNullOrEmpty(request.getReturnType())
				&& APIConstants.GSTR1A
						.equalsIgnoreCase(request.getReturnType())) {
			queryStr = createQueryDocStringForGstr1a(buildQuery);

		} else {
			queryStr = createQueryDocString(buildQuery);
	 ...
	 * @return
	 */
	private String createQueryDocString(String buildQuery) {
		// TODO Auto-generated method stub
		...
		String queryStr = "SELECT 'DOC_ISSUE' AS TAX_DOC_TYPE,"
				...
				+ "FROM GETGSTR1_DOCISSUED_SUMMARY "
