 ...
 *
 */
@Component("CewbErrorReportsDaoImpl")
@Slf4j
public class CewbErrorReportsDaoImpl implements Gstr1VerticalReportsDao {
	...
	private String createVerticalErrorQueryString(String buildQuery) {
		...
		if ((GSTConstants.CEWB).equalsIgnoreCase(fileType)) {
			...
			builder.append(
					"FROM CEWB_AS_ENTERED HI LEFT OUTER JOIN  TF_VERTICAL_ERROR_INFO() ERR ON ");
			builder.append(
					"HI.ID = ERR.COMMON_ID  AND HI.FILE_ID=ERR.FILE_ID LEFT OUTER JOIN ");
			builder.append(
					"FILE_STATUS FIL ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID ");
			builder.append(
					"AND ERR.INV_KEY=HI.DOC_KEY WHERE HI.IS_ERROR = TRUE AND ");
