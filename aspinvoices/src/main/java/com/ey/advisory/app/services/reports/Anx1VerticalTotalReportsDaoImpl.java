...
import com.ey.advisory.app.data.views.client.Anx13H3IVerticalTotalDto;
import com.ey.advisory.app.data.views.client.Anx1B2CVerticalTotalDto;
import com.ey.advisory.app.data.views.client.Anx1Table4VerticalTotalDto;
 ...
 *
 */
@Component("Anx1VerticalTotalReportsDaoImpl")
public class Anx1VerticalTotalReportsDaoImpl
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Anx1VerticalTotalReportsDaoImpl.class);
	...
	public List<Object> getVerticalReports(
		...
		String queryStr = createVerticalTotalQueryString(buildb2cQuery.toString(), 
		...
		LOGGER.debug("ResultSet reading " + list);
	...
	private Object convertVerticalProcessed(Object[] arr) {
		...
		Anx1B2CVerticalTotalDto obj1 = new Anx1B2CVerticalTotalDto();
		Anx13H3IVerticalTotalDto obj2 = new Anx13H3IVerticalTotalDto();
		Anx1Table4VerticalTotalDto obj3 = new Anx1Table4VerticalTotalDto();
		if ((DownloadReportsConstant.B2C).equalsIgnoreCase(fileType)) {
			...
			obj1.setAspErrorCode(arr[41] != null ? arr[41].toString() : null);
			obj1.setAspErrorDesc(arr[42] != null ? arr[42].toString() : null);
			obj1.setAspInformationCode(arr[43] != null ? arr[43].toString() : null);
			obj1.setAspInformationDesc(arr[44] != null ? arr[44].toString() : null);
			obj = obj1;
		}
		if ((DownloadReportsConstant.TABLE3H3I).equalsIgnoreCase(fileType)) {
			...
			obj2.setAspInformationCode(arr[39] != null ? arr[39].toString() : null);
			obj2.setAspInformationDesc(arr[40] != null ? arr[40].toString() : null);
			obj2.setAspErrorCode(arr[37] != null ? arr[37].toString() : null);
			obj2.setAspErrorDesc(arr[38] != null ? arr[38].toString() : null);
		...
		if ((DownloadReportsConstant.TABLE4).equalsIgnoreCase(fileType)) {
			...
			obj3.setAspErrorCode(arr[26] != null ? arr[26].toString() : null);
			obj3.setAspErrorDesc(arr[27] != null ? arr[27].toString() : null);
			obj3.setAspInformationCode(arr[28] != null ? arr[28].toString() : null);
			obj3.setAspInformationDesc(arr[29] != null ? arr[29].toString() : null);
			obj = obj3;
	...
	private String createVerticalTotalQueryString(String buildb2cQuery,
		...
		if ((DownloadReportsConstant.B2C).equalsIgnoreCase(fileType)) {
			queryStr = "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "TRIM(', ' FROM IFNULL(STRING_AGG(A_ERROR_CODE,',' ),'') ) " 
                    + "AS ASP_ERROR_CODE,"
                    + "TRIM(', ' FROM IFNULL(STRING_AGG(A_ERROR_DESCRIPTION,',' ),'') ) "
                    + "AS ASP_ERROR_DESCRIPTION,"
					...
					+ "STATE_CESS_AMT,TOTAL_VALUE,PROFIT_CENTER,PLANT,DIVISION,"
					+ "LOCATION,SALES_ORG,DISTRIBUTION_CHANNEL,"
					+ "USER_ACCESS1,USER_ACCESS2,USER_ACCESS3,USER_ACCESS4,"
					+ "USER_ACCESS5,USER_ACCESS6,USER_DEFINED1,USER_DEFINED2,USER_DEFINED3,"
					+ "(CASE WHEN ERROR_TYPE='ERROR' THEN ERROR_CODE END) "
					+ "AS A_ERROR_CODE,(CASE WHEN ERROR_TYPE='ERROR' "
					+ "THEN ERROR_DESCRIPTION END) AS A_ERROR_DESCRIPTION,"
					+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_CODE END) "
					+ "AS A_INFO_CODE,(CASE WHEN ERROR_TYPE='INFO' "
					+ "THEN ERROR_DESCRIPTION END)AS A_INFO_DESCRIPTION FROM "
					+ "ANX_AS_ENTERED_B2C HI LEFT OUTER JOIN "
					+ "ANX_VERTICAL_ERROR ERR ON  HI.ID= ERR.COMMON_ID "
					+ "AND HI.FILE_ID= ERR.FILE_ID LEFT OUTER JOIN FILE_STATUS "
					+ "FIL ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID "
					+ "AND ERR.INV_KEY=HI.B2C_INVKEY WHERE "
					+ buildb2cQuery + ")"
					+ " GROUP BY "
			...
			queryStr = "SELECT RETURN_TYPE,CUST_GSTIN,RETURN_PERIOD,TRAN_FLAG,"
					...
					+ "AVAIL_CGST,AVAIL_SGST,AVAIL_CESS,PROFIT_CENTER,"
					+ "PLANT,DIVISION,LOCATION,"
                    ...
                    + "TRIM(', ' FROM IFNULL(STRING_AGG(A_INFO_DESCRIPTION,',' ),'') ) "
                    + "AS ASP_INFO_DESCRIPTION "
					+ "FROM (SELECT HI.ID,RETURN_TYPE,CUST_GSTIN,RETURN_PERIOD,"
					+ "TRAN_FLAG,SUPPLIER_GSTIN_PAN,SUPPLIER_NAME,"
					+ "DIFF_PERCENT,SEC7_OF_IGST_FLAG,AUTOPOPULATE_TO_REFUND,"
					+ "POS,HSNORSAC,TAXABLE_VALUE,TAX_RATE,IGST_AMT,CGST_AMT,"
					+ "SGST_AMT,CESS_AMT,"
					+ "TOTAL_VALUE,ELGBL_INDICATOR,AVAIL_IGST,"
					+ "AVAIL_CGST,AVAIL_SGST,AVAIL_CESS,PROFIT_CENTER,"
					+ "PLANT,DIVISION,LOCATION,PURCHAGE_ORG,USER_ACCESS1,"
					+ "USER_ACCESS2,USER_ACCESS3,USER_ACCESS4,USER_ACCESS5,"
					+ "USER_ACCESS6,USERDEFINED1,USERDEFINED2,USERDEFINED3,"
					+ "(CASE WHEN ERROR_TYPE='ERROR' THEN ERROR_CODE END) "
					+ "AS A_ERROR_CODE,(CASE WHEN ERROR_TYPE='ERROR' "
					+ "THEN ERROR_DESCRIPTION END) AS A_ERROR_DESCRIPTION,"
					+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_CODE END) AS A_INFO_CODE,"
					+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_DESCRIPTION END) "
					+ "AS A_INFO_DESCRIPTION FROM ANX_AS_ENTERED_3H_3I "
					...
					+ "AND ERR.FILE_ID=FIL.ID AND ERR.INV_KEY=HI.INVKEY_3H_3I WHERE "
					+ build3h3iQuery + ")"
					+ " GROUP BY "
					...
					+ "TAXABLE_VALUE,TAX_RATE,IGST_AMT,CGST_AMT,SGST_AMT,"
					+ "CESS_AMT,TOTAL_VALUE,ELGBL_INDICATOR,AVAIL_IGST,"
					+ "AVAIL_CGST,AVAIL_SGST,AVAIL_CESS,"
					+ "PROFIT_CENTER,PLANT,DIVISION,LOCATION,"
					+ "PURCHAGE_ORG,USER_ACCESS1,USER_ACCESS2,USER_ACCESS3,"
					+ "USER_ACCESS4,USER_ACCESS5,USER_ACCESS6,USERDEFINED1,"
					+ "USERDEFINED2,USERDEFINED3";
			...
			queryStr = "SELECT RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					...
					+ "DISTRIBUTION_CHANNEL,USER_ACCESS1,"
					+ "USER_ACCESS2,USER_ACCESS3,USER_ACCESS4,USER_ACCESS5,"
                    ...
                    + "TRIM(', ' FROM IFNULL(STRING_AGG(A_INFO_DESCRIPTION,',' ),'') ) "
                    + "AS ASP_INFO_DESCRIPTION "
					+ "FROM (SELECT HI.ID,RETURN_TYPE,"
					+ "SUPPLIER_GSTIN,RETURN_PERIOD,ECOM_GSTIN,"
					+ "ECOM_VAL_SUPMADE,ECOM_VAL_SUPRET,ECOM_NETVAL_SUP,"
					...
					+ "USER_DEFINED3,"
					+ "(CASE WHEN ERROR_TYPE='ERROR' "
					+ "THEN ERROR_CODE END) AS A_ERROR_CODE,"
					+ "(CASE WHEN ERROR_TYPE='ERROR' "
					+ "THEN ERROR_DESCRIPTION END) AS A_ERROR_DESCRIPTION,"
					+ "(CASE WHEN ERROR_TYPE='INFO' THEN ERROR_CODE END) "
					+ "AS A_INFO_CODE,(CASE WHEN ERROR_TYPE='INFO' "
					+ "THEN ERROR_DESCRIPTION END)AS A_INFO_DESCRIPTION "
					+ "FROM ANX_AS_ENTERED_TABLE4 HI LEFT OUTER JOIN "
					+ "ANX_VERTICAL_ERROR ERR ON  HI.ID= ERR.COMMON_ID AND "
					+ "HI.FILE_ID= ERR.FILE_ID LEFT OUTER JOIN FILE_STATUS FIL "
					+ "ON HI.FILE_ID=FIL.ID AND ERR.FILE_ID=FIL.ID AND "
					+ "ERR.INV_KEY=HI.TABLE4_INVKEY WHERE "
					+ buildtable4Query + ")"
					+ " GROUP BY "
					+ "ID,RETURN_TYPE,SUPPLIER_GSTIN,RETURN_PERIOD,"
					+ "ECOM_GSTIN,ECOM_VAL_SUPMADE,ECOM_VAL_SUPRET,"
