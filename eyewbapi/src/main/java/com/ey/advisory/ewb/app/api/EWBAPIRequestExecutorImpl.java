 ...
 *         Error Scenarios received either from NIC or Gateway
 */
@Slf4j
@Component
public class EWBAPIRequestExecutorImpl implements EWBAPIRequestExecutor {
		...
		while (true) {
			try {
				if (LOGGER.isDebugEnabled())
					LOGGER.debug("Hitting EWB API {} for {} time", reqUrl,
							(retryCount + 1));
				...
				pair = ex.getRespPair();
				LOGGER.error(
						"We have received Invalid Response from EWB API,"
								+ " Response is {},"
								+ " Hence About retry EWB API {} for {} time",
	...
	private Pair<String, Integer> executeApi(HttpUriRequest req)
		...
		try {
			...
			LOGGER.error("Error While invoking EWB API,", e);
		...
		if (isRetryable)
			throw new PerformRetryException(pair);
		if (LOGGER.isDebugEnabled())
			LOGGER.debug("EWB has Returned Valid Response - {}", apiResp);
		return pair;
