...
@Slf4j
@Component("CompressAndZipXlsxFiles")
public class CompressAndZipXlsxFiles {
	...
	private static String getZipOutputFileName(Long configId) {
		...
		if (configId == 1L) {
			...
			fileName = "Gstr3bSaveSubmit" + "_" + timeMilli;
		} else if (configId == 2L) {
			DateTimeFormatter dtf = DateTimeFormatter
					.ofPattern("yyyyMMddHHmmss");
			String timeMilli = dtf.format(LocalDateTime.now());
			fileName = "VendorComplianceSummary" + "_" + timeMilli;
		} else if (configId == 3L) {
			DateTimeFormatter dtf = DateTimeFormatter
					.ofPattern("yyyyMMddHHmmss");
			String timeMilli = dtf.format(LocalDateTime.now());
			fileName = "CustomerComplianceSummary" + "_" + timeMilli;
		} else if (configId == 4L) {
			DateTimeFormatter dtf = DateTimeFormatter
					.ofPattern("yyyyMMddHHmmss");
			String timeMilli = dtf.format(LocalDateTime.now());
			fileName = "MyComplianceSummary" + "_" + timeMilli;
		} else if (configId == 5L) {
			DateTimeFormatter dtf = DateTimeFormatter
					.ofPattern("yyyyMMddHHmmss");
			String timeMilli = dtf.format(LocalDateTime.now());
			fileName = "VendorComplianceHistory" + "_" + timeMilli;
		} else if (configId == 6L) {
			DateTimeFormatter dtf = DateTimeFormatter
					.ofPattern("yyyyMMddHHmmss");
			String timeMilli = dtf.format(LocalDateTime.now());
			fileName = "CustomerComplianceHistory" + "_" + timeMilli;
		} else if (configId == 7L) {
			DateTimeFormatter dtf = DateTimeFormatter
					.ofPattern("yyyyMMddHHmmss");
			String timeMilli = dtf.format(LocalDateTime.now());
			fileName = "MyComplianceHistory" + "_" + timeMilli;
		} else {
			fileName = "ReconReports" + "_" + configId;
		}
		return fileName;
	}
