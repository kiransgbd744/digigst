...
@Slf4j
@RestController
public class Gstr6SaveToGstnApiJobInsertionController {
	@Autowired
	...
	public ResponseEntity<String> createGstr6SaveToGstnJob(
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"gstr6SaveToGstnJob Request received from UI as {} ",
						jsonParam);
			}
			...
			if (gstr1Pairs != null && !gstr1Pairs.isEmpty()) {
				for (int i = 0; i < gstr1Pairs.size(); i++) {
					...
					if (lastJobStatusCode != null && lastJobStatusCode < 30) {
						...
						json.addProperty("msg", "GSTR6 is Inprogress Already.");
						respBody.add(json);
						continue;
					}*/
					if (!gstnUserRequestUtil.isNextSaveRequestEligible(
							pair.getValue0(), pair.getValue1(), "SAVE", "GSTR6",
							groupCode)) {
						JsonObject json = new JsonObject();
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"GSTR6 SAVE is Inprogress, New request Cannot be taken.");
						respBody.add(json);
						continue;
					}
					...
					Long userRequestId = gstnUserRequestUtil
							.createGstnUserRequest(pair.getValue0(),
									...
									APIConstants.GSTR6.toUpperCase(), groupCode,
									userName, false, false,
									dto.getIsCrossItcUserInput());
					...
					asyncJobsService.createJob(groupCode,
							JobConstants.GSTR6_SAVETOGSTN, eachJson.toString(),
