...
import com.ey.advisory.app.services.docs.einvoice.DailyOutwardReconService;
...
@RestController
@Slf4j
public class DailyOutwardReconController {

	@Autowired
	@Qualifier("DailyOutwardReconServiceImpl")
	private DailyOutwardReconService dailyOutwardReconService;
	...
	@PostMapping(value = "/api/dailyOutwardRecon", produces = {
	...
	public ResponseEntity<String> dailyOutwardReconAPi(ServletRequest request,
		...
		try {
			...
			dailyOutwardReconService.persistData(payloadData);
			...
			LOGGER.error("dailyOutwardRecon method exception ", ex);
			resp.add("hdr",
	...
	@PostMapping(value = "/ui/getDailyReconResults.do", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getDailyReconResults(
		...
		try {
			...
			List<DailyOutwardAndInwardReconDto> dailyReconResp = dailyOutwardReconService
					.getData(request);
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"end of dailyOutwardReconService.getData(request)" + "{}",
					resp.toString());
		}
