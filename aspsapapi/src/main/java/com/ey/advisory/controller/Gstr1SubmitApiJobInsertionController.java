...
@RestController
@Slf4j
public class Gstr1SubmitApiJobInsertionController {
	...
	@PostMapping(value = "/ui/Gstr1GstnSubmit", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> createGstr1GstnSubmitJob(
			@RequestBody String request) {

		JsonObject resp = new JsonObject();
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr1GstnSubmit Request received from UI as {} ",
						request);
			}
			...
			for (Gstr1SubmitGstnDto dto : dtos) {
				...
				try {
					...
					List<GstnSubmitEntity> activeSubmitRequests = gstr1SubmitRepo
					...
					if (activeSubmitRequests == null
						...
						job = asyncJobsService.createJob(groupCode,
								JobConstants.GSTR1_GSTN_SUBMIT, jsonParam,
						...
						JsonObject json = new JsonObject();
						msg = "Gstr1 has Already submitted.";
						json.addProperty("gstin", gstin);
						json.addProperty("msg", msg);
						respBody.add(json);
						LOGGER.error(msg + " for {} ", gstin);
					...
					json.addProperty("msg",
							"SUBMIT GSTR1  Request initiated Successfully");
					respBody.add(json);
	...
	private Gstr1SubmitGstnDto createBatchAndSave(String groupCode,
		...
		entity.setReturnType(APIConstants.GSTR1.toUpperCase());
		entity.setStatus(APIConstants.INITIATED.toUpperCase());
		entity.setCreatedBy(userName);
		entity.setCreatedOn(now);
		// Save new Batch
		entity = gstr1SubmitRepo.save(entity);
