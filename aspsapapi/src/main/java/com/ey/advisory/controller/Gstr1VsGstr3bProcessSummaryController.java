...
@Slf4j
@RestController
public class Gstr1VsGstr3bProcessSummaryController {
	...
	@RequestMapping(value = "/ui/gstr1VsGstr3bProcessSummary", method = RequestMethod.POST, produces = {
	...
	public ResponseEntity<String> getProcessedRecords(
		...
		try {
			...
			String msg = "Unexpected error while retriving Gstr1vs3b processed records";
			LOGGER.error(msg, ex);
			JsonObject resp = new JsonObject();
			resp.add("hdr", new Gson().toJsonTree(new APIRespDto("E", msg)));
			return new ResponseEntity<>(resp.toString(),
