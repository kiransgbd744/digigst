...
import com.ey.advisory.app.data.services.pdf.Gstr6AspSummaryPDFGenerationReportImpl;
import com.ey.advisory.app.data.services.pdf.Gstr6MultiplePdfZipReportService;
...
@Slf4j
@RestController
public class Gstr6PdfReportController {
	...
	// original
	/*
	 * @PostMapping(value = "/ui/gstr6PdfReport", produces = {
	 * MediaType.APPLICATION_JSON_VALUE }) public void
	 * generateGstr6PdfReport(@RequestBody String jsonString,
	 * HttpServletResponse response) throws Exception {
	 * 
	 * String fileName = null; InputStream inputStream = null; Gson gson =
	 ...
	 * 
	 * try { File tempDir = createTempDir();
	 * 
	 * 
	 * String zipFileName = zipReportService .generateGstr6PdfZip(tempDir,
	 * reqDto);
	 * 
	 * fileName = "GSTR6PDFReports"; // LocalDateTime reqRTime =
	 * LocalDateTime.now();
	 * 
	 * String recTime = getStandardTime(LocalDateTime.now());
	 * 
	 * 
	 * // String recTime = reqRTime.toString(); String reqReceivedTime =
	...
	public void generateGstr6PdfReport(@RequestBody String jsonString,
			HttpServletResponse response) throws Exception {
		...
		for (String gstn : gstinList) {
			...
			try {
				...
				LOGGER.error(
						"Exception while generating Gstr1SummaryReport PDF ",
						ex);
			}
	...
	@PostMapping(value = "/ui/gstr6PdfReportAsync", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> generateGstr6PdfReportAsync(
		...
		try {
			...
			entity.setDataType("GSTR 6");
			entity.setReportCateg("GSTR 6");
			entity.setReportType(ReportTypeConstants.GSTR6_PDF);
			...
			asyncJobsService.createJob(groupCode,
					JobConstants.GSTR6_BULK_PDF_DOWNLOAD, jobParams.toString(),
			...
			LOGGER.error("Exception while generating Bulk Gstr6 PDF ", e);
