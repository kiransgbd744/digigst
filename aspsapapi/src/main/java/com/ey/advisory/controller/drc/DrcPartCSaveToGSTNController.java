...
import com.ey.advisory.app.data.services.drc01c.DrcPartCSaveToGSTN;
...
@RestController
@Slf4j
public class DrcPartCSaveToGSTNController {
	
	@Autowired
	@Qualifier("DrcPartCSaveToGSTNImpl")
	DrcPartCSaveToGSTN drcService;
	
	private static final String GSTIN = "gstins";

	@PostMapping(value = "/ui/drcPartCSaveBulk", produces = {
	...
	public ResponseEntity<String> saveToGstn(
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				String msg = "Begin DrcPartBCSaveToGSTNController"
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = "End DrcPartBCSaveToGSTNController"
