...
@Slf4j
@RestController
public class Gstr1SummaryPollingController {
	...
	@PostMapping(value = "/ui/gstr1GetSummary", produces = {
	...
	@PostMapping(value = "/ui/gstr1BulkGetSumm", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	...
	@GetMapping(value = "/ui/downloadGstr1GetSumErrResp", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	...
	private void downloadGstr1GetSummErrDetails(String refId,
		...
		try {
		 ...
		 LOGGER.debug("An error occurred while closing writer", e);
		        }
		    } if (inputStream != null) {
		        try {
		            inputStream.close();
	...
	@PostMapping(value = "/ui/fetchGstr1GetSummary", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	...
	@PostMapping(value = "/ui/fetchGstr1BulkGetSumm", produces = {
			MediaType.APPLICATION_JSON_VALUE })
			...
			for (JsonElement gstin : gstinArray) {
				...
				if (gstinAuthMap.get(gstin.getAsString()).equalsIgnoreCase("I")) {
					...
					asyncJobsService.createJob(groupCode,
							JobConstants.GSTR1_GET_SUMMARY, jobj.toString(),
	...
	private Pair<String, String> doGetCalls(String gstin, String returnPeriod,
		...
		Gstr1SaveBatchEntity existingEntry = saveStatusRepo
				.findGstr1PollingRefId(gstin, returnPeriod);
		...
		if (LOGGER.isDebugEnabled()) {
			String msg = String
					.format("GSTR1 Generate summary (PTF) Gstn Call initiated"
		...
		if ("Success".equalsIgnoreCase(apiStatus)) {
			responseMsg = String.format(
					"GSTR1 Generate Summary (PTF) has been"
	...
	private Pair<String, String> saveRefId(String gstin, String ret_period,
		...
		if (resp.isSuccess()) {
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"GSTR1 Generate Summary (PTF) Gstn Call Completed"
			...
			saveStatusRepo.softDeleteGstr1PollingEntries(gstin, ret_period);
			...
			LOGGER.error(
					"Generate GSTR1 summary failed for gstin{} and taxPeriod{}."
