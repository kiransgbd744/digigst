...
import com.ey.advisory.app.data.services.drc01c.DRC01CSaveStatus;
...
@RestController
@Slf4j
public class DRC1CSaveStatusController {

	@Autowired
	@Qualifier("DRC01CSaveStatusImpl")
	DRC01CSaveStatus saveStatus;
	
	@Autowired
	@Qualifier("DRC01CSignAndFileServiceImpl")
	DRC01CSignAndFileService fileStatus;
	
	@PostMapping(value = "/ui/getDrc01CSaveStatus", produces = {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Begin save status in DRC01C with gstin : %s , taxPeriod"
						+ ": %s",gstin, taxPeriod);
				LOGGER.debug(msg);
			}

			List<Gst3BSaveStatusDto> saveStatusList = saveStatus
	...
	@PostMapping(value = "/ui/getDrc01CFilingStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getFilingDetails(
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String
						.format("Begin get Filing status in DRC01C with gstin"
								+ " : %s , taxPeriod"
								+ ": %s", gstin, taxPeriod);
				LOGGER.debug(msg);
			}

			List<Gstr3BFilingDetailsDTO> fileStatusList = fileStatus
					.getDrc01CFilingDetails(gstin, taxPeriod);
