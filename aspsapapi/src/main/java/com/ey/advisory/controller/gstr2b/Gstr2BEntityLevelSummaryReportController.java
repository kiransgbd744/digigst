...
import com.ey.advisory.app.gstr2b.summary.Gstr2BSummaryReportDownloadService;
...
@Slf4j
@RestController
public class Gstr2BEntityLevelSummaryReportController {

	@Autowired
	@Qualifier("Gstr2BSummaryReportDownloadService")
	private Gstr2BSummaryReportDownloadService service;

	@PostMapping(value = "/ui/gstr2BSuammaryReport", produces = {
			MediaType.APPLICATION_JSON_VALUE })

	public void getSummaryReport(@RequestBody String jsonString,
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Begin Gstr2BEntityLevelSummaryReportController"
							+ ".getSummaryReport() method, "
							+ "Parsing Input request %s ",jsonString);
				LOGGER.debug(msg);
			}
			...
			Workbook workbook = service.writeToExcel(gstnsList, fromTaxPeriod,
					toTaxPeriod);
			
			String fileName = DocumentUtility.getUniqueFileName(
					ConfigConstants.GSTR2B_SUMMARY_REPORT);
			...
			String msg = "Error occured while generating GSTR 2B Entity "
