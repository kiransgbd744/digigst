...
@RestController
@Slf4j
public class Gstr2XGetApiJobInsertionController {
	...
	@PostMapping(value = "/ui/Gstr2xGstnGetSection", produces = {
			MediaType.APPLICATION_JSON_VALUE })

	public ResponseEntity<String> createGstr2XGstnGetJob(
			@RequestBody String request) {
		JsonObject resp = new JsonObject();
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Gstr2XGstnGetSection Request received from UI as {} ",
						request);
			}
			...
			return gstr2XGetCallservice(dtos);
		} catch (Exception ex) {
			String msg = "Unexpected error while creating async jobs for GSTR2X Get ";
			LOGGER.error(msg,ex);
	...
	private JsonArray getAllActiveGstnList(List<Gstr1GetInvoicesReqDto> dtos) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Getting all the active GSTNs for GSTR2X GET");
		}
		String msg = "";
		...
		if (dtos != null && !dtos.isEmpty()) {
			...
			for (Gstr1GetInvoicesReqDto dto : dtos) {
				...
				dto.getGstr2XSections().replaceAll(String::toUpperCase);
	...
	private Gstr1GetInvoicesReqDto createBatchAndSave(String groupCode,
		...
		batchRepo.softlyDelete(dto.getType().toUpperCase(),
				APIConstants.GSTR2X.toUpperCase(), dto.getGstin(),
				dto.getReturnPeriod());
		GetAnx1BatchEntity batch = batchUtil.makeBatchGstr1(dto, dto.getType(),
				APIConstants.GSTR2X.toUpperCase());
		// Save new Batch
	...
	public ResponseEntity<String> gstr2XGetCallservice(
		...
		for (Gstr1GetInvoicesReqDto dto : dtos) {
			...
			Boolean isFailedGet = dto.getIsFailed() == null ? false
					: dto.getIsFailed();
			dto.setType(APIConstants.TCSANDTDS);
			List<String> gstr2XSections = dto.getGstr2XSections();
			...
			if (isFailedGet == true) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("GSTR2X Failed GET Request received");
				}
				...
				failedBatchs = batchRepo.findBatchByStatus(dto.getGstin(),
						dto.getReturnPeriod(),
						APIConstants.GSTR2X.toUpperCase(), failedStatus);
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"GSTR2x Failed GET eligible sections are {} ",
							failedBatchs);
				}
			} else {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("GSTR2x All GET Request received");
				}
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"GSTR2x All GET Not eligible sections are {} ",
							initAndInProgressStatus);
				}
			}
			...
			Long userRequestId = gstnUserRequestUtil.createGstnUserRequest(
					...
					APIConstants.GSTR2X.toUpperCase(), groupCode, userName, false, false, false);
			dto.setUserRequestId(userRequestId);

			if (isEligible(isFailedGet, failedBatchs,
					initAndInprogressBatchs, APIConstants.TCSANDTDS,
					gstr2XSections)) {
				dto.setType(APIConstants.TCSANDTDS.toUpperCase());
				dto = createBatchAndSave(groupCode, dto);
				jsonParam = gson.toJson(dto);
				job = asyncJobsService.createJob(groupCode,
						JobConstants.GSTR2X_GSTN_GET_SECTION, jsonParam,
				...
				APIParams params = new APIParams(groupCode, APIProviderEnum.GSTN,
						APIIdentifiers.GSTR2X_GET_TCS_TDS, param1, param2);
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Job Created for Gstr2X Get as {} ", job);
				}
			}
			...
			json.addProperty("msg",
					"Get GSTR2X  Request Initiated Successfully");
			respBody.add(json);
