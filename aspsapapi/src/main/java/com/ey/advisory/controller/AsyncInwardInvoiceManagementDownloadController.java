...
@RestController
@Slf4j
public class AsyncInwardInvoiceManagementDownloadController {
	...
	@Autowired
	@Qualifier("AsyncInwardInvManagementReportHandlerImpl")
	private AsyncInvManagementReportHandler asyncInvManagementReportHandler;

	/*@Autowired
	@Qualifier("AsyncInvManagementReportHandlerImpl")
	private AsyncInvManagementReportHandlerImpl asyncInwardInvAccVoucherNoReportHandlerImpl;
*/
	@PostMapping(value = "/ui/downloadInwardInvoiceManagementCsvReports")
	public ResponseEntity<String> ReportProcess(
		...
		try {
			...
			Optional<String> optionalAccVoucherNo = Optional
					.ofNullable(reqDto.getAccVoucherNo());
			List<String> accVoucherNums = reqDto.getAccVoucherNum();
			...
			if (accVoucherNumLength > 2000) {
			    ...
			    String msg = "Accounting voucher Numbers have exceeded the limit of 2000 characters";
...
//doc num validation
			...
			int docNumtotal = docNo.stream().mapToInt(String::length).sum();
			if (docNumtotal > 2000) {
				APIRespDto dto = new APIRespDto("Failed",
			...
			asyncInvManagementReportHandler
			...
			asyncJobsService.createJob(groupCode,
					JobConstants.INWARD_INVOICE_MANAGEMENT_DOWNLOAD_REPORT,
			...
			String reportType = getReportType(reqDto.getProcessingStatus());
	...
	private String getReportType(String processingStatus) {
		...
		try {
			...
			case "P":
				reportType = "Inward_InvoiceMgt_ProcessedRecords";
				break;

			case "E":
				reportType = "Inward_InvoiceMgt_ErrorRecords";
				break;

			case "I":
				reportType = "Inward_InvoiceMgt_ProcessedInfoData";
				break;

			case "totalData":
				reportType = "Inward_InvoiceMgt_data";
				break;
	...
	@GetMapping(value = "/ui/inwardInvoiceManagementDownloadDocument")
