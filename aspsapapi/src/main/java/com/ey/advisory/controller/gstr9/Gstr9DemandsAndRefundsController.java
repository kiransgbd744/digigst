...
import com.ey.advisory.app.data.services.gstr9.Gstr9DemandsAndRefundServie;
import com.ey.advisory.app.docs.dto.gstr9.Gstr9DemandAndrefundDashboardDto;
import com.ey.advisory.app.docs.dto.gstr9.Gstr9DemandsAndRefundsSaveUserInputDto;
...
@RestController
@Slf4j
public class Gstr9DemandsAndRefundsController {

	@Autowired
	@Qualifier("Gstr9DemandsAndRefundServieImpl")
	private Gstr9DemandsAndRefundServie service;

	@PostMapping(value = "/ui/getGstr9DemandsAndRefunds", produces = {
	...
	public ResponseEntity<String> getGstr9TaxPaidData(
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("INSIDE Gstr9DemandsAndRefundsController");
			}
			...
			List<Gstr9DemandAndrefundDashboardDto> respData = service
					.getGstr9DemandsAndRefundDetails(gstin, taxPeriod, formattedFy);
	...
	@PostMapping(value = "/ui/saveGstr9DemandAndRefundUserInputData", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> saveGstr9DemandsAndRefundsUserInputData(
			@RequestBody String jsonString) {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("INSIDE saveGstr9DemandsAndRefundsUserInputData");
			}
			JsonObject reqObject = (new JsonParser()).parse(jsonString)
					.getAsJsonObject();
			JsonObject reqJson = reqObject.get("req").getAsJsonObject();

			Gstr9DemandsAndRefundsSaveUserInputDto userInput = gson.fromJson(
					reqJson, Gstr9DemandsAndRefundsSaveUserInputDto.class);
			...
			if (Strings.isNullOrEmpty(gstin)
					|| Strings.isNullOrEmpty(formattedFy)) {
			...
			String resp = service.saveGstr9DemandsAndRefundsUserInputData(gstin,
					formattedFy, status, userInput.getUserInputList());
			...
			String msg = "Unexpected error while saving Gstr9 Demands and "
					+ "Refunds Data ";
			LOGGER.error(msg, ex);
			JsonObject errorResp = new JsonObject();
			errorResp.add("hdr", gson.toJsonTree(APIRespDto.creatErrorResp()));
