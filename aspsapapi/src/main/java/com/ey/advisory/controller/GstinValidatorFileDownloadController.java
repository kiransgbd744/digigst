...
@Slf4j
@RestController
public class GstinValidatorFileDownloadController {
	
	@Autowired
	@Qualifier("GstinValidatorConfigRepository")
	GstinValidatorConfigRepository gstinValidatorRepo;
	
	
	@GetMapping(value = "/ui/downloadGstinValidatorExcelDocument")
	public void ExcelDownload(HttpServletRequest request,
		...
		GstinValidatorEntity configEntity = gstinValidatorRepo
		...
		/*Document document = DocumentUtility.downloadDocument(sendFileName,
				fileFolder);*/
		String docId = configEntity.getDocId();
		Document document = null;
		if (Strings.isNullOrEmpty(docId)) {
			document = DocumentUtility.downloadDocument(sendFileName,
					fileFolder);
		} else {
