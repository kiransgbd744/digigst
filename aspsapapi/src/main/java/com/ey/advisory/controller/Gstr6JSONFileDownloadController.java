...
@RestController
@Slf4j
public class Gstr6JSONFileDownloadController {
	...
	@RequestMapping(value = "/ui/gstr6JsonDownloadDocument", method = RequestMethod.POST, produces = {
		...
		try {
			...
			if (Strings.isNullOrEmpty(returnPeriod)
				...
				if (Strings.isNullOrEmpty(returnPeriod)
					...
					Gstr1SaveBatchEntity entity = repo
							.findByGstinAndReturnPeriodAndRefIdAndSection(
									supplierGstin, returnPeriod, refId,
									"ER","GSTR6");
				...
				if (tempDir.list().length > 0) {
					...
					zipFileName = combineAndZipCsvFiles.zipfolder(
							taxPeriodValue, tempDir, supplierGstin, "GSTR6",
							"_" + section);
