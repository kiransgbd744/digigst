...
@Slf4j
@RestController
public class SignAndFileStage2Controller {
	...
	@PostMapping(value = "/signreturn/execSignAndFileStage2", produces = {
	...
	public ResponseEntity<String> execSignAndFileStage2(String username,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"Received GSTR1 Sign&File Request for ID -{}"
		...
		try {
			...
			if (opEntity.isPresent()) {
				...
				String errMsg = String.format(
						"GSTR1 Sign & File is not initiated for id %s", appkey);
				LOGGER.error(errMsg);
			...
			persistenceMngr.createJob(groupCode, "GSTR1SignAndFile",
			...
			LOGGER.error("Exception while GSTR1 Sign&FIle Stage2 ", ex);
