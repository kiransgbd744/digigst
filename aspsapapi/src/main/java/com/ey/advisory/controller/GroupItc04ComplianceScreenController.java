...
@RestController
@Slf4j
public class GroupItc04ComplianceScreenController {
	...
	@PostMapping(value = "/ui/getGroupItc04Compliance", produces = {
	...
	public ResponseEntity<String> fetchProcessedRecords(
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("itc04 groupComplienceHistory request{}");
		}
		...
		try {
			...
			String msg = "Unexpected error while retrieving Itc04 compliance records";
			LOGGER.error(msg, ex);
			JsonObject resp = new JsonObject();
			resp.add("hdr", new Gson().toJsonTree(new APIRespDto("E", msg)));
			return new ResponseEntity<>(resp.toString(),
