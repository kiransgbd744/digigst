...
import com.ey.advisory.app.services.docs.einvoice.TransactionalOutwardService;
...
@Slf4j
@RestController
public class TransactionalOutwardController {

	@Autowired
	private TransactionalOutwardService transactionalOutwardService;
	...
	@PostMapping(value = "/api/TransactionalOutward", produces = {
	...
	public ResponseEntity<String> dailyOutwardReconAPi(ServletRequest request,
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("TransactionalOutward request payload is {}", jsonString);
			}
			...
			if (payloadData != null) {
				transactionalOutwardService.persistData(
			...
			LOGGER.error("TransactionalOutward method exception ", ex);
			resp.add("hdr",
	...
	@PostMapping(value = "/ui/getTransactionalOutwardDocs", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getDailyReconResults(
		...
		try {
			...
			TransactionalReportingDto transResp = transactionalOutwardService
					.getData(request);
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"end of TransactionalOutward.getData(request)" + "{}",
					resp.toString());
		}
