...
import com.ey.advisory.app.services.reports.Ret1ProcessedScreenReportHandler;
 ...
 */
@RestController
public class Ret1ProcessedSumScreendDownloadController {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Ret1ProcessedSumScreendDownloadController.class);

	@Autowired
	@Qualifier("Ret1ProcessedScreenReportHandler")
	private Ret1ProcessedScreenReportHandler ret1ProcessedScreenReportHandler;

	@Autowired
	@Qualifier("BasicCommonSecParamRSReports")
	BasicCommonSecParamRSReports basicCommonSecParamRSReports;

	@RequestMapping(value = "/ui/downloadRet1ProcessedReport", method = RequestMethod.POST, produces = {
		...
		try {
			...
			if (criteria.getType() != null
					&& criteria.getType().equalsIgnoreCase(
							DownloadReportsConstant.RET1PROCESSEDSUMMARY)) {
				Ret1ProcessedRecordsRequestDto dto = new Ret1ProcessedRecordsRequestDto();
				...
				dto.setGstins(setDataSecurity.getGstins());
				workbook = ret1ProcessedScreenReportHandler
						.findProcessedScreen(dto);
				fileName = "RET-1_Processed_Summary_Screen_Download";
			} else {
