...
@Slf4j
@RestController
public class Itc04SaveToGstnApiJobInsertionController {
	@Autowired
	...
	@PostMapping(value = "/ui/itc04SaveToGstnJob", produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> createGstr6SaveToGstnJob(@RequestBody String jsonParam) {
		try {
			...
			if (gstr1Pairs == null || gstr1Pairs.isEmpty()) {
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("No active GSTNs found for ITC04 save");
				}
			...
			if (gstr1Pairs != null && !gstr1Pairs.isEmpty()) {
				for (int i = 0; i < gstr1Pairs.size(); i++) {
					 ...
					 * pair.getValue1(), groupCode); // Just to make sure the
					 * last SaveToGstn job has ran. if (lastJobStatusCode !=
					 * null && lastJobStatusCode < 30) { JsonObject json = new
					 * JsonObject(); json.addProperty("gstin",
					 * pair.getValue0()); json.addProperty("msg",
					 * "GSTR6 is Inprogress Already."); respBody.add(json);
					 * continue; }
					 */
					...
					if (sections == null || sections.isEmpty()) {
						JsonObject json = new JsonObject();
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg", "No Section is selected to SAVE ITC04");
						respBody.add(json);
						continue;
					}
					...
					SaveToGstnSectionWiseReqDto sectionDto = new SaveToGstnSectionWiseReqDto(pair.getValue0(),
							pair.getValue1(), userRequestId, sections);
					 ...
					 * JobConstants.ITC04_SAVETOGSTN, eachJson.toString(),
					...
					json.addProperty("msg",
							"ITC04 Save for selected(active) GSTINs is initiated. Please check the save status");
					respBody.add(json);
				}
