...
import com.ey.advisory.app.data.erp.vertical.services.DocSeriesJsonToCsvConverterAndUploaderService;
import com.ey.advisory.app.docs.dto.InvDocSeriesHeaderReqDto;
import com.ey.advisory.app.services.docs.InvDocSeriesPayloadService;
...
@RestController
@Slf4j
public class InvDocSeriesController {

	@Autowired
	@Qualifier("InvDocSeriesPayloadServiceImpl")
	private InvDocSeriesPayloadService invDocSeriesPayloadService;

	@Autowired
	@Qualifier("DocSeriesJsonToCsvConverterAndUploaderService")
	private DocSeriesJsonToCsvConverterAndUploaderService convAndUpdService;

	@PostMapping(value = "/api/saveDocSeries", produces = {
	...
	public ResponseEntity<String> saveDocumentsFromERP(ServletRequest request,
		...
		try {
			...
			InvDocSeriesHeaderReqDto document = gsonEwb.fromJson(reqJson,
					InvDocSeriesHeaderReqDto.class);
			String sgstin = document.getSgstin();
			String retPeriod = document.getReturnPeriod();
			invDocSeriesPayloadService.persistPayload(cntrlPayloadId,
			...
			if (fileName == null || fileName.isEmpty()) {
				...
				invDocSeriesPayloadService.updateError(cntrlPayloadId,
			...
			responseEntity = new ResponseEntity<>(resp.toString(),
					HttpStatus.OK);
			invDocSeriesPayloadService.update(cntrlPayloadId, APIConstants.P);
			...
			LOGGER.error("invDocSeries method exception ", ex);
			resp.add("hdr", new Gson()
