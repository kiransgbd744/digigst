...
@Slf4j
@RestController
public class Gstr6SignAndFileStage2Controller {
	@Autowired
	...
	@PostMapping(value = "/signreturn/Gstr6SignAndFileStage2", produces = {
	...
	public ResponseEntity<String> execSignAndFileStage2(String username,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"Received GSTR6 Sign&File Request for ID -{}"
		...
		try {
			...
			if (opEntity.isPresent()) {
				...
				String errMsg = String.format(
						"GSTR6 Sign & File is not initiated for id %s", appkey);
				LOGGER.error(errMsg);
			...
			persistenceMngr.createJob(groupCode, "GSTR6SignAndFile",
			...
			LOGGER.error("Exception while GSTR6 Sign&FIle Stage2 ", ex);
