...
import com.ey.advisory.app.data.services.drc.DRC01BSaveStatus;
...
@RestController
@Slf4j
public class DRC1BSaveStatusController {

	@Autowired
	@Qualifier("DRC01BSaveStatusImpl")
	DRC01BSaveStatus gstr3BSaveStatus;
	
	@Autowired
	@Qualifier("DRC01BSignAndFileServiceImpg")
	 DRC01BSignAndFileService fileStatus;
	
	@PostMapping(value = "/ui/getDrcSaveStatus", produces = {
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"Begin save status in DRC01B with gstin : %s , taxPeriod"
						+ ": %s",gstin, taxPeriod);
				LOGGER.debug(msg);
			}
	...
	@PostMapping(value = "/ui/getDrcFilingStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getFilingDetails(
		...
		try {
			...
			List<Gstr3BFilingDetailsDTO> fileStatusList = fileStatus
					.getDrc01BFilingDetails(gstin, taxPeriod);
