...
@Slf4j
@RestController
public class Itc04SignAndFileStage2Controller {
	...
	@PostMapping(value = "/signreturn/itc04ExecSignAndFileStage2", produces = {
	...
	public ResponseEntity<String> execSignAndFileStage2(String username,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"Received ITC04 Sign&File Request for ID -{}"
		...
		try {
			...
			if (opEntity.isPresent()) {
				...
				String errMsg = String.format(
						"ITC04 Sign & File is not initiated for id %s", appkey);
				LOGGER.error(errMsg);
			...
			persistenceMngr.createJob(groupCode, "ITC04SignAndFile",
			...
			LOGGER.error("Exception while ITC04 Sign&FIle Stage2 ", ex);
