...
@Slf4j
@RestController
public class Gstr1ASignAndFileStage2Controller {
	...
	@PostMapping(value = "/signreturn/execSignAndFileGstr1AStage2", produces = {
	...
	public ResponseEntity<String> execSignAndFileStage2(String username,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"Received GSTR1A Sign&File Request for ID -{}"
		...
		try {
			...
			if (opEntity.isPresent()) {
				...
				String errMsg = String.format(
						"GSTR1A Sign & File is not initiated for id %s", appkey);
				LOGGER.error(errMsg);
			...
			persistenceMngr.createJob(groupCode, "GSTR1ASignAndFile",
					jsonParams.toString(), "SYSTEM", 50L, null, 0L);
			resp = new JsonObject();
			JsonElement respBody = gson
					.toJsonTree("Signature Has been generated Successfully");
			resp.add("hdr", gson.toJsonTree(APIRespDto.createSuccessResp()));
			resp.add("resp", respBody);
			return new ResponseEntity<>("true", HttpStatus.OK);
		} catch (Exception ex) {
			LOGGER.error("Exception while GSTR1A Sign&FIle Stage2 ", ex);
