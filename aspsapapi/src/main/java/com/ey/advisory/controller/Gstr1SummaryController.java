 ...
 */
@RestController
public class Gstr1SummaryController {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr1SummaryController.class);
	...
	@PostMapping(value = "/ui/gstr1SummaryScreen", produces = {
	...
	public ResponseEntity<String> eyGstr1Summary(
		...
		try {
			...
			String lastUpdatedDate = dateFetchDao
					.getLastUpdatedGstr1SummStatus(annexure1SummaryRequest);
			...
			/*
			 * String requestForApprovalflag= requestForApproval.
	...
	@PostMapping(value = "/ui/getGstnSubmitStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	...
	//
	// This API gives Gstn Submit Status
	@PostMapping(value = "/ui/getGstnSaveFileStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getGstnSaveStatsu(
		...
		try {
			...
			GetGstnSubmitStatusDto loadForGetGstnSubmitStatus = dateFetchDao
					.loadForGetGstnSaveStatus(annexure1SummaryRequest);
	...
	// This API gives Gstn Submit Status
	@PostMapping(value = "/ui/getSaveGstnStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getSaveGstnStatsu(
		...
		try {
			...
			/*
			 * JsonElement respBody = gson .toJsonTree(lastUpdatedDate);
			 * 
			 */
	...
	@PostMapping(value = "/ui/gstr1DifferenceSummary", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	...
	public Gstr1FlagRespDto loadHsnFlagection(Annexure1SummaryReqDto request) {
		...
		Optional<GstnUserRequestEntity> previousEntity = Optional.empty();
		...
		if (gstinList != null && !gstinList.isEmpty()) {
		    previousEntity = gstnUserRequestRepo
		        .findTop1ByGstinAndTaxPeriodAndRequestTypeAndReturnTypeAndIsDeleteFalseOrderByIdDesc(
		            gstinList.get(0), taxPeriodReq, SAVE, GSTR1);
		}
