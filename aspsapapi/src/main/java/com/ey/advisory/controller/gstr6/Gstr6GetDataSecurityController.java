...
@RestController
public class Gstr6GetDataSecurityController {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(Gstr6GetDataSecurityController.class);

	@PostMapping(value = { "/ui/getGstr6DataSecForUser" }, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<String> getGstr6DataSecForUser(
			@RequestBody String reqJson) {
		LOGGER.debug(
				"Gstr6GetDataSecurityController getGstr6DataSecForUser begin");
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr6GetDataSecurityController User Name: {}",
						userName);
			}
			...
			getDataSecurity = dataSecurityService
					.getDataPermissionForUser(listEntities, userName, "ISD");
