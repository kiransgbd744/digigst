...
import com.ey.advisory.app.data.erp.vertical.services.DistributionJsonToCsvConverterAndUploaderService;
import com.ey.advisory.app.docs.dto.DistributionHeaderReqDto;
import com.ey.advisory.app.services.docs.DistributionPayloadServiceImpl;
...
@RestController
@Slf4j
public class DistributionApiPushController {

	@Autowired
	@Qualifier("DistributionPayloadServiceImpl")
	private DistributionPayloadServiceImpl distributionPayloadService;

	@Autowired
	@Qualifier("DistributionJsonToCsvConverterAndUploaderService")
	private DistributionJsonToCsvConverterAndUploaderService convAndUpdService;

	@PostMapping(value = "/api/savegstr6Distribution", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> savegstr6Distribution(ServletRequest request,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("requestHeaders are  {}", hdrmap);
		}
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Gstr6 request payload is {}", jsonString);
			}
			...
			DistributionHeaderReqDto document = gsonEwb.fromJson(reqJson,
					DistributionHeaderReqDto.class);
			String sgstin = document.getIsdGstin();
			String retPeriod = document.getReturnPeriod();
			distributionPayloadService.persistPayload(cntrlPayloadId,
			...
			String fileName = convAndUpdService.convertAndUpload(document,cntrlPayloadId);
			if (fileName == null || fileName.isEmpty()) {
				String errCode = "ER8666";
				String msg = "failed to upload the docSeries";
				distributionPayloadService.updateError(cntrlPayloadId,
			...
			responseEntity = new ResponseEntity<>(resp.toString(),
					HttpStatus.OK);
			distributionPayloadService.update(cntrlPayloadId, APIConstants.P);
			...
			LOGGER.error("gstr6Api push method exception ", ex);
			resp.add("hdr", new Gson()
