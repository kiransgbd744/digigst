...
import com.ey.advisory.service.days.revarsal180.Reversal180DaysResponseDwnldServiceImpl;
...
@RestController
@Slf4j
public class Rev180RespPSDTotalErrReportReqController {
	...
	@Autowired
	Reversal180DaysResponseDwnldServiceImpl ser;

	@PostMapping(value = "/ui/rev180RespReportRequest")
	public ResponseEntity<String> rev180ReprtReq(
			@RequestBody String jsonParams) {

		if (LOGGER.isDebugEnabled()) {
			String msg = "Inside Rev180RespPSDTotalErrReportReqController";
			LOGGER.debug(msg);
		}
...

		{
			...
			// ser.generateReports(id);

			asyncJobsService.createJob(TenantContext.getTenantId(),
					JobConstants.REVERSAL_180_DAYS_RESP_REPORT,
			...
			} else if (ReportTypeConstants.REV_RESP180_PROCESSED
					.equalsIgnoreCase(reqDto.getType())) {
				recordType = "Processed Records";

			}

			jobParams.addProperty("reportType", recordType);
			...
			String msg = "Error Occured in Rev180RespPSDTotalErrReportReqController";
	...
	@GetMapping(value = "/ui/rev180RespReportDownload")
	public void fileDownloads(HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		LOGGER.debug("Inside rev180RespReportDownload ");
		...
		if (entity.isPresent()) {
			...
			String fileFolder = GSTConstants.REVERSAL_180_DAYS_RESPONSE_FOLDER;
