 ...
 */
@RestController
public class Gstr6aStatusReportsController {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr6aStatusReportsController.class);
	...
	@RequestMapping(value = "/ui/gstr6ASectionwiseDownloads", method = RequestMethod.POST, produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public void gstr6ASectionwiseDownloads(@RequestBody String jsonString, HttpServletResponse response) {

		Gson gson = GsonUtil.newSAPGsonInstance();
		String groupcode = TenantContext.getTenantId();
		TenantContext.setTenantId(groupcode);
		JsonArray jsonReqArray = (new JsonParser()).parse(jsonString).getAsJsonObject().get("req").getAsJsonArray();
		@SuppressWarnings("serial")
		Type listType = new TypeToken<List<Gstr6SaveStatusDownloadReqDto>>() {
		}.getType();

		List<Gstr6SaveStatusDownloadReqDto> reqDtos = gson.fromJson(jsonReqArray, listType);
		...
		try {
			...
			workbook = gstr6SaveStatusDownloadV2ReportHandler.downloadGstr6aSaveSectionsReport(reqDtos);

			fileName = "GSTR-6A_DownloadReport";
			if (workbook == null) {
