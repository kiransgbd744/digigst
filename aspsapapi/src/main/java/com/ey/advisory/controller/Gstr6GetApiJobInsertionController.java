...
@RestController
@Slf4j
public class Gstr6GetApiJobInsertionController {
	...
	public ResponseEntity<String> createGstr6GstnGetJob(
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"Gstr6GstnGetSection Request received from UI as {} ",
						request);
			}
			...
			List<Gstr6GetInvoicesReqDto> dtos = gson.fromJson(asJsonArray,
					listType);
			return createGstr6Gstn(dtos);
		} catch (Exception ex) {
			String msg = "Unexpected error while creating async jobs for GSTR6 Get ";
			LOGGER.error(msg, ex);
	...
	public ResponseEntity<String> createGstr6Gstn(
			List<Gstr6GetInvoicesReqDto> dtos) {
		...
		try {
			...
			for (Gstr6GetInvoicesReqDto dto : dtos) {
				...
				if (isFailedGet == true) {
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(
								"GSTR6 Failed GET eligible sections are {} ",
								failedBatchs);
					}
				} else {
					if (LOGGER.isDebugEnabled()) {
					...
					if (LOGGER.isDebugEnabled()) {
						LOGGER.debug(
								"GSTR6 All GET Not eligible sections are {} ",
								initAndInProgressStatus);
					}
				}
				...
				Long userRequestId = gstnUserRequestUtil.createGstnUserRequest(
						...
						APIConstants.GSTR6.toUpperCase(), groupCode, userName,
						false, false, false);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					job = asyncJobsService.createJob(groupCode,
							JobConstants.GSTR6_GSTN_GET_SECTION, jsonParam,
					...
					APIParams params = new APIParams(groupCode,
							APIProviderEnum.GSTN, APIIdentifiers.GSTR6_GET_B2B,
							param1, param2);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					APIParams params = new APIParams(groupCode,
							APIProviderEnum.GSTN, APIIdentifiers.GSTR6_GET_B2BA,
							param1, param2);
					JsonElement apiParams = gson.toJsonTree(params);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					APIParams params = new APIParams(groupCode,
							APIProviderEnum.GSTN, APIIdentifiers.GSTR6_GET_CDN,
							param1, param2);
					JsonElement apiParams = gson.toJsonTree(params);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					APIParams params = new APIParams(groupCode,
							APIProviderEnum.GSTN, APIIdentifiers.GSTR6_GET_CDNA,
							param1, param2);
					JsonElement apiParams = gson.toJsonTree(params);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					APIParams params = new APIParams(groupCode,
							APIProviderEnum.GSTN, APIIdentifiers.GSTR6_GET_ISD,
							param1, param2);
					JsonElement apiParams = gson.toJsonTree(params);
				...
				if (isEligible(isFailedGet, failedBatchs,
					...
					APIParams params = new APIParams(groupCode,
							APIProviderEnum.GSTN, APIIdentifiers.GSTR6_GET_ISDA,
							param1, param2);
					JsonElement apiParams = gson.toJsonTree(params);
				...
				/*
				 * if (isEligible(isFailedGet, failedBatchs,
				 * initAndInprogressBatchs, APIConstants.ITC, gstr6Sections)) {
				 * dto.setType(APIConstants.ITC.toUpperCase()); dto =
				 * createBatchAndSave(groupCode, dto); jsonParam =
				 * gson.toJson(dto); job = asyncJobsService.createJob(groupCode,
				 * JobConstants.GSTR6_GSTN_GET_SECTION, jsonParam, userName,
				...
				if (isEligible(isFailedGet, failedBatchs,
						initAndInprogressBatchs, APIConstants.LATEFEE,
						gstr6Sections)) {
				...
				json.addProperty("msg",
						"Get GSTR6  Request Initiated Successfully");
				respBody.add(json);
	...
	private Gstr6GetInvoicesReqDto createBatchAndSave(String groupCode,
			Gstr6GetInvoicesReqDto dto) {
