...
import com.ey.advisory.app.services.reports.Gstr1GetEInvoicesProcessedReportHandler;
 ...
 */
@RestController
public class Gstr1GetEInvoicesReportsController {

	private static final Logger LOGGER = LoggerFactory.getLogger(Gstr1GetEInvoicesReportsController.class);

	@Autowired
	@Qualifier("Gstr1GetEInvoicesProcessedReportHandler")
	private Gstr1GetEInvoicesProcessedReportHandler gstr1GetEInvoicesProcessedReportHandler;

	@Autowired
	@Qualifier("BasicCommonSecParamRSReports")
	BasicCommonSecParamRSReports basicCommonSecParamRSReports;

	@RequestMapping(value = "/ui/downloadGstr1InvoicesReport", method = RequestMethod.POST, produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public void downloadGstr1InvoicesReport(@RequestBody String jsonString, HttpServletResponse response) {
		...
		Gstr1EInvReportsReqDto reqDtoCriterias = gson.fromJson(json, Gstr1EInvReportsReqDto.class);

		try {
			String fileName = null;
			Workbook workbook = null;

			workbook = gstr1GetEInvoicesProcessedReportHandler.downloadProcessedReport(reqDtoCriterias);
			...
			fileName = "GSTR1_E_Invoice_Records" + "_" + date + "" + time;
			...
			String msg = "Unexpected error while retrieving " + "Data from Report ";
			LOGGER.error(msg, ex);
			JsonObject resp = new JsonObject();
			resp.add("hdr", new Gson().toJsonTree(new APIRespDto("E", msg)));
		}
