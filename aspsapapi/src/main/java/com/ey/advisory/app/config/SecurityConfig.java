	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.sessionManagement(
				...
				.authorizeHttpRequests(authorizeRequests -> authorizeRequests.requestMatchers("/api/**").permitAll()
						.requestMatchers("/ui/**").permitAll().requestMatchers("/actuator/**").permitAll())
				.csrf(csrf -> csrf.ignoringRequestMatchers("/api/**", "/ui/**","/actuator/**"));

		http.addFilterBefore(new APIRequestGroupResolverFilter(), UsernamePasswordAuthenticationFilter.class);
