...
public class APIRequestGroupResolverFilter implements Filter {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(APIRequestGroupResolverFilter.class);
	...
	public void doFilter(ServletRequest servletrequest, ServletResponse servletresponse, FilterChain filterChain)
		...
		if (action.contains("generateAccessToken")) {
			LOGGER.debug("Action url is 'generateAccessToken' or UI Request," + " SO skipping the filter");
			filterChain.doFilter(request, response);
			return;
		}
		
		if (action.contains("/actuator")) {
		...
		if (request.getRequestURI().contains("/api")) {
			...
			if (jwtToken != null && jwtToken.length() != 0) {
				LOGGER.debug("Token received while intercepting API Request is - {}", jwtToken);
				try {
					String uType = extractAttributeFromJWT(jwtToken, "utype");

					if (Strings.isNullOrEmpty(uType)
							|| !uType.equalsIgnoreCase("API")) {
					...
					LOGGER.error("Exception while extracting the Nested token", e);
					jsonResp.add("hdr", new Gson().toJsonTree(new APIRespDto("E", null)));
					jsonResp.addProperty("errMsg", "Exception while Authenticating API Auth Info");
					response.getWriter().println(jsonResp.toString());
					response.setStatus(HttpStatus.UNAUTHORIZED.value());
				}

			} else {
				LOGGER.error("Request doesn't contain while Authenticating API Auth Info ");
				jsonResp.add("hdr", new Gson().toJsonTree(new APIRespDto("E", null)));
				jsonResp.addProperty("errMsg", "Request doesn't contain while Authenticating API Auth Info");
				response.getWriter().println(jsonResp.toString());
				response.setStatus(HttpStatus.UNAUTHORIZED.value());
			}
	...
	private String extractJwtToken(HttpServletRequest request) {
		...
		LOGGER.debug("Debug Extracting request , authorizationHeader is {}", authorizationHeader);
		LOGGER.error("Error Extracting request , authorizationHeader is {}", authorizationHeader);
	...
	private static String extractAttributeFromJWT(String jwtToken, String attName) throws Exception {
