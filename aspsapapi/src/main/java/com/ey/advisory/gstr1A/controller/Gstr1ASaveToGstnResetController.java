...
@Slf4j
@RestController
public class Gstr1ASaveToGstnResetController {
	...
	@PostMapping(value = "/ui/gstr1ASaveToGstnResetAndSaveJob", produces = {
			...
			if (gstr1Pairs != null && !gstr1Pairs.isEmpty()) {
				...
				for (int i = 0; i < gstr1Pairs.size(); i++) {
					...
					if (!gstnUserRequestUtil.isNextSaveRequestEligible(
							...
							APIConstants.SAVE, APIConstants.GSTR1A.toUpperCase(),
							groupCode)) {

						JsonObject json = new JsonObject();
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"GSTR1A SAVE is Inprogress, New request Cannot be taken.");
						respBody.add(json);
						continue;
					...
					resetHandler.createSectionWiseJobsForGstr1A(pair, dto, groupCode);

					if (!isSuccessMsgAddedInResp) {
						JsonObject json = new JsonObject();
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"GSTR1A Save for selected(active) GSTINs is initiated. Please check the save status");
						respBody.add(json);
	...
	@PostMapping(value = "/ui/gstr1ASaveToGstnReset", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> resetAspData(@RequestBody String jsonParam) {
		...
		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"gstr1ASaveToGstnReset Request received from UI as {} ",
						jsonParam);
			}
			...
			List<Pair<String, String>> gstr1Pairs = screenExtractor
					.getGstr1ACombinationPairs(dto, groupCode);
			...
			if (gstr1Pairs != null && !gstr1Pairs.isEmpty()) {
				...
				for (int i = 0; i < gstr1Pairs.size(); i++) {
					...
					if (sections == null || sections.isEmpty()) {
						...
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"No Section is selected to GSTR1A section reset at DigiGST");
						respBody.add(json);
						continue;
					}
					// reset the asp data
					resetHandler.resetGstr1AAspTableData(pair, dto,
							APIConstants.ASP);
					if (!isSuccessMsgAddedInResp) {
						JsonObject json = new JsonObject();
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"GSTR1A section reset at DigiGST is successful");
						respBody.add(json);
	...
	// done
	@PostMapping(value = "/ui/gstr1AAspResetNonRespondedDataOnly", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> resetAspNonrespondedDataOnly(
			@RequestBody String jsonParam) {

		try {
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"gstr1AAspResetNonRespondedDataOnly Request received from UI as {} ",
						jsonParam);
			}
			Gson gson = GsonUtil.newSAPGsonInstance();
			JsonObject resp = new JsonObject();
			...
			if (gstr1Pairs != null && !gstr1Pairs.isEmpty()) {
				...
				for (int i = 0; i < gstr1Pairs.size(); i++) {
					...
					if (sections == null || sections.isEmpty()) {
						...
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"No Section is selected to GSTR1A section No Response reset at DigiGST");
						respBody.add(json);
						continue;
					}
					// reset the non responded asp data
					resetHandler.resetGstr1ANonRespondedAspTableData(pair, dto,
							APIConstants.ASP);
					if (!isSuccessMsgAddedInResp) {
						JsonObject json = new JsonObject();
						/* json.addProperty("gstin", pair.getValue0()); */
						json.addProperty("msg",
								...
								"GSTR1A section No Response reset at DigiGST is successful");
						respBody.add(json);
	...
	// Delete AutoDrafted GSTN Data(Delete GSTN data based on deletion file)
	// functionality
	@PostMapping(value = "/ui/gstr1ADeleteGstnDataJob", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	...
	// DONE
	@PostMapping(value = "/ui/gstr1AGstnResetJob", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> gstnReset(@RequestBody String jsonParam) {
		...
		try {
			...
			if (gstr1Pairs == null || gstr1Pairs.isEmpty()) {
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("No active GSTNs found for GSTR1A Delete");
				}
				resp.add("hdr", gson.toJsonTree(APIRespDto.creatErrorResp()));
				resp.add("resp", gson.toJsonTree(respBody));
			...
			if (gstr1Pairs != null && !gstr1Pairs.isEmpty()) {
				...
				for (int i = 0; i < gstr1Pairs.size(); i++) {
					...
					if (!gstnUserRequestUtil.isNextSaveRequestEligible(
						...
						json.addProperty("gstin", pair.getValue0());
						json.addProperty("msg",
								"GSTR1A Delete (Full Data) is Inprogress, New request Cannot be taken.");
						respBody.add(json);
						continue;

					}
					// create new async job for each section
					resetHandler.createResetGstr1AGstnJob(pair, dto, groupCode);
					...
					if (!isSuccessMsgAddedInResp) {
						...
						json.addProperty("msg",
								...
								"GSTR1A Delete (Full Data) for selected(active) GSTINs is initiated. Please check the save status");
						respBody.add(json);
	...
	private JsonArray getAllActiveGstnList(
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Getting all the active GSTNs for GSTR1A save");
		}
