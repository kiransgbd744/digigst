...
@Slf4j
@RestController
public class Gstr1ASaveResetSectionSelectionController {

	@Autowired
	@Qualifier("batchSaveStatusRepository")
	private Gstr1BatchRepository batchSaveStatusRepo;

	@PostMapping(value = "/ui/gstr1ASaveToGstnSectionSelection", produces = {
	...
	public ResponseEntity<String> gstr1SaveSectionSelection(
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(
					"gstr1ASaveSectionSelection Request received from UI as {} ",
					request);
		}
		...
		try {
			...
			String msg = "Unexpected error while fetching  Gstr1A Save Reset sections";
