...
import com.ey.advisory.app.services.daos.get2a.AnxGet2aprocessedService;
...
@RestController
public class Anx2GetProcessedDataController {
...
@Autowired
@Qualifier("AnxGet2aprocessedServiceImpl")
private AnxGet2aprocessedService anxGet2aprocessedService;

private static final Logger LOGGER = LoggerFactory
		.getLogger(Anx2GetProcessedDataController.class);

@RequestMapping(value = "/ui/getGet2aProcessedData", method = RequestMethod.POST, produces = {
		MediaType.APPLICATION_JSON_VALUE })
public ResponseEntity<String> getGet2aProcessedData(
	...
	try {
		Anx2GetProcessedRequestDto criteria = gson.fromJson(json,
				Anx2GetProcessedRequestDto.class);

		/*List<String> gstins = criteria.getGstins();
		List<Long> entityIds = criteria.getEntityId();
		if (gstins == null || gstins.size() == 0) {
			gstins = anxGet2adatasecurityService.getSecurityGstins(entityIds);
		...
		*/
		...
		List<Anx2GetProcessedResponseDto> recResponse = anxGet2aprocessedService
				.getAnx2Get2aProcessedRecords(criteria);
