...
@RestController
@Slf4j
public class Gstr2Avs3bReportsController {

	
	@Autowired
	@Qualifier("Gstr2Avs3bReviewSummaryReportHandler")
	private Gstr2Avs3bReviewSummaryReportHandler gstr1vs3bReviewSummaryReportHandler;
	...
	@RequestMapping(value = "/ui/gstr2Avs3bReviewSummaryDownload", method = RequestMethod.POST, produces = {
	...
	public void downloadreviewSummReport(@RequestBody String jsonString,
		...
		try {
			...
			if(criteria.getType().equalsIgnoreCase("P")){
				fileName = "EntityLevelSummary_GSTR2AvsGSTR3B_"+criteria.getTaxPeriodFrom()
				+"_"+criteria.getTaxPeriodTo() +"_"+ date + "" + time;
			}else{
				fileName = req.getGstin().get(0)+"_"+"GSTR2AvsGSTR3B_"+criteria.getTaxPeriodFrom()
				+"_"+criteria.getTaxPeriodTo() +"_"+ date + "" + time;
			}
			if (workbook == null) {
