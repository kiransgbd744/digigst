...
@RestController
@Slf4j
public class ReportFileStatusDownloadController {

	@Autowired
	@Qualifier("ReportFileStatusDownloadServiceImpl")
	private ReportFileStatusDownloadService service;

	@RequestMapping(value = "/ui/reportFileDownloadIdWise", method = RequestMethod.POST, produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getdownloadDataforRecon(
			@RequestBody String jsonString) {

		LOGGER.debug("Inside ReportFileStatusDownloadController"
		...
		try {
			...
			List<ReportDownloadDto> respList = service.getDownloadData(reqDto.getConfigId());
