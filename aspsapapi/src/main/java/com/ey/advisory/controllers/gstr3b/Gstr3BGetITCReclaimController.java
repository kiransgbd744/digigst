...
import com.ey.advisory.services.gstr3b.itc.reclaim.Gstr3BGetITCReclaimService;
...
@RestController
@Slf4j
public class Gstr3BGetITCReclaimController {

	@Autowired
	@Qualifier("Gstr3BGetITCReclaimServiceImpl")
	private Gstr3BGetITCReclaimService service;

	@PostMapping(value = "/ui/getGstr3BITCReclaim", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getITCReclaimData(
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("INSIDE Gstr3BGetITCReclaimController");
			}
			...
			String status = service.get3BReclaimAmount(gstin, taxPeriod);
