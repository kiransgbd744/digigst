...
import com.ey.advisory.app.services.reports.AspProcessVsSubmitReportHandler;
...
@RestController
@Slf4j
public class AspVsSubmittedReportsController {
	@Autowired
	@Qualifier("AspProcessVsSubmitReportHandler")
	private AspProcessVsSubmitReportHandler aspProcessVsSubmitReportHandler;

	@Autowired
	@Qualifier("BasicCommonSecParamRSReports")
	BasicCommonSecParamRSReports basicCommonSecParamRSReports;

	@RequestMapping(value = "/ui/aspProcessedVsSubmitDownloadsreport", method = RequestMethod.POST, produces = {
	...
	public void aspProcessedVsSubmitDownloadsreport(
		...
		try {
			...
			if (criteria.getType() != null
				...
				fileName = "Processed DigiGST vs Submitted GSTN_" + convertNow;
			}
			if (workbook == null) {
