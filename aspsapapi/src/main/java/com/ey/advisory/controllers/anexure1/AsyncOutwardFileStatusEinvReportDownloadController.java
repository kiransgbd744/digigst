...
@RestController
@Slf4j
public class AsyncOutwardFileStatusEinvReportDownloadController {
	...
	AsyncReportHandler asyncFileStatusEinvReportHandlerImpl;

	@PostMapping(value = "/ui/downloadOutwardFileStatusEinvCsvReports")
	public ResponseEntity<String> downloadOutwardFileStatusEinvCsvReports(
		...
		try {
			...
			asyncFileStatusEinvReportHandlerImpl.setDataToEntity(entity,
					reqDto);
			...
			if ("OUTWARD".equalsIgnoreCase(entity.getDataType())
				...
				asyncJobsService.createJob(groupCode,
						JobConstants.OUTWARD_FILE_ST_EINV_DOWN_PROCESSOR,
	...
	private String getReportType(String type, String status) {
		...
		try {
			if (ReportTypeConstants.EINV_NOT_APPLICABLE
					.equalsIgnoreCase(type)) {

				reportType = ReportTypeConstants.EINV_NOT_APPLICABLE;
				if (!Strings.isNullOrEmpty(status)) {
					reportType += status;
				}
			} else if (ReportTypeConstants.EINV_APPLICABLE
					.equalsIgnoreCase(type)) {
				reportType = ReportTypeConstants.EINV_APPLICABLE;
				if (!Strings.isNullOrEmpty(status)) {
					reportType += status;
				}
			} else if (ReportTypeConstants.EINV_IRN_GENERATE
					.equalsIgnoreCase(type)) {

				reportType = ReportTypeConstants.EINV_IRN_GENERATE;
				if (!Strings.isNullOrEmpty(status)) {
					reportType += status;
				}
			} else if (ReportTypeConstants.EINV_IRN_CANCELED
					.equalsIgnoreCase(type)) {

				reportType = ReportTypeConstants.EINV_IRN_CANCELED;
				if (!Strings.isNullOrEmpty(status)) {
					reportType += status;
				}
			} else if (ReportTypeConstants.EINV_ERROR.equalsIgnoreCase(type)) {

				reportType = ReportTypeConstants.EINV_ERROR;
				if (!Strings.isNullOrEmpty(status)) {
					reportType += status;
				}
			} else if (ReportTypeConstants.EINV_ERROR_FROM_IRP
					.equalsIgnoreCase(type)) {

				reportType = ReportTypeConstants.EINV_ERROR_FROM_IRP;
				if (!Strings.isNullOrEmpty(status)) {
					reportType += status;
				}
			} else {
	...
	@GetMapping(value = "/ui/OutwardFileStatusEinvDownloadDocument")
