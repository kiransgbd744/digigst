...
import com.ey.advisory.app.gstr3b.Gstr3BGstinDetailedSummaryService;
...
@Slf4j
@RestController
public class Gstr3BGstinDashboardSaveChangesController {

	@Autowired
	@Qualifier("Gstr3BGstinDetailedSummaryServiceImpl")
	private Gstr3BGstinDetailedSummaryService service;
	
	@Autowired
	@Qualifier("Table4d1ValidationtSaveUpdateServiceImpl")
	private Table4d1ValidationtSaveUpdateServiceImpl validationService;

	@PostMapping(value = "/ui/saveGstinDashboardUserInput", produces = {
	...
	public ResponseEntity<String> saveGstinDashboardUserInput(
		...
		try {
			...
			service.saveGstinDashboardUserInputs(gstin, taxPeriod,
					userInputList, status);
			
			validationService.update4D1ValidationStatus(gstin, taxPeriod);
			
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(
						"INSIDE Gstr3BGstinDashboardSaveChangesController update4D1ValidationStatus() method called");
			}
	...
	@PostMapping(value = "/ui/deleteGstinDashboardUserInput", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> deleteGstinDashboardUserInput(
			@RequestBody String jsonString) {
		...
		try {
			...
			service.deleteGstinDashboardUserInputs(gstin, taxPeriod,
					userInputList, status);
