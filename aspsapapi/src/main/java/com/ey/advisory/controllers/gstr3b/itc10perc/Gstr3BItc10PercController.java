...
import com.ey.advisory.app.gstr3b.itc10perc.service.Gstr3BItc10PercService;
...
@RestController
@Slf4j
public class Gstr3BItc10PercController {
	
	@Autowired
	private Gstr3BItc10PercService service;

	@PostMapping(value = "/ui/getItc10Perc", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getItc10PercData(
		...
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("INSIDE Gstr3BItc10PercController");
			}
			...
			Long entityId = requestObject.has("entityId")
					? requestObject.get("entityId").getAsLong() : 0;
			
			List<Gstr3BItc10PercDto> respList = service.getItcData(gstin, 
					taxPeriod,entityId);
