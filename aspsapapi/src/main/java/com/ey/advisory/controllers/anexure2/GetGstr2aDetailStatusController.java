...
import com.ey.advisory.app.services.daos.get2a.GetGstr2aSuccessStatusService;
...
@RestController
public class GetGstr2aDetailStatusController {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(GetGstr2aDetailStatusController.class);
	...
	@Autowired
	@Qualifier("GetGstr2aSuccessStatusService")
	private GetGstr2aSuccessStatusService getGstr2aSuccessStatusService;

	@PostMapping(value = "/ui/getGstr2aDetailStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getGstr2aDetailStatus(
		...
		try {
			...
			List<GetGstr2aDetailStatusRespDto> summaryList = getGstr2aDetailStatusService
	...
	@PostMapping(value = "/ui/getGstr2aSuccessStatus", produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<String> getGstr2aSuccessStatus(
		...
		try {
			...
			GetGstr6DetailStatusMainDetailsRespDto summaryList = getGstr2aSuccessStatusService
					.findByCriteria(criteria);
			JsonObject resps = new JsonObject();
			JsonElement respBody = gson.toJsonTree(summaryList);
			resps.add("hdr", gson.toJsonTree(APIRespDto.createSuccessResp()));
			...
			String msg = "Unexpected error while retrieving Detail Status ";
			LOGGER.error(msg, ex);
			JsonObject resp = new JsonObject();
			resp.add("hdr", new Gson().toJsonTree(new APIRespDto("E", msg)));
			return new ResponseEntity<>(resp.toString(),
