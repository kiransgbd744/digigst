...
public class RunnableTaskProcessor implements Runnable {
	...
	public void run() {
		try {
			...
			Pair<String, String> resourcePair = jobUrlResolver
					.resolveJobUrl(message);
			if (LOGGER.isDebugEnabled()) {
				String msg = String.format(
						"About to post the Job %s to asyncjobs url %s",
						message.getMessageType(), resourcePair.getValue0());
				LOGGER.debug(msg);
			}
			updateJobBundle(message, resourcePair.getValue1());
			HttpPost httpPost = new HttpPost(resourcePair.getValue0());
			...
			if (statusCode > 400 && statusCode < 600) {
				String msg = String.format(
						...
						statusCode);
				updatePeriodicJobDetails(message);
				LOGGER.error(msg);
			}
	...
	private RequestConfig getRequestConfig() {
		...
		Map<String, Config> configMap = configManager.getConfigs("EYInternal",
				"timeout");
		int connectionTimeout = configMap
				.containsKey("timeout.internal.conn_timeout")
						? Integer.valueOf(
								configMap.get("timeout.internal.conn_timeout")
										.getValue())
						: 5000;
		int readTimeout = configMap.containsKey("timeout.internal.read_timeout")
				? Integer.valueOf(configMap.get("timeout.internal.read_timeout")
						.getValue())
				: 5000;
		int connectionManagerTimeout = configMap
				.containsKey("timeout.internal.conn_mngr_timeout")
						? Integer.valueOf(configMap
								.get("timeout.internal.conn_mngr_timeout")
								.getValue())
						: 5000;
	...
	private void updateJobBundle(Message msg, String jobBundle) {
		AsyncExecJobRepository repo = StaticContextHolder.getBean(
				"asyncExecJobRepository", AsyncExecJobRepository.class);
		Long jobId = msg.getId();
		int cnt = repo.updateJobBundle(jobId, jobBundle);
		if (cnt == 0 && LOGGER.isDebugEnabled()) {
			LOGGER.debug("The Job Bundle is updated for the job Id {}", jobId);
		}
	}

	private void updatePeriodicJobDetails(Message msg) {
		  AsyncExecJobRepository repo = StaticContextHolder.getBean(
		            "asyncExecJobRepository", AsyncExecJobRepository.class);
		    AsyncExecJob asyncJob = repo.findByJobId(msg.getId());
		    if (!asyncJob.isPeriodic()) {
		        return;
		    }
		    ...
		    try {
		        Optional<PeriodicExecJob> optionalPeriodicJob = periodicJobRepo
		                .findById(asyncJob.getPeriodicJobId());

		        if (optionalPeriodicJob.isPresent()) {
		            PeriodicExecJob periodicJob = optionalPeriodicJob.get();
		            periodicJob.setJobstartDate(asyncJob.getJobStartDate());
		            periodicJob.setJobCompletionDate(new Date());
		            periodicJob.setUpdatedDate(new Date());
		            periodicJobRepo.save(periodicJob);
		        } else {
		            LOGGER.warn("No PeriodicExecJob found with ID: " + asyncJob.getPeriodicJobId());
		        }
		    } catch (Exception ex) {
		        LOGGER.error("Exception while marking Periodic JOB", ex);
		    }
		...
		PeriodicExecJobRepository periodicJobRepo = StaticContextHolder
				.getBean(PeriodicExecJobRepository.class);
		try {
			PeriodicExecJob periodicJob = periodicJobRepo
					.findById(asyncJob.getPeriodicJobId()).get();
			periodicJob.setJobstartDate(asyncJob.getJobStartDate());
			periodicJob.setJobCompletionDate(new Date());
			periodicJob.setUpdatedDate(new Date());
			periodicJobRepo.save(periodicJob);
